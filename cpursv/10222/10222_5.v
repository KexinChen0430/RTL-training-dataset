...
module FSM(clk,reset,rom_addr,rom_q,ram_a_addr,ram_b_addr,ram_b_w,pe,
           done);
  ...
  always @(posedge clk)
      if (reset) state <= START;
        ...
        case (state)
          ...
          READ_SRC2: if (times == 0) state <= DON;
              else state <= CALC;
  ...
  always @(posedge clk)
      if (reset) rom_addr <= 0;
        ...
        begin
          if (loop1[0] && (rom_addr == LOOP1_END)) rom_addr <= LOOP1_START;
          else if (loop2[0] && (rom_addr == LOOP2_END)) rom_addr <= LOOP2_START;
          else rom_addr <= 1'd1+rom_addr;
        end
        
  
  always @(posedge clk)
      if (reset) loop1 <= ~0;
      else if ((rom_addr == LOOP1_END) && (state == WAIT)) loop1 <= loop1>>1;
        
  
  always @(posedge clk)
      if (reset) loop2 <= ~0;
      else if ((rom_addr == LOOP2_END) && (state == WAIT)) loop2 <= loop2>>1;
        
  
  always @(posedge clk)
      if (reset) count <= 0;
      else if (state == READ_SRC1) count <= times;
      else if (state == CALC) count <= count+(0-1'd1);
  ...
  always @(state or op or src2 or dest)
      case (state)
