...
module mux_sram  #(parameter  DATA_WIDTH = 32, ADDR_WIDTH = 20)
  (input  boot_mode,
   ...
   input  [(-1)+DATA_WIDTH:0] boot_mem_rd_data,
   input  inst_mem_wr_en,
   input  inst_mem_rd_en,
   input  [(-1)+DATA_WIDTH:0] inst_mem_wr_data,
   output reg [(-1)+DATA_WIDTH:0] inst_mem_rd_data,
   input  [ADDR_WIDTH-1:0] inst_mem_addr,
   output reg sram_mem_wr_en,
   output reg sram_mem_rd_en,
   output reg [(-1)+DATA_WIDTH:0] sram_mem_wr_data,
   input  [(-1)+DATA_WIDTH:0] sram_mem_rd_data,
