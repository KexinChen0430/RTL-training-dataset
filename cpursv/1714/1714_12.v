  ...
  always @(*)
      begin
        case (foo[1<<1:0])
          ...
          default: code[9:1<<1] <= (b+a)+1'b1;

        endcase

      end
  assign code1[1:0] = (a[6:5] & (((~a[6:5] | ~{a[4],b[6]}) | a[6:5]) | (~a[6:5] & (({a[4],b[6]} & ((~a[6:5] | ~{a[4],b[6]}) | a[6:5])) | a[6:5])))) ^ ({a[4],b[6]} & (((~a[6:5] | ~{a[4],b[6]}) | a[6:5]) | (~a[6:5] & (({a[4],b[6]} & ((~a[6:5] | ~{a[4],b[6]}) | a[6:5])) | a[6:5]))));
  assign foo = {1+1<<1{1'b0}};
  assign egg = {78{1'b0}};
  assign baz = {(-((bus_width*3)-1))+((bus_width*4)+1){1'b1}};
