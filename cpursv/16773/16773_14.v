...
module buffer_int  #(parameter  BUF_NUM = 0, BUF_SIZE = 9)
  (input  clk,
   ...
   output en_o,
   output we_o,
   output reg [(0-1)+BUF_SIZE:0] addr_o,
  ...
  always @(posedge clk)
      if (rst) ctrl_reg <= 0;
      else if ((ctrl_word[31:28] == BUF_NUM) & go) ctrl_reg <= ctrl_word;
        
  wire [(0-1)+BUF_SIZE:0]  firstline = ctrl_reg[(0-1)+BUF_SIZE:0];

  wire [(0-1)+BUF_SIZE:0]  lastline = ctrl_reg[(0-1)+BUF_SIZE<<1:BUF_SIZE];
          ...
          WRITING: begin
                if (wr_ready_i) 
                  begin
                    ...
                    else if ((addr_o == lastline) || wr_eop) state <= DONE;
  ...
  assign wr_occ = wr_flags_i[3:1+1];
  assign wr_error = wr_eop & wr_sop;
