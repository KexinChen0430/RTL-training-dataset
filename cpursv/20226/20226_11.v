...
module arbiter(output wire [31:0] wb_m0_vcache_dat_o,
      ...
      if (wb_rst) cellram_mst_sel <= 0;
        ...
        begin
          if (cellram_mst_sel == 2'b00) 
            begin
              if (wb_m0_vcache_cyc_i & wb_m0_vcache_stb_i) cellram_mst_sel[1] <= 1;
  ...
  always @(posedge wb_clk or posedge wb_rst)
      ...
      else if (|cellram_rst_counter) cellram_rst_counter <= cellram_rst_counter+(0-1);
...
(cellram_mst_sel[1] ? wb_m0_vcache_stb_i : 0) & !|cellram_rst_counter;
  ...
  assign wb_m1_cpu_err_o = cellram_mst_sel[0] & cellram_arb_reset;
  assign wb_m1_cpu_rty_o = 0;
  assign wb_m0_vcache_err_o = cellram_mst_sel[1] & cellram_arb_reset;
  assign wb_m0_vcache_rty_o = 0;
  
  always @(posedge wb_clk)
      if (wb_rst) cellram_arb_timeout <= 0;
      else if (wb_s0_cellram_wb_ack_i) cellram_arb_timeout <= 0;
      else if (wb_s0_cellram_wb_stb_o & wb_s0_cellram_wb_cyc_o) cellram_arb_timeout <= 1+cellram_arb_timeout;
