...
module pixelq_op_INPUT_STREAM_reg_slice  #(parameter  N = 8)
  ...
  assign load_p1 = 
((state == ONE) && s_valid && (m_ready && s_valid)) || 
                   (((state == TWO) || ((state == ONE) && s_valid)) && 
((state == TWO) || ((state == TWO) && m_ready) || m_ready) && m_ready) || 
                   (
(((state == ZERO) && s_valid) || (m_ready && s_valid)) && 
(((state == ONE) || (state == ZERO)) && 
((m_ready && s_valid) || (state == ZERO))));
