...
module wb_ram  #(parameter 
  ...
  (input  wire clk,
   input  wire [(-1)+ADDR_WIDTH:0] adr_i,
  ...
  reg   ack_o_reg = 1'b0;
  reg  [DATA_WIDTH+(-1):0] mem[(-1)+((1+1)**VALID_ADDR_WIDTH):0];
  wire [(-1)+VALID_ADDR_WIDTH:0]  adr_i_valid = adr_i>>ADDR_WIDTH-VALID_ADDR_WIDTH;
  ...
  begin
    for (i = 0; i < ((1+1)**VALID_ADDR_WIDTH); i = i+((1+1)**((1/(1+1))*VALID_ADDR_WIDTH)))
        begin
          for (j = i; j < (i+((1+1)**((1/(1+1))*VALID_ADDR_WIDTH))); j = j+1)
              begin
                mem[j] = 0;
              end
        end
