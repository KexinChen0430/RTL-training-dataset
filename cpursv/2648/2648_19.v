...
module main(CLK,HSYNC,VSYNC,R,G,B,XCLK,PCLK,D,HIN,VIN);
  ...
  always @(posedge PCLK)
      begin
        if (VIN) PVCNT <= 7'd0;
          ...
          begin
            if (PXCNT != 8'h0) PVCNT <= 7'd1+PVCNT;
  ...
  always @(negedge CLK)
      begin
        ...
        else if (HCNT == 16'd1302) 
          begin
            HSYNC <= 1'b0;
            VCNT <= 16'h1+VCNT;
          end
        else if (HCNT == 16'd1493) 
          begin
            HSYNC <= 1'b1;
          end
        else if (HCNT == 16'd1270) 
          begin
            drawH <= 1'b0;
          end
        ...
        else if (VCNT == 16'd515) 
          begin
            VSYNC <= 1'b1;
          end
        else if (VCNT == 16'd524) 
          begin
            drawV <= 1'b1;
            VCNT <= 16'h0;
          end
        else if (VCNT == 16'd480) 
          begin
            drawV <= 1'b0;
          end
          
        if ((PXCNT[15:9] == 0) && !BCNT && 
            (!VIN && (PVCNT[15:8] == 0) && HIN)) 
          begin
