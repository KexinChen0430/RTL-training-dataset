...
module qsys_serial_host(output reg sdo,
                        input  sdi,
                        input  clk,
                        input  sle,
                        output reg srdy,
  ...
  parameter  bus_ready = 8'd1+initial_state;

  parameter  bus_transmit_start = 8'd1+bus_ready;

  parameter  bus_transmit_ready = 8'd1+bus_transmit_start;

  parameter  bus_address_ready = 8'd1+bus_transmit_ready;

  parameter  bus_data_wait = 8'd1+bus_address_ready;

  parameter  bus_data_ready = 8'd1+bus_data_wait;

  parameter  bus_transmit_back = 8'd1+bus_data_ready;
  ...
  always @(state or sle or avm_M1_waitrequest)
      begin
        case (state)
          ...
          bus_transmit_back: nextstate <= 1+state;
  ...
  always @(posedge clk)
      begin
        if ((state >= bus_transmit_back) && 
            (state < bus_transmit_finish)) 
          begin
            integer i;
          ...
          begin
            case (state)
              ...
              bus_transmit_ready: avm_M1_address <= data_buffer[63:32];

              bus_address_ready: begin
                    if (data_buffer[64] == 1'b0) 
                      begin
                        avm_M1_read <= 1'b1;
                        avm_M1_write <= 1'b0;
                      end
  ...
  always @(posedge clk)
      begin
        if ((state >= bus_data_ready) && 
            (state < ((-1)+bus_transmit_finish))) srdy <= 1;
        else srdy <= 0;
      end
endmodule
