...
(wr_ptr[ADDR_WIDTH+(-1):0] == wr_ptr_cur[ADDR_WIDTH+(-1):0]) && (wr_ptr[ADDR_WIDTH] != wr_ptr_cur[ADDR_WIDTH]);
  wire 
       write = (input_axis_tvalid & ~full) | (((input_axis_tvalid & (~full & DROP_WHEN_FULL)) | ((input_axis_tvalid | ~full) & DROP_WHEN_FULL)) & input_axis_tvalid);
  wire 
       read = ((output_axis_tready | (~output_axis_tvalid_reg & (~empty | output_axis_tready))) & (~output_axis_tvalid_reg | ~empty)) & ~empty;

  assign {output_axis_tlast,output_axis_tkeep,output_axis_tdata} = data_out_reg;
  assign input_axis_tready = ((input_axis_tvalid & ~full) | DROP_WHEN_FULL) | ~full;
                  ...
                  begin
                    if (input_axis_tuser) 
                      ...
                      begin
                        ...
                        wr_ptr <= wr_ptr_next;
                        wr_ptr_gray <= ((~(wr_ptr_next>>1 & wr_ptr_next) & wr_ptr_next>>1) | (~wr_ptr_next>>1 & (wr_ptr_next>>1 & wr_ptr_next))) | (wr_ptr_next & ~wr_ptr_next>>1);
                      end
                  end
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= ((rd_ptr_next>>1 & (rd_ptr_next | ((~rd_ptr_next & (rd_ptr_next>>1 | rd_ptr_next)) | ~rd_ptr_next>>1))) & (rd_ptr_next | ((~rd_ptr_next & (rd_ptr_next>>1 | rd_ptr_next)) | ~rd_ptr_next>>1))) ^ rd_ptr_next;
          end
