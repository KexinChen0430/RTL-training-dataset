...
module axis_async_frame_fifo_64  #(parameter 
  ...
  reg   drop_frame = 1'b0;
  reg  [(KEEP_WIDTH+1)+(1+((-1)+DATA_WIDTH)):0] 
       data_out_reg = {1'b0,{KEEP_WIDTH{1'b0}},{DATA_WIDTH{1'b0}}};
  reg  [(KEEP_WIDTH+1)+(1+((-1)+DATA_WIDTH)):0] mem[(1<<1**ADDR_WIDTH)-1:0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(KEEP_WIDTH+1)+(1+((-1)+DATA_WIDTH)):0] 
...
(wr_ptr[ADDR_WIDTH+(-1):0] == wr_ptr_cur[ADDR_WIDTH+(-1):0]) && (wr_ptr[ADDR_WIDTH] != wr_ptr_cur[ADDR_WIDTH]);
  wire 
       write = (input_axis_tvalid & ~full) | (((~full | input_axis_tvalid) & DROP_WHEN_FULL) & input_axis_tvalid);
  wire 
                  ...
                  begin
                    if (input_axis_tuser) 
                      ...
                      begin
                        ...
                        wr_ptr <= wr_ptr_next;
                        wr_ptr_gray <= ((wr_ptr_next>>1 & ~wr_ptr_next) | ~wr_ptr_next>>1) & (wr_ptr_next | wr_ptr_next>>1);
                      end
                  end
