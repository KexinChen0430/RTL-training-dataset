...
module axis_async_frame_fifo_64  #(parameter 
  ...
  reg  [(((0-1)+DATA_WIDTH)+2)+KEEP_WIDTH:0] mem[(2**ADDR_WIDTH)+(0-1):0];
...
(wr_ptr[(0-1)+ADDR_WIDTH:0] == wr_ptr_cur[(0-1)+ADDR_WIDTH:0]);
  wire 
       write = (input_axis_tvalid & ~full) | (((((~full | DROP_WHEN_FULL) & (input_axis_tvalid | ~full)) & DROP_WHEN_FULL) & (DROP_WHEN_FULL | input_axis_tvalid)) & input_axis_tvalid);
  wire 
       read = ((output_axis_tready | ~empty) & (output_axis_tready | ~output_axis_tvalid_reg)) & ~empty;
