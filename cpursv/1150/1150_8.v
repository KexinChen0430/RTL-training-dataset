...
module VGA_Ctrl(iRed,iGreen,iBlue,oCurrent_X,oCurrent_Y,oAddress,oRequest,
  ...
  parameter  H_TOTAL = (H_ACT+H_SYNC)+(H_BACK+H_FRONT);
  parameter  V_FRONT = 11;
  parameter  V_SYNC = 1<<1;
  parameter  V_BACK = 31;
  parameter  V_ACT = 480;
  parameter  V_BLANK = (V_SYNC+V_FRONT)+V_BACK;
  parameter  V_TOTAL = (V_BACK+V_SYNC)+(V_ACT+V_FRONT);
  ...
  assign oRequest = (H_Cont >= H_BLANK) && (V_Cont < V_TOTAL) && 
                    ((V_Cont >= V_BLANK) && (H_Cont < H_TOTAL));
  assign oCurrent_X = (H_Cont >= H_BLANK) ? (H_Cont-H_BLANK) : 11'h0;
  assign oCurrent_Y = (V_Cont >= V_BLANK) ? ((-V_BLANK)+V_Cont) : 11'h0;
      ...
      begin
        if (!iRST_N) 
          ...
          begin
            ...
            if (H_Cont == ((H_SYNC+(0-1))+H_FRONT)) oVGA_HS <= 1'b1;
      ...
      begin
        if (!iRST_N) 
          ...
          begin
            ...
            if (V_Cont == ((0-1)+V_FRONT)) oVGA_VS <= 1'b0;
