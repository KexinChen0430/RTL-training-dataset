...
module VGA_Ctrl(iRed,iGreen,iBlue,oCurrent_X,oCurrent_Y,oAddress,oRequest,
  ...
  parameter  H_TOTAL = H_BACK+((H_FRONT+H_ACT)+H_SYNC);
  ...
  assign oRequest = (H_Cont < H_TOTAL) && (V_Cont < V_TOTAL) && 
                    ((V_Cont >= V_BLANK) && (H_Cont >= H_BLANK));
  assign oCurrent_X = (H_Cont >= H_BLANK) ? (H_Cont+(-H_BLANK)) : 11'h0;
  assign oCurrent_Y = (V_Cont >= V_BLANK) ? ((0-V_BLANK)+V_Cont) : 11'h0;
      ...
      begin
        if (!iRST_N) 
          ...
          begin
            ...
            if (H_Cont == (H_FRONT+((-1)+H_SYNC))) oVGA_HS <= 1'b1;
