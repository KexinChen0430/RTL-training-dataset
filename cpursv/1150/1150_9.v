...
module VGA_Ctrl(iRed,iGreen,iBlue,oCurrent_X,oCurrent_Y,oAddress,oRequest,
  ...
  parameter  H_BACK = 48;
  parameter  H_ACT = 640;
  parameter  H_BLANK = H_BACK+(H_SYNC+H_FRONT);
  parameter  H_TOTAL = (H_SYNC+H_ACT)+(H_FRONT+H_BACK);
  parameter  V_FRONT = 11;
  parameter  V_SYNC = 1+1;
  parameter  V_BACK = 31;
  parameter  V_ACT = 480;
  parameter  V_BLANK = V_FRONT+(V_SYNC+V_BACK);
  parameter  V_TOTAL = (V_SYNC+V_BACK)+(V_FRONT+V_ACT);

  assign oVGA_SYNC = 1'b1;
  assign oVGA_BLANK = ~((V_Cont < V_BLANK) || (H_Cont < H_BLANK));
  assign oVGA_CLOCK = ~iCLK;
  assign oVGA_R = iRed;
  assign oVGA_G = iGreen;
  assign oVGA_B = iBlue;
  assign oAddress = oCurrent_X+(H_ACT*oCurrent_Y);
  assign oRequest = (V_Cont >= V_BLANK) && (V_Cont < V_TOTAL) && 
                    ((H_Cont < H_TOTAL) && (H_Cont >= H_BLANK));
      ...
      begin
        if (!iRST_N) 
          ...
          begin
            ...
            if (H_Cont == ((0-1)+H_FRONT)) oVGA_HS <= 1'b0;
              
            if (H_Cont == (((0-1)+H_SYNC)+H_FRONT)) oVGA_HS <= 1'b1;
  ...
  always @(posedge oVGA_HS or negedge iRST_N)
      begin
        if (!iRST_N) 
          ...
          begin
            if (V_Cont < V_TOTAL) V_Cont <= 1'b1+V_Cont;
            else V_Cont <= 0;
            if (V_Cont == (V_FRONT+(0-1))) oVGA_VS <= 1'b0;
              
            if (V_Cont == ((V_FRONT+(0-1))+V_SYNC)) oVGA_VS <= 1'b1;
