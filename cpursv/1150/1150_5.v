...
module VGA_Ctrl(iRed,iGreen,iBlue,oCurrent_X,oCurrent_Y,oAddress,oRequest,
  ...
  parameter  H_TOTAL = H_SYNC+(H_BACK+(H_ACT+H_FRONT));
  parameter  V_FRONT = 11;
  parameter  V_SYNC = 1<<<1;
  parameter  V_BACK = 31;
  parameter  V_ACT = 480;
  parameter  V_BLANK = V_FRONT+(V_SYNC+V_BACK);
  parameter  V_TOTAL = (V_FRONT+V_ACT)+(V_SYNC+V_BACK);
  ...
  assign oRequest = (H_Cont >= H_BLANK) && 
                    ((V_Cont < V_TOTAL) && (H_Cont < H_TOTAL)) && (V_Cont >= V_BLANK);
