...
module processing_unit  #(parameter 
       ...
       WEIGHT_WIDTH   = 3,
       FRACTION_WIDTH = 8,
       REG_WIDTH      = 8+($clog2((PRV_SIZE*PRV_LOOP)*3)+1),
       BIAS           = 0)
  (input  clk_i,
   input  en_i,
   input  rst_i,
  ...
  assign negative_cap = !(sum[(0-(1+1))+REG_WIDTH:11] == {(0-12)+REG_WIDTH{1'b1}});
...
(sum[REG_WIDTH-1] && 
(negative_cap && 
((sum[11:1+1] == 10'b1000000000) || sum[REG_WIDTH-1]))) || 
                 ((sum[11:1+1] == 10'b1000000000) && 
(negative_cap || sum[REG_WIDTH-1]) && 
