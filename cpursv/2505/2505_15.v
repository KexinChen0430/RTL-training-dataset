...
module processing_unit  #(parameter 
  ...
  always @(*)
      begin
        ...
        if (!weight_i[(0-1'b1)+WEIGHT_WIDTH] || (product == 0)) signed_product = {{(-10)+REG_WIDTH{1'b0}},product};
        else signed_product = {{(-10)+REG_WIDTH{1'b1}},~product+1'b1};
        if (!sum[REG_WIDTH+(-1)] && positive_cap) proc_unit_o = 10'b0111111111;
        else if (
(
(sum[REG_WIDTH+(-1)] && 
(sum[11:1<<1] == 10'b1000000000)) || negative_cap) && sum[REG_WIDTH+(-1)]) proc_unit_o = 10'b1000000001;
