...
module processing_unit  #(parameter 
       ...
       WEIGHT_WIDTH   = 3,
       FRACTION_WIDTH = 8,
       REG_WIDTH      = $clog2((PRV_LOOP*PRV_SIZE)*3)+(8+1),
       BIAS           = 0)
  (input  clk_i,
   input  en_i,
   input  rst_i,
  ...
  always @(*)
      begin
        ...
        if (!sum[REG_WIDTH+(0-1)] && positive_cap) proc_unit_o = 10'b0111111111;
        else if (
((sum[11:1<<<1] == 10'b1000000000) && 
sum[REG_WIDTH+(0-1)]) || (sum[REG_WIDTH+(0-1)] && negative_cap)) proc_unit_o = 10'b1000000001;
