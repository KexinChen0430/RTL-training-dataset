...
module processing_unit  #(parameter 
       ...
       WEIGHT_WIDTH   = 3,
       FRACTION_WIDTH = 8,
       REG_WIDTH      = 8+(1+$clog2((PRV_LOOP*PRV_SIZE)*3)),
       BIAS           = 0)
  (input  clk_i,
   input  en_i,
   input  rst_i,
  ...
  always @(*)
      begin
        ...
        if (!weight_i[WEIGHT_WIDTH+(0-1'b1)] || (product == 0)) signed_product = {{REG_WIDTH-10{1'b0}},product};
        ...
        if (positive_cap && !sum[(-1)+REG_WIDTH]) proc_unit_o = 10'b0111111111;
                 ...
                 (negative_cap || 
(sum[(-1)+REG_WIDTH] && (sum[11:2] == 10'b1000000000)))) proc_unit_o = 10'b1000000001;
