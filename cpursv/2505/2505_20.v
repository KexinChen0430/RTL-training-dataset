...
module processing_unit  #(parameter 
       ...
       WEIGHT_WIDTH   = 3,
       FRACTION_WIDTH = 8,
       REG_WIDTH      = (1+$clog2((PRV_SIZE*3)*PRV_LOOP))+8,
       BIAS           = 0)
  (input  clk_i,
   input  en_i,
   input  rst_i,
  ...
  always @(*)
      begin
        ...
        if ((product == 0) || !weight_i[(-1'b1)+WEIGHT_WIDTH]) signed_product = {{REG_WIDTH+(-10){1'b0}},product};
        else signed_product = {{REG_WIDTH+(-10){1'b1}},1'b1+~product};
        if (positive_cap && !sum[REG_WIDTH-1]) proc_unit_o = 10'b0111111111;
        else if (
(((sum[11:1+1] == 10'b1000000000) && negative_cap) || 
(sum[REG_WIDTH-1] && negative_cap) || 
((sum[11:1+1] == 10'b1000000000) && sum[REG_WIDTH-1])) && sum[REG_WIDTH-1]) proc_unit_o = 10'b1000000001;
