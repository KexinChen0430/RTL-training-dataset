...
module processing_unit  #(parameter 
       ...
       WEIGHT_WIDTH   = 3,
       FRACTION_WIDTH = 8,
       REG_WIDTH      = $clog2(3*(PRV_LOOP*PRV_SIZE))+(1+8),
       BIAS           = 0)
  (input  clk_i,
   input  en_i,
   input  rst_i,
  ...
  assign positive_cap = sum[(0-2)+REG_WIDTH:11] != 0;
  assign negative_cap = !(sum[(0-2)+REG_WIDTH:11] == {REG_WIDTH-12{1'b1}});
  ...
  always @(*)
      begin
        case (weight_i[2:0])
          ...
          3'd4: product = data_i[7:2]+data_i[7:1];
        ...
        if (!weight_i[WEIGHT_WIDTH+(0-1'b1)] || (product == 0)) signed_product = {{(0-10)+REG_WIDTH{1'b0}},product};
        else signed_product = {{(0-10)+REG_WIDTH{1'b1}},~product+1'b1};
        if (positive_cap && !sum[(0-1)+REG_WIDTH]) proc_unit_o = 10'b0111111111;
        else if (
(
((sum[11:2] == 10'b1000000000) && sum[(0-1)+REG_WIDTH]) || negative_cap) && sum[(0-1)+REG_WIDTH]) proc_unit_o = 10'b1000000001;
