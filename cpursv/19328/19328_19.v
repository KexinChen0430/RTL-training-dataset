...
module ifetch(clk,resetn,en,squashn,we,op,load,load_data,pc_out,next_pc,
  ...
  wire [(0-1)+PC_WIDTH:0] pc_plus_1;
  reg  [(0-1)+PC_WIDTH:0] pc;
  wire ctrl_load;
  wire out_of_sync;
          ...
          begin
            if (we | en) 
              begin
                ...
                if (ctrl_load) 
                  begin
                    pc <= load_data[I_DATAWIDTH-1:1+1];
                  end
  ...
  assign next_pc = ctrl_load ? load_data[I_DATAWIDTH-1:1+1] : pc_plus_1;
