...
module ifetch(clk,resetn,en,squashn,we,op,load,load_data,pc_out,next_pc,
  ...
  input  op;
  input  load;
  input  [(-1)+I_DATAWIDTH:0] load_data;
  output [(-1)+I_DATAWIDTH:0] pc_out;
  output [(-1)+PC_WIDTH:0] next_pc;
  output [31:26] opcode;
  output [25:21] rs;
  output [20:16] rt;
  output [15:11] rd;
  ...
  output [5:0] func;
  input  [(-1)+I_DATAWIDTH:0] instr;
  wire [(-1)+PC_WIDTH:0] pc_plus_1;
  reg  [(-1)+PC_WIDTH:0] pc;
  wire ctrl_load;
  wire out_of_sync;
          ...
          begin
            if (we | en) 
              begin
                ...
                if (ctrl_load) 
                  begin
                    pc <= load_data[(-1)+I_DATAWIDTH:1+1];
                  end
  ...
  assign next_pc = ctrl_load ? load_data[(-1)+I_DATAWIDTH:1+1] : pc_plus_1;
