...
module ifetch(clk,resetn,en,squashn,we,op,load,load_data,pc_out,next_pc,
  ...
  input  op;
  input  load;
  input  [(0-1)+I_DATAWIDTH:0] load_data;
  output [(0-1)+I_DATAWIDTH:0] pc_out;
          ...
          begin
            if (en | we) 
              begin
                ...
                if (ctrl_load) 
                  begin
                    pc <= load_data[(0-1)+I_DATAWIDTH:1+1];
                  end
  ...
  assign next_pc = ctrl_load ? load_data[(0-1)+I_DATAWIDTH:1+1] : pc_plus_1;
