...
module BusArbiter  #(parameter  bus_width = 32, bus_max_devices = 16)
  (input  clk,
   ...
   inout  [(-1)+bus_max_devices:0] receiver_device,
   inout  [(-1)+bus_max_devices:0] buffer_full,
   input  [(-1)+bus_max_devices:0] write_request,
   output reg [(-1)+bus_max_devices:0] voice);

  reg  [(-1)+bus_max_devices:0]  operating_device = 0;
  ...
  generate
      ...
      for (device = 1; device < bus_max_devices; device = device+1)
          begin : voice_gen
            ...
            always @(negedge clk)
                if (
(operating_device && ~write_request[operating_device]) || !operating_device) 
                  begin
                    voice[device] = write_request[device] && !write_request[(-1)+device:0];
