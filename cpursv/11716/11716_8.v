...
module usb_ep(input  clk,
  ...
  always @(*)
      begin
        if (!direction_in && (!ep_out_stall || setup || !ep_setup) && 
            (!ep_setup || setup) && 
            ((!ep_out_stall || setup || !ep_setup) && 
(ep_out_full || (!ep_out_stall || setup))) && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (ep_in_full && !ep_in_stall && (!ep_out_stall || !ep_setup) && 
                !ep_setup) 
              begin
                handshake = hs_ack;
              end
            else if (ep_in_stall && (!ep_setup || setup) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
...
(!ep_out_stall || setup || !ep_setup || 
(!ep_out_stall && ep_out_full)) && 
((!ep_out_stall && ep_out_full) || (!ep_setup || setup))))) 
              begin
                handshake = hs_ack;
              end
