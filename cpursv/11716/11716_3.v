...
module usb_ep(input  clk,
  ...
  always @(*)
      begin
        if (setup && 
            (!direction_in && 
(setup || ep_out_full || !ep_out_stall) && (setup || ep_out_full))) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          ...
          begin
            if ((setup || ep_out_full || !ep_out_stall) && 
                ((ep_out_full && !ep_out_stall) || (setup || !ep_setup)) && (setup || !ep_setup) && 
                ((setup || ep_out_full) && 
((setup || !ep_out_stall || !ep_setup) && 
(setup || ep_out_full)) && (setup || !ep_out_stall))) 
              begin
                handshake = hs_ack;
              end
            else if (ep_out_stall && 
                     ((ep_out_full && !ep_out_stall) || (setup || !ep_setup)) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
