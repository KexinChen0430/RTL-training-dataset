...
module usb_ep(input  clk,
  ...
  always @(*)
      begin
        if (!direction_in && (setup || !ep_setup || ep_out_full) && 
            ((!ep_out_stall || setup) && (setup || ep_out_full)) && setup) toggle = 1'b0;
...
(setup || !ep_setup || ep_out_full)) && !ep_setup && !ep_in_stall) 
              begin
                handshake = hs_ack;
              end
            else if (ep_in_stall && (setup || !ep_setup || ep_out_full) && 
                     !ep_setup) 
              begin
                handshake = hs_stall;
              end
          ...
          begin
            if ((setup || !ep_setup) && 
                ((!ep_out_stall || setup) && (setup || ep_out_full))) 
              begin
                handshake = hs_ack;
              end
