...
(setup || (!ep_out_stall && !ep_setup)) || 
(ep_out_full || setup || 
(setup || (!ep_out_stall && !ep_setup)))) && (!ep_setup || (!ep_out_stall || setup)) && 
            (!ep_out_stall || setup || ep_out_full) && (ep_out_full || setup) && 
            (
(ep_out_full && (!ep_out_stall || !ep_setup) && 
!ep_out_stall) || (!ep_out_stall && !ep_setup) || setup) && 
            (!direction_in && 
(!ep_setup || (!ep_out_stall || setup) || 
(ep_out_full && (!ep_out_stall || !ep_setup) && 
!ep_out_stall)) && (!ep_out_stall || setup) && (ep_out_full || setup)) && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (ep_in_full && !ep_in_stall && !ep_setup) 
              begin
                handshake = hs_ack;
              end
