...
module usb_ep(input  clk,
  ...
  always @(*)
      begin
        if ((setup || !ep_out_stall) && 
            (!direction_in && (setup || !ep_out_stall)) && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (ep_in_full && !ep_in_stall && 
                ((!ep_setup || !ep_out_stall || setup) && 
(!ep_setup || !ep_out_stall || setup || ep_out_full) && 
(!ep_setup || 
(setup || ((setup && !ep_out_stall) || ep_out_full)))) && !ep_setup) 
              begin
                handshake = hs_ack;
              end
...
(!ep_setup || !ep_out_stall || setup) && ep_out_full && !ep_setup) || setup) && (setup || !ep_out_stall)) 
              begin
                handshake = hs_ack;
              end
