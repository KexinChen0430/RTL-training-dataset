  ...
  always @(*)
      begin
        if (!direction_in && 
            (
(!ep_setup && !ep_setup && 
(!ep_out_stall || (!ep_setup && setup))) || 
((setup || (ep_out_full || (!ep_setup && setup))) && 
(!ep_out_stall || (!ep_setup && setup) || setup))) && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (!ep_in_stall && 
                (ep_in_full && (!ep_setup || !ep_out_stall) && !ep_setup)) 
              begin
                handshake = hs_ack;
              end
            else if (ep_in_stall && 
                     (!ep_setup || (setup && !ep_out_stall) || setup) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
          ...
          begin
            if ((setup || (ep_out_full || (!ep_setup && setup))) && 
                (!ep_out_stall || (!ep_setup && setup) || setup) && (!ep_setup || (setup && !ep_out_stall) || setup)) 
              begin
                handshake = hs_ack;
              end
