...
module usb_ep(input  clk,
  ...
  always @(*)
      begin
        if ((setup || !ep_out_stall) && 
            ((ep_out_full || setup) && !direction_in) && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (ep_in_full && (!ep_setup && !ep_in_stall)) 
              begin
                handshake = hs_ack;
              end
            else if (ep_in_stall && 
                     (setup || (ep_out_full && !ep_out_stall) || !ep_setup) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
          ...
          begin
            if (
((setup || !ep_out_stall) && !ep_setup && 
(
((!ep_out_stall || (setup || !ep_setup)) && 
(ep_out_full || setup || (!ep_out_stall && !ep_setup)) && (ep_out_full || setup) && !ep_out_stall) || ((ep_out_full && !ep_setup) || setup))) || 
                (ep_out_full && 
(
(
(setup || !ep_setup || 
(ep_out_full || setup || (!ep_out_stall && !ep_setup))) && !ep_out_stall && 
(
((!ep_out_stall || (setup || !ep_setup)) && 
(ep_out_full || setup || (!ep_out_stall && !ep_setup)) && (ep_out_full || setup) && !ep_out_stall && setup) || !ep_setup)) || ep_out_full) && 
(
((!ep_out_stall || (setup || !ep_setup)) && 
(ep_out_full || setup || (!ep_out_stall && !ep_setup)) && (ep_out_full || setup) && !ep_out_stall && setup) || ((setup || !ep_out_stall) && !ep_setup) || setup)) || setup) 
              begin
                handshake = hs_ack;
              end
            else if (ep_out_stall && 
                     (!ep_out_stall || (setup || !ep_setup)) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
  ...
  wire 
       flush = ctrl_wr_data[3] || ctrl_wr_data[4] || ctrl_wr_data[5];
