...
(
(setup && 
((!ep_out_stall || (setup || ep_out_full)) && !ep_setup)) || !ep_out_stall)) && !direction_in && 
            ((setup || ep_out_full) && 
(
(!ep_setup && 
((!ep_setup && ep_out_full) || !ep_out_stall || setup) && !ep_out_stall) || (!ep_setup || !ep_out_stall || setup)) && 
(setup || 
(
(setup && 
((!ep_out_stall || (setup || ep_out_full)) && !ep_setup)) || !ep_out_stall))) && 
            ((!ep_setup && ep_out_full) || !ep_out_stall || setup) && setup) toggle = 1'b0;
      ...
      begin
        if (direction_in) 
          begin
            ...
            else if (ep_in_stall && (ep_out_full || (!ep_setup || setup)) && 
                     !ep_setup) 
              begin
                handshake = hs_stall;
              end
