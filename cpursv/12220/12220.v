...
module de_top_misc(input  de_clk,
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ca_busyi <= 1'b0;
          ...
          ca_busyi <= ~pc_empty | ((busy_hb & de_busy_sync) | (~pc_mc_rdy & ca_busyi));
      end
  ...
  assign de_trnsp_2 = ((dr_style_2[1] & ~dr_style_2[0]) & ~dx_blt_actv_2) | ((dr_style_2[1] & ~dr_style_2[0]) & (dr_style_2[3] | dr_style_2[2]));
