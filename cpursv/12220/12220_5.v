...
module de_top_misc(input  de_clk,
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ca_busyi <= 1'b0;
          ...
          ca_busyi <= (de_busy_sync & ((busy_hb & (busy_hb | ca_busyi)) & (ca_busyi | de_busy_sync))) | ((~pc_empty | ~pc_mc_rdy) & (~pc_empty | ca_busyi));
      end
  ...
  assign de_trnsp_2 = (~dx_blt_actv_2 | (dr_style_2[3] | dr_style_2[2])) & (dr_style_2[1] & ~dr_style_2[0]);
      ...
      begin
        if (!hb_rstn) 
          ...
          begin
            ...
            if (((~abort_cmd_flag & (abort_cmd_flag_d | deb_last)) & (abort_cmd_flag_d | ~deb)) | (((~abort_cmd_flag | ((~abort_cmd_flag | ~deb) & deb_last)) & ~deb) & deb_last)) deb_clr_hold <= ~deb_clr_hold;
  ...
  assign busy_and_not_noop = (line_actv_1 && busy_hb) || (busy_hb && blt_actv_1);
  ...
  always @(posedge hb_clk or negedge hb_rstn)
      begin
        if (!hb_rstn) dx_deb <= 1'b0;
                 ...
                 ((((deb_clr_q2 & (~deb_clr_q2 | deb_clr_q2)) & (~deb_clr_q1 | ~deb_clr_q2)) & ~deb_clr_q1) | (~deb_clr_q2 & ((~deb_clr_q1 | deb_clr_q1) & (deb_clr_q1 | deb_clr_q2))))) dx_deb <= 1'b0;
