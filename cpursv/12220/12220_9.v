...
module de_top_misc(input  de_clk,
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ca_busyi <= 1'b0;
          ...
          ca_busyi <= (ca_busyi & (~pc_mc_rdy | ~pc_empty)) | ((busy_hb | ~pc_empty) & (~pc_empty | de_busy_sync));
      end
  ...
  assign de_trnsp_2 = ((dr_style_2[2] | (dr_style_2[3] | ~dx_blt_actv_2)) & dr_style_2[1]) & ((dr_style_2[1] & ~dr_style_2[0]) | (~dr_style_2[0] & ~dx_blt_actv_2));
  ...
  always @(posedge de_clk)
      begin
        clip_d <= (wb_clip | line_actv_2) & (wb_clip | clip);
        clip_dd <= clip_d;
      ...
      begin
        if (!hb_rstn) 
          ...
          begin
            ...
            if ((((~abort_cmd_flag | ~deb) & (~abort_cmd_flag | deb_last)) & (abort_cmd_flag_d | deb_last)) & (~deb | abort_cmd_flag_d)) deb_clr_hold <= ~deb_clr_hold;
