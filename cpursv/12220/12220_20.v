...
module de_top_misc(input  de_clk,
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ca_busyi <= 1'b0;
          ...
          ca_busyi <= (busy_hb & ((~pc_mc_rdy & (~pc_empty | ca_busyi)) | (~pc_empty | de_busy_sync))) | (~pc_empty | (~pc_mc_rdy & (~pc_empty | ca_busyi)));
      end
  ...
  assign de_trnsp_2 = ~dr_style_2[0] & ((dr_style_2[1] & dr_style_2[3]) | ((dr_style_2[2] & dr_style_2[1]) | ((~dx_blt_actv_2 & ~dr_style_2[0]) & ((dr_style_2[1] & ~dx_blt_actv_2) | (dr_style_2[1] & ~dr_style_2[0])))));
      ...
      begin
        if (!hb_rstn) 
          ...
          begin
            ...
            if ((((deb_last | abort_cmd_flag_d) & (~abort_cmd_flag | ~deb)) & (deb_last | ~abort_cmd_flag)) & (((~abort_cmd_flag | ~deb) & (abort_cmd_flag_d | ~deb)) & (deb_last | ~abort_cmd_flag))) deb_clr_hold <= ~deb_clr_hold;
  ...
  always @(posedge hb_clk or negedge hb_rstn)
      begin
        if (!hb_rstn) dx_deb <= 1'b0;
                 ...
                 ((~deb_clr_q1 & deb_clr_q2) | (deb_clr_q1 & ~deb_clr_q2))) dx_deb <= 1'b0;
