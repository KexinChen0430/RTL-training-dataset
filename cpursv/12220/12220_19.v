...
module de_top_misc(input  de_clk,
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ca_busyi <= 1'b0;
          ...
          ca_busyi <= ((((~pc_mc_rdy | busy_hb) & (de_busy_sync | ~pc_mc_rdy)) & (de_busy_sync | (ca_busyi | ~pc_empty))) & (~pc_empty | (ca_busyi | busy_hb))) | ~pc_empty;
      end
  ...
  assign de_trnsp_2 = ((dr_style_2[1] & dr_style_2[2]) & ~dr_style_2[0]) | (((~dr_style_2[0] & (dr_style_2[1] & dr_style_2[3])) | ((~dx_blt_actv_2 & ~dr_style_2[0]) | ((~dr_style_2[0] & (dr_style_2[1] & dr_style_2[3])) & ((~dx_blt_actv_2 & (~dr_style_2[0] | dr_style_2[1])) & dr_style_2[1])))) & dr_style_2[1]);
  assign stpl_2[1] = ~line_actv_2 & dr_style_2[3];
  assign stpl_2[0] = ~dr_style_2[3] & (dr_style_2[2] & ~line_actv_2);
  
  always @(posedge de_clk)
      begin
        tmp_rstn <= hb_rstn & sys_locked;
        de_rstn <= tmp_rstn;
      end
  ...
  always @(posedge de_clk)
      begin
        clip_d <= (wb_clip | clip) & (line_actv_2 | wb_clip);
        clip_dd <= clip_d;
      ...
      begin
        if (!hb_rstn) 
          ...
          begin
            ...
            if ((((abort_cmd_flag_d | ((~deb | abort_cmd_flag_d) & deb_last)) & ~abort_cmd_flag) | ~deb) & ((((((abort_cmd_flag_d | deb_last) & (~deb | ~abort_cmd_flag)) & (~abort_cmd_flag | deb_last)) & ~abort_cmd_flag) & (((~deb & (~deb | ~abort_cmd_flag)) & (~abort_cmd_flag | deb_last)) & ((deb_last & (~abort_cmd_flag | deb_last)) & (~deb | ~abort_cmd_flag)))) | ((deb_last & (abort_cmd_flag_d | ((~deb | abort_cmd_flag_d) & deb_last))) | (~abort_cmd_flag & abort_cmd_flag_d)))) deb_clr_hold <= ~deb_clr_hold;
  ...
  assign busy_and_not_noop = (blt_actv_1 || line_actv_1) && busy_hb;
  
  always @(posedge hb_clk or negedge hb_rstn)
      begin
        if (!hb_rstn) dx_deb <= 1'b0;
        else if (cmd_trig_comb && 
                 !((opc_1 == LD_TPAL) || (opc_1 == LD_TEX))) dx_deb <= 1'b1;
        else if (!busy_and_not_noop && 
                 (((deb_clr_q2 & (~deb_clr_q2 | deb_clr_q2)) & (~deb_clr_q1 | ~deb_clr_q2)) | (((~deb_clr_q1 | ~deb_clr_q2) & deb_clr_q1) & ~deb_clr_q2))) dx_deb <= 1'b0;
