...
module de_top_misc(input  de_clk,
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ca_busyi <= 1'b0;
          ...
          ca_busyi <= ((~pc_mc_rdy & ca_busyi) | ~pc_empty) | (busy_hb & (de_busy_sync | ~pc_empty));
      end
  ...
  assign de_trnsp_2 = (~dr_style_2[0] & dr_style_2[1]) & ((dr_style_2[2] & ~dr_style_2[0]) | (dr_style_2[1] & (((~dx_blt_actv_2 & dr_style_2[1]) & (~dx_blt_actv_2 & ~dr_style_2[0])) | (~dr_style_2[0] & dr_style_2[3]))));
      ...
      begin
        if (!hb_rstn) 
          ...
          begin
            ...
            if (((~deb | abort_cmd_flag_d) & (~abort_cmd_flag | (~deb & deb_last))) & (((abort_cmd_flag_d | deb_last) & (~abort_cmd_flag | ~deb)) & (deb_last | ~abort_cmd_flag))) deb_clr_hold <= ~deb_clr_hold;
  ...
  assign busy_and_not_noop = ((busy_hb && line_actv_1) || blt_actv_1) && busy_hb;
  
  always @(posedge hb_clk or negedge hb_rstn)
      begin
        if (!hb_rstn) dx_deb <= 1'b0;
        else if (!((opc_1 == LD_TPAL) || (opc_1 == LD_TEX)) && 
                 cmd_trig_comb) dx_deb <= 1'b1;
        else if (!busy_and_not_noop && (deb_clr_q2 ^ deb_clr_q1)) dx_deb <= 1'b0;
