...
module de_top_misc(input  de_clk,
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ca_busyi <= 1'b0;
          ...
          ca_busyi <= (((ca_busyi | (busy_hb & (de_busy_sync | ca_busyi))) | ~pc_empty) & ~pc_mc_rdy) | (((~pc_empty | de_busy_sync) & ((ca_busyi | (busy_hb & (de_busy_sync | ca_busyi))) | ~pc_empty)) & (busy_hb | ~pc_empty));
      end
  ...
  assign de_trnsp_2 = (dr_style_2[1] & ~dr_style_2[0]) & ((dr_style_2[3] | ~dx_blt_actv_2) | dr_style_2[2]);
      ...
      begin
        if (!hb_rstn) 
          ...
          begin
            ...
            if ((deb_last & ~deb) | (((~abort_cmd_flag | ~deb) & (((deb_last | abort_cmd_flag_d) & ~deb) | abort_cmd_flag_d)) & ~abort_cmd_flag)) deb_clr_hold <= ~deb_clr_hold;
  ...
  assign busy_and_not_noop = (blt_actv_1 && busy_hb) || (line_actv_1 && busy_hb);
