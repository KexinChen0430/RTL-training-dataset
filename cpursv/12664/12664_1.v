      ...
      for (i = 0; i < in0_WIDTH; i = 1+i)
          begin : loop
            if (in0_WIDTH == 1) assign comb_tmp[i] = dividend_tmp[i][0];
              ...
              assign comb_tmp[i] = {remd_tmp[i][(0-1<<<1)+in0_WIDTH:0],dividend_tmp[i][in0_WIDTH-1]};
                  ...
                  if (ce) 
                    begin
                      if (in0_WIDTH == 1) dividend_tmp[1+i] <= ~cal_tmp[i][cal_WIDTH];
                        ...
                        dividend_tmp[1+i] <= {dividend_tmp[i][(0-1<<<1)+in0_WIDTH:0],~cal_tmp[i][cal_WIDTH]};
                      divisor_tmp[1+i] <= divisor_tmp[i];
                      remd_tmp[1+i] <= cal_tmp[i][cal_WIDTH] ? comb_tmp[i] : cal_tmp[i][in0_WIDTH-1:0];
                    end
