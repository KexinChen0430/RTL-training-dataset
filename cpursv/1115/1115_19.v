...
module eth_random(MTxClk,Reset,StateJam,StateJam_q,RetryCnt,NibCnt,ByteCnt,
  ...
  assign Feedback = ~((x[9] ^ ((((x[2] | (~x[2] & x[9])) | ~x[9]) & x[2]) & x[9])) ^ ((x[2] | (~x[2] & x[9])) & ((x[2] | (~x[2] & x[9])) & (((~(x[9] & x[2]) & (((((x[2] | (~x[2] & x[9])) | ~x[9]) & x[2]) & (x[2] | ~x[2])) & x[2])) | (x[9] & x[2])) & ((~(x[9] & x[2]) & (((((x[2] | (~x[2] & x[9])) | ~x[9]) & x[2]) & (x[2] | ~x[2])) & x[2])) | ~(((((x[2] | (~x[2] & x[9])) | ~x[9]) & x[2]) & (x[2] | ~x[2])) & x[2]))))));
  assign Random[0] = x[0];
  ...
  always @(posedge MTxClk or posedge Reset)
      begin
        if (Reset) RandomLatched <= #Tp 10'h000;
          ...
          begin
            if (StateJam & StateJam_q) RandomLatched <= #Tp Random;
