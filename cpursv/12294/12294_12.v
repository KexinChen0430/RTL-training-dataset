...
module ycrcb2rgb(R,G,B,clk,rst,Y,Cr,Cb);
  ...
  always @(posedge clk or posedge rst)
      if (rst) 
        ...
        begin
          X_int <= (Y_reg+(0-'d64))*const1;
          A_int <= const2*(Cr_reg+(0-'d512));
          B1_int <= const3*(Cr_reg+(0-'d512));
          B2_int <= (Cb_reg-'d512)*const4;
          C_int <= (Cb_reg-'d512)*const5;
        end
