...
module GameLoader(input  clk,
  ...
  assign mem_write = 
(indata_clk && ((bytes_left != 0) && (state == 1))) || 
                     (indata_clk && (state == 1<<<1) && (bytes_left != 0));
  wire [1<<<1:0] 
       prg_size = (prgrom <= 1) ? 0 : 
(prgrom <= 1<<<1) ? 1 : 
(prgrom <= 4) ? 1<<<1 : 
            ...
            case (state)
              ...
              0: if (indata_clk) 
                    begin
                      ...
                      if (ctr == 4'b1111) 
                        state <= ((ines[1] == 8'h45) && (ines[1<<<1] == 8'h53) && 
                                ((ines[0] == 8'h4E) && 
((ines[3] == 8'h1A) && !ines[6][1<<<1]))) ? 1 : 3;
