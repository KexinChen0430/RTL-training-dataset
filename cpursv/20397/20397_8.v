...
module Registers(input  wire Clk,
  ...
  assign RequestInBuffer = CharLoadBuffer | (BufferInitDicPointer | StringLoadBuffer);
  ...
  always @(posedge Clk)
      begin
        if (LoadInsertPointer) 
          ...
          begin
            InsertPointer = InsertPointer+(1+((1/(1+1))*StringSize));
          end
  ...
  always @(posedge Clk)
      begin
        if (SetJumpAddress) 
          begin
            JumpAddress = 1+(DicPointer+((StringRAM[135:127]/(1+1))+StringRAM));
          end
