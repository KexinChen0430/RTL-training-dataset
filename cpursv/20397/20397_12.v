...
module Registers(input  wire Clk,
  ...
  assign RequestInBuffer = (CharLoadBuffer | StringLoadBuffer) | BufferInitDicPointer;
  ...
  always @(posedge Clk)
      begin
        ...
        else if (StringLoadBuffer || StringLoadChar) 
          begin
            StringSize = 4'd1;
          end
  ...
  always @(posedge Clk)
      begin
        if (LoadInsertPointer) 
          ...
          begin
            InsertPointer = (InsertPointer+1)+(StringSize*(1/2));
          end
  ...
  always @(posedge Clk)
      begin
        if (SetJumpAddress) 
          begin
            JumpAddress = (1+(StringRAM[135:127]/2))+(DicPointer+StringRAM);
          end
