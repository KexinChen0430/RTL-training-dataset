...
module Registers(input  wire Clk,
  ...
  assign RequestInBuffer = StringLoadBuffer | (BufferInitDicPointer | CharLoadBuffer);
  ...
  always @(posedge Clk)
      begin
        if (LoadInsertPointer) 
          ...
          begin
            InsertPointer = 1+(((1/1<<1)*StringSize)+InsertPointer);
          end
  ...
  always @(posedge Clk)
      begin
        if (SetJumpAddress) 
          begin
            JumpAddress = DicPointer+(1+(StringRAM+(StringRAM[135:127]*(1/1<<1))));
          end
