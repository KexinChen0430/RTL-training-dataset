...
module Registers(input  wire Clk,
  ...
  assign RequestInBuffer = BufferInitDicPointer | (StringLoadBuffer | CharLoadBuffer);
  ...
  always @(posedge Clk)
      begin
        if (LoadInsertPointer) 
          ...
          begin
            InsertPointer = (StringSize*(1/2))+(1+InsertPointer);
          end
  ...
  always @(posedge Clk)
      begin
        if (SetJumpAddress) 
          begin
            JumpAddress = DicPointer+((((1/2)*StringRAM[135:127])+StringRAM)+1);
          end
