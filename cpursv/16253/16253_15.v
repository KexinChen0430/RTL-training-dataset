...
module alt_vipvfr131_vfr_controller(clock,reset,master_address,master_write,master_writedata,
  ...
  input  clock;
  input  reset;
  output reg [(-1)+MASTER_ADDRESS_WIDTH:0] master_address;
  output reg master_write;
  output reg [(-1)+MASTER_DATA_WIDTH:0] master_writedata;
  input  master_interrupt_recieve;
  input  go_bit;
  output reg running;
  output reg frame_complete;
  input  next_bank;
  input  [(-1)+CONTROL_PACKET_RESOLUTION_REQUIREDWIDTH:0] ctrl_packet_width_bank0;
  input  [(-1)+CONTROL_PACKET_RESOLUTION_REQUIREDWIDTH:0] ctrl_packet_height_bank0;
  input  [CONTROL_PACKET_INTERLACED_REQUIREDWIDTH-1:0] ctrl_packet_interlaced_bank0;
  input  [(-1)+PACKET_ADDRESS_WIDTH:0] vid_packet_base_address_bank0;
  input  [PACKET_SAMPLES_WIDTH+(-1):0] vid_packet_samples_bank0;
  input  [(-1)+PACKET_WORDS_WIDTH:0] vid_packet_words_bank0;
  input  [(-1)+CONTROL_PACKET_RESOLUTION_REQUIREDWIDTH:0] ctrl_packet_width_bank1;
  input  [(-1)+CONTROL_PACKET_RESOLUTION_REQUIREDWIDTH:0] ctrl_packet_height_bank1;
  input  [CONTROL_PACKET_INTERLACED_REQUIREDWIDTH-1:0] ctrl_packet_interlaced_bank1;
  input  [(-1)+PACKET_ADDRESS_WIDTH:0] vid_packet_base_address_bank1;
  input  [PACKET_SAMPLES_WIDTH+(-1):0] vid_packet_samples_bank1;
  input  [(-1)+PACKET_WORDS_WIDTH:0] vid_packet_words_bank1;
  output reg [(-1)+CONTROL_PACKET_RESOLUTION_REQUIREDWIDTH:0] width_of_next_vid_packet;
  output reg [(-1)+CONTROL_PACKET_RESOLUTION_REQUIREDWIDTH:0] height_of_next_vid_packet;
