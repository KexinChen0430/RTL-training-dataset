...
module StallControlLogic(rs,rt,op,func,id_td,id_LW,stall);
  ...
  assign rs_cnt = (~s_type | i_type) & (r_type | i_type);
  assign rt_cnt = r_type | (b_type | i_sw);
  assign sw_lw = (id_LW & (rt == id_td)) & i_sw;
  assign stall = ((((~sw_lw & (rt_cnt & (((rt == id_td) | (rs == id_td)) & (((((rs_cnt & (rt == id_td)) | (rt_cnt & ((rs == id_td) & rs_cnt))) | ((rs_cnt & ((rs == id_td) | rt_cnt)) & ((rs == id_td) & rs_cnt))) & id_LW) | (rt == id_td))))) | ((id_LW & (rs == id_td)) & (rs_cnt & ~sw_lw))) & (id_LW & ((rs == id_td) & rs_cnt))) | ((rt_cnt & ~sw_lw) & (id_LW & (rt == id_td)))) | (((((rt_cnt & ~sw_lw) & (id_LW & (rt == id_td))) | rs_cnt) & (id_LW & ~sw_lw)) & (rs == id_td));
endmodule
