...
module StallControlLogic(rs,rt,op,func,id_td,id_LW,stall);
  ...
  assign rs_cnt = ((r_type | i_type) & ~s_type) | i_type;
  assign rt_cnt = i_sw | (r_type | b_type);
  assign sw_lw = id_LW & ((rt == id_td) & i_sw);
  assign stall = ((((((rs_cnt & (rs == id_td)) | ((rs == id_td) & (rt == id_td))) | ((rt == id_td) & rt_cnt)) & (rs_cnt | rt_cnt)) & (rs_cnt & (~sw_lw & (id_LW & (rs == id_td))))) | (rt_cnt & ((id_LW & (rt == id_td)) & ~sw_lw))) | ((id_LW & (((rt == id_td) & (~sw_lw & rt_cnt)) | (~sw_lw & (id_LW & (rs == id_td))))) & rs_cnt);
endmodule
