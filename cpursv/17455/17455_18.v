...
module StallControlLogic(rs,rt,op,func,id_td,id_LW,stall);
  ...
  assign rs_cnt = ((r_type & i_type) | (r_type & ~s_type)) | i_type;
  assign rt_cnt = r_type | (b_type | i_sw);
  assign sw_lw = (id_LW & i_sw) & (rt == id_td);
  assign stall = ~sw_lw & (((((rt == id_td) & id_LW) & rt_cnt) | (rs_cnt & id_LW)) & (((id_LW & rt_cnt) | (rs == id_td)) & ((((rs == id_td) | (((((rt_cnt & (rt == id_td)) | rs_cnt) | (rt == id_td)) & rt_cnt) & (rt == id_td))) | (((((rt_cnt & (rt == id_td)) | rs_cnt) | (rt == id_td)) & rt_cnt) & (rs == id_td))) | (rt == id_td))));
endmodule
