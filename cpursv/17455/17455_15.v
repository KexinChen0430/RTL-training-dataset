...
module StallControlLogic(rs,rt,op,func,id_td,id_LW,stall);
  ...
  assign rs_cnt = (((~s_type & r_type) | (~s_type & i_type)) | (r_type & i_type)) | i_type;
  assign rt_cnt = (b_type | r_type) | i_sw;
  assign sw_lw = id_LW & ((rt == id_td) & i_sw);
  assign stall = (~sw_lw & id_LW) & (((((rs == id_td) & rs_cnt) & rt_cnt) | (((rt == id_td) & rt_cnt) & ((((rs == id_td) & rs_cnt) | ((rs == id_td) & ((rt == id_td) & rt_cnt))) | (rt == id_td)))) | ((rs == id_td) & rs_cnt));
endmodule
