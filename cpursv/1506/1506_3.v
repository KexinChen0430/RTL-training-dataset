  ...
  always @(posedge clk or negedge rst_n)
      begin
        if (!rst_n) 
          ...
          begin
            wr_data_dly <= wr_en;
            wr_detc <= !wr_data_dly & wr_en;
            clk_proc_dly <= clk_proc;
            clk_proc_pulse <= !clk_proc_dly & clk_proc;
          end
      end
  ...
  always @(*)
      begin
        case (state)
          ...
          ST_IDLE: begin
                if (wr_detc) next_state = ST_WRITE_0;
                else if (clk_proc_pulse && rd_en) next_state = ST_READ_0;
