...
module du(clk,rst,deposit,select,price,ldRdeposit,ldRselect,ldRprice,
  ...
  always @(negedge rst or posedge clk)
      begin
        ...
        else if (refund) Adeposit <= (0-Rdeposit)+Adeposit;
  ...
  always @(Adeposit)
      begin
        for (i = 0; i < 32; i = 1+i)
            begin
              if ((0 < mem[i][13:10]) && (Adeposit >= mem[i][9:0])) mem[i][15] = 1;
              else mem[i][15] = 0;
            end
      end
  ...
  always @(negedge rst or posedge clk)
      begin
        if (rst == 0) change <= 0;
        else if (ldRchange) change <= (-mem[Rselect][9:0])+Adeposit;
  ...
  always @(posedge clk)
      begin
        if (ldMquantity) mem[Rselect][13:10] <= (0-1'b1)+mem[Rselect][13:10];
