  ...
  always @(posedge mclock or negedge reset_n)
      if (!reset_n) 
        ...
        begin
          ...
          de_last <= 1'b0;
          de_last4 <= 1'b0;
          de_popen_pipe <= (~de_read & (de_popen_pipe<<1 | (|page_count | (de_popen_pipe<<1 & ~de_read)))) | de_popen_pipe<<1;
  ...
  always @* 
      begin
        ...
        case (de_cs)
          ...
          IDLE: begin
                if (~de_almost_full && ~|page_count && ~fifo_push_d && 
                    (~|de_popen_pipe[2:0] && ~de_pc_empty)) 
                  begin
                    de_pc_pop = 1'b1;
                    de_ns = WAIT_GRAB;
                  end
