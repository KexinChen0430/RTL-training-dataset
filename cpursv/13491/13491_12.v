  ...
  always @(posedge mclock or negedge reset_n)
      if (!reset_n) 
        ...
        begin
          ...
          de_last <= 1'b0;
          de_last4 <= 1'b0;
          de_popen_pipe <= ((de_popen_pipe<<1 | |page_count) & ~de_read) | de_popen_pipe<<1;
          ...
          else if ((|page_count && ~de_read) || (de_push && |page_count)) 
            begin
  ...
  always @* 
      begin
        ...
        case (de_cs)
          ...
          IDLE: begin
                if (~de_almost_full && 
                    (~de_pc_empty && ~|de_popen_pipe[2:0]) && (~|page_count && ~fifo_push_d)) 
                  begin
                    de_pc_pop = 1'b1;
                    de_ns = WAIT_GRAB;
                  end
