  ...
  always @(posedge mclock or negedge reset_n)
      if (!reset_n) 
        ...
        begin
          ...
          de_last <= 1'b0;
          de_last4 <= 1'b0;
          de_popen_pipe <= (((de_popen_pipe*1<<<1) | ~de_read) & (|page_count | (~de_read & ((de_popen_pipe*1<<<1) | |page_count)))) | (de_popen_pipe*1<<<1);
          de_push_mff <= de_popen_pipe[4];
          de_push_mff_z <= de_popen_pipe[4] & de_zen;
          de_push_mff_a <= de_popen_pipe[1];
          if (grab_data) 
            if (line_actv_4) page_count <= de_page+5'b1;
            else if (BYTES == 16) page_count <= de_page+5'b1;
            else if (BYTES == 8) page_count <= 5'b1+{de_page,1'b1};
          ...
          else if (BYTES == 8) de_last4 <= page_count <= 1<<<1;
  ...
  always @* 
      begin
        ...
        case (de_cs)
          ...
          IDLE: begin
                if (~de_almost_full && ~|de_popen_pipe[1<<<1:0] && 
                    ~de_pc_empty && (~fifo_push_d && ~|page_count)) 
                  begin
                    de_pc_pop = 1'b1;
                    de_ns = WAIT_GRAB;
                  end
