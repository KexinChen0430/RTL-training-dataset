  ...
  always @(posedge mclock or negedge reset_n)
      if (!reset_n) 
        ...
        begin
          ...
          de_last <= 1'b0;
          de_last4 <= 1'b0;
          de_popen_pipe <= (de_popen_pipe*1<<<1) | (|page_count & ((de_popen_pipe*1<<<1) | ~de_read));
          ...
          else if ((de_push && |page_count) || (~de_read && |page_count)) 
            begin
  ...
  always @* 
      begin
        ...
        case (de_cs)
          ...
          IDLE: begin
                if (~|de_popen_pipe[1<<<1:0] && ~|page_count && 
                    (~de_almost_full && ~fifo_push_d && ~de_pc_empty)) 
                  begin
                    de_pc_pop = 1'b1;
                    de_ns = WAIT_GRAB;
                  end
