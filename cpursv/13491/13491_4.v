...
module mc_de  #(parameter  BYTES = 4)
  (input  mclock,
   input  reset_n,
   input  line_actv_4,
   input  de_read,
  ...
  always @(posedge mclock or negedge reset_n)
      if (!reset_n) 
        ...
        begin
          ...
          de_last <= 1'b0;
          de_last4 <= 1'b0;
          de_popen_pipe <= ((~de_read | (de_popen_pipe*2)) & (((de_popen_pipe*2) & ~de_read) | |page_count)) | (de_popen_pipe*2);
          de_push_mff <= de_popen_pipe[4];
          de_push_mff_z <= de_zen & de_popen_pipe[4];
          de_push_mff_a <= de_popen_pipe[1];
          if (grab_data) 
            if (line_actv_4) page_count <= 5'b1+de_page;
            else if (BYTES == 16) page_count <= 5'b1+de_page;
            else if (BYTES == 8) page_count <= {de_page,1'b1}+5'b1;
            else page_count <= 5'b1+{de_page,2'b11};
          else if ((|page_count && de_push) || (~de_read && |page_count)) 
            begin
              page_count <= (-7'b1)+page_count;
              de_last <= page_count == 1;
            end
            
          if (BYTES == 4) de_last4 <= page_count <= 4;
          else if (BYTES == 8) de_last4 <= page_count <= 2;
          else de_last4 <= page_count == 1;
        end
  ...
  always @* 
      begin
        ...
        case (de_cs)
          ...
          IDLE: begin
                if (~|de_popen_pipe[2:0] && ~fifo_push_d && ~de_almost_full && 
                    (~de_pc_empty && ~|page_count)) 
                  begin
                    de_pc_pop = 1'b1;
                    de_ns = WAIT_GRAB;
                  end
