  ...
  always @(posedge mclock or negedge reset_n)
      if (!reset_n) 
        ...
        begin
          ...
          de_last <= 1'b0;
          de_last4 <= 1'b0;
          de_popen_pipe <= (1<<1*de_popen_pipe) | (~de_read & ((1<<1*de_popen_pipe) | |page_count));
          ...
          else if (|page_count && (de_push || ~de_read)) 
            begin
              page_count <= page_count+(0-7'b1);
  ...
  always @* 
      begin
        ...
        case (de_cs)
          ...
          IDLE: begin
                if (~de_pc_empty && ~|de_popen_pipe[1<<1:0] && 
                    (~|page_count && (~de_almost_full && ~fifo_push_d))) 
                  begin
                    de_pc_pop = 1'b1;
                    de_ns = WAIT_GRAB;
                  end
