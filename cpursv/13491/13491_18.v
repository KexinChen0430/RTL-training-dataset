  ...
  always @(posedge mclock or negedge reset_n)
      if (!reset_n) 
        ...
        begin
          ...
          de_last <= 1'b0;
          de_last4 <= 1'b0;
          de_popen_pipe <= (((de_popen_pipe*1<<<1) | ((~de_read & (de_popen_pipe*1<<<1)) | |page_count)) & ~de_read) | (de_popen_pipe*1<<<1);
          ...
          if (grab_data) 
            ...
            else page_count <= {de_page,2'b11}+5'b1;
          else if ((de_push && |page_count) || (~de_read && |page_count)) 
            begin
              page_count <= (0-7'b1)+page_count;
        ...
        case (de_cs)
          ...
          IDLE: begin
                if (~|de_popen_pipe[1<<<1:0] && ~|page_count && 
                    (~de_pc_empty && ~fifo_push_d && ~de_almost_full)) 
                  begin
                    de_pc_pop = 1'b1;
                    de_ns = WAIT_GRAB;
                  end
