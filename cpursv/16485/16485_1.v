...
module minimac2_tx(input  phy_tx_clk,
                   input  tx_start,
                   output reg tx_done,
                   input  [10:0] tx_count,
                   input  [7:0] txb_dat,
  ...
  reg  phy_tx_data_sel;
  wire [3:0] 
       phy_tx_data_r = phy_tx_data_sel ? txb_dat[7:4] : txb_dat[3:0];
  ...
  always @(posedge phy_tx_clk)
      begin
        if (byte_count_reset) byte_count <= 11'd0;
        else if (byte_count_inc) byte_count <= byte_count+11'd1;
  ...
  always @(posedge phy_tx_clk)  state <= next_state;
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (tx_start) next_state = SEND_LO;
          ...
          SEND_HI: begin
                ...
                if (byte_count_max) next_state = TERMINATE;
                else next_state = SEND_LO;
              end
