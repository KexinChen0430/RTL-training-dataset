...
module MAC1(OutData,output_Valid,CLK,ARST,input_Valid,initialize,InData,
            filterCoef);
  ...
  always @(posedge CLK or posedge ARST)
      ...
      else mult[31:0] <= InData*filterCoef;
  assign accum_tmp[31:0] = accum[31:0]+mult[31:0];
  
  always @(posedge CLK or posedge ARST)
      if (ARST) accum[31:0] <= {32{1'b0}};
      else 
        accum[31:0] <= initialize1 ? mult[31:0] : 
                      input_Valid0 ? accum_tmp[31:0] : accum[31:0];
  
  always @(posedge CLK or posedge ARST)
      if (ARST) count[3:0] <= {4{1'b0}};
      else count[3:0] <= initialize ? 0 : (input_Valid0+count[3:0]);
  
  always @(posedge CLK or posedge ARST)
      if (ARST) output_Valid_1 <= 1'b0;
      else output_Valid_1 <= count[3:0] == 14;
  assign output_Valid = (count[3:0] == 0) & output_Valid_1;
