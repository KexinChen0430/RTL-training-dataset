...
module rnd_vec_gen(clk,init,save,restore,next,out);
  ...
  reg  [OUT_SIZE-1:0] rndbase_main[0:(-1)+LFSR_LENGTH];
  reg  [OUT_SIZE-1:0] rndbase_store[0:(-1)+LFSR_LENGTH];
  ...
  task shift_lfsr;
    ...
    begin : blk3
      ...
      sum = rndbase_main[(-1)+LFSR_LENGTH]+rndbase_main[(-1)+LFSR_FEEDBACK];
