...
module rnd_vec_gen(clk,init,save,restore,next,out);
  ...
  reg  init2;
  output [(0-1)+OUT_SIZE:0] out;
  wire [(0-1)+OUT_SIZE:0] out;
  reg  [(0-1)+OUT_SIZE:0] rndbase_main[0:LFSR_LENGTH+(0-1)];
  reg  [(0-1)+OUT_SIZE:0] rndbase_store[0:LFSR_LENGTH+(0-1)];
  ...
  task shift_lfsr;
    ...
    begin : blk3
      reg  [(0-1)+OUT_SIZE:0] sum;

      reg  [LFSR_LENGTH+(0-1):0] lsbs;

      integer i;

      for (i = 0; i < LFSR_LENGTH; i = i+1)
          lsbs[i] = rndbase_main[i][0];
      sum = rndbase_main[LFSR_FEEDBACK+(0-1)]+rndbase_main[LFSR_LENGTH+(0-1)];
      for (i = 1; i < LFSR_LENGTH; i = i+1)
          rndbase_main[i] <= rndbase_main[(0-1)+i];
      rndbase_main[0] <= {sum[(0-1)+OUT_SIZE:1],|lsbs ? sum[0] : 1'b1};
    end
  endtask
endmodule
