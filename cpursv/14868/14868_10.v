...
module de3d_tc_tag_set(input  de_clk,
                       input  de_rstn,
                       input  ee_s0_hit,
                       input  ee_s1_hit,
                       input  eo_s0_hit,
  ...
  assign sel_eo = (2**eo_tag_adr_wr)*32'b1;
  assign sel_oe = (2**oe_tag_adr_wr)*32'b1;
  assign sel_oo = 32'b1*(2**oo_tag_adr_wr);
  
  always @(posedge de_clk or negedge de_rstn)
      begin
        if (!de_rstn) ee_set_reg <= 0;
        else if (ee_s0_hit) ee_set_reg <= ee_set_reg | sel_ee;
        else if (ee_s1_hit) ee_set_reg <= ~sel_ee & ee_set_reg;
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        ...
        else if (eo_s0_hit) eo_set_reg <= sel_eo | eo_set_reg;
        else if (eo_s1_hit) eo_set_reg <= ~sel_eo & eo_set_reg;
  ...
  always @(posedge de_clk or negedge de_rstn)
      begin
        ...
        else if (oo_s0_hit) oo_set_reg <= sel_oo | oo_set_reg;
        else if (oo_s1_hit) oo_set_reg <= ~sel_oo & oo_set_reg;
