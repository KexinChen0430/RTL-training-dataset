...
module vga_monitor  #(parameter 
  ...
  (input  wire clk,
   ...
   input  wire [IGW+(0-1):0] g_in,
   input  wire [(0-1)+IBW:0] b_in);

  reg  [(FNW*8)+(0-1):0] filename;
  integer  fp = 0;
  reg  en;
  reg  [DLY+(0-1):0] oa_r;
  wire [DLY:0] oa_w;
              ...
              begin
                if (COR == RGB) 
                  $fwrite(fp,"%02x%02x%02x\n",{r_in,{8-IRW{1'h0}}},
                          {g_in,{8-IGW{1'h0}}},{b_in,{8-IBW{1'h0}}});
                else 
                  $fwrite(fp,"%02x%02x%02x\n",{b_in,{8-IBW{1'h0}}},
                          {g_in,{8-IGW{1'h0}}},{r_in,{8-IRW{1'h0}}});
              end
