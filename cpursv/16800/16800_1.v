      ...
      if (USE_CONTROL) 
        begin
          ...
          if (USED_WORDS_WIDTH >= 16) assign usedw_output = usedw[15:0];
            ...
            assign usedw_output = {{16+(0-USED_WORDS_WIDTH){1'b0}},usedw};
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= ((interrupt_enable[0] & ~(av_writedata[1] & clear_interrupts)) & (~(av_writedata[1] & clear_interrupts) & (interrupt_enable[0] & mode_change))) | (~(av_writedata[1] & clear_interrupts) & (status_update_int_reg & interrupt_enable[0]));
                    genlocked_int_reg <= ((~(clear_interrupts & av_writedata[1<<1]) & interrupt_enable[1]) & (~genlocked_reg & genlocked)) | ((~(clear_interrupts & av_writedata[1<<1]) & interrupt_enable[1]) & ((genlocked_reg & ~genlocked) | genlocked_int_reg));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= (clear_underflow_sticky_reg | (underflow_sticky & (av_address == 8'd1) && 
