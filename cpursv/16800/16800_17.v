...
module alt_vipitc131_IS2Vid_control  #(parameter 
  ...
  (input  wire rst,
   ...
   input  wire [NO_OF_MODES_INT+(0-1):0] mode_match,
  ...
  generate
      if (USE_CONTROL) 
        begin
          ...
          reg  [NO_OF_MODES_INT+(0-1):0] is_mode_match;
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= ((mode_change & ~(av_writedata[1] & clear_interrupts)) & interrupt_enable[0]) | ((interrupt_enable[0] & status_update_int_reg) & ~(av_writedata[1] & clear_interrupts));
                    genlocked_int_reg <= ((genlocked & ~genlocked_reg) & (interrupt_enable[1] & ~(clear_interrupts & av_writedata[1<<1]))) | (((interrupt_enable[1] & ~(clear_interrupts & av_writedata[1<<1])) & (~genlocked | genlocked_int_reg)) & (genlocked_int_reg | genlocked_reg));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= ((av_address == 8'd1) && av_write && av_writedata[1<<1] | (underflow_sticky & clear_underflow_sticky_reg)) & underflow_sticky;
