              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (~(av_writedata[1] & clear_interrupts) & ((mode_change | ~(av_writedata[1] & clear_interrupts)) & (status_update_int_reg | mode_change))) & interrupt_enable[0];
                    genlocked_int_reg <= interrupt_enable[1] & (~(clear_interrupts & av_writedata[1<<1]) & (((((genlocked_reg | genlocked) & ~genlocked_reg) | ((genlocked_reg | genlocked) & genlocked_reg)) & ~genlocked) | ((genlocked_int_reg & (interrupt_enable[1] & ~(clear_interrupts & av_writedata[1<<1]))) | (genlocked & ~genlocked_reg))));
...
(av_writedata[1<<1] && av_write) & underflow_sticky);
