      ...
      if (USE_CONTROL) 
        begin
          ...
          if (USED_WORDS_WIDTH >= 16) assign usedw_output = usedw[15:0];
            ...
            assign usedw_output = {{(0-USED_WORDS_WIDTH)+16{1'b0}},usedw};
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (~(av_writedata[1] & clear_interrupts) & (status_update_int_reg & interrupt_enable[0])) | (~(av_writedata[1] & clear_interrupts) & (mode_change & interrupt_enable[0]));
                    genlocked_int_reg <= (((interrupt_enable[1] & (~(clear_interrupts & av_writedata[1<<<1]) & ((genlocked_reg | ~genlocked_reg) & ((((((genlocked_reg | genlocked) & (genlocked_reg | ~genlocked_reg)) & genlocked) & ~genlocked_reg) | ~genlocked) & genlocked)))) ^ (interrupt_enable[1] & (~(clear_interrupts & av_writedata[1<<<1]) & ((genlocked_reg | ~genlocked_reg) & ((~genlocked & genlocked_reg) | (genlocked_reg & ~genlocked_reg)))))) | (~(clear_interrupts & av_writedata[1<<<1]) & genlocked_int_reg)) & interrupt_enable[1];
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= ((av_address == 8'd1) && 
(av_write && av_writedata[1<<<1]) & underflow_sticky) | (clear_underflow_sticky_reg & underflow_sticky);
