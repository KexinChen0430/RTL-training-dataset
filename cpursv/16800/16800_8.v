              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (mode_change | (status_update_int_reg & ~(av_writedata[1] & clear_interrupts))) & (interrupt_enable[0] & ~(av_writedata[1] & clear_interrupts));
                    genlocked_int_reg <= (((~(av_writedata[1<<<1] & clear_interrupts) & interrupt_enable[1]) & ((((genlocked_int_reg | genlocked_reg) | (genlocked & ~genlocked_reg)) & (genlocked_int_reg | ~genlocked_reg)) & (genlocked_int_reg | genlocked))) | ((~(av_writedata[1<<<1] & clear_interrupts) & interrupt_enable[1]) & genlocked_reg)) & ((~genlocked | genlocked_int_reg) | (genlocked & ~genlocked_reg));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= (clear_underflow_sticky_reg | (av_write && (av_address == 8'd1) && av_writedata[1<<<1] & underflow_sticky)) & underflow_sticky;
