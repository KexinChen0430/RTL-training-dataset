              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (mode_change | ((~(clear_interrupts & av_writedata[1]) & status_update_int_reg) & interrupt_enable[0])) & (interrupt_enable[0] & ~(clear_interrupts & av_writedata[1]));
                    genlocked_int_reg <= ((~(av_writedata[1+1] & clear_interrupts) & interrupt_enable[1]) & ((genlocked_reg | (genlocked & ~genlocked_reg)) & ((genlocked & ~genlocked_reg) | (genlocked_int_reg | ~genlocked)))) | ((~(av_writedata[1+1] & clear_interrupts) & interrupt_enable[1]) & genlocked_int_reg);
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= (underflow_sticky & clear_underflow_sticky_reg) | (underflow_sticky & av_write && 
