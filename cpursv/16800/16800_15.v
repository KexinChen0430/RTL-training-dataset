...
module alt_vipitc131_IS2Vid_control  #(parameter 
       USE_CONTROL      = 1,
       NO_OF_MODES_INT  = 1,
       USED_WORDS_WIDTH = 15)
  (input  wire rst,
   ...
   input  wire [(0-1)+USED_WORDS_WIDTH:0] usedw,
  ...
  generate
      if (USE_CONTROL) 
        begin
          ...
          if (USED_WORDS_WIDTH >= 16) assign usedw_output = usedw[15:0];
          else assign usedw_output = {{16-USED_WORDS_WIDTH{1'b0}},usedw};
          if (NO_OF_MODES_INT >= 16) 
            begin
          ...
          assign av_readdata = (av_address == 8'd1) ? {{12{1'b0}},genlocked,underflow_sticky,1'b0,enable_resync} : 
                       (av_address == 8'd2) ? {{13{1'b0}},genlocked_int_reg,status_update_int_reg,1'b0} : 
                       (av_address == 8'd3) ? usedw_output : 
                       (av_address == 8'd4) ? is_mode_match_output : {{11{1'b0}},genlock_enable,interrupt_enable,enable_reg};
          assign av_waitrequest = av_write & ~(is_side_registers | av_write_ack);
          assign is_side_registers = av_address <= 8'd4;
          assign write_trigger = ~is_side_registers & av_write;
          assign write_trigger_ack = write_trigger_ack_reg;
          assign status_update_int = genlocked_int_reg | status_update_int_reg;
          assign clear_interrupts = (av_address == 8'd2) && av_write;
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= ((status_update_int_reg & ~(clear_interrupts & av_writedata[1])) & interrupt_enable[0]) | ((interrupt_enable[0] & mode_change) & ~(clear_interrupts & av_writedata[1]));
                    genlocked_int_reg <= ((((interrupt_enable[1] & ~(av_writedata[1<<1] & clear_interrupts)) & genlocked_reg) | (((genlocked_int_reg | genlocked) & (interrupt_enable[1] & (genlocked_int_reg | ~genlocked_reg))) & ~(av_writedata[1<<1] & clear_interrupts))) & ~genlocked) | ((((interrupt_enable[1] & ~(av_writedata[1<<1] & clear_interrupts)) & genlocked_reg) | (((genlocked_int_reg | genlocked) & (interrupt_enable[1] & (genlocked_int_reg | ~genlocked_reg))) & ~(av_writedata[1<<1] & clear_interrupts))) & ((genlocked & (genlocked_int_reg | ~genlocked_reg)) | genlocked_int_reg));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= underflow_sticky & (clear_underflow_sticky_reg | av_writedata[1<<1] && av_write && (av_address == 8'd1));
