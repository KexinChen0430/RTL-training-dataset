      ...
      if (USE_CONTROL) 
        begin
          ...
          if (NO_OF_MODES_INT >= 16) 
            ...
            begin
              assign is_mode_match_output = {{(-NO_OF_MODES_INT)+16{1'b0}},is_mode_match};
            end
          ...
          assign av_waitrequest = ~(av_write_ack | is_side_registers) & av_write;
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (~(clear_interrupts & av_writedata[1]) & (interrupt_enable[0] & status_update_int_reg)) | (interrupt_enable[0] & ((~(clear_interrupts & av_writedata[1]) & mode_change) & interrupt_enable[0]));
                    genlocked_int_reg <= ((interrupt_enable[1] & ((genlocked & ~(av_writedata[1<<1] & clear_interrupts)) | (genlocked_int_reg & ~(av_writedata[1<<1] & clear_interrupts)))) & (genlocked_int_reg | ~genlocked_reg)) | ((~(av_writedata[1<<1] & clear_interrupts) & (genlocked_reg & interrupt_enable[1])) & ~genlocked);
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= ((av_write && 
((av_address == 8'd1) && av_writedata[1<<1]) | clear_underflow_sticky_reg) & (underflow_sticky | av_write && 
((av_address == 8'd1) && av_writedata[1<<1]))) & underflow_sticky;
