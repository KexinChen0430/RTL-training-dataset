              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (~(clear_interrupts & av_writedata[1]) & interrupt_enable[0]) & (status_update_int_reg | (mode_change & ~(clear_interrupts & av_writedata[1])));
                    genlocked_int_reg <= ~(av_writedata[1<<1] & clear_interrupts) & ((interrupt_enable[1] & ((genlocked_reg & ~(av_writedata[1<<1] & clear_interrupts)) ^ (~(av_writedata[1<<1] & clear_interrupts) & genlocked))) | (interrupt_enable[1] & genlocked_int_reg));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= (underflow_sticky & av_write && 
((av_address == 8'd1) && av_writedata[1<<1])) | (clear_underflow_sticky_reg & underflow_sticky);
