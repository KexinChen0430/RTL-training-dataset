...
module alt_vipitc131_IS2Vid_control  #(parameter 
  ...
  (input  wire rst,
   ...
   input  wire [USED_WORDS_WIDTH+(0-1):0] usedw,
      ...
      if (USE_CONTROL) 
        begin
          ...
          if (NO_OF_MODES_INT >= 16) 
            ...
            begin
              assign is_mode_match_output = {{(0-NO_OF_MODES_INT)+16{1'b0}},is_mode_match};
            end
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (interrupt_enable[0] & ~(clear_interrupts & av_writedata[1])) & (status_update_int_reg | (mode_change & ~(clear_interrupts & av_writedata[1])));
                    genlocked_int_reg <= (~(av_writedata[1<<<1] & clear_interrupts) & (interrupt_enable[1] & genlocked_int_reg)) | ((interrupt_enable[1] & ~(av_writedata[1<<<1] & clear_interrupts)) & (((~genlocked_reg | genlocked_reg) & (genlocked_reg | genlocked)) & ((genlocked_int_reg | ~genlocked) | (~genlocked_reg & genlocked))));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= (underflow_sticky & clear_underflow_sticky_reg) | ((av_address == 8'd1) && 
(av_write && av_writedata[1<<<1]) & underflow_sticky);
