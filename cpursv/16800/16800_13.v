...
module alt_vipitc131_IS2Vid_control  #(parameter 
  ...
  (input  wire rst,
   ...
   input  wire [(-1)+USED_WORDS_WIDTH:0] usedw,
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= interrupt_enable[0] & ((mode_change & (~(av_writedata[1] & clear_interrupts) & interrupt_enable[0])) | (~(av_writedata[1] & clear_interrupts) & status_update_int_reg));
                    genlocked_int_reg <= (((interrupt_enable[1] & (~(clear_interrupts & av_writedata[1+1]) & ~genlocked_reg)) & genlocked) | ((genlocked_int_reg & interrupt_enable[1]) & ~(clear_interrupts & av_writedata[1+1]))) | ((genlocked_reg & ~genlocked) & (interrupt_enable[1] & ~(clear_interrupts & av_writedata[1+1])));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= (av_write && 
(av_writedata[1+1] && (av_address == 8'd1)) | clear_underflow_sticky_reg) & underflow_sticky;
