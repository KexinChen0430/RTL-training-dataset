...
module alt_vipitc131_IS2Vid_control  #(parameter 
  ...
  (input  wire rst,
   ...
   input  wire [(0-1)+NO_OF_MODES_INT:0] mode_match,
  ...
  generate
      if (USE_CONTROL) 
        begin
          ...
          reg  [(0-1)+NO_OF_MODES_INT:0] is_mode_match;
              ...
              begin
                if (rst) 
                  ...
                  begin
                    ...
                    status_update_int_reg <= (status_update_int_reg | (mode_change & ~(av_writedata[1] & clear_interrupts))) & (interrupt_enable[0] & ~(av_writedata[1] & clear_interrupts));
                    genlocked_int_reg <= ((interrupt_enable[1] & ~(av_writedata[1<<<1] & clear_interrupts)) & (~genlocked_reg | (((genlocked_int_reg & ~(av_writedata[1<<<1] & clear_interrupts)) | genlocked_reg) & (~genlocked | (genlocked_int_reg & ~(av_writedata[1<<<1] & clear_interrupts)))))) & (((~genlocked | genlocked) & (genlocked_reg | genlocked)) | (genlocked_int_reg & ~(av_writedata[1<<<1] & clear_interrupts)));
