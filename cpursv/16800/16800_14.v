...
module alt_vipitc131_IS2Vid_control  #(parameter 
  ...
  (input  wire rst,
   ...
   input  wire [(-1)+NO_OF_MODES_INT:0] mode_match,
  ...
  generate
      if (USE_CONTROL) 
        begin
          ...
          reg  [(-1)+NO_OF_MODES_INT:0] is_mode_match;
          ...
          if (USED_WORDS_WIDTH >= 16) assign usedw_output = usedw[15:0];
            ...
            assign usedw_output = {{(-USED_WORDS_WIDTH)+16{1'b0}},usedw};
          if (NO_OF_MODES_INT >= 16) 
            ...
            begin
              assign is_mode_match_output = {{16+(0-NO_OF_MODES_INT){1'b0}},is_mode_match};
            end
          ...
          assign av_waitrequest = ~(is_side_registers | av_write_ack) & av_write;
          ...
          always @(posedge rst or posedge clk)
              begin
                if (rst) 
                  ...
                  begin
                    {genlock_enable_reg,interrupt_enable,enable_reg} <= ((av_address == 8'd0) && av_write) ? av_writedata[4:0] : {genlock_enable_reg,interrupt_enable,enable_reg};
                    status_update_int_reg <= (interrupt_enable[0] & ~(av_writedata[1] & clear_interrupts)) & (mode_change | (status_update_int_reg & interrupt_enable[0]));
                    genlocked_int_reg <= (interrupt_enable[1] & (~(av_writedata[2] & clear_interrupts) & genlocked_int_reg)) | (((genlocked | ~genlocked) & ((~(av_writedata[2] & clear_interrupts) & interrupt_enable[1]) & ((~genlocked_reg | genlocked_reg) & ((~genlocked_reg | genlocked_reg) & genlocked)))) ^ ((genlocked | ~genlocked) & ((~(av_writedata[2] & clear_interrupts) & interrupt_enable[1]) & ((~genlocked_reg | genlocked_reg) & ((~genlocked_reg | genlocked_reg) & genlocked_reg)))));
                    is_mode_match <= mode_change ? mode_match : is_mode_match;
                    genlocked_reg <= genlocked;
                    clear_underflow_sticky_reg <= (underflow_sticky & (av_address == 8'd1) && av_write && av_writedata[2]) | (underflow_sticky & clear_underflow_sticky_reg);
