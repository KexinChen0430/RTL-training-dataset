...
module arSRLFIFOD(CLK,RST_N,ENQ,DEQ,FULL_N,EMPTY_N,D_IN,D_OUT,CLR);
  ...
  reg  [(0-1)+width:0] dat[depth+(0-1):0];
  ...
  always @(posedge CLK)
      begin
        if (!RST_N || CLR) 
          ...
          begin
            if (!ENQ && sdx) pos <= (0-1)+pos;
              
            if (ENQ && !sdx) pos <= pos+1;
              
            sempty <= !ENQ && ((!ENQ && (sdx && (pos == 1))) || (pos == 0));
            sfull <= ((ENQ && !sdx) || (pos == (depth+(0-1)))) && 
                    (((pos == (depth-(1+1))) || (pos == (depth+(0-1)))) && 
((pos == (depth-(1+1))) || !sdx) && !sdx);
            if (sdx) 
              begin
                dempty <= 1'b0;
              end
  ...
  assign sdx = !sempty && 
               ((!sempty && !dempty && DEQ) || 
(dempty || (!sempty && dempty && (!dempty && DEQ))));
  assign FULL_N = !sfull;
