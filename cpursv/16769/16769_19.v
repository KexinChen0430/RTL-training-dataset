  ...
  always @(posedge CLK)
      begin
        if (NOTE < 12) 
          ...
          begin
            ADDER <= ADDER_tbl[NOTE+(0-12)]>>9;
          end
        else if (NOTE < 36) 
          begin
            ADDER <= ADDER_tbl[(0-24)+NOTE]>>8;
          end
        else if (NOTE < 48) 
          begin
          ...
          begin
            ADDER <= ADDER_tbl[(-84)+NOTE]>>3;
          end
        else if (NOTE < 108) 
          begin
            ADDER <= ADDER_tbl[(-96)+NOTE]>>2;
          end
        else if (NOTE < 120) 
          begin
            ADDER <= ADDER_tbl[NOTE+(0-108)]>>1;
          end
        else 
          begin
            ADDER <= ADDER_tbl[(-120)+NOTE];
          end
      end
endmodule
