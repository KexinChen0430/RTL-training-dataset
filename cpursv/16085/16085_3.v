...
module SPIFSM  #(parameter  SPPRWidth = 4, SPRWidth = 4, DataWidth = 8)
  (input  Reset_n_i,
   ...
   input  Start_i,
   output reg Done_o,
   output reg [DataWidth+(0-1):0] Byte0_o,
   output reg [DataWidth+(0-1):0] Byte1_o,
   input  SPI_Transmission_i,
   output reg SPI_Write_o,
   output reg SPI_ReadNext_o,
   output reg [DataWidth+(0-1):0] SPI_Data_o,
   input  [DataWidth+(0-1):0] SPI_Data_i,
        ...
        if (!Reset_n_i) 
          ...
          begin
            if (SPI_FSM_TimerPreset) 
              ...
              begin
                SPI_FSM_Timer <= (0-1'b1)+SPI_FSM_Timer;
              end
