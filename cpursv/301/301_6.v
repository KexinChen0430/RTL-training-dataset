...
module fillbox_act(input  wire [27:0] vram,
  ...
  reg   done_ff1 = 0;
  reg   done_ff2 = 0;
  wire [27:0]  awaddr_tmp = {17'b0,offset}+vram_y;

  assign done = ~done_ff2 && done_ff1;
  
  always @(posedge clk)
      begin
        done_ff2 <= done_ff1;
        done_ff1 <= (expected_count == real_count) && 
                   ((width_work == 10'd0) && (height_work == 10'd0));
  ...
  always @(posedge clk)
      begin
        ...
        else if (bready) real_count <= real_count+10'd1;
        ...
        case (state_y)
          ...
          WAIT: begin
                if (end_x) 
                  begin
                    state_y <= START;
                    height_work <= (-10'd1)+height_work;
                    vram_y <= SCREEN_WIDTH+vram_y;
                    start_x <= 'b0;
                  end
