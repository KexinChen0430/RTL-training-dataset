...
module alt_vipitc131_common_sample_counter  #(parameter 
  ...
  (input  wire rst,
   ...
   output wire [(-1)+LOG2_NUMBER_OF_COLOUR_PLANES:0] sample_ticks);
  ...
  generate
      if (NUMBER_OF_COLOUR_PLANES == 1) 
        ...
        begin
          reg  [(-1)+LOG2_NUMBER_OF_COLOUR_PLANES:0] count_valids;
          ...
          always @(posedge rst or posedge clk)
              begin
                if (rst) 
                  ...
                  begin
                    if (sclr) 
                      count_valids <= {{(-1)+LOG2_NUMBER_OF_COLOUR_PLANES{1'b0}},count_cycle};
                    else 
