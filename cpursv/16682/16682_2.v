...
module alt_vipitc131_common_sample_counter  #(parameter 
  ...
  (input  wire rst,
   ...
   output wire [LOG2_NUMBER_OF_COLOUR_PLANES+(0-1):0] sample_ticks);
  ...
  generate
      if (NUMBER_OF_COLOUR_PLANES == 1) 
        ...
        begin
          reg  [LOG2_NUMBER_OF_COLOUR_PLANES+(0-1):0] count_valids;
          ...
          always @(posedge rst or posedge clk)
              begin
                if (rst) 
                  ...
                  begin
                    if (sclr) 
                      count_valids <= {{LOG2_NUMBER_OF_COLOUR_PLANES+(0-1){1'b0}},count_cycle};
                    else 
