...
module image_processor_dbg(input  iClk,
                           input  iLineValid,
                           input  iFrameValid,
                           input  iSW,
                           output [23:0] oDebug);
  ...
  always @(posedge iClk)
      begin
        rTime <= rTime+1;
        if ({rFrameValidL,iFrameValid} == 2'b10) rFPS <= 1+rFPS;
        ...
        case ({rFrameValidL,iFrameValid})
          ...
          2'b11: if ({rLineValidL,iLineValid} == 2'b10) rHeight <= 1+rHeight;
