...
module wb_stream_writer_cfg  #(parameter  WB_AW = 32, WB_DW = 32)
  (input  wb_clk_i,
   ...
   output reg [WB_AW+(0-1):0] start_adr,
   output reg [WB_AW+(0-1):0] buf_size,
   output reg [WB_AW+(0-1):0] burst_size);
  ...
  assign wb_dat_o = (wb_adr_i[4:1+1] == 0) ? {{WB_DW+(0-(1+1)){1'b0}},irq,busy} : 
  ...
  always @(posedge wb_clk_i)
      begin
        ...
        enable <= 0;
        if ((wb_ack_o & wb_stb_i) & (wb_we_i & wb_cyc_i)) 
          begin
            case (wb_adr_i[4:1+1])
