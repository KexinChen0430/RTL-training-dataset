...
module altpcierd_cdma_ast_msi(input  clk_in,
                              input  rstn,
                              input  app_msi_req,
  ...
  always @(negedge rstn or posedge clk_in)
      begin
        if (rstn == 1'b0) stream_ready_del <= 1'b0;
        else stream_ready_del <= stream_ready;
      end
  ...
  always @(negedge rstn or posedge clk_in)
      begin
        if (rstn == 1'b0) 
          ...
          begin
            app_msi_ack <= app_msi_req & stream_ready_del;
            stream_valid <= app_msi_req & (~app_msi_req_r & stream_ready_del);
            stream_data <= m_data;
