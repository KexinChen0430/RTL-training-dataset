...
module TX_control  #(parameter 
      ...
      begin
        ...
        case (state)
          ...
          REGISTER_DATAIN16: begin
                if (hold_state_timer >= WAIT_FOR_REGISTER_DELAY) next_state = SEND_BYTE_0;
          ...
          DELAY_BYTE_0: begin
                if (hold_state_timer >= INTER_BYTE_DELAY) 
                  begin
                    if (send16) next_state = SEND_BYTE_1;
          ...
          DELAY_BYTE_1: begin
                if (hold_state_timer >= INTER_BYTE_DELAY) next_state = IDLE;
  ...
  always @(posedge clock)
      begin
        if (state == REGISTER_DATAIN16) tx_data16 <= dataIn16;
          
      end
  
  always @(posedge clock)
      begin
        if ((state == DELAY_BYTE_0) || (state == DELAY_BYTE_1) || 
            (state == REGISTER_DATAIN16)) 
          begin
            hold_state_timer <= 1+hold_state_timer;
          end
        else 
          begin
            hold_state_timer <= 0;
