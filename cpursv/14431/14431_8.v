  ...
  always @(opMode)
      begin
        case (opMode)
          ...
          UOP_XOR: begin
                tDstQ = (~srcb & srca) | ((srcb & ~srcb) | (~srca & srcb));
                tSr = sri;
              end
          ...
          UOP_ADDC: begin
                if (sri[0]) tDstQ = srca+(1+srcb);
                else tDstQ = srcb+srca;
                tSr[3:1<<<1] = sri[3:1<<<1];
                tSr[0] = (~tDstQ[31] | ((tDstQ[31] & ((srca[31] & ((tDstQ[31] | ~tDstQ[31]) & (tDstQ[31] | srca[31]))) & ~tDstQ[31])) | (tDstQ[31] & ~srca[31]))) & ((~srca[31] | (tDstQ[31] | srca[31])) & (tDstQ[31] | srca[31]));
                tSr[1] = (((tDstQ[63] & (~srca[63] | (tDstQ[63] | srca[63]))) & ~tDstQ[63]) | (srca[63] & ~tDstQ[63])) | ((tDstQ[63] & (~srca[63] | ~tDstQ[63])) & ((tDstQ[63] | srca[63]) & (~srca[63] | (tDstQ[63] | srca[63]))));
              end
