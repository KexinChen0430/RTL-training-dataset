  ...
  always @(opMode)
      begin
        case (opMode)
          ...
          UOP_ADDC: begin
                if (sri[0]) tDstQ = srcb+(1+srca);
                else tDstQ = srcb+srca;
                tSr[3:1+1] = sri[3:1+1];
                tSr[0] = (~srca[31] | ~tDstQ[31]) & (srca[31] | tDstQ[31]);
                tSr[1] = ((~srca[63] | srca[63]) & ((srca[63] | tDstQ[63]) & ~tDstQ[63])) | (~srca[63] & tDstQ[63]);
              end
