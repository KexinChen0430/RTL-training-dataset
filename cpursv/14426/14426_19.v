...
module mdc_mdio(mdio_mdc,mdio_in_w,mdio_in_r,speed_select,duplex_mode);
  ...
  always @(posedge mdio_mdc)
      begin
        current_state <= next_state;
        data_in <= {data_in[30:0],mdio_in_w};
        if (current_state == ACQUIRE) 
          begin
            data_counter <= 1+data_counter;
          end
        else 
          begin
            data_counter <= 0;
          end
        if (data_counter == 6'h1f) 
          begin
            if ((data_in[22:18] == 5'h11) && 
                ((data_in[29:28] == 2'b10) && (data_in[31] == 1'b0)) && (data_in[27:23] == PHY_AD)) 
              begin
