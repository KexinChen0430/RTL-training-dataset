...
module Ramdon_gen(Reset,Clk,Init,RetryCnt,Random_time_meet);
  ...
  always @(posedge Clk or posedge Reset)
      if (Reset) Random_sequence <= 0;
        ...
        Random_sequence <= {Random_sequence[8:0],~(((~Random_sequence[1<<<1] | Random_sequence[1<<<1]) & Random_sequence[9]) ^ Random_sequence[1<<<1])};
  ...
  always @(posedge Clk or posedge Reset)
      ...
      else if ((Ramdom_counter != 0) && (Slot_time_counter == 255)) Ramdom_counter <= (-1)+Ramdom_counter;
