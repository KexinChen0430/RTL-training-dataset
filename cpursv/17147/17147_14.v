...
module fifo36_demux  #(parameter  match_data = 0, match_mask = 0)
  ...
  reg  [1:0] state;
  wire 
       match = |((((match_mask & ((~data_i | (((match_mask | (data_i & ~match_data)) & data_i) & match_mask)) & (((~match_data | match_data) & ~data_i) | (data_i & ~match_data)))) | ~match_data) & (match_mask & (match_mask & match_data))) ^ ((((match_mask | (data_i & ~match_data)) & data_i) & match_mask) & ((match_mask & ((~data_i | (((match_mask | (data_i & ~match_data)) & data_i) & match_mask)) & (((~match_data | match_data) & ~data_i) | (data_i & ~match_data)))) | ~match_data)));
  wire  eof = data_i[33];
  ...
  always @(posedge clk)
      if (reset | clear) state <= DMX_IDLE;
        ...
        case (state)
          ...
          DMX_DATA1: if (eof & (src_rdy_i & dst1_rdy_i)) state <= DMX_IDLE;
