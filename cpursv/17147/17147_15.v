...
module fifo36_demux  #(parameter  match_data = 0, match_mask = 0)
  ...
  reg  [1:0] state;
  wire 
       match = |((((~match_data | (match_data & match_mask)) | ~(match_mask & data_i)) & (((data_i | match_data) & (match_mask | (~data_i & match_data))) & match_mask)) & (~(match_mask & data_i) | (data_i & ~(match_data & match_mask))));
  wire  eof = data_i[33];
