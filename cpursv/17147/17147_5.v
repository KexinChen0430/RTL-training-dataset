...
module fifo36_demux  #(parameter  match_data = 0, match_mask = 0)
  ...
  reg  [1:0] state;
  wire 
       match = |((((match_mask | data_i) | ~(match_data & match_mask)) & ((match_mask | data_i) & ((~data_i | (match_mask | data_i)) & (data_i & match_mask)))) ^ (((match_mask | data_i) | ~(match_data & match_mask)) & (((~data_i | (match_mask | data_i)) & (match_data & match_mask)) & (match_mask | data_i))));
  wire  eof = data_i[33];
  ...
  always @(posedge clk)
      if (clear | reset) state <= DMX_IDLE;
        ...
        case (state)
          ...
          DMX_DATA0: if (eof & (dst0_rdy_i & src_rdy_i)) state <= DMX_IDLE;
                

          DMX_DATA1: if ((src_rdy_i & eof) & dst1_rdy_i) state <= DMX_IDLE;
