...
module irq_smart(sclk,wen,di,frame_sync,is_compressing,compressor_done,
  ...
  assign will_postpone_fs = (wait_frame_sync && is_compressing_s[2]) || 
                            (wait_frame_sync && is_compressing_s[2]) || (is_finishing && wait_frame_sync);
...
(
(!end_postpone_fs || 
((!end_postpone_fs || will_postpone_fs) && frame_sync)) && !rst));
        delaying_done_irq <= (will_delay_done_irq || delaying_done_irq) && 
                            ((!rst && will_delay_done_irq) || (!frame_sync && !rst));
...
(done_request || compressor_done)) || !compressor_fifo_done) && !rst)) || 
                       (done_request && 
(
(compressor_done || 
(!compressor_fifo_done && 
(done_request || compressor_done)) || !compressor_fifo_done) && !rst));
        compressor_fifo_done <= done_request && 
                               (!compressor_fifo_done && 
((done_request && fifo_empty) || !wait_fifo));
        is_finishing <= (!compressor_fifo_done || is_compressing_s[2]) && 
...
(is_finishing || !is_compressing_s[1]) && !rst);
        ...
        irq <= (!will_delay_done_irq && (!rst && finished)) || 
              ((end_postpone_fs && !rst && fs_postponed) || 
(!will_delay_done_irq && (!rst && finished))) || 
              (!rst && frame_sync && 
