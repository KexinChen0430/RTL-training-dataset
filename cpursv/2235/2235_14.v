...
module irq_smart(sclk,wen,di,frame_sync,is_compressing,compressor_done,
  ...
  assign will_postpone_fs = (wait_frame_sync && is_finishing) || 
                            (wait_frame_sync && is_finishing) || (is_compressing_s[2] && wait_frame_sync);
  assign finished = !is_finishing && was_finishing;
  assign end_postpone_fs = (delaying_done_irq && frame_sync) || finished || 
                           frame_sync;
  assign will_delay_done_irq = wait_frame_sync && (!fs_postponed && finished);
...
(!end_postpone_fs && (will_postpone_fs && frame_sync)) || (will_postpone_fs && frame_sync));
        delaying_done_irq <= (will_delay_done_irq && !rst) || 
                            (delaying_done_irq && 
((!frame_sync || will_delay_done_irq) && !rst));
        is_compressing_s[2:0] <= {is_compressing_s[1:0],is_compressing};
        done_request <= (compressor_done && !rst) || 
                       (!compressor_fifo_done && 
((compressor_done && !rst) || (!rst && done_request)));
        compressor_fifo_done <= done_request && !compressor_fifo_done && 
                               (fifo_empty || (!wait_fifo && done_request));
        is_finishing <= (!compressor_fifo_done && (is_finishing && !rst)) || 
                       (is_compressing_s[2] && 
((!is_compressing_s[1] || !compressor_fifo_done) && 
(is_finishing || !is_compressing_s[1]) && !rst));
        was_finishing <= is_finishing;
        irq <= 
(!rst && 
(
(
((frame_sync && !will_postpone_fs) || fs_postponed || 
(fs_postponed && 
(
(delaying_done_irq || (frame_sync && !will_postpone_fs)) && frame_sync)) || 
(
(delaying_done_irq || (frame_sync && !will_postpone_fs)) && frame_sync)) && 
((frame_sync && !will_postpone_fs) || end_postpone_fs)) || (delaying_done_irq && frame_sync))) || (!rst && (fs_postponed && end_postpone_fs)) || 
              (!rst && 
