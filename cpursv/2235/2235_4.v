...
((!end_postpone_fs || will_postpone_fs) && frame_sync) || !end_postpone_fs) && 
                       (((!rst && fs_postponed) || frame_sync) && 
((will_postpone_fs && !rst) || (!rst && fs_postponed)));
        delaying_done_irq <= 
...
(compressor_done || 
(!compressor_fifo_done && (!wait_fifo && done_request)) || done_request) && !rst) || (compressor_done && !rst);
        compressor_fifo_done <= !compressor_fifo_done && done_request && 
                               (!wait_fifo || fifo_empty);
        is_finishing <= 
(is_finishing || 
...
(is_compressing_s[2] && 
(!is_compressing_s[1] || 
(fifo_empty && !compressor_fifo_done) || !compressor_fifo_done)) || (!compressor_fifo_done && !wait_fifo) || 
!compressor_fifo_done) && !rst;
        was_finishing <= is_finishing;
        irq <= 
(
(frame_sync || 
((finished && !will_delay_done_irq) || 
(frame_sync && !will_postpone_fs))) && !rst && 
((finished && !will_delay_done_irq) || 
(frame_sync && !will_postpone_fs) || delaying_done_irq)) || 
              (!rst && 
(fs_postponed && 
(
(
...
((frame_sync && !will_postpone_fs) || end_postpone_fs) || frame_sync)) || end_postpone_fs)));
      end
endmodule
