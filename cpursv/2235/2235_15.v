...
module irq_smart(sclk,wen,di,frame_sync,is_compressing,compressor_done,
  ...
  assign will_postpone_fs = wait_frame_sync && 
                            (is_compressing_s[2] || is_finishing);
...
(fs_postponed && !end_postpone_fs && !rst)) && (frame_sync || (fs_postponed && !end_postpone_fs));
        delaying_done_irq <= 
(will_delay_done_irq || 
((will_delay_done_irq || delaying_done_irq) && 
!frame_sync)) && !rst;
        is_compressing_s[2:0] <= {is_compressing_s[1:0],is_compressing};
        done_request <= (!rst && !compressor_fifo_done && done_request) || 
                       (compressor_done && !rst);
        compressor_fifo_done <= (!wait_fifo || (done_request && fifo_empty)) && 
                               (done_request && !compressor_fifo_done);
        is_finishing <= !rst && 
                       (
(is_compressing_s[2] && (!is_compressing_s[1] && !rst)) || is_finishing) && 
                       (
(is_compressing_s[2] && (!is_compressing_s[1] && !rst)) || !compressor_fifo_done);
...
(
(
(fs_postponed || delaying_done_irq || !will_postpone_fs) && frame_sync) || fs_postponed) && !rst)) || 
...
(
(
(fs_postponed || delaying_done_irq || !will_postpone_fs) && frame_sync) || fs_postponed) && !rst)) || (end_postpone_fs && fs_postponed && !rst) || 
(
(
((frame_sync && !will_postpone_fs) || delaying_done_irq) && (!rst && frame_sync)) || (!will_delay_done_irq && !rst))) && finished) || (end_postpone_fs && fs_postponed && !rst) || 
              (!rst && 
(
(!will_postpone_fs && 
((end_postpone_fs && fs_postponed) || frame_sync)) || 
(((end_postpone_fs && fs_postponed) || frame_sync) && 
delaying_done_irq)));
      end
endmodule
