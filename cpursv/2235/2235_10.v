...
module irq_smart(sclk,wen,di,frame_sync,is_compressing,compressor_done,
  ...
  assign end_postpone_fs = finished || 
                           (frame_sync && 
(
(fs_postponed || 
(delaying_done_irq || !will_postpone_fs)) && (frame_sync || fs_postponed) && delaying_done_irq)) || frame_sync;
...
(
(!wait_fifo && (done_request && !compressor_fifo_done)) || fifo_empty) && (done_request && !compressor_fifo_done);
        is_finishing <= 
((!compressor_fifo_done && is_finishing) || 
!is_compressing_s[1]) && !rst && 
                       ((!compressor_fifo_done && is_finishing) || 
is_compressing_s[2]);
        was_finishing <= is_finishing;
        irq <= (!will_delay_done_irq && finished && !rst) || 
              (!rst && 
(
(frame_sync && 
(
...
(
((frame_sync && !will_postpone_fs) || end_postpone_fs) && 
(
(fs_postponed || 
(delaying_done_irq || !will_postpone_fs)) && (frame_sync || fs_postponed)))));
      end
endmodule
