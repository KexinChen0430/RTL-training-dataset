...
(frame_sync || fs_postponed) && will_postpone_fs));
        delaying_done_irq <= (!frame_sync || will_delay_done_irq) && !rst && 
                            (delaying_done_irq || will_delay_done_irq);
        is_compressing_s[2:0] <= {is_compressing_s[1:0],is_compressing};
        done_request <= 
((!compressor_fifo_done || compressor_done) && 
(!rst && done_request)) || 
                       (compressor_done && !rst && 
(!compressor_fifo_done || compressor_done));
        compressor_fifo_done <= 
(!wait_fifo && done_request && 
...
(
(is_compressing_s[2] || 
(fifo_empty && !compressor_fifo_done) || !compressor_fifo_done) && !is_compressing_s[1]) || (fifo_empty && !compressor_fifo_done) || 
!compressor_fifo_done) && 
                       (!rst && 
(is_finishing || 
(!is_compressing_s[1] && is_compressing_s[2] && 
is_finishing) || (!is_compressing_s[1] && is_compressing_s[2])));
        was_finishing <= is_finishing;
        irq <= (delaying_done_irq && (!rst && frame_sync)) || 
              (!rst && (frame_sync && !will_postpone_fs)) || 
              (
(
(
((!will_delay_done_irq && finished) || end_postpone_fs) && fs_postponed) || (!will_delay_done_irq && finished)) && !rst);
      end
endmodule
