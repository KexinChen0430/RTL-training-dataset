...
module irq_smart(sclk,wen,di,frame_sync,is_compressing,compressor_done,
  ...
  assign end_postpone_fs = (frame_sync && !will_postpone_fs) || finished || 
                           frame_sync;
...
(will_postpone_fs || (!end_postpone_fs && fs_postponed)) && !rst)) || 
                       (
(will_postpone_fs || (!end_postpone_fs && fs_postponed)) && !rst && frame_sync);
        delaying_done_irq <= 
(will_delay_done_irq || 
...
(done_request && compressor_done) || compressor_done) && !rst);
        compressor_fifo_done <= 
(!compressor_fifo_done && (done_request && !wait_fifo)) || 
                               (!compressor_fifo_done && (fifo_empty && done_request));
        is_finishing <= 
((!rst && is_compressing_s[2]) || 
(is_finishing && (!rst && !compressor_fifo_done))) && 
                       ((is_finishing && (!rst && !compressor_fifo_done)) || 
!is_compressing_s[1]);
...
(
((!rst && fs_postponed) || 
((frame_sync && !will_postpone_fs) || delaying_done_irq)) && ((!rst && frame_sync) || (!rst && fs_postponed)) && 
(frame_sync && delaying_done_irq)) || 
(
((frame_sync && delaying_done_irq) || end_postpone_fs || 
frame_sync) && (end_postpone_fs || !will_postpone_fs) && 
(
((!rst && fs_postponed) || 
((frame_sync && !will_postpone_fs) || delaying_done_irq)) && ((!rst && frame_sync) || (!rst && fs_postponed)))));
      end
