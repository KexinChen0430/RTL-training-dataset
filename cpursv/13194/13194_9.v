...
module dc_adout(input  pixclock,
                input  dcnreset,
                input  hnreset,
                input  vsync,
                input  cblank,
  ...
  assign pop = (counter == 0) & cblank;
  assign active = ~((hdisbl & cblank) & (vdisbl & rsuppr));
  ...
  always @(posedge pixclock or negedge hnreset)
      if (!hnreset) 
        ...
        begin
          ...
          cblankd <= cblank;
          if (!(rsuppr & vsync)) lcount <= 12'b0;
          else if (cblankd & ~cblank) lcount <= 12'h1+lcount;
            
          if (!((cblank & rsuppr) & vsync)) pcount <= 10'b0;
          else if (pop) pcount <= 10'h1+pcount;
            
          if (pcount == ff_stp) hdisbl <= 1'b1;
          else if ((pcount == ff_rls) || !cblank) hdisbl <= 1'b0;
            
          if (lcount == {1'b0,ovtl_y}) vdisbl <= 1'b1;
          else if (!vsync || (lcount == ff_yrls)) vdisbl <= 1'b0;
