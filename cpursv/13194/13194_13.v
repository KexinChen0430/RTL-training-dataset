...
module dc_adout(input  pixclock,
  ...
  assign active = ~((vdisbl & (hdisbl & rsuppr)) & cblank);
  ...
  always @(posedge pixclock or negedge hnreset)
      if (!hnreset) 
        ...
        begin
          ...
          else if (~cblank & cblankd) lcount <= 12'h1+lcount;
            
          if (!((rsuppr & cblank) & vsync)) pcount <= 10'b0;
          ...
          else if ((lcount == ff_yrls) || !vsync) vdisbl <= 1'b0;
