...
module Bus_Arbiter(input  Clock_in,
                   input  Reset_in,
                   input  Inst_Read_in,
                   input  Inst_Write_in,
  ...
  always @(posedge Clock_in)
      begin
        if (Reset_in) Bus_Locked = 2'b00;
        else 
          begin
            if (Bus_Locked == 2'b00) 
              begin
                if (!SD_Ready_out && (Inst_Read_in | Inst_Write_in)) Bus_Locked = 2'b10;
                  
                if ((Data_Write_in | Data_Read_in) && !SD_Ready_out) Bus_Locked = 2'b01;
                  
              end
              
            if (Bus_Locked == 2'b01) 
              begin
                if ((Inst_Read_in | Inst_Write_in) && 
                    (!SD_Ready_out && ~(Data_Write_in | Data_Read_in))) Bus_Locked = 2'b10;
                  
                if (!SD_Ready_out && ~(Inst_Read_in | Inst_Write_in) && 
                    ~(Data_Write_in | Data_Read_in)) Bus_Locked = 2'b00;
                  
              end
              
            if (Bus_Locked == 2'b10) 
              begin
                if ((Data_Write_in | Data_Read_in) && 
                    (!SD_Ready_out && ~(Inst_Read_in | Inst_Write_in))) Bus_Locked = 2'b01;
