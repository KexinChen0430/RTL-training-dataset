          ...
          begin
            if (Bus_Locked == 2'b00) 
              begin
                ...
                if (!SD_Ready_out && (Data_Write_in | Data_Read_in)) Bus_Locked = 2'b01;
                  
              end
              
            if (Bus_Locked == 2'b01) 
              begin
                if (~(Data_Write_in | Data_Read_in) && !SD_Ready_out && 
                    (Inst_Write_in | Inst_Read_in)) Bus_Locked = 2'b10;
                  
                if (~(Data_Write_in | Data_Read_in) && 
                    (!SD_Ready_out && ~(Inst_Write_in | Inst_Read_in))) Bus_Locked = 2'b00;
...
~(Inst_Write_in | Inst_Read_in))) Bus_Locked = 2'b01;
