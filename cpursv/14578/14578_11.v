      ...
      begin
        if (Reset_in) Bus_Locked = 2'b00;
          ...
          begin
            if (Bus_Locked == 2'b00) 
              begin
                if ((Inst_Write_in | Inst_Read_in) && !SD_Ready_out) Bus_Locked = 2'b10;
            ...
            if (Bus_Locked == 2'b01) 
              begin
                if (!SD_Ready_out && 
                    (~(Data_Write_in | Data_Read_in) && 
(Inst_Write_in | Inst_Read_in))) Bus_Locked = 2'b10;
                  
                if (~(Data_Write_in | Data_Read_in) && 
                    (~(Inst_Write_in | Inst_Read_in) && !SD_Ready_out)) Bus_Locked = 2'b00;
                  
              end
              
            if (Bus_Locked == 2'b10) 
              begin
                if (~(Inst_Write_in | Inst_Read_in) && !SD_Ready_out && 
                    (Data_Write_in | Data_Read_in)) Bus_Locked = 2'b01;
