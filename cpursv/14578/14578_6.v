        ...
        if (Reset_in) Bus_Locked = 2'b00;
          ...
          begin
            ...
            if (Bus_Locked == 2'b01) 
              begin
                if (~(Data_Read_in | Data_Write_in) && 
                    (Inst_Read_in | Inst_Write_in) && !SD_Ready_out) Bus_Locked = 2'b10;
                  
                if (!SD_Ready_out && ~(Data_Read_in | Data_Write_in) && 
                    ~(Inst_Read_in | Inst_Write_in)) Bus_Locked = 2'b00;
            ...
            if (Bus_Locked == 2'b10) 
              begin
                if (!SD_Ready_out && 
                    ((Data_Read_in | Data_Write_in) && 
~(Inst_Read_in | Inst_Write_in))) Bus_Locked = 2'b01;
