...
module i2s_reader_phy(input  rst,
                      input  clk,
                      output [31:0] debug,
                      input  i_enable,
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            o_wfifo_strobe <= 0;
            if ((o_wfifo_activate == 0) && 
                ((state == READ) && (i_wfifo_ready > 0))) 
              begin
                r_count <= 0;
                if (i_wfifo_ready[0]) 
                  begin
            ...
            case (state)
              ...
              READ: begin
                    if (!i_enable) 
                      ...
                      begin
                        if (r_count < i_wfifo_size) 
                          begin
                            if (i_i2s_lr != r_prev_i2s_lr) 
                              begin
                        ...
                        read_word <= {2*read_word[30:0],i_i2s_data};
                      end
                  end
