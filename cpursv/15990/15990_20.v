...
module sky130_fd_io__top_refgen(VINREF,VOUTREF,REFLEAK_BIAS,VCCD,VCCHIB,VDDA,VDDIO,VDDIO_Q,
  ...
  reg  [1:0] vsel;
  wire 
       pwr_good_active_mode = (VDDIO === 1) && (VDDIO_Q === 1) && (VSSD === 0) && 
(VCCD === 1);
  wire 
       pwr_good_hold_mode = (VDDIO === 1) && (VDDIO_Q === 1) && (VSSD === 0);

  
  always @(*)
      begin
        if (((^HLD_H_N === 1'bx) && !pwr_good_hold_mode) || 
            ((^HLD_H_N === 1'bx) && (OD_H === 0)) || 
            (!pwr_good_hold_mode && 
((^OD_H === 1'bx) && 
(!pwr_good_hold_mode || (^HLD_H_N === 1'bx)))) || (^OD_H === 1'bx) || 
            (
(((^HLD_H_N === 1'bx) && !pwr_good_hold_mode) || 
((OD_H === 0) && 
(!pwr_good_hold_mode || (^HLD_H_N === 1'bx)) && !pwr_good_hold_mode) || (OD_H === 0) || !pwr_good_hold_mode) && (!pwr_good_hold_mode || (^HLD_H_N === 1'bx)))) 
          begin
            dm_final = 3'bxxx;
            slow_final = 1'bx;
          ...
          begin
            ...
            inp_dis_final = (!pwr_good_active_mode || (^inp_dis === 1'bx)) ? 1'bx : inp_dis;
