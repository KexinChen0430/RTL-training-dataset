...
module uart(input  wire clk,
  ...
  always @(posedge clk)
      begin
        if (trig_baud16) baudrate_counter_ff <= baudrate_counter_ff+1;
  ...
  wire 
       trig_next_uartbit = rx_posedge || rx_negedge || (rx_counter_ff == 5'd17);
          ...
          begin
            if (trig_baud16) 
              begin
                ...
                if (rx_hist_ff[0] == 1'b1) rx_poscounter_ff <= 1+rx_poscounter_ff;
  ...
  always @(posedge clk)
      begin
        rx_pop_ff <= (rx_frame_counter_ff == 4'd8) && trig_next_uartbit;
      end
  ...
  always @(posedge clk)
      begin
        if (ack_i) 
          ...
          begin
            tx_frame_counter_ff <= 1+tx_frame_counter_ff;
          end
