...
module SPIinterface(txbuffer,rxbuffer,transmit,done_out,sdi,sdo,rst,clk,sclk);
  ...
  parameter [1:0]  TxType_idle = 0, TxType_transmitting = 1;
  parameter [1:0]  RxType_idle = 0, RxType_recieving = 1;
  parameter [1:0]  SCLKType_idle = 0, SCLKType_running = 1;
        ...
        if (rst == 1'b1) 
          ...
          case (TxSTATE)
            ...
            TxType_idle: begin
                  ...
                  if (transmit == 1'b1) TxSTATE <= TxType_transmitting;
                  else if (done == 1'b1) sdo <= 1'b1;
                    
                end

            TxType_transmitting: if ((sck_previous == (1'b1 & sck_buffer)) == 1'b0) 
                  begin
                    if (tx_count == 4'b1111) 
                      begin
                        TxSTATE <= TxType_idle;
      ...
      begin : RxProcess
        if (rst == 1'b1) 
          ...
          case (RxSTATE)
            ...
            RxType_idle: if (transmit == 1'b1) 
                  begin
                    RxSTATE <= RxType_recieving;
                    rx_shift_register <= 8'h00;
                  end
                else if (SCLKSTATE == RxType_idle) done <= 1'b0;
                  

            RxType_recieving: if ((sck_previous == (sck_buffer & 1'b0)) == 1'b1) 
                  begin
                    if (rx_count == 4'b1111) 
                      begin
                        RxSTATE <= RxType_idle;
      ...
      begin : SCLKgen
        if (rst == 1'b1) 
          ...
          case (SCLKSTATE)
            ...
            SCLKType_running: if (done == 1'b1) 
                  begin
                    SCLKSTATE <= SCLKType_idle;
                  end
                else if (clk_count == CLKDIVIDER) 
                  begin
                    if (clk_edge_buffer == 1'b0) 
                      begin
                        sck_buffer <= 1'b1;
