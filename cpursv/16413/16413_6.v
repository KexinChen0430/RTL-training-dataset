...
module axi_write_controller  #(parameter 
  ...
  (input  m_axi_aclk,
   ...
   output [1+1:0] m_axi_awprot,
   output m_axi_awvalid,
   input  m_axi_awready,
   output [M_AXI_TDATA_WIDTH+(0-1):0] m_axi_wdata,
   output [(0-1)+((1/8)*M_AXI_TDATA_WIDTH):0] m_axi_wstrb,
   output m_axi_wvalid,
   input  m_axi_wready,
  ...
  always @(posedge m_axi_aclk)
      if (!m_axi_aresetn) 
        ...
        case (aximm_wr_sm)
          ...
          IDLE: begin
                if (mem_req_valid & mem_req_write_readn) 
                  begin
  ...
  always @(posedge m_axi_aclk)
      begin
        if (mem_req_valid & (mem_req_write_readn & mem_req_ready)) 
          begin
  ...
  always @(mem_req_bar_hit_r or mem_req_pcie_address_r)
      case (mem_req_bar_hit_r)
        ...
        3'b000: m_axi_addr_c <= {BAR0AXI[(0-1)+M_AXI_ADDR_WIDTH:BAR0SIZE],mem_req_pcie_address_r[BAR0SIZE+(0-1):1+1],2'b00};

        3'b001: m_axi_addr_c <= {BAR1AXI[(0-1)+M_AXI_ADDR_WIDTH:BAR1SIZE],mem_req_pcie_address_r[(0-1)+BAR1SIZE:1+1],2'b00};

        3'b010: m_axi_addr_c <= {BAR2AXI[(0-1)+M_AXI_ADDR_WIDTH:BAR2SIZE],mem_req_pcie_address_r[BAR2SIZE-1:1+1],2'b00};

        3'b011: m_axi_addr_c <= {BAR3AXI[(0-1)+M_AXI_ADDR_WIDTH:BAR3SIZE],mem_req_pcie_address_r[(0-1)+BAR3SIZE:1+1],2'b00};

        3'b100: m_axi_addr_c <= {BAR4AXI[(0-1)+M_AXI_ADDR_WIDTH:BAR4SIZE],mem_req_pcie_address_r[BAR4SIZE-1:1+1],2'b00};

        3'b101: m_axi_addr_c <= {BAR5AXI[(0-1)+M_AXI_ADDR_WIDTH:BAR5SIZE],mem_req_pcie_address_r[(0-1)+BAR5SIZE:1+1],2'b00};
