...
module axi_write_controller  #(parameter 
  ...
  (input  m_axi_aclk,
   ...
   output [((1/8)*M_AXI_TDATA_WIDTH)+(0-1):0] m_axi_wstrb,
   output m_axi_wvalid,
   input  m_axi_wready,
  ...
  always @(mem_req_bar_hit_r or mem_req_pcie_address_r)
      case (mem_req_bar_hit_r)
        ...
        3'b000: m_axi_addr_c <= {BAR0AXI[M_AXI_ADDR_WIDTH+(0-1):BAR0SIZE],mem_req_pcie_address_r[BAR0SIZE-1:1<<1],2'b00};

        3'b001: m_axi_addr_c <= {BAR1AXI[M_AXI_ADDR_WIDTH+(0-1):BAR1SIZE],mem_req_pcie_address_r[BAR1SIZE-1:1<<1],2'b00};

        3'b010: m_axi_addr_c <= {BAR2AXI[M_AXI_ADDR_WIDTH+(0-1):BAR2SIZE],mem_req_pcie_address_r[BAR2SIZE-1:1<<1],2'b00};

        3'b011: m_axi_addr_c <= {BAR3AXI[M_AXI_ADDR_WIDTH+(0-1):BAR3SIZE],mem_req_pcie_address_r[BAR3SIZE-1:1<<1],2'b00};

        3'b100: m_axi_addr_c <= {BAR4AXI[M_AXI_ADDR_WIDTH+(0-1):BAR4SIZE],mem_req_pcie_address_r[BAR4SIZE+(0-1):1<<1],2'b00};

        3'b101: m_axi_addr_c <= {BAR5AXI[M_AXI_ADDR_WIDTH+(0-1):BAR5SIZE],mem_req_pcie_address_r[BAR5SIZE+(0-1):1<<1],2'b00};
