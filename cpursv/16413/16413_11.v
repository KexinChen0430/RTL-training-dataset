...
module axi_write_controller  #(parameter 
  ...
  always @(posedge m_axi_aclk)
      begin
        if (mem_req_write_readn & (mem_req_ready & mem_req_valid)) 
          begin
  ...
  always @(mem_req_bar_hit_r or mem_req_pcie_address_r)
      case (mem_req_bar_hit_r)
        ...
        3'b001: m_axi_addr_c <= {BAR1AXI[(-1)+M_AXI_ADDR_WIDTH:BAR1SIZE],mem_req_pcie_address_r[(-1)+BAR1SIZE:1<<<1],2'b00};

        3'b010: m_axi_addr_c <= {BAR2AXI[(-1)+M_AXI_ADDR_WIDTH:BAR2SIZE],mem_req_pcie_address_r[(-1)+BAR2SIZE:1<<<1],2'b00};

        3'b011: m_axi_addr_c <= {BAR3AXI[(-1)+M_AXI_ADDR_WIDTH:BAR3SIZE],mem_req_pcie_address_r[(-1)+BAR3SIZE:1<<<1],2'b00};

        3'b100: m_axi_addr_c <= {BAR4AXI[(-1)+M_AXI_ADDR_WIDTH:BAR4SIZE],mem_req_pcie_address_r[(-1)+BAR4SIZE:1<<<1],2'b00};

        3'b101: m_axi_addr_c <= {BAR5AXI[(-1)+M_AXI_ADDR_WIDTH:BAR5SIZE],mem_req_pcie_address_r[(-1)+BAR5SIZE:1<<<1],2'b00};
