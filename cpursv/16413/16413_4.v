...
module axi_write_controller  #(parameter 
  ...
  always @(mem_req_bar_hit_r or mem_req_pcie_address_r)
      case (mem_req_bar_hit_r)
        ...
        3'b010: m_axi_addr_c <= {BAR2AXI[M_AXI_ADDR_WIDTH+(-1):BAR2SIZE],mem_req_pcie_address_r[(-1)+BAR2SIZE:1<<1],2'b00};

        3'b011: m_axi_addr_c <= {BAR3AXI[M_AXI_ADDR_WIDTH+(-1):BAR3SIZE],mem_req_pcie_address_r[(-1)+BAR3SIZE:1<<1],2'b00};

        3'b100: m_axi_addr_c <= {BAR4AXI[M_AXI_ADDR_WIDTH+(-1):BAR4SIZE],mem_req_pcie_address_r[(-1)+BAR4SIZE:1<<1],2'b00};

        3'b101: m_axi_addr_c <= {BAR5AXI[M_AXI_ADDR_WIDTH+(-1):BAR5SIZE],mem_req_pcie_address_r[(-1)+BAR5SIZE:1<<1],2'b00};
