...
module axi_write_controller  #(parameter 
  ...
  always @(mem_req_bar_hit_r or mem_req_pcie_address_r)
      case (mem_req_bar_hit_r)
        ...
        3'b000: m_axi_addr_c <= {BAR0AXI[M_AXI_ADDR_WIDTH+(-1):BAR0SIZE],mem_req_pcie_address_r[(-1)+BAR0SIZE:2],2'b00};

        3'b001: m_axi_addr_c <= {BAR1AXI[M_AXI_ADDR_WIDTH+(-1):BAR1SIZE],mem_req_pcie_address_r[BAR1SIZE+(-1):2],2'b00};

        3'b010: m_axi_addr_c <= {BAR2AXI[M_AXI_ADDR_WIDTH+(-1):BAR2SIZE],mem_req_pcie_address_r[(-1)+BAR2SIZE:2],2'b00};
