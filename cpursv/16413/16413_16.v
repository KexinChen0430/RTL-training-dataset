...
module axi_write_controller  #(parameter 
  ...
  always @(posedge m_axi_aclk)
      begin
        if ((mem_req_ready & mem_req_write_readn) & mem_req_valid) 
          begin
  ...
  always @(mem_req_bar_hit_r or mem_req_pcie_address_r)
      case (mem_req_bar_hit_r)
        ...
        3'b010: m_axi_addr_c <= {BAR2AXI[M_AXI_ADDR_WIDTH-1:BAR2SIZE],mem_req_pcie_address_r[(0-1)+BAR2SIZE:2],2'b00};

        3'b011: m_axi_addr_c <= {BAR3AXI[M_AXI_ADDR_WIDTH-1:BAR3SIZE],mem_req_pcie_address_r[(0-1)+BAR3SIZE:2],2'b00};
