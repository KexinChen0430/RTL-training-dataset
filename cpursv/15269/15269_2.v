...
module color_position  #(parameter  THRESHOLD = 20, COLOR_WIDTH = 10, DISP_WIDTH = 11)
  ...
  assign x_diff = (x_pos > x_obj) ? (x_pos+(0-x_obj)) : (x_obj+(0-x_pos));
  assign y_diff = (y_pos > y_obj) ? (y_pos+(0-y_obj)) : (y_obj-y_pos);
  assign vga_is_object = (y_diff < THRESHOLD) & (x_diff < THRESHOLD);
  assign x_diff2 = (x_pos > x_kalman) ? (x_pos-x_kalman) : (x_kalman+(0-x_pos));
  assign y_diff2 = (y_pos > y_kalman) ? ((0-y_kalman)+y_pos) : (y_kalman-y_pos);
