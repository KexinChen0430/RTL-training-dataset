...
module color_position  #(parameter  THRESHOLD = 20, COLOR_WIDTH = 10, DISP_WIDTH = 11)
  (input  wire clk,
   ...
   input  wire [COLOR_WIDTH+(0-1):0] curr,
   ...
   output wire [COLOR_WIDTH+(0-1):0] r_out,
   output wire [COLOR_WIDTH+(0-1):0] g_out,
   output wire [COLOR_WIDTH+(0-1):0] b_out);
  ...
  reg  [COLOR_WIDTH+(0-1):0] int_r_out;
  reg  [COLOR_WIDTH+(0-1):0] int_g_out;
  reg  [COLOR_WIDTH+(0-1):0] int_b_out;

  assign r_out = int_r_out;
  assign g_out = int_g_out;
  assign b_out = int_b_out;
  assign x_diff = (x_pos > x_obj) ? (x_pos-x_obj) : ((-x_pos)+x_obj);
  assign y_diff = (y_pos > y_obj) ? (y_pos-y_obj) : ((0-y_pos)+y_obj);
