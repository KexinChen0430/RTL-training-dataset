...
module color_position  #(parameter  THRESHOLD = 20, COLOR_WIDTH = 10, DISP_WIDTH = 11)
  ...
  assign y_diff = (y_pos > y_obj) ? (y_pos-y_obj) : ((-y_pos)+y_obj);
  assign vga_is_object = (x_diff < THRESHOLD) & (y_diff < THRESHOLD);
  assign x_diff2 = (x_pos > x_kalman) ? ((-x_kalman)+x_pos) : (x_kalman-x_pos);
  assign y_diff2 = (y_pos > y_kalman) ? (y_pos-y_kalman) : ((-y_pos)+y_kalman);
  ...
  always @(posedge clk or negedge aresetn)
      begin
        ...
        else if (vga_is_kalman & (enable & enable_kalman)) 
          begin
