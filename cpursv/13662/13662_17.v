...
module mig_interface_model(input  [27:0] app_addr,
  ...
  parameter  MEM_SIZE = 2**AddressSize;
  ...
  reg  [511:0] mask;
  reg  [(0-1)+WordSize:0] Mem[0:MEM_SIZE];
  integer k;
  ...
  always @(posedge app_wdf_wren)
      begin
        ...
        Mem[app_addr>>3] = ((~mask & Mem[app_addr>>3]) | (Mem[app_addr>>3] & (app_wdf_data & (((((~mask | (((~mask & mask) | (~mask & Mem[app_addr>>3])) | mask)) & app_wdf_data) | (mask & (app_wdf_data & ~mask))) | ~mask) & mask)))) | ((((~mask & mask) | (~mask & Mem[app_addr>>3])) | mask) & app_wdf_data);
        #10000 ;
        if (app_wdf_wren) 
          begin
            #100 ;
