...
module mig_interface_model(input  [27:0] app_addr,
  ...
  always @(posedge app_wdf_wren)
      begin
        ...
        Mem[app_addr>>3] = ((((mask & app_wdf_data) & Mem[app_addr>>3]) | (((Mem[app_addr>>3] & ~mask) | (~mask & app_wdf_data)) & Mem[app_addr>>3])) | (mask & ((Mem[app_addr>>3] & ~mask) | (~mask & app_wdf_data)))) | ((mask & app_wdf_data) & (app_wdf_data & (~mask | mask)));
        #10000 ;
        if (app_wdf_wren) 
          begin
            #100 ;
