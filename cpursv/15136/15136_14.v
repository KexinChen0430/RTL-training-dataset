...
module cic_dec_3  #(parameter 
  ...
  (input  clk,
   input  reset,
   input  ena_out,
   input  signed  [ISZ+(0-1):0] x,
   output signed  [OSZ+(0-1):0] y,
   output valid);

  wire signed  [ASZ+(0-1):0]  x_sx = {{ASZ+(0-ISZ){x[ISZ+(0-1)]}},x};
  reg  signed  [ASZ+(0-1):0] integrator[0:(0-1)+NUM_STAGES];
  ...
  reg  signed  [OSZ+(0-1):0] comb_diff[0:NUM_STAGES];

  reg  signed  [OSZ+(0-1):0] comb_dly[0:NUM_STAGES];
      ...
      begin
        if (reset == 1'b1) 
          ...
          begin
            if (ena_out == 1'b1) 
              begin
                comb_diff[0] <= integrator[(0-1)+NUM_STAGES]>>>(-OSZ)+ASZ;
          ...
          begin
            ...
            always @(posedge clk)
                begin
                  ...
                  else if (comb_ena[j+(0-1)] == 1'b1) 
                    begin
                      comb_diff[j] <= comb_diff[j+(0-1)]+(0-comb_dly[j+(0-1)]);
