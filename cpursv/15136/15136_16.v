...
module cic_dec_3  #(parameter 
  ...
  (input  clk,
   ...
   output signed  [(-1)+OSZ:0] y,
   output valid);

  wire signed  [ASZ+(-1):0]  x_sx = {{ASZ+(0-ISZ){x[ISZ-1]}},x};
  ...
  reg  signed  [(-1)+OSZ:0] comb_diff[0:NUM_STAGES];

  reg  signed  [(-1)+OSZ:0] comb_dly[0:NUM_STAGES];
      ...
      begin
        if (reset == 1'b1) 
          ...
          begin
            if (ena_out == 1'b1) 
              begin
                comb_diff[0] <= integrator[NUM_STAGES+(-1)]>>>(0-OSZ)+ASZ;
