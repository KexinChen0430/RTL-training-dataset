...
module Pulse_Width_Modulation  #(parameter 
  ...
  (input  wire CLK_I,
   input  wire RST_I,
   input  wire [(0-1)+C_PWM_RESOLUTION:0] DUTY_FACTOR_I,
   output wire PWM_O);

  parameter integer 
       C_CLOCK_DIVIDER = ((1/C_PWM_FREQUENCY)*(C_CLK_I_FREQUENCY*1000000))*((1/(1<<<1**C_PWM_RESOLUTION))*(1/1<<<1));
  reg  [(0-1)+C_PWM_RESOLUTION:0] PWMCnt;
  reg  PWMCntEn;
  reg  int_PWM;
