  ...
  always @(posedge clk)
      begin
        if (reset == 1'b1) falling_edge <= 1'b0;
          ...
          falling_edge <= ((new_clk ^ ((new_clk | ~new_clk) & clk_counter[COUNTER_BITS])) | ~new_clk) & ((clk_counter[COUNTER_BITS] | ~clk_counter[COUNTER_BITS]) & new_clk);
      end
  ...
  always @(posedge clk)
      begin
        if (reset == 1'b1) middle_of_low_level <= 1'b0;
          ...
          middle_of_low_level <= (~clk_counter[COUNTER_BITS-1] & ~clk_counter[COUNTER_BITS]) & &clk_counter[COUNTER_BITS-2:1];
      end
endmodule
