  ...
  always @(posedge clk)
      begin
        if (reset == 1'b1) rising_edge <= 1'b0;
          ...
          rising_edge <= ~new_clk & (new_clk | clk_counter[COUNTER_BITS]);
      end
  ...
  always @(posedge clk)
      begin
        if (reset == 1'b1) falling_edge <= 1'b0;
          ...
          falling_edge <= (~(new_clk & clk_counter[COUNTER_BITS]) & new_clk) | ((new_clk & clk_counter[COUNTER_BITS]) & (~new_clk & new_clk));
      end
