...
module Registrodesplazamiento  #(parameter  Width = 10)
  ...
  always @(posedge CLK)
      begin
        if (reset) 
          ...
          begin
            Aux <= {Indato,Aux[(Width*10)+(0-1):Width]};
          end
          
      end
  assign Y0 = Aux[Width+(0-1):0];
  assign Y1 = Aux[Width<<<1-1:Width];
  assign Y2 = Aux[(3*Width)-1:Width<<<1];
  assign Y3 = Aux[(0-1)+(Width*4):3*Width];
  assign Y4 = Aux[(0-1)+(Width*5):Width*4];
  ...
  assign Y9 = Aux[(Width*10)+(0-1):9*Width];
endmodule
