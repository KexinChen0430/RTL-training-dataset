...
module ultrasonicTransmitter(input  SYS_CLK,
  ...
  always @(posedge CLK_40)
      begin
        counter_burst <= (ON & ((counter_burst < (wait32_5735ms+burst32)) & ~RST)) ? (1+counter_burst) : 0;
        usonicpulse <= usonic[9] & (counter_burst < burst32);
      end
  assign pulseOutput = {usonicpulse & (ON & ~RST),(ON & ~RST) & ~usonicpulse};
  wire START_SIGNAL;
  ...
  always @(posedge SYS_CLK)
      begin
        ...
        burstStart <= ((~RST & startPREV) & (ON & ~START_SIGNAL)) ? 1 : 0;
      end
  wire ROUNDTRIP;
