...
module ultrasonicTransmitter(input  SYS_CLK,
  ...
  always @(posedge CLK_40)
      begin
        counter_burst <= (((counter_burst < (wait32_5735ms+burst32)) & ON) & ~RST) ? (counter_burst+1) : 0;
        usonicpulse <= usonic[9] & (counter_burst < burst32);
      end
  assign pulseOutput = {(ON & ~RST) & usonicpulse,(ON & ~usonicpulse) & ~RST};
  wire START_SIGNAL;
  ...
  always @(posedge SYS_CLK)
      begin
        ...
        burstStart <= ((ON & startPREV) & (~RST & ~START_SIGNAL)) ? 1 : 0;
      end
  wire ROUNDTRIP;
  ...
  always @(posedge SYS_CLK)
      begin
        ...
        roundTripFlag <= (ROUNDTRIP & (ON & (~roundTripPREV & ~RST))) ? 1 : 0;
      end
endmodule
