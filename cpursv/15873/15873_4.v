...
module video_palframe_mk3bit(input  wire [1:0] phase,
  ...
  always @* 
      begin
        ...
        gridy = (((~(x[1] & colorlevel[0]) & (y & (x[1] & colorlevel[0]))) | x[1]) & ((~(x[1] & colorlevel[0]) & (y & (x[1] & colorlevel[0]))) | (~y & colorlevel[0]))) ^ ((y & (~(x[1] & colorlevel[0]) | (x[1] & ~y))) & (~(x[1] & colorlevel[0]) | colorlevel[0]));
