...
module myvrlg(input  wire rst,
  ...
  assign tx_hs_data = inst1_inst_snk_data[(0-1)+64:0];
  assign inst_inst_rx_hs_valid = rx_hs_valid;
  assign rx_hs_ready = !0 ? inst_inst_rx_hs_ready : 1;
  assign inst0_snk_data = rx_hs_data;
  
  always @(inst_inst_tx_hs_ready or inst_inst_rx_hs_valid or inst_inst_hs_en_inst_state)
      begin : PT_HS_TOP_INST_HS_EN_INST_RDY
        if ((inst_inst_hs_en_inst_state == 0) || 
            inst_inst_tx_hs_ready) 
          begin
  ...
  always @(posedge clk)
      begin : PT_HS_TOP_INST_HS_EN_INST_CLK_PRCS
        if (rst) 
          begin
  ...
  always @(posedge clk or posedge rst)
      begin : PT_HS_TOP_INST_CLK_PRCS_HS
        if (rst == 1) 
          begin
          ...
          begin
            if (inst_inst_hs_en) 
              begin
                inst_inst_tx_hs_data <= inst0_snk_data[(0-1)+64:0];
              end
