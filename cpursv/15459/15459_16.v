...
module oh_par2ser  #(parameter  PW = 64, SW = 1, CW = $clog2(PW/SW))
  ...
  always @(posedge clk or negedge nreset)
      ...
      else if (busy & shift) count[CW+(0-1):0] <= (0-1'b1)+count[CW+(0-1):0];
  ...
  always @(posedge clk or negedge nreset)
      if (!nreset) shiftreg[PW-1:0] = 'b0;
        ...
        shiftreg[PW-1:0] = {shiftreg[(0-1)+(PW+(0-SW)):0],{SW{fill}}};
        
  assign dout[SW-1:0] = lsbfirst ? shiftreg[SW-1:0] : shiftreg[PW-1:PW+(0-SW)];
endmodule
