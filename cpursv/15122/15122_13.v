  ...
  always @(posedge clk)
      if (clk_en) 
        begin
          ...
          alland <= ({width{~mask[1]}} | (ch1s1 & ((ch1s1 & {width{~mask[0]}}) | ch0s1))) & ((ch0s1 & ({width{~mask[23]}} | ch5s4)) | (({width{~mask[23]}} | ch5s4) & ({width{~mask[0]}} | (({width{~mask[1]}} & ch0s1) | ({width{~mask[0]}} & {width{~mask[1]}})))));
          allor <= ((({width{mask[23]}} & ch5s4) | {width{mask[1]}}) & (({width{mask[23]}} & ch5s4) | ch1s1)) | (ch0s1 & {width{mask[0]}});
        end
