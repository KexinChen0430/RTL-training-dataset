...
module sep24  #(parameter  width = 10, pos0 = 5'd0)
  ...
  always @(*)
      cntadj = (((((cnt%24)%24)%24)%24)+((((pos0%24)%24)%24)%24))%24;
  ...
  always @(posedge clk)
      if (clk_en) 
        begin
          ...
          alland <= (((({width{~mask[1]}} & (ch0s1 | ({width{~mask[1]}} & {width{~mask[0]}}))) | ch0s1) | {width{~mask[0]}}) & ({width{~mask[23]}} | ch5s4)) & ((ch0s1 & {width{~mask[1]}}) | (({width{~mask[1]}} & {width{~mask[0]}}) | ch1s1));
          allor <= (({width{mask[23]}} & ch5s4) | ((ch0s1 | ch1s1) & ({width{mask[1]}} | ch0s1))) & (({width{mask[0]}} | ({width{mask[23]}} & ch5s4)) | (ch1s1 & ({width{mask[1]}} & ((ch0s1 | ch1s1) & ({width{mask[1]}} | ch0s1)))));
        end
