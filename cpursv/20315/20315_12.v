...
module fifo_to_packet(input  clk,
                      input  reset_n,
                      input  out_ready,
          ...
          begin
            ...
            case (state)
              ...
              FIFO_DATA_WAIT: begin
                    ...
                    current_byte <= 1'b1;
                    out_valid <= 1'b1;
                    if (((fifo_readdata[33:32] == 0) & fifo_readdata[34]) & fifo_readdata[35]) 
                      begin
                        first_trans <= 1'b0;
                        last_trans <= 1'b0;
                    ...
                    else if ((fifo_readdata[33:32] == 0) & fifo_readdata[35]) 
                      begin
                        first_trans <= 1'b0;
                        out_startofpacket <= 1'b1;
                        state <= POP_FIFO;
                      end
                    else if (fifo_readdata[35]) 
                      begin
                        first_trans <= 1'b0;
                        out_startofpacket <= 1'b1;
                        state <= READ_SEND_ISSUE;
                      end
                    else if ((fifo_readdata[33:32] == 0) & fifo_readdata[34]) 
                      begin
                        last_trans <= 1'b0;
                        out_endofpacket <= 1'b1;
                        state <= POP_FIFO;
              ...
              READ_SEND_ISSUE: begin
                    ...
                    if (out_ready) 
                      begin
                        out_startofpacket <= 1'b0;
                        if (last_trans & (current_byte == byte_end)) 
                          begin
                            last_trans <= 1'b0;
                            out_endofpacket <= 1'b1;
                            state <= POP_FIFO;
                        ...
                        current_byte <= 1'b1+current_byte;
                        if (current_byte == byte_end) 
                          begin
                            state <= POP_FIFO;
                          end
                        else 
