...
module cu(clk,rst,deposited,selected,cancel,maintenance,purchase,
          ...
          clrRprice,clrA,clrRproduct,clrRchange,clrRpurchase,refundall,
          depositall,state);
  ...
  always @(pstate or cancel or maintenance or deposited or selected or purchase)
      begin
        case (pstate)

          S_init: nstate = S_wait;

          S_wait: if (maintenance) nstate = S_maintenance;
              else if (deposited) nstate = S_deposit;
              else if (cancel) nstate = S_cancel;
              else if (selected) nstate = S_select;
              else nstate = S_wait;

          S_select: if (purchase) nstate = S_purchase;
              else nstate = S_wait;

          S_purchase: nstate = S_init;

          S_maintenance: if (maintenance) nstate = S_maintenance;
              else nstate = S_init;
  ...
  always @(pstate or selected)
      begin
        case (pstate)
          ...
          S_maintenance: if (selected) cv = 16'b0111_1100_1000_0110;
  ...
  assign refundall = ((state == S_maintenance) || (state == S_cancel)) ? 1'b1 : 1'b0;
