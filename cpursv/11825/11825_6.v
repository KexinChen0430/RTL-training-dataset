...
module UART_RX(input  RxD_ser,
  ...
  always @(posedge sys_clk)
      if (Baud16Tick) RxD_sync <= {RxD_sync[0],RxD_ser};
  ...
  always @(posedge sys_clk)
      if (Baud16Tick) 
        begin
          if (RxD_sync[1] && (RxD_cnt != 3'b111)) RxD_cnt <= 1+RxD_cnt;
          else if ((RxD_cnt != 3'b000) && ~RxD_sync[1]) RxD_cnt <= (0-1)+RxD_cnt;
  ...
  assign CTS = state < (1+1);
  reg  [3:0] bit_spacing;

  
  always @(posedge sys_clk)
      if (state == 0) bit_spacing <= 0;
      else if (Baud16Tick) bit_spacing <= 1+bit_spacing;
  ...
  always @(posedge sys_clk)
      if (Baud16Tick && (next_bit && state[3])) TxD_par <= {RxD_bit,TxD_par[7:1]};
