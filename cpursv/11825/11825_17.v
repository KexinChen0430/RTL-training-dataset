...
module UART_RX(input  RxD_ser,
  ...
  always @(posedge sys_clk)
      if (Baud16Tick) 
        begin
          if ((RxD_cnt != 3'b111) && RxD_sync[1]) RxD_cnt <= 1+RxD_cnt;
          else if ((RxD_cnt != 3'b000) && ~RxD_sync[1]) RxD_cnt <= RxD_cnt-1;
  ...
  always @(posedge sys_clk)
      ...
      else if (Baud16Tick) bit_spacing <= bit_spacing+1;
  ...
  always @(posedge sys_clk)
      if (next_bit && state[3] && Baud16Tick) TxD_par <= {RxD_bit,TxD_par[7:1]};
