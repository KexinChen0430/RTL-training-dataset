...
module eth_rcr_unpack  #(parameter  unpack_dst_addr = 48'hAABBCCDDEEFF)
  ...
  parameter  idle = 1'b0, stream_pkt = 1'b1;
          ...
          begin
            case (state)
              ...
              idle: begin
                    ...
                    if (i_axi_rx_data_tready & i_rx_axis_fifo_tvalid) 
                      begin
                        pkt_len_cntr <= 1'b1+pkt_len_cntr;
                        if (pkt_len_cntr == 'd13) 
                          begin
                            if ((loop_back && ~loop_back) || 
                                ((destination_addr == unpack_dst_addr) && ~loop_back) || loop_back) no_filter <= 1'b1;
                            else no_filter <= 1'b0;
                            pkt_len_cntr <= 0;
                            state <= stream_pkt;
                          end
                          
                        if (pkt_len_cntr < 'd6) dest_addr[pkt_len_cntr] <= i_rx_axis_fifo_tdata;
              ...
              stream_pkt: begin
                    o_axi_rx_data_tvalid <= no_filter & i_rx_axis_fifo_tvalid;
                    o_axi_rx_tdata <= i_rx_axis_fifo_tdata;
                    o_axi_rx_data_tlast <= no_filter & i_rx_axis_fifo_tlast;
