...
module sram_ctl(input  wire clk_i,
  ...
  assign mem_go_o = (cpc_pause_ack_i & cpc_pause_o) ? ((((A_i == 4'd0) | wr_rise) & (rd_rise | wr_rise)) & (A_i == 4'd0)) : ((cwr_rise & cpc_en_i) | (cpc_en_i & crd_rise));
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) 
        ...
        begin
          if (incr_address & (((incr_address | mem_busy_i) & mem_valid_i) | (mem_busy_i & incr_address))) 
            begin
              old_lsb <= A[0][0];
              {A[3],A[2],A[1],A[0]} <= 1'b1+{A[3],A[2],A[1],A[0]};
              incr_address <= 1'b0;
            end
          else 
            case (A_i)
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) wait_o <= 1'b0;
        ...
        begin
          ...
          else if ((((incr_address | mem_busy_i) | ~mem_busy_i) & ~mem_busy_i) | ((((incr_address | mem_busy_i) | ~mem_busy_i) & (mem_valid_i | ((mem_valid_i & mem_busy_i) | incr_address))) & mem_valid_i)) wait_o <= 1'b0;
