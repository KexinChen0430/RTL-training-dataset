...
module sram_ctl(input  wire clk_i,
  ...
  assign mem_go_o = (cpc_pause_o & cpc_pause_ack_i) ? ((A_i == 4'd0) & (wr_rise | rd_rise)) : ((cwr_rise | ((cpc_en_i | (crd_rise & cwr_rise)) & crd_rise)) & cpc_en_i);
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) 
        ...
        begin
          if (incr_address & ((incr_address | (mem_busy_i & mem_valid_i)) & (((mem_valid_i | incr_address) & mem_busy_i) | mem_valid_i))) 
            begin
              old_lsb <= A[0][0];
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) wait_o <= 1'b0;
        ...
        begin
          ...
          else if (~mem_busy_i | ((mem_busy_i & mem_valid_i) | (mem_valid_i & incr_address))) wait_o <= 1'b0;
