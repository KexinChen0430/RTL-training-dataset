...
module sram_ctl(input  wire clk_i,
                input  wire reset_i,
                input  wire [3:0] A_i,
                input  wire [7:0] D_i,
                output wire [7:0] D_o,
  ...
  assign mem_A_o = (cpc_pause_ack_i & cpc_pause_o) ? {1'b0,A[2],A[1],A[0][7:1]} : {1'b0,cpc_A_i[23:1]};
  assign mem_D_o = (cpc_pause_ack_i & cpc_pause_o) ? {D_i,D_i} : {cpc_D_i,cpc_D_i};
  assign mem_go_o = (cpc_pause_ack_i & cpc_pause_o) ? ((A_i == 4'd0) & (wr_rise | rd_rise)) : (cpc_en_i & (cwr_rise | crd_rise));
  assign mem_cmd_o = (cpc_pause_ack_i & cpc_pause_o) ? {control_ops,rd_i,control_ops ? 1'b1 : A[0][0]} : {1'b0,cpc_rd_i,cpc_A_i[0]};
  ...
  always @(posedge clk_i)
      track_rd <= {track_rd[0],rd_i};
  
  always @(posedge clk_i)
      track_wr <= {track_wr[0],wr_i};
  
  always @(posedge clk_i)
      track_crd <= {track_rd[0],cpc_rd_i};
  
  always @(posedge clk_i)
      track_cwr <= {track_wr[0],cpc_wr_i};
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) 
        ...
        begin
          if ((mem_busy_i | (incr_address & mem_valid_i)) & incr_address) 
            begin
              old_lsb <= A[0][0];
            ...
            case (A_i)
              ...
              4'b0000: begin
                    if (wr_rise | rd_rise) incr_address <= 1'b1;
                      
                  end

              4'b1000,4'b1001,4'b1010,4'b1011: begin
                    if (rd_i) DOUT <= A[A_i[1:0]];
                    else if (wr_i) A[A_i[1:0]] <= D_i;
              ...
              4'b1111: begin
                    ...
                    else if (rd_i) DOUT <= {cpc_pause_ack_i,control_ops,6'd0};
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) wait_o <= 1'b0;
        ...
        begin
          if (rd_rise && (A_i == 4'd0)) wait_o <= 1'b1;
          else if ((((~mem_busy_i | (((mem_valid_i & mem_busy_i) | incr_address) & mem_busy_i)) & (mem_valid_i | (((mem_valid_i & mem_busy_i) | incr_address) & (mem_busy_i | (incr_address & mem_valid_i))))) & mem_valid_i) | ((~mem_busy_i | (((mem_valid_i & mem_busy_i) | incr_address) & mem_busy_i)) & ~mem_busy_i)) wait_o <= 1'b0;
