...
module sram_ctl(input  wire clk_i,
  ...
  assign mem_go_o = (cpc_pause_o & cpc_pause_ack_i) ? ((wr_rise | rd_rise) & (A_i == 4'd0)) : ((cwr_rise | crd_rise) & cpc_en_i);
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) 
        ...
        begin
          if ((mem_valid_i | mem_busy_i) & incr_address) 
            begin
              old_lsb <= A[0][0];
  ...
  always @(posedge clk_i or posedge reset_i)
      if (reset_i) wait_o <= 1'b0;
        ...
        begin
          ...
          else if ((((~mem_busy_i | ((((mem_busy_i & incr_address) | mem_valid_i) & mem_busy_i) | ((mem_valid_i | mem_busy_i) & incr_address))) & (mem_valid_i | mem_busy_i)) & mem_valid_i) | ((~mem_busy_i | ((((mem_busy_i & incr_address) | mem_valid_i) & mem_busy_i) | ((mem_valid_i | mem_busy_i) & incr_address))) & ~mem_busy_i)) wait_o <= 1'b0;
