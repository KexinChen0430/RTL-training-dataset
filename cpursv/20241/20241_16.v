...
module denise_bitplane_shifter(input  wire clk,
                               ...
                               input  wire hires,
                               input  wire shres,
                               input  wire [1<<<1+(0-1):0] fmode,
                               input  wire [64+(0-1):0] data_in,
                               input  wire [8-1:0] scroll,
                               output wire out);

  reg  [6+(0-1):0] fmode_mask;
  reg  [64+(0-1):0] shifter;
  reg  [64+(0-1):0] scroller;
  reg  shift;
  reg  [6+(0-1):0] select;
  wire scroller_out;
  ...
  always @(*)
      begin
        if (shres) 
          ...
          begin
            shift = (~c1 & (~c3 | c3)) ^ ((c3 | ~c1) & (((c1 | ~c1) & c3) & (~c3 | c3)));
            select[5:0] = fmode_mask & scroll[6:1];
          end
        else 
          begin
            shift = ((~c1 & (~c3 | c3)) & ((c1 & (c3 | ~c1)) | ~c3)) & ((c3 | ~c1) & ~c3);
