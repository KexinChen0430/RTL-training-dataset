...
module my_uart_tx(rst_n,baud_clk,tx_start,tx_data,tx_enable,tx_complete,
  ...
  always @(negedge tx_start or negedge tx_complete_reg or negedge rst_n)
      begin
        ...
        else if (!tx_complete_reg) tx_enable_reg <= 1'b0;
        else if (!tx_start) tx_enable_reg <= 1'b1;
  ...
  always @(negedge baud_clk or negedge tx_enable_reg or negedge rst_n)
      begin
        ...
        else if (!tx_enable_reg) tx_complete_reg <= 1'b1;
        else if (!baud_clk) 
          begin
            if (!tx_count) tx_complete_reg <= 1'b0;
