      ...
      begin
        if (!RST_N || CLR) 
          ...
          begin
            ...
            sempty <= (pos == 0) || 
                     (((pos == 1) || (pos == 0)) && 
((!ENQ && (pos == 0)) || ((pos == 1) && DEQ && !ENQ)));
            sfull <= 
((pos == ((0-1)+depth)) && (pos == (depth+(-1<<<1)))) || 
                    (
(
((pos == ((0-1)+depth)) && (pos == (depth+(-1<<<1)))) || (ENQ && !DEQ) || (pos == ((0-1)+depth))) && (pos == (depth+(-1<<<1)))) || (pos == ((0-1)+depth));
            if ((!sempty && (dempty && (!dempty || !sempty))) || 
                (!sempty && (DEQ && !dempty))) 
              begin
                dreg <= dat[pos-1];
                dempty <= 1'b0;
