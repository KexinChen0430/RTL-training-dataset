...
module timer(pclk,nreset,bus_write_en,bus_read_en,bus_addr,bus_write_data,
             bus_read_data,fabint);
  ...
  always @(posedge pclk)
      if (~nreset) fabint <= 1'b0;
      else 
        begin
          if (timer_interrupt) fabint <= 1'b1;
          else fabint <= 1'b0;
        end
  ...
  always @*   nextCounter = counterReg+1;
              ...
              if (overflowReset) 
                ...
                begin
                  if (counterReg == overflowReg) 
                    begin
                      counterReg <= 32'd0;
                      if (overflowEn && interruptEn) 
                        begin
                          timer_interrupt <= 1'b1;
                          interrupt_status[0] <= 1'b1;
                        end
                      else timer_interrupt <= 1'b0;
                    end
                  else 
                    begin
                      if (compareEn && (counterReg == compareReg) && interruptEn) 
                        begin
                          timer_interrupt <= 1'b1;
