...
module v_axi4s_vid_out_v3_0_vid_out_formatter  #(parameter  DATA_WIDTH = 24)
  (input  wire video_out_clk,
   input  wire rst,
   input  wire vid_ce,
   input  wire [(0-1)+DATA_WIDTH:0] odf_data,
   ...
   output wire [(0-1)+DATA_WIDTH:0] video_data);

  reg  [(0-1)+DATA_WIDTH:0] in_data_mux;
  reg  in_de_mux;
  reg  in_vsync_mux;
  reg  in_hsync_mux;
  reg  in_vblank_mux;
      ...
      begin
        if (vid_ce) 
          begin
            ...
            vblank_rising <= !odf_vblank_1 && odf_vblank;
          end
  ...
  always @(posedge video_out_clk)
      begin
        if (!locked || (!first_full_frame || rst)) 
          begin
            in_de_mux <= 0;
            in_vsync_mux <= 0;
            in_hsync_mux <= 0;
          ...
          begin
            ...
            if (fifo_rd_en) in_data_mux <= odf_data;
