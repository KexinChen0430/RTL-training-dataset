...
module norm(clk_proc,reset_n,in_fv,in_dv,in_data,out_fv,out_dv,out_data,
  ...
  wire [(-1)+IN_SIZE:0] shiftout;
  reg  [5:0] counter;
  reg  [18:0] acc;
  reg  [18:0] acc_d;
  reg  fsm,fsm_new;

  assign dv_s = ((~in_fv & ((in_fv & in_dv) | (in_dv & (out_fv & ~in_fv)))) | (out_fv & ~in_fv)) | (((in_fv | ~in_fv) & in_dv) & (in_fv & in_dv));
  ...
  always @(posedge clk_proc or negedge reset_n)
      begin
        if (reset_n == 1'b0) div_token <= {NPIPELINE{1'b0}};
          ...
          div_token[(-1)+NPIPELINE:0] <= {div_token[NPIPELINE+(-1<<<1):0],shift_dv};
        else div_token <= div_token;
      end
  assign out_dv = dv_s & div_token[(-1)+NPIPELINE];
  wire [23:0] out_div;
...
.lpm_hint(LPM_REMAINDERPOSITIVE=TRUE)) lpm_divide_inst(.quotient(out_div),
                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                     .clken(dv_s & (div_token[(-1)+NPIPELINE] | (shift_dv & dv_s))),.aclr(1'b0));
