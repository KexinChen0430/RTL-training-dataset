...
module norm(clk_proc,reset_n,in_fv,in_dv,in_data,out_fv,out_dv,out_data,
  ...
  output out_fv;
  output out_dv;
  output [OUT_SIZE+(0-1):0] out_data;
  input  addr_rel_i;
  input  wr_i;
  input  [31:0] datawr_i;
  input  rd_i;
  ...
  assign dv_s = (out_fv & ((in_dv | ((out_fv | (~in_fv & out_fv)) | ((in_dv & out_fv) | (in_fv & in_dv)))) & (~in_fv | (in_fv & in_dv)))) | ((in_fv & (~in_fv & out_fv)) | (in_fv & in_dv));
...
.number_of_taps(1),.tap_distance(NWORDS),.width(IN_SIZE)) altshift_taps_inst_0(.clock(clk_proc),
  ...
  always @(posedge clk_proc or negedge reset_n)
      ...
      else if (dv_s) counter <= counter+1'b1;
  ...
  always @(posedge clk_proc or negedge reset_n)
      begin
        if (reset_n == 1'b0) div_token <= {NPIPELINE{1'b0}};
          ...
          div_token[(0-1)+NPIPELINE:0] <= {div_token[(0-1<<<1)+NPIPELINE:0],shift_dv};
        else div_token <= div_token;
      end
  assign out_dv = dv_s & div_token[(0-1)+NPIPELINE];
  wire [23:0] out_div;
...
.lpm_hint(LPM_REMAINDERPOSITIVE=TRUE)) lpm_divide_inst(.quotient(out_div),
                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                     .clken(dv_s & (shift_dv | div_token[(0-1)+NPIPELINE])),.aclr(1'b0));
