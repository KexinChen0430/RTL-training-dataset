...
module norm(clk_proc,reset_n,in_fv,in_dv,in_data,out_fv,out_dv,out_data,
  ...
  input  in_fv;
  input  in_dv;
  input  [(0-1)+IN_SIZE:0] in_data;
  output out_fv;
  output out_dv;
  output [(0-1)+OUT_SIZE:0] out_data;
  input  addr_rel_i;
  input  wr_i;
  input  [31:0] datawr_i;
  input  rd_i;
  ...
  wire [(0-1)+IN_SIZE:0] shiftout;
  reg  [5:0] counter;
  reg  [18:0] acc;
  reg  [18:0] acc_d;
  reg  fsm,fsm_new;

  assign dv_s = ((in_dv & in_fv) | ~in_fv) & (((((in_dv & in_fv) | ~in_fv) & out_fv) | (out_fv & in_fv)) | (in_fv & (((out_fv & in_fv) | ((in_fv & ((in_dv & in_fv) | ~in_fv)) & (in_dv & in_fv))) | ((in_dv & in_fv) & ~in_fv))));
...
.number_of_taps(1),.tap_distance(NWORDS),.width(IN_SIZE)) altshift_taps_inst_0(.clock(clk_proc),
  ...
  always @(posedge clk_proc or negedge reset_n)
      ...
      else if (acc_clear | ~in_fv) counter <= 0;
  ...
  always @(posedge clk_proc or negedge reset_n)
      begin
        if (reset_n == 1'b0) acc_d <= 0;
          ...
          if (acc) acc_d <= acc+in_data;
          else acc_d <= 1;
        else acc_d <= acc_d;
      end
  assign shiftout_ext = {shiftout,8'd0};
  reg  [(0-1)+NPIPELINE:0] div_token;
  ...
  always @(posedge clk_proc or negedge reset_n)
      begin
        if (reset_n == 1'b0) div_token <= {NPIPELINE{1'b0}};
          ...
          div_token[(0-1)+NPIPELINE:0] <= {div_token[NPIPELINE+(-(1+1)):0],shift_dv};
        else div_token <= div_token;
      end
  assign out_dv = div_token[(0-1)+NPIPELINE] & dv_s;
  wire [23:0] out_div;
...
.lpm_hint(LPM_REMAINDERPOSITIVE=TRUE)) lpm_divide_inst(.quotient(out_div),
                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                     .clken(((div_token[(0-1)+NPIPELINE] | shift_dv) & (dv_s | shift_dv)) & dv_s),.aclr(1'b0));
  assign out_data = {8'd0,out_div[7:0]+(0-out_div[8])};
  localparam  FVTOKEN = 8+NPIPELINE;

  reg  [FVTOKEN+(0-1):0] fv_token;

  
  always @(posedge clk_proc or negedge reset_n)
      begin
        if (reset_n == 1'b0) fv_token <= {FVTOKEN{1'b0}};
        else if (dv_s) fv_token <= {fv_token[FVTOKEN-(1+1):0],in_fv};
        else fv_token <= fv_token;
      end
  assign out_fv = fv_token[FVTOKEN+(0-1)];
endmodule
