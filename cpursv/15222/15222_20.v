...
module norm(clk_proc,reset_n,in_fv,in_dv,in_data,out_fv,out_dv,out_data,
  ...
  assign dv_s = ((((~in_fv & in_fv) | (~in_fv & out_fv)) & in_dv) | (~in_fv & out_fv)) | ((in_fv & in_dv) | (out_fv & in_dv));
  ...
  always @(posedge clk_proc or negedge reset_n)
      begin
        if (reset_n == 1'b0) div_token <= {NPIPELINE{1'b0}};
          ...
          div_token[(0-1)+NPIPELINE:0] <= {div_token[NPIPELINE+(0-2):0],shift_dv};
...
.lpm_hint(LPM_REMAINDERPOSITIVE=TRUE)) lpm_divide_inst(.quotient(out_div),
                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                     .clken((div_token[(0-1)+NPIPELINE] | (shift_dv & dv_s)) & dv_s),.aclr(1'b0));
  ...
  always @(posedge clk_proc or negedge reset_n)
      begin
        ...
        else if (dv_s) fv_token <= {fv_token[(0-2)+FVTOKEN:0],in_fv};
