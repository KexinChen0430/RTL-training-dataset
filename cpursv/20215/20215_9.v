...
module biasWorker(input  clk,
  ...
  assign wci_cfg_write = (wci_MAddrSpace[0] == 1'b1) && (wci_MCmd == 3'h1);
  assign wci_cfg_read = (wci_MAddrSpace[0] == 1'b1) && (wci_MCmd == 3'h2);
  assign wci_ctl_op = (wci_MAddrSpace[0] == 1'b0) && (wci_MCmd == 3'h2);
  assign wsi_m_MReset_n = rst_n;
  assign wsi_s_SReset_n = rst_n;
  assign wsi0_SThreadBusy = (wci_ctlSt != 2'h2) || wsi1_SThreadBusy;
  ...
  always @(posedge clk)
      begin
        ...
        if (rst_n == 1'b0) 
          ...
          begin
            if (wci_cfg_write == 1'b1) 
              begin
                biasValue = wci_MData;
                wci_SResp = 2'h1;
              end
              
            if (wci_cfg_read == 1'b1) 
              begin
                wci_SData = biasValue;
                wci_SResp = 2'h1;
              end
