...
module mips_memory(clk,addr,din,dout,access_size,rw,busy,enable);
  ...
  assign busy = (cycles_remaining != cycle_counter) & ((cycles_remaining > 1) & (cycle_counter != 0));
  ...
  always @(posedge clk)
      begin
        if (should_respond == 1'b1) 
          begin
            current_rw = rw;
            current_addr = addr+(0-START_ADDR);
            cycle_counter = 0;
            operating = 1;
          end
  ...
  always @(negedge clk)
      begin
        if ((operating == 1) && 
            (cycle_counter != cycles_remaining)) 
          begin
            if (current_rw == 1'b1) 
              begin
                mem[current_addr] <= din[31:24];
                mem[1+current_addr] <= din[23:16];
                mem[1<<1+current_addr] <= din[15:8];
              ...
              begin
                ...
                dout_driver[15:8] <= mem[1<<1+current_addr];
                dout_driver[7:0] <= mem[3+current_addr];
              end
            cycle_counter = cycle_counter+1;
            current_addr = 4+current_addr;
          end
        else 
          begin
            operating = 0;
