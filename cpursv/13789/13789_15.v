...
module rfid_reader_rx(reset,clk,tag_backscatter,rx_done,rx_timeout,miller,trext,
                      divide_ratio,rtcal_counts,trcal_counts,tari_counts,rx_data,
                      rx_dataidx);
  ...
  assign rx_timeout = rx_counter > (rtcal_counts*(2**2));
  reg  previousbit;
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset) 
          ...
          begin
            if (tag_backscatter != previousbit) 
              begin
                edgeclk <= 1;
  ...
  always @(posedge edgeclk or posedge reset)
      begin
        if (reset) 
          begin
            rx_state <= 0;
          ...
          begin
            case (rx_state)
              ...
              STATE_CLK_DN: begin
                    if (~trext & isfm0) rx_period <= rx_counter_by2;
