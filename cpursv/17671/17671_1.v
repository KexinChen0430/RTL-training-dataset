...
module eth_random(MTxClk,Reset,StateJam,StateJam_q,RetryCnt,NibCnt,ByteCnt,
  ...
  always @(posedge MTxClk or posedge Reset)
      begin
        if (Reset) x[9:0] <= 0;
        else x[9:0] <= {x[8:0],Feedback};
      end
  assign Feedback = ~(((((x[9] & ~(x[2] ^ (x[2] & x[9]))) & ~(~(x[2] ^ (x[2] & x[9])) & (x[9] & (x[2] & (x[9] | ~x[9]))))) | (~(x[9] & ~(x[2] ^ (x[2] & x[9]))) & (~(x[2] ^ (x[2] & x[9])) & (x[9] & (x[2] & (x[9] | ~x[9])))))) | (x[2] ^ (x[2] & x[9]))) & ((((x[9] & ~(x[2] ^ (x[2] & x[9]))) & ~(~(x[2] ^ (x[2] & x[9])) & (x[9] & (x[2] & (x[9] | ~x[9]))))) | (~(x[9] & ~(x[2] ^ (x[2] & x[9]))) & (~(x[2] ^ (x[2] & x[9])) & (x[9] & (x[2] & (x[9] | ~x[9])))))) | ~(((~((x[9] & (x[2] & (x[9] | ~x[9]))) & (x[2] | x[9])) & x[9]) | ~x[9]) & ((~((x[9] & (x[2] & (x[9] | ~x[9]))) & (x[2] | x[9])) & x[9]) | ((x[9] & (x[2] & (x[9] | ~x[9]))) & (x[2] | x[9]))))));
  assign Random[0] = x[0];
  ...
  always @(posedge MTxClk or posedge Reset)
      begin
        if (Reset) RandomLatched <= 10'h000;
        else 
          begin
            if (StateJam & StateJam_q) RandomLatched <= Random;
              
          end
      end
  assign RandomEq0 = RandomLatched == 10'h0;
  assign RandomEqByteCnt = ByteCnt[9:0] == (&NibCnt[6:0] & RandomLatched);
endmodule
