...
module debug_lsa(input  av_clk,
  ...
  always @(posedge av_clk or posedge av_rst)
      if (av_rst) 
        ...
        begin
          ...
          sync_av <= (sync_lsa_av == sync_av) ? (2'd1+sync_av) : sync_av;
        end
  ...
  always @(posedge lsa_clk)
      sample_done <= (sample_waddr == 11'h000) && lsa_arm;
  
  always @(posedge lsa_clk)
      if (!lsa_arm) sample_waddr <= 11'h001;
      else if (|sample_waddr[9:0] && !sample_waddr[10]) sample_waddr <= sample_waddr+11'd1;
      else if (sample_waddr == 11'h400) 
        sample_waddr <= (lsa_trigger || lsa_force) ? 11'h401 : 11'h400;
      else if (sample_waddr != 11'h000) sample_waddr <= 10'd1+sample_waddr;
  ...
  always @(posedge av_clk)
      ...
      else if (av_address == 10'd1) av_readdata <= av_live;
      else av_readdata <= sample_data[av_address];
endmodule
