...
module lcd_sync(input  rst,
  ...
  wire 
       lcd_hsync_end = (lcd_hsync_i == 0) && (lcd_hsync_q == 1);

  wire 
       lcd_vsync_end = (lcd_vsync_q == 0) && (lcd_vsync_i == 1);

  
  always @(posedge lcd_pclk_i)
      begin
        if (rst) 
          begin
            lcd_x_q <= 0;
