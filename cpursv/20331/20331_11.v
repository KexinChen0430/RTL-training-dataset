...
module shift_division_result(quotient,fraction,res);
  ...
  parameter signed  [31:0]  vec_MSB = f_width+(q_width-1);
  parameter signed  [31:0]  result_MSB = (-1)+(q_width+fraction_width);
  parameter signed  [31:0]  result_LSB = vec_MSB+(0-result_MSB);
          ...
          for (i = vec_MSB; i >= 0; i = (-1)+i)
              begin : u0
                if (i < shift_value) 
                  ...
                  begin : u2
                    assign vec[i] = temp[i+(0-shift_value)];
                  end
              end
        end
      else 
