...
module alt_vipitc131_IS2Vid_calculate_mode(input  [3:0] trs,
  ...
  assign v_total = is_v_blank+((is_interlaced ? is_v1_blank : 16'd0)+v_active_lines);
  ...
  assign h_total_minus_one_nxt = (-16'd1)+(is_sample_count_f0+is_h_blank);
  ...
  assign f_rising_edge_nxt = (0-is_ap_line)+is_f_rising_edge;
  assign f_falling_edge_nxt = v_total+(-(is_ap_line-is_f_falling_edge));
  assign total_line_count_f0_nxt_full = (((-16'd1)+is_line_count_f0)+(is_v_blank-is_v_front_porch))+is_v1_front_porch;
  assign total_line_count_f1_nxt_full = (is_line_count_f1+(-is_v1_front_porch))+((is_v1_blank+(-16'd1))+is_v_front_porch);
  ...
  assign f2_anc_v_start_nxt = v_total+(0-(is_ap_line-is_anc_line));
