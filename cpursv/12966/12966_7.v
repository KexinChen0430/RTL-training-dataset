...
module multiplier(clk,sign,enable,multi_1,multi_2,product,multready);
  ...
  always @(posedge clk)
      begin
        if ((i == 7'd32) & enable) 
          begin
            multi_2_copy <= ((sign || multi_1[31]) && multi_2[31] && sign) ? {32'd0,~multi_2+1'b1} : {32'd0,multi_2};
            multi_1_copy <= (multi_1[31] && sign) ? (~multi_1+1'b1) : multi_1;
            neg_output <= 
(multi_1[31] || 
((multi_2[31] || multi_1[31]) && ~multi_1[31])) && (sign || multi_1[31]) && 
                         (
((~multi_2[31] && (sign || multi_1[31]) && sign) || 
~multi_1[31]) && (sign && (multi_2[31] || ~multi_2[31])));
            i <= (-1'b1)+i;
          end
        else if ((enable & i) > 0) 
          begin
