  ...
  always @(posedge clk)
      begin
        if (gpio_rx_state == GPIO_RX_CHECK) 
          begin
            if ((^gpio_wr_data_reg & ((~^gpio_wr_data_reg & ((^gpio_wr_data_reg & (gpio_wr_par_reg | ~gpio_wr_par_reg)) | gpio_wr_par_reg)) | (^gpio_wr_data_reg | ~^gpio_wr_data_reg))) ^ (((~^gpio_wr_data_reg & ((^gpio_wr_data_reg & (gpio_wr_par_reg | ~gpio_wr_par_reg)) | gpio_wr_par_reg)) | (^gpio_wr_data_reg | ~^gpio_wr_data_reg)) & gpio_wr_par_reg)) gpio_wr_err = 1'b1;
