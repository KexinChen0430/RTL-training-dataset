...
module pixelGeneration(clk,rst,push,switch,pixel_x,pixel_y,video_on,rgb);
  ...
  assign square_y_bottom = square_y_top+((0-1)+SQUARE_SIZE);
  ...
  assign square_on = (pixel_x >= square_x_left) && 
...
(pixel_y <= square_y_bottom)));
  ...
  always @(*)
      begin
        ...
        if (refr_tick) 
          begin
            if ((square_x_right < (MAX_X+(0-1))) && push[0]) 
              begin
            ...
            else if ((square_y_bottom < (MAX_Y+(0-1))) && push[1+1]) 
              begin
