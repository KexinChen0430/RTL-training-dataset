...
module pixelGeneration(clk,rst,push,switch,pixel_x,pixel_y,video_on,rgb);
  ...
  assign square_y_bottom = SQUARE_SIZE+(square_y_top+(-1));
  assign square_x_right = ((-1)+square_x_left)+SQUARE_SIZE;
...
(pixel_x <= square_x_right)) && (pixel_y <= square_y_bottom);
  ...
  always @(*)
      begin
        ...
        if (refr_tick) 
          begin
            if ((square_x_right < (MAX_X-1)) && push[0]) 
              begin
                square_x_next = SQUARE_VEL+square_x_reg;
              end
              ...
              begin
                square_y_next = square_y_reg+(0-SQUARE_VEL);
              end
