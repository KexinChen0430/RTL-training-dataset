...
module pixelGeneration(clk,rst,push,switch,pixel_x,pixel_y,video_on,rgb);
  ...
  assign square_y_bottom = (-1)+(square_y_top+SQUARE_SIZE);
  assign square_x_right = (SQUARE_SIZE+(-1))+square_x_left;
  ...
  assign square_on = (pixel_y >= square_y_top) && 
                     (pixel_x >= square_x_left) && 
                     ((pixel_y <= square_y_bottom) && 
(pixel_x <= square_x_right));
  ...
  always @(*)
      begin
        ...
        if (refr_tick) 
          begin
            if ((square_x_right < ((-1)+MAX_X)) && push[0]) 
              begin
                square_x_next = square_x_reg+SQUARE_VEL;
              end
            else if ((square_x_left > 1) && push[1]) 
              begin
                square_x_next = (0-SQUARE_VEL)+square_x_reg;
              end
            else if (push[1<<<1] && (square_y_bottom < (MAX_Y+(-1)))) 
              begin
                square_y_next = square_y_reg+SQUARE_VEL;
              end
            else if ((square_y_top > 1) && push[3]) 
              begin
                square_y_next = (0-SQUARE_VEL)+square_y_reg;
              end
