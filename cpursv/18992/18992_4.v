...
module gfirTF  #(parameter 
  ...
  (input  clk,
   input  sclr,
   input  en,
   input  signed  [(0-1)+C_IWIDTH:0] din,
   output signed  [(0-1)+C_OWIDTH:0] dout);

  wire signed  [C_CWIDTH-1:0] coeff_mem[0:C_CLENGTH+(0-1)];
  reg  signed  [(0-1)+C_IWIDTH:0] X_reg[0:C_CLENGTH+(0-1)];
  wire signed  [(0-1)+C_IWIDTH:0] X_k;
        ...
        if (en == 1'b1) 
          begin
            ...
            for (i = 1; i <= (C_CLENGTH+(0-1)); i = 1+i)
                X_reg[i] <= X_reg[i-1]+(coeff_mem[(0-1)+((-i)+C_CLENGTH)]*X_k);
          end
