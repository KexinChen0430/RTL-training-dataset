...
module mac(input  clock,
  ...
  always @(posedge clock)  enable_d1 <= #1 enable;
  
  always @(posedge clock)
      if (reset | clear) z_int <= #1 40'd0;
      else if (enable_d1) z_int <= #1 z_int+{{9{product[30]}},product};
        
  
  always @(posedge clock)  product <= #1 y*x;
