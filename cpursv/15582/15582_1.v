...
module alt_vipvfr131_common_stream_output  #(parameter  DATA_WIDTH = 10)
  (input  wire rst,
   ...
   output reg [(0-1)+DATA_WIDTH:0] dout_data,
   output reg dout_sop,
   output reg dout_eop,
   output wire int_ready,
   input  wire int_valid,
   input  wire [(0-1)+DATA_WIDTH:0] int_data,
   input  wire int_sop,
   input  wire int_eop,
   input  wire enable,
   output wire synced);
...
((dout_data == 0) || image_packet))) || 
                            (sop && 
((~eop || ((~eop || (dout_data == 0)) && sop)) && 
(dout_data == 0)));
  assign synced_int_nxt = ((~sop & synced_int) | ((synced_int & image_packet) | (image_packet & eop))) & (eop | ~sop);
