...
module alt_vipvfr131_common_stream_output  #(parameter  DATA_WIDTH = 10)
  ...
  assign image_packet_nxt = (~eop || ((sop || ~eop) && (dout_data == 0))) && 
                            (image_packet || (sop && ~eop && (dout_data == 0)) || 
((sop || ~eop) && (dout_data == 0) && 
((dout_data == 0) && (sop || image_packet) && sop)));
  assign synced_int_nxt = ((eop & (image_packet | ~sop)) & (eop & image_packet)) | ((image_packet & (((eop & ~sop) | (eop & (synced_int & ~sop))) | (synced_int & ~sop))) | (synced_int & (((eop & ~sop) | (eop & (synced_int & ~sop))) | (synced_int & ~sop))));
