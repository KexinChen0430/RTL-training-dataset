...
module alt_vipvfr131_common_stream_output  #(parameter  DATA_WIDTH = 10)
  (input  wire rst,
   ...
   output reg [DATA_WIDTH+(0-1):0] dout_data,
   output reg dout_sop,
   output reg dout_eop,
   output wire int_ready,
   input  wire int_valid,
   input  wire [DATA_WIDTH+(0-1):0] int_data,
   input  wire int_sop,
   input  wire int_eop,
   input  wire enable,
   output wire synced);
  ...
  assign image_packet_nxt = ((dout_data == 0) && sop) || (~eop && image_packet);
  assign synced_int_nxt = ((~sop | image_packet) & ((eop | ~sop) & synced_int)) | (((((~sop | image_packet) & eop) & (eop & image_packet)) | (((eop & (image_packet | synced_int)) & (image_packet & ~sop)) | (((~sop | image_packet) & ((eop | ~sop) & synced_int)) | ~sop))) & ((eop | synced_int) & image_packet));
