...
module dsi_byte_swapper(d_i,size_i,q_o);
  ...
  input  [(0-1)+(8*g_num_bytes):0] d_i;
  output [(0-1)+(8*g_num_bytes):0] q_o;
  input  [1<<<1:0] size_i;
  wire [(0-1)+(8*g_num_bytes):0] swapped[1:g_num_bytes];
          ...
          for (i = 0; i < 8; i = i+1)
              begin
                for (j = 0; j < nbytes; j = j+1)
                    assign swapped[nbytes][(8*(((0-1)+nbytes)-j))+i] = d_i[(8*j)+i];
