...
module tx_reset_sm(input  refclkdiv2,
                   input  rst_n,
  ...
  reg  [(0-1)+STATEWIDTH:0] cs;
  reg  [(0-1)+STATEWIDTH:0] ns;
  ...
  reg  [TIMER1WIDTH+(0-1):0] counter1;
  ...
  reg  [(0-1)+TIMER2WIDTH:0] counter2;
  ...
  always @(posedge refclkdiv2 or posedge reset_timer2)
      begin
        if (reset_timer2) 
          ...
          begin
            if (counter2[count_index] == 1) TIMER2 <= 1;
            else 
              begin
                TIMER2 <= 0;
                counter2 <= counter2+1;
      ...
      begin : NEXT_STATE
        ...
        case (cs)
          ...
          WAIT_FOR_TIMER1: begin
                tx_pcs_rst_ch_c_int = 4'hF;
                rst_qd_c_int = 1;
                if (TIMER1) ns = CHECK_PLOL;
                else ns = WAIT_FOR_TIMER1;
              end
          ...
          WAIT_FOR_TIMER2: begin
                ...
                rst_qd_c_int = 0;
                if (TIMER2) 
                  if (tx_pll_lol_qd_s_int) ns = QUAD_RESET;
                  else ns = NORMAL;
                else ns = WAIT_FOR_TIMER2;
              end
