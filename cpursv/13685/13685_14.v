...
module axi_ad7980_dev_if(input  fpga_clk_i,
                         input  adc_clk_i,
                         input  reset_i,
                         output [15:0] data_o,
  ...
  parameter real  ADC_CYCLE_TIME = 1e-06;
  parameter real  ADC_CONV_TIME = 1e-06;
  parameter [6:0] 
       ADC_CYCLE_CNT = (ADC_CYCLE_TIME*FPGA_CLOCK_FREQ)-1;
  parameter [6:0]  ADC_CNV_CNT = ADC_CONV_TIME*FPGA_CLOCK_FREQ;
  parameter  ADC_SCLK_PERIODS = 5'd15;

  assign adc_cnv_o = adc_cnv_s;
  assign adc_sclk_s = adc_clk_en & adc_clk_i;
      ...
      begin
        if (reset_i == 1'b1) 
          ...
          begin
            if (adc_tcycle_cnt != 0) 
              begin
                adc_tcycle_cnt <= (0-1)+adc_tcycle_cnt;
              end
  ...
  always @(negedge adc_clk_i)
      begin
        ...
        adc_clk_en <= (
((adc_state_m1 == ADC_END_CNV_STATE) || 
(sclk_clk_cnt != 0)) && 
                     ((adc_state_m1 == ADC_READ_CNV_RESULT) || 
(adc_state_m1 == ADC_END_CNV_STATE))) ? 1'b1 : 1'b0;
      end
  ...
  always @(adc_state or adc_tcycle_cnt or adc_tcnv_cnt or sclk_clk_cnt)
      begin
