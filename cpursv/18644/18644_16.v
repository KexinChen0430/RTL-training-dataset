...
module image_processing_module(input  clk,
                               ...
                               output [addr_width+(0-1):0] ram_addr);
  ...
  reg  [addr_width+(0-1):0] reg_ram_addr;
  reg  [3:0] con_ready;
      ...
      if (wr_mode == 0) 
        begin
          ...
          always @(posedge clk or negedge rst_n or negedge in_enable)
              begin
                if (~in_enable || ~rst_n) reg_ram_addr <= im_width*row_init;
                else if (reg_ram_addr == ((0-1)+(im_width*im_height))) reg_ram_addr <= 0;
