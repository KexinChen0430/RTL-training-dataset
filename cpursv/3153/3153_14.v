...
module matrix_prod(clk_i,reset_n_i,sclr_i,pix01_i,pix10_i,pix12_i,pix21_i,sin_i,
  ...
  input  [(-1)+COEF_WIDTH:0] sin_i;
  input  [(-1)+COEF_WIDTH:0] cos_i;
  output [(DATA_WIDTH+1)+COEF_WIDTH:0] data_o;
  ...
  reg  signed  [(DATA_WIDTH+1)+COEF_WIDTH:0] finaladd;
  wire signed  [(-1)+COEF_WIDTH:0] sin_wire;
  wire signed  [(-1)+COEF_WIDTH:0] cos_wire;

  
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) tmpsin <= {DATA_WIDTH+1{1'b0}};
      else if (dv0_i) tmpsin <= pix21_i+(0-pix01_i);
      else tmpsin <= tmpsin;
  
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) tmpcos <= {DATA_WIDTH+1{1'b0}};
      else if (dv0_i) tmpcos <= pix10_i-pix12_i;
      else tmpcos <= tmpcos;
  ...
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) 
        begin
          multsin <= {(DATA_WIDTH+1)+COEF_WIDTH{1'b0}};
          multcos <= {(DATA_WIDTH+1)+COEF_WIDTH{1'b0}};
        end
      else if (dv1_i) 
        begin
  ...
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) 
        begin
          finaladd <= {((DATA_WIDTH+1)+COEF_WIDTH)+1{1'b0}};
        end
      else if (dv2_i) 
        begin
