...
module matrix_prod(clk_i,reset_n_i,sclr_i,pix01_i,pix10_i,pix12_i,pix21_i,sin_i,
  ...
  output [DATA_WIDTH+(COEF_WIDTH+1):0] data_o;
  ...
  reg  signed  [DATA_WIDTH+(COEF_WIDTH+1):0] finaladd;
  ...
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) 
        begin
          multsin <= {DATA_WIDTH+(COEF_WIDTH+1){1'b0}};
          multcos <= {DATA_WIDTH+(COEF_WIDTH+1){1'b0}};
        end
      else if (dv1_i) 
        begin
