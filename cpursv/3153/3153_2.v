...
module matrix_prod(clk_i,reset_n_i,sclr_i,pix01_i,pix10_i,pix12_i,pix21_i,sin_i,
  ...
  output [1+(COEF_WIDTH+DATA_WIDTH):0] data_o;
  ...
  reg  signed  [1+(COEF_WIDTH+DATA_WIDTH):0] finaladd;
  ...
  always @(posedge clk_i or negedge reset_n_i)
      ...
      else if (dv0_i) tmpcos <= (0-pix12_i)+pix10_i;
      else tmpcos <= tmpcos;
  ...
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) 
        begin
          multsin <= {1+(COEF_WIDTH+DATA_WIDTH){1'b0}};
          multcos <= {1+(COEF_WIDTH+DATA_WIDTH){1'b0}};
        end
      else if (dv1_i) 
        begin
  ...
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) 
        begin
          finaladd <= {COEF_WIDTH+(2+DATA_WIDTH){1'b0}};
        end
      else if (dv2_i) 
        begin
