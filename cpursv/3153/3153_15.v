...
module matrix_prod(clk_i,reset_n_i,sclr_i,pix01_i,pix10_i,pix12_i,pix21_i,sin_i,
  ...
  input  [COEF_WIDTH+(0-1):0] sin_i;
  input  [COEF_WIDTH+(0-1):0] cos_i;
  ...
  wire signed  [COEF_WIDTH+(0-1):0] sin_wire;
  wire signed  [COEF_WIDTH+(0-1):0] cos_wire;
  ...
  always @(posedge clk_i or negedge reset_n_i)
      if (reset_n_i == 0) 
        begin
          finaladd <= {((DATA_WIDTH+COEF_WIDTH)+1)+1{1'b0}};
        end
      else if (dv2_i) 
        begin
