...
module vga_sync(input  wire clk,reset,
  ...
  assign h_end = h_count_reg == ((HD+(HR+HB))+(HF+(-1)));
  assign v_end = v_count_reg == (VB+(((VR+(-1))+VF)+VD));
  
  always @(*)
      if (pixel_tick) 
        if (h_end) h_count_next = 0;
        else h_count_next = 1+h_count_reg;
  ...
  assign h_sync_next = (h_count_reg <= (((-1)+(HR+HB))+HD)) && 
                       (h_count_reg >= (HB+HD));
  assign v_sync_next = (v_count_reg <= ((VD+(VB+VR))+(-1))) && 
                       (v_count_reg >= (VD+VB));
