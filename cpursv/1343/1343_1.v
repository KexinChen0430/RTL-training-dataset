...
module vga_sync(input  wire clk,reset,
  ...
  assign h_end = h_count_reg == (((HB+HF)+(HR+(-1)))+HD);
  assign v_end = v_count_reg == (VF+(VB+(VD+(VR+(-1)))));
  ...
  assign h_sync_next = (h_count_reg <= ((HD+(-1))+(HB+HR))) && 
                       (h_count_reg >= (HB+HD));
  assign v_sync_next = (v_count_reg <= (VB+(VD+(VR+(-1))))) && 
                       (v_count_reg >= (VD+VB));
