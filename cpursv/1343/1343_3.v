...
module vga_sync(input  wire clk,reset,
  ...
  assign h_end = h_count_reg == (HB+((HD+(HF+(-1)))+HR));
  assign v_end = v_count_reg == (((VR+VB)+((-1)+VD))+VF);
  ...
  assign h_sync_next = (h_count_reg <= ((HD+(HB+(-1)))+HR)) && 
                       (h_count_reg >= (HD+HB));
  assign v_sync_next = (v_count_reg <= ((VR+VB)+((-1)+VD))) && 
                       (v_count_reg >= (VD+VB));
