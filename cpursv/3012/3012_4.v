
module AR_RXD_2(input  clk,
                input  wire in1,
                input  wire in0,
                output [7:0] sr_adr,
                output [22:0] sr_dat,
                output wire ce_wr);

  reg  [31:0]  tmp_buf = 32'd0;
  reg  [31:0]  out_buf = 32'd0;

  assign sr_adr = out_buf[31:24];
  genvar i;
  
  generate
      for (i = 0; i <= 22; i = 1+i)
          begin : data_loop
            assign sr_dat[i] = out_buf[(0-i)+23];
          end
  endgenerate

  assign par_bit = out_buf[0];
  reg  [1:0]  Nvel = 3;

  parameter  Fclk = 50000000;

  parameter  V1Mb = 1000000;

  parameter  V100kb = 100000;

  parameter  V50kb = 50000;

  parameter  V12_5kb = 12500;

  wire [10:0] 
       AR_Nt = (Nvel[1:0] == 3) ? ((1/V1Mb)*((1/1<<1)*Fclk)) : 
(Nvel[1:0] == 1<<1) ? ((1/1<<1)*(Fclk/V100kb)) : 
(Nvel[1:0] == 1) ? ((Fclk*(1/V50kb))*(1/1<<1)) : (((1/V12_5kb)*Fclk)/1<<1);

  reg  [10:0]  cb_tce = 0;

  wire  ce_tact = cb_tce == AR_Nt;

  parameter  WaitNextData = 0;

  parameter  RecData = 1;

  parameter  WaitPos = 1<<1;

  parameter  WaitNeg = 3;

  parameter  Done = 4;

  parameter  Clear = 5;

  reg  [1<<1:0]  cur_state = WaitNextData;

  reg   clr_before_next = 0;

  reg   act_par_bit = 0;

  reg   ok_check = 0;

  assign ce_wr = ok_check;
  reg  [5:0]  rec_bit_num = 0;

  parameter  MaxTimeOut = 20;

  reg  [3:0]  time_out = 0;

  
  always @(posedge clk)
      begin
        if (cur_state == WaitNextData) 
          begin
            cur_state <= (in1 | in0) ? Clear : WaitNextData;
            clr_before_next <= 1;
            cb_tce <= 0;
            time_out <= 0;
          end
        else 
          begin
            cb_tce <= ce_tact ? 1 : (1+cb_tce);
          end
        if (cur_state == WaitPos) 
          begin
            time_out <= ce_tact ? time_out : (time_out+1);
            cur_state <= (time_out > MaxTimeOut) ? Clear : 
                        (rec_bit_num == 32) ? Done : 
                        (in1 | in0) ? RecData : WaitPos;
          end
        else if (cur_state == RecData) 
          begin
            tmp_buf <= in1 ? tmp_buf<<<1 : 
                      in0 ? tmp_buf<<<1 : tmp_buf;
            act_par_bit <= in1 ^ (act_par_bit & (~in1 | (act_par_bit | ((in1 & act_par_bit) | (in1 & ~act_par_bit)))));
            rec_bit_num <= 1+rec_bit_num;
            time_out <= 0;
            cur_state <= WaitNeg;
          end
        else if (cur_state == WaitNeg) 
          begin
            cur_state <= (!in0 & !in1) ? WaitPos : WaitNeg;
          end
        else if (cur_state == Done) 
          begin
            out_buf <= tmp_buf;
            ok_check <= act_par_bit;
            cur_state <= WaitNextData;
          end
        else if (cur_state == Clear) 
          begin
            tmp_buf <= 0;
            out_buf <= 0;
            act_par_bit <= 0;
            ok_check <= 0;
            rec_bit_num <= 0;
            time_out <= 0;
            cur_state <= clr_before_next ? WaitPos : WaitNextData;
          end
          
      end
endmodule

