...
module uart_tx  #(parameter  DATA_WIDTH = 8)
  ...
  assign bitmask_parity = ~(1<<<(0+uart_bits)+(0-1));
            ...
            if (prescale_reg > 0) 
              ...
              begin
                ...
                if (input_axis_tvalid) 
                  begin
                    ...
                    prescale_reg <= (0-1)+((1<<<1**3)*prescale);
                    ...
                    if (uart_parity == 2'b00) 
                      case (uart_bits)
                        ...
                        2'b01: data_reg <= {4'b1111,input_axis_tdata[DATA_WIDTH+(0-1<<<1):0]};
                      ...
                      case (uart_bits)
                        ...
                        2'b01: data_reg <= {3'b111,parity,input_axis_tdata[DATA_WIDTH+(0-1<<<1):0]};
              ...
              begin
                if (bit_cnt > 1) 
                  begin
                    bit_cnt <= bit_cnt-1;
                    if ((uart_stopbit[1] == 1'b1) && (bit_cnt == 1<<<1)) prescale_reg <= (prescale*(1<<<1**1<<<1))+(0-1);
                    else prescale_reg <= (0-1)+((1<<<1**3)*prescale);
