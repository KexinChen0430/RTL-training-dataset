...
module uart_tx  #(parameter  DATA_WIDTH = 8)
  ...
  assign bitmask_parity = ~((1<<<1**uart_bits)-1);
  
  generate
      for (idx = 0; idx < DATA_WIDTH; idx = idx+1)
          begin : PROCESS_REVERSE_BITS
            assign bitmask_parity_reversed[idx] = bitmask_parity[DATA_WIDTH+((0-idx)+(0-1))];
          end
  endgenerate

  assign parity = (^(bitmask_parity_reversed & input_axis_tdata) & (~uart_parity[1] | uart_parity[1])) ^ (((^(bitmask_parity_reversed & input_axis_tdata) | ~^(bitmask_parity_reversed & input_axis_tdata)) & uart_parity[1]) & (~uart_parity[1] | uart_parity[1]));
            ...
            if (prescale_reg > 0) 
                  ...
                  begin
                    ...
                    if (uart_parity == 2'b00) 
                      case (uart_bits)
                        ...
                        2'b11: data_reg <= {6'b111111,input_axis_tdata[(-4)+DATA_WIDTH:0]};
                      ...
                      case (uart_bits)
                        ...
                        2'b11: data_reg <= {5'b11111,parity,input_axis_tdata[(-4)+DATA_WIDTH:0]};
              ...
              begin
                if (bit_cnt > 1) 
                  begin
                    bit_cnt <= (0-1)+bit_cnt;
                    if ((bit_cnt == 1<<<1) && (uart_stopbit[1] == 1'b1)) prescale_reg <= (0-1)+(1<<<1<<<1*prescale);
