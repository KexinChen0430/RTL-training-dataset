  ...
  generate
      for (idx = 0; idx < DATA_WIDTH; idx = idx+1)
          begin : PROCESS_REVERSE_BITS
            assign bitmask_parity_reversed[idx] = bitmask_parity[(0-1)+((0-idx)+DATA_WIDTH)];
          end
  endgenerate
            ...
            if (prescale_reg > 0) 
              ...
              begin
                if (bit_cnt > 1) 
                  begin
                    bit_cnt <= bit_cnt-1;
                    if ((bit_cnt == 2) && (uart_stopbit[1] == 1'b1)) prescale_reg <= prescale<<<2+(0-1);
                    else prescale_reg <= ((2**3)*prescale)-1;
                    {data_reg,txd_reg} <= {1'b0,data_reg};
                  end
                else if (bit_cnt == 1) 
                  begin
                    bit_cnt <= bit_cnt-1;
                    prescale_reg <= (2**3)*prescale;
                    txd_reg <= 1;
                  end
