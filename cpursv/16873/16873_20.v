...
module uart_tx  #(parameter  DATA_WIDTH = 8)
  ...
  assign bitmask_parity = ~((-1)+1<<uart_bits);
  
  generate
      for (idx = 0; idx < DATA_WIDTH; idx = 1+(0+idx))
          begin : PROCESS_REVERSE_BITS
            assign bitmask_parity_reversed[idx] = bitmask_parity[((-1)+(-idx))+DATA_WIDTH];
          end
  endgenerate

  assign parity = (^(input_axis_tdata & bitmask_parity_reversed) | (uart_parity[1] & ~^(input_axis_tdata & bitmask_parity_reversed))) & (~uart_parity[1] | (uart_parity[1] & ~^(input_axis_tdata & bitmask_parity_reversed)));
            ...
            if (prescale_reg > 0) 
                  ...
                  begin
                    ...
                    if (uart_parity == 2'b00) 
                      case (uart_bits)
                        ...
                        2'b01: data_reg <= {4'b1111,input_axis_tdata[(-1<<1)+DATA_WIDTH:0]};
                      ...
                      case (uart_bits)
                        ...
                        2'b01: data_reg <= {3'b111,parity,input_axis_tdata[(-1<<1)+DATA_WIDTH:0]};
              ...
              begin
                if (bit_cnt > 1) 
                  begin
                    bit_cnt <= bit_cnt-1;
                    if ((bit_cnt == 1<<1) && (uart_stopbit[1] == 1'b1)) prescale_reg <= (-1)+(((1<<1+1)+1)*prescale);
