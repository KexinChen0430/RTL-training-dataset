...
module uart_tx  #(parameter  DATA_WIDTH = 8)
  ...
  assign parity = ^(input_axis_tdata & bitmask_parity_reversed) ^ (uart_parity[1] & (^(input_axis_tdata & bitmask_parity_reversed) | ~^(input_axis_tdata & bitmask_parity_reversed)));
            ...
            if (prescale_reg > 0) 
              ...
              begin
                if (bit_cnt > 1) 
                  begin
                    bit_cnt <= bit_cnt-1;
                    if ((uart_stopbit[1] == 1'b1) && (bit_cnt == 2)) prescale_reg <= (((2+1)+1)*prescale)-1;
