...
module uart_tx  #(parameter  DATA_WIDTH = 8)
  ...
  assign parity = (~uart_parity[1] & (((~^(bitmask_parity_reversed & input_axis_tdata) | ^(bitmask_parity_reversed & input_axis_tdata)) & uart_parity[1]) | ^(bitmask_parity_reversed & input_axis_tdata))) | (uart_parity[1] ^ (^(bitmask_parity_reversed & input_axis_tdata) & uart_parity[1]));
            ...
            if (prescale_reg > 0) 
              ...
              begin
                if (bit_cnt > 1) 
                  begin
                    bit_cnt <= bit_cnt-1;
                    if ((uart_stopbit[1] == 1'b1) && (bit_cnt == (1+1))) prescale_reg <= prescale<<(1+1)-1;
