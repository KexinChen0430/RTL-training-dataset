...
module ALU(clk,control,data_input1,data_input2,data_output,zero);
  ...
  always @(posedge clk)
      begin
        if (control == 4'b0010) 
          begin
            data_output = data_input2+data_input1;
            zero = 0;
          end
        else if (control == 4'b0110) 
          begin
            data_output = data_input1+(0-data_input2);
            if (data_output == 0) zero = 1;
            else zero = 0;
          end
        else if (control == 4'b0000) 
          begin
          ...
          begin
            if (data_input2 < data_input1) data_output = 1;
            else data_output = 0;
            zero = 0;
          end
