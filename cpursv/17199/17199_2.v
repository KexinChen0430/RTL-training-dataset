...
module altor32_wb_fetch(input  clk_i,
  ...
  parameter  FETCH_BYTES_W = (FETCH_WORDS_W+1)+1;
  parameter  WB_CTI_BURST = 3'b010;
  parameter  WB_CTI_FINAL = 3'b111;
  reg  [(-1)+FETCH_WORDS_W:0] fetch_word_q;
  reg  [(-1)+FETCH_WORDS_W:0] resp_word_q;
  wire [(-1)+FETCH_WORDS_W:0]  next_word_w = 1+fetch_word_q;
  wire 
       penultimate_word_w = fetch_word_q == ({FETCH_WORDS_W{1'b1}}+(-1));
  wire 
       final_resp_w = ~burst_i | (resp_word_q == {FETCH_WORDS_W{1'b1}});
