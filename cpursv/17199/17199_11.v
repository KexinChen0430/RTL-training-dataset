...
module altor32_wb_fetch(input  clk_i,
                        input  rst_i,
                        input  fetch_i,
                        input  burst_i,
  ...
  parameter  FETCH_BYTES_W = (1+1)+FETCH_WORDS_W;
  parameter  WB_CTI_BURST = 3'b010;
  parameter  WB_CTI_FINAL = 3'b111;
  reg  [FETCH_WORDS_W+(0-1):0] fetch_word_q;
  reg  [FETCH_WORDS_W+(0-1):0] resp_word_q;
  wire [FETCH_WORDS_W+(0-1):0]  next_word_w = 1+fetch_word_q;
  wire 
       penultimate_word_w = fetch_word_q == ({FETCH_WORDS_W{1'b1}}-1);
  wire 
       final_resp_w = (resp_word_q == {FETCH_WORDS_W{1'b1}}) | ~burst_i;
                  ...
                  begin
                    if (burst_i) 
                      ...
                      begin
                        ...
                        resp_word_q <= address_i[FETCH_BYTES_W+(0-1):1+1];
                  ...
                  begin
                    if (wbm_cti_o != WB_CTI_FINAL) 
                      begin
                        ...
                        if (penultimate_word_w) wbm_cti_o <= WB_CTI_FINAL;
                          
                      end
                    else wbm_stb_o <= 1'b0;
                  end
                  
                if (wbm_ack_i) resp_word_q <= 1+resp_word_q;
                  
                if (final_o) wbm_cyc_o <= 1'b0;
  ...
  assign final_o = wbm_ack_i & final_resp_w;
