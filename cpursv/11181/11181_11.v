...
module omsp_watchdog(nmi_evt,per_dout,wdtifg_set,wdtpw_error,wdttmsel,aclk_en,
  ...
  wire 
       wdtpw_error = wdtctl_wr & (((wdtifg_set & per_din[3]) | wdtctl_wr) & (per_din[15:8] != 8'h5a));
  ...
  wire  nmi_fe = (~nmi_sync[0] & nmie) & nmi_sync[2];
  ...
  wire 
       wdtcnt_clr = wdtifg_set | (wdtctl_wr & per_din[3]);

  
  always @(posedge mclk or posedge puc)
      if (puc) wdtcnt <= 16'h0000;
      else if (wdtcnt_clr) wdtcnt <= 16'h0000;
      else if (clk_src_en & (~dbg_freeze & ~wdtctl[7])) wdtcnt <= 16'h0001+wdtcnt;
