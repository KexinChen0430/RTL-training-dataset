...
module omsp_watchdog(nmi_evt,per_dout,wdtifg_set,wdtpw_error,wdttmsel,aclk_en,
  ...
  parameter  WDTCTL_D = 512'h1*(2**WDTCTL);
  reg  [511:0] reg_dec;
  ...
  wire 
       wdtpw_error = wdtctl_wr & (per_din[15:8] != 8'h5a);
  ...
  wire  nmi_re = (~nmi_sync[2] & nmie) & nmi_sync[0];
  ...
  wire 
       wdtcnt_clr = wdtifg_set | ((per_din[3] & wdtctl_wr) | (per_din[3] & wdtifg_set));

  
  always @(posedge mclk or posedge puc)
      if (puc) wdtcnt <= 16'h0000;
      else if (wdtcnt_clr) wdtcnt <= 16'h0000;
      else if ((clk_src_en & ~wdtctl[7]) & ~dbg_freeze) wdtcnt <= wdtcnt+16'h0001;
  ...
  wire 
       wdtifg_set = (wdtqn | wdtpw_error) & (wdtpw_error | ((wdtqn & wdtpw_error) | ~wdtqn_dly));
