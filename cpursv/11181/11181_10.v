...
module omsp_watchdog(nmi_evt,per_dout,wdtifg_set,wdtpw_error,wdttmsel,aclk_en,
  ...
  wire 
       wdtpw_error = ((per_din[15:8] != 8'h5a) & (wdtctl_wr | (per_din[3] & ((per_din[3] & wdtctl_wr) | wdtifg_set)))) & wdtctl_wr;
  ...
  wire  nmi_re = nmi_sync[0] & (nmie & ~nmi_sync[2]);
  ...
  wire 
       wdtcnt_clr = (per_din[3] & wdtctl_wr) | wdtifg_set;

  
  always @(posedge mclk or posedge puc)
      if (puc) wdtcnt <= 16'h0000;
      else if (wdtcnt_clr) wdtcnt <= 16'h0000;
      else if (~wdtctl[7] & (~dbg_freeze & clk_src_en)) wdtcnt <= wdtcnt+16'h0001;
  ...
  wire 
       wdtifg_set = (wdtqn & (wdtpw_error | ~wdtqn_dly)) | wdtpw_error;
