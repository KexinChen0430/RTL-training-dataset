...
module omsp_watchdog(nmi_evt,per_dout,wdtifg_set,wdtpw_error,wdttmsel,aclk_en,
  ...
  wire 
       wdtpw_error = wdtctl_wr & ((wdtctl_wr | (wdtifg_set & per_din[3])) & (per_din[15:8] != 8'h5a));
  ...
  wire  nmi_re = (nmie & nmi_sync[0]) & ~nmi_sync[2];

  wire  nmi_fe = nmi_sync[2] & (nmie & ~nmi_sync[0]);
  ...
  wire 
       wdtcnt_clr = (wdtctl_wr & per_din[3]) | wdtifg_set;

  
  always @(posedge mclk or posedge puc)
      if (puc) wdtcnt <= 16'h0000;
      else if (wdtcnt_clr) wdtcnt <= 16'h0000;
      else if (~wdtctl[7] & (~dbg_freeze & clk_src_en)) wdtcnt <= 16'h0001+wdtcnt;
  ...
  wire 
       wdtifg_set = (((wdtqn & ~wdtqn_dly) | (wdtpw_error & wdtqn)) | (~wdtqn_dly & wdtpw_error)) | wdtpw_error;
