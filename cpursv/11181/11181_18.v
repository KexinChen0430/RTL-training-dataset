...
module omsp_watchdog(nmi_evt,per_dout,wdtifg_set,wdtpw_error,wdttmsel,aclk_en,
  ...
  wire 
       wdtpw_error = ((wdtifg_set | wdtctl_wr) & ((per_din[15:8] != 8'h5a) & (wdtctl_wr | (per_din[3] | (wdtifg_set & wdtctl_wr))))) & wdtctl_wr;
  ...
  wire  nmi_re = nmie & (~nmi_sync[2] & nmi_sync[0]);
  ...
  wire 
       wdtcnt_clr = wdtifg_set | (wdtctl_wr & (wdtifg_set | per_din[3]));

  
  always @(posedge mclk or posedge puc)
      if (puc) wdtcnt <= 16'h0000;
      else if (wdtcnt_clr) wdtcnt <= 16'h0000;
      else if (clk_src_en & (~wdtctl[7] & ~dbg_freeze)) wdtcnt <= 16'h0001+wdtcnt;
  ...
  wire 
       wdtifg_set = (~wdtqn_dly | wdtpw_error) & (wdtpw_error | wdtqn);
