      ...
      if (~nrst) cnt <= 0;
        ...
        begin
          if (reset) cnt <= 0;
            ...
            begin
              if (rd & wr) cnt <= (nsin+cnt)+(0-nsout);
                
              if (~rd & wr) cnt <= nsin+cnt;
                
              if (rd & ~wr) cnt <= (0-nsout)+cnt;
                
            end
        end
        
  assign shift_out = {8'h0,buffer}*(1<<1**nsout);
  assign space_left = rd ? (nsout+(24-cnt)) : (24-cnt);
  assign shift_in = (1<<1**space_left)*{24'h0,din};
      ...
      if (~nrst) buffer <= 0;
        ...
        begin
          if (reset) buffer <= 0;
            ...
            begin
              if (rd & wr) buffer <= shift_in[31:8] | shift_out[23:0];
