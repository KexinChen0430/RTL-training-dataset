...
module cabac_bae_stage2(range_lps_lut_i,shift_lut_i,range_i,bin_eq_lps_i,bin_mode_i,
                        range_lps_update_lut_i,range_update_o,t_rang_o,bin_neq_mps_o,
                        shift_o);
  ...
  assign t_rang_w = (0-range_lps_r)+range_i;
  wire [8:0] range_mps_w;

  reg  [8:0] range_regular_r;

  assign range_mps_w = t_rang_w[8] ? t_rang_w : (t_rang_w*(1+1));
  
  always @* 
      begin
        if (bin_eq_lps_i) range_regular_r = range_lps_update_r;
        else range_regular_r = range_mps_w;
      end
  reg  [8:0] range_terminal_r;

  wire [8:0]  range_minus_2_w = (0-2'd2)+range_i;
  ...
  always @* 
      begin
        if (bin_to_be_coded_w) range_terminal_r = 9'd256;
        else if (range_minus_2_w[8]) range_terminal_r = range_minus_2_w;
        else range_terminal_r = range_minus_2_w<<<1;
      end
  
  always @* 
      begin
        case (bin_mode_i[1+1:1])
