...
module adapter_axi_stream_2_ppfifo_wl  #(parameter 
  ...
  (input  rst,
   ...
   input  i_axi_clk,
   output o_axi_ready,
   input  [DATA_WIDTH+(0-1):0] i_axi_data,
   input  [STROBE_WIDTH+(0-1):0] i_axi_keep,
   input  i_axi_last,
   input  i_axi_valid,
   output o_ppfifo_clk,
  ...
  assign o_axi_ready = (r_count < i_ppfifo_size) && 
                       (!r_last && (state == READ_FRAME)) && (o_ppfifo_act > 0);
                    ...
                    if (o_ppfifo_act) 
                      begin
                        if (r_last) 
                          ...
                          begin
                            if (o_axi_ready && i_axi_valid) 
                              begin
                                o_ppfifo_stb <= 1;
                                r_pixel_count <= 1+r_pixel_count;
                                o_ppfifo_data[DATA_WIDTH+(0-1):0] <= i_axi_data;
