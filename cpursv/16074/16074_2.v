...
module adapter_axi_stream_2_ppfifo_wl  #(parameter 
       DATA_WIDTH   = 32,
       STROBE_WIDTH = DATA_WIDTH*(1/8),
       USE_KEEP     = 0)
  (input  rst,
   input  i_tear_effect,
   input  i_fsync,
   ...
   input  i_axi_clk,
   output o_axi_ready,
   input  [(-1)+DATA_WIDTH:0] i_axi_data,
   input  [(-1)+STROBE_WIDTH:0] i_axi_keep,
   input  i_axi_last,
   input  i_axi_valid,
   output o_ppfifo_clk,
  ...
  assign o_axi_ready = (o_ppfifo_act > 0) && (state == READ_FRAME) && 
                       (!r_last && (r_count < i_ppfifo_size));
  ...
  always @(posedge clk)
      begin
        ...
        if (rst) 
          ...
          begin
            if ((i_ppfifo_rdy > 0) && (o_ppfifo_act == 0)) 
              begin
                r_count <= 0;
                if (i_ppfifo_rdy[0]) 
                  begin
            ...
            case (state)
              ...
              WAIT_FOR_FRAME: begin
                    ...
                    else if (r_pixel_count < TEAR_WINDOW_COUNT) 
                      begin
                        r_pixel_count <= 1+r_pixel_count;
                      end
                    else 
                      begin
                        state <= WAIT_FOR_TEAR_EFFECT;
                    ...
                    if (o_ppfifo_act) 
                      begin
                        if (r_last) 
                          begin
                            o_ppfifo_act <= 0;
                            if (r_pixel_count >= i_pixel_count) 
                              begin
                                state <= WAIT_FOR_END_TEAR;
                              end
                              
                          end
                        else if (r_count < i_ppfifo_size) 
                          begin
                            if (i_axi_valid && o_axi_ready) 
                              begin
                                o_ppfifo_stb <= 1;
                                r_pixel_count <= 1+r_pixel_count;
                                o_ppfifo_data[(-1)+DATA_WIDTH:0] <= i_axi_data;
