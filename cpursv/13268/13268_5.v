...
module zclock(input  fclk,
  ...
  always @(posedge fclk)
      if (zpos) 
        begin
          old_rfsh_n <= rfsh_n;
          if (!rfsh_n && old_rfsh_n) int_turbo <= turbo;
  ...
  assign io = m1_n & (~iorq_n & external_port);
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (io && (!io_r && int_turbo[1] && zpos)) io_wait_cnt[3] <= 1'b1;
      else if (io_wait_cnt[3]) io_wait_cnt <= io_wait_cnt+4'd1;
  ...
  assign h_precend_2 = pre_cend && !precend_cnt;
  ...
  always @(posedge fclk)
      begin
        zpos <= (zclk_out & ~stall) & pre_zpos;
      end
  
  always @(posedge fclk)
      begin
        zneg <= (pre_zneg & ~stall) & ~zclk_out;
      end
