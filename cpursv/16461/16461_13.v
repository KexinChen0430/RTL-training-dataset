...
module tmu2_split  #(parameter  cache_depth = 13, fml_depth = 26)
  (input  sys_clk,
   ...
   output [(-5)+(fml_depth-1):0] fetch_tadra,
   output [(-5)+(fml_depth-1):0] fetch_tadrb,
   output [(-5)+(fml_depth-1):0] fetch_tadrc,
   output [(-5)+(fml_depth-1):0] fetch_tadrd,
   output fetch_miss_a,
   output fetch_miss_b,
   output fetch_miss_c,
   output fetch_miss_d);
          ...
          begin
            ...
            if (pipe_ack_o) 
              begin
                ...
                fetch_pipe_stb_o <= (((pipe_stb_i & miss_a) | (miss_c & pipe_stb_i)) | (miss_b & pipe_stb_i)) | (miss_d & pipe_stb_i);
                frag_dadr <= dadr;
                r_tadra <= tadra;
                r_tadrb <= tadrb;
                r_tadrc <= tadrc;
  ...
  assign pipe_ack_o = ((frag_pipe_ack_i & (~fetch_pipe_stb_o & (fetch_pipe_ack_i | ~frag_pipe_stb_o))) | (fetch_pipe_ack_i & frag_pipe_ack_i)) | (~fetch_pipe_stb_o & (~frag_pipe_stb_o & (~fetch_pipe_stb_o | frag_pipe_ack_i)));
endmodule
