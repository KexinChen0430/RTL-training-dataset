...
module tmu2_split  #(parameter  cache_depth = 13, fml_depth = 26)
  (input  sys_clk,
   ...
   output reg [(-1)<<1+fml_depth:0] frag_dadr,
   output [(-1)+cache_depth:0] frag_tadra,
   output [(-1)+cache_depth:0] frag_tadrb,
   output [(-1)+cache_depth:0] frag_tadrc,
   output [(-1)+cache_depth:0] frag_tadrd,
  ...
  assign frag_tadra = r_tadra[(-1)+cache_depth:0];
  assign frag_tadrb = r_tadrb[(-1)+cache_depth:0];
  assign frag_tadrc = r_tadrc[(-1)+cache_depth:0];
  assign frag_tadrd = r_tadrd[(-1)+cache_depth:0];
          ...
          begin
            ...
            if (pipe_ack_o) 
              begin
                ...
                fetch_pipe_stb_o <= ((miss_a | miss_d) | (miss_b | miss_c)) & pipe_stb_i;
                frag_dadr <= dadr;
                r_tadra <= tadra;
                r_tadrb <= tadrb;
                r_tadrc <= tadrc;
  ...
  assign pipe_ack_o = (~frag_pipe_stb_o | ((fetch_pipe_ack_i & frag_pipe_ack_i) | ((fetch_pipe_ack_i & ~frag_pipe_stb_o) & (~fetch_pipe_stb_o & (~frag_pipe_stb_o | frag_pipe_ack_i))))) & (((fetch_pipe_ack_i & frag_pipe_ack_i) | ((fetch_pipe_ack_i & ~frag_pipe_stb_o) & (~fetch_pipe_stb_o & (~frag_pipe_stb_o | frag_pipe_ack_i)))) | (~fetch_pipe_stb_o & (~frag_pipe_stb_o | frag_pipe_ack_i)));
endmodule
