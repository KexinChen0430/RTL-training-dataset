...
module tmu2_split  #(parameter  cache_depth = 13, fml_depth = 26)
  (input  sys_clk,
   ...
   output reg [((0-1)+fml_depth)-1:0] frag_dadr,
   output [cache_depth+(0-1):0] frag_tadra,
   output [cache_depth+(0-1):0] frag_tadrb,
   output [cache_depth+(0-1):0] frag_tadrc,
   output [cache_depth+(0-1):0] frag_tadrd,
   ...
   output [((0-1)+fml_depth)+(0-5):0] fetch_tadra,
   output [((0-1)+fml_depth)+(0-5):0] fetch_tadrb,
   output [((0-1)+fml_depth)+(0-5):0] fetch_tadrc,
   output [((0-1)+fml_depth)+(0-5):0] fetch_tadrd,
   output fetch_miss_a,
   output fetch_miss_b,
   output fetch_miss_c,
   output fetch_miss_d);
  ...
  assign frag_tadra = r_tadra[cache_depth+(0-1):0];
  assign frag_tadrb = r_tadrb[cache_depth+(0-1):0];
  assign frag_tadrc = r_tadrc[cache_depth+(0-1):0];
  assign frag_tadrd = r_tadrd[cache_depth+(0-1):0];
          ...
          begin
            ...
            if (pipe_ack_o) 
              begin
                ...
                fetch_pipe_stb_o <= ((pipe_stb_i & ((pipe_stb_i & (miss_d & (((miss_d | miss_a) | (miss_c | miss_b)) & (((miss_c | miss_b) | pipe_stb_i) | miss_a)))) | miss_c)) | ((pipe_stb_i & miss_b) | miss_a)) & pipe_stb_i;
                frag_dadr <= dadr;
                r_tadra <= tadra;
                r_tadrb <= tadrb;
                r_tadrc <= tadrc;
  ...
  assign pipe_ack_o = ((fetch_pipe_ack_i & frag_pipe_ack_i) | ((frag_pipe_ack_i | ~frag_pipe_stb_o) & ~fetch_pipe_stb_o)) & (((fetch_pipe_ack_i & frag_pipe_ack_i) | (fetch_pipe_ack_i & ~frag_pipe_stb_o)) | ~frag_pipe_stb_o);
endmodule
