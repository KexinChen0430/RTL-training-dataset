...
module tmu2_split  #(parameter  cache_depth = 13, fml_depth = 26)
          ...
          begin
            ...
            if (pipe_ack_o) 
              begin
                ...
                fetch_pipe_stb_o <= pipe_stb_i & ((miss_d | (miss_a & pipe_stb_i)) | ((miss_c | miss_b) & pipe_stb_i));
                frag_dadr <= dadr;
                r_tadra <= tadra;
                r_tadrb <= tadrb;
                r_tadrc <= tadrc;
  ...
  assign pipe_ack_o = (((frag_pipe_ack_i | ~frag_pipe_stb_o) & ((~frag_pipe_stb_o | fetch_pipe_ack_i) & ~fetch_pipe_stb_o)) | (((fetch_pipe_ack_i & frag_pipe_ack_i) | ~frag_pipe_stb_o) & fetch_pipe_ack_i)) & (((frag_pipe_ack_i | ~frag_pipe_stb_o) & ((~frag_pipe_stb_o | fetch_pipe_ack_i) & ~fetch_pipe_stb_o)) | frag_pipe_ack_i);
endmodule
