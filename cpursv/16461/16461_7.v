...
module tmu2_split  #(parameter  cache_depth = 13, fml_depth = 26)
  (input  sys_clk,
   ...
   input  pipe_stb_i,
   output pipe_ack_o,
   input  [((-1)+fml_depth)-1:0] dadr,
   input  [(-1)+fml_depth:0] tadra,
   input  [(-1)+fml_depth:0] tadrb,
   input  [(-1)+fml_depth:0] tadrc,
   input  [(-1)+fml_depth:0] tadrd,
   input  [5:0] x_frac,
   input  [5:0] y_frac,
   input  miss_a,
   input  miss_b,
   ...
   output reg [((-1)+fml_depth)-1:0] frag_dadr,
   ...
   output [((-1)+fml_depth)+(-5):0] fetch_tadra,
   output [((-1)+fml_depth)+(-5):0] fetch_tadrb,
   output [((-1)+fml_depth)+(-5):0] fetch_tadrc,
   output [((-1)+fml_depth)+(-5):0] fetch_tadrd,
   output fetch_miss_a,
   output fetch_miss_b,
   output fetch_miss_c,
   output fetch_miss_d);

  reg  [(-1)+fml_depth:0] r_tadra;
  reg  [(-1)+fml_depth:0] r_tadrb;
  reg  [(-1)+fml_depth:0] r_tadrc;
  reg  [(-1)+fml_depth:0] r_tadrd;
  reg  r_miss_a;
  reg  r_miss_b;
  reg  r_miss_c;
  reg  r_miss_d;
  ...
  assign fetch_tadra = r_tadra[(-1)+fml_depth:5];
  assign fetch_tadrb = r_tadrb[(-1)+fml_depth:5];
  assign fetch_tadrc = r_tadrc[(-1)+fml_depth:5];
  assign fetch_tadrd = r_tadrd[(-1)+fml_depth:5];
          ...
          begin
            ...
            if (pipe_ack_o) 
              begin
                ...
                fetch_pipe_stb_o <= (((miss_a | miss_d) | miss_c) | miss_b) & pipe_stb_i;
                frag_dadr <= dadr;
                r_tadra <= tadra;
                r_tadrb <= tadrb;
                r_tadrc <= tadrc;
  ...
  assign pipe_ack_o = ((~fetch_pipe_stb_o & (~frag_pipe_stb_o | frag_pipe_ack_i)) & ((~fetch_pipe_stb_o | fetch_pipe_ack_i) & (~frag_pipe_stb_o | fetch_pipe_ack_i))) | ((~frag_pipe_stb_o | fetch_pipe_ack_i) & ((~fetch_pipe_stb_o | fetch_pipe_ack_i) & frag_pipe_ack_i));
endmodule
