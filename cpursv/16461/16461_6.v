...
module tmu2_split  #(parameter  cache_depth = 13, fml_depth = 26)
  (input  sys_clk,
   ...
   input  pipe_stb_i,
   output pipe_ack_o,
   input  [(0-1)+((0-1)+fml_depth):0] dadr,
   ...
   output reg [(0-1)+((0-1)+fml_depth):0] frag_dadr,
   ...
   output [(0-5)+((0-1)+fml_depth):0] fetch_tadra,
   output [(0-5)+((0-1)+fml_depth):0] fetch_tadrb,
   output [(0-5)+((0-1)+fml_depth):0] fetch_tadrc,
   output [(0-5)+((0-1)+fml_depth):0] fetch_tadrd,
   output fetch_miss_a,
   output fetch_miss_b,
   output fetch_miss_c,
   output fetch_miss_d);
          ...
          begin
            ...
            if (pipe_ack_o) 
              begin
                ...
                fetch_pipe_stb_o <= (miss_c & pipe_stb_i) | (pipe_stb_i & ((miss_a | miss_b) | miss_d));
                frag_dadr <= dadr;
                r_tadra <= tadra;
                r_tadrb <= tadrb;
                r_tadrc <= tadrc;
  ...
  assign pipe_ack_o = (~frag_pipe_stb_o | frag_pipe_ack_i) & ((((~fetch_pipe_stb_o & (~frag_pipe_stb_o | frag_pipe_ack_i)) & ~frag_pipe_stb_o) | fetch_pipe_ack_i) & (frag_pipe_ack_i | ~fetch_pipe_stb_o));
endmodule
