...
module sim_camera(input  i_cam_in_clk,
                  input  i_cam_rst,
                  input  i_flash,
                  output o_pix_clk,
  ...
  always @(posedge i_cam_in_clk)
      begin
        if (!i_cam_rst) 
          ...
          begin
            if (r_vblank_count < VBLANK_COUNT) 
              begin
                r_vblank_count <= 1+r_vblank_count;
              end
              
            if (r_hblank_count < HBLANK_COUNT) 
              begin
                r_hblank_count <= 1+r_hblank_count;
              end
            ...
            case (state)
              ...
              WRITE_ROW: begin
                    o_vsync <= 1;
                    o_hsync <= 1;
                    if (r_byte_count < BYTE_COUNT) 
                      begin
                        o_pix_data <= r_data;
                        r_data <= 1+r_data;
                        r_byte_count <= 1+r_byte_count;
                      end
                    else 
                      begin
                        if (r_row_count < (ROW_COUNT-1)) 
                          begin
                            r_hblank_count <= 0;
                            o_hsync <= 0;
                            state <= HBLANK;
                            r_row_count <= 1+r_row_count;
                          end
                        else 
                          begin
                            r_vblank_count <= 0;
