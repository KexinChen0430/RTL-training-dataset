...
module oh_clockdiv_model(clkout,clkin,divcfg,reset);
  ...
  always @(posedge clkin or posedge reset)
      if (reset) counter[7:0] <= 8'b00000001;
      else if (posedge_match) counter[7:0] <= 8'b00000001;
      else counter[7:0] <= counter[7:0]+8'b00000001;
  assign posedge_match = counter[7:0] == divcfg_dec[7:0];
  assign negedge_match = counter[7:0] == {1'b0,divcfg_dec[7:1]};
  
  always @(posedge clkin or posedge reset)
      if (reset) clkout_reg <= 1'b0;
      else if (posedge_match) clkout_reg <= 1'b1;
      else if (negedge_match) clkout_reg <= 1'b0;
