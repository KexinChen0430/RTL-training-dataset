...
module rfid_reader_tx(reset,clk,reader_modulation,tx_done,tx_running,tx_go,
                      send_trcal,delim_counts,pw_counts,rtcal_counts,trcal_counts,
                      tari_counts,tx_packet_length,tx_packet_data);
  ...
  wire 
      current_bit         ,
      data0_bit_end       ,
      data0_bit_transition,
      data1_bit_end       ,
      data1_bit_transition,
      bit_transition      ,
      bit_end             ;
  ...
  assign data0_end_count = pw_counts+tari_counts;
  assign data1_end_count = pw_counts+tari_counts<<<1;
  assign data0_tran_count = tari_counts;
  assign data1_tran_count = tari_counts<<<1;
  ...
  assign bit_transition = data1_bit_transition | (((data0_bit_transition & (!current_bit | (data1_bit_transition & data0_bit_transition))) | (!current_bit & data1_bit_transition)) | (data1_bit_transition & data0_bit_transition));
  assign bit_end = data1_bit_end | (((data1_bit_end & !current_bit) | data0_bit_end) & !current_bit);
  wire 
      rtcal_end       ,
      rtcal_transition,
      trcal_end       ,
  ...
  assign trcal_end_count = pw_counts+trcal_counts;
              ...
              STATE_IDLE: begin
                    ...
                    if (tx_go) 
                      begin
                        ...
                        tx_running <= 1;
                        modout <= 0;
                        current_tx_bit <= tx_packet_length+(0-1);
                      end
                    else 
                      begin
                        tx_running <= 0;
