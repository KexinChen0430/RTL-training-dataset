...
module opc3cpu(inout  [15:0] data,
               output [15:0] address,
               output rnw,
               input  clk,
               input  reset_b);
  ...
  reg  C_q;
  wire 
       writeback_w = (FSM_q == EXEC) && 
(
((IR_q == STAP) || 
((IR_q == STA) || ((FSM_q == EXEC) && (IR_q == STAP)))) && 
(((IR_q == STA) || (FSM_q == EXEC)) && 
((FSM_q == EXEC) || (IR_q == STAP)))) & reset_b;

  assign rnw = ~writeback_w;
  assign data = writeback_w ? ACC_q : 16'bz;
  assign address = (writeback_w || ((FSM_q == RDMEM2) || (FSM_q == RDMEM))) ? OR_q : PC_q;
  ...
  always @(posedge clk or negedge reset_b)
      if (!reset_b) PC_q <= 16'h0000;
      else if ((FSM_q == FETCH0) || (FSM_q == FETCH1)) PC_q <= 1+PC_q;
      else 
        case (IR_q)
