...
module opc3cpu(inout  [15:0] data,
  ...
  reg  C_q;
  wire 
       writeback_w = reset_b & (((IR_q == STAP) && (FSM_q == EXEC)) || 
((IR_q == STA) && (FSM_q == EXEC)));

  assign rnw = ~writeback_w;
  assign data = writeback_w ? ACC_q : 16'bz;
  assign address = ((FSM_q == RDMEM2) || writeback_w || (FSM_q == RDMEM)) ? OR_q : PC_q;
  ...
  always @(posedge clk or negedge reset_b)
      ...
      else if ((FSM_q == FETCH0) || (FSM_q == FETCH1)) PC_q <= PC_q+1;
      else 
        case (IR_q)
