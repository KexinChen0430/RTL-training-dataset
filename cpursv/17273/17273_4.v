...
((FSM_q == EXEC) && 
(
((FSM_q == EXEC) || ((IR_q == STA) && (IR_q == STAP))) && (IR_q == STA))) || ((FSM_q == EXEC) && (IR_q == STAP))) & reset_b;

  assign rnw = ~writeback_w;
  assign data = writeback_w ? ACC_q : 16'bz;
  assign address = ((FSM_q == RDMEM) || writeback_w || (FSM_q == RDMEM2)) ? OR_q : PC_q;
