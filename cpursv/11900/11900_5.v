...
module during_idle_run_state;
  ...
  assign baud_clk_o = run && main_count_rollover;
  ...
  always @(m1_state or mid_bit_count or serial_dat_i or verify_done or character_miscompare)
      begin : m1_state_logic
        ...
        case (m1_state)
          ...
          m1_measure_0: begin
                measure <= 1'b1;
                if (~serial_dat_i && mid_bit_count) m1_next_state <= m1_measure_1;
                else if (serial_dat_i && mid_bit_count) m1_next_state <= m1_idle;
          ...
          m1_measure_2: begin
                measure <= 1'b1;
                if (serial_dat_i && mid_bit_count) m1_next_state <= m1_measure_3;
                else if (~serial_dat_i && mid_bit_count) m1_next_state <= m1_idle;
