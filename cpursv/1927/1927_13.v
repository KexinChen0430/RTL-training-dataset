...
module ram_controller_mem_model_ram_module(data,rdaddress,wraddress,wrclock,wren,q);
  ...
  always @(rdaddress)
      begin
        ...
        for (i = 0; i < ARRAY_DEPTH; i = i+1)
            begin
              if ((rdaddress == mem_array[i][58+(0-1):58+(0-25)]) && 
                  mem_array[i][0]) 
                begin
                  out = mem_array[i][(0-1)+(58+(0-25)):((-32)+58)+(0-25)];
      ...
      if (wren) 
        begin
          ...
          for (i = 0; i < ARRAY_DEPTH; i = i+1)
              begin
                if (!data_written && 
                    (wraddress == mem_array[i][58+(0-1):58+(0-25)])) 
                  begin
                    mem_array[i][(0-1)+(58+(0-25)):((-32)+58)+(0-25)] <= data;
                    mem_array[i][0] <= 1'b1;
                    data_written = 1'b1;
                  end
