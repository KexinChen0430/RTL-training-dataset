...
module ram_controller_mem_model_ram_module(data,rdaddress,wraddress,wrclock,wren,q);
  ...
  always @(rdaddress)
      begin
        ...
        for (i = 0; i < ARRAY_DEPTH; i = i+1)
            begin
              if ((rdaddress == mem_array[i][(0-1)+58:58+(0-25)]) && 
                  mem_array[i][0]) 
                begin
                  out = mem_array[i][((0-25)+(0-1))+58:((0-25)+(0-32))+58];
  ...
  always @(posedge wrclock)
      if (wren) 
        begin
          ...
          for (i = 0; i < ARRAY_DEPTH; i = i+1)
              begin
                if ((wraddress == mem_array[i][(0-1)+58:58+(0-25)]) && 
                    !data_written) 
                  begin
                    mem_array[i][((0-25)+(0-1))+58:((0-25)+(0-32))+58] <= data;
                    mem_array[i][0] <= 1'b1;
                    data_written = 1'b1;
                  end
