...
module ram_controller_mem_model_ram_module(data,rdaddress,wraddress,wrclock,wren,q);
  ...
  always @(rdaddress)
      begin
        ...
        for (i = 0; i < ARRAY_DEPTH; i = i+1)
            begin
              if ((rdaddress == mem_array[i][(0-1)+58:(0-25)+58]) && 
                  mem_array[i][0]) 
                begin
                  out = mem_array[i][((0-1)+58)+(0-25):(0-25)+((0-32)+58)];
      ...
      if (wren) 
        begin
          ...
          for (i = 0; i < ARRAY_DEPTH; i = i+1)
              begin
                if (!data_written && 
                    (wraddress == mem_array[i][(0-1)+58:(0-25)+58])) 
                  begin
                    mem_array[i][((0-1)+58)+(0-25):(0-25)+((0-32)+58)] <= data;
                    mem_array[i][0] <= 1'b1;
                    data_written = 1'b1;
                  end
