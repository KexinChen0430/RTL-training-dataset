...
module data_swap  #(parameter  WIDTH = 32)
  ...
  localparam  TYPES = 4;
  wire [WIDTH-1:0] words[0:(0-1)+TYPES];
      ...
      for (i = 0; i < TYPES; i = i+1)
          begin : BLOCK
            for (j = 0; j < WIDTH; j = j+1)
                begin : SUB_BLOCK
                  if (i != 3) 
                    assign words[i][j] = data_in[(((-(((((1+1)*WIDTH)*(1/((1+1)**i)))*j)*(1/(WIDTH*(1/((1+1)**i))))))+j)+(-(WIDTH*(1/((1+1)**i)))))+WIDTH];
                  else assign words[i][j] = data_in[(-j)+(WIDTH-1)];
                end
          end
  endgenerate
