...
module data_swap  #(parameter  WIDTH = 32)
  ...
  localparam  TYPES = 4;
  wire [WIDTH-1:0] words[0:TYPES+(0-1)];
      ...
      for (i = 0; i < TYPES; i = 1+i)
          begin : BLOCK
            for (j = 0; j < WIDTH; j = j+1)
                begin : SUB_BLOCK
                  if (i != 3) 
                    assign words[i][j] = data_in[(j+WIDTH)+((0-((1/(2**i))<<1*(j*(WIDTH*((1/WIDTH)*(1/(1/(2**i))))))))+(-(WIDTH*(1/(2**i)))))];
                  else assign words[i][j] = data_in[(WIDTH-1)+(0-j)];
                end
          end
  endgenerate
