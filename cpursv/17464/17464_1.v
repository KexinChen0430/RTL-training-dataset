...
module data_swap  #(parameter  WIDTH = 32)
  ...
  localparam  TYPES = 4;
  wire [(0-1)+WIDTH:0] words[0:TYPES-1];
      ...
      for (i = 0; i < TYPES; i = 1+i)
          begin : BLOCK
            for (j = 0; j < WIDTH; j = 1+j)
                begin : SUB_BLOCK
                  if (i != 3) 
                    assign words[i][j] = data_in[((WIDTH+(-((1/WIDTH)*((WIDTH/(1<<<1**i))*(j/(1/(1<<<1**i))))<<<1)))+j)+(0-(WIDTH/(1<<<1**i)))];
                  else assign words[i][j] = data_in[(0-1)+(WIDTH+(-j))];
                end
          end
  endgenerate
