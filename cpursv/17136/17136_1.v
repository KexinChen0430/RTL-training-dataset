...
module phase_accumulator  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [(-1)+WIDTH:0] input_phase_tdata,
   input  wire input_phase_tvalid,
   output wire input_phase_tready,
   input  wire [(-1)+WIDTH:0] input_phase_step_tdata,
   input  wire input_phase_step_tvalid,
   output wire input_phase_step_tready,
   output wire [(-1)+WIDTH:0] output_phase_tdata,
   output wire output_phase_tvalid,
   input  wire output_phase_tready);

  reg  [(-1)+WIDTH:0]  phase_reg = INITIAL_PHASE;
  reg  [(-1)+WIDTH:0]  phase_step_reg = INITIAL_PHASE_STEP;
