...
module map_wall_edge(input  pixel_clk_i,
  ...
  parameter  X = 0;
  parameter  Y_START = VSYNC_VBI_LINE_COUNT+1<<1;
  parameter  Y_END = 237+Y_START;

  assign led_strip_address_valid_o = (pixel_x_i == X) && (pixel_y_i < Y_END) && 
                                     (pixel_y_i >= Y_START) && pixel_valid_i;
  assign led_strip_address_o = pixel_y_i+(0-Y_START);
endmodule
