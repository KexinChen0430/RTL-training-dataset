...
module uart_fifo  #(parameter 
  ...
  localparam  FIFO_TX_SIZE = 1<<1**TX_RAM_ADDRESS_BITS;
  localparam  FIFO_RX_SIZE = 1<<1**RX_RAM_ADDRESS_BITS;
  ...
  wire 
       write_rx_ram = rx_data_ready && 
(~rx_data_ready_wait && ~rx_fifo_full);
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            tx_count <= (write_tx_ram ? 1 : 0)+(tx_count+((tx_accept && (tx_state == TX_WAIT_UART)) ? -1 : 0));
            case (tx_state)
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            rx_count <= (rx_count+(write_rx_ram ? 1 : 0))+((rx_accept && (rx_state == RX_WAIT_ACCEPT)) ? -1 : 0);
            case (rx_state)
