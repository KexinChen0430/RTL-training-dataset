...
module uart_fifo  #(parameter 
  ...
  (input  reset,
   ...
   output reg [(-1)+RX_RAM_ADDRESS_BITS:0] rx_fifo_ram_read_address,
   output reg [(-1)+RX_RAM_ADDRESS_BITS:0] rx_fifo_ram_write_address,
   output rx_fifo_full,
  ...
  wire 
       write_rx_ram = ~rx_data_ready_wait && rx_data_ready && ~rx_fifo_full;
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            tx_count <= tx_count+((write_tx_ram ? 1 : 0)+(((tx_state == TX_WAIT_UART) && tx_accept) ? -1 : 0));
            case (tx_state)

              TX_IDLE: begin
                    if (~tx_accept && |tx_count) 
                      begin
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            rx_count <= (write_rx_ram ? 1 : 0)+(rx_count+((rx_accept && (rx_state == RX_WAIT_ACCEPT)) ? -1 : 0));
            case (rx_state)
