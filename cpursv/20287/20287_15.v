...
module uart_fifo  #(parameter 
  ...
  (input  reset,
   ...
   output reg [(-1)+TX_RAM_ADDRESS_BITS:0] tx_fifo_ram_read_address,
   output reg [(-1)+TX_RAM_ADDRESS_BITS:0] tx_fifo_ram_write_address,
  ...
  wire 
       write_rx_ram = ~rx_fifo_full && ~rx_data_ready_wait && rx_data_ready;
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            tx_count <= tx_count+((write_tx_ram ? 1 : 0)+((tx_accept && (tx_state == TX_WAIT_UART)) ? -1 : 0));
            case (tx_state)
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            rx_count <= ((write_rx_ram ? 1 : 0)+(((rx_state == RX_WAIT_ACCEPT) && rx_accept) ? -1 : 0))+rx_count;
            case (rx_state)
