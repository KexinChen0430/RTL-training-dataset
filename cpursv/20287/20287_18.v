...
module uart_fifo  #(parameter 
  ...
  (input  reset,
   ...
   output reg [(0-1)+RX_RAM_ADDRESS_BITS:0] rx_fifo_ram_read_address,
   output reg [(0-1)+RX_RAM_ADDRESS_BITS:0] rx_fifo_ram_write_address,
   output rx_fifo_full,
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            tx_count <= ((write_tx_ram ? 1 : 0)+((tx_accept && (tx_state == TX_WAIT_UART)) ? -1 : 0))+tx_count;
            case (tx_state)
