...
module uart_fifo  #(parameter 
  ...
  (input  reset,
   ...
   output reg [TX_RAM_ADDRESS_BITS+(0-1):0] tx_fifo_ram_read_address,
   output reg [TX_RAM_ADDRESS_BITS+(0-1):0] tx_fifo_ram_write_address,
   output reg [7:0] rx_data_out,
   output reg [RX_RAM_ADDRESS_BITS+(0-1):0] rx_fifo_ram_read_address,
   output reg [RX_RAM_ADDRESS_BITS+(0-1):0] rx_fifo_ram_write_address,
   output rx_fifo_full,
   output reg rx_fifo_ram_wren,
   output reg rx_data_out_ready);

  localparam  FIFO_TX_SIZE = 2**TX_RAM_ADDRESS_BITS;
  localparam  FIFO_RX_SIZE = 2**RX_RAM_ADDRESS_BITS;
  ...
  wire 
       write_rx_ram = ~rx_data_ready_wait && 
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            tx_count <= (((tx_accept && (tx_state == TX_WAIT_UART)) ? -1 : 0)+tx_count)+(write_tx_ram ? 1 : 0);
            case (tx_state)
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            else if (~rx_data_ready && rx_data_ready_wait) 
              begin
                rx_data_ready_wait <= 1'b0;
              end
              
            rx_fifo_ram_write_address <= rx_fifo_ram_write_address+(rx_fifo_ram_wren ? 1 : 0);
            rx_count <= (rx_count+(write_rx_ram ? 1 : 0))+(((rx_state == RX_WAIT_ACCEPT) && rx_accept) ? -1 : 0);
            case (rx_state)

              RX_IDLE: begin
                    if (~rx_accept && |rx_count) 
                      begin
              ...
              RX_WAIT_ACCEPT: begin
                    if (rx_accept) 
                      begin
                        rx_data_out_ready <= 1'b0;
                        rx_fifo_ram_read_address <= 1'b1+rx_fifo_ram_read_address;
                        rx_state <= RX_IDLE;
                      end
