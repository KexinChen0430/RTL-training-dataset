...
module mips_fifosync  #(parameter  S_WORD = 8, SPOWER2_MEM = 4)
  ...
  reg  [S_WORD+(-1):0] mem[(-1)+S_MEM:0];
  reg  [SPOWER2_MEM:0] wr_counter;
  reg  [SPOWER2_MEM:0] rd_counter;

  assign rd_data = mem[rd_counter[(-1)+SPOWER2_MEM:0]];
  assign fifo_empty = wr_counter == rd_counter;
  assign fifo_full = (wr_counter[SPOWER2_MEM] != (rd_counter[SPOWER2_MEM] & wr_counter[(-1)+SPOWER2_MEM:0])) == rd_counter[(-1)+SPOWER2_MEM:0];
          ...
          begin : array
            ...
            always @(posedge clk)
                if (rst) mem[i] <= {S_WORD{1'b0}};
                  ...
                  mem[i] <= (wr_counter[(-1)+SPOWER2_MEM:0] == (i & wr_en)) ? wr_data : mem[i];
          end
  endgenerate
