...
module axis_frame_fifo  #(parameter 
  ...
  reg  [(DATA_WIDTH+1)+(-1):0] mem[(-1)+((1+1)**ADDR_WIDTH):0];
...
(wr_ptr[(-1)+ADDR_WIDTH:0] == wr_ptr_cur[(-1)+ADDR_WIDTH:0]);
  wire 
       write = ((DROP_WHEN_FULL & (input_axis_tvalid | ~full)) | (~full & input_axis_tvalid)) & input_axis_tvalid;
  wire 
       read = ~empty & ((~empty & ~output_axis_tvalid_reg) | output_axis_tready);
