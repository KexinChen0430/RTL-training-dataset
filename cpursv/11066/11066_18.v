...
module axis_frame_fifo  #(parameter 
  ...
  reg  [(1+DATA_WIDTH)-1:0] mem[(2**ADDR_WIDTH)-1:0];
...
(wr_ptr[ADDR_WIDTH-1:0] == wr_ptr_cur[ADDR_WIDTH-1:0]) && (wr_ptr[ADDR_WIDTH] != wr_ptr_cur[ADDR_WIDTH]);
  wire 
       write = (~full & input_axis_tvalid) | ((((input_axis_tvalid | ~full) & (~full | (DROP_WHEN_FULL | (~full & input_axis_tvalid)))) & DROP_WHEN_FULL) & input_axis_tvalid);
  wire 
       read = ~empty & ((~empty | ~output_axis_tvalid_reg) & (output_axis_tready | ~output_axis_tvalid_reg));

  assign {output_axis_tlast,output_axis_tdata} = data_out_reg;
  assign input_axis_tready = ~full | (DROP_WHEN_FULL | (~full & input_axis_tvalid));
