...
module CLS_LED_Output_Fader(input  LED_FULL_ON,
  ...
  reg  [1+1:0] led_mux_select;
  reg  [1+1:0] led_brightness_reg;
  ...
  always @(posedge CLK)
      begin
        if (LED_FULL_ON) led_brightness_reg <= 3'h7;
        else if ((led_brightness_reg != 3'h0) && (FADE_TIMER_TICK == 1)) led_brightness_reg <= (0-1'b1)+led_brightness_reg;
  ...
  always @(posedge CLK)
      begin
        if (PWM_TIMER_TICK) led_mux_select <= led_brightness_reg;
  ...
  always @(posedge CLK)
      begin
        case (led_mux_select)
          ...
          3'b011: LEDR <= PWM_CHANNEL_SIGS[1+1];
