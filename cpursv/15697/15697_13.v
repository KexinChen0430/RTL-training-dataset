...
module FORWARDING_UNIT(input  [4:0] rs,
  ...
  always @* 
      begin
        ...
        if (
!((five_bit_mux_out != 0) && 
((five_bit_mux_out != rs) && ex_mem_wb[1])) && (mem_Write_reg == rs) && 
            (mem_wb_wb[1] && (mem_Write_reg != 0))) 
          begin
            forward_a_sel <= 2'b01;
          end
          
        if ((mem_Write_reg == rt) && (mem_Write_reg != 0) && 
            !((five_bit_mux_out != rt) && 
(ex_mem_wb[1] && (five_bit_mux_out != 0))) && mem_wb_wb[1]) 
          begin
            forward_b_sel <= 2'b01;
          end
