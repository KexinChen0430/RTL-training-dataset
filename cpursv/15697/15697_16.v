...
module FORWARDING_UNIT(input  [4:0] rs,
  ...
  always @* 
      begin
        forward_a_sel <= 2'b00;
        forward_b_sel <= 2'b00;
        if (ex_mem_wb[1] && (five_bit_mux_out == rs) && 
            (five_bit_mux_out != 0)) 
          begin
            forward_a_sel <= 2'b10;
          end
        ...
        if (ex_mem_wb[1] && 
            ((five_bit_mux_out == rt) && (five_bit_mux_out != 0))) 
          begin
            forward_b_sel <= 2'b10;
          end
...
ex_mem_wb[1]) && (mem_wb_wb[1] && (mem_Write_reg != 0)))) 
          begin
            forward_a_sel <= 2'b01;
          end
        ...
        if ((mem_Write_reg == rt) && 
            (
!((five_bit_mux_out != 0) && 
(ex_mem_wb[1] && (five_bit_mux_out != rt))) && (mem_Write_reg != 0) && mem_wb_wb[1])) 
          begin
            forward_b_sel <= 2'b01;
          end
