...
module driver_vga_640x480(clk_vga,hs,vs,hc_visible,vc_visible);
  ...
  assign hc_visible = ((hc < ((-hfp)+hpixels)) && (hc > (hsc+hbp))) ? (hc+(0-(hsc+hbp))) : 10'd0;
  assign vc_visible = ((vc < ((0-vfp)+vlines)) && (vc > (vsc+vbp))) ? ((0-(vsc+vbp))+vc) : 10'd0;
  
  always @(*)
      if (hc == hpixels) hc_next = 10'd0;
      else hc_next = 10'd1+hc;
  
  always @(*)
      if (hc == 10'd0) 
        if (vc == vlines) vc_next = 10'd0;
        else vc_next = vc+10'd1;
      else vc_next = vc;
