...
module master_updateable_megarom(inout  wire [7:0] D,
  ...
  assign bbc_nCE = installed_in_bbc_master ? 1'b0 : (cpld_JP[0] && cpld_JP[1]);
  assign reading_memory = accessing_memory && rnw;
  assign flash_nOE = !(reading_memory || 
(
(!bbc_nCE || (!bbc_nCE && reading_memory) || reading_memory) && 
(!bbc_nCE && 
(
(allowing_bbc_access && (reading_memory || !bbc_A[16]) && 
!bbc_A[16]) || (!bbc_A[16] && (!bbc_nCE && reading_memory)) || 
(!bbc_A[16] && reading_memory)) && allowing_bbc_access)));
  assign writing_memory = !rnw && accessing_memory;
  assign flash_nWE = !(writing_memory && !allowing_bbc_access);
  assign D = ((allowing_bbc_access == 1'b0) && 
             ((rnw == 1'b0) && (driving_bus == 1'b1))) ? spi_D : 8'bZZZZZZZZ;
