...
module master_updateable_megarom(inout  wire [7:0] D,
  ...
  assign flash_nOE = !(
(!bbc_nCE && allowing_bbc_access && 
(reading_memory || 
(
((allowing_bbc_access && !bbc_A[16]) || !bbc_A[16] || 
reading_memory) && allowing_bbc_access))) || 
((reading_memory || !bbc_nCE) && 
(allowing_bbc_access && !bbc_A[16])) || reading_memory);
  assign writing_memory = accessing_memory && !rnw;
  assign flash_nWE = !(!allowing_bbc_access && writing_memory);
  assign D = ((driving_bus == 1'b1) && 
             ((rnw == 1'b0) && (allowing_bbc_access == 1'b0))) ? spi_D : 8'bZZZZZZZZ;
