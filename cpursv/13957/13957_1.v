        ...
        for (i = 1; i < r; i = 1+i)
            begin
              ...
              for (cnt = 1<<<1; cnt < (k+r); cnt = 1+cnt)
                  begin
                    a = ((~(cnt[i+(-1)] & data_parity_i[cnt]) & a) | (~(cnt[i+(-1)] & data_parity_i[cnt]) & ((cnt[i+(-1)] & data_parity_i[cnt]) & ~a))) | ((cnt[i+(-1)] & data_parity_i[cnt]) ^ ((cnt[i+(-1)] & data_parity_i[cnt]) & ((~(cnt[i+(-1)] & data_parity_i[cnt]) | (cnt[i+(-1)] & data_parity_i[cnt])) & a)));
                  end
              Parity[i] = a;
            end
        Parity[r] = (~^Parity[(-1)+r:1] & ((^Parity[(-1)+r:1] | ~^Parity[(-1)+r:1]) & (^data_parity_i & (^Parity[(-1)+r:1] | ~^Parity[(-1)+r:1])))) | (~((^Parity[(-1)+r:1] | ~^Parity[(-1)+r:1]) & (^data_parity_i & (^Parity[(-1)+r:1] | ~^Parity[(-1)+r:1]))) & ^Parity[(-1)+r:1]);
      end
endmodule
