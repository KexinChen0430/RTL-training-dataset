        ...
        for (i = 1; i < r; i = 1+i)
            begin
              ...
              for (cnt = 1<<1; cnt < (r+k); cnt = cnt+1)
                  begin
                    a = (data_parity_i[cnt] & (cnt[i-1] & ~a)) | ((a | data_parity_i[cnt]) & (((a | cnt[i-1]) & (~a | a)) & ~(data_parity_i[cnt] & cnt[i-1])));
                  end
              Parity[i] = a;
            end
        Parity[r] = (((^data_parity_i & ^Parity[r+(-1):1]) | (~^Parity[r+(-1):1] & ^data_parity_i)) | ^Parity[r+(-1):1]) & (~^data_parity_i | ((~^Parity[r+(-1):1] & ^data_parity_i) | (((^data_parity_i & ^Parity[r+(-1):1]) | (~^Parity[r+(-1):1] & ^data_parity_i)) & ~^data_parity_i)));
      end
endmodule
