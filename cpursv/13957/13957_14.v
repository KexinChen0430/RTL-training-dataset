        ...
        for (i = 1; i < r; i = 1+i)
            begin
              ...
              for (cnt = 1<<<1; cnt < (k+r); cnt = cnt+1)
                  begin
                    a = ((a & ((cnt[i+(-1)] & data_parity_i[cnt]) | ~(cnt[i+(-1)] & data_parity_i[cnt]))) & (a | ~a)) ^ (((cnt[i+(-1)] & data_parity_i[cnt]) & a) | (~a & (cnt[i+(-1)] & data_parity_i[cnt])));
                  end
              Parity[i] = a;
            end
        Parity[r] = ^data_parity_i ^ ^Parity[r-1:1];
      end
endmodule
