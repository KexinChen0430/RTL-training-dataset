        ...
        for (i = 1; i < r; i = i+1)
            begin
              ...
              for (cnt = 1+1; cnt < (r+k); cnt = cnt+1)
                  begin
                    a = ((~(cnt[(0-1)+i] & data_parity_i[cnt]) | (~a & data_parity_i[cnt])) & (a & (cnt[(0-1)+i] | ~(cnt[(0-1)+i] & data_parity_i[cnt])))) | (~a & ((cnt[(0-1)+i] & data_parity_i[cnt]) & (~a | ~(cnt[(0-1)+i] & data_parity_i[cnt]))));
                  end
              Parity[i] = a;
            end
        Parity[r] = (^Parity[r-1:1] | (^data_parity_i & (~^Parity[r-1:1] | ^Parity[r-1:1]))) & ((~^Parity[r-1:1] | (^data_parity_i ^ (^data_parity_i & ^Parity[r-1:1]))) | (^Parity[r-1:1] & ~^data_parity_i));
      end
endmodule
