        ...
        for (i = 1; i < r; i = i+1)
            begin
              ...
              for (cnt = 1+1; cnt < (r+k); cnt = cnt+1)
                  begin
                    a = ((data_parity_i[cnt] & cnt[i+(-1)]) ^ (a & (data_parity_i[cnt] & cnt[i+(-1)]))) ^ ((((~(data_parity_i[cnt] & cnt[i+(-1)]) | data_parity_i[cnt]) & (cnt[i+(-1)] | ~(data_parity_i[cnt] & cnt[i+(-1)]))) & a) & (~(data_parity_i[cnt] & cnt[i+(-1)]) | ~a));
                  end
              Parity[i] = a;
            end
        Parity[r] = ((^data_parity_i & ^Parity[(-1)+r:1]) ^ ^data_parity_i) | (~^data_parity_i & (^Parity[(-1)+r:1] | ^data_parity_i));
      end
endmodule
