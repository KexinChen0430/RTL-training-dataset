        ...
        for (i = 1; i < r; i = 1+i)
            begin
              ...
              for (cnt = 2; cnt < (r+k); cnt = 1+cnt)
                  begin
                    a = ((cnt[(0-1)+i] & (~(cnt[(0-1)+i] & data_parity_i[cnt]) & ((((a | data_parity_i[cnt]) & ~(cnt[(0-1)+i] & data_parity_i[cnt])) | data_parity_i[cnt]) & a))) | ((cnt[(0-1)+i] & ~a) & data_parity_i[cnt])) | (~(cnt[(0-1)+i] & data_parity_i[cnt]) & ((((a | data_parity_i[cnt]) & ~(cnt[(0-1)+i] & data_parity_i[cnt])) | data_parity_i[cnt]) & a));
                  end
              Parity[i] = a;
            end
        Parity[r] = ((^Parity[r+(0-1):1] | ((^data_parity_i | ^Parity[r+(0-1):1]) & ~^Parity[r+(0-1):1])) & ^data_parity_i) ^ ^Parity[r+(0-1):1];
      end
endmodule
