        ...
        for (i = 1; i < r; i = i+1)
            begin
              ...
              for (cnt = 2; cnt < (k+r); cnt = cnt+1)
                  begin
                    a = ((a & ((~(cnt[i-1] & data_parity_i[cnt]) | data_parity_i[cnt]) & (~(cnt[i-1] & data_parity_i[cnt]) | cnt[i-1]))) & (~(cnt[i-1] & data_parity_i[cnt]) | ((~a & cnt[i-1]) & data_parity_i[cnt]))) | ((((cnt[i-1] & data_parity_i[cnt]) & (a | ~a)) & ~(cnt[i-1] & data_parity_i[cnt])) | (~a & ((cnt[i-1] & data_parity_i[cnt]) & (a | ~a))));
                  end
              Parity[i] = a;
            end
