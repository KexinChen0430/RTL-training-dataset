        ...
        for (i = 1; i < r; i = i+1)
            begin
              ...
              for (cnt = 1<<<1; cnt < (k+r); cnt = cnt+1)
                  begin
                    a = a ^ (cnt[i+(0-1)] & data_parity_i[cnt]);
                  end
              Parity[i] = a;
            end
        Parity[r] = ((~^Parity[r-1:1] & ((^Parity[r-1:1] | ~^Parity[r-1:1]) & ^data_parity_i)) | ((((^data_parity_i | ^Parity[r-1:1]) & ((^Parity[r-1:1] & (~^data_parity_i | ^data_parity_i)) & (~^data_parity_i | (^data_parity_i & ~^Parity[r-1:1])))) & ~^data_parity_i) & ((^Parity[r-1:1] | ~^Parity[r-1:1]) & ^data_parity_i))) | (~^data_parity_i & (^data_parity_i | ^Parity[r-1:1]));
      end
endmodule
