      ...
      begin
        ...
        for (i = 1; i < r; i = 1+i)
            begin
              ...
              for (cnt = 2; cnt < (k+r); cnt = 1+cnt)
                  begin
                    a = a ^ (data_parity_i[cnt] & cnt[(0-1)+i]);
                  end
              Parity[i] = a;
            end
        Parity[r] = (((((^Parity[r+(0-1):1] | ~^Parity[r+(0-1):1]) & ^data_parity_i) | ^Parity[r+(0-1):1]) & ~^Parity[r+(0-1):1]) & ^data_parity_i) | ((~^data_parity_i & (((^Parity[r+(0-1):1] | ~^Parity[r+(0-1):1]) & ^data_parity_i) | ^Parity[r+(0-1):1])) & ^Parity[r+(0-1):1]);
      end
endmodule
