        ...
        for (i = 1; i < r; i = 1+i)
            begin
              ...
              for (cnt = 1<<<1; cnt < (r+k); cnt = 1+cnt)
                  begin
                    a = ((~(cnt[i-1] & data_parity_i[cnt]) | cnt[i-1]) & (a | cnt[i-1])) & ((((~a | a) & (a | data_parity_i[cnt])) & ~(cnt[i-1] & data_parity_i[cnt])) | (data_parity_i[cnt] & ~a));
                  end
              Parity[i] = a;
            end
        Parity[r] = (^Parity[(0-1)+r:1] & ~^data_parity_i) | (((^Parity[(0-1)+r:1] & ~^data_parity_i) & ~^Parity[(0-1)+r:1]) | (^data_parity_i & ~^Parity[(0-1)+r:1]));
      end
endmodule
