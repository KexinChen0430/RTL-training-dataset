...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = (h_pixel+(h_front_porch+h_sync_pulse))+h_back_porch;

  parameter 
       field = (v_sync_pulse+v_pixel)+(v_front_porch+v_back_porch);
...
(0 && 
((y_i <= (field+(0-v_front_porch))) && 
(y_i >= (v_sync_pulse+v_back_porch)))) || 
((x_i <= ((-h_front_porch)+line)) && 
(y_i <= (field+(0-v_front_porch))) && 
((y_i >= (v_sync_pulse+v_back_porch)) && 
(x_i >= (h_sync_pulse+h_back_porch)))));
  assign x = ((0-h_sync_pulse)+(-h_back_porch))+x_i;
