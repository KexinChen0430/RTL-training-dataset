...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = ((h_back_porch+h_pixel)+h_sync_pulse)+h_front_porch;

  parameter 
       field = (v_sync_pulse+v_back_porch)+(v_front_porch+v_pixel);
...
(y_i <= (field-v_front_porch)) && ((x_i <= (line-h_front_porch)) && 0)) || 
(0 && 
((y_i >= (v_sync_pulse+v_back_porch)) && 
(y_i <= (field-v_front_porch)))) || 0) && (0 || (y_i >= (v_sync_pulse+v_back_porch)));
  assign x = (x_i-h_back_porch)-h_sync_pulse;
  assign y = (-v_back_porch)+(y_i+(0-v_sync_pulse));
endmodule
