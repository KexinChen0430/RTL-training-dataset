...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = (h_sync_pulse+(h_pixel+h_front_porch))+h_back_porch;
...
(x_i <= (line-h_front_porch)) && 0) || ((y_i >= (v_sync_pulse+v_back_porch)) && 0) || 
((x_i >= (h_sync_pulse+h_back_porch)) && 
...
(
((x_i >= (h_sync_pulse+h_back_porch)) && 
(x_i <= (line-h_front_porch))) || ((x_i >= (h_sync_pulse+h_back_porch)) && 0)) && (0 || (y_i <= (field+(0-v_front_porch))))) || 0);
  assign x = (-h_back_porch)+(x_i+(0-h_sync_pulse));
