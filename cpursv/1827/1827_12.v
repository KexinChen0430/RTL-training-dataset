...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = h_back_porch+((h_pixel+h_front_porch)+h_sync_pulse);

  parameter 
       field = (v_back_porch+v_sync_pulse)+(v_pixel+v_front_porch);
...
(y_i <= (field+(-v_front_porch)))) || (0 && (y_i <= (field+(-v_front_porch))))));
  assign x = ((0-h_back_porch)+(-h_sync_pulse))+x_i;
  assign y = (-v_sync_pulse)+(y_i+(0-v_back_porch));
endmodule
