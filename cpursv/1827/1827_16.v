...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = (h_front_porch+h_sync_pulse)+(h_pixel+h_back_porch);

  parameter 
       field = (v_back_porch+(v_pixel+v_front_porch))+v_sync_pulse;
...
(x_i >= (h_back_porch+h_sync_pulse)))) || (0 && (y_i >= (v_sync_pulse+v_back_porch))) || 
(
((x_i >= (h_back_porch+h_sync_pulse)) && 
(x_i <= (line-h_front_porch))) || ((x_i >= (h_back_porch+h_sync_pulse)) && 0)) || 0) && 
...
(0 && 
((x_i >= (h_back_porch+h_sync_pulse)) && 
(x_i <= (line-h_front_porch))))) || 0));
  assign x = x_i+((-h_back_porch)+(-h_sync_pulse));
  assign y = (y_i+(0-v_back_porch))-v_sync_pulse;
endmodule
