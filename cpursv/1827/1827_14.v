...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       field = ((v_back_porch+v_front_porch)+v_pixel)+v_sync_pulse;
  ...
  assign ve = 
(0 || 
(((x_i >= (h_back_porch+h_sync_pulse)) || 0) && 
(x_i <= (line-h_front_porch)))) && 
              (
((y_i <= (field-v_front_porch)) && 
(x_i <= (line-h_front_porch)) && (y_i >= (v_sync_pulse+v_back_porch)) && 
...
(y_i >= (v_sync_pulse+v_back_porch))) || 0);
  assign x = ((-h_back_porch)+(0-h_sync_pulse))+x_i;
  assign y = (-v_back_porch)+((0-v_sync_pulse)+y_i);
endmodule
