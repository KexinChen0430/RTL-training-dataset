...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = (h_pixel+h_back_porch)+(h_sync_pulse+h_front_porch);

  parameter 
       field = (v_pixel+(v_front_porch+v_back_porch))+v_sync_pulse;
...
(y_i >= (v_back_porch+v_sync_pulse)) && (x_i >= (h_sync_pulse+h_back_porch)) && 
...
(x_i <= (line-h_front_porch)))) || (x_i >= (h_sync_pulse+h_back_porch))) && 
((y_i <= (field-v_front_porch)) && 
(y_i >= (v_back_porch+v_sync_pulse))) && 0) || 
((y_i <= (field-v_front_porch)) && 
(y_i >= (v_back_porch+v_sync_pulse))))) || 0));
  assign x = x_i+((0-h_back_porch)+(0-h_sync_pulse));
  assign y = (y_i-v_back_porch)+(0-v_sync_pulse);
endmodule
