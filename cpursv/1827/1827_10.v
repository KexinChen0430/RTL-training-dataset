...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = ((h_back_porch+h_sync_pulse)+h_front_porch)+h_pixel;

  parameter 
       field = (v_sync_pulse+v_front_porch)+(v_pixel+v_back_porch);
  ...
  assign ve = 
(
((0 && (y_i >= (v_sync_pulse+v_back_porch))) || 
((y_i <= ((-v_front_porch)+field)) && 
(y_i >= (v_sync_pulse+v_back_porch)) && 0) || 
((y_i <= ((-v_front_porch)+field)) && 
(y_i >= (v_sync_pulse+v_back_porch))) || 0) && 
(
((x_i <= (line+(-h_front_porch))) && 
(x_i >= (h_back_porch+h_sync_pulse))) || ((x_i <= (line+(-h_front_porch))) && 0)) && 
(
((y_i <= ((-v_front_porch)+field)) && 
(y_i >= (v_sync_pulse+v_back_porch)) && 0) || 
((x_i <= (line+(-h_front_porch))) && 
(x_i >= (h_back_porch+h_sync_pulse))) || (0 && (x_i >= (h_back_porch+h_sync_pulse))) || 
((y_i >= (v_sync_pulse+v_back_porch)) && 
(x_i <= (line+(-h_front_porch))) && 
((y_i <= ((-v_front_porch)+field)) && 
(x_i >= (h_back_porch+h_sync_pulse)))) || 0)) || 0;
  assign x = ((-h_back_porch)+x_i)-h_sync_pulse;
