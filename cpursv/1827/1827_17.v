...
module vga(hsync,vsync,x,y,ve,newline,newfield,clk_p,rst);
  ...
  parameter 
       line = h_front_porch+(h_back_porch+(h_sync_pulse+h_pixel));

  parameter 
       field = v_sync_pulse+(v_front_porch+(v_back_porch+v_pixel));
...
(x_i <= ((-h_front_porch)+line)) && 
(
((y_i <= (field-v_front_porch)) && 
(y_i >= (v_back_porch+v_sync_pulse))) || ((y_i <= (field-v_front_porch)) && 0))) || 
              ((x_i <= ((-h_front_porch)+line)) && 0 && 
(x_i >= (h_back_porch+h_sync_pulse))) || (0 && (x_i >= (h_back_porch+h_sync_pulse))) || 0;
  assign x = x_i+((0-h_back_porch)+(-h_sync_pulse));
  assign y = y_i+((0-v_sync_pulse)+(0-v_back_porch));
endmodule
