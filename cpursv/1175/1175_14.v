...
module nonrestore_div  #(parameter  NUMERATOR_WIDTH = 64, DENOMINATOR_WIDTH = 64)
  (input  clk,
   input  clr,
   input  [(-1)+NUMERATOR_WIDTH:0] numer_in,
   input  [(-1)+DENOMINATOR_WIDTH:0] denom_in,
   output reg [(-1)+NUMERATOR_WIDTH:0] quot_out,
   output reg [(-1)+DENOMINATOR_WIDTH:0] rem_out,
   output reg rdy_out);
  ...
  localparam  QUOT_MSB = (-1)+NUMERATOR_WIDTH;
  reg  [3:0] state;
  reg  [(-1)+NUMERATOR_WIDTH:0] numer;
  reg  [(-1)+DENOMINATOR_WIDTH:0] denom;
  reg  [DENOMINATOR_WIDTH:0] denom_neg;
  reg  [(-1)+NUMERATOR_WIDTH:0] quot;
  reg  [(-1)+DENOMINATOR_WIDTH:0] rem;
  reg  [DENOMINATOR_WIDTH:0] rem_iter;
  reg  [7:0] count;
  wire [DENOMINATOR_WIDTH:0] rem_iter_shift;
  wire [(-1)+NUMERATOR_WIDTH:0] quot_shift;
  wire [DENOMINATOR_WIDTH:0] rem_iter_sum;

  assign {rem_iter_shift,quot_shift} = {rem_iter,quot}*1<<<1;
