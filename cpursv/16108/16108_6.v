...
module raycast_stack(clk,push,pop,data_i,data_o);
  ...
  input  pop;
  input  [(-1)+dw:0] data_i;
  output [(-1)+dw:0] data_o;
  reg  [(-1)+dw:0] stack_data[0:depth+(-1)];
  reg  [(-1)+depth_log2:0]  stack_ptr = 0;
  wire [(-1)+depth_log2:0]  stack_ptr_inc = stack_ptr+1;
  wire [(-1)+depth_log2:0]  stack_ptr_dec = stack_ptr-1;
