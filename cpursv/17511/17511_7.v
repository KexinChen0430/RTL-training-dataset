...
module MemoryMultiplex(input  clk,
  ...
  assign memory_read_cpu = (saved_prg_read || !(chr_read || chr_write)) && 
                           (prg_read || 
(saved_prg_read && 
((prg_read && saved_prg_read) || 
!(chr_read || chr_write))) || saved_prg_read) && !(chr_read || chr_write);
  assign memory_dout = chr_write ? chr_din : prg_din;
  
  always @(posedge clk)
      if (ce) 
        begin
          if (chr_read || chr_write) 
            begin
              saved_prg_read <= prg_read || 
                               (saved_prg_read && 
((prg_read && saved_prg_read) || 
!(chr_read || chr_write))) || saved_prg_read;
