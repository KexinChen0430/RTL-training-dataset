...
module b2m_video(input  clk50,
  ...
  always @(*)
      begin
        ...
        hr = ((h_cnt0 < 10'd480) && (h_cnt0 >= 10'd420)) ? 1'b0 : 1'b1;
        vr = ((v_cnt0 >= 10'd565) && (v_cnt0 < 10'd571)) ? 1'b0 : 1'b1;
        vout = (h_cnt0 < 10'd389) && 
               ((h_cnt0 >= 10'd5) && (v_cnt0 < 10'd512));
        addr = {h_cnt0[8:3],v_cnt0[8:1]};
        drq = (state != 0) && 
              ((h_cnt0[2] == 0) && (h_cnt0[8:7] < 2'b11)) && (v_cnt0[9] == 0);
        vid_irq = ~v_cnt0[9];
      end
  reg  [1:0] state;
  ...
  always @(posedge clk50)
      begin
        ...
        if (state == 2'b00) 
          begin
            if ((h_cnt0[8:3] < 6'h30) && (h_cnt0[2:0] == 3'b100)) data0 <= idata;
            else data0 <= {1'b0,data0[15:1]};
            if ((h_cnt0+1'b1) == 10'd532) 
              begin
                h_cnt0 <= 0;
                if ((1'b1+v_cnt0) == 10'd624) v_cnt0 <= 0;
                else v_cnt0 <= 1'b1+v_cnt0;
              end
            else h_cnt0 <= h_cnt0+1'b1;
          end
          
        if (!pal_we_n) palette[pal_idx] <= pal_data;
          
        addr1 <= {h_cnt1[8:3],v_cnt1[8:1]};
        drq1 <= (h_cnt1[8:7] < 2'b11) && (h_cnt1[2] == 0) && 
               (v_cnt1[9] == 0);
        irq1 <= ~v_cnt1[9];
      end
endmodule
