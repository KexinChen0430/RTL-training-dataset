...
module gci_std_display_sequencer  #(parameter 
       P_AREA_H     = 640,
       P_AREA_V     = 480,
       P_AREAA_HV_N = 19,
       P_MEM_ADDR_N = 23)
  (input  wire iCLOCK,
   ...
   input  wire iIF_BUSY,
   output wire [(-1)+P_MEM_ADDR_N:0] oIF_ADDR,
        ...
        if (!inRESET) 
          ...
          begin
            case (b_clear_state)
              ...
              P_L_CLEAR_STT_IDLE: begin
                    if (iIF_VALID) 
                      begin
                        b_clear_state <= P_L_CLEAR_STT_CLEAR;
                        {b_clear_color_r,b_clear_color_g,b_clear_color_b} <= iIF_DATA[23:0];
                      end
                      
                  end

              P_L_CLEAR_STT_CLEAR: begin
                    if (b_clear_counter == (P_AREA_V*P_AREA_H)) 
                      begin
  ...
  always @(posedge iCLOCK or negedge inRESET)
      begin
        if (!inRESET) 
          ...
          begin
            if (b_clear_state == P_L_CLEAR_STT_CLEAR) 
              begin
                if (!iIF_BUSY) 
                  begin
                    b_clear_counter <= {{P_AREAA_HV_N+(-1){1'b0}},1'b1}+b_clear_counter;
                  end
  ...
  assign oIF_VALID = (b_clear_state == P_L_CLEAR_STT_CLEAR) && !iIF_BUSY;
