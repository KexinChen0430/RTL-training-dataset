...
module gci_std_display_sequencer  #(parameter 
  ...
  (input  wire iCLOCK,
   ...
   input  wire iIF_BUSY,
   output wire [P_MEM_ADDR_N+(0-1):0] oIF_ADDR,
  ...
  reg  [P_AREAA_HV_N+(0-1):0] b_clear_counter;
          ...
          begin
            case (b_clear_state)
              ...
              P_L_CLEAR_STT_CLEAR: begin
                    if (b_clear_counter == (P_AREA_H*P_AREA_V)) 
                      begin
          ...
          begin
            if (b_clear_state == P_L_CLEAR_STT_CLEAR) 
              begin
                if (!iIF_BUSY) 
                  begin
                    b_clear_counter <= b_clear_counter+{{P_AREAA_HV_N+(0-1){1'b0}},1'b1};
                  end
