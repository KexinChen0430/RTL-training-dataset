...
module debug_state(input  av_clk,
  ...
  always @(posedge lsa_clk)
      ...
      else if (!sample_waddr[8] && |sample_waddr[7:0]) sample_waddr <= sample_waddr+9'd1;
      else if (sample_waddr == 9'h100) 
        sample_waddr <= (lsa_trigger || lsa_force) ? 9'h101 : 9'h100;
      else if (sample_next && (sample_waddr != 9'h000)) sample_waddr <= sample_waddr+9'd1;
  ...
  always @(posedge lsa_clk)
      begin
        ...
        sample_cnt <= (!sample_running || sample_next) ? 32'd1 : 
                     (sample_cnt != 32'hFFFFFFFF) ? (sample_cnt+32'd1) : sample_cnt;
      end
