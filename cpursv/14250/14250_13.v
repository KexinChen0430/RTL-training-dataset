...
module alu(output reg [31:0] ALUResult,
  ...
  assign {Cout,S} = A+(control[2]+muxBout);
  assign andResult = muxBout & A;
  assign orResult = muxBout | A;
  assign SLT = {{31{1'b0}},{S[31]}};
  
  always @(control[1:0] or andResult or orResult or S or SLT or ALUResult)
      begin
        case (control[1:0])
