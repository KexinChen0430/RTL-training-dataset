...
(yCounter <= C_VERT_SYNC_END));
  ...
  always @(pixel_colour)
      begin
        ...
        if (MONOCHROME == FALSE) 
          begin
            for (index = 10-BITS_PER_COLOUR_CHANNEL; index >= 0; index = (0-BITS_PER_COLOUR_CHANNEL)+index)
                begin
                  for (sub_index = (0-1)+BITS_PER_COLOUR_CHANNEL; sub_index >= 0; sub_index = sub_index+(0-1))
                      begin
                        VGA_R[index+sub_index] <= pixel_colour[sub_index+BITS_PER_COLOUR_CHANNEL<<1];
