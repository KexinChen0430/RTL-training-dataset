...
module vga_controller(vga_clock,resetn,pixel_colour,memory_address,VGA_R,VGA_G,
  ...
  input  [(MONOCHROME == TRUE) ? 0 : ((3*BITS_PER_COLOUR_CHANNEL)-1):0] pixel_colour;
        ...
        if (MONOCHROME == FALSE) 
          begin
            for (index = 10-BITS_PER_COLOUR_CHANNEL; index >= 0; index = index-BITS_PER_COLOUR_CHANNEL)
                begin
                  for (sub_index = BITS_PER_COLOUR_CHANNEL+(0-1); sub_index >= 0; sub_index = sub_index-1)
                      begin
                        VGA_R[index+sub_index] <= pixel_colour[sub_index+(1<<<1*BITS_PER_COLOUR_CHANNEL)];
                        VGA_G[index+sub_index] <= pixel_colour[sub_index+BITS_PER_COLOUR_CHANNEL];
