...
module vga_controller(vga_clock,resetn,pixel_colour,memory_address,VGA_R,VGA_G,
  ...
  input  [(MONOCHROME == TRUE) ? 0 : ((-1)+(BITS_PER_COLOUR_CHANNEL*3)):0] pixel_colour;
  ...
  assign xCounter_clear = xCounter == ((-1)+C_HORZ_TOTAL_COUNT);
  ...
  assign yCounter_clear = yCounter == ((-1)+C_VERT_TOTAL_COUNT);
...
(yCounter <= C_VERT_SYNC_END));
  ...
  always @(pixel_colour)
      begin
        ...
        if (MONOCHROME == FALSE) 
          begin
            for (index = (-BITS_PER_COLOUR_CHANNEL)+10; index >= 0; index = index-BITS_PER_COLOUR_CHANNEL)
                begin
                  for (sub_index = (-1)+BITS_PER_COLOUR_CHANNEL; sub_index >= 0; sub_index = sub_index-1)
                      begin
                        VGA_R[sub_index+index] <= pixel_colour[sub_index+(1<<<1*BITS_PER_COLOUR_CHANNEL)];
