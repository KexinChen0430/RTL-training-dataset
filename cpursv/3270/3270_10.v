        ...
        if (MONOCHROME == FALSE) 
          begin
            for (index = 10+(0-BITS_PER_COLOUR_CHANNEL); index >= 0; index = index+(0-BITS_PER_COLOUR_CHANNEL))
                begin
                  for (sub_index = BITS_PER_COLOUR_CHANNEL-1; sub_index >= 0; sub_index = sub_index+(0-1))
                      begin
                        VGA_R[index+sub_index] <= pixel_colour[BITS_PER_COLOUR_CHANNEL<<1+sub_index];
                        VGA_G[index+sub_index] <= pixel_colour[BITS_PER_COLOUR_CHANNEL+sub_index];
                        VGA_B[index+sub_index] <= pixel_colour[sub_index];
                      end
                end
          end
        else 
