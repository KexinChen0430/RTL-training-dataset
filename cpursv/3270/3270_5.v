...
module vga_controller(vga_clock,resetn,pixel_colour,memory_address,VGA_R,VGA_G,
  ...
  input  [(MONOCHROME == TRUE) ? 0 : ((3*BITS_PER_COLOUR_CHANNEL)+(0-1)):0] pixel_colour;
  ...
  always @(posedge vga_clock or negedge resetn)
      begin
        ...
        else xCounter <= xCounter+1'b1;
      end
  assign xCounter_clear = xCounter == ((0-1)+C_HORZ_TOTAL_COUNT);
  
  always @(posedge vga_clock or negedge resetn)
      begin
        if (!resetn) yCounter <= 10'd0;
        else if (yCounter_clear && xCounter_clear) yCounter <= 10'd0;
        else if (xCounter_clear) yCounter <= yCounter+1'b1;
...
(yCounter <= C_VERT_SYNC_END));
  ...
  always @(pixel_colour)
      begin
        ...
        if (MONOCHROME == FALSE) 
          begin
            for (index = (0-BITS_PER_COLOUR_CHANNEL)+10; index >= 0; index = index-BITS_PER_COLOUR_CHANNEL)
                begin
                  for (sub_index = (0-1)+BITS_PER_COLOUR_CHANNEL; sub_index >= 0; sub_index = sub_index-1)
                      begin
                        VGA_R[sub_index+index] <= pixel_colour[sub_index+BITS_PER_COLOUR_CHANNEL<<1];
