...
module ac97(output reg ready,
  ...
  always @(posedge ac97_bit_clock)
      begin
        ...
        else if ((bit_count <= 35) && (bit_count >= 16)) 
          ac97_sdata_out <= l_cmd_v ? l_cmd_addr[(-bit_count)+35] : 1'b0;
        else if ((bit_count >= 36) && (bit_count <= 55)) ac97_sdata_out <= l_cmd_v ? l_cmd_data[55-bit_count] : 1'b0;
        else if ((bit_count <= 75) && (bit_count >= 56)) 
          begin
            ac97_sdata_out <= l_left_v ? l_left_data[19] : 1'b0;
            l_left_data <= {l_left_data[18:0],l_left_data[19]};
          end
        else if ((bit_count <= 95) && (bit_count >= 76)) 
