...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = (&Inst[1:0] & ~Inst[2]) & ((Inst[5] & Inst[3]) & ~Inst[4]);
  assign beq = (&~Inst[1:0] & &~Inst[5:3]) & Inst[2];
  assign j = &~Inst[5:2] & Inst[1];
  assign addi = Inst[3] & (&~Inst[2:0] & &~Inst[5:4]);
  assign subi = &~Inst[5:3] & Inst[2:0];
  assign jr0 = (Fn[3] & RFormat) & (&~Fn[2:0] & &~Fn[5:4]);
  assign RegDst[0] = RFormat;
  assign RegDst[1] = Inst[0] & j;
  assign ALUSrc = (addi | subi) | (lw | sw);
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = Inst[0] & j;
  assign RegWrite = (~jr0 & (addi | (lw | ((Inst[0] & j) | (lw & Inst[0]))))) | (~jr0 & (RFormat | subi));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
  ...
  assign FlushIF = ((jr0 | beq && Compare) | j) ? 1'b1 : 1'b0;
endmodule
