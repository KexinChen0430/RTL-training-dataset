...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = (Inst[3] & &Inst[1:0]) & (~Inst[2] & (Inst[5] & ~Inst[4]));
  ...
  assign jr0 = (RFormat & (Fn[3] & &~Fn[5:4])) & &~Fn[2:0];
  assign RegDst[0] = RFormat;
  assign RegDst[1] = Inst[0] & j;
  assign ALUSrc = (addi | lw) | (sw | subi);
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = Inst[0] & j;
  assign RegWrite = ((((~jr0 & subi) | j) & (Inst[0] | (~jr0 & subi))) & ~jr0) | (((addi | lw) & ~jr0) | (~jr0 & RFormat));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
