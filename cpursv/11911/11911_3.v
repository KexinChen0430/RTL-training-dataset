...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = &Inst[1:0] & (((Inst[3] & ~Inst[4]) & ~Inst[2]) & Inst[5]);
  ...
  assign ALUSrc = (sw | lw) | (subi | addi);
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = j & Inst[0];
  assign RegWrite = (~jr0 & (subi | (RFormat | lw))) | ((Inst[0] & ((~jr0 & j) | (~jr0 & addi))) | (((~jr0 & j) | (~jr0 & addi)) & addi));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
