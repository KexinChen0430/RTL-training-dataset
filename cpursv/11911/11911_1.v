...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign ALUSrc = ((subi | addi) | lw) | sw;
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = j & Inst[0];
  assign RegWrite = ((~jr0 & subi) | (~jr0 & ((lw | (j & RFormat)) | RFormat))) | ((((j & (Inst[0] | (subi | addi))) | addi) | subi) & ~jr0);
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
  ...
  assign FlushIF = (jr0 | (Compare && beq | j)) ? 1'b1 : 1'b0;
endmodule
