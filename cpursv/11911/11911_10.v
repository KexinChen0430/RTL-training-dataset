...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = (Inst[5] & (~Inst[2] & (~Inst[4] & &Inst[1:0]))) & Inst[3];
  ...
  assign ALUSrc = ((lw | (addi & (lw | RFormat))) | addi) | (subi | sw);
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = j & Inst[0];
  assign RegWrite = ((subi & ~jr0) | ((addi | (subi & ~jr0)) & ~jr0)) | ((((~jr0 & Inst[0]) | (~jr0 & (lw | (j & (lw | RFormat))))) | (RFormat & ~jr0)) & ((j | ((lw | (addi & (lw | RFormat))) | addi)) | (((j & (lw | RFormat)) | RFormat) & ((j | RFormat) | (lw | (j & (lw | RFormat)))))));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
  ...
  assign FlushIF = (jr0 | (j | beq && Compare)) ? 1'b1 : 1'b0;
endmodule
