...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = (Inst[5] & (~Inst[4] & Inst[3])) & (~Inst[2] & &Inst[1:0]);
  ...
  assign jr0 = Fn[3] & ((&~Fn[2:0] & RFormat) & &~Fn[5:4]);
  assign RegDst[0] = RFormat;
  assign RegDst[1] = Inst[0] & j;
  assign ALUSrc = subi | ((sw | lw) | addi);
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = Inst[0] & j;
  assign RegWrite = (~jr0 & ((j & lw) | ((Inst[0] | RFormat) & j))) | ((((~jr0 & ((lw | RFormat) & j)) | ((RFormat | ((lw | RFormat) & j)) & ~jr0)) | (~jr0 & lw)) | ((~jr0 & addi) | (subi & ~jr0)));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
  ...
  assign FlushIF = (j | (Compare && beq | jr0)) ? 1'b1 : 1'b0;
endmodule
