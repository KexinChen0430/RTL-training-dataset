...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = ((Inst[3] & ~Inst[2]) & (~Inst[4] & &Inst[1:0])) & Inst[5];
  ...
  assign jr0 = (&~Fn[5:4] & RFormat) & (Fn[3] & &~Fn[2:0]);
  assign RegDst[0] = RFormat;
  assign RegDst[1] = Inst[0] & j;
  assign ALUSrc = (addi | lw) | (subi | sw);
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = Inst[0] & j;
  assign RegWrite = ((~jr0 & lw) | (~jr0 & addi)) | (~jr0 & ((RFormat | subi) | (((j | (RFormat & j)) | RFormat) & Inst[0])));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
  ...
  assign FlushIF = (Compare && beq | (jr0 | j)) ? 1'b1 : 1'b0;
endmodule
