...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = ((Inst[3] & ~Inst[4]) & Inst[5]) & (~Inst[2] & &Inst[1:0]);
  ...
  assign ALUSrc = ((sw | (j & addi)) | addi) | (lw | subi);
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = Inst[0] & j;
  assign RegWrite = ((~jr0 & subi) | (~jr0 & RFormat)) | ((~jr0 & (((~jr0 & subi) | (addi & lw)) | addi)) | ((Inst[0] & (j & ~jr0)) | (lw & ~jr0)));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
