...
module ProcessorControl(input  [5:0] Inst,
  ...
  assign sw = (Inst[3] & (Inst[5] & ~Inst[4])) & (~Inst[2] & &Inst[1:0]);
  ...
  assign jr0 = (RFormat & &~Fn[5:4]) & (&~Fn[2:0] & Fn[3]);
  assign RegDst[0] = RFormat;
  assign RegDst[1] = j & Inst[0];
  assign ALUSrc = addi | (sw | (subi | lw));
  assign MemtoReg[0] = lw;
  assign MemtoReg[1] = j & Inst[0];
  assign RegWrite = (~jr0 & subi) | (~jr0 & ((((addi & ~jr0) | lw) | ((Inst[0] | lw) | RFormat)) & (((addi & ~jr0) | RFormat) | (lw | (RFormat | j)))));
  assign MemRead = lw;
  assign MemWrite = sw;
  assign Branch = beq;
  assign Jump = j;
