...
module tmds_encoder_24(input  clk,
  ...
  always @* 
      begin
        ...
        M_num_ones_d = ((data_in[7 -: 1]+data_in[6 -: 1])+((data_in[1<<1 -: 1]+data_in[1 -: 1])+(data_in[3 -: 1]+data_in[0 -: 1])))+(data_in[5 -: 1]+data_in[4 -: 1]);
        M_din_d = data_in;
...
((M_num_ones_q > 3'h4) || (M_num_ones_q == 3'h4)));
        ...
        for (i = 1'h1; i < 4'h8; i = 1+i)
            begin
              data9[i -: 1] = xor_flag ? ~(data9[i+(0-1'h1) -: 1] ^ ((~data9[i+(0-1'h1) -: 1] & M_din_q[i -: 1]) | (data9[i+(0-1'h1) -: 1] & M_din_q[i -: 1]))) : (data9[i+(0-1'h1) -: 1] ^ ((~data9[i+(0-1'h1) -: 1] & M_din_q[i -: 1]) | (data9[i+(0-1'h1) -: 1] & M_din_q[i -: 1])));
            end
        data9[8 -: 1] = ~xor_flag;
        M_num_ones9_d = ((data9[6 -: 1]+data9[7 -: 1])+((data9[4 -: 1]+data9[3 -: 1])+(data9[0 -: 1]+(data9[1<<1 -: 1]+data9[1 -: 1]))))+data9[5 -: 1];
        M_data9_pipe_d = data9;
        cond_flip = (M_count_q == 1'h0) || (M_num_ones9_q == 3'h4);
        flip = 
(
(~M_count_q[4 -: 1] && 
((3'h4 > M_num_ones9_q) || (M_num_ones9_q > 3'h4)) && M_count_q[4 -: 1]) || (M_count_q[4 -: 1] && (3'h4 > M_num_ones9_q)) || 
~M_count_q[4 -: 1]) && 
               (
(~M_count_q[4 -: 1] && 
((3'h4 > M_num_ones9_q) || (M_num_ones9_q > 3'h4)) && M_count_q[4 -: 1]) || (M_count_q[4 -: 1] && (3'h4 > M_num_ones9_q)) || 
          ...
          begin
            if (cond_flip) 
              begin
                ...
                M_count_d = ~M_data9_pipe_q[8 -: 1] ? (((4'h8+(-M_num_ones9_q))+M_count_q)-M_num_ones9_q) : (M_count_q+((-(4'h8+(-M_num_ones9_q)))+M_num_ones9_q));
              end
            else 
              begin
                if (flip) 
                  begin
                    ...
                    M_count_d = ((4'h8+(-M_num_ones9_q))+(M_count_q+{M_data9_pipe_q[8 -: 1],1'h0}))-M_num_ones9_q;
                  end
                else 
                  begin
                    M_dout_d[9 -: 1] = 1'h0;
