...
module regfile_cr  #(parameter  WIDTH = 32, RSELWIDTH = 4)
  ...
  reg  [WIDTH+(0-1):0] R[0:(2**RSELWIDTH)+(0-1)];
  integer k;

  
  initial  
  begin
    for (k = 0; k < (2**RSELWIDTH); k = k+1)
        R[k] = 0;
  end
  ...
  always @(posedge clk)
      begin
        if (we) 
          begin
            if (wsel[RSELWIDTH+(0-1)]) 
              begin
                R[wsel[RSELWIDTH+(0-2):0]] <= ~wdata;
              end
            else 
              begin
                R[wsel[RSELWIDTH+(0-2):0]] <= wdata;
  ...
  assign adata = asel[RSELWIDTH+(0-1)] ? ~R[asel[RSELWIDTH+(0-2):0]] : R[asel];
endmodule
