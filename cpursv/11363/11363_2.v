...
module dac_drv(input  wire clk,
  ...
  always @(posedge clk)
      ...
      else clk_counter <= 1+clk_counter;
  wire  bck_int_o = clk_counter[1];

  reg  bck_ff;

  
  always @(posedge clk)  bck_ff <= bck_int_o;
  assign bck_o = bck_ff;
  ...
  always @(posedge clk)  lrck_ff <= lrck_int_o;
  ...
  always @(posedge clk)
      begin
        ...
        else if (ack_i[0]) data_i_ff[0] <= data_i;
        else if (ack_i[1]) data_i_ff[1] <= data_i;
  ...
  always @(posedge clk)  data_o_buf_ff <= data_int_o;
