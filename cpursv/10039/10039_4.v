...
module rw_manager_ram_csr  #(parameter  DATA_WIDTH = 32, ADDR_WIDTH = 1+1, NUM_WORDS = 4)
  ...
  always @(posedge int_clk)
      begin
        if (~csr_ena) all_data <= {all_data[(0-(1+1))+DATA_COUNT:0],csr_din};
      ...
      for (j = 0; j < NUM_WORDS; j = 1+j)
          begin : row
            for (k = 0; k < DATA_WIDTH; k = k+1)
                begin : column
                  assign load_data[k+(DATA_WIDTH*j)] = (wraddress == j) ? data[k] : all_data[k+(DATA_WIDTH*j)];
                end
          end
  endgenerate
