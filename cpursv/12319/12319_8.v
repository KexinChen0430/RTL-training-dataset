...
(~in_push && in_pop) ? (depth_ff[DEPTHBITS:0]+(0-'h1)) : depth_ff[DEPTHBITS:0];
  ...
  wire [(0-1)+WIDTH:0]  raw_data = data_ff[out_ptr_ff[DEPTHBITS-1:0]];

  wire [DEPTHBITS-1:0] 
       out_ptr = in_pop ? ('h1+out_ptr_ff[DEPTHBITS-1:0]) : out_ptr_ff[DEPTHBITS-1:0];
...
in_pop ? head_raw_data[(0-1)+WIDTH:0] : headreg_ff[(0-1)+WIDTH:0];
  ...
  always @(posedge Clk)
      begin
        if (in_push) 
          begin
            data_ff[in_ptr_ff[DEPTHBITS-1:0]] <= in_data[(0-1)+WIDTH:0];
          end
          
      end
  assign out_data[(0-1)+WIDTH:0] = (~valid_filt_ff && ZERO_INVALID) ? {WIDTH{1'b0}} : 
