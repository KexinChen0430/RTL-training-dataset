...
module axi_traffic_gen_v2_0_7_ex_fifo  #(parameter 
  ...
  reg  [WIDTH-1:0] data_ff[(-1)+DEPTH:0];
  ...
  wire [(-1)+DEPTHBITS:0] 
       in_ptr = in_push ? (in_ptr_ff[(-1)+DEPTHBITS:0]+'h1) : in_ptr_ff[(-1)+DEPTHBITS:0];
  wire [DEPTHBITS:0] 
...
(~in_push && in_pop) ? (depth_ff[DEPTHBITS:0]-'h1) : depth_ff[DEPTHBITS:0];
  ...
  wire [WIDTH-1:0]  raw_data = data_ff[out_ptr_ff[(-1)+DEPTHBITS:0]];

  wire [(-1)+DEPTHBITS:0] 
       out_ptr = in_pop ? (out_ptr_ff[(-1)+DEPTHBITS:0]+'h1) : out_ptr_ff[(-1)+DEPTHBITS:0];

  wire [WIDTH-1:0] 
       head_raw_data = depth_was1 ? in_data[WIDTH-1:0] : raw_data[WIDTH-1:0];

  wire [WIDTH-1:0] 
       headreg = (!valid_ff && in_push) ? in_data[WIDTH-1:0] : 
in_pop ? head_raw_data[WIDTH-1:0] : headreg_ff[WIDTH-1:0];
  ...
  always @(posedge Clk)
      begin
        if (in_push) 
          begin
            data_ff[in_ptr_ff[(-1)+DEPTHBITS:0]] <= in_data[WIDTH-1:0];
          end
          
      end
  assign out_data[WIDTH-1:0] = (~valid_filt_ff && ZERO_INVALID) ? {WIDTH{1'b0}} : 
                               HEADREG ? headreg_ff[WIDTH-1:0] : raw_data[WIDTH-1:0];
