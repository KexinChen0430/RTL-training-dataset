...
module axi_traffic_gen_v2_0_7_ex_fifo  #(parameter 
  ...
  reg  [WIDTH-1:0] data_ff[DEPTH+(0-1):0];
  ...
  wire [DEPTHBITS-1:0] 
       in_ptr = in_push ? (in_ptr_ff[DEPTHBITS-1:0]+'h1) : in_ptr_ff[DEPTHBITS-1:0];
  wire [DEPTHBITS:0] 
       depth = (~in_pop && in_push) ? (depth_ff[DEPTHBITS:0]+'h1) : 
(in_pop && ~in_push) ? (depth_ff[DEPTHBITS:0]+(0-'h1)) : depth_ff[DEPTHBITS:0];
