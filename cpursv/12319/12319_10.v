...
module axi_traffic_gen_v2_0_7_ex_fifo  #(parameter 
  ...
  (input  Clk,
   input  rst_l,
   input  [WIDTH+(0-1):0] in_data,
   input  in_push,
   input  in_pop,
   output [WIDTH+(0-1):0] out_data,
   output is_full,
   output is_notfull,
   output is_empty,
   output out_valid,
  ...
  reg  [WIDTH+(0-1):0] data_ff[DEPTH+(0-1):0];
  reg  [(0-1)+DEPTHBITS:0] out_ptr_ff;
  reg  [(0-1)+DEPTHBITS:0] in_ptr_ff;
  reg  [DEPTHBITS:0] depth_ff;
  reg  [WIDTH+(0-1):0] headreg_ff;
  reg  full_ff,notfull_ff,valid_ff,valid_filt_ff;
  wire [(0-1)+DEPTHBITS:0] 
       in_ptr = in_push ? (in_ptr_ff[(0-1)+DEPTHBITS:0]+'h1) : in_ptr_ff[(0-1)+DEPTHBITS:0];
  wire [DEPTHBITS:0] 
       depth = (~in_pop && in_push) ? (depth_ff[DEPTHBITS:0]+'h1) : 
(~in_push && in_pop) ? (depth_ff[DEPTHBITS:0]-'h1) : depth_ff[DEPTHBITS:0];
  ...
  wire [WIDTH+(0-1):0]  raw_data = data_ff[out_ptr_ff[(0-1)+DEPTHBITS:0]];

  wire [(0-1)+DEPTHBITS:0] 
       out_ptr = in_pop ? ('h1+out_ptr_ff[(0-1)+DEPTHBITS:0]) : out_ptr_ff[(0-1)+DEPTHBITS:0];

  wire [WIDTH+(0-1):0] 
       head_raw_data = depth_was1 ? in_data[WIDTH+(0-1):0] : raw_data[WIDTH+(0-1):0];

  wire [WIDTH+(0-1):0] 
       headreg = (in_push && !valid_ff) ? in_data[WIDTH+(0-1):0] : 
in_pop ? head_raw_data[WIDTH+(0-1):0] : headreg_ff[WIDTH+(0-1):0];
  ...
  always @(posedge Clk)
      begin
        ...
        headreg_ff[WIDTH+(0-1):0] <= rst_l ? headreg[WIDTH+(0-1):0] : {WIDTH{1'b0}};
      end
  integer i;
  ...
  always @(posedge Clk)
      begin
        if (in_push) 
          begin
            data_ff[in_ptr_ff[(0-1)+DEPTHBITS:0]] <= in_data[WIDTH+(0-1):0];
          end
          
      end
  assign out_data[WIDTH+(0-1):0] = (~valid_filt_ff && ZERO_INVALID) ? {WIDTH{1'b0}} : 
                                   HEADREG ? headreg_ff[WIDTH+(0-1):0] : raw_data[WIDTH+(0-1):0];
