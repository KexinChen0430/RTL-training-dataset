...
module axi_traffic_gen_v2_0_7_ex_fifo  #(parameter 
  ...
  wire [DEPTHBITS-1:0] 
       in_ptr = in_push ? ('h1+in_ptr_ff[DEPTHBITS-1:0]) : in_ptr_ff[DEPTHBITS-1:0];
  wire [DEPTHBITS:0] 
...
(in_pop && ~in_push) ? (depth_ff[DEPTHBITS:0]-'h1) : depth_ff[DEPTHBITS:0];
  ...
  wire [(-1)+WIDTH:0]  raw_data = data_ff[out_ptr_ff[DEPTHBITS-1:0]];

  wire [DEPTHBITS-1:0] 
       out_ptr = in_pop ? (out_ptr_ff[DEPTHBITS-1:0]+'h1) : out_ptr_ff[DEPTHBITS-1:0];

  wire [(-1)+WIDTH:0] 
       head_raw_data = depth_was1 ? in_data[(-1)+WIDTH:0] : raw_data[(-1)+WIDTH:0];

  wire [(-1)+WIDTH:0] 
       headreg = (in_push && !valid_ff) ? in_data[(-1)+WIDTH:0] : 
in_pop ? head_raw_data[(-1)+WIDTH:0] : headreg_ff[(-1)+WIDTH:0];
  ...
  always @(posedge Clk)
      begin
        if (in_push) 
          begin
            data_ff[in_ptr_ff[DEPTHBITS-1:0]] <= in_data[(-1)+WIDTH:0];
          end
          
      end
  assign out_data[(-1)+WIDTH:0] = (~valid_filt_ff && ZERO_INVALID) ? {WIDTH{1'b0}} : 
