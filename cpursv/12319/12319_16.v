...
module axi_traffic_gen_v2_0_7_ex_fifo  #(parameter 
  ...
  reg  [(0-1)+WIDTH:0] data_ff[DEPTH+(0-1):0];
  reg  [DEPTHBITS+(0-1):0] out_ptr_ff;
  reg  [DEPTHBITS+(0-1):0] in_ptr_ff;
  reg  [DEPTHBITS:0] depth_ff;
  reg  [(0-1)+WIDTH:0] headreg_ff;
  reg  full_ff,notfull_ff,valid_ff,valid_filt_ff;
  wire [DEPTHBITS+(0-1):0] 
       in_ptr = in_push ? (in_ptr_ff[DEPTHBITS+(0-1):0]+'h1) : in_ptr_ff[DEPTHBITS+(0-1):0];
  wire [DEPTHBITS:0] 
       depth = (~in_pop && in_push) ? ('h1+depth_ff[DEPTHBITS:0]) : 
(~in_push && in_pop) ? ((-'h1)+depth_ff[DEPTHBITS:0]) : depth_ff[DEPTHBITS:0];
  ...
  wire [(0-1)+WIDTH:0]  raw_data = data_ff[out_ptr_ff[DEPTHBITS+(0-1):0]];

  wire [DEPTHBITS+(0-1):0] 
       out_ptr = in_pop ? ('h1+out_ptr_ff[DEPTHBITS+(0-1):0]) : out_ptr_ff[DEPTHBITS+(0-1):0];

  wire [(0-1)+WIDTH:0] 
       head_raw_data = depth_was1 ? in_data[(0-1)+WIDTH:0] : raw_data[(0-1)+WIDTH:0];

  wire [(0-1)+WIDTH:0] 
       headreg = (!valid_ff && in_push) ? in_data[(0-1)+WIDTH:0] : 
in_pop ? head_raw_data[(0-1)+WIDTH:0] : headreg_ff[(0-1)+WIDTH:0];
  ...
  always @(posedge Clk)
      begin
        in_ptr_ff[DEPTHBITS+(0-1):0] <= rst_l ? in_ptr[DEPTHBITS+(0-1):0] : {DEPTHBITS{1'b0}};
        out_ptr_ff[DEPTHBITS+(0-1):0] <= rst_l ? out_ptr[DEPTHBITS+(0-1):0] : 
                                        HEADREG ? {{DEPTHBITS+(0-1){1'b0}},{1'b1}} : {DEPTHBITS{1'b0}};
  ...
  always @(posedge Clk)
      begin
        if (in_push) 
          begin
            data_ff[in_ptr_ff[DEPTHBITS+(0-1):0]] <= in_data[(0-1)+WIDTH:0];
          end
