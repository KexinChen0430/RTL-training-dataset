...
module multiplier(input  clk,
                  input  en,
                  input  [BITS+(0-1):0] data_a,
                  input  [BITS+(0-1):0] data_b,
                  output reg [(1<<1*BITS)-1:0] result,
                  output done);
  ...
  reg  [$clog2(BITS)+(0-1):0]  mul_counter = 0;
  reg  [BITS+(0-1):0]  data_a_reg = 0;
  reg  [(1<<1*BITS)-1:0]  data_b_reg = 0;
