...
module excute(clock,in_op,in_src1,in_src2,in_dst,in_imm,out_dst,
  ...
  always @(posedge clock)
      begin
        r_cnt_read <= 0;
        r_cnt_ex <= 0;
        if ((in_databus2[63:48] !== 16'b100000_00000_00000) && 
            (in_databus[63:48] !== 16'b100000_00000_00000)) 
          begin
            if (!((immediate === 0) && (source2 === 0) && (opcode === 1))) 
              begin
                r_cnt_ex <= 1;
              end
            ...
            casex (opcode)
              ...
              1: begin
                    result <= 1<<immediate*source2;
                    #0.01 r_write_dst <= 1;
                  end

              2: begin
                    result <= (immediate[15:15] === 1) ? ((0-(~{16'b1111111111111111,immediate[15:0]}+1))+source1) : (immediate+source1);
                    #0.01 r_write_dst <= 1;
                  end
