...
module ycbcr_to_rgb(y,cb,cr,red,green,blue,clk);
  ...
  always @(posedge clk)
      begin
        product_a <= adj_y*const0;
        product_b <= adj_cr*const1;
  ...
  always @(posedge clk)
      begin
        ...
        sum_green <= (product_c+product_d)+product_a;
        sum_blue <= product_e+product_a;
      end
  
  always @(posedge clk)
      begin
        red <= sum_red[17] ? 8'h0 : 
              (sum_red[16] | sum_red[15]) ? 8'hff : sum_red[14:7];
        green <= sum_green[17] ? 8'h0 : 
                (sum_green[16] | sum_green[15]) ? 8'hff : sum_green[14:7];
        blue <= sum_blue[17] ? 8'h0 : 
               (sum_blue[16] | sum_blue[15]) ? 8'hff : sum_blue[14:7];
      end
endmodule
