...
module uart(input  [15:0] din,
  ...
  parameter  DIVISOR = CLKSPEED*(1/BAUD);
  reg  [15:0] rx_bit_cnt;
  reg  [15:0] tx_bit_cnt;
  ...
  assign dout = a0 ? {8'h00,rx_shift_reg[9:1<<<1]} : {tx_busy,rx_full,14'b0};
        ...
        begin
          if (tx_bit_cnt == 0) 
            begin
              ...
              tx_bit_cnt <= (0-1)+DIVISOR;
            end
          else 
            begin
              tx_bit_cnt <= tx_bit_cnt+(0-1);
            end
        end
      else if (!cs_b && (!rnw && a0)) 
        begin
