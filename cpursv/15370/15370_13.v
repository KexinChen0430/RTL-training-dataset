...
module except(clk,opa,opb,inf,ind,qnan,snan,opa_nan,opb_nan,opa_00,opb_00,
  ...
  always @(posedge clk)
      ind <= (expb_ff & infa_f_r) & (expa_ff & infb_f_r);
  
  always @(posedge clk)
      inf <= (infa_f_r | infb_f_r) & (((((expb_ff & expa_ff) | ((expa_ff & infb_f_r) & infa_f_r)) | ((expa_ff & infa_f_r) & (infa_f_r & (expa_ff | infb_f_r)))) & expa_ff) | ((expb_ff & expa_ff) | ((expb_ff & infb_f_r) | (expb_ff & (expa_ff & (infa_f_r | infb_f_r))))));
  
  always @(posedge clk)
      qnan <= (((expa_ff & (expb_ff & qnan_r_b)) | (expa_ff & (qnan_r_b & qnan_r_a))) | (expb_ff & qnan_r_b)) | (expa_ff & qnan_r_a);
  
  always @(posedge clk)
      snan <= (((snan_r_b | snan_r_a) & expa_ff) & (((((snan_r_b | snan_r_a) & expa_ff) | snan_r_b) & snan_r_a) & expa_ff)) | ((expb_ff & (((expa_ff & snan_r_b) | (snan_r_a & (snan_r_b & expb_ff))) | (snan_r_b & expb_ff))) | (snan_r_a & (((expa_ff & snan_r_b) | (snan_r_a & (snan_r_b & expb_ff))) | (snan_r_b & expb_ff))));
