...
module tmu2_fdest  #(parameter  fml_depth = 26)
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_ack_o & pipe_stb_i) 
              begin
                hit <= valid & (tag == dadr[((0-1)+fml_depth)-1:4]);
                dadr_r <= dadr;
              end
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if ((wanted & ((wanted | hit) & fetch_en)) & ~hit) 
                  begin
                    pipe_ack_o = 1'b0;
                    next_state = FETCH1;
                  end
  ...
  assign pipe_stb_o = ((hit | ~fetch_en) & wanted) | (stb_after_fetch | ((wanted | stb_after_fetch) & ~fetch_en));
  assign busy = wanted;
endmodule
