...
module tmu2_fdest  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   input  [15:0] color,
   input  [(1<<<1*(-1))+fml_depth:0] dadr,
   output pipe_stb_o,
   input  pipe_ack_i,
   output reg [15:0] color_f,
   output [(1<<<1*(-1))+fml_depth:0] dadr_f,
   output reg [15:0] dcolor);

  reg  valid;
  reg  [fml_depth+((-1)+((-4)+(-1))):0] tag;
  reg  [(1<<<1*(-1))+fml_depth:0] dadr_r;
  reg  hit;
  reg  tag_we;
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_ack_o & pipe_stb_i) 
              begin
                hit <= (tag == dadr[(1<<<1*(-1))+fml_depth:4]) & valid;
                dadr_r <= dadr;
              end
  ...
  always @(posedge sys_clk)
      begin
        ...
        else if (tag_we) tag <= dadr_r[(1<<<1*(-1))+fml_depth:4];
  ...
  assign fml_adr = {dadr_r[(1<<<1*(-1))+fml_depth:4],5'd0};
  reg  wanted;
  ...
  assign pipe_stb_o = (((hit & (wanted | ~fetch_en)) & (~fetch_en | hit)) & wanted) | (((~fetch_en | stb_after_fetch) & (wanted | (hit | stb_after_fetch))) & ((wanted | stb_after_fetch) | wanted));
  assign busy = wanted;
endmodule
