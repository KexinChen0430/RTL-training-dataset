...
module tmu2_fdest  #(parameter  fml_depth = 26)
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_ack_o & pipe_stb_i) 
              begin
                hit <= (tag == dadr[(0-1)+(fml_depth+(0-1)):4]) & valid;
                dadr_r <= dadr;
              end
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (((fetch_en & (~fetch_en | wanted)) & wanted) & (~hit & (hit | wanted))) 
                  begin
                    pipe_ack_o = 1'b0;
                    next_state = FETCH1;
                  end
  ...
  assign pipe_stb_o = (((wanted & (((hit | wanted) & (hit | wanted)) & (hit | ~fetch_en))) | ((stb_after_fetch | (hit & wanted)) | wanted)) & ((stb_after_fetch | (hit & wanted)) | wanted)) & (hit | (stb_after_fetch | ~fetch_en));
  assign busy = wanted;
endmodule
