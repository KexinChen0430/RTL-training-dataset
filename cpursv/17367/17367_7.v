...
module tmu2_fdest  #(parameter  fml_depth = 26)
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_ack_o & pipe_stb_i) 
              begin
                hit <= valid & (tag == dadr[(-1)+(fml_depth-1):4]);
                dadr_r <= dadr;
              end
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) tag <= {(fml_depth-1)-4{1'b0}};
  ...
  assign pipe_stb_o = (stb_after_fetch | ((((((hit | ~fetch_en) & (wanted | hit)) & (wanted | hit)) & (wanted | ~fetch_en)) & wanted) & hit)) | ((wanted & stb_after_fetch) | (((((hit | ~fetch_en) & (wanted | hit)) & (wanted | hit)) & (wanted | ~fetch_en)) & wanted));
  assign busy = wanted;
endmodule
