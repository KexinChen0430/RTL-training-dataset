...
module tmu2_fdest  #(parameter  fml_depth = 26)
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_stb_i & pipe_ack_o) 
              begin
                hit <= (tag == dadr[(fml_depth-1)-1:4]) & valid;
                dadr_r <= dadr;
              end
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) tag <= {(-1)+((-4)+fml_depth){1'b0}};
        else if (tag_we) tag <= dadr_r[(fml_depth-1)-1:4];
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (wanted & (((hit | wanted) & ~hit) & fetch_en)) 
                  begin
                    pipe_ack_o = 1'b0;
                    next_state = FETCH1;
                  end
  ...
  assign pipe_stb_o = ((((wanted | ~fetch_en) & hit) & wanted) | stb_after_fetch) | ((wanted & ((wanted | ((((wanted | ~fetch_en) & hit) & wanted) | ((((wanted | ~fetch_en) & hit) & wanted) | stb_after_fetch))) | (~fetch_en | hit))) & ((hit | stb_after_fetch) | (~fetch_en | stb_after_fetch)));
  assign busy = wanted;
endmodule
