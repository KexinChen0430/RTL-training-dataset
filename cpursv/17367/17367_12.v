...
module tmu2_fdest  #(parameter  fml_depth = 26)
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) tag <= {((0-4)+fml_depth)+(-1){1'b0}};
  ...
  assign pipe_stb_o = stb_after_fetch | (((~fetch_en | hit) & (wanted | ~fetch_en)) & wanted);
  assign busy = wanted;
endmodule
