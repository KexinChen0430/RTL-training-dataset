...
module tmu2_fdest  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   output [(0-1)+(fml_depth-1):0] dadr_f,
   output reg [15:0] dcolor);

  reg  valid;
  reg  [((0-1)+(fml_depth-1))+(0-4):0] tag;
  reg  [(0-1)+(fml_depth-1):0] dadr_r;
  reg  hit;
  reg  tag_we;
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_ack_o & pipe_stb_i) 
              begin
                hit <= (tag == dadr[(0-1)+(fml_depth-1):4]) & valid;
                dadr_r <= dadr;
              end
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) tag <= {(0-1)+((0-4)+fml_depth){1'b0}};
        else if (tag_we) tag <= dadr_r[(0-1)+(fml_depth-1):4];
  ...
  assign fml_adr = {dadr_r[(0-1)+(fml_depth-1):4],5'd0};
  reg  wanted;
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (fetch_en & (((wanted | hit) & ~hit) & wanted)) 
                  begin
                    pipe_ack_o = 1'b0;
                    next_state = FETCH1;
                  end
  ...
  assign pipe_stb_o = ((~fetch_en | (hit | stb_after_fetch)) & ((stb_after_fetch | wanted) | ~fetch_en)) & (stb_after_fetch | wanted);
  assign busy = wanted;
endmodule
