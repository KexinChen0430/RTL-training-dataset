...
module tmu2_fdest  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   output [(fml_depth-1)+(0-1):0] dadr_f,
   output reg [15:0] dcolor);

  reg  valid;
  reg  [((0-4)+(0-1))+(fml_depth-1):0] tag;
  reg  [(fml_depth-1)+(0-1):0] dadr_r;
  reg  hit;
  reg  tag_we;
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_stb_i & pipe_ack_o) 
              begin
                hit <= (tag == dadr[(fml_depth-1)+(0-1):4]) & valid;
                dadr_r <= dadr;
              end
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) tag <= {fml_depth+((0-4)+(0-1)){1'b0}};
        else if (tag_we) tag <= dadr_r[(fml_depth-1)+(0-1):4];
  ...
  assign fml_adr = {dadr_r[(fml_depth-1)+(0-1):4],5'd0};
  reg  wanted;
  ...
  assign pipe_stb_o = ((stb_after_fetch | wanted) & (stb_after_fetch | ~fetch_en)) | (wanted & (hit & ((~fetch_en | wanted) & (~fetch_en | hit))));
  assign busy = wanted;
endmodule
