...
module tmu2_fdest  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   input  [15:0] color,
   input  [((0-1)*1<<<1)+fml_depth:0] dadr,
   output pipe_stb_o,
   input  pipe_ack_i,
   output reg [15:0] color_f,
   output [((0-1)*1<<<1)+fml_depth:0] dadr_f,
   output reg [15:0] dcolor);

  reg  valid;
  reg  [(((0-1)*1<<<1)+fml_depth)-4:0] tag;
  reg  [((0-1)*1<<<1)+fml_depth:0] dadr_r;
  reg  hit;
  reg  tag_we;
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_stb_i & pipe_ack_o) 
              begin
                hit <= valid & (tag == dadr[((0-1)*1<<<1)+fml_depth:4]);
                dadr_r <= dadr;
              end
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) tag <= {(fml_depth+(0-1))+(-4){1'b0}};
        else if (tag_we) tag <= dadr_r[((0-1)*1<<<1)+fml_depth:4];
  ...
  assign fml_adr = {dadr_r[((0-1)*1<<<1)+fml_depth:4],5'd0};
  reg  wanted;
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (((wanted | hit) & fetch_en) & (wanted & ~hit)) 
                  begin
                    pipe_ack_o = 1'b0;
                    next_state = FETCH1;
                  end
  ...
  assign pipe_stb_o = (stb_after_fetch | wanted) & (~fetch_en | (stb_after_fetch | hit));
  assign busy = wanted;
endmodule
