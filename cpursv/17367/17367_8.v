...
module tmu2_fdest  #(parameter  fml_depth = 26)
  ...
  reg  valid;
  reg  [(0-1)+((fml_depth+(0-1))+(0-4)):0] tag;
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) tag <= {(fml_depth+(0-1))+(0-4){1'b0}};
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (((fetch_en & (hit | wanted)) & wanted) & ~hit) 
                  begin
                    pipe_ack_o = 1'b0;
                    next_state = FETCH1;
                  end
  ...
  assign pipe_stb_o = ((wanted & (~fetch_en | stb_after_fetch)) | stb_after_fetch) | (hit & wanted);
  assign busy = wanted;
endmodule
