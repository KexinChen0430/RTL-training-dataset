...
module tmu2_fdest  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   output [((-1)+(-1))+fml_depth:0] dadr_f,
   output reg [15:0] dcolor);

  reg  valid;
  reg  [(((-1)+(-1))+fml_depth)-4:0] tag;
  reg  [((-1)+(-1))+fml_depth:0] dadr_r;
  reg  hit;
  reg  tag_we;
        ...
        if (sys_rst) 
          ...
          begin
            ...
            if (pipe_ack_o & pipe_stb_i) 
              begin
                hit <= (tag == dadr[((-1)+(-1))+fml_depth:4]) & valid;
                dadr_r <= dadr;
              end
  ...
  always @(posedge sys_clk)
      begin
        ...
        else if (tag_we) tag <= dadr_r[((-1)+(-1))+fml_depth:4];
  ...
  assign fml_adr = {dadr_r[((-1)+(-1))+fml_depth:4],5'd0};
  reg  wanted;
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (~hit & (((fetch_en & (wanted | hit)) & (~fetch_en | wanted)) & wanted)) 
                  begin
                    pipe_ack_o = 1'b0;
                    next_state = FETCH1;
                  end
  ...
  assign pipe_stb_o = ((stb_after_fetch | (stb_after_fetch | ((stb_after_fetch | ~fetch_en) & wanted))) | wanted) & ((stb_after_fetch | (~fetch_en | wanted)) & ((hit | stb_after_fetch) | ~fetch_en));
  assign busy = wanted;
endmodule
