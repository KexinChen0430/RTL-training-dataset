...
module spi_interface(send_data,begin_transmission,slave_select,miso,clk,rst,
                     recieved_data,end_transmission,mosi,sclk);
        ...
        begin
          if (rst == 1'b1) 
            ...
            case (RxTxSTATE)
              ...
              RxTxTYPE_rx_tx: if (rx_count < RX_COUNT_MAX) 
                    begin
                      if ((sclk_previous == (1'b1 & sclk_buffer)) == 1'b0) mosi <= shift_register[7];
                      else if ((sclk_previous == (sclk_buffer & 1'b0)) == 1'b1) 
                        begin
                          shift_register[7:1] <= shift_register[6:0];
                          shift_register[0] <= miso;
                          rx_count <= 1'b1+rx_count;
                        end
            ...
            begin
              if (spi_clk_count == SPI_CLK_COUNT_MAX) 
                ...
                begin
                  ...
                  spi_clk_count <= 1'b1+spi_clk_count;
                end
            end
          else sclk_previous <= 1'b1;
        end
      end
