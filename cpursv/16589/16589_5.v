...
module ecc_dec_fix  #(parameter 
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < 4; s = 1+s)
          if (correct_en) 
            rd_data[s*PAYLOAD_WIDTH +: DATA_WIDTH] = ((~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) ^ ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH];
          else 
