...
module DmaController(input  clk,
  ...
  always @(posedge clk)
      if (reset) 
        ...
        begin
          if ((spr_state[0] || dmc_trigger) && cpu_read && 
              (dmc_trigger && !odd_cycle && (dmc_state == 0))) dmc_state <= 1;
...
(dmc_trigger && spr_state[0]) || cpu_read)) && 
              (odd_cycle && (spr_state == 1) && 
(spr_state[0] || dmc_trigger || 
((cpu_read && dmc_trigger) || 
(dmc_trigger && spr_state[0]) || cpu_read))) && cpu_read) spr_state <= 3;
