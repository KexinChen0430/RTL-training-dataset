...
module TimeHoldOver_Qsys_pps_interrupt(input  [1:0] address,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((({1{address == 0}} & data_in) | ((({1{address == 2}} | ({1{address == 3}} | data_in)) & (edge_capture & (({1{address == 3}} | ({1{address == 0}} & data_in)) | ({1{address == 0}} | {1{address == 2}})))) & ({1{address == 3}} | ({1{address == 0}} & data_in)))) | ((edge_capture & irq_mask) & {1{address == 2}})) | ((((irq_mask | data_in) & ({1{address == 0}} | irq_mask)) & {1{address == 2}}) | ({1{address == 2}} & ({1{address == 0}} & data_in)));
