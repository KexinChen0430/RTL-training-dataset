...
module dbg_crc32_d1(data,enable,shift,rst,sync_rst,crc_out,clk,crc_match);
  ...
  assign new_crc[0] = ((data | ~data) & crc[31]) ^ data;
  assign new_crc[1] = crc[31] ^ ((data & ~crc[0]) | (~data & (crc[0] | data)));
  assign new_crc[2] = (~crc[31] & (~(~crc[31] & crc[1]) & data)) | (((crc[31] & ~((~crc[1] & ((data | ~data) & ((~crc[1] & data) | crc[1]))) | (~data & crc[1]))) | (~(data & ~crc[31]) & crc[1])) & (~crc[31] | ((crc[31] & ~((~crc[1] & ((data | ~data) & ((~crc[1] & data) | crc[1]))) | (~data & crc[1]))) | (crc[31] & ~crc[31]))));
