...
module dbg_crc32_d1(data,enable,shift,rst,sync_rst,crc_out,clk,crc_match);
  ...
  assign new_crc[0] = data ^ (crc[31] & (data | ~data));
  assign new_crc[1] = (((data | ~data) | ~((~data & (crc[0] & ((crc[0] | data) & (data | ~data)))) | ((~data | (~crc[0] & data)) & data))) & crc[31]) ^ ((~data & (crc[0] & ((crc[0] | data) & (data | ~data)))) | ((~data | (~crc[0] & data)) & data));
  assign new_crc[2] = ((data & (~crc[1] | ~data)) | (crc[1] & ~data)) ^ (((~((data & (~crc[1] | ~data)) | (crc[1] & ~data)) | (~crc[1] & data)) | crc[1]) & crc[31]);
