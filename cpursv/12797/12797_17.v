...
module rcn_master_slave(input  rst,
                        input  clk,
                        input  [68:0] rcn_in,
                        output [68:0] rcn_out,
                        input  cs,
  ...
  wire 
       my_resp = !rin_d1[67] && 
(rin_d1[68] && (rin_d1[65:60] == my_id));

  wire 
       my_req = rin[68] && rin[67] && 
((my_mask[23:2] & rin[55:34]) == my_base[23:2]);
  ...
  always @(posedge clk or posedge rst)
      if (rst) 
        ...
        begin
          ...
          rout <= my_req_d1 ? resp : 
                 req_valid ? req : 
                 my_resp ? 69'd0 : rin_d1;
        end
  assign busy = !my_resp && rin_d1[68];
  assign req_valid = cs && !(!my_resp && rin_d1[68]);
  assign req = {1'b1,1'b1,wr,my_id,mask,addr[23:2],seq,wdata};
  assign rdone = !rin_d1[66] && my_resp;
  assign wdone = rin_d1[66] && my_resp;
