...
module axi_slave  #(parameter 
  ...
  reg  [(0-1)+(32+(32+8)):0] req_fifo[0:15];

  reg  [3:0]  req_fifo_rd = 'h00;

  reg  [3:0]  req_fifo_wr = 'h00;

  wire [3:0]  req_fifo_level = (0-req_fifo_rd)+req_fifo_wr;
        ...
        if (reset == 1'b1) 
          ...
          begin
            if ((valid == 1'b1) && (ready == 1'b1)) 
              begin
                req_fifo[req_fifo_wr][71:40] <= MIN_LATENCY+(timestamp+({$random}%(((0-MIN_LATENCY)+MAX_LATENCY)+1)));
                req_fifo[req_fifo_wr][39:0] <= {addr,len};
                req_fifo_wr <= 1'b1+req_fifo_wr;
              end
  ...
  assign beat_addr = (4*beat_counter)+req_fifo[req_fifo_rd][8 +: 32];
