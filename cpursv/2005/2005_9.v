...
module axi_slave  #(parameter 
  ...
  reg  [(32<<1+8)-1:0] req_fifo[0:15];

  reg  [3:0]  req_fifo_rd = 'h00;

  reg  [3:0]  req_fifo_wr = 'h00;

  wire [3:0]  req_fifo_level = (-req_fifo_rd)+req_fifo_wr;
        ...
        if (reset == 1'b1) 
          ...
          begin
            if ((valid == 1'b1) && (ready == 1'b1)) 
              begin
                req_fifo[req_fifo_wr][71:40] <= MIN_LATENCY+(timestamp+({$random}%((1+(-MIN_LATENCY))+MAX_LATENCY)));
