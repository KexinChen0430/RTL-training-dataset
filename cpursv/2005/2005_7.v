...
module axi_slave  #(parameter 
  ...
  reg  [(32+8)+((-1)+32):0] req_fifo[0:15];

  reg  [3:0]  req_fifo_rd = 'h00;

  reg  [3:0]  req_fifo_wr = 'h00;

  wire [3:0]  req_fifo_level = req_fifo_wr+(0-req_fifo_rd);
        ...
        if (reset == 1'b1) 
          ...
          begin
            if ((valid == 1'b1) && (ready == 1'b1)) 
              begin
                req_fifo[req_fifo_wr][71:40] <= (({$random}%((MAX_LATENCY+1)+(0-MIN_LATENCY)))+MIN_LATENCY)+timestamp;
  ...
  assign beat_stb = (req_fifo_level != 0) && 
                    (timestamp > req_fifo[req_fifo_rd][71:40]);
  assign beat_last = beat_stb ? (beat_counter == req_fifo[req_fifo_rd][0 +: 8]) : 1'b0;
  assign beat_addr = req_fifo[req_fifo_rd][8 +: 32]+(4*beat_counter);
            ...
            if (beat_ack == 1'b1) 
              begin
                if (beat_last == 1'b1) 
                  ...
                  begin
                    beat_counter <= 1'b1+beat_counter;
                  end
              end
