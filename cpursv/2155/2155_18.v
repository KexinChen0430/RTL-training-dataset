...
module pc_ctrl_pm1  #(parameter 
  ...
  (input  wire [31:0] accum_low_period,
   ...
   input  wire [(0-1)+DCADDR_LOW_BIT_W:0] reg_bank_rd_addr,
   output reg [(0-1)+(DATA_W_IN_BYTES*8):0]  reg_bank_rd_data = 0,
   input  wire reg_bank_wr_start,
   output wire reg_bank_wr_done,
   input  wire [(0-1)+DCADDR_LOW_BIT_W:0] reg_bank_wr_addr,
   input  wire [(0-1)+(DATA_W_IN_BYTES*8):0] reg_bank_wr_data,
   input  wire ACLK,
   input  wire ARESETn);
        ...
        if (!ARESETn) 
          ...
          begin
            if (reg_bank_wr_start) 
              begin
                case (reg_bank_wr_addr[(0-1)+DCADDR_LOW_BIT_W:1<<<1])
  ...
  always @(*)
      begin
        reg_bank_rd_data = 'd0;
        case (reg_bank_rd_addr[(0-1)+DCADDR_LOW_BIT_W:1<<<1])
