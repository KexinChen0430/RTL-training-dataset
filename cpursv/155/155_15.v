
module mac_scale(input  wire signed  [39:0] to_scaling,
                 input  wire [2:0] c_scalefactor,
                 output wire scale_overflow,
                 output reg [39:0] from_scaling);

  reg  scale_pos_overflow;
  reg  scale_neg_overflow;

  assign scale_overflow = scale_neg_overflow | scale_pos_overflow;
  
  always @(*)
      begin
        scale_pos_overflow = 0;
        scale_neg_overflow = 0;
        case (c_scalefactor)

          3'b000: begin
                from_scaling = to_scaling;
              end

          3'b001: begin
                from_scaling = 2*to_scaling;
                scale_pos_overflow = ~to_scaling[39] & to_scaling[38];
                scale_neg_overflow = ~to_scaling[38] & to_scaling[39];
              end

          3'b010: begin
                from_scaling = to_scaling<<<2;
                scale_pos_overflow = |to_scaling[38:37] & ~to_scaling[39];
                scale_neg_overflow = ~&to_scaling[38:37] & to_scaling[39];
              end

          3'b011: begin
                from_scaling = {to_scaling[39],to_scaling[39:1]};
              end

          3'b100: begin
                from_scaling = {{2{to_scaling[39]}},to_scaling[39:2]};
              end

          3'b101: begin
                from_scaling = {{3{to_scaling[39]}},to_scaling[39:3]};
              end

          3'b110: begin
                from_scaling = {{4{to_scaling[39]}},to_scaling[39:4]};
              end

          3'b111: begin
                from_scaling = to_scaling*(2**16);
                scale_pos_overflow = |to_scaling[38:23] & ~to_scaling[39];
                scale_neg_overflow = ~&to_scaling[38:23] & to_scaling[39];
              end

        endcase

      end
endmodule

