...
(
(setup || 
((setup && !ep_setup) || (!ep_out_stall && ep_out_full))) && !ep_setup && !ep_out_stall) || (!ep_out_stall || setup || !ep_setup)) && (!ep_out_stall || setup))) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          ...
          begin
            if (((!ep_out_stall && setup) || !ep_setup || setup) && 
                (((!ep_out_stall || setup) && ep_out_full) || 
(
((ep_out_full && !ep_setup) || 
(ep_out_full || (!ep_out_stall && setup) || setup)) && ((!ep_out_stall && setup) || !ep_setup || setup) && 
(ep_out_full || (!ep_out_stall && setup) || setup))) && 
                (setup || 
((setup && !ep_setup) || (!ep_out_stall && ep_out_full)))) 
              begin
                handshake = hs_ack;
              end
  ...
  wire 
       flush = ctrl_wr_data[5] || ctrl_wr_data[4] || ctrl_wr_data[3];
