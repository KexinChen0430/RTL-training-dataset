...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (setup && 
            ((setup || (!ep_setup && ep_out_full)) && !direction_in)) toggle = 1'b0;
...
(setup || 
(
(setup && (!ep_out_stall || !ep_setup) && !ep_out_stall) || ep_out_full)) && 
                (
((!ep_setup || setup) && (ep_out_full || !ep_setup) && 
ep_out_full) || 
(
((!ep_setup && !ep_out_stall) || 
(ep_out_full && !ep_out_stall) || setup) && 
((!ep_setup && ep_out_full) || 
(!ep_setup && !ep_out_stall) || setup))) && (!ep_out_stall || setup)) 
              begin
                handshake = hs_ack;
              end
