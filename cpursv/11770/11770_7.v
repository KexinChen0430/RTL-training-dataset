...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if ((ep_out_full || setup) && !direction_in && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (!ep_setup && (!ep_in_empty && !ep_in_stall)) 
              begin
                handshake = hs_ack;
              end
          ...
          begin
            if ((!ep_setup || setup || !ep_out_stall) && 
                (ep_out_full || setup) && 
                (setup || 
(!ep_out_stall || 
(setup && (!ep_setup || !ep_out_stall) && !ep_setup))) && ((ep_out_full || setup) && (!ep_setup || setup))) 
              begin
                handshake = hs_ack;
              end
            else if (ep_out_stall && 
                     (!ep_setup || setup || 
((!ep_setup || setup || !ep_out_stall) && 
(ep_out_full || setup) && 
(setup || 
(!ep_out_stall || 
(setup && (!ep_setup || !ep_out_stall) && !ep_setup))))) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
