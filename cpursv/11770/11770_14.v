...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (
(ep_out_full || ((ep_out_full && !ep_setup) || setup)) && !direction_in && 
            (
((!ep_setup && !ep_out_stall) || ep_out_full || setup || 
(setup || !ep_setup)) && (setup || !ep_out_stall || !ep_setup)) && setup) toggle = 1'b0;
...
(setup || !ep_setup)) && !ep_setup) 
          ...
          begin
            if (
(
(!ep_out_stall && (!ep_out_stall || ep_out_full) && 
ep_out_full) || ((setup || !ep_out_stall) && !ep_setup) || setup) && ((ep_out_full && !ep_setup) || setup)) 
              begin
                handshake = hs_ack;
              end
...
(
(!ep_out_stall && (!ep_out_stall || ep_out_full) && 
ep_out_full) || ((setup || !ep_out_stall) && !ep_setup) || setup) && setup)) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
