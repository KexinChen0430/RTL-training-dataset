...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if ((setup || ep_out_full) && !direction_in && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (!ep_in_empty && 
                (!ep_in_stall && (setup || !ep_setup) && !ep_setup)) 
              begin
                handshake = hs_ack;
              end
...
((setup || ep_out_full) && 
(!ep_out_stall && 
(setup || ep_out_full || (setup || !ep_setup)))) || (setup || !ep_setup)) && (!ep_out_stall || setup) && 
                (
(
((setup || ep_out_full) && 
(!ep_out_stall && 
(setup || ep_out_full || (setup || !ep_setup)))) || (setup || !ep_setup)) && (setup || ep_out_full)) && 
                ((setup || (ep_out_full || !ep_out_stall)) && 
(
(!ep_out_stall && 
(setup || ep_out_full || (setup || !ep_setup)) && !ep_setup) || (setup || !ep_setup || !ep_out_stall)) && (setup || !ep_setup))) 
              begin
                handshake = hs_ack;
              end
  ...
  wire 
       flush = ctrl_wr_data[5] || 
(ctrl_wr_data[3] || ctrl_wr_data[4]);
