...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (!direction_in && 
            (setup || ep_out_full || 
((setup || !ep_setup || (!ep_out_stall || setup)) && 
((!ep_out_stall && !ep_setup) || ep_out_full) && ep_out_full && (setup || !ep_setup))) && 
            ((setup || ep_out_full) && 
(setup || !ep_setup || (!ep_out_stall || setup)) && (setup || !ep_setup)) && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (!ep_in_stall && !ep_in_empty && 
                ((ep_out_full || (setup || !ep_setup)) && 
(setup || !ep_setup || (!ep_out_stall || setup))) && !ep_setup) 
              begin
                handshake = hs_ack;
              end
            else if (!ep_setup && ep_in_stall) 
              begin
                handshake = hs_stall;
              end
            else 
...
(setup || ep_out_full || 
((setup || !ep_setup || (!ep_out_stall || setup)) && 
((!ep_out_stall && !ep_setup) || ep_out_full) && ep_out_full && 
...
(setup || !ep_setup || (!ep_out_stall || setup)))))) 
  ...
  wire 
       flush = ctrl_wr_data[5] || ctrl_wr_data[3] || ctrl_wr_data[4];
