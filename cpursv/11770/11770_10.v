...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (!direction_in && 
            ((!ep_setup || setup) && 
((setup || !ep_out_stall || (!ep_setup || setup)) && 
(!ep_out_stall || ep_out_full || setup))) && 
            (
(
((setup || (ep_out_full || !ep_setup)) && 
(ep_out_full || (!ep_setup && setup)) && (!ep_out_stall || ep_out_full)) || setup) && 
(
(((!ep_out_stall && ep_out_full) || setup) && 
ep_out_full) || (!ep_setup || setup)) && (!ep_setup || setup)) && setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (!ep_in_stall && (!ep_in_empty && !ep_setup)) 
              begin
                handshake = hs_ack;
              end
          ...
          begin
            if (((!ep_out_stall && ep_out_full) || setup) && 
                ((!ep_setup || setup) && 
((setup || !ep_out_stall || (!ep_setup || setup)) && 
(!ep_out_stall || ep_out_full || setup)))) 
              begin
  ...
  wire 
       flush = ctrl_wr_data[3] || ctrl_wr_data[5] || ctrl_wr_data[4];
