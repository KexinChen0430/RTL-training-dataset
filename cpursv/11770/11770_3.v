...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (setup && 
            ((setup || ep_out_full) && !direction_in && 
(setup || 
(((setup || ep_out_full) && !ep_setup && setup) || 
(!ep_out_stall && ep_out_full))))) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (!ep_in_stall && !ep_in_empty && (setup || !ep_setup) && 
                !ep_setup) 
              begin
                handshake = hs_ack;
              end
          ...
          begin
            if (setup || (!ep_out_stall && !ep_setup && ep_out_full)) 
              begin
                handshake = hs_ack;
              end
