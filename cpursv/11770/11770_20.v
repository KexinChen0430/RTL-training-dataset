  ...
  always @(*)
      begin
        if (setup && 
            (!direction_in && 
(
((setup || !ep_out_stall || !ep_setup) && ep_out_full && 
(!ep_setup || setup) && !ep_setup) || (setup || !ep_out_stall || !ep_setup)) && (!ep_setup || setup))) toggle = 1'b0;
        ...
        if (direction_in) 
          begin
            if (!ep_in_stall && !ep_in_empty && 
                (setup || (ep_out_full || !ep_setup)) && !ep_setup) 
              begin
                handshake = hs_ack;
              end
...
(
(
(setup && (!ep_out_stall || ep_out_full) && !ep_out_stall) || !ep_setup) && !ep_out_stall)) && (setup || ep_out_full)) 
              begin
                handshake = hs_ack;
              end
