...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (setup && 
            ((setup || (!ep_setup && ep_out_full) || !ep_setup) && 
!direction_in)) toggle = 1'b0;
        ...
        if (direction_in) 
          begin
            if (!ep_in_stall && !ep_in_empty && 
                (!ep_setup || !ep_out_stall) && !ep_setup) 
              begin
                handshake = hs_ack;
              end
            else if (ep_in_stall && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
...
(ep_out_full || (setup || (!ep_setup && ep_out_full)))) && (setup || (!ep_setup && ep_out_full) || !ep_setup)) 
              begin
                handshake = hs_ack;
              end
            else if (ep_out_stall && 
                     (setup || (!ep_setup && ep_out_full) || !ep_setup) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
  ...
  wire 
       flush = ctrl_wr_data[4] || 
(ctrl_wr_data[3] || ctrl_wr_data[5]);
