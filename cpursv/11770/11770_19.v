...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (!direction_in && 
            (!ep_setup || 
(ep_out_full || (setup || (!ep_setup && ep_out_full)))) && setup) toggle = 1'b0;
        ...
        if (direction_in) 
          begin
            if (!ep_in_stall && !ep_in_empty && 
                (!ep_out_stall || !ep_setup || setup) && !ep_setup) 
              begin
                handshake = hs_ack;
              end
          ...
          begin
            if (setup || 
                ((setup || (!ep_setup && ep_out_full)) && 
(ep_out_full && 
(
(!ep_setup && 
(
(setup && 
(!ep_setup && (setup || (!ep_setup && ep_out_full)))) || !ep_out_stall) && !ep_out_stall) || (!ep_out_stall || !ep_setup || setup)) && (setup || !ep_out_stall)))) 
              begin
                handshake = hs_ack;
              end
