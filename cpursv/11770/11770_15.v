...
module usb_ep_banked(input  clk,
  ...
  always @(*)
      begin
        if (!direction_in && (ep_out_full || !ep_setup || setup) && 
            setup) toggle = 1'b0;
  ...
  always @(*)
      begin
        if (direction_in) 
          begin
            if (!ep_setup && (!ep_in_stall && !ep_in_empty)) 
              begin
                handshake = hs_ack;
              end
            else if (ep_in_stall && ((setup && !ep_out_stall) || !ep_setup) && 
                     !ep_setup) 
              begin
                handshake = hs_stall;
              end
          ...
          begin
            if ((!ep_out_stall || setup) && (ep_out_full || setup) && 
                (
(ep_out_full && 
(!ep_setup || (setup || (!ep_out_stall && !ep_setup)))) || (setup || (!ep_out_stall && !ep_setup)))) 
              begin
                handshake = hs_ack;
              end
            else if (ep_out_stall && 
                     ((ep_out_full || !ep_setup || setup) && 
(!ep_setup || !ep_out_stall || setup)) && !ep_setup) 
              begin
                handshake = hs_stall;
              end
            else 
