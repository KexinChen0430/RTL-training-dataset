...
module Altera_UP_PS2_Data_In(clk,reset,wait_for_incoming_data,start_receiving_data,
        ...
        case (s_ps2_receiver)
          ...
          PS2_STATE_0_IDLE: begin
                if ((wait_for_incoming_data == 1'b1) && 
                    (received_data_en == 1'b0)) ns_ps2_receiver = PS2_STATE_1_WAIT_FOR_DATA;
                else if ((start_receiving_data == 1'b1) && 
                         (received_data_en == 1'b0)) ns_ps2_receiver = PS2_STATE_2_DATA_IN;
                  
              end

          PS2_STATE_1_WAIT_FOR_DATA: begin
                if ((ps2_clk_posedge == 1'b1) && (ps2_data == 1'b0)) ns_ps2_receiver = PS2_STATE_2_DATA_IN;
                else if (wait_for_incoming_data == 1'b0) ns_ps2_receiver = PS2_STATE_0_IDLE;
                  
              end

          PS2_STATE_2_DATA_IN: begin
                if ((data_count == 3'h7) && (ps2_clk_posedge == 1'b1)) ns_ps2_receiver = PS2_STATE_3_PARITY_IN;
  ...
  always @(posedge clk)
      begin
        ...
        else if ((ps2_clk_posedge == 1'b1) && 
                 (s_ps2_receiver == PS2_STATE_2_DATA_IN)) data_count <= data_count+3'h1;
  ...
  always @(posedge clk)
      begin
        if (reset == 1'b1) data_shift_reg <= 8'h00;
                 ...
                 (s_ps2_receiver == PS2_STATE_2_DATA_IN)) data_shift_reg <= {ps2_data,data_shift_reg[7:1]};
