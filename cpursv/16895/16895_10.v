...
module dma_sequencer(clk,rst_n,addr,wd,rd,req,rnw,ack,done,dma_req,dma_addr,
  ...
  always @* 
      for (i = 1; i <= DEVNUM; i = i+1)
          begin
            pri_in[i] = (i == 1) ? pri_out[DEVNUM] : pri_out[(-1)+i];
            pri_out[i] = muxend[i] | (((pri_in[i] | muxend[i]) & (~req[i] | (muxend[i] & pri_in[i]))) & pri_in[i]);
  ...
  always @* 
      begin
        ...
        dma_req = 1'b0;
        for (i = 1; i <= DEVNUM; i = i+1)
            dma_req = req[i] | dma_req;
        dma_wd = 8'd0;
        for (i = 1; i <= DEVNUM; i = i+1)
            dma_wd = ((muxbeg[i] == 1'b1) ? wd[i] : 8'd0) | dma_wd;
        dma_addr = 21'd0;
        for (i = 1; i <= DEVNUM; i = i+1)
