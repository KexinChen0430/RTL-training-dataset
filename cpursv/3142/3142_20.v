  ...
  always @(posedge wbm_clk_i)
      begin
        if (wbm_rst_i) 
          ...
          begin
            cntr_r <= 7;
            if (((state_r == state_m_dat) | (state_r == state_s_dat)) | (state_r == state_m_adr)) 
              begin
                cntr_r <= cntr_r-1;
              end
  ...
  always @(posedge wbm_clk_i)
      begin
        ...
        case (state_r)
          ...
          state_m_dat | state_m_adr: gic_chksum_r <= ((gic_dat_i & ~(~chksum_operand & gic_chksum_r)) & ~chksum_operand) | ((chksum_operand & ~((gic_dat_i & ~gic_chksum_r) | (~gic_dat_i & gic_chksum_r))) | (~(~chksum_operand & gic_dat_i) & (~chksum_operand & gic_chksum_r)));

          state_s_resp: gic_chksum_r <= 0;

          state_s_dat: gic_chksum_r <= ((((~(((wbm_dat_r[4*cntr_r +: 4] & (gic_chksum_r & ~wbm_dat_r[4*cntr_r +: 4])) | (~gic_chksum_r & wbm_dat_r[4*cntr_r +: 4])) ^ ((~gic_chksum_r | gic_chksum_r) & (gic_chksum_r & (~gic_chksum_r | (~wbm_dat_r[4*cntr_r +: 4] & (~gic_chksum_r | gic_chksum_r)))))) | (~(wbm_dat_r[4*cntr_r +: 4] & ~chksum_operand) & ~chksum_operand)) & (gic_chksum_r | ~(((wbm_dat_r[4*cntr_r +: 4] & (gic_chksum_r & ~wbm_dat_r[4*cntr_r +: 4])) | (~gic_chksum_r & wbm_dat_r[4*cntr_r +: 4])) ^ ((~gic_chksum_r | gic_chksum_r) & (gic_chksum_r & (~gic_chksum_r | (~wbm_dat_r[4*cntr_r +: 4] & (~gic_chksum_r | gic_chksum_r)))))))) | (wbm_dat_r[4*cntr_r +: 4] & ~chksum_operand)) & (~(~chksum_operand & gic_chksum_r) | ((~(((wbm_dat_r[4*cntr_r +: 4] & (gic_chksum_r & ~wbm_dat_r[4*cntr_r +: 4])) | (~gic_chksum_r & wbm_dat_r[4*cntr_r +: 4])) ^ ((~gic_chksum_r | gic_chksum_r) & (gic_chksum_r & (~gic_chksum_r | (~wbm_dat_r[4*cntr_r +: 4] & (~gic_chksum_r | gic_chksum_r)))))) | (~(wbm_dat_r[4*cntr_r +: 4] & ~chksum_operand) & ~chksum_operand)) & (gic_chksum_r | ~(((wbm_dat_r[4*cntr_r +: 4] & (gic_chksum_r & ~wbm_dat_r[4*cntr_r +: 4])) | (~gic_chksum_r & wbm_dat_r[4*cntr_r +: 4])) ^ ((~gic_chksum_r | gic_chksum_r) & (gic_chksum_r & (~gic_chksum_r | (~wbm_dat_r[4*cntr_r +: 4] & (~gic_chksum_r | gic_chksum_r)))))))))) & ((((chksum_operand | gic_chksum_r) & (chksum_operand | (~(wbm_dat_r[4*cntr_r +: 4] & ~chksum_operand) & ~chksum_operand))) | ~chksum_operand) & ((wbm_dat_r[4*cntr_r +: 4] & ~(~chksum_operand & gic_chksum_r)) | ((chksum_operand | gic_chksum_r) & (chksum_operand | (~(wbm_dat_r[4*cntr_r +: 4] & ~chksum_operand) & ~chksum_operand)))));
