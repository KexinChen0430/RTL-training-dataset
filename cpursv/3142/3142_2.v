  ...
  always @(posedge wbm_clk_i)
      begin
        ...
        case (state_r)
          ...
          state_m_dat | state_m_adr: gic_chksum_r <= (gic_chksum_r | ((gic_dat_i & ~gic_chksum_r) | (chksum_operand & ~((~gic_dat_i | (gic_dat_i & ~gic_chksum_r)) & (gic_dat_i | gic_chksum_r))))) & ((chksum_operand & ~((~gic_dat_i | (gic_dat_i & ~gic_chksum_r)) & (gic_dat_i | gic_chksum_r))) | (~chksum_operand & (~gic_dat_i | (gic_dat_i & ~gic_chksum_r))));

          state_s_resp: gic_chksum_r <= 0;

          state_s_dat: gic_chksum_r <= ((~chksum_operand | chksum_operand) & ((wbm_dat_r[4*cntr_r +: 4] & ~gic_chksum_r) | (gic_chksum_r & (~gic_chksum_r | ((~gic_chksum_r | gic_chksum_r) & ~wbm_dat_r[4*cntr_r +: 4]))))) ^ chksum_operand;
