...
module acl_work_group_dispatcher  #(parameter  WIDTH = 32, NUM_COPIES = 1, RUN_FOREVER = 0)
  ...
  wire 
       last_group = (last_group_id[1] & last_group_id[0]) & last_group_id[1<<1];
      ...
      begin
        if (~resetn) 
          ...
          begin
            ...
            global_id_base_out[1] <= {WIDTH{1'b0}};
            global_id_base_out[1<<1] <= {WIDTH{1'b0}};
            max_group_id[0] <= (-2'b01)+num_groups[0];
          ...
          begin
            if ((started & stall_in) != (~dispatched_all_groups & {NUM_COPIES{1'b1}})) 
              begin
                if (bump_group_id[0]) 
