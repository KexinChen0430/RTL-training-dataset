...
module acl_work_group_dispatcher  #(parameter  WIDTH = 32, NUM_COPIES = 1, RUN_FOREVER = 0)
  (input  clock,
   input  resetn,
   input  start,
   input  [(0-1)+WIDTH:0] num_groups[1<<1:0],
   input  [(0-1)+WIDTH:0] local_size[1<<1:0],
   input  [(0-1)+NUM_COPIES:0] stall_in,
   output [(0-1)+NUM_COPIES:0] valid_out,
   output reg [(0-1)+WIDTH:0] group_id_out[1<<1:0],
   output reg [(0-1)+WIDTH:0] global_id_base_out[1<<1:0],
   output start_out,
   output reg dispatched_all_groups);

  reg  started;
  reg  delayed_start;
  reg  [(0-1)+WIDTH:0] max_group_id[1<<1:0];
  reg  [(0-1)+WIDTH:0] group_id[1<<1:0];
  ...
  wire 
       last_group = last_group_id[0] & (last_group_id[1] & last_group_id[1<<1]);
  ...
  assign bump_group_id[1<<1] = last_group_id[1] && last_group_id[0];
      ...
      begin
        if (~resetn) 
          ...
          begin
            ...
            max_group_id[1<<1] <= num_groups[1<<1]+(0-2'b01);
            started <= 1'b1;
          ...
          begin
            if ((stall_in & started) != (~dispatched_all_groups & {NUM_COPIES{1'b1}})) 
              begin
                if (bump_group_id[0]) 
                ...
                if (bump_group_id[1<<1]) 
                  global_id_base_out[1<<1] <= last_group_id[1<<1] ? {WIDTH{1'b0}} : (global_id_base_out[1<<1]+local_size[1<<1]);
