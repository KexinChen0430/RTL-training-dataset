...
module acl_work_group_dispatcher  #(parameter  WIDTH = 32, NUM_COPIES = 1, RUN_FOREVER = 0)
  (input  clock,
   input  resetn,
   input  start,
   input  [WIDTH+(0-1):0] num_groups[1<<1:0],
   input  [WIDTH+(0-1):0] local_size[1<<1:0],
   input  [(0-1)+NUM_COPIES:0] stall_in,
   output [(0-1)+NUM_COPIES:0] valid_out,
   output reg [WIDTH+(0-1):0] group_id_out[1<<1:0],
   output reg [WIDTH+(0-1):0] global_id_base_out[1<<1:0],
   output start_out,
   output reg dispatched_all_groups);

  reg  started;
  reg  delayed_start;
  reg  [WIDTH+(0-1):0] max_group_id[1<<1:0];
  reg  [WIDTH+(0-1):0] group_id[1<<1:0];
  ...
  wire 
       last_group = last_group_id[0] & (last_group_id[1<<1] & last_group_id[1]);
