...
module parameterized_RAM_last  #(parameter  SIZE = 10, LOOP = 1<<1)
  ...
  reg  [7:0] RAM[((SIZE*LOOP)*1<<1)-1'b1:0];
  reg  [$clog2((SIZE*LOOP)*1<<1):0]  rd_addr = 0;
  reg  [$clog2(((SIZE*LOOP)*1<<1)-1'b1):0]  wr_addr = 0;
  ...
  always @(posedge clk_i)
      begin
        ...
        else if (wr_addr == (((SIZE*LOOP)*1<<1)-1)) 
          begin
            sector_written[1] <= 1;
            wr_addr <= 0;
          end
        ...
        if (rd_i) 
          begin
            ...
            else if (rd_addr == (((SIZE*LOOP)*1<<1)-1)) 
              begin
                sector_written[1] <= 0;
                rd_addr <= 0;
              end
