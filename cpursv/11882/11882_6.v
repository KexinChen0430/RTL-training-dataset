...
module CSA_nBits(S,C,Ovfl,W,X,Y,Z,Cin);
  ...
  input  Cin;
  wire [width:0] 
       S1 = {1'b0,(~Y | ~((W | (X & ~W)) & (~W | (~X & W)))) & (((W | (X & ~W)) | (~((W | (X & ~W)) & (~W | (~X & W))) & Y)) & ((~((W | (X & ~W)) & (~W | (~X & W))) & Y) | (~W | (~X & W))))};
  wire [width:0] 
       C1 = {(W & (Y | X)) | (X & ((X & (W | Y)) | Y)),Cin};
  wire [width:0]  Z1 = {1'b0,Z};
  wire [width:0] 
       S2 = (~Z | ~(C1 ^ S1)) & (((S1 | C1) | (~(C1 ^ S1) & Z)) & ((~C1 | (C1 & ~S1)) | (~(C1 ^ S1) & Z)));
  wire [width+1:0]  C2 = {(S1 | (C1 & Z)) & ((S1 & C1) | Z),1'b0};
