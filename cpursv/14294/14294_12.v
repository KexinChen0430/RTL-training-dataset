...
(Nvel[1:0] == 1) ? ((1/V50kb)*((1/2)*Fclk)) : (Fclk/(V12_5kb*2));
  reg  [10:0]  cb_tce = 0;
  reg  [7:0]  sr_adr = 0;
  reg  [22:0]  sr_dat = 0;
        ...
        if ((cb_bit == 32) || reset) 
          ...
          begin
            ...
            QM <= start ? 0 : 
                 (ce_tact & en_tx) ? !QM : QM;
            cb_bit <= start ? 0 : 
                     ((en_tx_word & (ce | ((sr_adr[7] & ((T_cp | sr_adr[7]) & ce)) | start))) & ce) ? (1+cb_bit) : cb_bit;
            en_tx_word <= start ? 1 : 
                         ce_end_word ? 0 : en_tx_word;
            en_tx <= start ? 1 : 
                    (T_cp & ((T_cp | sr_adr[7]) & ce)) ? 0 : en_tx;
            FT_cp <= (start | (T_cp & ((ce | ((sr_adr[7] & ((T_cp | sr_adr[7]) & ce)) | start)) & (T_cp | sr_adr[7])))) ? 1 : 
                    ((((T_cp & FT_cp) | sr_adr[7]) & ((T_cp | sr_adr[7]) & ce)) & ((((FT_cp | (sr_adr[7] & T_cp)) | (T_cp | sr_adr[7])) & T_adr_dat) & sr_adr[7])) ? !FT_cp : FT_cp;
            sr_adr <= start ? ADR : 
                     (en_tx & ce) ? ((2**(sr_dat[0] | 1))*sr_adr) : sr_adr;
            sr_dat <= start ? DAT : 
