...
(Nvel[1:0] == 1) ? ((1/V50kb)*(Fclk/2)) : ((Fclk/2)*(1/V12_5kb));
  reg  [10:0]  cb_tce = 0;
  reg  [7:0]  sr_adr = 0;
  reg  [22:0]  sr_dat = 0;
  ...
  assign SDAT = (T_cp & (FT_cp | sr_adr[7])) | sr_adr[7];
        ...
        if (reset || (cb_bit == 32)) 
          ...
          begin
            ...
            FT_cp <= ((ce & T_cp) | start) ? 1 : 
                    (((FT_cp | sr_adr[7]) & T_adr_dat) & (sr_adr[7] & ce)) ? !FT_cp : FT_cp;
            sr_adr <= start ? ADR : 
                     ((en_tx & ((T_cp | ce) | (start | T_cp))) & ce) ? sr_adr<<<(sr_dat[0] | 1) : sr_adr;
            sr_dat <= start ? DAT : 
                     ((en_tx & ((T_cp | ce) | (start | T_cp))) & ce) ? sr_dat>>1 : sr_dat;
          end
      end
endmodule
