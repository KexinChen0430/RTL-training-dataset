...
(Nvel[1:0] == 1) ? (((1/V50kb)*(1/2))*Fclk) : (Fclk/(V12_5kb*2));
  ...
  assign SDAT = sr_adr[7] | ((T_cp & sr_adr[7]) | (FT_cp & T_cp));
  ...
  always @(posedge clk)
      begin
        if ((cb_bit == 32) || reset) 
          ...
          begin
            cb_tce <= ((((T_cp & start) | ((((ce & T_cp) | start) & ce) & start)) | (ce_tact | ((T_cp & start) | ((((ce & T_cp) | start) & ce) & start)))) | start) ? 1 : (1+cb_tce);
            QM <= start ? 0 : 
                 (ce_tact & en_tx) ? !QM : QM;
            cb_bit <= start ? 0 : 
            ...
            FT_cp <= ((ce & T_cp) | start) ? 1 : 
                    ((sr_adr[7] & T_adr_dat) & ce) ? !FT_cp : FT_cp;
            sr_adr <= start ? ADR : 
                     (ce & (en_tx & (ce | (T_cp | start)))) ? sr_adr<<<(1 | sr_dat[0]) : sr_adr;
            sr_dat <= start ? DAT : 
                     (ce & (en_tx & (ce | (T_cp | start)))) ? sr_dat>>1 : sr_dat;
          end
      end
endmodule
