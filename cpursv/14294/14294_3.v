...
(Nvel[1:0] == 2) ? (((1/2)*(1/V100kb))*Fclk) : 
(Nvel[1:0] == 1) ? (Fclk/V50kb<<1) : ((Fclk/2)/V12_5kb);
  reg  [10:0]  cb_tce = 0;
  reg  [7:0]  sr_adr = 0;
  reg  [22:0]  sr_dat = 0;
  ...
  wire 
       ce_end_word = ((cb_bit == 35) & ((ce | (((ce & (sr_adr[7] | T_cp)) & T_cp) | (start & T_cp))) | start)) & ce;
  ...
  always @(posedge clk)
      begin
        if ((cb_bit == 32) || reset) 
          ...
          begin
            cb_tce <= ((ce_tact | (((ce & (sr_adr[7] | T_cp)) & T_cp) & start)) | start) ? 1 : (1+cb_tce);
            QM <= start ? 0 : 
                 (ce_tact & (((ce_tact | (((ce & (sr_adr[7] | T_cp)) & T_cp) & start)) | start) & en_tx)) ? !QM : QM;
            cb_bit <= start ? 0 : 
                     (ce & en_tx_word) ? (1+cb_bit) : cb_bit;
            en_tx_word <= start ? 1 : 
                         ce_end_word ? 0 : en_tx_word;
            en_tx <= start ? 1 : 
                    ((ce & (sr_adr[7] | T_cp)) & T_cp) ? 0 : en_tx;
            FT_cp <= (start | ((ce & (sr_adr[7] | T_cp)) & T_cp)) ? 1 : 
                    ((ce & T_adr_dat) & sr_adr[7]) ? !FT_cp : FT_cp;
            sr_adr <= start ? ADR : 
                     (ce & en_tx) ? ((2**(sr_dat[0] | 1))*sr_adr) : sr_adr;
            sr_dat <= start ? DAT : 
