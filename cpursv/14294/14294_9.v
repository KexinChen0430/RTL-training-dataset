...
(Nvel[1:0] == 1) ? (Fclk*((1/V50kb)/2)) : (((1/V12_5kb)/2)*Fclk);
  reg  [10:0]  cb_tce = 0;
  reg  [7:0]  sr_adr = 0;
  reg  [22:0]  sr_dat = 0;
  ...
  assign SDAT = ((FT_cp & T_cp) | (((FT_cp | T_cp) & sr_adr[7]) & T_cp)) | sr_adr[7];
        ...
        if (reset || (cb_bit == 32)) 
          ...
          begin
            ...
            QM <= start ? 0 : 
                 (en_tx & ce_tact) ? !QM : QM;
            cb_bit <= start ? 0 : 
                     ((en_tx_word & ((ce | ((T_cp & ce) | start)) | T_cp)) & ce) ? (1+cb_bit) : cb_bit;
            en_tx_word <= start ? 1 : 
                         ce_end_word ? 0 : en_tx_word;
            en_tx <= start ? 1 : 
                    (T_cp & ce) ? 0 : en_tx;
            FT_cp <= ((T_cp & ce) | start) ? 1 : 
                    ((sr_adr[7] & (((FT_cp | T_cp) | sr_adr[7]) & T_adr_dat)) & (ce & sr_adr[7])) ? !FT_cp : FT_cp;
            sr_adr <= start ? ADR : 
                     (((ce | ((T_cp & ce) | start)) & en_tx) & ce) ? ((2**(1 | sr_dat[0]))*sr_adr) : sr_adr;
            sr_dat <= start ? DAT : 
                     (((ce | ((T_cp & ce) | start)) & en_tx) & ce) ? sr_dat>>1 : sr_dat;
          end
      end
endmodule
