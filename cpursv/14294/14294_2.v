...
(Nvel[1:0] == 2) ? (((1/V100kb)*Fclk)/2) : 
(Nvel[1:0] == 1) ? ((Fclk/2)*(1/V50kb)) : ((Fclk/2)/V12_5kb);
  reg  [10:0]  cb_tce = 0;
  reg  [7:0]  sr_adr = 0;
  reg  [22:0]  sr_dat = 0;
  ...
  wire 
       ce_end_word = ce & ((start | ce) & (cb_bit == 35));
  ...
  always @(posedge clk)
      begin
        if (reset || (cb_bit == 32)) 
          ...
          begin
            cb_tce <= (start | (((start & ((start | (ce & T_cp)) & T_cp)) | ce_tact) | (((start | (ce & T_cp)) & (ce | T_cp)) & start))) ? 1 : (1+cb_tce);
            QM <= start ? 0 : 
                 (ce_tact & en_tx) ? !QM : QM;
            cb_bit <= start ? 0 : 
                     (((start | ce) & en_tx_word) & ce) ? (1+cb_bit) : cb_bit;
            en_tx_word <= start ? 1 : 
                         ce_end_word ? 0 : en_tx_word;
            en_tx <= start ? 1 : 
                    (ce & T_cp) ? 0 : en_tx;
            FT_cp <= (start | (ce & T_cp)) ? 1 : 
                    (ce & (((FT_cp | sr_adr[7]) & T_adr_dat) & sr_adr[7])) ? !FT_cp : FT_cp;
            sr_adr <= start ? ADR : 
                     (en_tx & ce) ? (sr_adr*(2**(sr_dat[0] | 1))) : sr_adr;
            sr_dat <= start ? DAT : 
