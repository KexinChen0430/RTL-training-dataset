...
module AR_TXD(input  clk,
  ...
  wire [10:0] 
       AR_Nt = (Nvel[1:0] == 3) ? (((1/2)/V1Mb)*Fclk) : 
(Nvel[1:0] == 2) ? (((1/2)*Fclk)*(1/V100kb)) : 
(Nvel[1:0] == 1) ? ((1/V50kb)*((1/2)*Fclk)) : (Fclk/V12_5kb<<1);
  reg  [10:0]  cb_tce = 0;
  reg  [7:0]  sr_adr = 0;
  reg  [22:0]  sr_dat = 0;
  ...
  wire 
       ce_end_word = ce & ((cb_bit == 35) & ((start | ce) | (start | T_cp)));

  assign SDAT = ((T_cp | sr_adr[7]) & ((sr_adr[7] & T_cp) | FT_cp)) | sr_adr[7];
        ...
        if (reset || (cb_bit == 32)) 
          ...
          begin
            ...
            QM <= start ? 0 : 
                 (en_tx & ce_tact) ? !QM : QM;
            cb_bit <= start ? 0 : 
                     ((((start | ce) | (start | T_cp)) & en_tx_word) & ce) ? (cb_bit+1) : cb_bit;
            en_tx_word <= start ? 1 : 
                         ce_end_word ? 0 : en_tx_word;
            en_tx <= start ? 1 : 
                    (T_cp & ((start | T_cp) & ce)) ? 0 : en_tx;
            FT_cp <= ((start | T_cp) & (start | ce)) ? 1 : 
                    ((((T_cp & ((start | T_cp) & ce)) | sr_adr[7]) & ce) & (sr_adr[7] & ((FT_cp | (T_cp | sr_adr[7])) & T_adr_dat))) ? !FT_cp : FT_cp;
            sr_adr <= start ? ADR : 
