        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              GET: begin
                    proceso <= r_data[(-1)+datawidth:((-1)+datawidth)-crcorder];
                    state = COMP;
                  end
              ...
              XOR: begin
                    proceso <= (proceso & ~polinomio) | ((polinomio & (proceso & ((polinomio | proceso) | ~polinomio))) ^ polinomio);
                    state = CORRER;
                  end

              CORRER: begin
                    r_data <= {proceso[crcorder:0],r_data[((-1)<<1+datawidth)+(-crcorder):0],1'b0};
