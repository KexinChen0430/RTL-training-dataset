        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              GET: begin
                    proceso <= r_data[datawidth+(-1):((0-crcorder)+datawidth)+(-1)];
                    state = COMP;
                  end
              ...
              XOR: begin
                    proceso <= proceso ^ ((proceso | ~proceso) & ((proceso | ~proceso) & polinomio));
                    state = CORRER;
                  end

              CORRER: begin
                    r_data <= {proceso[crcorder:0],r_data[(((0-crcorder)+datawidth)+(-1))+(-1):0],1'b0};
