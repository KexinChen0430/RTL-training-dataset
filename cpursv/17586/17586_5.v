        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              GET: begin
                    proceso <= r_data[datawidth+(-1):(datawidth+(-crcorder))+(-1)];
                    state = COMP;
                  end
              ...
              XOR: begin
                    proceso <= (polinomio & (~proceso | (proceso | polinomio))) ^ proceso;
                    state = CORRER;
                  end

              CORRER: begin
                    r_data <= {proceso[crcorder:0],r_data[((datawidth+(-crcorder))+(-1))-1:0],1'b0};
