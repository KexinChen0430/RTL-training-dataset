        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              XOR: begin
                    proceso <= (((polinomio & (polinomio & (proceso | ~proceso))) & ~proceso) ^ ((proceso & polinomio) & ~proceso)) | (~polinomio & (polinomio | proceso));
                    state = CORRER;
                  end

              CORRER: begin
                    r_data <= {proceso[crcorder:0],r_data[((0-crcorder)+(-1))+(datawidth+(-1)):0],1'b0};
