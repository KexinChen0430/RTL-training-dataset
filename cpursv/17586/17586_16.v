        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              XOR: begin
                    proceso <= (proceso & ((~proceso | proceso) | ((proceso | (polinomio & ~proceso)) & ~proceso))) ^ (((~proceso | proceso) | ((proceso | (polinomio & ~proceso)) & ~proceso)) & (((polinomio & proceso) | (polinomio & ~proceso)) & (~proceso | proceso)));
                    state = CORRER;
                  end

              CORRER: begin
                    r_data <= {proceso[crcorder:0],r_data[((0-1)+(datawidth+(0-1)))+(-crcorder):0],1'b0};
