        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              XOR: begin
                    proceso <= ((((proceso | polinomio) | ~polinomio) & proceso) & ((~proceso | proceso) | ((proceso | polinomio) & ~proceso))) ^ (polinomio & ((~proceso | proceso) | ((proceso | polinomio) & ~proceso)));
                    state = CORRER;
                  end

              CORRER: begin
                    r_data <= {proceso[crcorder:0],r_data[((datawidth-1)+(-crcorder))+(0-1):0],1'b0};
