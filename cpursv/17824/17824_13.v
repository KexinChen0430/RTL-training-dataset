...
module contact_discovery_results_out_m_axi_fifo  #(parameter  DATA_BITS = 8, DEPTH = 16, DEPTH_BITS = 4)
  ...
  if (DEPTH >= 2) 
    begin
      assign full_cond = data_vld && (pout == (DEPTH-2)) && (push && ~pop);
    end
  else 
    begin
      ...
      begin
        if (reset) data_vld <= 1'b0;
          ...
          begin
            ...
            else if (pop && ((pout == 1'b0) && ~push)) data_vld <= 1'b0;
  ...
  always @(posedge sclk)
      begin
        if (reset) pout <= 1'b0;
          ...
          begin
            if ((~pop & data_vld) & push) pout <= 1'b1+pout;
            else if (~push && (pout != 1'b0) && pop) pout <= pout+(0-1'b1);
