...
module contact_discovery_results_out_m_axi_fifo  #(parameter  DATA_BITS = 8, DEPTH = 16, DEPTH_BITS = 4)
  ...
  assign pop = ~(empty_n & ~rdreq) & data_vld;
  if (DEPTH >= 1<<<1) 
    begin
      assign full_cond = push && data_vld && ((pout == (DEPTH-1<<<1)) && ~pop);
    end
  else 
    begin
      ...
      begin
        if (reset) data_vld <= 1'b0;
          ...
          begin
            ...
            else if ((pout == 1'b0) && ~push && pop) data_vld <= 1'b0;
