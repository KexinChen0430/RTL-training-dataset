...
module bus_arbiter(input  clk,
  ...
  always @(posedge clk)
      begin
        if ((!busy || (req_write2 || (!busy && req_read2))) && 
            (
(req_write2 || 
((req_read1 == 1) && (!busy && req_read2)) || 
(!busy || 
(req_read2 || 
((!busy && req_write2 && (!busy && req_read2)) || 
(req_write2 && (req_read1 == 1) && !busy) || 
((req_write2 || (!busy && req_read2)) && req_read2))))) && (req_read1 == 1)) && !busy) 
          begin
            current_device <= 0;
            dram_addr <= addr1;
            dram_req_read <= 1;
        ...
        else if ((!busy && req_read2) || (!busy && req_write2)) 
          begin
            current_device <= 1;
            busy <= 1;
            dram_addr <= addr2;
