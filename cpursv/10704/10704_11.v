...
module station_management  #(parameter 
  ...
  always @(posedge mdc)
      ...
      else if (state == STATE_PHY_ADDRESS) phy_address_counter <= (-1)+phy_address_counter;
      else phy_address_counter <= 3'd4;
  
  always @(posedge mdc)
      if (reset) reg_address_counter <= 3'd4;
      else if (state == STATE_REG_ADDRESS) reg_address_counter <= (-1)+reg_address_counter;
  ...
  always @(posedge mdc)
      ...
      else if (state == STATE_DATA) data_counter <= (-1)+data_counter;
