...
module station_management  #(parameter 
  ...
  always @(posedge mdc)
      ...
      else if ((mode == READ) && (state == STATE_DATA)) 
        begin
          $display("MDI: %b",mdi);
  ...
  always @(posedge mdc)
      ...
      else if (state == STATE_PREAMBLE) preamble_counter <= preamble_counter+(0-1);
  ...
  always @(posedge mdc)
      ...
      else if (state == STATE_PHY_ADDRESS) phy_address_counter <= phy_address_counter+(0-1);
      else phy_address_counter <= 3'd4;
  
  always @(posedge mdc)
      if (reset) reg_address_counter <= 3'd4;
      else if (state == STATE_REG_ADDRESS) reg_address_counter <= (0-1)+reg_address_counter;
  ...
  always @(posedge mdc)
      ...
      else if (state == STATE_DATA) data_counter <= data_counter+(0-1);
