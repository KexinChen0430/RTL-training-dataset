...
module RegBank  #(parameter 
       ...
       USER_STACK           = 8191,
       OS_START             = 3072,
       SP_KEEPER_REGISTER   = 6,
       SYSTEM_CALL_REGISTER = 7,
       PC_KEEPER_REGISTER   = 13)
  (input  enable,reset,slow_clock,fast_clock,
   input  [1+1:0] control,
   input  [3:0] register_source_A,register_source_B,register_Dest,
   input  [WORD_SIZE+(0-1):0] ALU_result,data_from_memory,
   input  [WORD_SIZE+(0-1):0] new_SP,new_PC,
   output reg [WORD_SIZE+(0-1):0] read_data_A,read_data_B,
   output reg [WORD_SIZE+(0-1):0] current_PC,current_SP,memory_output,
   input  [(0-1)+SPECREG_LENGTH:0] special_register);

  reg  [WORD_SIZE+(0-1):0] Bank[15:0];
  wire RD_isnt_special;

  assign RD_isnt_special = (register_Dest != 14) && 
                           (register_Dest != PC_REGISTER);
            ...
            if (enable) 
              begin
                case (control)
                  ...
                  5: begin
                        if (RD_isnt_special) Bank[register_Dest] <= special_register;
