...
module clk_sel(input  h_reset_n,
               input  t_crt_clk,
               input  m_sr01_b3,
               input  a_ar10_b6,
               input  final_sh_ld,
  ...
  always @(posedge t_crt_clk)
      if (~crt_4_clk_ff[1] & ~crt_2_clk_ff) clk_sel_reg <= clk_sel_ctl;
  ...
  always @(posedge t_crt_clk or negedge h_reset_n)
      if (~h_reset_n) int_sh_ld_ff <= 1'b0;
      else int_sh_ld_ff <= final_sh_ld;
  
  always @(posedge t_crt_clk or negedge h_reset_n)
      if (~h_reset_n) sh_ld_ctl_ff <= 1'b0;
      else if (pre_load & (cclk_en & ~final_sh_ld)) sh_ld_ctl_ff <= dclk_by_2;
        
  assign sh_ld = sh_ld_ctl_ff ? (final_sh_ld & int_sh_ld_ff) : final_sh_ld;
  
  always @(posedge t_crt_clk or negedge h_reset_n)
      if (~h_reset_n) crt_2_clk_ff <= 1'b0;
      else crt_2_clk_ff <= ~crt_2_clk_ff;
  
  always @(posedge t_crt_clk or negedge h_reset_n)
      if (~h_reset_n) crt_4_clk_ff <= 3'b0;
      else 
        begin
  ...
  always @(posedge t_crt_clk or negedge h_reset_n)
      if (~h_reset_n) 
        ...
        begin
          ...
          if (~sel_sh_ld_pulse_store & final_sh_ld) 
            begin
  ...
  assign sel_sh_ld = sel_sh_ld_reg | final_sh_ld;
endmodule
