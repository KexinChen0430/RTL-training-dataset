...
module ID_stage(clk,rst,ID_stall,ID_flush,EX_stall,ID_reg_dst,ID_data_a,
  ...
  always @(posedge clk)
      begin
        ...
        EX_aluc <= rst ? 21'b0 : 
                  EX_stall ? EX_aluc : 
                  (ID_stall | ID_flush) ? 21'b0 : ID_aluc;
        EX_sign <= rst ? 0 : 
                  EX_stall ? EX_sign : ID_sign;
        EX_memread <= rst ? 0 : 
                     EX_stall ? EX_memread : 
                     (ID_stall | ID_flush) ? 0 : ID_memread;
        EX_memwrite <= rst ? 0 : 
                      EX_stall ? EX_memwrite : 
                      (ID_stall | ID_flush) ? 0 : ID_memwrite;
        EX_memaddr <= rst ? 0 : 
                     EX_stall ? EX_memaddr : 
                     (ID_stall | ID_flush) ? 0 : ID_memaddr;
        EX_load_op <= rst ? 9'b0 : 
        ...
        EX_regwrite <= rst ? 0 : 
                      ...
                      (ID_stall | ID_flush) ? 0 : ID_regwrite;
        EX_memtoreg <= rst ? 0 : 
