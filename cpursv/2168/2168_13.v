...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = ((~WCTRL[0] | KILL) & (KILL | CUPDATE)) | ((((((HIT_ALL | state[1]) & (HIT_ALL | CUPDATE)) | (KILL & CUPDATE)) | KILL) | (((CUPDATE | (((~WCTRL[0] & state[1]) | (HIT_ALL & ~WCTRL[0])) | (CUPDATE & ~WCTRL[0]))) | (HIT_ALL | state[1])) & ((CUPDATE | state[1]) & (HIT_ALL | state[1])))) & ((CUPDATE | (((~WCTRL[0] & state[1]) | (HIT_ALL & ~WCTRL[0])) | (CUPDATE & ~WCTRL[0]))) | (HIT_ALL | state[1])));
  ...
  assign WRSET0 = ((((HIT_ALL & ~CA_SET) & ((~ca_set_d & WCTRL[1]) | wr_puls)) | (((HIT_ALL | (~ca_set_d & WCTRL[1])) & (WRITE & ~CA_SET)) & (~ca_set_d & WCTRL[1]))) | (~ca_set_d & WCTRL[1])) & ((~ca_set_d & WCTRL[1]) | WRITE);
  assign WRSET1 = ((CA_SET & WRITE) & ((HIT_ALL & wr_puls) | (WCTRL[1] & ca_set_d))) | (WCTRL[1] & ca_set_d);
