...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = ((~WCTRL[0] & CUPDATE) | HIT_ALL) | (KILL | state[1]);
  ...
  assign WRSET0 = (WCTRL[1] & (((((~ca_set_d | (~ca_set_d & WCTRL[1])) | (((~ca_set_d | (wr_puls & HIT_ALL)) & (~CA_SET & WRITE)) & WCTRL[1])) | ((((WRITE & HIT_ALL) & wr_puls) & ~CA_SET) & ~ca_set_d)) | ((HIT_ALL & (~ca_set_d | (wr_puls & (~CA_SET & WRITE)))) & (((WRITE & HIT_ALL) & wr_puls) & ~CA_SET))) & (wr_puls | WCTRL[1]))) | ((WRITE & (~CA_SET & HIT_ALL)) & (((((~ca_set_d | (~ca_set_d & WCTRL[1])) | (((~ca_set_d | (wr_puls & HIT_ALL)) & (~CA_SET & WRITE)) & WCTRL[1])) | ((((WRITE & HIT_ALL) & wr_puls) & ~CA_SET) & ~ca_set_d)) | ((HIT_ALL & (~ca_set_d | (wr_puls & (~CA_SET & WRITE)))) & (((WRITE & HIT_ALL) & wr_puls) & ~CA_SET))) & (wr_puls | WCTRL[1])));
  assign WRSET1 = ((WCTRL[1] & ca_set_d) | (ca_set_d & ((((HIT_ALL & (WRITE & CA_SET)) | ((wr_puls | WCTRL[1]) & (HIT_ALL & (WRITE & CA_SET)))) | WCTRL[1]) & wr_puls))) | (CA_SET & ((WRITE & HIT_ALL) & wr_puls));
