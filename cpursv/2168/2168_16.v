...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = ((state[1] | (state[1] & ~WCTRL[0])) | ((~WCTRL[0] & HIT_ALL) | (~WCTRL[0] & CUPDATE))) | ((HIT_ALL | KILL) | ((state[1] | (KILL | ((~WCTRL[0] | (HIT_ALL & state[1])) | HIT_ALL))) & ((CUPDATE & (HIT_ALL | state[1])) | state[1])));
  ...
  assign WRSET0 = ((~ca_set_d & WCTRL[1]) | (((WRITE & HIT_ALL) & (wr_puls & ~CA_SET)) & WCTRL[1])) | ((~CA_SET & WRITE) & ((wr_puls & ((HIT_ALL | ((((~ca_set_d & WCTRL[1]) | (~CA_SET & WRITE)) & HIT_ALL) & (~ca_set_d & WCTRL[1]))) | (~ca_set_d & WCTRL[1]))) | (~ca_set_d & WCTRL[1])));
  assign WRSET1 = ((HIT_ALL | (ca_set_d & ((WRITE & CA_SET) & (((((CA_SET & (WCTRL[1] | ((WRITE & HIT_ALL) & wr_puls))) | WCTRL[1]) | wr_puls) & HIT_ALL) | WCTRL[1])))) | (WCTRL[1] & ca_set_d)) & (((CA_SET | ca_set_d) & (ca_set_d | (wr_puls & WRITE))) & ((CA_SET & (WCTRL[1] | ((WRITE & HIT_ALL) & wr_puls))) | WCTRL[1]));
