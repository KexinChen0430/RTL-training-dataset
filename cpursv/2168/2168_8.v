...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = (((((KILL | state[1]) | (((CUPDATE | (state[1] & HIT_ALL)) | state[1]) | HIT_ALL)) & ~WCTRL[0]) | (state[1] & HIT_ALL)) | HIT_ALL) | (KILL | ((CUPDATE & ((((~WCTRL[0] & state[1]) | (HIT_ALL & ~WCTRL[0])) | state[1]) | HIT_ALL)) | state[1]));
  ...
  assign WRSET0 = (((wr_puls | ((wr_puls | ~ca_set_d) & WCTRL[1])) & ((~CA_SET & HIT_ALL) | (~ca_set_d & WCTRL[1]))) & WRITE) | (~ca_set_d & WCTRL[1]);
  assign WRSET1 = ((((((((HIT_ALL | (HIT_ALL & (WCTRL[1] | (wr_puls & (WRITE & ~CA_SET))))) | WCTRL[1]) & ((((HIT_ALL & CA_SET) & (wr_puls & WRITE)) | (((wr_puls & WRITE) | WCTRL[1]) & CA_SET)) | WCTRL[1])) & (HIT_ALL & wr_puls)) | ((WRITE & CA_SET) & ((((HIT_ALL & wr_puls) & ca_set_d) | (wr_puls & (ca_set_d & WCTRL[1]))) | (ca_set_d & WCTRL[1])))) | (((((HIT_ALL & wr_puls) & ca_set_d) | (wr_puls & (ca_set_d & WCTRL[1]))) | (ca_set_d & WCTRL[1])) & WCTRL[1])) & (WRITE & CA_SET)) | (((ca_set_d & ((wr_puls & WRITE) & CA_SET)) | (ca_set_d & WCTRL[1])) & HIT_ALL)) | (WCTRL[1] & ((ca_set_d & ((wr_puls & WRITE) & CA_SET)) | (ca_set_d & WCTRL[1])));
