...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = ((KILL | ((~WCTRL[0] & HIT_ALL) | (CUPDATE & ~WCTRL[0]))) | (~WCTRL[0] & ((HIT_ALL | state[1]) & ((CUPDATE | (CUPDATE & state[1])) | state[1])))) | ((((CUPDATE | (CUPDATE & state[1])) | state[1]) | ((HIT_ALL | state[1]) & (HIT_ALL | CUPDATE))) & ((state[1] | ~WCTRL[0]) | HIT_ALL));
  ...
  assign WRSET0 = (((WCTRL[1] | ((HIT_ALL & wr_puls) & ~CA_SET)) & (WRITE | WCTRL[1])) & ((WRITE & ~CA_SET) & ((wr_puls | ~ca_set_d) & (~ca_set_d | HIT_ALL)))) | (((~ca_set_d & ((WRITE & ~CA_SET) & (WCTRL[1] | (HIT_ALL & wr_puls)))) | ((HIT_ALL & (((WCTRL[1] & ~ca_set_d) | ~CA_SET) & (WRITE | (WCTRL[1] & ~ca_set_d)))) & (WCTRL[1] & ~ca_set_d))) | (WCTRL[1] & ~ca_set_d));
  assign WRSET1 = (((wr_puls & ((ca_set_d | CA_SET) & ((HIT_ALL & WRITE) | ca_set_d))) & WCTRL[1]) | (WCTRL[1] & ca_set_d)) | (((WCTRL[1] & ca_set_d) | ((((wr_puls & ca_set_d) | (WCTRL[1] & ca_set_d)) | wr_puls) & (HIT_ALL & CA_SET))) & WRITE);
