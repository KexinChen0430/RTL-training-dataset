...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = (((HIT_ALL | (KILL & state[1])) | KILL) | ((((~WCTRL[0] | ((CUPDATE | state[1]) & HIT_ALL)) | HIT_ALL) & CUPDATE) | (state[1] & CUPDATE))) | ((~WCTRL[0] | state[1]) & (state[1] | HIT_ALL));
  ...
  assign WRSET0 = (WCTRL[1] & ~ca_set_d) | (WRITE & ((((((((wr_puls & (WRITE & HIT_ALL)) & ~CA_SET) & ~ca_set_d) | ((WCTRL[1] & ~ca_set_d) & wr_puls)) | (WCTRL[1] & ~ca_set_d)) | wr_puls) & (~CA_SET & HIT_ALL)) | (WCTRL[1] & ~ca_set_d)));
  assign WRSET1 = (ca_set_d & WCTRL[1]) | ((wr_puls & (WRITE & HIT_ALL)) & CA_SET);
