...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = (((state[1] | ~WCTRL[0]) | KILL) & (((((((CUPDATE & (state[1] | HIT_ALL)) | ((wr_puls & HIT_ALL) & (CA_SET & WRITE))) | HIT_ALL) | (KILL & state[1])) | KILL) | (state[1] | HIT_ALL)) | ((((state[1] & ~WCTRL[0]) | (((CUPDATE | (HIT_ALL & CUPDATE)) | HIT_ALL) & ~WCTRL[0])) | CUPDATE) | (state[1] | HIT_ALL)))) | HIT_ALL;
  ...
  assign WRSET0 = ((~ca_set_d & ((wr_puls | WCTRL[1]) & (HIT_ALL & (~CA_SET & WRITE)))) | (WCTRL[1] & ~ca_set_d)) | (HIT_ALL & (((WCTRL[1] | (WCTRL[1] & ~ca_set_d)) | (HIT_ALL & (wr_puls & (~CA_SET & WRITE)))) & (((WRITE & wr_puls) | ~ca_set_d) & (~ca_set_d | ~CA_SET))));
  assign WRSET1 = ((ca_set_d & WCTRL[1]) | (WCTRL[1] & (((CA_SET & WRITE) & HIT_ALL) & (wr_puls | ca_set_d)))) | ((wr_puls & HIT_ALL) & (CA_SET & WRITE));
