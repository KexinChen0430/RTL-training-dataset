...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WRCRAM0 = ~CA_SET & (~WCTRL[0] & CUPDATE);
  assign WRCRAM1 = (~WCTRL[0] & CUPDATE) & CA_SET;
  assign WE_CV = ((state[1] & ~WCTRL[0]) | (~WCTRL[0] & CUPDATE)) | (((state[1] | (KILL & state[1])) | KILL) | (HIT_ALL | KILL));
  ...
  assign WRSET0 = (((((WRITE & (~CA_SET & HIT_ALL)) & (((~ca_set_d | (HIT_ALL & (wr_puls & ~CA_SET))) & WRITE) | ~ca_set_d)) | (WCTRL[1] & ((WRITE & ~CA_SET) & ((wr_puls & ((((~ca_set_d | (HIT_ALL & (wr_puls & ~CA_SET))) & WRITE) | ~ca_set_d) | HIT_ALL)) | ~ca_set_d)))) | (~ca_set_d & WCTRL[1])) & WCTRL[1]) | (((((WRITE & (~CA_SET & HIT_ALL)) & (((~ca_set_d | (HIT_ALL & (wr_puls & ~CA_SET))) & WRITE) | ~ca_set_d)) | (WCTRL[1] & ((WRITE & ~CA_SET) & ((wr_puls & ((((~ca_set_d | (HIT_ALL & (wr_puls & ~CA_SET))) & WRITE) | ~ca_set_d) | HIT_ALL)) | ~ca_set_d)))) | (~ca_set_d & WCTRL[1])) & wr_puls);
  assign WRSET1 = (((((WCTRL[1] | ((wr_puls & CA_SET) & WRITE)) & HIT_ALL) | WCTRL[1]) & ((ca_set_d | wr_puls) & ((ca_set_d | (wr_puls & (WRITE & (~CA_SET & HIT_ALL)))) | HIT_ALL))) & ca_set_d) | ((WRITE & CA_SET) & ((((WCTRL[1] | ((wr_puls & CA_SET) & WRITE)) & HIT_ALL) | WCTRL[1]) & ((ca_set_d | wr_puls) & ((ca_set_d | (wr_puls & (WRITE & (~CA_SET & HIT_ALL)))) | HIT_ALL))));
