...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WRCRAM1 = CA_SET & (CUPDATE & ~WCTRL[0]);
  assign WE_CV = ((((((state[1] | (KILL & HIT_ALL)) | KILL) | (HIT_ALL | CUPDATE)) & ~WCTRL[0]) | (CUPDATE & HIT_ALL)) | HIT_ALL) | (((state[1] | (KILL & HIT_ALL)) | KILL) | ((((state[1] | HIT_ALL) & CUPDATE) | HIT_ALL) & (((state[1] | (KILL & HIT_ALL)) | KILL) | (~WCTRL[0] | HIT_ALL))));
  ...
  assign WRSET0 = (((~ca_set_d | ((~ca_set_d | ~CA_SET) & WRITE)) & (wr_puls | ~ca_set_d)) & (((WCTRL[1] | (WCTRL[1] & (((wr_puls & ~ca_set_d) | (((HIT_ALL & ~CA_SET) & WRITE) & (WCTRL[1] & ~ca_set_d))) | (WCTRL[1] & ~ca_set_d)))) | ((((wr_puls & ~ca_set_d) | (((HIT_ALL & ~CA_SET) & WRITE) & (WCTRL[1] & ~ca_set_d))) | (WCTRL[1] & ~ca_set_d)) & ((HIT_ALL & ~CA_SET) & WRITE))) | ((((HIT_ALL & ~CA_SET) & WRITE) & (WCTRL[1] | wr_puls)) & (WRITE & (wr_puls & (HIT_ALL & ~CA_SET)))))) & (HIT_ALL | ~ca_set_d);
  assign WRSET1 = ((CA_SET & wr_puls) & (WRITE & HIT_ALL)) | ((WCTRL[1] & ca_set_d) | (((CA_SET & wr_puls) & (WRITE & HIT_ALL)) & ca_set_d));
