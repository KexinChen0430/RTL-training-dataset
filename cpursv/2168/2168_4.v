...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = ((HIT_ALL | KILL) | (((~WCTRL[0] | ((HIT_ALL | CUPDATE) & state[1])) | state[1]) & ((state[1] | ((~CA_SET & HIT_ALL) & (WRITE & wr_puls))) | HIT_ALL))) | ((HIT_ALL | ~WCTRL[0]) & CUPDATE);
  ...
  assign WRSET0 = (((HIT_ALL & (WRITE & ~CA_SET)) & (~ca_set_d | (((~ca_set_d | (WRITE & ~CA_SET)) & (HIT_ALL | ~ca_set_d)) & wr_puls))) | ((WCTRL[1] & ((~CA_SET & HIT_ALL) & (WRITE & wr_puls))) | (~ca_set_d & WCTRL[1]))) & ((wr_puls | (CA_SET & (WCTRL[1] | (wr_puls & (WRITE & HIT_ALL))))) | WCTRL[1]);
  assign WRSET1 = (((wr_puls & ((HIT_ALL & CA_SET) & WRITE)) & WCTRL[1]) | (WCTRL[1] & ca_set_d)) | ((wr_puls & (((WCTRL[1] | ((WCTRL[1] & ca_set_d) | (ca_set_d & (wr_puls & ((HIT_ALL & CA_SET) & WRITE))))) | ((wr_puls & ((HIT_ALL & CA_SET) & WRITE)) & (((wr_puls | (CA_SET & (WCTRL[1] | (wr_puls & (WRITE & HIT_ALL))))) | WCTRL[1]) & ((HIT_ALL & CA_SET) & WRITE)))) | ((HIT_ALL & CA_SET) & WRITE))) & (wr_puls & ((HIT_ALL & CA_SET) & WRITE)));
