...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = ((~WCTRL[0] & (CUPDATE | state[1])) | KILL) | (((state[1] | (HIT_ALL & (CUPDATE | state[1]))) | HIT_ALL) | KILL);
  ...
  assign WRSET0 = (((((HIT_ALL & ~CA_SET) & (wr_puls & WRITE)) & WCTRL[1]) | (((~CA_SET & WRITE) & ((~ca_set_d & WCTRL[1]) | HIT_ALL)) & (~ca_set_d & WCTRL[1]))) | (~ca_set_d & WCTRL[1])) | (WRITE & (((wr_puls | (~ca_set_d & (((~CA_SET & WRITE) & HIT_ALL) & (WCTRL[1] | wr_puls)))) | (~ca_set_d & WCTRL[1])) & ((~ca_set_d & WCTRL[1]) | (HIT_ALL & ~CA_SET))));
  assign WRSET1 = (ca_set_d & ((((ca_set_d & ((((WRITE | (((HIT_ALL & ~CA_SET) & (wr_puls & WRITE)) & WCTRL[1])) | WCTRL[1]) & (CA_SET & HIT_ALL)) | WCTRL[1])) & (WCTRL[1] | wr_puls)) | (((WRITE & CA_SET) & (WCTRL[1] | (HIT_ALL & wr_puls))) & ((CA_SET & wr_puls) & WRITE))) | (((CA_SET & wr_puls) & WRITE) & WCTRL[1]))) | (((((ca_set_d & ((((WRITE | (((HIT_ALL & ~CA_SET) & (wr_puls & WRITE)) & WCTRL[1])) | WCTRL[1]) & (CA_SET & HIT_ALL)) | WCTRL[1])) & (WCTRL[1] | wr_puls)) | (((WRITE & CA_SET) & (WCTRL[1] | (HIT_ALL & wr_puls))) & ((CA_SET & wr_puls) & WRITE))) | (((CA_SET & wr_puls) & WRITE) & WCTRL[1])) & HIT_ALL);
