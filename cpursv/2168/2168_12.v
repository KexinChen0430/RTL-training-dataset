...
module DCA_CONTROL(BCLK,MCLK,BRESET,CUPDATE,DRAM_ACC,CA_SET,HIT_ALL,WRCFG,
  ...
  assign WE_CV = ((CUPDATE & ((state[1] | HIT_ALL) | (HIT_ALL | ~WCTRL[0]))) | state[1]) | ((HIT_ALL | KILL) | ((state[1] | HIT_ALL) & CUPDATE));
  ...
  assign WRSET0 = ((WCTRL[1] & ~ca_set_d) & ((((~CA_SET & HIT_ALL) & WRITE) & (wr_puls | ((WCTRL[1] | WRITE) & (WCTRL[1] | (wr_puls & (CA_SET & HIT_ALL)))))) | WCTRL[1])) | (((((~CA_SET & HIT_ALL) & WRITE) & (wr_puls | ((WCTRL[1] | WRITE) & (WCTRL[1] | (wr_puls & (CA_SET & HIT_ALL)))))) | WCTRL[1]) & ((((wr_puls & (((~CA_SET & WRITE) | (~ca_set_d & ((~CA_SET & WRITE) & ((HIT_ALL & wr_puls) | WCTRL[1])))) | (WCTRL[1] & ~ca_set_d))) | ((WCTRL[1] & ~ca_set_d) & (~CA_SET & ((((WCTRL[1] & ~ca_set_d) | HIT_ALL) & WRITE) | (WCTRL[1] & ~ca_set_d))))) | (WCTRL[1] & ~ca_set_d)) & HIT_ALL));
  assign WRSET1 = ((wr_puls & (WRITE & CA_SET)) & (HIT_ALL & ((WCTRL[1] | WRITE) & (WCTRL[1] | (wr_puls & (CA_SET & HIT_ALL)))))) | ((ca_set_d & ((wr_puls & (WRITE & CA_SET)) & (HIT_ALL & ((WCTRL[1] | WRITE) & (WCTRL[1] | (wr_puls & (CA_SET & HIT_ALL))))))) | (ca_set_d & WCTRL[1]));
