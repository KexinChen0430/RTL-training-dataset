...
(is_req && 
(!tx_ena_bus && 
((!send_req && is_req) || (!send_req && !send_resp)) && (!is_req || !send_req)))) || 
                    (
((!send_resp && !is_req) || 
((!send_req && (!send_resp && !is_req)) || 
(!send_req && !is_req)) || 
((!send_req && is_req) || 
((is_req && (!send_resp && !is_req)) || 
(!send_resp && (!send_req && is_req))))) && (!tx_ena_bus && is_req)) || 
                    (is_req && 
(!tx_ena_bus && 
((!send_req && is_req) || (!send_req && !send_resp)) && (!is_req || !send_req))) || 
                    (
...
((!send_req && is_req) || 
((is_req && (!send_resp && !is_req)) || 
(!send_resp && (!send_req && is_req))))) && !send_resp && !tx_ena_bus && !is_req)) 
                  begin
                    bus_state <= IDLE;
                  end
