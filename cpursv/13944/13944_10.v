      ...
      begin
        case (bus_state)
          ...
          WAIT: begin
                if (
(
((!send_resp && !send_req) || (is_req && !send_req)) && 
(
(!tx_ena_bus && ((!tx_ena_bus || is_req) && !is_req)) || (!tx_ena_bus && !send_req))) || 
                    (
(!send_resp && 
(
(!tx_ena_bus && ((!tx_ena_bus || is_req) && !is_req)) || (!tx_ena_bus && !send_req))) || 
...
((!is_req && !send_req) || 
(
((!send_resp && !send_req) || (is_req && !send_req)) && !is_req) || (((is_req && !send_req) || !send_resp) && !is_req) || 
(is_req && !send_req))))) 
                  begin
                    bus_state <= IDLE;
                  end
