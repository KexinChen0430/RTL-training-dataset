...
module cordic_stage(clock,reset,enable,xi,yi,zi,constant,xo,yo,zo);
  ...
  always @(posedge clock)
      if (reset) 
        ...
        begin
          xo <= z_is_pos ? (xi-{{shift+1{yi[bitwidth+(0-1)]}},yi[bitwidth+(0-1<<1):shift]}) : ({{shift+1{yi[bitwidth+(0-1)]}},yi[bitwidth+(0-1<<1):shift]}+xi);
          yo <= z_is_pos ? (yi+{{shift+1{xi[bitwidth+(0-1)]}},xi[bitwidth+(0-1<<1):shift]}) : ((0-{{shift+1{xi[bitwidth+(0-1)]}},xi[bitwidth+(0-1<<1):shift]})+yi);
