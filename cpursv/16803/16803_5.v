...
module cordic_stage(clock,reset,enable,xi,yi,zi,constant,xo,yo,zo);
  ...
  always @(posedge clock)
      if (reset) 
        ...
        begin
          xo <= z_is_pos ? (xi-{{shift+1{yi[bitwidth-1]}},yi[bitwidth-1<<<1:shift]}) : (xi+{{shift+1{yi[bitwidth-1]}},yi[bitwidth-1<<<1:shift]});
          yo <= z_is_pos ? (yi+{{shift+1{xi[bitwidth-1]}},xi[bitwidth-1<<<1:shift]}) : (yi+(-{{shift+1{xi[bitwidth-1]}},xi[bitwidth-1<<<1:shift]}));
