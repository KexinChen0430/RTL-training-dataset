...
module cordic_stage(clock,reset,enable,xi,yi,zi,constant,xo,yo,zo);
  ...
  always @(posedge clock)
      if (reset) 
        ...
        begin
          xo <= z_is_pos ? (xi+(0-{{1+shift{yi[bitwidth-1]}},yi[bitwidth-1<<1:shift]})) : (xi+{{1+shift{yi[bitwidth-1]}},yi[bitwidth-1<<1:shift]});
          yo <= z_is_pos ? ({{1+shift{xi[bitwidth-1]}},xi[bitwidth-1<<1:shift]}+yi) : (yi-{{1+shift{xi[bitwidth-1]}},xi[bitwidth-1<<1:shift]});
