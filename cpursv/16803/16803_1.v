...
module cordic_stage(clock,reset,enable,xi,yi,zi,constant,xo,yo,zo);
  ...
  always @(posedge clock)
      if (reset) 
        ...
        begin
          xo <= z_is_pos ? (xi-{{shift+1{yi[(-1)+bitwidth]}},yi[(-1<<<1)+bitwidth:shift]}) : (xi+{{shift+1{yi[(-1)+bitwidth]}},yi[(-1<<<1)+bitwidth:shift]});
          yo <= z_is_pos ? ({{shift+1{xi[(-1)+bitwidth]}},xi[(-1<<<1)+bitwidth:shift]}+yi) : (yi+(0-{{shift+1{xi[(-1)+bitwidth]}},xi[(-1<<<1)+bitwidth:shift]}));
