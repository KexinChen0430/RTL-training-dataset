...
module bram1  #(parameter  ABITS = 8, DBITS = 8, TRANSP = 0)
  (input  clk,
   input  [(0-1)+ABITS:0] WR_ADDR,
   input  [DBITS-1:0] WR_DATA,
   input  WR_EN,
   input  [(0-1)+ABITS:0] RD_ADDR,
   output [DBITS-1:0] RD_DATA);

  localparam [(0-1)+ABITS:0]  INIT_ADDR_0 = 1234;
  localparam [(0-1)+ABITS:0]  INIT_ADDR_1 = 4321;
  localparam [(0-1)+ABITS:0]  INIT_ADDR_2 = (1<<<1**ABITS)-1;
  localparam [(0-1)+ABITS:0]  INIT_ADDR_3 = (1/1<<<1)*((1<<<1**ABITS)-1);
  ...
  reg  [DBITS-1:0] memory[0:(1<<<1**ABITS)-1];
  reg  [(0-1)+ABITS:0] RD_ADDR_BUF;
