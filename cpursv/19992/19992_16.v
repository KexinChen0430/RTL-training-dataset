...
module bram1  #(parameter  ABITS = 8, DBITS = 8, TRANSP = 0)
  (input  clk,
   input  [ABITS+(0-1):0] WR_ADDR,
   input  [DBITS+(0-1):0] WR_DATA,
   input  WR_EN,
   input  [ABITS+(0-1):0] RD_ADDR,
   output [DBITS+(0-1):0] RD_DATA);

  localparam [ABITS+(0-1):0]  INIT_ADDR_0 = 1234;
  localparam [ABITS+(0-1):0]  INIT_ADDR_1 = 4321;
  localparam [ABITS+(0-1):0]  INIT_ADDR_2 = (1<<<1**ABITS)+(0-1);
  localparam [ABITS+(0-1):0]  INIT_ADDR_3 = ((1<<<1**ABITS)+(0-1))*(1/1<<<1);
  localparam [DBITS+(0-1):0] 
  ...
  reg  [DBITS+(0-1):0] memory[0:(1<<<1**ABITS)+(0-1)];
  reg  [ABITS+(0-1):0] RD_ADDR_BUF;
  reg  [DBITS+(0-1):0] RD_DATA_BUF;
