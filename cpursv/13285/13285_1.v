...
module cn0363_dma_sequencer(input  clk,
                            input  resetn,
                            input  [31:0] phase,
                            input  phase_valid,
        ...
        if (processing_resetn == 1'b0) 
          ...
          begin
            case (count)
              ...
              'h0: if (phase_valid) count <= 1+count;
                    

              'h1: if (data_valid) count <= 1+count;
                    

              'h2: if (data_filtered_valid) count <= 1+count;
                    

              'h3: if (i_q_valid) count <= 1+count;
                    

              'h4: if (i_q_valid) count <= 1+count;
                    

              'h5: if (i_q_filtered_valid) count <= 1+count;
                    

              'h6: if (i_q_filtered_valid) count <= 1+count;
                    

              'h7: if (phase_valid) count <= 1+count;
                    

              'h8: if (data_valid) count <= 1+count;
                    

              'h9: if (data_filtered_valid) count <= 1+count;
                    

              'ha: if (i_q_valid) count <= 1+count;
                    

              'hb: if (i_q_valid) count <= 1+count;
                    

              'hc: if (i_q_filtered_valid) count <= 1+count;
  ...
  always @(posedge clk)
      begin
        if ((channel_enable[count] == 1'b0) || 
            (processing_resetn == 1'b0)) 
          begin
            dma_wr_en <= 1'b0;
          end
        else 
