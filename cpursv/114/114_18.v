...
module state_control(input  CLK,
  ...
  always @(*)
      begin
        if ((iSW > 0) && ((iSW <= 9) && ~occupied[(0-1)+iSW])) 
          begin
  ...
  always @(*)
      begin
        ...
        if ((py1_draw && (state == PLAYER1)) || 
            (
((state == PLAYER2) && (py1_draw && (state == PLAYER1))) || 
((py2_draw || (state == PLAYER1)) && 
(py2_draw || py1_draw) && (state == PLAYER2)))) next_occupied = occupied | 1'b1<<(-1'b1)<<iSW;
          
        if (py2_draw && (state == PLAYER2)) next_symbol = 1'b1<<(-1'b1)<<iSW | symbol;
