  ...
  always @(posedge CLK)
      begin
        if (RESET) 
          ...
          begin
            if ((state == PLAYER1) && 
                ((py2_draw && (state == PLAYER2)) || py1_draw || 
(state == PLAYER2)) && py1_draw) 
              begin
                occupied[(-1)+iSW] = 1'b1;
                symbol[(-1)+iSW] = 1'b0;
              end
            else if (py2_draw && (state == PLAYER2)) 
              begin
                occupied[(-1)+iSW] = 1'b1;
                symbol[(-1)+iSW] = 1'b1;
              end
  ...
  always @(*)
      begin
        ...
        next_symbol = symbol;
        if (
((py2_draw && (state == PLAYER2)) || 
((state == PLAYER1) && 
((py2_draw && (state == PLAYER2)) || py1_draw || 
(state == PLAYER2)))) && ((py2_draw && (state == PLAYER2)) || py1_draw)) next_occupied = (1<<((-1'b1)+iSW)*1'b1) | occupied;
          
        if (py2_draw && (state == PLAYER2)) next_symbol = symbol | (1<<((-1'b1)+iSW)*1'b1);
...
((state == PLAYER1) && 
((py2_draw && (state == PLAYER2)) || py1_draw || 
(state == PLAYER2)))) && ((py2_draw && (state == PLAYER2)) || py1_draw)) 
              begin
