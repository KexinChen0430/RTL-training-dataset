...
module state_control(input  CLK,
  ...
  always @(*)
      begin
        ...
        next_symbol = symbol;
        if (((state == PLAYER1) && py1_draw) || 
            ((py1_draw && (py2_draw && (state == PLAYER1))) || 
(
((state == PLAYER2) && ((state == PLAYER1) && py1_draw)) || ((state == PLAYER2) && py2_draw)))) next_occupied = occupied | (1'b1<<(-1'b1)*1<<<iSW);
          
        if ((state == PLAYER2) && py2_draw) next_symbol = (1'b1<<(-1'b1)*1<<<iSW) | symbol;
        ...
        if (WIN == PLAYER1_WIN) next_win = PLAYER1_WIN;
          ...
          begin
            ...
            if (((state == PLAYER1) && py1_draw) || 
                ((py1_draw && (py2_draw && (state == PLAYER1))) || 
(
((state == PLAYER2) && ((state == PLAYER1) && py1_draw)) || ((state == PLAYER2) && py2_draw)))) 
              begin
