...
module state_control(input  CLK,
  ...
  always @(*)
      begin
        ...
        next_symbol = symbol;
        if ((py2_draw && (state == PLAYER2)) || 
            ((state == PLAYER1) && py1_draw)) next_occupied = ((2**iSW)*1'b1)<<(0-1'b1) | occupied;
          
        if (py2_draw && (state == PLAYER2)) next_symbol = ((2**iSW)*1'b1)<<(0-1'b1) | symbol;
