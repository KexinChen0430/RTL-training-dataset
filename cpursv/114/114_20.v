...
module state_control(input  CLK,
  ...
  always @(*)
      begin
        ...
        next_symbol = symbol;
        if (
((py2_draw || py1_draw) && (state == PLAYER1) && 
((py1_draw && (state == PLAYER1)) || 
(py1_draw && (state == PLAYER2)) || (state == PLAYER2))) || 
            (
((state == PLAYER2) && 
...
(py1_draw && 
(((state == PLAYER1) && py2_draw) || 
((state == PLAYER2) && py2_draw))))) next_occupied = occupied | (1<<(-1'b1)*1'b1<<<iSW);
          
        if ((state == PLAYER2) && py2_draw) next_symbol = (1<<(-1'b1)*1'b1<<<iSW) | symbol;
