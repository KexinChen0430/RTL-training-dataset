  ...
  always @(posedge CLK)
      begin
        if (RESET) 
          ...
          begin
            if ((py1_draw || py2_draw) && 
                ((state == PLAYER1) || py2_draw) && py1_draw && (state == PLAYER1)) 
              begin
                occupied[iSW-1] = 1'b1;
                symbol[iSW-1] = 1'b0;
              end
...
((state == PLAYER1) || py2_draw) && py1_draw && (state == PLAYER1)))) || 
            ((py1_draw || py2_draw) && 
((state == PLAYER1) || py2_draw) && py1_draw && (state == PLAYER1))) next_occupied = occupied | (1<<<(iSW-1'b1)*1'b1);
          
        if ((state == PLAYER2) && py2_draw) next_symbol = (1<<<(iSW-1'b1)*1'b1) | symbol;
