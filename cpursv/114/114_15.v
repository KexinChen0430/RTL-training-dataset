...
module state_control(input  CLK,
  ...
  always @(*)
      begin
        ...
        next_symbol = symbol;
        if ((py1_draw && (state == PLAYER1)) || 
            ((py1_draw && (state == PLAYER1) && py2_draw) || 
(
((state == PLAYER2) && (py1_draw && (state == PLAYER1))) || 
((py1_draw && (state == PLAYER1) && py2_draw) || 
(py2_draw && (state == PLAYER2)))))) next_occupied = occupied | (1<<iSW*1'b1<<<(0-1'b1));
          
        if (py2_draw && (state == PLAYER2)) next_symbol = (1<<iSW*1'b1<<<(0-1'b1)) | symbol;
