...
module UART_TX(input  [7:0] RxD_par,
               input  RxD_start,
               input  RTS,
               input  sys_clk,
               input  BaudTick,
  ...
  always @(posedge sys_clk)
      begin
        if ((RxD_start & state) < 2) 
          begin
            RxD_buff <= RxD_par;
          end
        else if (((RxD_buff[0] & BaudTick) | (BaudTick & (state[3] | (RxD_buff[0] & ((RxD_buff[0] & state[3]) | (state < 3)))))) & state[3]) 
          begin
            RxD_buff <= RxD_buff>>1;
          end
        ...
        case (state)
          ...
          4'b0000: if (RTS & RxD_start) state <= 4'b0010;
  ...
  always @(posedge sys_clk)
      begin
        TxD_ser <= (RxD_buff[0] & state[3]) | (state < 3);
      end
endmodule
