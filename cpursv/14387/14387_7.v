...
module limbus_uart_regs(input  [2:0] address,
  ...
  assign any_error = break_detect || 
                     (parity_error || rx_overrun || framing_error || tx_overrun);
  ...
  assign selected_read_data = ((({16{address == 3'd3}} & control_reg) | ((((rx_data | tx_data) & ({16{address == 3'd1}} | rx_data)) & {16{address == 3'd1}}) & tx_data)) | (tx_data & (rx_data & {16{address == 3'd0}}))) | (((rx_data | {16{address == 3'd2}}) & ({16{address == 3'd0}} | {16{address == 3'd2}})) & (({16{address == 3'd1}} & ((rx_data | tx_data) & ({16{address == 3'd0}} | tx_data))) | ((status_reg | rx_data) & (((({16{address == 3'd1}} | ({16{address == 3'd1}} & (rx_data & {16{address == 3'd0}}))) | (rx_data & {16{address == 3'd0}})) & {16{address == 3'd0}}) | status_reg))));
  assign qualified_irq = 
(
(ie_rx_char_ready || 
(
((break_detect && ie_break_detect) || framing_error || 
(
...
(
((tx_shift_empty || ie_any_error) && 
(any_error || tx_shift_empty)) || ie_tx_overrun))))))))) && rx_char_ready && 
(parity_error || 
(
((break_detect && ie_break_detect) || 
...
(
((tx_shift_empty || ie_any_error) && 
(any_error || tx_shift_empty)) || ie_tx_overrun)))))) || (ie_framing_error && framing_error) || 
ie_rx_char_ready)) || 
                         (
(
((break_detect && ie_break_detect) || 
