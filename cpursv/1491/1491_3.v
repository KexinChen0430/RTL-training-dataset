  ...
  always @(posedge aclk)
      begin
        if (axis_tready & axis_tvalid) 
          begin
            if (rst || 
                (((line_count >= ((-1)+active_lines)) && rst) || eol)) pixel_count <= 0;
...
(sof || 
(INTERLACE && 
((INTERLACE || sof) && (!fid || INTERLACE) && fid))) && INTERLACE && 
                               (((!fid && (sof || fid)) || (!sof && INTERLACE)) && 
(sof || fid))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
