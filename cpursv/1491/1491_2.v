...
module axis_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  ...
  always @(posedge aclk)
      if (axis_tready) 
        begin
          if (((line_count >= (active_lines-1)) || rst) && 
              (eol || rst)) 
            begin
              line_count <= 0;
            end
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (((sof && !fid) || !sof) && 
                               (((!fid || (fid && INTERLACE)) && sof && INTERLACE) || 
!sof) && 
                               ((fid && INTERLACE) || 
((!fid || (fid && INTERLACE)) && sof && INTERLACE))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
