...
module axis_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  ...
  always @(posedge aclk)
      begin
        if (axis_tvalid & axis_tready) 
          begin
            if (rst || (eol && (line_count >= (active_lines+(-1)))) || eol) pixel_count <= 0;
  ...
  always @(posedge aclk)
      if (axis_tready) 
        begin
          if (rst || (eol && (line_count >= (active_lines+(-1))))) 
            begin
              line_count <= 0;
            end
      ...
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (
(!fid || 
(INTERLACE && (!sof && ((sof && !fid) || INTERLACE)))) && INTERLACE && ((fid || INTERLACE) && (fid || sof))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
