...
module axis_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  ...
  assign eol = pixel_count == ((-1)+active_pixels);
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= ((!sof || (sof && (!fid && INTERLACE))) && 
                               (
(INTERLACE && 
((INTERLACE || ((sof || fid) && !fid)) && fid)) || (sof && (!fid && INTERLACE)))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
