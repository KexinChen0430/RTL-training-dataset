...
module axis_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  (input  wire aclk,
   ...
   output reg [(-1)+DATA_WIDTH:0] axis_tdata_video,
      ...
      begin
        if (axis_tvalid & axis_tready) 
          begin
            ...
            else pixel_count <= 1+pixel_count;
          end
  ...
  always @(posedge aclk)
      if (axis_tready) 
        begin
          if (rst || 
              (((line_count >= ((-1)+active_lines)) || rst) && 
((rst || eol) && (line_count >= ((-1)+active_lines))))) 
            begin
              line_count <= 0;
            end
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (((INTERLACE && sof) || (INTERLACE && fid)) && 
                               (!sof || 
(INTERLACE && 
(!fid && ((INTERLACE || sof) && (INTERLACE || !fid))) && 
(((!sof && fid) || sof) && 
(
(INTERLACE && 
(!fid && ((INTERLACE || sof) && (INTERLACE || !fid)))) || fid))))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
