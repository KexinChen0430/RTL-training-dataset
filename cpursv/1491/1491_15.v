        ...
        if (axis_tvalid & axis_tready) 
          begin
            if ((rst && (line_count >= ((0-1)+active_lines))) || 
                (eol && (line_count >= ((0-1)+active_lines))) || eol || rst) pixel_count <= 0;
  ...
  always @(posedge aclk)
      if (axis_tready) 
        begin
          if (rst || 
              (
(eol && ((line_count >= ((0-1)+active_lines)) || rst)) || (rst && (line_count >= ((0-1)+active_lines))))) 
            begin
              line_count <= 0;
            end
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (INTERLACE && 
                               (((!fid && sof) || !sof) && 
((!fid && sof) || (fid && INTERLACE)))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
