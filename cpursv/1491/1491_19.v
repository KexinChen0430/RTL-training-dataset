...
module axis_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  ...
  always @(posedge aclk)
      begin
        if (axis_tvalid & axis_tready) 
          begin
            if (rst || 
                ((rst && (line_count >= (active_lines+(0-1)))) || eol)) pixel_count <= 0;
  ...
  always @(posedge aclk)
      if (axis_tready) 
        begin
          if (((line_count >= (active_lines+(0-1))) && eol) || rst) 
            begin
              line_count <= 0;
            end
          else if (eol) line_count <= 1+line_count;
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (
((sof && (INTERLACE || !fid) && INTERLACE) || 
(INTERLACE && ((INTERLACE || !fid) && fid))) && 
                               (!sof || 
(
((sof && (INTERLACE || !fid) && INTERLACE) || 
(INTERLACE && ((INTERLACE || !fid) && fid))) && ((INTERLACE || sof) && !fid && INTERLACE)))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
