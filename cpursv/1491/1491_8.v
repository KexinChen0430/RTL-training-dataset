  ...
  always @(posedge aclk)
      begin
        if (axis_tready & axis_tvalid) 
          begin
            if (rst || ((line_count >= ((-1)+active_lines)) && eol) || eol) pixel_count <= 0;
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (INTERLACE && (!fid || !sof) && 
                               ((fid || INTERLACE) && (fid || sof) && 
(((INTERLACE || !fid) && sof && INTERLACE && !fid) || 
!sof))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
