      ...
      if (axis_tready) 
        begin
          if (rst || 
              (
(eol && 
((line_count >= (active_lines-1)) || (eol && rst))) || ((line_count >= (active_lines-1)) && rst))) 
            begin
              line_count <= 0;
            end
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (((sof && !fid) || ((!fid || !sof) && fid)) && 
                               ((sof && !fid) || INTERLACE) && ((fid || (sof && !fid)) && INTERLACE)) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
