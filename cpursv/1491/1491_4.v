...
module axis_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  ...
  always @(posedge aclk)
      begin
        if (axis_tready & axis_tvalid) 
          begin
            if (eol || 
                (((line_count >= (active_lines+(-1))) && eol) || rst)) pixel_count <= 0;
  ...
  always @(posedge aclk)
      if (axis_tready) 
        begin
          if (((line_count >= (active_lines+(-1))) && eol) || rst) 
            begin
              line_count <= 0;
            end
        ...
        if (rst) 
          ...
          begin
            axis_tdata_video <= ((sof || fid) && (!fid || !sof) && 
                               (INTERLACE && (!fid || !sof))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
