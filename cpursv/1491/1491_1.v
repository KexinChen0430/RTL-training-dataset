...
module axis_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  ...
  always @(posedge aclk)
      if (axis_tready) 
        begin
          if (rst || ((line_count >= ((-1)+active_lines)) && eol)) 
            begin
              line_count <= 0;
            end
  ...
  always @(posedge aclk)
      begin
        if (rst) 
          ...
          begin
            axis_tdata_video <= (
((INTERLACE || sof) && (sof || fid) && (!sof && INTERLACE)) || 
                               (
(fid || 
(
((!fid && sof) || (INTERLACE || (fid && (!fid && sof)))) && !fid && INTERLACE)) && (sof || fid) && 
(
((!fid && sof) || (INTERLACE || (fid && (!fid && sof)))) && !fid && INTERLACE))) ? {~line_count,~pixel_count[11:0]} : {line_count,pixel_count[11:0]};
            axis_tlast <= eol;
            axis_tuser_sof <= sof;
            if (INTERLACE) 
              begin
