...
module Computer_Datapath_RegisterFile(output reg [(-1)+WORD_WIDTH:0] ADDR_bus_out,
  ...
  parameter 
       CNTRL_WIDTH = (DR_WIDTH+FS_WIDTH)+(SA_WIDTH+(SB_WIDTH+CNTRL_FLAGS_WIDTH));
  ...
  reg  [(-1)+WORD_WIDTH:0] SYNC_RAM0[(2**DR_WIDTH)-1:0];
  reg  [(-1)+WORD_WIDTH:0] SYNC_RAM1[(2**DR_WIDTH)-1:0];
  reg  [DR_WIDTH-1:0] i;
