...
module Computer_Datapath_RegisterFile(output reg [WORD_WIDTH+(-1):0] ADDR_bus_out,
  ...
  parameter 
       CNTRL_WIDTH = ((DR_WIDTH+FS_WIDTH)+CNTRL_FLAGS_WIDTH)+(SB_WIDTH+SA_WIDTH);
  ...
  reg  [WORD_WIDTH+(-1):0] SYNC_RAM0[(-1)+((1+1)**DR_WIDTH):0];
  reg  [WORD_WIDTH+(-1):0] SYNC_RAM1[(-1)+((1+1)**DR_WIDTH):0];
  reg  [DR_WIDTH-1:0] i;
