...
module relay_encode(clk,reset,mode,data_in,data_out);
  ...
  always @(posedge clk)
      begin
        ...
        if (|data_out_counter == 1'b1) data_out_counter = (-1)+data_out_counter;
          
        if (|data_out_delay_counter == 1'b1) data_out_delay_counter = (-1)+data_out_delay_counter;
          
        if ((counter[3:0] == 4'b0) && (mode == 1'b0)) 
          begin
            if ((buffer_in[7:6] == 2'b0) && 
                ((buffer_in[3:0] == 4'hf) && (comm_active == 1'b0))) 
              begin
                bit_counter = 3'b0;
                comm_active = 1'b1;
                received_zero = 1'b0;
          ...
          begin
            if ((buffer_in[3:1] == 3'b0) && 
                ((comm_active == 1'b0) && (buffer_in[7:5] == 3'b111))) 
              begin
                bit_counter = 3'b0;
                comm_active = 1'b1;
                received_zero = 1'b0;
