...
module arbiter_for_dcache(clk,rst,dcache_done_access,v_dc_download,dc_download_flits,
                          v_cpu,cpu_access_flits,v_m_d_areg,m_d_areg_flits,flits_dc,
                          v_flits_dc,re_dc_download_flits,re_cpu_access_flits,
                          re_m_d_areg_flits,cpu_done_access,dc_download_done_access,
      ...
      begin
        ...
        case (state)
          ...
          idle: begin
                ...
                else if ((v_vector == 3'b001) || 
                         ((v_vector == 3'b100) || (v_vector == 3'b010))) 
                  begin
                    select = v_vector;
                  end
                  
                if (select == 3'b001) nstate = mem_busy;
                else if (select == 3'b010) nstate = dc_busy;
                else if (select == 3'b100) nstate = cpu_busy;
  ...
  always @(posedge clk)
      begin
        if (rst) priority_3 <= 3'b001;
        else if (update_priority_3) priority_3 <= {priority_3[1:0],priority_3[2]};
          
      end
  
  always @(posedge clk)
      begin
        if (rst) priority_2 <= 2'b01;
        else if (update_priority_2) priority_2 <= {priority_2[0],priority_2[1]};
