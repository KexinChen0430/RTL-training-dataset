...
module axis_crosspoint  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [(0-1)+(DATA_WIDTH*S_COUNT):0] s_axis_tdata,
   ...
   output wire [(DATA_WIDTH*M_COUNT)+(0-1):0] m_axis_tdata,
   ...
   input  wire [(M_COUNT*$clog2(S_COUNT))+(0-1):0] select);

  parameter  CL_S_COUNT = $clog2(S_COUNT);
  reg  [(0-1)+(DATA_WIDTH*S_COUNT):0]  s_axis_tdata_reg = {DATA_WIDTH*S_COUNT{1'b0}};
  ...
  reg  [(DATA_WIDTH*M_COUNT)+(0-1):0]  m_axis_tdata_reg = {DATA_WIDTH*M_COUNT{1'b0}};
  ...
  reg  [(0-1)+(M_COUNT*CL_S_COUNT):0]  select_reg = {M_COUNT*CL_S_COUNT{1'b0}};
