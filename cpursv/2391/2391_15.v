...
module axis_crosspoint  #(parameter 
  ...
  (input  wire clk,
   ...
   output wire [(0-1)+(DEST_WIDTH*M_COUNT):0] m_axis_tdest,
  ...
  reg  [(0-1)+(DEST_WIDTH*M_COUNT):0]  m_axis_tdest_reg = {DEST_WIDTH*M_COUNT{1'b0}};
  ...
  always @(posedge clk)
      begin
        ...
        for (i = 0; i < M_COUNT; i = i+1)
            begin
              m_axis_tdata_reg[DATA_WIDTH*i +: DATA_WIDTH] <= s_axis_tdata_reg[select_reg[CL_S_COUNT*i +: CL_S_COUNT]*DATA_WIDTH +: DATA_WIDTH];
              ...
              m_axis_tid_reg[ID_WIDTH*i +: ID_WIDTH] <= s_axis_tid_reg[ID_WIDTH*select_reg[CL_S_COUNT*i +: CL_S_COUNT] +: ID_WIDTH];
