...
module axis_crosspoint  #(parameter 
  ...
  (input  wire clk,
   ...
   output wire [(-1)+(M_COUNT*DATA_WIDTH):0] m_axis_tdata,
  ...
  reg  [(-1)+(M_COUNT*DATA_WIDTH):0]  m_axis_tdata_reg = {M_COUNT*DATA_WIDTH{1'b0}};
      ...
      begin
        ...
        for (i = 0; i < M_COUNT; i = i+1)
            begin
              ...
              m_axis_tuser_reg[i*USER_WIDTH +: USER_WIDTH] <= s_axis_tuser_reg[USER_WIDTH*select_reg[i*CL_S_COUNT +: CL_S_COUNT] +: USER_WIDTH];
            end
      end
endmodule
