...
module axis_crosspoint  #(parameter 
  ...
  (input  wire clk,
   ...
   output wire [(M_COUNT*DATA_WIDTH)+(0-1):0] m_axis_tdata,
   output wire [(0-1)+(M_COUNT*KEEP_WIDTH):0] m_axis_tkeep,
   output wire [M_COUNT-1:0] m_axis_tvalid,
   output wire [M_COUNT-1:0] m_axis_tlast,
   output wire [(M_COUNT*ID_WIDTH)+(0-1):0] m_axis_tid,
   output wire [(DEST_WIDTH*M_COUNT)+(0-1):0] m_axis_tdest,
  ...
  reg  [(M_COUNT*DATA_WIDTH)+(0-1):0]  m_axis_tdata_reg = {M_COUNT*DATA_WIDTH{1'b0}};
  ...
  reg  [(M_COUNT*ID_WIDTH)+(0-1):0]  m_axis_tid_reg = {M_COUNT*ID_WIDTH{1'b0}};
  reg  [(DEST_WIDTH*M_COUNT)+(0-1):0]  m_axis_tdest_reg = {DEST_WIDTH*M_COUNT{1'b0}};
  ...
  always @(posedge clk)
      begin
        ...
        for (i = 0; i < M_COUNT; i = 1+i)
            begin
              m_axis_tdata_reg[DATA_WIDTH*i +: DATA_WIDTH] <= s_axis_tdata_reg[DATA_WIDTH*select_reg[i*CL_S_COUNT +: CL_S_COUNT] +: DATA_WIDTH];
