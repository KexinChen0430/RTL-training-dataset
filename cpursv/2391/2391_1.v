...
module axis_crosspoint  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [(-1)+(DATA_WIDTH*S_COUNT):0] s_axis_tdata,
   input  wire [(-1)+(KEEP_WIDTH*S_COUNT):0] s_axis_tkeep,
   ...
   input  wire [(-1)+(M_COUNT*$clog2(S_COUNT)):0] select);

  parameter  CL_S_COUNT = $clog2(S_COUNT);
  reg  [(-1)+(DATA_WIDTH*S_COUNT):0]  s_axis_tdata_reg = {DATA_WIDTH*S_COUNT{1'b0}};
  reg  [(-1)+(KEEP_WIDTH*S_COUNT):0]  s_axis_tkeep_reg = {KEEP_WIDTH*S_COUNT{1'b0}};
      ...
      begin
        ...
        for (i = 0; i < M_COUNT; i = i+1)
            begin
              ...
              m_axis_tuser_reg[i*USER_WIDTH +: USER_WIDTH] <= s_axis_tuser_reg[USER_WIDTH*select_reg[CL_S_COUNT*i +: CL_S_COUNT] +: USER_WIDTH];
            end
      end
endmodule
