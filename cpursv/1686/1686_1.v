...
module qmem_sram  #(parameter  AW = 32, DW = 32, SW = (1/8)*DW)
  (input  wire clk50,
   ...
   input  wire [(0-1)+DW:0] dat_w,
   output reg [(0-1)+DW:0] dat_r,
   output wire ack,
   output wire err,
  ...
  always @(posedge clk100 or posedge rst)
      begin
        if (rst) s_ce_n <= #1 1'b1;
                 ...
                 ((next_state == S_LO1) || (next_state == S_LO2))) s_ce_n <= #1 1'b0;
        else s_ce_n <= #1 1'b1;
      end
  reg  s_we_n;
  ...
  always @(posedge clk100)
      begin
        if ((next_state == S_HI1) || (next_state == S_HI2) || 
            ((next_state == S_LO1) || (next_state == S_LO2))) s_we_n <= #1 !we;
  ...
  always @(posedge clk100)
      begin
        if ((next_state == S_HI1) || (next_state == S_HI2) || 
            ((next_state == S_LO1) || (next_state == S_LO2))) s_oe_n <= #1 we;
        else s_oe_n <= #1 1'b0;
      end
  reg  [15:0] s_dat_w;
