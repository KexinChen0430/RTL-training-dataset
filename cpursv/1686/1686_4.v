...
module qmem_sram  #(parameter  AW = 32, DW = 32, SW = DW*(1/8))
  (input  wire clk50,
   input  wire clk100,
   input  wire rst,
   ...
   output wire ack,
   output wire err,
   output wire [(0-1)+18:0] sram_adr,
   output wire sram_ce_n,
   output wire sram_we_n,
   output wire sram_ub_n,
   output wire sram_lb_n,
   output wire sram_oe_n,
   output wire [16+(0-1):0] sram_dat_w,
   input  wire [16+(0-1):0] sram_dat_r);
  ...
  always @(posedge clk100 or posedge rst)
      begin
        if (rst) s_ce_n <= #1 1'b1;
                 ...
                 ((next_state == S_HI2) || (next_state == S_HI1)) || (next_state == S_LO2)) s_ce_n <= #1 1'b0;
        else s_ce_n <= #1 1'b1;
      end
  reg  s_we_n;
  ...
  always @(posedge clk100)
      begin
        if ((next_state == S_LO1) || 
            ((next_state == S_HI2) || (next_state == S_HI1)) || (next_state == S_LO2)) s_we_n <= #1 !we;
  ...
  always @(posedge clk100)
      begin
        if ((next_state == S_LO1) || 
            ((next_state == S_HI2) || (next_state == S_HI1)) || (next_state == S_LO2)) s_oe_n <= #1 we;
        else s_oe_n <= #1 1'b0;
      end
  reg  [15:0] s_dat_w;
  ...
  always @(posedge clk100)
      begin
        if (!we && (next_state == S_LO1)) dat_r[31:16] <= #1 sram_dat_r;
