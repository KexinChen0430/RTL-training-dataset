...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= ((set_burst & ~reset) & ~clr_burst) | (~clr_burst & (state_burst & ~reset));
        state_quiet <= ((set_quiet & ~reset) & ~clr_quiet) | (~clr_quiet & (~clr_quiet & (state_quiet & ~reset)));
      end
  assign set_burst = (((((~TXCOMFINISH | (TXCOMINIT & state_idle)) | state_idle) & (TXCOMINIT & state_idle)) | (((~TXCOMFINISH | TXCOMWAKE) & ~TXCOMFINISH) | ((~TXCOMFINISH | TXCOMWAKE) & state_idle))) & ((TXCOMINIT | ((state_idle & TXCOMWAKE) | state_quiet)) & (state_idle | state_quiet))) & (((TXCOMWAKE | TXCOMINIT) & ((state_idle & ((state_idle & TXCOMWAKE) | (clr_quiet & state_quiet))) | (TXCOMINIT & state_idle))) | clr_quiet);
