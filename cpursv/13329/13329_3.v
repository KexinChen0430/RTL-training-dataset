...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= (~clr_burst & (set_burst & ~reset)) | ((~reset & state_burst) & ~clr_burst);
        state_quiet <= (~clr_quiet & (~clr_quiet & (~reset & state_quiet))) | ((set_quiet & ~reset) & ~clr_quiet);
      end
  assign set_burst = ((((state_idle | (((TXCOMINIT | (clr_quiet & state_quiet)) & (clr_quiet & state_quiet)) | ((TXCOMINIT | (clr_quiet & state_quiet)) & state_idle))) & (TXCOMWAKE | TXCOMINIT)) | (clr_quiet & state_quiet)) & (TXCOMWAKE & ((~TXCOMFINISH & (state_idle | (((TXCOMINIT | (clr_quiet & state_quiet)) & (clr_quiet & state_quiet)) | ((TXCOMINIT | (clr_quiet & state_quiet)) & state_idle)))) | ((state_idle | (((TXCOMINIT | (clr_quiet & state_quiet)) & (clr_quiet & state_quiet)) | ((TXCOMINIT | (clr_quiet & state_quiet)) & state_idle))) & state_idle)))) | ((((state_idle | (((TXCOMINIT | (clr_quiet & state_quiet)) & (clr_quiet & state_quiet)) | ((TXCOMINIT | (clr_quiet & state_quiet)) & state_idle))) & (TXCOMWAKE | TXCOMINIT)) | (clr_quiet & state_quiet)) & ((clr_quiet & (~TXCOMFINISH & state_quiet)) | ((((clr_quiet & (~TXCOMFINISH & state_quiet)) | TXCOMINIT) & (state_idle | TXCOMINIT)) & state_idle)));
  assign set_quiet = clr_burst & ((bursts_cnt < (bursts_cnt_togo+(0-1))) & state_burst);
  ...
  assign stopwatch_clr = (state_idle | ((TXCOMINIT & state_idle) | set_burst)) | set_quiet;
