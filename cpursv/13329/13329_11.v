...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= (~clr_burst & ~reset) & ((state_burst | set_burst) & (set_burst | ~reset));
        state_quiet <= ~reset & (((set_quiet & ~clr_quiet) | state_quiet) & ~clr_quiet);
      end
  assign set_burst = (((~TXCOMFINISH | (((state_idle | ~TXCOMFINISH) & TXCOMWAKE) | ((state_idle | ~TXCOMFINISH) & TXCOMINIT))) & (state_idle | (clr_quiet & state_quiet))) & (((state_idle | (clr_quiet & state_quiet)) & TXCOMINIT) | (clr_quiet & state_quiet))) | (((~TXCOMFINISH | (((state_idle | ~TXCOMFINISH) & TXCOMWAKE) | ((state_idle | ~TXCOMFINISH) & TXCOMINIT))) & (state_idle | (clr_quiet & state_quiet))) & TXCOMWAKE);
  ...
  assign stopwatch_clr = ((state_idle & TXCOMINIT) | (set_burst | set_quiet)) | state_idle;
  
  always @(posedge clk)
      stopwatch <= (stopwatch_clr | reset) ? 0 : (burst_len_mult+stopwatch);
