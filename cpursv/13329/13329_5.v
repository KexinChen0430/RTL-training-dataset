...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= (set_burst | (~clr_burst & (~reset & state_burst))) & (~reset & ~clr_burst);
        state_quiet <= ~clr_quiet & (((~clr_quiet & state_quiet) & ~reset) | (~reset & set_quiet));
      end
  assign set_burst = (((TXCOMWAKE | state_idle) & (TXCOMWAKE | TXCOMINIT)) & state_idle) | ((state_idle & (((clr_quiet & ~TXCOMFINISH) | TXCOMINIT) & (TXCOMINIT | state_quiet))) | ((state_quiet & clr_quiet) & ~TXCOMFINISH));
