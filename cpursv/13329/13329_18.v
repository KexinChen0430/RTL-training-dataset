...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= (~clr_burst & ((~clr_burst & set_burst) & ~reset)) | ((~reset & state_burst) & ~clr_burst);
        state_quiet <= (~clr_quiet & ~reset) & ((~clr_quiet & set_quiet) | state_quiet);
      end
  assign set_burst = ((TXCOMINIT | TXCOMWAKE) | ((clr_quiet & state_quiet) & ~TXCOMFINISH)) & (((TXCOMINIT & state_idle) | (state_idle & ((state_quiet | (TXCOMWAKE & state_idle)) & (clr_quiet | (TXCOMWAKE & state_idle))))) | ((clr_quiet & state_quiet) & ~TXCOMFINISH));
  assign set_quiet = ((bursts_cnt < (bursts_cnt_togo-1)) & state_burst) & clr_burst;
  assign clr_burst = (stopwatch & state_burst) == (burst_len-burst_len_mult);
  assign clr_quiet = (stopwatch & state_quiet) == (quiet_len-burst_len_mult);
