...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        issued_init <= ((reset | issued_wake) | TXCOMFINISH) ? 1'b0 : 
                      TXCOMINIT ? 1'b1 : 
                      state_idle ? 1'b0 : issued_init;
        issued_wake <= ((reset | TXCOMFINISH) | issued_init) ? 1'b0 : 
                      TXCOMWAKE ? 1'b1 : 
  ...
  always @(posedge clk)
      begin
        state_burst <= (~reset & ~clr_burst) & (set_burst | (~clr_burst & state_burst));
        state_quiet <= ((state_quiet & ~reset) & ~clr_quiet) | ((set_quiet & ~reset) & ~clr_quiet);
      end
  assign set_burst = ((((state_idle & ((TXCOMINIT & state_idle) | TXCOMWAKE)) | (((TXCOMWAKE | ~TXCOMFINISH) & state_idle) | ((TXCOMWAKE | ~TXCOMFINISH) & (((TXCOMINIT | ~TXCOMFINISH) & state_idle) | ((TXCOMINIT | ~TXCOMFINISH) & ~TXCOMFINISH))))) & state_idle) | ((((state_idle & ((TXCOMINIT & state_idle) | TXCOMWAKE)) | (((TXCOMWAKE | ~TXCOMFINISH) & state_idle) | ((TXCOMWAKE | ~TXCOMFINISH) & (((TXCOMINIT | ~TXCOMFINISH) & state_idle) | ((TXCOMINIT | ~TXCOMFINISH) & ~TXCOMFINISH))))) & clr_quiet) & state_quiet)) & (TXCOMINIT | (TXCOMWAKE | (clr_quiet & state_quiet)));
