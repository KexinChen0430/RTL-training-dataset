...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        ...
        issued_wake <= ((reset | issued_init) | TXCOMFINISH) ? 1'b0 : 
                      TXCOMWAKE ? 1'b1 : 
  ...
  always @(posedge clk)
      begin
        state_burst <= ((~reset & ~clr_burst) & ((~clr_burst & state_burst) & ~reset)) | (~clr_burst & (set_burst & ~reset));
        state_quiet <= ~reset & ((~clr_quiet & set_quiet) | (~clr_quiet & (state_quiet & ~clr_quiet)));
      end
  assign set_burst = ((((TXCOMINIT & state_idle) | ~TXCOMFINISH) | state_idle) & (~TXCOMFINISH | (TXCOMINIT | TXCOMWAKE))) & ((((clr_quiet | state_idle) & (clr_quiet | TXCOMINIT)) & (state_quiet | (TXCOMINIT & state_idle))) | (((clr_quiet & state_quiet) | state_idle) & (TXCOMINIT | TXCOMWAKE)));
  assign set_quiet = (state_burst & (bursts_cnt < ((0-1)+bursts_cnt_togo))) & clr_burst;
