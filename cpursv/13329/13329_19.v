...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= (~clr_burst & ((set_burst & ~clr_burst) & ~reset)) | ((~clr_burst & state_burst) & ~reset);
        state_quiet <= ((set_quiet & ~clr_quiet) & ~reset) | (~clr_quiet & (~reset & state_quiet));
      end
  assign set_burst = (((state_idle & TXCOMWAKE) | (((TXCOMINIT & (state_idle & (((TXCOMWAKE | (clr_quiet & state_quiet)) | TXCOMINIT) & (state_idle | (TXCOMWAKE | (clr_quiet & state_quiet)))))) | ((state_idle & (TXCOMINIT | (state_idle & TXCOMWAKE))) & (clr_quiet & state_quiet))) | ~TXCOMFINISH)) & (state_quiet & (clr_quiet & ((state_idle & TXCOMWAKE) | (((TXCOMINIT & (state_idle & (((TXCOMWAKE | (clr_quiet & state_quiet)) | TXCOMINIT) & (state_idle | (TXCOMWAKE | (clr_quiet & state_quiet)))))) | ((state_idle & (TXCOMINIT | (state_idle & TXCOMWAKE))) & (clr_quiet & state_quiet))) | ~TXCOMFINISH))))) | (state_idle & (TXCOMINIT | (state_idle & TXCOMWAKE)));
  assign set_quiet = clr_burst & (state_burst & (bursts_cnt < ((-1)+bursts_cnt_togo)));
  assign clr_burst = (stopwatch & state_burst) == (burst_len-burst_len_mult);
  assign clr_quiet = (stopwatch & state_quiet) == ((-burst_len_mult)+quiet_len);
  assign quiet_len = issued_wake ? quiet_len_wake : quiet_len_init;
  assign stopwatch_clr = set_burst | (state_idle | ((TXCOMINIT & (state_idle & (((TXCOMWAKE | (clr_quiet & state_quiet)) | TXCOMINIT) & (state_idle | (TXCOMWAKE | (clr_quiet & state_quiet)))))) | set_quiet));
