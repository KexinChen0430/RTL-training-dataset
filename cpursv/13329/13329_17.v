...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= ((~reset & (~clr_burst & state_burst)) | set_burst) & (~clr_burst & ~reset);
        state_quiet <= (~reset & (~clr_quiet & state_quiet)) | ((~clr_quiet & set_quiet) & ~reset);
      end
  assign set_burst = (TXCOMINIT & (state_idle & (TXCOMINIT | (TXCOMWAKE | (state_quiet & clr_quiet))))) | ((((((TXCOMINIT | (state_quiet & clr_quiet)) & (state_quiet & clr_quiet)) | ((TXCOMINIT | (state_quiet & clr_quiet)) & state_idle)) | state_idle) & (((state_idle | ~TXCOMFINISH) & ((TXCOMINIT | TXCOMWAKE) & (state_idle | TXCOMWAKE))) | ~TXCOMFINISH)) & (TXCOMWAKE | (state_quiet & clr_quiet)));
  ...
  assign stopwatch_clr = state_idle | ((set_burst | set_quiet) | (TXCOMINIT & (state_idle & (TXCOMINIT | (TXCOMWAKE | (state_quiet & clr_quiet))))));
