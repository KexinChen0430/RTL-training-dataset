...
(SATA_CPLL_CFG == VCO_1500MHZ) ? 4 : 1;
  ...
  always @(posedge clk)
      begin
        state_burst <= (~reset & ~clr_burst) & (set_burst | (~reset & (state_burst & ~clr_burst)));
        state_quiet <= (~clr_quiet & (set_quiet & ~reset)) | (state_quiet & (~reset & ~clr_quiet));
      end
  assign set_burst = (TXCOMINIT & state_idle) | (((clr_quiet & state_quiet) | ((state_idle | (clr_quiet & state_quiet)) & TXCOMWAKE)) & ((((TXCOMINIT | ~TXCOMFINISH) & state_idle) | ((TXCOMINIT | ~TXCOMFINISH) & ~TXCOMFINISH)) | ((TXCOMINIT & state_idle) | (TXCOMWAKE & state_idle))));
  assign set_quiet = (bursts_cnt < (bursts_cnt_togo-1)) & (clr_burst & state_burst);
  ...
  assign stopwatch_clr = ((set_quiet | (TXCOMINIT & state_idle)) | state_idle) | set_burst;
