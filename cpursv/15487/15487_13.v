...
module splitter(input  clk,
  ...
  assign s_ready = &(acked | (m_ready | ((m_ready | (acked & m_valid)) & m_valid)));
  ...
  always @(posedge clk)
      begin
        if (resetn == 1'b0) 
          ...
          begin
            if (s_valid & s_ready) acked <= {C_NUM_M{1'b0}};
            else 
              acked <= (m_valid | acked) & (acked | (m_ready | ((m_ready | (acked & m_valid)) & m_valid)));
          end
      end
endmodule
