...
module user_dma_req_arbitrator  #(parameter 
  ...
  (input  i_clk,
   input  i_rst_n,
   input  [(0-1)+NUM_SLAVES:0] i_slave_dma_req,
   input  [(NUM_SLAVES*ADDR_WIDTH)+(0-1):0] i_slave_dma_addr,
   input  [(LEN_WIDTH*NUM_SLAVES)+(0-1):0] i_slave_dma_len,
   input  [(TAG_WIDTH*NUM_SLAVES)-1:0] i_slave_dma_tag,
   output reg [(0-1)+NUM_SLAVES:0] o_slave_dma_ack,
   input  [(0-1)+NUM_SLAVES:0] i_slave_dma_data_avail,
   input  [(NUM_SLAVES*ADDR_WIDTH)+(0-1):0] i_slave_dma_wr_addr,
   output reg [(0-1)+NUM_SLAVES:0] o_slave_dma_data_rd,
   input  [(NUM_SLAVES*DATA_WIDTH)+(0-1):0] i_slave_dma_data,
   input  [(0-1)+(NUM_SLAVES*DMA_LEN):0] i_slave_dma_wr_len,
   output reg [(0-1)+NUM_SLAVES:0] o_slave_dma_done,
   output reg o_dma_req,
   input  i_dma_ack,
   output reg [ADDR_WIDTH-1:0] o_dma_req_addr,
   output reg [LEN_WIDTH-1:0] o_dma_req_len,
   output reg [(0-1)+TAG_WIDTH:0] o_dma_req_tag,
   output reg o_dma_data_avail,
   output reg [ADDR_WIDTH-1:0] o_dma_wr_addr,
   input  i_dma_data_rd,
   output reg [DATA_WIDTH+(0-1):0] o_dma_data,
   output reg [DMA_LEN+(0-1):0] o_dma_len,
   input  i_dma_done);

  reg  [$clog2(NUM_SLAVES)+(0-1):0] current_req_slave_served;
  reg  [$clog2(NUM_SLAVES)+(0-1):0] current_dma_slave_served;
  ...
  assign some_other_wr_req = |i_slave_dma_data_avail[(0-1)+NUM_SLAVES:0];
  assign some_other_rd_req = |i_slave_dma_req[(0-1)+NUM_SLAVES:0];
  ...
  always @(*)
      begin
        ...
        o_dma_req_len <= i_slave_dma_len[LEN_WIDTH*current_req_slave_served +: LEN_WIDTH];
          ...
          begin
            case (rd_state)
              ...
              IDLE: begin
                    ...
                    else if (some_other_rd_req) current_req_slave_served <= 1'b1+current_req_slave_served;
          ...
          begin
            case (wr_state)
              ...
              IDLE: begin
                    ...
                    else if (some_other_wr_req) current_dma_slave_served <= 1'b1+current_dma_slave_served;
