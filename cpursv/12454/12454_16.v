...
module user_dma_req_arbitrator  #(parameter 
  ...
  (input  i_clk,
   ...
   input  [(NUM_SLAVES*LEN_WIDTH)-1:0] i_slave_dma_len,
   input  [(NUM_SLAVES*TAG_WIDTH)+(0-1):0] i_slave_dma_tag,
   ...
   input  [(0-1)+(DMA_LEN*NUM_SLAVES):0] i_slave_dma_wr_len,
   output reg [NUM_SLAVES-1:0] o_slave_dma_done,
   output reg o_dma_req,
   input  i_dma_ack,
   output reg [(0-1)+ADDR_WIDTH:0] o_dma_req_addr,
   output reg [LEN_WIDTH+(0-1):0] o_dma_req_len,
   output reg [TAG_WIDTH+(0-1):0] o_dma_req_tag,
   output reg o_dma_data_avail,
   output reg [(0-1)+ADDR_WIDTH:0] o_dma_wr_addr,
   input  i_dma_data_rd,
  ...
  always @(*)
      begin
        ...
        o_dma_req_addr <= i_slave_dma_addr[ADDR_WIDTH*current_req_slave_served +: ADDR_WIDTH];
        o_dma_req_len <= i_slave_dma_len[LEN_WIDTH*current_req_slave_served +: LEN_WIDTH];
        o_dma_req_tag <= i_slave_dma_tag[TAG_WIDTH*current_req_slave_served +: TAG_WIDTH];
