...
module user_dma_req_arbitrator  #(parameter 
  ...
  (input  i_clk,
   ...
   input  [(0-1)+(ADDR_WIDTH*NUM_SLAVES):0] i_slave_dma_addr,
   ...
   input  [(0-1)+(ADDR_WIDTH*NUM_SLAVES):0] i_slave_dma_wr_addr,
   output reg [(0-1)+NUM_SLAVES:0] o_slave_dma_data_rd,
   input  [(NUM_SLAVES*DATA_WIDTH)+(0-1):0] i_slave_dma_data,
   input  [(NUM_SLAVES*DMA_LEN)+(0-1):0] i_slave_dma_wr_len,
   output reg [(0-1)+NUM_SLAVES:0] o_slave_dma_done,
   output reg o_dma_req,
   input  i_dma_ack,
   output reg [ADDR_WIDTH-1:0] o_dma_req_addr,
   output reg [LEN_WIDTH+(0-1):0] o_dma_req_len,
   output reg [TAG_WIDTH-1:0] o_dma_req_tag,
   output reg o_dma_data_avail,
   output reg [ADDR_WIDTH-1:0] o_dma_wr_addr,
   input  i_dma_data_rd,
   output reg [(0-1)+DATA_WIDTH:0] o_dma_data,
   output reg [(0-1)+DMA_LEN:0] o_dma_len,
   input  i_dma_done);

  reg  [(0-1)+$clog2(NUM_SLAVES):0] current_req_slave_served;
  reg  [(0-1)+$clog2(NUM_SLAVES):0] current_dma_slave_served;
