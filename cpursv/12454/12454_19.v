...
module user_dma_req_arbitrator  #(parameter 
  ...
  (input  i_clk,
   ...
   input  [(-1)+(ADDR_WIDTH*NUM_SLAVES):0] i_slave_dma_addr,
   input  [(-1)+(NUM_SLAVES*LEN_WIDTH):0] i_slave_dma_len,
   ...
   input  [(-1)+(ADDR_WIDTH*NUM_SLAVES):0] i_slave_dma_wr_addr,
   output reg [NUM_SLAVES+(-1):0] o_slave_dma_data_rd,
   input  [(-1)+(NUM_SLAVES*DATA_WIDTH):0] i_slave_dma_data,
   input  [(-1)+(DMA_LEN*NUM_SLAVES):0] i_slave_dma_wr_len,
   output reg [NUM_SLAVES+(-1):0] o_slave_dma_done,
   output reg o_dma_req,
   input  i_dma_ack,
   output reg [ADDR_WIDTH-1:0] o_dma_req_addr,
   output reg [(-1)+LEN_WIDTH:0] o_dma_req_len,
   output reg [(-1)+TAG_WIDTH:0] o_dma_req_tag,
  ...
  reg  [(-1)+$clog2(NUM_SLAVES):0] current_req_slave_served;
  reg  [(-1)+$clog2(NUM_SLAVES):0] current_dma_slave_served;
