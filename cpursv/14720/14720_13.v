...
module template_periph_16b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  wire [DEC_WD-1:0]  reg_addr = {per_addr[(-1<<1)+DEC_WD:0],1'b0};
  wire [DEC_SZ+(0-1):0] 
       reg_dec = ((((CNTRL1_D & ({DEC_SZ{reg_addr == CNTRL1}} | CNTRL2_D)) | CNTRL2_D) & CNTRL1_D) & {DEC_SZ{reg_addr == CNTRL1}}) | ((((CNTRL3_D & {DEC_SZ{reg_addr == CNTRL3}}) | {DEC_SZ{reg_addr == CNTRL2}}) & ((CNTRL3_D & {DEC_SZ{reg_addr == CNTRL3}}) | CNTRL2_D)) | ({DEC_SZ{reg_addr == CNTRL4}} & CNTRL4_D));
  wire  reg_write = |(reg_sel & per_we);
  wire  reg_read = reg_sel & ~|per_we;
  wire [DEC_SZ+(0-1):0]  reg_wr = {DEC_SZ{reg_write}} & reg_dec;
