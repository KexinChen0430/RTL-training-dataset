...
module template_6x4_600x400(clk,hc,vc,matrix_x,matrix_y,lines);
  ...
  assign vc_template = ((vc <= CUADRILLA_YF) && (vc > CUADRILLA_YI)) ? (vc+(0-CUADRILLA_YI)) : 11'd0;
  
  always @(*)
      if (hc_template == 'd0) {col_next,matrix_x_next} = {d_col,3'd0};
      else if (hc_template > {col,zeros_col}) {col_next,matrix_x_next} = {col+d_col,matrix_x+3'd1};
      else {col_next,matrix_x_next} = {col,matrix_x};
  
  always @(*)
      if (vc_template == 'd0) {row_next,matrix_y_next} = {d_row,2'd0};
      else if (vc_template > {row,zeros_row}) {row_next,matrix_y_next} = {row+d_row,matrix_y+2'd1};
  ...
  always @(*)
      if ((hc == CUADRILLA_XF) || (vc == CUADRILLA_YF) || 
          ((hc == (CUADRILLA_XI+11'd1)) || 
(vc == (CUADRILLA_YI+11'd1)))) lines = 1'b1;
