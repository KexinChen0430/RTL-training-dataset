...
module template_6x4_600x400(clk,hc,vc,matrix_x,matrix_y,lines);
  ...
  assign vc_template = ((vc <= CUADRILLA_YF) && (vc > CUADRILLA_YI)) ? ((-CUADRILLA_YI)+vc) : 11'd0;
  
  always @(*)
      if (hc_template == 'd0) {col_next,matrix_x_next} = {d_col,3'd0};
      else if (hc_template > {col,zeros_col}) {col_next,matrix_x_next} = {d_col+col,3'd1+matrix_x};
  ...
  always @(*)
      if ((vc == (CUADRILLA_YI+11'd1)) || 
          ((hc == (CUADRILLA_XI+11'd1)) || 
((vc == CUADRILLA_YF) || (hc == CUADRILLA_XF)))) lines = 1'b1;
