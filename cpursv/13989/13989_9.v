        ...
        if (width > 1) 
          begin
            for (i = width-1<<<1; i >= 0; i = i-1)
                breg[i] = ((((~breg[1+i] & (gin[i] | breg[1+i])) | (~breg[1+i] | breg[1+i])) & gin[i]) & (~breg[1+i] | breg[1+i])) ^ ((~breg[1+i] | breg[1+i]) & (((gin[i] | ~gin[i]) & breg[1+i]) & ((~breg[1+i] & (gin[i] | breg[1+i])) | (~breg[1+i] | breg[1+i]))));
          end
