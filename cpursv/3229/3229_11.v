...
module i2s(clk_i,rst_i,pcm_data_i,pcm_fifo_empty_i,pcm_fifo_rd_o,
           pcm_fifo_ur_o,bclk_o,ws_o,data_o);
  ...
  always @(posedge clk_i or posedge rst_i)
      begin
        if (rst_i == 1'b1) 
          ...
          begin
            if (audio_clock_div == ((-1)+CLK_DIVISOR)) 
              begin
      ...
      begin
        if (rst_i == 1'b1) 
          ...
          begin
            ...
            if ((audio_clock == 1'b0) && (prev_audio_clock == 1'b1)) 
              begin
                bclk_o <= 1'b0;
                if (bit_count == 0) 
                  begin
                    data_o <= (word_sel == 1'b0) ? input_reg0[15] : input_reg1[15];
                    word_sel <= ~word_sel;
                  ...
                  begin
                    ...
                    bit_count <= (bit_count == 15) ? 0 : (1+bit_count);
                  end
              end
            else if ((audio_clock == 1'b1) && (prev_audio_clock == 1'b0)) 
              begin
                bclk_o <= 1'b1;
              end
