...
module regfile(output [15:0] a,
  ...
  assign b = (~addr_b[3] & b_byte) ? {{8{b8[7]}},b8} : r[addr_b];
      ...
      begin
        if (rst) 
          ...
          begin
            if (wr) 
              begin
                if ((addr_d[3:2] | word_op) == 2'b10) r[addr_d] <= word_op ? d[15:0] : {{8{d[7]}},d[7:0]};
                else if (~((addr_d[2] & ~((~addr_d[2] | addr_d[2]) & addr_d[3])) | (((~addr_d[2] | addr_d[2]) & addr_d[3]) & ~addr_d[2]))) r[addr_d][7:0] <= d[7:0];
