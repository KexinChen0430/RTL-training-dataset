      ...
      begin
        if (BUS_RD) 
          begin
            ...
            else if (((0-1)+BUS_ADD) < IO_BYTES) BUS_DATA_OUT <= INPUT_DATA[IO_BYTES+(0-BUS_ADD)];
            else if ((BUS_ADD-(IO_BYTES+1)) < IO_BYTES) BUS_DATA_OUT <= OUTPUT_DATA[(2*IO_BYTES)-BUS_ADD];
            else if ((BUS_ADD-(1+(2*IO_BYTES))) < IO_BYTES) BUS_DATA_OUT <= DIRECTION_DATA[(IO_BYTES*3)+(0-BUS_ADD)];
  ...
  always @(posedge BUS_CLK)
      begin
        if (RST) 
          ...
          begin
            if (((0-1)+BUS_ADD) < IO_BYTES) 
               ;
            else if ((BUS_ADD-(IO_BYTES+1)) < IO_BYTES) OUTPUT_DATA[(2*IO_BYTES)-BUS_ADD] <= BUS_DATA_IN;
            else if ((BUS_ADD-(1+(2*IO_BYTES))) < IO_BYTES) DIRECTION_DATA[(IO_BYTES*3)+(0-BUS_ADD)] <= BUS_DATA_IN;
