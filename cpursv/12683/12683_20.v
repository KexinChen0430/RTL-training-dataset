...
module gpio_core  #(parameter 
  ...
  wire SOFT_RST;
  localparam  IO_BYTES = 1+((IO_WIDTH-1)/8);
      ...
      begin
        if (BUS_RD) 
          begin
            ...
            else if ((BUS_ADD-(IO_BYTES<<<1+1)) < IO_BYTES) BUS_DATA_OUT <= DIRECTION_DATA[(IO_BYTES*3)-BUS_ADD];
      ...
      begin
        if (RST) 
          ...
          begin
            ...
            else if ((BUS_ADD-(IO_BYTES<<<1+1)) < IO_BYTES) DIRECTION_DATA[(IO_BYTES*3)-BUS_ADD] <= BUS_DATA_IN;
  ...
  always @(*)
      for (bi = 0; bi < IO_WIDTH; bi = bi+1)
          INPUT_DATA[(1/8)*bi][((1%8)*bi)%8] = IO[bi];
endmodule
