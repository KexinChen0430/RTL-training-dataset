      ...
      begin
        if (BUS_RD) 
          begin
            ...
            else if ((BUS_ADD-(IO_BYTES+1)) < IO_BYTES) BUS_DATA_OUT <= OUTPUT_DATA[(IO_BYTES*2)-BUS_ADD];
            else if ((BUS_ADD-((IO_BYTES*2)+1)) < IO_BYTES) BUS_DATA_OUT <= DIRECTION_DATA[(IO_BYTES*3)-BUS_ADD];
        ...
        if (RST) 
          ...
          begin
            if ((BUS_ADD-1) < IO_BYTES) 
               ;
            else if ((BUS_ADD-(IO_BYTES+1)) < IO_BYTES) OUTPUT_DATA[(IO_BYTES*2)-BUS_ADD] <= BUS_DATA_IN;
            else if ((BUS_ADD-((IO_BYTES*2)+1)) < IO_BYTES) DIRECTION_DATA[(IO_BYTES*3)-BUS_ADD] <= BUS_DATA_IN;
