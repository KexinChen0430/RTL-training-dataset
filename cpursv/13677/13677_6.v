...
module hardReset(output wire Start_tHardResetComplete_timer,
  ...
  assign Hard_Reset_Request = TRANSMIT[0] && TRANSMIT[2] && !TRANSMIT[1];
  assign Cable_Reset_Request = TRANSMIT[1] && TRANSMIT[2] && !TRANSMIT[0];
  assign Request_PHY_to_Send_Hard_Reset = Hard_Reset_Request && 
                                          (CurrentState == Construct_Message);
  assign Request_PHY_to_Send_Cable_Reset = (CurrentState == Construct_Message) && 
                                           Cable_Reset_Request;
