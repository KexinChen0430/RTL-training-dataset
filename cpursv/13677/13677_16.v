...
module hardReset(output wire Start_tHardResetComplete_timer,
  ...
  assign Hard_Reset_Request = !TRANSMIT[1] && TRANSMIT[2] && TRANSMIT[0];
  assign Cable_Reset_Request = TRANSMIT[2] && (!TRANSMIT[0] && TRANSMIT[1]);
  assign Request_PHY_to_Send_Hard_Reset = (CurrentState == Construct_Message) && 
                                          Hard_Reset_Request;
  assign Request_PHY_to_Send_Cable_Reset = Cable_Reset_Request && 
                                           (CurrentState == Construct_Message);
  ...
  always @(*)
      begin
        ...
        case (CurrentState)
          ...
          Wait_for_Reset_Request: begin
                if (Hard_Reset_Request || Cable_Reset_Request) 
                  begin
