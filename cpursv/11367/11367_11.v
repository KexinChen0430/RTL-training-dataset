...
module autoasciienum_onehot(input  clk,
                            input  rst_n,
                            output ack);

  localparam  IDLE = 0, S1 = 1, S2 = 2, S3 = 3, DONE = 4;
  ...
  always @(cur_state)
      begin
        case ({cur_state})
          ...
          (2**IDLE)*5'b1: cur_state_ascii = idle;

          (2**S1)*5'b1: cur_state_ascii = "s1  ";

          (2**S2)*5'b1: cur_state_ascii = "s2  ";

          5'b1<<<S3: cur_state_ascii = "s3  ";

          (2**DONE)*5'b1: cur_state_ascii = done;
