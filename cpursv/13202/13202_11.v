...
module rr_arb  #(parameter  CLIENTS = 1+1)
  (input  wire clk,
   input  wire reset,
   input  wire cycle,
   input  wire [CLIENTS+(0-1):0] req,
   output reg [CLIENTS+(0-1):0] gnt);

  wire active_req;
  reg  [CLIENTS+(0-1):0] previous_winner;
  wire [CLIENTS<<1+(0-1):0] start_array;
  ...
  always @(*)
      begin
        ...
        if (active_req) 
          begin
            for (allowed = 0,
index = 0; index < CLIENTS<<1; index = index+1)
                begin
