...
module vga_sync(input  wire clk,
  ...
  assign line_pulse = pix_pulse && 
                      (q_hcnt == ((H_BP+(H_FP+(H_DISP+H_RT)))-1));
  assign screen_pulse = line_pulse && 
                        (q_vcnt == ((-1)+((V_BP+V_FP)+(V_RT+V_DISP))));
  assign hsync = q_hsync;
  assign vsync = q_vsync;
  assign x = q_hcnt;
  assign y = q_vcnt;
  assign x_next = d_hcnt;
  assign y_next = (y == ((V_RT+V_BP)+((V_DISP+(0-10'h001))+V_FP))) ? 10'h000 : (q_vcnt+10'h001);
  assign en = q_en;
        ...
        if (rst) 
          ...
          begin
            ...
            d_hcnt = line_pulse ? 10'h000 : 
                     pix_pulse ? (10'h001+q_hcnt) : q_hcnt;
            d_vcnt = screen_pulse ? 10'h000 : 
                     line_pulse ? (q_vcnt+10'h001) : q_vcnt;
            d_hsync = (q_hcnt >= (H_DISP+H_FP)) && 
                      (q_hcnt < (H_FP+(H_DISP+H_RT)));
            d_vsync = (q_vcnt < ((V_DISP+V_FP)+V_RT)) && 
                      (q_vcnt >= (V_DISP+V_FP));
