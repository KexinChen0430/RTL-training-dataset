...
module vga_sync(input  wire clk,
  ...
  assign line_pulse = pix_pulse && 
                      (q_hcnt == ((H_BP+((H_FP+H_RT)+H_DISP))+(0-1)));
  assign screen_pulse = line_pulse && 
                        (q_vcnt == (((0-1)+((V_BP+V_FP)+V_DISP))+V_RT));
  assign hsync = q_hsync;
  assign vsync = q_vsync;
  assign x = q_hcnt;
  assign y = q_vcnt;
  assign x_next = d_hcnt;
  assign y_next = (y == (V_FP+((V_BP+(0-10'h001))+(V_RT+V_DISP)))) ? 10'h000 : (10'h001+q_vcnt);
  assign en = q_en;
      ...
      begin
        if (rst) 
          ...
          begin
            ...
            d_hsync = (q_hcnt < ((H_FP+H_RT)+H_DISP)) && 
                      (q_hcnt >= (H_DISP+H_FP));
            d_vsync = (q_vcnt < (V_RT+(V_FP+V_DISP))) && 
                      (q_vcnt >= (V_FP+V_DISP));
