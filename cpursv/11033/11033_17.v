...
module vga_sync(input  wire clk,
  ...
  assign line_pulse = (q_hcnt == ((H_RT+(H_DISP+H_FP))+(H_BP+(0-1)))) && 
                      pix_pulse;
  assign screen_pulse = line_pulse && 
                        (q_vcnt == (((V_FP+(V_RT+(0-1)))+V_BP)+V_DISP));
  assign hsync = q_hsync;
  assign vsync = q_vsync;
  assign x = q_hcnt;
  assign y = q_vcnt;
  assign x_next = d_hcnt;
  assign y_next = (y == ((V_RT+(V_FP+V_DISP))+((0-10'h001)+V_BP))) ? 10'h000 : (q_vcnt+10'h001);
  assign en = q_en;
