...
module vga_sync(input  wire clk,
  ...
  assign line_pulse = (q_hcnt == ((H_RT+(0-1))+(H_DISP+(H_FP+H_BP)))) && 
                      pix_pulse;
  assign screen_pulse = line_pulse && 
                        (q_vcnt == (V_FP+((V_RT+(0-1))+(V_DISP+V_BP))));
  assign hsync = q_hsync;
  assign vsync = q_vsync;
  assign x = q_hcnt;
  assign y = q_vcnt;
  assign x_next = d_hcnt;
  assign y_next = (y == (((V_RT+V_DISP)+V_FP)+((0-10'h001)+V_BP))) ? 10'h000 : (10'h001+q_vcnt);
  assign en = q_en;
      ...
      begin
        if (rst) 
          ...
          begin
            ...
            d_vsync = (q_vcnt < ((V_RT+V_DISP)+V_FP)) && 
                      (q_vcnt >= (V_DISP+V_FP));
