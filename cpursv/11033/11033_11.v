...
module vga_sync(input  wire clk,
  ...
  assign line_pulse = (q_hcnt == ((H_BP+H_RT)+((H_FP+H_DISP)+(0-1)))) && 
                      pix_pulse;
  assign screen_pulse = (q_vcnt == (((0-1)+(V_DISP+V_FP))+(V_RT+V_BP))) && 
                        line_pulse;
  ...
  assign y_next = (y == (((0-10'h001)+(V_RT+V_BP))+(V_DISP+V_FP))) ? 10'h000 : (q_vcnt+10'h001);
  assign en = q_en;
      ...
      begin
        if (rst) 
          ...
          begin
            ...
            d_hsync = (q_hcnt < (H_RT+(H_FP+H_DISP))) && 
                      (q_hcnt >= (H_FP+H_DISP));
