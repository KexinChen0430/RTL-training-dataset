...
module vga_sync(input  wire clk,
  ...
  assign line_pulse = (q_hcnt == (((H_FP+H_BP)+H_RT)+((0-1)+H_DISP))) && 
                      pix_pulse;
  assign screen_pulse = line_pulse && 
                        (q_vcnt == ((V_BP+(V_RT+V_FP))+(V_DISP+(0-1))));
  assign hsync = q_hsync;
  assign vsync = q_vsync;
  assign x = q_hcnt;
  assign y = q_vcnt;
  assign x_next = d_hcnt;
  assign y_next = (y == ((V_DISP+(-10'h001))+(V_BP+(V_RT+V_FP)))) ? 10'h000 : (10'h001+q_vcnt);
  assign en = q_en;
  ...
  always @(*)
      begin
        if (rst) 
          ...
          begin
            d_mod4_cnt = 2'h1+q_mod4_cnt;
            d_hcnt = line_pulse ? 10'h000 : 
                     pix_pulse ? (q_hcnt+10'h001) : q_hcnt;
            d_vcnt = screen_pulse ? 10'h000 : 
                     line_pulse ? (10'h001+q_vcnt) : q_vcnt;
            d_hsync = (q_hcnt >= (H_DISP+H_FP)) && 
                      (q_hcnt < (H_FP+(H_RT+H_DISP)));
            d_vsync = (q_vcnt >= (V_DISP+V_FP)) && 
                      (q_vcnt < ((V_DISP+V_FP)+V_RT));
            d_en = (q_vcnt < V_DISP) && (q_hcnt < H_DISP);
          end
      end
endmodule
