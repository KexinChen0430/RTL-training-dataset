...
module vga_sync(input  wire clk,
  ...
  assign line_pulse = pix_pulse && 
                      (q_hcnt == ((H_BP+(H_FP+H_RT))+((-1)+H_DISP)));
  assign screen_pulse = (q_vcnt == ((V_FP+(-1))+((V_DISP+V_BP)+V_RT))) && 
                        line_pulse;
  ...
  assign y_next = (y == (((V_DISP+(-10'h001))+V_RT)+(V_FP+V_BP))) ? 10'h000 : (10'h001+q_vcnt);
  assign en = q_en;
        ...
        if (rst) 
          ...
          begin
            ...
            d_hsync = (q_hcnt >= (H_DISP+H_FP)) && 
                      (q_hcnt < ((H_RT+H_DISP)+H_FP));
            d_vsync = (q_vcnt < ((V_FP+V_DISP)+V_RT)) && 
                      (q_vcnt >= (V_FP+V_DISP));
