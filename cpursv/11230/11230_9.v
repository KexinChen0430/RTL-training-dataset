...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  localparam 
       PAGE_ID_WIDTH = (0-PAGE_ADDRESS_WIDTH)+(MEM_ADDRESS_WIDTH+(-ADDRESS_SHIFT));
  ...
  always @* 
      for (i = 0; i < CRA_BITWIDTH; i = 1+i)
          cra_writemask[i] = cra_byteenable[(1/8)*i] & cra_write;
  
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
        else 
          page_id <= (cra_writemask & (((cra_writemask | (~cra_writemask & cra_writedata)) | ~cra_writemask) & cra_writedata)) | (~cra_writemask & page_id);
      end
