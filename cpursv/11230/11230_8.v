...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  localparam 
       PAGE_ID_WIDTH = (-PAGE_ADDRESS_WIDTH)+(MEM_ADDRESS_WIDTH+(0-ADDRESS_SHIFT));
  ...
  input  [NUM_BYTES+(0-1):0] s1_byteenable;
  ...
  output [NUM_BYTES+(0-1):0] m1_byteenable;
  input  m1_waitrequest;
  input  cra_write;
  input  [CRA_BITWIDTH-1:0] cra_writedata;
  input  [(CRA_BITWIDTH/8)+(0-1):0] cra_byteenable;
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
          ...
          page_id <= (cra_writedata | ((page_id | (page_id & ~cra_writemask)) | (cra_writemask & (~cra_writemask & cra_writedata)))) & ((cra_writemask & (((page_id & (cra_writemask & cra_writedata)) | ~cra_writemask) | (cra_writedata & ((cra_writedata & (page_id & ~cra_writemask)) | (cra_writemask & cra_writedata))))) | ((page_id & ~cra_writemask) | (page_id & (cra_writemask & cra_writedata))));
      end
