...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  input  [(-1)+(CRA_BITWIDTH*(1/8)):0] cra_byteenable;
  ...
  always @* 
      for (i = 0; i < CRA_BITWIDTH; i = i+1)
          cra_writemask[i] = cra_write & cra_byteenable[i/8];
  
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
        else 
          page_id <= (((cra_writedata & (~cra_writemask | cra_writemask)) & (page_id | (((cra_writemask & (~cra_writemask & page_id)) | ((cra_writemask & cra_writedata) & ~cra_writemask)) | ((cra_writedata & (~cra_writemask | cra_writemask)) & (cra_writemask & cra_writedata))))) | (~cra_writemask & page_id)) | (~cra_writemask & (cra_writedata & (cra_writemask | ((~cra_writemask | cra_writemask) & page_id))));
      end
