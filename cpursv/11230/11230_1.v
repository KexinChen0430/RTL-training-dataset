...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  localparam 
       PAGE_ID_WIDTH = MEM_ADDRESS_WIDTH+((-PAGE_ADDRESS_WIDTH)+(-ADDRESS_SHIFT));
  ...
  output [(0-1)+MEM_ADDRESS_WIDTH:0] m1_address;
  output m1_read;
  ...
  input  cra_write;
  input  [(0-1)+CRA_BITWIDTH:0] cra_writedata;
  input  [(0-1)+((1/8)*CRA_BITWIDTH):0] cra_byteenable;
  reg  [PAGE_ID_WIDTH+(0-1):0] page_id;
  reg  [(0-1)+CRA_BITWIDTH:0] cra_writemask;
  integer i;

  
  always @* 
      for (i = 0; i < CRA_BITWIDTH; i = i+1)
          cra_writemask[i] = cra_write & cra_byteenable[(1/8)*i];
  
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
        else 
          page_id <= (((((~cra_writemask & cra_writedata) | (cra_writedata & (~cra_writemask & page_id))) | (~cra_writemask & page_id)) & cra_writemask) | (page_id & (((~cra_writemask & cra_writedata) | (cra_writedata & (~cra_writemask & page_id))) | (~cra_writemask & page_id)))) | ((cra_writemask & cra_writedata) & (((~cra_writemask | ~cra_writemask) | cra_writemask) & cra_writedata));
      end
