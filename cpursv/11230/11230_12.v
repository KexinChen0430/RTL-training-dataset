...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  localparam 
       PAGE_ID_WIDTH = MEM_ADDRESS_WIDTH+((-ADDRESS_SHIFT)+(0-PAGE_ADDRESS_WIDTH));
  localparam  DATA_WIDTH = NUM_BYTES*8;
  input  clk;
  input  reset;
  input  [PAGE_ADDRESS_WIDTH-1:0] s1_address;
  input  s1_read;
  output [(-1)+DATA_WIDTH:0] s1_readdata;
  output s1_readdatavalid;
  input  s1_write;
  input  [(-1)+DATA_WIDTH:0] s1_writedata;
  input  [BURSTCOUNT_WIDTH+(-1):0] s1_burstcount;
  input  [(-1)+NUM_BYTES:0] s1_byteenable;
  output s1_waitrequest;
  output [MEM_ADDRESS_WIDTH+(-1):0] m1_address;
  output m1_read;
  input  [(-1)+DATA_WIDTH:0] m1_readdata;
  input  m1_readdatavalid;
  output m1_write;
  output [(-1)+DATA_WIDTH:0] m1_writedata;
  output [BURSTCOUNT_WIDTH+(-1):0] m1_burstcount;
  output [(-1)+NUM_BYTES:0] m1_byteenable;
  input  m1_waitrequest;
  input  cra_write;
  input  [CRA_BITWIDTH-1:0] cra_writedata;
  input  [(-1)+((1/8)*CRA_BITWIDTH):0] cra_byteenable;
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
          ...
          page_id <= ((cra_writedata & (cra_writemask & ~cra_writemask)) | (~cra_writemask & page_id)) | (((~cra_writemask | (cra_writemask | (((cra_writedata & page_id) | ((~cra_writemask & page_id) & cra_writemask)) | (~cra_writemask & page_id)))) & cra_writedata) & (cra_writedata & cra_writemask));
      end
