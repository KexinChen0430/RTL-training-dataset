...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  localparam 
       PAGE_ID_WIDTH = ((0-ADDRESS_SHIFT)+(-PAGE_ADDRESS_WIDTH))+MEM_ADDRESS_WIDTH;
  localparam  DATA_WIDTH = 8*NUM_BYTES;
  input  clk;
  input  reset;
  input  [(-1)+PAGE_ADDRESS_WIDTH:0] s1_address;
  input  s1_read;
  ...
  input  [((1/8)*CRA_BITWIDTH)+(-1):0] cra_byteenable;
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
          ...
          page_id <= ((~cra_writemask & (cra_writedata | page_id)) & (cra_writemask | ((~cra_writemask & page_id) | ((cra_writemask & cra_writedata) & page_id)))) | ((cra_writemask & (cra_writedata | page_id)) & ((cra_writedata & (~cra_writemask | cra_writemask)) | ~cra_writemask));
      end
