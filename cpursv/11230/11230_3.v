...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  localparam 
       PAGE_ID_WIDTH = MEM_ADDRESS_WIDTH+((0-ADDRESS_SHIFT)+(0-PAGE_ADDRESS_WIDTH));
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
          ...
          page_id <= ((cra_writemask | (page_id & (~cra_writemask | cra_writemask))) & (cra_writedata & (~cra_writemask | cra_writemask))) | (((cra_writedata & ~cra_writemask) & cra_writemask) | (page_id & ~cra_writemask));
      end
