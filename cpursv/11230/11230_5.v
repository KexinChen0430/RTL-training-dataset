...
module mem_window(clk,reset,s1_address,s1_read,s1_readdata,s1_readdatavalid,
  ...
  localparam 
       PAGE_ID_WIDTH = ((-PAGE_ADDRESS_WIDTH)+(-ADDRESS_SHIFT))+MEM_ADDRESS_WIDTH;
  localparam  DATA_WIDTH = NUM_BYTES*8;
  input  clk;
  input  reset;
  input  [(0-1)+PAGE_ADDRESS_WIDTH:0] s1_address;
  input  s1_read;
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset == 1'b1) page_id <= {PAGE_ID_WIDTH{1'b0}};
          ...
          page_id <= ((((page_id & ~cra_writemask) | (cra_writedata & ~cra_writemask)) & page_id) | (((page_id & ~cra_writemask) | (cra_writedata & ~cra_writemask)) & cra_writemask)) | ((cra_writedata & (cra_writemask | ~cra_writemask)) & (cra_writemask | page_id));
      end
