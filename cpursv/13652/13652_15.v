...
module adapter_block_fifo_2_axi_stream  #(parameter 
       DATA_WIDTH   = 24,
  ...
  (input  rst,
   ...
   input  [(0-1)+(1+DATA_WIDTH):0] i_block_fifo_data,
   output o_block_fifo_stb,
   input  [3:0] i_axi_user,
   input  i_axi_clk,
   output [3:0] o_axi_user,
   input  i_axi_ready,
   output [(0-1)+DATA_WIDTH:0] o_axi_data,
   output o_axi_last,
  ...
  localparam  IDLE = 0;
  localparam  READY = 1;
  localparam  RELEASE = 1<<1;
  reg  [3:0] state;
  reg  [23:0] r_count;

  assign o_axi_data = i_block_fifo_data[(0-1)+DATA_WIDTH:0];
  assign o_block_fifo_stb = o_axi_valid & i_axi_ready;
  if (USER_IN_DATA) 
    begin
  ...
  assign o_axi_last = ((r_count+1) >= i_block_fifo_size) & (o_axi_valid & o_block_fifo_act);
        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              IDLE: begin
                    o_block_fifo_act <= 0;
                    if (i_block_fifo_rdy && !o_block_fifo_act) 
                      begin
                        r_count <= 0;
                        o_block_fifo_act <= 1;
                        state <= READY;
              ...
              READY: begin
                    if (r_count < i_block_fifo_size) 
                      begin
                        o_axi_valid <= 1;
                        if (i_axi_ready && o_axi_valid) 
                          begin
                            r_count <= r_count+1;
                            if ((r_count+1) >= i_block_fifo_size) 
                              begin
                                o_axi_valid <= 0;
                              end
