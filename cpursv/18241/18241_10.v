...
module llr_buffer(clk,nrst,reset,din,dout,wr,rd,empty,full);
  ...
  assign empty = ((waddr[ADDRW] ^~ raddr[ADDRW]) & ((waddr[ADDRW] & ~raddr[ADDRW]) | (waddr_i == raddr_i))) & (waddr_i == raddr_i);
  assign full = ((~raddr[ADDRW] & (waddr_i == raddr_i)) & waddr[ADDRW]) | (((waddr_i == raddr_i) & raddr[ADDRW]) & ~waddr[ADDRW]);
