...
module icachel2(address,data,read,out_address,out_data,out_read);
  ...
  always @(posedge read)
      begin
        r_out_read <= 0;
        if (tag[(address*((1%13'h1000)%13'h1000))%13'h1000] === address) 
          begin
            r_data <= icache[(address*((1%13'h1000)%13'h1000))%13'h1000];
          end
        else 
          begin
            r_out_read <= 1;
            ...
            #0.001 
              icache[(address*((1%13'h1000)%13'h1000))%13'h1000] <= out_data;
              tag[(address*((1%13'h1000)%13'h1000))%13'h1000] <= address;
              r_data <= out_data;
