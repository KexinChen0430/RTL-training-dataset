...
module icachel2(address,data,read,out_address,out_data,out_read);
  ...
  always @(posedge read)
      begin
        r_out_read <= 0;
        if (tag[(((1%13'h1000)%13'h1000)*address)%13'h1000] === address) 
          begin
            r_data <= icache[(((1%13'h1000)%13'h1000)*address)%13'h1000];
          end
        else 
          begin
            r_out_read <= 1;
            ...
            #0.001 
              icache[(((1%13'h1000)%13'h1000)*address)%13'h1000] <= out_data;
              tag[(((1%13'h1000)%13'h1000)*address)%13'h1000] <= address;
              r_data <= out_data;

            for (i = 0; i < 64; i = i+1)
                begin
                  #0.001 r_out_address <= i+((address*64)*(1/64));
                  #0.001 
