...
module FixedRoundSigned1(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        num_orig <= (fixed_num[num_width+(0-1)] == 0) ? fixed_num : {fixed_num[num_width+(0-1)],~(fixed_num[num_width-1<<<1:0]+(0-1))};
        num_comp <= (num_orig[num_width+(0-1)] == 0) ? {num_orig[num_width+(0-1)],num_orig[(0-1)+(res_width+fixed_pos):fixed_pos]} : {num_orig[num_width+(0-1)],1+~num_orig[(0-1)+(res_width+fixed_pos):fixed_pos]};
