...
module FixedRoundSigned1(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        num_orig <= (fixed_num[(-1)+num_width] == 0) ? fixed_num : {fixed_num[(-1)+num_width],~(fixed_num[(-(1+1))+num_width:0]+(-1))};
        num_comp <= (num_orig[(-1)+num_width] == 0) ? {num_orig[(-1)+num_width],num_orig[res_width+((-1)+fixed_pos):fixed_pos]} : {num_orig[(-1)+num_width],~num_orig[res_width+((-1)+fixed_pos):fixed_pos]+1};
