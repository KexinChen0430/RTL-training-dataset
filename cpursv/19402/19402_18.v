...
module FixedRoundSigned1(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        num_orig <= (fixed_num[(-1)+num_width] == 0) ? fixed_num : {fixed_num[(-1)+num_width],~(fixed_num[num_width+(0-1<<1):0]-1)};
        ...
        reg_overflow <= (num_orig[num_width+(0-1<<1):res_width+fixed_pos] == 0) ? 0 : 1;
