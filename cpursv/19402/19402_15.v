...
module FixedRoundSigned1(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        ...
        num_comp <= (num_orig[(-1)+num_width] == 0) ? {num_orig[(-1)+num_width],num_orig[(fixed_pos+res_width)-1:fixed_pos]} : {num_orig[(-1)+num_width],~num_orig[(fixed_pos+res_width)-1:fixed_pos]+1};
