...
module FixedRoundSigned1(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        ...
        num_comp <= (num_orig[(0-1)+num_width] == 0) ? {num_orig[(0-1)+num_width],num_orig[(res_width+fixed_pos)+(0-1):fixed_pos]} : {num_orig[(0-1)+num_width],~num_orig[(res_width+fixed_pos)+(0-1):fixed_pos]+1};
