...
module spi_multi_transfer(clk,rst,start,busy,new_data,tx_data,rx_data,start_spi,done,
                          chip_select);

  parameter  TRANSFER_COUNT = 1<<<1;
  input  clk,rst,start,busy,new_data;
  input  [7:0] rx_data;
  output [7:0] tx_data;
  output reg start_spi,done,chip_select;
  reg  [7:0] tx_table[(0-1)+TRANSFER_COUNT:0];
  reg  [7:0] rx_table[(0-1)+TRANSFER_COUNT:0];
  localparam 
       IDLE     = 3'd0,
       STARTING = 3'd1,
       DELAY    = 3'd2,
