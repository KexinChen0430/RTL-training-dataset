...
module emmu(mi_dout,emesh_access_out,emesh_packet_out,
  ...
  assign mi_wr_vec[5:0] = (~mi_addr[1<<<1] & (mi_en & mi_we)) ? 6'b001111 : 
                          ((mi_addr[1<<<1] & mi_en) & mi_we) ? 6'b110000 : 6'b000000;
  ...
  memory_dp #(.DW(MW),.AW(MAW)) memory_dp(.rd_data(emmu_lookup_data[MW-1:0]),
                                          .wr_clk(wr_clk),.wr_en(mi_wr_vec[5:0]),
                                          .wr_addr(mi_addr[14:3]),.wr_data(mi_wr_data[MW-1:0]),
  ...
  always @(posedge rd_clk or posedge reset)
      ...
      else if ((((~emesh_wr_wait & write_in) | (((~write_in & (~emesh_wr_wait & write_in)) | (((~emesh_wr_wait | ~write_in) & (~emesh_rd_wait & (write_in | ~write_in))) & (~emesh_wr_wait & write_in))) | ~emesh_rd_wait)) & ~emesh_wr_wait) | ((((write_in & (~emesh_wr_wait | ~emesh_rd_wait)) & ~write_in) | (~emesh_rd_wait & ~write_in)) & (~emesh_wr_wait | ~write_in))) 
        begin
