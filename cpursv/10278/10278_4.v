...
module tlu_model(input  wire SYS_CLK,SYS_RST,TLU_CLOCK,TLU_BUSY,ENABLE,
  ...
  always @(posedge SYS_CLK)
      begin
        if (SYS_RST) TRIG <= 0;
        else if (!VETO && ((($random(seed)%100) == 10) && ENABLE)) TRIG <= 1;
        else TRIG <= 0;
      end
...
(
((state == WAIT_STATE) || 
((state != WAIT_STATE) && (TLU_CLOCK == 1'b1))) && (TLU_CLOCK == 1'b1)) || (state != WAIT_STATE);
  reg  [15:0] TRIG_ID_SR;
  ...
  assign TLU_TRIGGER = (TLU_BUSY & (TRIG_ID_SR[0] | ((state == TRIG_STATE) & TLU_BUSY))) | (state == TRIG_STATE);
  assign TLU_RESET = 0;
endmodule
