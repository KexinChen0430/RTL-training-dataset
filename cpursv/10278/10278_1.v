...
module tlu_model(input  wire SYS_CLK,SYS_RST,TLU_CLOCK,TLU_BUSY,ENABLE,
  ...
  always @(posedge SYS_CLK)
      begin
        if (SYS_RST) TRIG <= 0;
        else if (ENABLE && ((($random(seed)%100) == 10) && !VETO)) TRIG <= 1;
        else TRIG <= 0;
      end
  ...
  always @(posedge SYS_CLK)
      begin
        ...
        else if (TRIG) TRIG_ID <= TRIG_ID+1;
  ...
  assign VETO = ((state != WAIT_STATE) || (state == WAIT_STATE)) && 
                ((TLU_CLOCK == 1'b1) || (state != WAIT_STATE));
  reg  [15:0] TRIG_ID_SR;
  ...
  assign TLU_TRIGGER = (state == TRIG_STATE) | (TLU_BUSY & TRIG_ID_SR[0]);
  assign TLU_RESET = 0;
endmodule
