...
module packet_generator(input  clk,
  ...
  reg  [31:0] crc_out;
  wire 
       calc_crc = ~(state[31:1<<1] == 30'h3FFF_FFFF) & (dst_rdy_i & src_rdy_o);

  
  always @(posedge clk)
      if (clear | reset) seq <= 0;
      else if (dst_rdy_i & (eof_o & src_rdy_o)) seq <= 1+seq;
        
  
  always @(posedge clk)
      if (clear | reset) state <= 0;
      else if (dst_rdy_i & src_rdy_o) 
        if (state == ((-1)+len)) state <= 32'hFFFF_FFFC;
        else state <= 1+state;
  ...
  wire  clear_crc = dst_rdy_i & (eof_o & src_rdy_o);
