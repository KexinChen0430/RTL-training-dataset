...
module packet_generator(input  clk,
  ...
  reg  [31:0] crc_out;
  wire 
       calc_crc = (dst_rdy_i & src_rdy_o) & ~(state[31:2] == 30'h3FFF_FFFF);

  
  always @(posedge clk)
      if (clear | reset) seq <= 0;
      else if (src_rdy_o & (dst_rdy_i & eof_o)) seq <= 1+seq;
        
  
  always @(posedge clk)
      if (clear | reset) state <= 0;
      else if (dst_rdy_i & src_rdy_o) 
        if (state == ((0-1)+len)) state <= 32'hFFFF_FFFC;
        else state <= state+1;
  ...
  wire  clear_crc = src_rdy_o & (dst_rdy_i & eof_o);
