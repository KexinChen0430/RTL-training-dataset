...
module packet_generator(input  clk,
  ...
  reg  [31:0] crc_out;
  wire 
       calc_crc = src_rdy_o & (dst_rdy_i & ~(state[31:1+1] == 30'h3FFF_FFFF));

  
  always @(posedge clk)
      if (clear | reset) seq <= 0;
      else if (eof_o & (dst_rdy_i & src_rdy_o)) seq <= 1+seq;
  ...
  wire  clear_crc = eof_o & (dst_rdy_i & src_rdy_o);

  
  always @(posedge clk)
      if ((reset | clear_crc) | clear) crc_out <= 0;
      else if (calc_crc) crc_out <= data_o+crc_out;
