...
module altera_tse_fake_master(input  clk,
                              input  reset,
        ...
        if (reset) 
          ...
          begin
            if (sd_loopback_r1 != sd_loopback_r2) 
              ...
              begin
                if ((next_state == IDLE) && (state == WRITE_DATA) && 
                    (phy_mgmt_writedata[0] == sd_loopback)) 
                  begin
                    bit_event <= 1'b0;
                  end
  ...
  always @(*)
      begin
        case (state)
          ...
          IDLE: begin
                if (bit_event) next_state = WRITE_DATA;
                else next_state = IDLE;
              end

          WRITE_DATA: begin
                if (!phy_mgmt_waitrequest) next_state = IDLE;
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset) 
          ...
          begin
            if ((next_state == WRITE_DATA) && (state == IDLE)) 
              begin
                phy_mgmt_writedata <= {31'b0,sd_loopback};
              end
            else if ((next_state == IDLE) && (state == WRITE_DATA)) 
              begin
                phy_mgmt_writedata <= 32'b0;
              end
