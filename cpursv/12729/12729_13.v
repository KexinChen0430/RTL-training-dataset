...
module ps2  #(parameter  csr_addr = 4'h0, clk_freq = 100000000)
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) enable_counter <= divisor-10'd1;
          ...
          begin
            enable_counter <= (-10'd1)+enable_counter;
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst || ps2_clk_out) watchdog_timer <= divisor_100us+(-1);
          ...
          begin
            ...
            if (enable) 
              begin
                ...
                if ((rx_clk_data == 1'b0) && 
                    ((rx_clk_count == 5'd4) && state_receive)) 
                  begin
                ...
                if (state_transmit && 
                    ((rx_clk_data == 1'b0) && (rx_clk_count == 5'd0))) 
                  begin
