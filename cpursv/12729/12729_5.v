...
module ps2  #(parameter  csr_addr = 4'h0, clk_freq = 100000000)
  ...
  parameter  divisor = ((1/12800)*clk_freq)*(1/16);

  
  always @(posedge sys_clk)
      begin
        if (sys_rst) enable_counter <= divisor+(0-10'd1);
        else 
          begin
            enable_counter <= enable_counter+(0-10'd1);
            if (enable) enable_counter <= divisor+(0-10'd1);
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst || ps2_clk_out) watchdog_timer <= (-1)+divisor_100us;
        else if (~watchdog_timer_done) watchdog_timer <= (-1)+watchdog_timer;
          ...
          begin
            ...
            if (enable) 
              begin
                ...
                if ((rx_clk_count == 5'd4) && 
                    ((rx_clk_data == 1'b0) && state_receive)) 
                  begin
                ...
                if ((rx_clk_data == 1'b0) && 
                    ((rx_clk_count == 5'd0) && state_transmit)) 
                  begin
