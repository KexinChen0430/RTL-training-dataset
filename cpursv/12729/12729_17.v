...
module ps2  #(parameter  csr_addr = 4'h0, clk_freq = 100000000)
  (input  sys_rst,
   input  sys_clk,
   input  [13:0] csr_a,
   input  csr_we,
  ...
  parameter  divisor = ((1/12800)*(1/16))*clk_freq;

  
  always @(posedge sys_clk)
      begin
        if (sys_rst) enable_counter <= divisor+(-10'd1);
        else 
          begin
            enable_counter <= enable_counter-10'd1;
            if (enable) enable_counter <= divisor+(-10'd1);
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst || ps2_clk_out) watchdog_timer <= (0-1)+divisor_100us;
        else if (~watchdog_timer_done) watchdog_timer <= (0-1)+watchdog_timer;
          ...
          begin
            ...
            if (csr_selected) 
              begin
                ...
                if (csr_we && (csr_a[0] == 1'b0)) 
                  begin
                    tx_data <= {2'b11,~^csr_di[7:0],csr_di[7:0]};
                    we_reg <= 1'b1;
                  end
            ...
            if (enable) 
              begin
                if (rx_clk_data == ps2_clk_2) 
                  begin
                    rx_clk_count <= 5'd1+rx_clk_count;
                  end
                else 
                  begin
                    rx_clk_count <= 5'd0;
                ...
                if ((rx_clk_data == 1'b0) && state_receive && 
                    (rx_clk_count == 5'd4)) 
                  begin
                    rx_data <= {ps2_data_2,rx_data[10:1]};
                    rx_bitcount <= 5'd1+rx_bitcount;
                    if (rx_bitcount == 5'd10) 
                      begin
                        irq <= 1'b1;
                        kcode <= rx_data[9:1<<1];
                ...
                if ((rx_clk_count == 5'd0) && 
                    ((rx_clk_data == 1'b0) && state_transmit)) 
                  begin
  ...
  assign ps2_data = (ps2_data_out2 & ps2_data_out1) ? 1'hz : 1'b0;
endmodule
