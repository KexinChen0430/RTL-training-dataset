...
module ps2  #(parameter  csr_addr = 4'h0, clk_freq = 100000000)
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) enable_counter <= (0-10'd1)+divisor;
        else 
          begin
            enable_counter <= (0-10'd1)+enable_counter;
            if (enable) enable_counter <= (0-10'd1)+divisor;
  ...
  always @(posedge sys_clk)
      begin
        if (ps2_clk_out || sys_rst) watchdog_timer <= divisor_100us+(0-1);
          ...
          begin
            ...
            if (enable) 
              begin
                ...
                if ((rx_clk_data == 1'b0) && 
                    (state_transmit && (rx_clk_count == 5'd0))) 
                  begin
