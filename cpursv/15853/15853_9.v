...
module MemMux(input  MEM_MUX_CLK,
  ...
  assign MEMORY_DATA = (IS_SELECTED_PORTA && !MEMORY_CE && MEMORY_OE && !MEMORY_WE) ? PORTA_DATA : 8'hzz;
  assign PORTA_DATA = (!PORTA_CE && (!PORTA_OE && PORTA_WE)) ? PORTA_BUFFDATA : 8'hzz;
  ...
  always @(negedge MEM_MUX_CLK)
      begin
        if (Sel == 2'b11) PORTA_BUFFDATA <= MEMORY_DATA;
          
        if (Sel == 2'b01) PORTB_BUFFDATA <= MEMORY_DATA;
