...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  always @* 
      begin
        pre_extend_array[EXTEND_ARRAY_SIZE-1:DWIDTH_RATIO*(1/1<<1)] = extend_array[(0-1)+((0-(DWIDTH_RATIO*(1/1<<1)))+EXTEND_ARRAY_SIZE):0];
        for (j = 0; j < (DWIDTH_RATIO*(1/1<<1)); j = j+1)
            begin
              pre_extend_array[j] = ctl_doing_rd[((0-1)+(-j))+(DWIDTH_RATIO*(1/1<<1))];
            end
      end
            ...
            if (oct_lat < (DELAY_ARRAY_SIZE-(DWIDTH_RATIO*(1/1<<1)))) 
              begin
                for (l = 0; l < (DWIDTH_RATIO*(1/1<<1)); l = l+1)
                    begin
                      oct_out[(0-l)+((DWIDTH_RATIO*(1/1<<1))-1)] <= delay_array_temp[l+oct_lat];
                    end
              end
            else oct_out <= 0;
          end
