...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  input  wire [(DWIDTH_RATIO*(1/(1+1)))-1:0] ctl_doing_rd;
  output reg [(DWIDTH_RATIO*(1/(1+1)))-1:0] oct_out;
  localparam  DELAY_ARRAY_SIZE = 32;
  localparam  MAX_EXTENSION = 14;
  localparam 
       EXTEND_ARRAY_SIZE = (DWIDTH_RATIO*(1/(1+1)))+MAX_EXTENSION;
  reg  [DELAY_ARRAY_SIZE-1:0] delay_array;
  reg  [EXTEND_ARRAY_SIZE-1:0] extend_array;
  reg  [(DWIDTH_RATIO*(1/(1+1)))-1:0] extended_value;
  wire [(DWIDTH_RATIO*(1/(1+1)))-1:0] pre_extended_value;
  reg  [EXTEND_ARRAY_SIZE-1:0] pre_extend_array;
  reg  [MAX_EXTENSION:0] extend_using_this_bit_mask;
  wire [MAX_EXTENSION:0] 
      dwidth_scaled_pre_extend_array[(DWIDTH_RATIO*(1/(1+1)))-1:0];
  integer i;
  integer j;
  integer k;
  integer l;
  ...
  always @* 
      begin
        pre_extend_array[EXTEND_ARRAY_SIZE-1:DWIDTH_RATIO*(1/(1+1))] = extend_array[(0-1)+((-(DWIDTH_RATIO*(1/(1+1))))+EXTEND_ARRAY_SIZE):0];
        for (j = 0; j < (DWIDTH_RATIO*(1/(1+1))); j = j+1)
            begin
              pre_extend_array[j] = ctl_doing_rd[((-j)+(0-1))+(DWIDTH_RATIO*(1/(1+1)))];
            end
      end
  ...
  always @(posedge phy_clk_1x or negedge reset_phy_clk_1x_n)
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          ...
          begin
            delay_array_temp = {delay_array[((-(DWIDTH_RATIO*(1/(1+1))))+DELAY_ARRAY_SIZE)-1:0],extended_value};
            extend_array <= pre_extend_array;
            delay_array <= delay_array_temp;
            if (oct_lat < ((-(DWIDTH_RATIO*(1/(1+1))))+DELAY_ARRAY_SIZE)) 
              begin
                for (l = 0; l < (DWIDTH_RATIO*(1/(1+1))); l = 1+l)
                    begin
                      oct_out[((DWIDTH_RATIO*(1/(1+1)))-1)-l] <= delay_array_temp[l+oct_lat];
                    end
              end
            else oct_out <= 0;
          end
  ...
  generate
      genvar bits;
      for (bits = 0; bits < (DWIDTH_RATIO*(1/(1+1))); bits = bits+1)
