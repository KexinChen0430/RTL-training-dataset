...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  input  wire [(-1)+(DWIDTH_RATIO*(1/(1+1))):0] ctl_doing_rd;
  output reg [(-1)+(DWIDTH_RATIO*(1/(1+1))):0] oct_out;
  ...
  reg  [(-1)+(DWIDTH_RATIO*(1/(1+1))):0] extended_value;
  wire [(-1)+(DWIDTH_RATIO*(1/(1+1))):0] pre_extended_value;
  reg  [(-1)+EXTEND_ARRAY_SIZE:0] pre_extend_array;
  reg  [MAX_EXTENSION:0] extend_using_this_bit_mask;
  wire [MAX_EXTENSION:0] 
      dwidth_scaled_pre_extend_array[(-1)+(DWIDTH_RATIO*(1/(1+1))):0];
  integer i;
  integer j;
  integer k;
  integer l;
  ...
  always @* 
      begin
        pre_extend_array[(-1)+EXTEND_ARRAY_SIZE:DWIDTH_RATIO*(1/(1+1))] = extend_array[((-(DWIDTH_RATIO*(1/(1+1))))+(-1))+EXTEND_ARRAY_SIZE:0];
        for (j = 0; j < (DWIDTH_RATIO*(1/(1+1))); j = 1+j)
            begin
              pre_extend_array[j] = ctl_doing_rd[(DWIDTH_RATIO*(1/(1+1)))+((0-j)+(-1))];
            end
      end
            ...
            if (oct_lat < ((-(DWIDTH_RATIO*(1/(1+1))))+DELAY_ARRAY_SIZE)) 
              begin
                for (l = 0; l < (DWIDTH_RATIO*(1/(1+1))); l = 1+l)
                    begin
                      oct_out[(DWIDTH_RATIO*(1/(1+1)))+((-1)+(-l))] <= delay_array_temp[oct_lat+l];
                    end
              end
            else oct_out <= 0;
          end
  ...
  always @(extended_value or pre_extended_value)
      begin
        if (extended_value < (MAX_EXTENSION+1)) 
          begin
            for (k = 0; k < (DWIDTH_RATIO*(1/(1+1))); k = k+1)
                begin
