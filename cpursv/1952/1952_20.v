...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  input  wire [(-1)+((1/2)*DWIDTH_RATIO):0] ctl_doing_rd;
  output reg [(-1)+((1/2)*DWIDTH_RATIO):0] oct_out;
  localparam  DELAY_ARRAY_SIZE = 32;
  localparam  MAX_EXTENSION = 14;
  localparam 
       EXTEND_ARRAY_SIZE = MAX_EXTENSION+((1/2)*DWIDTH_RATIO);
  reg  [DELAY_ARRAY_SIZE-1:0] delay_array;
  reg  [(-1)+EXTEND_ARRAY_SIZE:0] extend_array;
  reg  [(-1)+((1/2)*DWIDTH_RATIO):0] extended_value;
  wire [(-1)+((1/2)*DWIDTH_RATIO):0] pre_extended_value;
  reg  [(-1)+EXTEND_ARRAY_SIZE:0] pre_extend_array;
  reg  [MAX_EXTENSION:0] extend_using_this_bit_mask;
  wire [MAX_EXTENSION:0] 
      dwidth_scaled_pre_extend_array[(-1)+((1/2)*DWIDTH_RATIO):0];
  integer i;
  integer j;
  integer k;
  integer l;
  ...
  always @* 
      begin
        pre_extend_array[(-1)+EXTEND_ARRAY_SIZE:(1/2)*DWIDTH_RATIO] = extend_array[((-1)+EXTEND_ARRAY_SIZE)+(-((1/2)*DWIDTH_RATIO)):0];
        for (j = 0; j < ((1/2)*DWIDTH_RATIO); j = j+1)
            begin
              pre_extend_array[j] = ctl_doing_rd[((-1)+((1/2)*DWIDTH_RATIO))+(0-j)];
            end
      end
      ...
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          begin
            ...
            oct_out <= {(1/2)*DWIDTH_RATIO{1'b0}};
          end
        else 
          begin
            delay_array_temp = {delay_array[(-1)+(DELAY_ARRAY_SIZE+(-((1/2)*DWIDTH_RATIO))):0],extended_value};
            extend_array <= pre_extend_array;
            delay_array <= delay_array_temp;
            if (oct_lat < (DELAY_ARRAY_SIZE+(-((1/2)*DWIDTH_RATIO)))) 
              begin
                for (l = 0; l < ((1/2)*DWIDTH_RATIO); l = 1+l)
                    begin
                      oct_out[((1/2)*DWIDTH_RATIO)+((-l)+(-1))] <= delay_array_temp[oct_lat+l];
                    end
              end
            else oct_out <= 0;
          end
  ...
  generate
      genvar bits;
      for (bits = 0; bits < ((1/2)*DWIDTH_RATIO); bits = bits+1)
  ...
  always @(extended_value or pre_extended_value)
      begin
        if (extended_value < (1+MAX_EXTENSION)) 
          begin
            for (k = 0; k < ((1/2)*DWIDTH_RATIO); k = 1+k)
                begin
