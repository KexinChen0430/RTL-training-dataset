...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  input  wire [(DWIDTH_RATIO/1<<<1)+(0-1):0] ctl_doing_rd;
  output reg [(DWIDTH_RATIO/1<<<1)+(0-1):0] oct_out;
  localparam  DELAY_ARRAY_SIZE = 32;
  localparam  MAX_EXTENSION = 14;
  localparam 
       EXTEND_ARRAY_SIZE = MAX_EXTENSION+(DWIDTH_RATIO/1<<<1);
  reg  [(0-1)+DELAY_ARRAY_SIZE:0] delay_array;
  reg  [EXTEND_ARRAY_SIZE+(0-1):0] extend_array;
  reg  [(DWIDTH_RATIO/1<<<1)+(0-1):0] extended_value;
  wire [(DWIDTH_RATIO/1<<<1)+(0-1):0] pre_extended_value;
  reg  [EXTEND_ARRAY_SIZE+(0-1):0] pre_extend_array;
  reg  [MAX_EXTENSION:0] extend_using_this_bit_mask;
  wire [MAX_EXTENSION:0] 
      dwidth_scaled_pre_extend_array[(DWIDTH_RATIO/1<<<1)+(0-1):0];
  integer i;
  integer j;
  integer k;
  integer l;
  ...
  always @* 
      begin
        pre_extend_array[EXTEND_ARRAY_SIZE+(0-1):DWIDTH_RATIO/1<<<1] = extend_array[(-(DWIDTH_RATIO/1<<<1))+(EXTEND_ARRAY_SIZE+(0-1)):0];
        for (j = 0; j < (DWIDTH_RATIO/1<<<1); j = 1+j)
            begin
              pre_extend_array[j] = ctl_doing_rd[(DWIDTH_RATIO/1<<<1)+((0-j)+(0-1))];
            end
      end
  reg  [(0-1)+DELAY_ARRAY_SIZE:0] delay_array_temp;
  ...
  always @(posedge phy_clk_1x or negedge reset_phy_clk_1x_n)
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          ...
          begin
            delay_array_temp = {delay_array[(DELAY_ARRAY_SIZE+(-(DWIDTH_RATIO/1<<<1)))+(0-1):0],extended_value};
            extend_array <= pre_extend_array;
            delay_array <= delay_array_temp;
            if (oct_lat < (DELAY_ARRAY_SIZE+(-(DWIDTH_RATIO/1<<<1)))) 
              begin
                for (l = 0; l < (DWIDTH_RATIO/1<<<1); l = l+1)
                    begin
                      oct_out[(0-1)+((-l)+(DWIDTH_RATIO/1<<<1))] <= delay_array_temp[oct_lat+l];
                    end
              end
            else oct_out <= 0;
          end
  ...
  always @(extended_value or pre_extended_value)
      begin
        if (extended_value < (MAX_EXTENSION+1)) 
          begin
            for (k = 0; k < (DWIDTH_RATIO/1<<<1); k = k+1)
                begin
