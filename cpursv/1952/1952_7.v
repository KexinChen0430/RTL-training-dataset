...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  always @* 
      begin
        pre_extend_array[EXTEND_ARRAY_SIZE+(0-1):DWIDTH_RATIO*(1/1<<<1)] = extend_array[(0-1)+((0-(DWIDTH_RATIO*(1/1<<<1)))+EXTEND_ARRAY_SIZE):0];
        for (j = 0; j < (DWIDTH_RATIO*(1/1<<<1)); j = 1+j)
            begin
              pre_extend_array[j] = ctl_doing_rd[((0-1)+(0-j))+(DWIDTH_RATIO*(1/1<<<1))];
            end
      end
  ...
  always @(posedge phy_clk_1x or negedge reset_phy_clk_1x_n)
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          ...
          begin
            delay_array_temp = {delay_array[(0-1)+(DELAY_ARRAY_SIZE-(DWIDTH_RATIO*(1/1<<<1))):0],extended_value};
            ...
            if (oct_lat < (DELAY_ARRAY_SIZE-(DWIDTH_RATIO*(1/1<<<1)))) 
              begin
                for (l = 0; l < (DWIDTH_RATIO*(1/1<<<1)); l = l+1)
                    begin
                      oct_out[(0-l)+((0-1)+(DWIDTH_RATIO*(1/1<<<1)))] <= delay_array_temp[oct_lat+l];
                    end
              end
            else oct_out <= 0;
          end
