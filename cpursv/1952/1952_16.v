...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  localparam 
       EXTEND_ARRAY_SIZE = (DWIDTH_RATIO/2)+MAX_EXTENSION;
  ...
  always @* 
      begin
        pre_extend_array[EXTEND_ARRAY_SIZE+(-1):DWIDTH_RATIO/2] = extend_array[(-1)+((0-(DWIDTH_RATIO/2))+EXTEND_ARRAY_SIZE):0];
        for (j = 0; j < (DWIDTH_RATIO/2); j = 1+j)
            begin
  ...
  always @(posedge phy_clk_1x or negedge reset_phy_clk_1x_n)
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          ...
          begin
            delay_array_temp = {delay_array[(DELAY_ARRAY_SIZE+(0-(DWIDTH_RATIO/2)))+(-1):0],extended_value};
            extend_array <= pre_extend_array;
            delay_array <= delay_array_temp;
            if (oct_lat < (DELAY_ARRAY_SIZE+(0-(DWIDTH_RATIO/2)))) 
              begin
                for (l = 0; l < (DWIDTH_RATIO/2); l = 1+l)
                    begin
                      oct_out[((DWIDTH_RATIO/2)-1)-l] <= delay_array_temp[l+oct_lat];
                    end
              end
            else oct_out <= 0;
          end
