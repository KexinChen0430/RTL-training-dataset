...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  always @* 
      begin
        pre_extend_array[(-1)+EXTEND_ARRAY_SIZE:DWIDTH_RATIO*(1/(1+1))] = extend_array[(-1)+((0-(DWIDTH_RATIO*(1/(1+1))))+EXTEND_ARRAY_SIZE):0];
            ...
            if (oct_lat < (DELAY_ARRAY_SIZE-(DWIDTH_RATIO*(1/(1+1))))) 
              begin
                for (l = 0; l < (DWIDTH_RATIO*(1/(1+1))); l = 1+l)
                    begin
                      oct_out[(-1)+((DWIDTH_RATIO*(1/(1+1)))+(-l))] <= delay_array_temp[oct_lat+l];
                    end
              end
            else oct_out <= 0;
          end
