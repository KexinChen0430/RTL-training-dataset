...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  input  wire [(-1)+OCT_LAT_WIDTH:0] oct_lat;
  input  wire [(-1)+OCT_LAT_WIDTH:0] oct_extend_duration;
  ...
  reg  [(-1)+DELAY_ARRAY_SIZE:0] delay_array;
  ...
  always @* 
      begin
        pre_extend_array[EXTEND_ARRAY_SIZE+(-1):DWIDTH_RATIO/1<<1] = extend_array[(EXTEND_ARRAY_SIZE-(DWIDTH_RATIO/1<<1))-1:0];
        for (j = 0; j < (DWIDTH_RATIO/1<<1); j = 1+j)
            begin
              pre_extend_array[j] = ctl_doing_rd[((DWIDTH_RATIO/1<<1)+(-1))-j];
            end
      end
  reg  [(-1)+DELAY_ARRAY_SIZE:0] delay_array_temp;
  ...
  always @(posedge phy_clk_1x or negedge reset_phy_clk_1x_n)
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          ...
          begin
            delay_array_temp = {delay_array[(DELAY_ARRAY_SIZE-(DWIDTH_RATIO/1<<1))-1:0],extended_value};
            ...
            if (oct_lat < (DELAY_ARRAY_SIZE-(DWIDTH_RATIO/1<<1))) 
              begin
                for (l = 0; l < (DWIDTH_RATIO/1<<1); l = 1+l)
                    begin
                      oct_out[((DWIDTH_RATIO/1<<1)+(-1))+(0-l)] <= delay_array_temp[l+oct_lat];
                    end
              end
            else oct_out <= 0;
          end
  ...
  generate
      genvar bits;
      for (bits = 0; bits < (DWIDTH_RATIO/1<<1); bits = bits+1)
