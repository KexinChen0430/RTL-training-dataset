...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  always @* 
      begin
        pre_extend_array[(-1)+EXTEND_ARRAY_SIZE:DWIDTH_RATIO*(1/1<<1)] = extend_array[((0-(DWIDTH_RATIO*(1/1<<1)))+(-1))+EXTEND_ARRAY_SIZE:0];
        for (j = 0; j < (DWIDTH_RATIO*(1/1<<1)); j = j+1)
            begin
              pre_extend_array[j] = ctl_doing_rd[((DWIDTH_RATIO*(1/1<<1))-1)+(0-j)];
            end
      end
  ...
  always @(posedge phy_clk_1x or negedge reset_phy_clk_1x_n)
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          ...
          begin
            delay_array_temp = {delay_array[(-1)+((0-(DWIDTH_RATIO*(1/1<<1)))+DELAY_ARRAY_SIZE):0],extended_value};
            extend_array <= pre_extend_array;
            delay_array <= delay_array_temp;
            if (oct_lat < ((0-(DWIDTH_RATIO*(1/1<<1)))+DELAY_ARRAY_SIZE)) 
              begin
