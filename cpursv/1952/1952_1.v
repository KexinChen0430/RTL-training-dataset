...
module ddr3_s4_amphy_phy_alt_mem_phy_oct_delay(phy_clk_1x,reset_phy_clk_1x_n,oct_lat,oct_extend_duration,
  ...
  localparam 
       EXTEND_ARRAY_SIZE = MAX_EXTENSION+(DWIDTH_RATIO/2);
  ...
  wire [MAX_EXTENSION:0] 
      dwidth_scaled_pre_extend_array[(DWIDTH_RATIO/2)-1:0];
  integer i;
  integer j;
  integer k;
  integer l;
  ...
  always @* 
      begin
        ...
        for (j = 0; j < (DWIDTH_RATIO/2); j = j+1)
            begin
              pre_extend_array[j] = ctl_doing_rd[((-j)+(0-1))+(DWIDTH_RATIO/2)];
            end
      end
  ...
  always @(posedge phy_clk_1x or negedge reset_phy_clk_1x_n)
      begin
        if (reset_phy_clk_1x_n == 1'b0) 
          ...
          begin
            delay_array_temp = {delay_array[((-(DWIDTH_RATIO/2))+DELAY_ARRAY_SIZE)+(0-1):0],extended_value};
            extend_array <= pre_extend_array;
            delay_array <= delay_array_temp;
            if (oct_lat < ((-(DWIDTH_RATIO/2))+DELAY_ARRAY_SIZE)) 
              begin
                for (l = 0; l < (DWIDTH_RATIO/2); l = 1+l)
                    begin
                      oct_out[((0-l)+(DWIDTH_RATIO/2))+(0-1)] <= delay_array_temp[l+oct_lat];
                    end
              end
            else oct_out <= 0;
          end
  ...
  always @(extended_value or pre_extended_value)
      begin
        if (extended_value < (1+MAX_EXTENSION)) 
          begin
            for (k = 0; k < (DWIDTH_RATIO/2); k = 1+k)
                begin
