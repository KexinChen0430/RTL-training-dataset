...
module SensorFSM  #(parameter  DataWidth = 8)
  (input  Reset_n_i,
   ...
   output [DataWidth<<1+(-1):0] SensorValue_o,
   output reg MeasureFSM_Start_o,
   input  MeasureFSM_Done_i,
   input  [(-1)+DataWidth:0] MeasureFSM_Byte0_i,
   input  [(-1)+DataWidth:0] MeasureFSM_Byte1_i,
   input  [DataWidth<<1+(-1):0] ParamThreshold_i,
   input  [DataWidth<<1+(-1):0] ParamCounterPreset_i);
  ...
  wire [DataWidth<<1+(-1):0] SensorValue;
  reg  [DataWidth<<1+(-1):0] Word0;
  wire [DataWidth<<1+(-1):0] AbsDiffResult;
  ...
  reg  [DataWidth<<1+(-1):0] SensorFSM_Timer;
        ...
        if (!Reset_n_i) 
          ...
          begin
            if (SensorFSM_TimerPreset) 
              ...
              begin
                SensorFSM_Timer <= (-1'b1)+SensorFSM_Timer;
              end
  ...
  wire [DataWidth<<1:0] DiffAB;

  wire [DataWidth<<1+(-1):0] DiffBA;

  assign DiffAB = (-{1'b0,Word0})+{1'b0,SensorValue};
  assign DiffBA = (-SensorValue)+Word0;
  assign AbsDiffResult = DiffAB[DataWidth<<1] ? DiffBA : DiffAB[DataWidth<<1+(-1):0];
