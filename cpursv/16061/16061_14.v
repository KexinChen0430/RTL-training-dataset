...
module SensorFSM  #(parameter  DataWidth = 8)
  (input  Reset_n_i,
   ...
   output [((1+1)*DataWidth)+(0-1):0] SensorValue_o,
   output reg MeasureFSM_Start_o,
   input  MeasureFSM_Done_i,
   input  [(0-1)+DataWidth:0] MeasureFSM_Byte0_i,
   input  [(0-1)+DataWidth:0] MeasureFSM_Byte1_i,
   input  [((1+1)*DataWidth)+(0-1):0] ParamThreshold_i,
   input  [((1+1)*DataWidth)+(0-1):0] ParamCounterPreset_i);
  ...
  wire [((1+1)*DataWidth)+(0-1):0] SensorValue;
  reg  [((1+1)*DataWidth)+(0-1):0] Word0;
  wire [((1+1)*DataWidth)+(0-1):0] AbsDiffResult;
  ...
  reg  [((1+1)*DataWidth)+(0-1):0] SensorFSM_Timer;
        ...
        if (!Reset_n_i) 
          ...
          begin
            if (SensorFSM_TimerPreset) 
              ...
              begin
                SensorFSM_Timer <= SensorFSM_Timer+(0-1'b1);
              end
  ...
  wire [(1+1)*DataWidth:0] DiffAB;

  wire [((1+1)*DataWidth)+(0-1):0] DiffBA;

  assign DiffAB = (-{1'b0,Word0})+{1'b0,SensorValue};
  assign DiffBA = Word0+(0-SensorValue);
  assign AbsDiffResult = DiffAB[(1+1)*DataWidth] ? DiffBA : DiffAB[((1+1)*DataWidth)+(0-1):0];
