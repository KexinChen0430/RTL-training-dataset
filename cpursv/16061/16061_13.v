...
module SensorFSM  #(parameter  DataWidth = 8)
  (input  Reset_n_i,
   ...
   output [(0-1)+((1+1)*DataWidth):0] SensorValue_o,
   ...
   input  [(0-1)+((1+1)*DataWidth):0] ParamThreshold_i,
   input  [(0-1)+((1+1)*DataWidth):0] ParamCounterPreset_i);
  ...
  wire [(0-1)+((1+1)*DataWidth):0] SensorValue;
  reg  [(0-1)+((1+1)*DataWidth):0] Word0;
  wire [(0-1)+((1+1)*DataWidth):0] AbsDiffResult;
  ...
  reg  [(0-1)+((1+1)*DataWidth):0] SensorFSM_Timer;
  ...
  wire [(0-1)+((1+1)*DataWidth):0] DiffBA;

  assign DiffAB = {1'b0,SensorValue}-{1'b0,Word0};
  assign DiffBA = (0-SensorValue)+Word0;
  assign AbsDiffResult = DiffAB[(1+1)*DataWidth] ? DiffBA : DiffAB[(0-1)+((1+1)*DataWidth):0];
