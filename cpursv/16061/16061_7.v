...
module SensorFSM  #(parameter  DataWidth = 8)
  (input  Reset_n_i,
   ...
   output [(1<<<1*DataWidth)+(-1):0] SensorValue_o,
   ...
   input  [(1<<<1*DataWidth)+(-1):0] ParamThreshold_i,
   input  [(1<<<1*DataWidth)+(-1):0] ParamCounterPreset_i);
  ...
  wire [(1<<<1*DataWidth)+(-1):0] SensorValue;
  reg  [(1<<<1*DataWidth)+(-1):0] Word0;
  wire [(1<<<1*DataWidth)+(-1):0] AbsDiffResult;
  ...
  reg  [(1<<<1*DataWidth)+(-1):0] SensorFSM_Timer;
  ...
  wire [(1<<<1*DataWidth)+(-1):0] DiffBA;

  assign DiffAB = {1'b0,SensorValue}+(0-{1'b0,Word0});
  assign DiffBA = (0-SensorValue)+Word0;
  assign AbsDiffResult = DiffAB[1<<<1*DataWidth] ? DiffBA : DiffAB[(1<<<1*DataWidth)+(-1):0];
