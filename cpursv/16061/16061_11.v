...
module SensorFSM  #(parameter  DataWidth = 8)
  (input  Reset_n_i,
   ...
   output [DataWidth<<1+(0-1):0] SensorValue_o,
   ...
   input  [DataWidth<<1+(0-1):0] ParamThreshold_i,
   input  [DataWidth<<1+(0-1):0] ParamCounterPreset_i);
  ...
  wire [DataWidth<<1+(0-1):0] SensorValue;
  reg  [DataWidth<<1+(0-1):0] Word0;
  wire [DataWidth<<1+(0-1):0] AbsDiffResult;
  ...
  reg  [DataWidth<<1+(0-1):0] SensorFSM_Timer;
  ...
  wire [DataWidth<<1+(0-1):0] DiffBA;

  assign DiffAB = {1'b0,SensorValue}+(0-{1'b0,Word0});
  assign DiffBA = Word0+(-SensorValue);
  assign AbsDiffResult = DiffAB[DataWidth<<1] ? DiffBA : DiffAB[DataWidth<<1+(0-1):0];
