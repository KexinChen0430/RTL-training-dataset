...
module timing_generator_VGA(input  wire clk,reset,
  ...
  assign h_end = h_count_reg == ((HD+(((-1)+HR)+HB))+HF);
  assign v_end = v_count_reg == (((VF+(-1))+VD)+(VR+VB));
  ...
  assign h_sync_next = (h_count_reg <= (HD+(((-1)+HR)+HB))) && 
                       (h_count_reg >= (HB+HD));
  assign v_sync_next = (v_count_reg <= ((VR+(-1))+(VB+VD))) && 
                       (v_count_reg >= (VB+VD));
