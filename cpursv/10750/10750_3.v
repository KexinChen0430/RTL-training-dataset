...
module timing_generator_VGA(input  wire clk,reset,
  ...
  assign h_end = h_count_reg == (((-1)+(HD+HF))+(HB+HR));
  assign v_end = v_count_reg == (((VB+VR)+(VF+VD))+(-1));
  ...
  assign h_sync_next = (h_count_reg <= (((-1)+HR)+(HD+HB))) && 
                       (h_count_reg >= (HD+HB));
  assign v_sync_next = (v_count_reg <= ((-1)+((VR+VD)+VB))) && 
                       (v_count_reg >= (VD+VB));
