...
module hls_contrast_stretch_AXILiteS_s_axi  #(parameter  C_S_AXI_ADDR_WIDTH = 6, C_S_AXI_DATA_WIDTH = 32)
  (input  wire ACLK,
   ...
   input  wire [(-1)+((1/8)*C_S_AXI_DATA_WIDTH):0] WSTRB,
   input  wire WVALID,
   output wire WREADY,
        ...
        if (ARESET) int_height[15:0] <= 0;
          ...
          begin
            if (w_hs && (waddr == ADDR_HEIGHT_DATA_0)) 
              int_height[15:0] <= (((WDATA[31:0] | int_width[15:0]) & wmask) | ((int_height[15:0] & ~wmask) | (int_height[15:0] & (WDATA[31:0] & ((wmask | int_max[7:0]) | ~wmask))))) & ((int_height[15:0] & ~wmask) | WDATA[31:0]);
        ...
        if (ARESET) int_width[15:0] <= 0;
          ...
          begin
            if ((waddr == ADDR_WIDTH_DATA_0) && w_hs) 
              int_width[15:0] <= (wmask & (WDATA[31:0] & (wmask | (int_height[15:0] & ~wmask)))) | (((~wmask | ((WDATA[31:0] & ((wmask | int_max[7:0]) | ~wmask)) & (wmask | int_max[7:0]))) & int_width[15:0]) | (int_height[15:0] & (wmask & (WDATA[31:0] & (wmask | (int_height[15:0] & ~wmask))))));
        ...
        if (ARESET) int_min[7:0] <= 0;
          ...
          begin
            if (w_hs && (waddr == ADDR_MIN_DATA_0)) 
              int_min[7:0] <= ((int_min[7:0] & ~wmask) | WDATA[31:0]) & ((wmask & (WDATA[31:0] & (wmask | (int_height[15:0] & ~wmask)))) | int_min[7:0]);
        ...
        if (ARESET) int_max[7:0] <= 0;
          ...
          begin
            if (w_hs && (waddr == ADDR_MAX_DATA_0)) 
              int_max[7:0] <= (~wmask & int_max[7:0]) | (((int_max[7:0] | WDATA[31:0]) & (~wmask | ((WDATA[31:0] & ((wmask | int_max[7:0]) | ~wmask)) & (wmask | int_max[7:0])))) & wmask);
