...
module wb_switch  #(parameter 
  ...
  wire [31:0] i_dat_s;
  wire i_bus_ack;
  wire [(((((32+4)+32)+1)+1)+1)-1:0] i_bus_m;

  assign m_dat_o = i_dat_s;
  assign m_ack_o = i_bus_ack;
  assign i_bus_ack = ((s0_ack_i | s1_ack_i) | s2_ack_i) | s3_ack_i;
  assign i_dat_s = ((({32{slave_sel[0]}} & s0_dat_i) | ({32{slave_sel[1]}} & s1_dat_i)) | ({32{slave_sel[2]}} & s2_dat_i)) | ({32{slave_sel[3]}} & s3_dat_i);
  ...
  assign {s0_adr_o,s0_sel_o,s0_dat_o,s0_we_o,s0_cyc_o} = i_bus_m[(((((32+4)+32)+1)+1)+1)-1:1];
  assign s0_stb_o = (i_bus_m[1] & i_bus_m[0]) & slave_sel[0];
  assign {s1_adr_o,s1_sel_o,s1_dat_o,s1_we_o,s1_cyc_o} = i_bus_m[(((((32+4)+32)+1)+1)+1)-1:1];
  assign s1_stb_o = (i_bus_m[1] & i_bus_m[0]) & slave_sel[1];
  assign {s2_adr_o,s2_sel_o,s2_dat_o,s2_we_o,s2_cyc_o} = i_bus_m[(((((32+4)+32)+1)+1)+1)-1:1];
  assign s2_stb_o = (i_bus_m[1] & i_bus_m[0]) & slave_sel[2];
  assign {s3_adr_o,s3_sel_o,s3_dat_o,s3_we_o,s3_cyc_o} = i_bus_m[(((((32+4)+32)+1)+1)+1)-1:1];
