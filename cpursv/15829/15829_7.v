                ...
                if (bit_cnt > (uart_data_width+(uart_stopbit ? 1<<<1 : 1))) 
                  ...
                  begin
                    ...
                    if ((bit_cnt == (uart_stopbit ? 3 : 1<<<1)) && 
                        (uart_stopbit[1] == 1'b1)) prescale_reg <= (prescale*(1<<<1**1<<<1))+(-1);
                    else prescale_reg <= prescale<<<3-1;
                    if ((bit_cnt == (uart_stopbit ? 3 : 1<<<1)) && 
                        (uart_parity == 2'b01)) 
                      begin
                        parity <= ~(rxd_reg ^ ((parity & (rxd_reg | ~rxd_reg)) & (rxd_reg | ~rxd_reg)));
                      end
...
((uart_parity == 2'b00) && (uart_parity != 2'b00))) && (rxd_reg == 1'b1)) || (uart_parity == 2'b00) || (parity == 1'b1)))) 
                      begin
