                ...
                if (bit_cnt > (uart_data_width+(uart_stopbit ? 1<<1 : 1))) 
                  ...
                  begin
                    ...
                    if ((uart_stopbit[1] == 1'b1) && 
                        (bit_cnt == (uart_stopbit ? 3 : 1<<1))) prescale_reg <= (1<<1*(1<<1*prescale))+(0-1);
                    else prescale_reg <= (0-1)+prescale<<<3;
                    if ((uart_parity == 2'b01) && 
                        (bit_cnt == (uart_stopbit ? 3 : 1<<1))) 
                      begin
                        parity <= ~((rxd_reg & ~parity) | ((~rxd_reg | ((parity | (rxd_reg & (parity | ~parity))) & (((parity | (rxd_reg & (parity | ~parity))) | ~rxd_reg) & ~parity))) & (((parity | (rxd_reg & (parity | ~parity))) | ~rxd_reg) & parity)));
                      end
...
((uart_parity == 2'b00) || (parity == 1'b1)))) 
              ...
              begin
                ...
                if (~rxd_reg) 
                  begin
                    prescale_reg <= (1<<1*(1<<1*prescale))-1<<1;
