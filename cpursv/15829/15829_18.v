                ...
                if (bit_cnt > (uart_data_width+(uart_stopbit ? 1<<1 : 1))) 
                  ...
                  begin
                    ...
                    if ((uart_stopbit[1] == 1'b1) && 
                        (bit_cnt == (uart_stopbit ? 3 : 1<<1))) prescale_reg <= (prescale*(1<<1+1<<1))+(-1);
                    else prescale_reg <= prescale<<<3+(-1);
                    if ((uart_parity == 2'b01) && 
                        (bit_cnt == (uart_stopbit ? 3 : 1<<1))) 
                      begin
                        parity <= ~((parity & (~rxd_reg & (parity | rxd_reg))) | ((~parity & (parity & (~rxd_reg & (parity | rxd_reg)))) | (~parity & (parity | rxd_reg))));
                      end
                  ...
                  begin
                    bit_cnt <= (-1)+bit_cnt;
                    if (((parity == 1'b1) || (uart_parity == 2'b00)) && 
                        (rxd_reg == 1'b1) && 
                        ((uart_parity != 2'b00) || 
((rxd_reg == 1'b1) && 
(
