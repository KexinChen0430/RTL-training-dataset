...
module lfsr_counter(clk,reset,out);
  ...
  always @(lfsr)
      begin
        ...
        fb = lfsr[WIDTH-1] ^ (((~lfsr[WIDTH-1] | ~fb_lsb) | lfsr[WIDTH-1]) & ((lfsr[WIDTH-1] & fb_lsb) | (fb_lsb & ~lfsr[WIDTH-1])));
        for (N = WIDTH; N >= 0; N = (-1)+N)
            if (TAP[N] == 1) 
              next_lfsr[N] = ((((~fb | fb) & ~lfsr[(-1)+N]) | ~fb) & ((~lfsr[(-1)+N] | lfsr[(-1)+N]) & fb)) ^ ((((~fb | fb) & ~lfsr[(-1)+N]) | ~fb) & ((~fb | fb) & lfsr[(-1)+N]));
