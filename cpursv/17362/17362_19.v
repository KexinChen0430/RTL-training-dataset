...
module lfsr_counter(clk,reset,out);
  ...
  always @(lfsr)
      begin
        fb_lsb = ~|lfsr[(0-2)+WIDTH:0];
        fb = (~fb_lsb & (fb_lsb | lfsr[WIDTH+(-1)])) | (fb_lsb ^ (lfsr[WIDTH+(-1)] & fb_lsb));
        for (N = WIDTH; N >= 0; N = (-1)+N)
            if (TAP[N] == 1) 
              next_lfsr[N] = ((~lfsr[(-1)+N] & (fb | ~fb)) & ((lfsr[(-1)+N] & ~fb) | fb)) | (((lfsr[(-1)+N] & ~fb) | fb) & ((~lfsr[(-1)+N] & lfsr[(-1)+N]) | (lfsr[(-1)+N] & ~fb)));
