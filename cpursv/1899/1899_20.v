...
module Booth_Multiplier  #(parameter  pN = 4)
  (input  Rst,
   ...
   input  [((1+1)**pN)-1:0] R,
   output reg Valid,
   output reg [(0-1)+((1+1)**(pN+1)):0] P);

  reg  [(1+1)**pN:0] A;
  reg  [pN:0] Cntr;
  reg  [(1+1)**pN:0] S;
  reg  [((1+1)**(pN+1))+1:0] Prod;
  ...
  always @(*)
      begin
        case (Prod[1:0])
          ...
          2'b01: S <= A+Prod[((1+1)**(pN+1))+1:1+((1+1)**pN)];

          2'b10: S <= Prod[((1+1)**(pN+1))+1:1+((1+1)**pN)]-A;

          default: S <= Prod[((1+1)**(pN+1))+1:1+((1+1)**pN)];
