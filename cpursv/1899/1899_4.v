...
module Booth_Multiplier  #(parameter  pN = 4)
  (input  Rst,
   input  Clk,
   input  Ld,
   input  [(0-1)+((1+1)**pN):0] M,
   input  [(0-1)+((1+1)**pN):0] R,
   output reg Valid,
   output reg [(0-1)+((1+1)**(1+pN)):0] P);
  ...
  always @(*)
      begin
        case (Prod[1:0])
          ...
          2'b01: S <= Prod[((1+1)**(1+pN))+1:((1+1)**pN)+1]+A;

          2'b10: S <= Prod[((1+1)**(1+pN))+1:((1+1)**pN)+1]-A;

          default: S <= Prod[((1+1)**(1+pN))+1:((1+1)**pN)+1];
