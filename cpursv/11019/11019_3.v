...
module button_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((({4{address == 3}} & edge_capture) | data_in) & {4{address == 0}}) | ((((({4{address == 3}} & edge_capture) | data_in) | (((({4{address == 1<<1}} | {4{address == 0}}) & (irq_mask | {4{address == 0}})) & irq_mask) & {4{address == 1<<1}})) & ({4{address == 3}} | (((({4{address == 1<<1}} | {4{address == 0}}) & (irq_mask | {4{address == 0}})) & irq_mask) & {4{address == 1<<1}}))) & ((((({4{address == 1<<1}} | {4{address == 0}}) & (irq_mask | {4{address == 0}})) & irq_mask) & {4{address == 1<<1}}) | edge_capture));
  ...
  always @(posedge clk or negedge reset_n)
      begin
        ...
        else if ((address == 1<<1) && (chipselect && ~write_n)) irq_mask <= writedata[3:0];
