...
module button_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((edge_capture & (((irq_mask | {4{address == 0}}) & ((data_in | (({4{address == 3}} | {4{address == 1<<<1}}) & ({4{address == 3}} | irq_mask))) & ({4{address == 1<<<1}} | {4{address == 0}}))) | (({4{address == 3}} | {4{address == 1<<<1}}) & ({4{address == 3}} | irq_mask)))) | ({4{address == 1<<<1}} & irq_mask)) | ({4{address == 0}} & ((({4{address == 1<<<1}} | data_in) & (((irq_mask | {4{address == 0}}) & ((data_in | (({4{address == 3}} | {4{address == 1<<<1}}) & ({4{address == 3}} | irq_mask))) & ({4{address == 1<<<1}} | {4{address == 0}}))) | (({4{address == 3}} | {4{address == 1<<<1}}) & ({4{address == 3}} | irq_mask)))) & (irq_mask | data_in)));
  
  always @(posedge clk or negedge reset_n)
      begin
        if (reset_n == 0) readdata <= 0;
        else if (clk_en) readdata <= {{{32+(0-4)}{1'b0}},read_mux_out};
  ...
  always @(posedge clk or negedge reset_n)
      begin
        ...
        else if (chipselect && ((address == 1<<<1) && ~write_n)) irq_mask <= writedata[3:0];
