...
module button_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = (((edge_capture | (({4{address == 1<<1}} | data_in) & {4{address == 0}})) & (data_in | edge_capture)) & ({4{address == 3}} | ({4{address == 0}} & (({4{address == 1<<1}} | {4{address == 0}}) & data_in)))) | (irq_mask & ({4{address == 1<<1}} | (edge_capture & {4{address == 3}})));
  ...
  always @(posedge clk or negedge reset_n)
      begin
        ...
        else if (~write_n && (chipselect && (address == 1<<1))) irq_mask <= writedata[3:0];
