...
module button_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ({4{address == 3}} & ((edge_capture | (({4{address == 1<<<1}} & ((data_in | {4{address == 1<<<1}}) & (data_in | irq_mask))) & irq_mask)) | (({4{address == 0}} & data_in) | (({4{address == 1<<<1}} & ((data_in | {4{address == 1<<<1}}) & (data_in | irq_mask))) & irq_mask)))) | (({4{address == 0}} & data_in) | (({4{address == 1<<<1}} & ((data_in | {4{address == 1<<<1}}) & (data_in | irq_mask))) & irq_mask));
  
  always @(posedge clk or negedge reset_n)
      begin
        if (reset_n == 0) readdata <= 0;
        else if (clk_en) readdata <= {{{(-4)+32}{1'b0}},read_mux_out};
