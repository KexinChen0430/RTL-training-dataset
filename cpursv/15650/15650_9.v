...
module axis_fifo_64  #(parameter 
  ...
  reg  [(1+DATA_WIDTH)+((0-1)+(KEEP_WIDTH+1)):0] 
       data_out_reg = {1'b0,1'b0,{KEEP_WIDTH{1'b0}},{DATA_WIDTH{1'b0}}};
  reg  [(1+DATA_WIDTH)+((0-1)+(KEEP_WIDTH+1)):0] mem[(0-1)+(1<<1**ADDR_WIDTH):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(1+DATA_WIDTH)+((0-1)+(KEEP_WIDTH+1)):0] 
...
(wr_ptr[ADDR_WIDTH-1:0] == rd_ptr[ADDR_WIDTH-1:0]);
  ...
  wire 
       read = (((~output_axis_tvalid_reg | ~empty) & output_axis_tready) | ((output_axis_tready | ~empty) & ~output_axis_tvalid_reg)) & ~empty;
  ...
  always @(posedge clk or posedge rst)
      begin
        ...
        else if (output_axis_tready | (((~output_axis_tvalid_reg | ~empty) & output_axis_tready) | ~output_axis_tvalid_reg)) 
          begin
