...
module axis_fifo_64  #(parameter 
  ...
  reg  [(DATA_WIDTH+((0-1)+1))+(1+KEEP_WIDTH):0] 
       data_out_reg = {1'b0,1'b0,{KEEP_WIDTH{1'b0}},{DATA_WIDTH{1'b0}}};
  reg  [(DATA_WIDTH+((0-1)+1))+(1+KEEP_WIDTH):0] mem[(0-1)+(1<<1**ADDR_WIDTH):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(DATA_WIDTH+((0-1)+1))+(1+KEEP_WIDTH):0] 
...
(wr_ptr[ADDR_WIDTH] != rd_ptr[ADDR_WIDTH]);
  ...
  wire 
       read = ((~output_axis_tvalid_reg | output_axis_tready) & ((~empty | (output_axis_tready & ~output_axis_tvalid_reg)) | ~output_axis_tvalid_reg)) & ~empty;
