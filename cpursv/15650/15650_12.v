...
module axis_fifo_64  #(parameter 
  ...
  reg  [((-1)+1)+((DATA_WIDTH+1)+KEEP_WIDTH):0] 
       data_out_reg = {1'b0,1'b0,{KEEP_WIDTH{1'b0}},{DATA_WIDTH{1'b0}}};
  reg  [((-1)+1)+((DATA_WIDTH+1)+KEEP_WIDTH):0] mem[(1<<<1**ADDR_WIDTH)-1:0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [((-1)+1)+((DATA_WIDTH+1)+KEEP_WIDTH):0] 
       data_in = {input_axis_tlast,input_axis_tuser,input_axis_tkeep,input_axis_tdata};
  wire 
       full = 
(wr_ptr[(-1)+ADDR_WIDTH:0] == rd_ptr[(-1)+ADDR_WIDTH:0]) && (wr_ptr[ADDR_WIDTH] != rd_ptr[ADDR_WIDTH]);
