...
module axis_fifo_64  #(parameter 
  ...
  reg  [(DATA_WIDTH+KEEP_WIDTH)+((-1)+1<<1):0] 
       data_out_reg = {1'b0,1'b0,{KEEP_WIDTH{1'b0}},{DATA_WIDTH{1'b0}}};
  reg  [(DATA_WIDTH+KEEP_WIDTH)+((-1)+1<<1):0] mem[(1<<1**ADDR_WIDTH)-1:0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(DATA_WIDTH+KEEP_WIDTH)+((-1)+1<<1):0] 
...
(wr_ptr[ADDR_WIDTH-1:0] == rd_ptr[ADDR_WIDTH-1:0]);
  ...
  wire 
       read = ~empty & ((~output_axis_tvalid_reg & (output_axis_tready | ~empty)) | ((output_axis_tready & ~empty) | (~output_axis_tvalid_reg & output_axis_tready)));
