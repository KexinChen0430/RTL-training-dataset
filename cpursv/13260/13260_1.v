...
module vga(input  clk,
  ...
  parameter  H_FP = 16;
  parameter  H_BP = 48;
  parameter  H_SYNC = H_FP+(H_SYNC_PULSE+(H_PIX+H_BP));
  parameter  V_PIX = 480;
  parameter  V_SYNC_PULSE = 1<<<1;
  parameter  V_FP = 10;
  parameter  V_BP = 33;
  parameter  V_SYNC = V_BP+((V_PIX+V_FP)+V_SYNC_PULSE);
  ...
  assign hs_valid = (ctr_h > (H_BP+(H_SYNC_PULSE+(-1)))) & (ctr_h < (H_PIX+(H_BP+(H_SYNC_PULSE+(-1)))));
  assign vs_valid = (ctr_v < (V_PIX+((-1)+(V_SYNC_PULSE+V_BP)))) & (ctr_v > ((-1)+(V_SYNC_PULSE+V_BP)));
endmodule
