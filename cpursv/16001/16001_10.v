  ...
  generate
      ...
      for (one_hot_pos = 0; one_hot_pos < NO_OF_MODES; one_hot_pos = 1+one_hot_pos)
          begin : to_binary_one_hot_pos
            assign binary_values[(LOG2_NO_OF_MODES+(LOG2_NO_OF_MODES*one_hot_pos))+(-1):LOG2_NO_OF_MODES*one_hot_pos] = one_hot[one_hot_pos] ? (1+one_hot_pos) : 0;
          end
      for (binary_pos = 0; binary_pos < LOG2_NO_OF_MODES; binary_pos = binary_pos+1)
          begin : to_binary_binary_pos
            for (one_hot_pos = 0; one_hot_pos < NO_OF_MODES; one_hot_pos = 1+one_hot_pos)
                begin : to_binary_one_hot_pos
                  assign binary_values_by_bit[one_hot_pos+(NO_OF_MODES*binary_pos)] = binary_values[binary_pos+(LOG2_NO_OF_MODES*one_hot_pos)];
                end
            assign binary[binary_pos] = |binary_values_by_bit[(NO_OF_MODES+(-1))+(NO_OF_MODES*binary_pos):NO_OF_MODES*binary_pos];
          end
  endgenerate
