...
module uart  #(parameter  cnt_max = 433)
  ...
  always @(posedge clk or posedge rst)
      begin
        if (rst) fifo_head <= 0;
          ...
          begin
            if (tx_st) fifo_head <= fifo_head+6'h1;
  ...
  always @(posedge clk or posedge rst)
      begin
        if (rst) fifo_tail <= 6'h0;
          ...
          begin
            if (din_en) fifo_tail <= fifo_tail+6'h1;
  ...
  always @(posedge clk or posedge rst)
      begin
        if (rst) of_err <= 0;
        else if ((fifo_head == fifo_nxt) & (din_en & !tx_st)) of_err <= 1;
  ...
  assign tx_st = (uart_en & ((fifo_head != fifo_tail) & !tx_en)) ? 1'b1 : 1'b0;
  ...
  always @(posedge clk or posedge rst)
      begin
        if (rst) tx_cnt <= 0;
          ...
          begin
            if (tx_st) tx_cnt <= 0;
            else if (tx_en) tx_cnt <= tx_cnt+4'h1;
  ...
  always @(posedge clk or posedge rst)
      begin
        if (rst) txd <= 1;
          ...
          begin
            if (tx_st) txd <= 0;
            else if ((tx_cnt & tx_en) == 4'h8) txd <= 1;
