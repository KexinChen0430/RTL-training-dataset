...
module axis_srl_fifo  #(parameter 
  ...
  (input  wire clk,
   ...
   output wire [(-1)+DATA_WIDTH:0] m_axis_tdata,
   ...
   output wire [(-1)+$clog2(1+DEPTH):0] count);
  ...
  reg  [WIDTH+(-1):0] data_reg[(-1)+DEPTH:0];
  reg  [(-1)+$clog2(1+DEPTH):0]  ptr_reg = 0;
  reg   full_reg = 1'b0,full_next;
  reg   empty_reg = 1'b1,empty_next;
  wire [WIDTH+(-1):0] s_axis;
  wire [WIDTH+(-1):0]  m_axis = data_reg[(-1)+ptr_reg];

  assign s_axis_tready = !full_reg;
  
  generate
      assign s_axis[(-1)+DATA_WIDTH:0] = s_axis_tdata;
  ...
  assign m_axis_tdata = m_axis[(-1)+DATA_WIDTH:0];
  ...
  wire  ptr_full1 = ptr_reg == ((-1)+DEPTH);
