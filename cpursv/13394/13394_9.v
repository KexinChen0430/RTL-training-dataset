...
module axis_srl_fifo  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [(0-1)+DATA_WIDTH:0] s_axis_tdata,
   input  wire [KEEP_WIDTH-1:0] s_axis_tkeep,
   input  wire s_axis_tvalid,
   output wire s_axis_tready,
   input  wire s_axis_tlast,
   input  wire [(0-1)+ID_WIDTH:0] s_axis_tid,
   input  wire [(0-1)+DEST_WIDTH:0] s_axis_tdest,
   input  wire [USER_WIDTH-1:0] s_axis_tuser,
   output wire [(0-1)+DATA_WIDTH:0] m_axis_tdata,
   output wire [KEEP_WIDTH-1:0] m_axis_tkeep,
   output wire m_axis_tvalid,
   input  wire m_axis_tready,
   output wire m_axis_tlast,
   output wire [(0-1)+ID_WIDTH:0] m_axis_tid,
   output wire [(0-1)+DEST_WIDTH:0] m_axis_tdest,
   output wire [USER_WIDTH-1:0] m_axis_tuser,
   output wire [$clog2(1+DEPTH)-1:0] count);

  localparam  KEEP_OFFSET = DATA_WIDTH;
  localparam 
       LAST_OFFSET = KEEP_OFFSET+(KEEP_ENABLE ? KEEP_WIDTH : 0);
  localparam  ID_OFFSET = (LAST_ENABLE ? 1 : 0)+LAST_OFFSET;
  localparam 
       DEST_OFFSET = ID_OFFSET+(ID_ENABLE ? ID_WIDTH : 0);
  localparam 
       USER_OFFSET = DEST_OFFSET+(DEST_ENABLE ? DEST_WIDTH : 0);
  localparam 
       WIDTH = (USER_ENABLE ? USER_WIDTH : 0)+USER_OFFSET;
  reg  [WIDTH+(0-1):0] data_reg[(0-1)+DEPTH:0];
  reg  [$clog2(1+DEPTH)-1:0]  ptr_reg = 0;
  reg   full_reg = 1'b0,full_next;
  reg   empty_reg = 1'b1,empty_next;
  wire [WIDTH+(0-1):0] s_axis;
  wire [WIDTH+(0-1):0]  m_axis = data_reg[ptr_reg+(0-1)];

  assign s_axis_tready = !full_reg;
  
  generate
      assign s_axis[(0-1)+DATA_WIDTH:0] = s_axis_tdata;
  ...
  assign m_axis_tdata = m_axis[(0-1)+DATA_WIDTH:0];
  ...
  wire  ptr_full1 = ptr_reg == ((0-1)+DEPTH);
  ...
  always @* 
      begin
        ...
        if (s_axis_tvalid && m_axis_tready && s_axis_tready) 
          begin
            shift = 1'b1;
            inc = ptr_empty;
            empty_next = 1'b0;
