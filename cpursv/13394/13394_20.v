...
module axis_srl_fifo  #(parameter 
  ...
  (input  wire clk,
   ...
   output wire [$clog2(DEPTH+1)+(0-1):0] count);
  ...
  reg  [(0-1)+WIDTH:0] data_reg[DEPTH+(0-1):0];
  reg  [$clog2(DEPTH+1)+(0-1):0]  ptr_reg = 0;
  reg   full_reg = 1'b0,full_next;
  reg   empty_reg = 1'b1,empty_next;
  wire [(0-1)+WIDTH:0] s_axis;
  wire [(0-1)+WIDTH:0]  m_axis = data_reg[(0-1)+ptr_reg];
