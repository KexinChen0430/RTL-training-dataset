...
module mig_7series_v2_3_tg_status  #(parameter  TCQ = 100, DWIDTH = 32)
  (input  clk_i,
   ...
   input  data_error_i,
   input  [(-1)+DWIDTH:0] cmp_data_i,
   input  [(-1)+DWIDTH:0] rd_data_i,
   input  [31:0] cmp_addr_i,
   input  [5:0] cmp_bl_i,
   input  mcb_cmd_full_i,
   input  mcb_wr_full_i,
   input  mcb_rd_empty_i,
   output reg [(-1)+(64+(1<<1*DWIDTH)):0] error_status,
   output error);
          ...
          begin
            if (data_error_i && (~data_error_r && ~error_set)) 
              begin
                ...
                error_set <= #TCQ 1'b1;
                error_status[(DWIDTH+64)+(-1):64] <= #TCQ cmp_data_i;
                error_status[(-1)+(64+(1<<1*DWIDTH)):DWIDTH+64] <= #TCQ rd_data_i;
              end
