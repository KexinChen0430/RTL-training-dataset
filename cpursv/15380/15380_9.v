...
module mig_7series_v2_3_tg_status  #(parameter  TCQ = 100, DWIDTH = 32)
  (input  clk_i,
   input  rst_i,
   ...
   output reg [64+((DWIDTH*1<<<1)+(0-1)):0] error_status,
   output error);
          ...
          begin
            if (data_error_i && (~data_error_r && ~error_set)) 
              begin
                ...
                error_status[64+((DWIDTH*1<<<1)+(0-1)):64+DWIDTH] <= #TCQ rd_data_i;
              end
