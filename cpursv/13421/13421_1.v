...
module raycast_slave(wb_clk,wb_rst,wb_adr_i,wb_dat_i,wb_we_i,wb_cyc_i,wb_stb_i,
                     wb_cti_i,wb_bte_i,wb_dat_o,wb_ack_o,wb_err_o,wb_rty_o,
                     rayc_start_o,ray_buf_adr_o,ray_buf_count_o,octree_adr_o,
                     fb_adr_o,rayc_finished_i,cache_hits_i,cache_miss_i,irq_o);
  ...
  always @(posedge wb_clk)
      if (wb_rst) 
        ...
        begin
          if ((wb_adr_i == 1) && wb_stb_i) status_reg <= 8'b0;
          else status_reg <= {7'b0,rayc_finished_i} | status_reg;
        end
  
  always @(posedge wb_clk)
      if (wb_rst) wb_ack_o <= 0;
      else if (wb_ack_o) wb_ack_o <= 0;
      else if (!wb_ack_o & wb_stb_i) wb_ack_o <= 1;
