...
module FixedRoundSigned(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        ...
        num_comp <= (num_orig[(-1)+num_width] == 0) ? {num_orig[(-1)+num_width],num_orig[(fixed_pos+(-1))+res_width:fixed_pos]} : {num_orig[(-1)+num_width],~num_orig[(fixed_pos+(-1))+res_width:fixed_pos]+1};
        reg_overflow <= (num_orig[(0-2)+num_width:res_width+fixed_pos] == 0) ? 0 : 1;
