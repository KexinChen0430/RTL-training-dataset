...
module FixedRoundSigned(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        ...
        num_comp <= (num_orig[num_width+(-1)] == 0) ? {num_orig[num_width+(-1)],num_orig[((-1)+res_width)+fixed_pos:fixed_pos]} : {num_orig[num_width+(-1)],~num_orig[((-1)+res_width)+fixed_pos:fixed_pos]+1};
