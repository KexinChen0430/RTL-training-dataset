...
module FixedRoundSigned(clk,fixed_num,round,overflow);
  ...
  always @(posedge clk)
      begin
        num_orig <= (fixed_num[(0-1)+num_width] == 0) ? fixed_num : {fixed_num[(0-1)+num_width],~((0-1)+fixed_num[(-1<<<1)+num_width:0])};
        num_comp <= (num_orig[(0-1)+num_width] == 0) ? {num_orig[(0-1)+num_width],num_orig[((0-1)+fixed_pos)+res_width:fixed_pos]} : {num_orig[(0-1)+num_width],1+~num_orig[((0-1)+fixed_pos)+res_width:fixed_pos]};
