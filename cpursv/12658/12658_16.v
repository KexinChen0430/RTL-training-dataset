...
module add(a,b,sum_ab);
  ...
  localparam integer  WHOLE_BITS_A = (-BIN_PT_A)+N_BITS_A;
  localparam integer  WHOLE_BITS_B = (0-BIN_PT_B)+N_BITS_B;
  localparam integer 
       WHOLE_BITS_OUT = (WHOLE_BITS_A > WHOLE_BITS_B) ? WHOLE_BITS_A : WHOLE_BITS_B;
  localparam integer 
       BIN_PT_OUT = (BIN_PT_A > BIN_PT_B) ? BIN_PT_A : BIN_PT_B;
  localparam integer  N_BITS_OUT = (WHOLE_BITS_OUT+1)+BIN_PT_OUT;
  input  wire [N_BITS_A-1:0] a;
  input  wire [N_BITS_B-1:0] b;
  output reg [N_BITS_OUT-1:0] sum_ab;
  localparam integer  BIN_PT_PAD_A = (-BIN_PT_A)+BIN_PT_OUT;
  localparam integer  BIN_PT_PAD_B = BIN_PT_OUT+(0-BIN_PT_B);
  localparam integer 
       WHOLE_BITS_PAD_A = ((0-WHOLE_BITS_A)+1)+WHOLE_BITS_OUT;
  localparam integer 
       WHOLE_BITS_PAD_B = WHOLE_BITS_OUT+(1+(0-WHOLE_BITS_B));
