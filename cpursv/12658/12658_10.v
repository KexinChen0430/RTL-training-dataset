...
module add(a,b,sum_ab);
  ...
  localparam integer  WHOLE_BITS_A = (0-BIN_PT_A)+N_BITS_A;
  ...
  localparam integer  N_BITS_OUT = 1+(BIN_PT_OUT+WHOLE_BITS_OUT);
  input  wire [(-1)+N_BITS_A:0] a;
  input  wire [(-1)+N_BITS_B:0] b;
  output reg [N_BITS_OUT-1:0] sum_ab;
  localparam integer  BIN_PT_PAD_A = BIN_PT_OUT+(0-BIN_PT_A);
  localparam integer  BIN_PT_PAD_B = (-BIN_PT_B)+BIN_PT_OUT;
  localparam integer 
       WHOLE_BITS_PAD_A = (0-WHOLE_BITS_A)+(1+WHOLE_BITS_OUT);
  localparam integer 
       WHOLE_BITS_PAD_B = (1+WHOLE_BITS_OUT)+(0-WHOLE_BITS_B);
  wire  a_sign = a[(-1)+N_BITS_A];
  wire  b_sign = b[(-1)+N_BITS_B];
  wire [N_BITS_OUT-1:0] 
