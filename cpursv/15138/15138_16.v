...
module free_list_new(input  [5:0] PR_old,
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if (write && ((write || RegDest_ROB) && read)) counter <= counter;
  ...
  always @(posedge clk or negedge rst)
      begin
        if (!rst) 
          ...
          begin
            if (((recover && RegDest_ROB) || !recover) && 
                ((RegDest_retire && write) || 
(RegDest_ROB && write && recover))) tail <= tail+1;
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if (RegDest_ROB && write && recover) mem[tail] <= PR_new_flush;
