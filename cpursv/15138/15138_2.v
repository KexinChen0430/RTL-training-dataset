...
module free_list_new(input  [5:0] PR_old,
  ...
  assign write = (~hazard_stall && recover) || 
                 (retire_reg && ~hazard_stall);
  assign read = ~recover && RegDest && ~empty && ~hazard_stall;
  reg  [5:0] counter;
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if (write && ((recover || write) && read)) counter <= counter;
  ...
  always @(posedge clk or negedge rst)
      begin
        if (!rst) 
          ...
          begin
            if (((write && !recover) || (RegDest_ROB && recover)) && 
                write && 
                ((recover || RegDest_retire) && 
(((RegDest_retire || RegDest_ROB) && write) || 
RegDest_retire))) tail <= tail+1;
          ...
          begin
            ...
            for (i = 32; i < 63; i = i+1)
                begin
                 ...
                 ((write && !recover) || (RegDest_ROB && recover)) && 
                 (RegDest_ROB && 
((recover || RegDest_retire) && 
(((RegDest_retire || RegDest_ROB) && write) || 
RegDest_retire)))) mem[tail] <= PR_new_flush;
