...
module free_list_new(input  [5:0] PR_old,
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if ((RegDest_ROB || write) && read && write) counter <= counter;
...
(RegDest_ROB && recover)) && (write || (RegDest_ROB && recover)))) tail <= 1+tail;
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if (recover && write && (RegDest_ROB && write)) mem[tail] <= PR_new_flush;
        else if (!recover && RegDest_retire && write) mem[tail] <= PR_old;
