...
module free_list_new(input  [5:0] PR_old,
  ...
  assign write = (~hazard_stall || recover) && (retire_reg || recover) && 
                 ~hazard_stall;
...
(
(
((RegDest_retire && (!recover && write)) || recover) && write) || RegDest_retire) && (RegDest_ROB || RegDest_retire) && 
                 (RegDest_ROB && recover) && write) mem[tail] <= PR_new_flush;
