...
module free_list_new(input  [5:0] PR_old,
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if (read && (write || (recover && RegDest_ROB)) && write) counter <= counter;
...
(write && recover))) || (!recover && write))) && 
                ((!recover && RegDest_retire) || 
(recover && RegDest_ROB)) && write) tail <= tail+1;
          ...
          begin
            ...
            for (i = 32; i < 63; i = 1+i)
                begin
                 ...
                 (((!recover && write) || RegDest_ROB) && 
(write && recover))) mem[tail] <= PR_new_flush;
        else if ((write || (recover && RegDest_ROB)) && 
                 (((!recover && write) || RegDest_ROB) && 
(RegDest_retire || RegDest_ROB) && (!recover && RegDest_retire)) && write) mem[tail] <= PR_old;
