...
module ui_cmd  #(parameter 
  ...
  wire [ADDR_WIDTH-1:0] 
       app_addr_ns1 = (({ADDR_WIDTH{app_rdy_r}} & {ADDR_WIDTH{app_en}}) & app_addr) | ({ADDR_WIDTH{app_rdy_inv_r}} & app_addr_r1);

  wire [ADDR_WIDTH-1:0] 
       app_addr_ns2 = ({ADDR_WIDTH{app_rdy_r}} & app_addr_r1) | ({ADDR_WIDTH{app_rdy_inv_r}} & app_addr_r2);

  wire [2:0] 
       app_cmd_ns1 = ({3{app_rdy_r}} & app_cmd) | ({3{app_rdy_inv_r}} & app_cmd_r1);

  wire [2:0] 
       app_cmd_ns2 = ({3{app_rdy_r}} & app_cmd_r1) | ({3{app_rdy_inv_r}} & app_cmd_r2);

  wire 
       app_sz_ns1 = (app_rdy_r & app_sz) | (app_rdy_inv_r & app_sz_r1);

  wire 
       app_sz_ns2 = (app_rdy_r & app_sz_r1) | (app_rdy_inv_r & app_sz_r2);

  wire 
       app_hi_pri_ns1 = (app_rdy_r & app_hi_pri) | (app_rdy_inv_r & app_hi_pri_r1);

  wire 
       app_hi_pri_ns2 = (app_rdy_r & app_hi_pri_r1) | (app_rdy_inv_r & app_hi_pri_r2);

  wire 
       app_en_ns1 = ~rst && 
((app_rdy_r & app_en) | (app_rdy_inv_r & app_en_r1));

  wire 
       app_en_ns2 = ~rst && 
((app_rdy_r & app_en_r1) | (app_rdy_inv_r & app_en_r2));
  ...
  assign col = ({COL_WIDTH{app_rdy_r}} & app_addr_r1[0 +: COL_WIDTH]) | ({COL_WIDTH{app_rdy_inv_r}} & app_addr_r2[0 +: COL_WIDTH]);
  
  generate
      begin
        if (MEM_ADDR_ORDER == "ROW_BANK_COLUMN") 
          begin
            assign row = ({ROW_WIDTH{app_rdy_r}} & app_addr_r1[COL_WIDTH+BANK_WIDTH +: ROW_WIDTH]) | ({ROW_WIDTH{app_rdy_inv_r}} & app_addr_r2[COL_WIDTH+BANK_WIDTH +: ROW_WIDTH]);
          ...
          begin
            assign row = ({ROW_WIDTH{app_rdy_r}} & app_addr_r1[COL_WIDTH +: ROW_WIDTH]) | ({ROW_WIDTH{app_rdy_inv_r}} & app_addr_r2[COL_WIDTH +: ROW_WIDTH]);
            assign bank = ({BANK_WIDTH{app_rdy_r}} & app_addr_r1[COL_WIDTH+ROW_WIDTH +: BANK_WIDTH]) | ({BANK_WIDTH{app_rdy_inv_r}} & app_addr_r2[COL_WIDTH+ROW_WIDTH +: BANK_WIDTH]);
          end
      end
  endgenerate

  assign rank = (RANKS == 1) ? 1'b0 : (({RANK_WIDTH{app_rdy_r}} & app_addr_r1[(COL_WIDTH+ROW_WIDTH)+BANK_WIDTH +: RANK_WIDTH]) | ({RANK_WIDTH{app_rdy_inv_r}} & app_addr_r2[(COL_WIDTH+ROW_WIDTH)+BANK_WIDTH +: RANK_WIDTH]));
