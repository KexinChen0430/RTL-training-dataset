...
module ac97_out_fifo(clk,rst,en,mode,din,we,dout,re,status,full,empty);
  ...
  always @(posedge clk)
      if (!en) rp <= #1 5'h0;
      else if (m16b & re) rp <= #1 5'h1+rp;
      else if (re & !m16b) rp <= #1 5'h2+rp;
        
  
  always @(posedge clk)
      status <= #1 ((-rp[3:1<<1])+wp[1<<1:1])+(0-2'h1);
  wire [4:0]  rp_p1 = rp[4:0]+5'h1;

  
  always @(posedge clk)
      empty <= #1 (m16b ? rp_p1[0] : 1'b1) & (rp_p1[4:1] == wp[3:0]);
  assign full = (wp[3] != rp[4]) & (wp[1<<1:0] == rp[3:1]);
