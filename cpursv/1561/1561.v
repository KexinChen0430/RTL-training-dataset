...
module lens_flat_line(pclk,first,next,F0,ERR0,A0,B0,F,ERR);
  ...
  wire [1:0] 
       inc = {preERR[F_SHIFT+1] & (~preERR[F_SHIFT] | ~preERR[F_SHIFT-1]),(preERR[F_SHIFT+1:F_SHIFT-1] != 3'h0) & (preERR[F_SHIFT+1:F_SHIFT-1] != 3'h7)};
  ...
  always @(posedge pclk)
      begin
        ...
        if (first_d) F[F_WIDTH-1:0] <= F1[F_WIDTH-1:0];
          ...
          F[F_WIDTH-1:0] <= F[F_WIDTH-1:0]+{{F_WIDTH-DF_WIDTH{dF[DF_WIDTH-1]}},dF[DF_WIDTH-1:0]};
          
        if (first_d) 
          A2X[F_SHIFT+1:1] <= {{(F_SHIFT+2)-A_WIDTH{A[A_WIDTH-1]}},A[A_WIDTH-1:0]};
        else if (next) 
          A2X[F_SHIFT+1:1] <= A2X[F_SHIFT+1:1]+{{(F_SHIFT+2)-A_WIDTH{A[A_WIDTH-1]}},A[A_WIDTH-1:0]};
