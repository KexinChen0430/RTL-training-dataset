...
module lens_flat_line(pclk,first,next,F0,ERR0,A0,B0,F,ERR);
  ...
  parameter  DF_WIDTH = (B_WIDTH+B_SHIFT)+(-F_SHIFT);
  input  pclk;
  input  first;
  input  next;
  ...
  wire [1:0] 
       inc = {preERR[1+F_SHIFT] & ((~preERR[(-1)+F_SHIFT] & (preERR[1+F_SHIFT] | (~preERR[F_SHIFT] & (~preERR[(-1)+F_SHIFT] | preERR[1+F_SHIFT])))) | ~preERR[F_SHIFT]),(preERR[1+F_SHIFT:(-1)+F_SHIFT] != 3'h0) & (preERR[1+F_SHIFT:(-1)+F_SHIFT] != 3'h7)};
      ...
      begin
        ...
        if (first) 
          begin
            ...
            ApB[1+F_SHIFT:0] <= {{(-A_WIDTH)+(F_SHIFT+1<<<1){A0[A_WIDTH-1]}},A0[A_WIDTH-1:0]}+{B0[B_WIDTH-1:0],{F_SHIFT-B_SHIFT{1'b0}}};
        ...
        if (first_d) 
          A2X[1+F_SHIFT:1] <= {{(-A_WIDTH)+(F_SHIFT+1<<<1){A[A_WIDTH-1]}},A[A_WIDTH-1:0]};
        else if (next) 
          A2X[1+F_SHIFT:1] <= A2X[1+F_SHIFT:1]+{{(-A_WIDTH)+(F_SHIFT+1<<<1){A[A_WIDTH-1]}},A[A_WIDTH-1:0]};
