...
module lens_flat_line(pclk,first,next,F0,ERR0,A0,B0,F,ERR);
  ...
  reg  [A_WIDTH-1:0] A;
  wire [1+F_SHIFT:0] 
       preERR = ((-{dF[1:0],{F_SHIFT{1'b0}}})+{A2X[1+F_SHIFT:1],1'b0})+ApB[1+F_SHIFT:0];
  wire [1:0] 
       inc = {(preERR[1+F_SHIFT] & ~preERR[(-1)+F_SHIFT]) | (~preERR[F_SHIFT] & preERR[1+F_SHIFT]),(preERR[1+F_SHIFT:(-1)+F_SHIFT] != 3'h7) & (preERR[1+F_SHIFT:(-1)+F_SHIFT] != 3'h0)};
      ...
      begin
        ...
        if (first) 
          begin
            ...
            dF[DF_WIDTH-1:0] <= B0[B_WIDTH-1:F_SHIFT+(0-B_SHIFT)];
            ERR[1+F_SHIFT:0] <= ERR0[1+F_SHIFT:0];
            ApB[1+F_SHIFT:0] <= {B0[B_WIDTH-1:0],{F_SHIFT+(0-B_SHIFT){1'b0}}}+{{1<<<1+(F_SHIFT+(-A_WIDTH)){A0[A_WIDTH-1]}},A0[A_WIDTH-1:0]};
        ...
        if (first_d) 
          ...
          A2X[1+F_SHIFT:1] <= {{1<<<1+(F_SHIFT+(-A_WIDTH)){A[A_WIDTH-1]}},A[A_WIDTH-1:0]}+A2X[1+F_SHIFT:1];
