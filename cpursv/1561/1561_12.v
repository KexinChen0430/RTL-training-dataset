...
module lens_flat_line(pclk,first,next,F0,ERR0,A0,B0,F,ERR);
  ...
  wire [1:0] 
       inc = {preERR[F_SHIFT+1] & (~preERR[F_SHIFT] | ((preERR[F_SHIFT+1] | ~preERR[F_SHIFT]) & ~preERR[F_SHIFT-1])),(preERR[F_SHIFT+1:F_SHIFT-1] != 3'h0) & (preERR[F_SHIFT+1:F_SHIFT-1] != 3'h7)};
      ...
      begin
        ...
        if (first) 
          begin
            ...
            dF[(-1)+DF_WIDTH:0] <= B0[B_WIDTH-1:F_SHIFT-B_SHIFT];
            ERR[F_SHIFT+1:0] <= ERR0[F_SHIFT+1:0];
            ApB[F_SHIFT+1:0] <= {{1<<<1+((-A_WIDTH)+F_SHIFT){A0[A_WIDTH-1]}},A0[A_WIDTH-1:0]}+{B0[B_WIDTH-1:0],{F_SHIFT-B_SHIFT{1'b0}}};
        ...
        if (first_d) F[F_WIDTH-1:0] <= F1[F_WIDTH-1:0];
          ...
          F[F_WIDTH-1:0] <= F[F_WIDTH-1:0]+{{F_WIDTH-DF_WIDTH{dF[(-1)+DF_WIDTH]}},dF[(-1)+DF_WIDTH:0]};
          
        if (first_d) 
          A2X[F_SHIFT+1:1] <= {{1<<<1+((-A_WIDTH)+F_SHIFT){A[A_WIDTH-1]}},A[A_WIDTH-1:0]};
        else if (next) 
          A2X[F_SHIFT+1:1] <= A2X[F_SHIFT+1:1]+{{1<<<1+((-A_WIDTH)+F_SHIFT){A[A_WIDTH-1]}},A[A_WIDTH-1:0]};
