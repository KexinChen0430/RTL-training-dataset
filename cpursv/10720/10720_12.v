...
module if_stage(input  clk,rst,
  ...
  wire 
       mem_op = (IF_ID_instruction[31:26] == LW) || 
(IF_ID_instruction[31:26] == SW);

  assign next_i_addr = 4+pc_reg;
  assign i_read_en = 1;
  ...
  always @(posedge clk)
      begin
        if (rst) pc_reg <= 0;
          ...
          begin
            if (pc_write && !(mem_op || pstop_i)) pc_reg <= pc_next;
          ...
          begin
            if (if_id_write_en) 
              begin
                ...
                IF_ID_instruction <= !(mem_op || pstop_i) ? i_instr_in : 0;
              end
