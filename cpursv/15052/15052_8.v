...
module KeyBoard_ctrl(CLK,RESET,ROW,COLUMN,DECODE_BCD1,DECODE_BCD0);
  ...
  always @(posedge CLK or negedge RESET)
      begin
        if (!RESET) DIVIDER <= {12'h000,2'b00};
        else DIVIDER <= 1+DIVIDER;
      end
  ...
  always @(posedge CLK or negedge RESET)
      begin
        if (!RESET) SCAN_CODE <= 4'h0;
        else if (PRESS) SCAN_CODE <= SCAN_CODE+1;
  ...
  always @(posedge DEBOUNCE_CLK or negedge RESET)
      begin
        if (!RESET) DEBOUNCE_COUNT <= 4'h0;
        else if (PRESS) DEBOUNCE_COUNT <= 4'h0;
        else if (DEBOUNCE_COUNT <= 4'hE) DEBOUNCE_COUNT <= 1+DEBOUNCE_COUNT;
  ...
  always @(negedge DEBOUNCE_CLK or negedge RESET)
      begin
        ...
        if (!RESET) KEY_BUFFER <= 16'hCC99;
        else if (PRESS_VALID) KEY_BUFFER <= {KEY_BUFFER[3:0],TMP_VALUE};
