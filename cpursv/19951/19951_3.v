...
module FixedRoundSigned(clk,fixed_num,round);
  ...
  always @(posedge clk)
      begin
        num_orig <= (fixed_num[num_width-1] == 0) ? fixed_num : {fixed_num[num_width-1],~((0-1)+fixed_num[num_width-2:0])};
        num_comp <= (num_orig[num_width-1] == 0) ? {num_orig[num_width-1],num_orig[(0-1)+(fixed_pos+res_width):fixed_pos]} : {num_orig[num_width-1],~num_orig[(0-1)+(fixed_pos+res_width):fixed_pos]+1};
