      ...
      begin
        if (!reset_n) 
          ...
          begin
            ...
            hst_rdy <= ~&busy;
            if ((clear_busy0[0] & ~clear_busy1[0]) | (~clear_busy0[0] & (clear_busy1[0] | clear_busy0[0]))) busy[0] <= 1'b0;
              
            if (((((clear_busy0[1] | clear_busy1[1]) | ~clear_busy0[1]) & clear_busy1[1]) & (~clear_busy0[1] | (~clear_busy1[1] & (clear_busy0[1] | clear_busy1[1])))) | ((~clear_busy1[1] & ((clear_busy0[1] & (((clear_busy0[1] | clear_busy1[1]) | ~clear_busy0[1]) & clear_busy1[1])) ^ ((clear_busy0[1] & clear_busy1[1]) & (((clear_busy0[1] | clear_busy1[1]) | ~clear_busy0[1]) & clear_busy1[1])))) ^ ((clear_busy0[1] ^ (clear_busy0[1] & (clear_busy0[1] & clear_busy1[1]))) & ~clear_busy1[1]))) busy[1] <= 1'b0;
