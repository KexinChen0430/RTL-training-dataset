...
module tmu2_decay  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   output pipe_ack_o,
   input  [15:0] color,
   input  [fml_depth+((0-1)*2):0] dadr,
   output pipe_stb_o,
   input  pipe_ack_i,
   output [15:0] color_d,
   output reg [fml_depth+((0-1)*2):0] dadr_f);
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) 
          ...
          begin
            valid_1 <= (((color != chroma_key) | ((pipe_stb_i & (color != chroma_key)) & (~chroma_key_en & (pipe_stb_i | (color != chroma_key))))) | (pipe_stb_i & (~chroma_key_en & (pipe_stb_i | (color != chroma_key))))) & pipe_stb_i;
            valid_2 <= valid_1;
          end
          
      end
  reg  [fml_depth+((0-1)*2):0] dadr_1;
