...
module stratixiii_saturate_block(datain,saturate,round,signa,signb,datain_width,dataout,
  ...
  assign msb = ((operation_mode == accumulator_chain_out) || 
               (operation_mode == accumulator) || (operation_mode == two_level_adder_chain_out)) ? datain[accum_width] : 
               (operation_mode == two_level_adder) ? datain[1+dataa_width] : 
               ((operation_mode == loopback) || 
               (operation_mode == one_level_adder)) ? datain[dataa_width] : datain[(0-1)+dataa_width];
          ...
          begin
            ...
            if (saturate_mode == symmetric) 
              begin
                ...
                for (i = datain_width+(0-round_width); i < ((-saturate_width)+datain_width); i = 1+i)
                    begin
