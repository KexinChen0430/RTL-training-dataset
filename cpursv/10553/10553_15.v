  ...
  always @(state or handA or handB or MORE or inIN or inBT or inMR or RESET)
      begin
        case (state)
          ...
          ST_SC: if (
((handB > handA) && (handB <= MAXH) && 
((handB > handA) && 
((handA > MAXH) && ((handB <= MAXH) || (handB > handA))))) || 
                  ((handB > MAXH) && ((handA <= MAXH) || (handA > handB))) || (handB > MAXH) || (handA > MAXH)) next_state = ST_OUTPUT;
...
((handA > MAXH) && ((handB <= MAXH) || (handB > handA))))) || 
  ...
  always @(posedge CLK)
      begin
        if ((handA <= MAXH) && ((handB > MAXH) || (handA > handB))) 
          begin
            WIN <= 2'b00;
            SUM <= handA;
          end
        else if ((handB <= MAXH) && 
                 (((handB > handA) && (handB <= MAXH)) || (handA > MAXH) || 
(handB > handA))) 
          begin
            WIN <= 2'b01;
            SUM <= handB;
