...
(((handB > MAXH) || (handA <= MAXH)) && (handA > handB)) || 
(((handA > handB) || (handB > MAXH)) && 
((handB > MAXH) || (handA <= MAXH))) || 
(((handA <= MAXH) || (handA > handB)) && 
((handB > MAXH) || (handA <= MAXH)))) && 
(((handA > MAXH) && (handB <= MAXH)) || 
((handA > handB) && 
((handB > MAXH) && ((handA <= MAXH) || (handA > handB)))))) || 
...
((handB > MAXH) || (handA <= MAXH))) || 
(((handA <= MAXH) || (handA > handB)) && 
((handB > MAXH) || (handA <= MAXH)))) && (handB > MAXH)))) next_state = ST_OUTPUT;
...
((handB > MAXH) || (handA <= MAXH)))) && (handB > MAXH)))) next_state = ST_OUTPUT;
  ...
  always @(posedge CLK)
      begin
        if ((handA <= MAXH) && ((handA > handB) || (handB > MAXH))) 
          begin
            WIN <= 2'b00;
            SUM <= handA;
          end
                 ...
                 (
(((handA > MAXH) || (handB <= MAXH)) && 
((handB <= MAXH) || (handB > handA)) && (handB > handA)) || (handA > MAXH))) 
          begin
            WIN <= 2'b01;
            SUM <= handB;
          end
