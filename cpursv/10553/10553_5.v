...
((handB > MAXH) || (handA <= MAXH)))) || (handA > MAXH) || 
                  (
((handA > MAXH) && 
((handB > handA) || ((handA > MAXH) && (handB <= MAXH))) && 
((handB > handA) && 
(
((handB <= MAXH) && 
((handB > handA) || ((handA > MAXH) && (handB <= MAXH)) || 
(handA > MAXH))) || (handA > MAXH)))) || (handB > MAXH))) next_state = ST_OUTPUT;
...
(handA > MAXH))) || (handA > MAXH)))) || (handB > MAXH))) next_state = ST_OUTPUT;
  ...
  always @(posedge CLK)
      begin
        if (((handB > MAXH) || (handA > handB)) && (handA <= MAXH)) 
          begin
            WIN <= 2'b00;
            SUM <= handA;
          end
