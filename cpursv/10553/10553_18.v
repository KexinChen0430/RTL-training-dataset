  ...
  always @(state or handA or handB or MORE or inIN or inBT or inMR or RESET)
      begin
        case (state)
          ...
          ST_SC: if (
(((handA > MAXH) || (handA <= MAXH)) && 
(
((handB > MAXH) && ((handA > handB) || (handA <= MAXH))) || (handA > MAXH))) || 
                  (
((handA > MAXH) || 
(((handA <= MAXH) || (handB > MAXH)) && (handA > handB)) || 
(((handA <= MAXH) || (handB > MAXH)) && 
((handB > MAXH) || (handA > handB)))) && (handB > MAXH))) next_state = ST_OUTPUT;
...
((handB > MAXH) || (handA > handB)))) && (handB > MAXH))) next_state = ST_OUTPUT;
  ...
  always @(posedge CLK)
      begin
        if (((handB > MAXH) || (handA > handB)) && 
            ((handA > handB) || (handA <= MAXH)) && (handA <= MAXH)) 
          begin
            WIN <= 2'b00;
            SUM <= handA;
          end
        else if (((handA > MAXH) || (handB > handA)) && (handB <= MAXH)) 
          begin
            WIN <= 2'b01;
            SUM <= handB;
          end
