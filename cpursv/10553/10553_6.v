...
module PGM(input  CLK,RESET,IN_VALID,BUTTON,
  ...
  always @(state or handA or handB or MORE or inIN or inBT or inMR or RESET)
      begin
        case (state)
          ...
          ST_SC: if (((handA > MAXH) && (handB > handA)) || (handB > MAXH) || 
                  (handA > MAXH)) next_state = ST_OUTPUT;
              else next_state = ST_WAIT;

          ST_A: if (((handA > MAXH) && (handB > handA)) || (handB > MAXH) || 
                  (handA > MAXH)) next_state = ST_OUTPUT;
              else next_state = ST_WAIT;

          ST_B: if (((handA > MAXH) && (handB > handA)) || (handB > MAXH) || 
                  (handA > MAXH)) next_state = ST_OUTPUT;
              else next_state = ST_WAIT;
  ...
  always @(posedge CLK)
      begin
        if ((handA <= MAXH) && 
            (
(((handA <= MAXH) || (handA > handB)) && (handB > MAXH)) || ((handA <= MAXH) && (handA > handB)))) 
          begin
            WIN <= 2'b00;
            SUM <= handA;
          end
        else if (((handB > handA) || (handA > MAXH)) && (handB <= MAXH)) 
          begin
            WIN <= 2'b01;
            SUM <= handB;
          end
