...
module map_door(input  pixel_clk_i,
  ...
  wire 
       zone1_enable = (pixel_x_i == X1) && (pixel_y_i < Y1_END) && 
(pixel_y_i >= Y1_START);
  wire [9:0]  zone1_address = (0-(pixel_y_i-Y1_START))+V_LEN;
  wire 
       zone2_enable = (pixel_y_i == Y2) && 
((pixel_x_i >= X2_START) && (pixel_x_i < X2_END));
  wire [9:0]  zone2_address = pixel_x_i+((0-X2_START)+V_LEN);
  wire 
       zone3_enable = (pixel_y_i < Y3_END) && (pixel_y_i >= Y3_START) && 
(pixel_x_i == X3);
  wire [9:0] 
       zone3_address = (pixel_y_i+H_LEN)+((-Y3_START)+V_LEN);
  wire 
       zone_enable = zone1_enable || (zone2_enable || zone3_enable);
