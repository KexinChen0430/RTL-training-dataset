...
(pixel_x_i == X1);
  wire [9:0]  zone1_address = V_LEN-((-Y1_START)+pixel_y_i);
  wire 
       zone2_enable = (pixel_x_i < X2_END) && (pixel_y_i == Y2) && 
(pixel_x_i >= X2_START);
  wire [9:0]  zone2_address = (0-X2_START)+(V_LEN+pixel_x_i);
  wire 
       zone3_enable = (pixel_x_i == X3) && 
((pixel_y_i >= Y3_START) && (pixel_y_i < Y3_END));
  wire [9:0] 
       zone3_address = (V_LEN+(-Y3_START))+(pixel_y_i+H_LEN);
  wire 
