...
module map_door(input  pixel_clk_i,
  ...
  wire 
       zone1_enable = (pixel_y_i < Y1_END) && (pixel_x_i == X1) && 
(pixel_y_i >= Y1_START);
  wire [9:0]  zone1_address = V_LEN+(0-(pixel_y_i-Y1_START));
  wire 
       zone2_enable = (pixel_x_i < X2_END) && 
((pixel_x_i >= X2_START) && (pixel_y_i == Y2));
  wire [9:0]  zone2_address = pixel_x_i+(V_LEN+(-X2_START));
  wire 
       zone3_enable = (pixel_y_i >= Y3_START) && (pixel_x_i == X3) && 
(pixel_y_i < Y3_END);
  wire [9:0] 
       zone3_address = (pixel_y_i-Y3_START)+(V_LEN+H_LEN);
  wire 
