...
module map_door(input  pixel_clk_i,
  ...
  wire 
       zone1_enable = (pixel_y_i < Y1_END) && 
((pixel_y_i >= Y1_START) && (pixel_x_i == X1));
  wire [9:0]  zone1_address = (-((-Y1_START)+pixel_y_i))+V_LEN;
  wire 
       zone2_enable = (pixel_x_i >= X2_START) && (pixel_y_i == Y2) && 
(pixel_x_i < X2_END);
  wire [9:0]  zone2_address = pixel_x_i+(V_LEN+(0-X2_START));
  wire 
       zone3_enable = (pixel_y_i < Y3_END) && (pixel_y_i >= Y3_START) && 
(pixel_x_i == X3);
  wire [9:0] 
       zone3_address = (pixel_y_i+(V_LEN+H_LEN))+(0-Y3_START);
  wire 
