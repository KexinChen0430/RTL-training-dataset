...
module map_door(input  pixel_clk_i,
  ...
  parameter  X2_END = H_LEN+X2_START;
  parameter  Y2 = Y_H+VSYNC_VBI_LINE_COUNT;
  parameter  X3 = X_V2;
  parameter  Y3_START = Y_V+VSYNC_VBI_LINE_COUNT;
  parameter  Y3_END = V_LEN+Y3_START;
  wire 
       zone1_enable = (pixel_y_i >= Y1_START) && 
((pixel_y_i < Y1_END) && (pixel_x_i == X1));
  wire [9:0]  zone1_address = V_LEN+(0-((-Y1_START)+pixel_y_i));
  wire 
       zone2_enable = (pixel_x_i >= X2_START) && 
((pixel_x_i < X2_END) && (pixel_y_i == Y2));
  wire [9:0]  zone2_address = (pixel_x_i+V_LEN)+(-X2_START);
  wire 
       zone3_enable = (pixel_y_i < Y3_END) && 
((pixel_x_i == X3) && (pixel_y_i >= Y3_START));
  wire [9:0] 
       zone3_address = (V_LEN+H_LEN)+((-Y3_START)+pixel_y_i);
  wire 
       zone_enable = zone2_enable || (zone1_enable || zone3_enable);

  assign led_strip_address_valid_o = zone_enable && pixel_valid_i;
