...
(pixel_y_i >= Y1_START);
  wire [9:0]  zone1_address = (-(pixel_y_i+(0-Y1_START)))+V_LEN;
  wire 
       zone2_enable = (pixel_x_i < X2_END) && (pixel_x_i >= X2_START) && 
(pixel_y_i == Y2);
  wire [9:0]  zone2_address = (-X2_START)+(pixel_x_i+V_LEN);
  wire 
       zone3_enable = (pixel_y_i < Y3_END) && 
((pixel_y_i >= Y3_START) && (pixel_x_i == X3));
  wire [9:0] 
       zone3_address = H_LEN+((V_LEN+(-Y3_START))+pixel_y_i);
  wire 
