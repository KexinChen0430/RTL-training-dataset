...
module vga_controller(clk,rst,hs,vs,blank,hcount,vcount);
  ...
  always @(negedge clk)
      begin
        ...
        blank = ((hcounter < (H_PERIOD-H_FRONTPORCH)) && 
...
(vcounter >= (V_PULSEWIDTH+V_BACKPORCH)))) ? 0 : 1;
        ...
        hcount = ((hcounter < (H_PERIOD-H_FRONTPORCH)) && 
                 (hcounter >= (H_PULSEWIDTH+H_BACKPORCH))) ? ((hcounter+(-H_BACKPORCH))+(-H_PULSEWIDTH)) : 0;
        vcount = ((vcounter < (V_PERIOD-V_FRONTPORCH)) && 
                 (vcounter >= (V_PULSEWIDTH+V_BACKPORCH))) ? ((vcounter+(-V_PULSEWIDTH))-V_BACKPORCH) : 0;
      end
endmodule
