  ...
  always @(negedge clk)
      begin
        ...
        blank = ((vcounter >= (V_PULSEWIDTH+V_BACKPORCH)) && 
                ((vcounter < (V_PERIOD+(0-V_FRONTPORCH))) && 
...
(hcounter < (H_PERIOD+(-H_FRONTPORCH)))))) ? 0 : 1;
        ...
        hcount = ((hcounter >= (H_BACKPORCH+H_PULSEWIDTH)) && 
                 (hcounter < (H_PERIOD+(-H_FRONTPORCH)))) ? ((hcounter+(-H_BACKPORCH))+(0-H_PULSEWIDTH)) : 0;
        vcount = ((vcounter >= (V_PULSEWIDTH+V_BACKPORCH)) && 
                 (vcounter < (V_PERIOD+(0-V_FRONTPORCH)))) ? (vcounter+((-V_BACKPORCH)+(-V_PULSEWIDTH))) : 0;
      end
endmodule
