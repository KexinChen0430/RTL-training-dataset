...
module vga_controller(clk,rst,hs,vs,blank,hcount,vcount);
  ...
  always @(negedge clk)
      begin
        ...
        blank = ((vcounter >= (V_PULSEWIDTH+V_BACKPORCH)) && 
                ((vcounter < ((-V_FRONTPORCH)+V_PERIOD)) && 
(hcounter < (H_PERIOD+(0-H_FRONTPORCH)))) && (hcounter >= (H_BACKPORCH+H_PULSEWIDTH))) ? 0 : 1;
        hcounter = 1+hcounter;
        ...
        hcount = ((hcounter < (H_PERIOD+(0-H_FRONTPORCH))) && 
                 (hcounter >= (H_BACKPORCH+H_PULSEWIDTH))) ? ((-H_BACKPORCH)+((-H_PULSEWIDTH)+hcounter)) : 0;
        vcount = ((vcounter >= (V_PULSEWIDTH+V_BACKPORCH)) && 
                 (vcounter < ((-V_FRONTPORCH)+V_PERIOD))) ? ((vcounter+(0-V_BACKPORCH))+(-V_PULSEWIDTH)) : 0;
      end
endmodule
