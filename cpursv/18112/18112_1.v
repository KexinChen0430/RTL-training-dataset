...
module vga_controller(clk,rst,hs,vs,blank,hcount,vcount);
  ...
  always @(negedge clk)
      begin
        ...
        blank = ((hcounter < (H_PERIOD+(0-H_FRONTPORCH))) && 
                (hcounter >= (H_PULSEWIDTH+H_BACKPORCH)) && 
                ((vcounter < (V_PERIOD-V_FRONTPORCH)) && 
(vcounter >= (V_PULSEWIDTH+V_BACKPORCH)))) ? 0 : 1;
        hcounter = hcounter+1;
        ...
        hcount = ((hcounter < (H_PERIOD+(0-H_FRONTPORCH))) && 
                 (hcounter >= (H_PULSEWIDTH+H_BACKPORCH))) ? ((0-H_BACKPORCH)+(hcounter-H_PULSEWIDTH)) : 0;
        vcount = ((vcounter < (V_PERIOD-V_FRONTPORCH)) && 
                 (vcounter >= (V_PULSEWIDTH+V_BACKPORCH))) ? ((-V_PULSEWIDTH)+(vcounter+(0-V_BACKPORCH))) : 0;
      end
endmodule
