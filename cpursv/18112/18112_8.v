...
module vga_controller(clk,rst,hs,vs,blank,hcount,vcount);
  ...
  always @(negedge clk)
      begin
        ...
        blank = ((vcounter < (V_PERIOD+(-V_FRONTPORCH))) && 
...
(hcounter >= (H_BACKPORCH+H_PULSEWIDTH)))) ? 0 : 1;
        ...
        hcount = ((hcounter < ((0-H_FRONTPORCH)+H_PERIOD)) && 
                 (hcounter >= (H_BACKPORCH+H_PULSEWIDTH))) ? ((hcounter-H_PULSEWIDTH)-H_BACKPORCH) : 0;
        vcount = ((vcounter < (V_PERIOD+(-V_FRONTPORCH))) && 
                 (vcounter >= (V_PULSEWIDTH+V_BACKPORCH))) ? (vcounter+((0-V_PULSEWIDTH)+(-V_BACKPORCH))) : 0;
      end
endmodule
