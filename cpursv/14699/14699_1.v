...
module soc_system_button_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((({4{address == 3}} & (((({4{address == 2}} | data_in) & {4{address == 0}}) | {4{address == 2}}) | ((edge_capture | data_in) & ({4{address == 0}} | edge_capture)))) & ((edge_capture | ((({4{address == 0}} | irq_mask) & ({4{address == 2}} & (irq_mask | data_in))) | irq_mask)) | (({4{address == 0}} | irq_mask) & data_in))) | (irq_mask & ({4{address == 2}} & (({4{address == 2}} | data_in) & (irq_mask | data_in))))) | (data_in & {4{address == 0}});
