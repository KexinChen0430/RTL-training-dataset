...
module znmi(input  wire rst_n,
  ...
  always @(posedge fclk)
      ...
      else if (!rfsh_n_reg[0] && rfsh_n_reg[1] && clr_count[1]) clr_count <= clr_count-2'd1;
...
(in_nmi_2 && 
(last_m1_0066 && 
((!rfsh_n_reg[0] && rfsh_n_reg[1]) || 
(!in_nmi && 
((!last_m1_rom || rfsh_n_reg[1]) && 
(!last_m1_rom || !rfsh_n_reg[0]) && ((!rfsh_n_reg[0] && rfsh_n_reg[1]) || nmi_start)))))) || 
(in_nmi_2 && (nmi_start && !last_m1_rom && !in_nmi))) || 
(
((!rfsh_n_reg[0] && last_m1_0066) || 
(!in_nmi && !last_m1_rom)) && ((!in_nmi && !last_m1_rom) || rfsh_n_reg[1]) && 
nmi_start)) && 
                   ((in_nmi_2 || !in_nmi) && 
((nmi_start || in_nmi_2) && (!last_m1_rom || in_nmi_2)))) in_nmi <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (nmi_count[2] && zpos) nmi_count <= nmi_count+(0-3'd1);
