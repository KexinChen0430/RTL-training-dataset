...
module znmi(input  wire rst_n,
  ...
  assign nmi_start = imm_nmi_now || (int_start && pending_nmi);
  
  always @(posedge fclk)
      if (clr_nmi) clr_count <= 2'd3;
      else if (clr_count[1] && (rfsh_n_reg[1] && !rfsh_n_reg[0])) clr_count <= (-2'd1)+clr_count;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi <= 1'b0;
        ...
        begin
          ...
          else if ((!rfsh_n_reg[0] || nmi_start) && 
                   ((rfsh_n_reg[1] && last_m1_0066) || nmi_start) && 
                   ((!last_m1_rom && !in_nmi) || 
(rfsh_n_reg[1] && (last_m1_0066 && !rfsh_n_reg[0]))) && 
                   ((!last_m1_rom || in_nmi_2) && 
((nmi_start || in_nmi_2) && (!in_nmi || in_nmi_2)))) in_nmi <= 1'b1;
