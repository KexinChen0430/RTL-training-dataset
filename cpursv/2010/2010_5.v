...
module znmi(input  wire rst_n,
  ...
  assign nmi_start = (int_start && pending_nmi) || imm_nmi_now;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi <= 1'b0;
        ...
        begin
          ...
          else if ((!last_m1_rom && !in_nmi && nmi_start) || 
                   (
((!rfsh_n_reg[0] && in_nmi_2 && rfsh_n_reg[1]) || 
(!last_m1_rom && !in_nmi && nmi_start)) && last_m1_0066)) in_nmi <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (zpos && nmi_count[1<<<1]) nmi_count <= nmi_count-3'd1;
