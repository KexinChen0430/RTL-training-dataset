...
module znmi(input  wire rst_n,
  ...
  always @(posedge fclk)
      ...
      else if (rfsh_n_reg[1] && !rfsh_n_reg[0] && clr_count[1]) clr_count <= (0-2'd1)+clr_count;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi_2 <= 1'b0;
        ...
        begin
          ...
          else if (rfsh_n_reg[1] && !rfsh_n_reg[0] && last_m1_0066) in_nmi_2 <= 1'b0;
...
(last_m1_0066 && (!rfsh_n_reg[0] && in_nmi_2) && 
rfsh_n_reg[1]) || 
                   (!last_m1_rom && !in_nmi && 
((in_nmi_2 || ((in_nmi_2 || !in_nmi) && nmi_start)) && 
(in_nmi_2 || !last_m1_rom)) && nmi_start)) in_nmi <= 1'b1;
