...
module znmi(input  wire rst_n,
  ...
  assign nmi_start = (pending_nmi || imm_nmi_now) && 
                     (int_start || imm_nmi_now);
  
  always @(posedge fclk)
      if (clr_nmi) clr_count <= 2'd3;
      else if (rfsh_n_reg[1] && !rfsh_n_reg[0] && clr_count[1]) clr_count <= clr_count-2'd1;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi_2 <= 1'b0;
        ...
        begin
          ...
          else if (last_m1_0066 && (rfsh_n_reg[1] && !rfsh_n_reg[0])) in_nmi_2 <= 1'b0;
...
(nmi_start || 
(last_m1_0066 && (rfsh_n_reg[1] && !rfsh_n_reg[0]))) && (in_nmi_2 || nmi_start) && 
                   (
(!last_m1_rom || 
((last_m1_0066 || !last_m1_rom) && 
((!rfsh_n_reg[0] || !last_m1_rom) && 
(rfsh_n_reg[1] || !last_m1_rom)) && in_nmi_2)) && 
(
(!in_nmi || 
(rfsh_n_reg[1] && (in_nmi_2 && !rfsh_n_reg[0]))) && (last_m1_0066 || !in_nmi)))) in_nmi <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (nmi_count[1<<<1] && zpos) nmi_count <= nmi_count+(0-3'd1);
