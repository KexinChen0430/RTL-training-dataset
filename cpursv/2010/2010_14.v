...
module znmi(input  wire rst_n,
  ...
  always @(posedge fclk)
      if (!was_m1_reg && was_m1) last_m1_rom <= csrom && (a[15:14] == 2'b00);
  ...
  assign nmi_start = (imm_nmi_now || int_start) && 
                     (imm_nmi_now || pending_nmi);
  
  always @(posedge fclk)
      if (clr_nmi) clr_count <= 2'd3;
      else if (clr_count[1] && (rfsh_n_reg[1] && !rfsh_n_reg[0])) clr_count <= clr_count-2'd1;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi_2 <= 1'b0;
        ...
        begin
          if (nmi_start && !in_nmi && last_m1_rom) in_nmi_2 <= 1'b1;
          else if (rfsh_n_reg[1] && (last_m1_0066 && !rfsh_n_reg[0])) in_nmi_2 <= 1'b0;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi <= 1'b0;
        ...
        begin
          if (pending_clr && !clr_count[1]) in_nmi <= 1'b0;
          else if (
(
(rfsh_n_reg[1] && (last_m1_0066 && !rfsh_n_reg[0]) && 
(!last_m1_rom && nmi_start)) || (nmi_start && (!in_nmi && !last_m1_rom)) || 
(last_m1_0066 && in_nmi_2)) && (!in_nmi || (last_m1_0066 && in_nmi_2)) && 
                   (
(
(((nmi_start && !in_nmi) || !rfsh_n_reg[0]) && 
(rfsh_n_reg[1] || (nmi_start && !in_nmi))) || (nmi_start && (!in_nmi && !last_m1_rom))) && 
((nmi_start && (!in_nmi && !last_m1_rom)) || 
((rfsh_n_reg[1] && !rfsh_n_reg[0]) || !last_m1_rom)))) in_nmi <= 1'b1;
            
        end
  
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) nmi_count <= 3'b000;
      else if (nmi_start && !in_nmi) nmi_count <= 3'b111;
      else if (zpos && nmi_count[1<<<1]) nmi_count <= (0-3'd1)+nmi_count;
