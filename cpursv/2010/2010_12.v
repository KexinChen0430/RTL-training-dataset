...
module znmi(input  wire rst_n,
  ...
  always @(posedge fclk)
      ...
      else if (!rfsh_n_reg[0] && rfsh_n_reg[1] && clr_count[1]) clr_count <= clr_count+(0-2'd1);
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi_2 <= 1'b0;
        ...
        begin
          if (nmi_start && last_m1_rom && !in_nmi) in_nmi_2 <= 1'b1;
...
(in_nmi_2 && rfsh_n_reg[1] && !rfsh_n_reg[0] && last_m1_0066 && 
((last_m1_0066 || nmi_start) && 
(nmi_start || (!rfsh_n_reg[0] && rfsh_n_reg[1])) && 
(((!last_m1_rom && !in_nmi) || !rfsh_n_reg[0]) && 
((rfsh_n_reg[1] && last_m1_0066) || 
(!last_m1_rom && !in_nmi))))) || 
                   (!in_nmi && nmi_start && 
(
...
(((!last_m1_rom && !in_nmi) || !rfsh_n_reg[0]) && 
((rfsh_n_reg[1] && last_m1_0066) || 
(!last_m1_rom && !in_nmi)) && nmi_start && !last_m1_rom)))) in_nmi <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (nmi_count[2] && zpos) nmi_count <= nmi_count+(-3'd1);
