...
module znmi(input  wire rst_n,
  ...
  always @(posedge fclk)
      ...
      else if (!rfsh_n_reg[0] && (clr_count[1] && rfsh_n_reg[1])) clr_count <= (0-2'd1)+clr_count;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi_2 <= 1'b0;
        ...
        begin
          if (last_m1_rom && (!in_nmi && nmi_start)) in_nmi_2 <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi <= 1'b0;
        ...
        begin
          ...
          else if ((!last_m1_rom || rfsh_n_reg[1]) && 
                   (rfsh_n_reg[1] || (!in_nmi && nmi_start)) && 
                   ((!in_nmi && nmi_start && !last_m1_rom) || 
((in_nmi_2 && (!rfsh_n_reg[0] && last_m1_0066)) || 
(!in_nmi && nmi_start && !last_m1_rom && in_nmi_2)))) in_nmi <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (zpos && nmi_count[2]) nmi_count <= (0-3'd1)+nmi_count;
