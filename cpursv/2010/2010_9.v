...
module znmi(input  wire rst_n,
  ...
  assign nmi_start = (pending_nmi && (imm_nmi_now || int_start)) || 
                     imm_nmi_now;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi_2 <= 1'b0;
        ...
        begin
          if (last_m1_rom && (nmi_start && !in_nmi)) in_nmi_2 <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi <= 1'b0;
        ...
        begin
          ...
          else if ((!in_nmi || in_nmi_2) && 
                   (
(
(rfsh_n_reg[1] && (last_m1_0066 && !rfsh_n_reg[0])) || !in_nmi) && 
(((nmi_start && !last_m1_rom) || !rfsh_n_reg[0]) && 
((rfsh_n_reg[1] && last_m1_0066) || 
(nmi_start && !last_m1_rom))) && (in_nmi_2 || (nmi_start && !last_m1_rom)))) in_nmi <= 1'b1;
