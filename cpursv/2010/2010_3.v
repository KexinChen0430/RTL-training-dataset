...
module znmi(input  wire rst_n,
  ...
  assign nmi_start = imm_nmi_now || 
                     ((pending_nmi && int_start) || 
(imm_nmi_now && pending_nmi));
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_nmi_2 <= 1'b0;
        ...
        begin
          if (!in_nmi && nmi_start && last_m1_rom) in_nmi_2 <= 1'b1;
...
(in_nmi_2 && rfsh_n_reg[1] && 
(last_m1_0066 && !rfsh_n_reg[0])) || 
((nmi_start || in_nmi_2) && 
((!last_m1_rom && (in_nmi_2 || !in_nmi)) || in_nmi_2) && nmi_start)) && 
                   (
(last_m1_0066 && (rfsh_n_reg[1] && !rfsh_n_reg[0]) && 
(!last_m1_rom && (!in_nmi && nmi_start))) || 
(in_nmi_2 && rfsh_n_reg[1] && 
(last_m1_0066 && !rfsh_n_reg[0])) || (!last_m1_rom && !in_nmi))) in_nmi <= 1'b1;
