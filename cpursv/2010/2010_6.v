...
module znmi(input  wire rst_n,
  ...
  always @(posedge fclk)
      ...
      else if (!rfsh_n_reg[0] && clr_count[1] && rfsh_n_reg[1]) clr_count <= clr_count-2'd1;
...
(
(
(last_m1_0066 && (rfsh_n_reg[1] && !rfsh_n_reg[0]) && 
in_nmi_2) || 
...
(
(nmi_start && 
((!last_m1_rom || last_m1_0066) && 
((!last_m1_rom || !rfsh_n_reg[0]) && 
(rfsh_n_reg[1] || !last_m1_rom)))) || 
(last_m1_0066 && (rfsh_n_reg[1] && !rfsh_n_reg[0])))))) || (!last_m1_rom && !in_nmi && nmi_start)) in_nmi <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (zpos && nmi_count[2]) nmi_count <= nmi_count-3'd1;
