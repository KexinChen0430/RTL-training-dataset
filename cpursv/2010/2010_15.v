...
module znmi(input  wire rst_n,
  ...
  always @(posedge fclk)
      if (was_m1 && !was_m1_reg) last_m1_rom <= csrom && (a[15:14] == 2'b00);
  ...
  always @(posedge fclk)
      ...
      else if (rfsh_n_reg[1] && clr_count[1] && !rfsh_n_reg[0]) clr_count <= (-2'd1)+clr_count;
...
((nmi_start && !in_nmi) || rfsh_n_reg[1]))) || (nmi_start && (!last_m1_rom && !in_nmi))) && 
                   (last_m1_0066 || 
(nmi_start && (!last_m1_rom && !in_nmi))) && 
                   (in_nmi_2 || (nmi_start && (!last_m1_rom && !in_nmi)))) in_nmi <= 1'b1;
  ...
  always @(posedge fclk or negedge rst_n)
      ...
      else if (zpos && nmi_count[1<<1]) nmi_count <= (-3'd1)+nmi_count;
