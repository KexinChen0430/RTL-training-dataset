...
module key(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ({1{address == 0}} | irq_mask) & (((irq_mask & {1{address == 1<<<1}}) | ({1{address == 1<<<1}} & (data_in & ({1{address == 0}} | irq_mask)))) | (data_in & ((({1{address == 0}} | ((irq_mask & {1{address == 1<<<1}}) & data_in)) | (irq_mask & {1{address == 1<<<1}})) | {1{address == 1<<<1}})));
