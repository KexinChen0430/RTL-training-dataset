...
module round  #(parameter 
  ...
  assign round_corr_rtz = |in[(-bits_out)+(bits_in-1):0] & in[bits_in-1];
  assign round_corr_nearest = in[(-bits_out)+(bits_in-1)];
  
  generate
      if (((-bits_out)+bits_in) > 1) 
        assign round_corr_nearest_safe = (&in[bits_in-(1+1):bits_out] & ~in[bits_in-1]) ? 0 : round_corr_nearest;
  ...
  assign out = round_corr+in[bits_in-1:(-bits_out)+bits_in];
  assign err = (0-{out,{(-bits_out)+bits_in{1'b0}}})+in;
endmodule
