...
module round  #(parameter 
  ...
  assign round_corr_rtz = |in[(bits_in-bits_out)-1:0] & in[(-1)+bits_in];
  assign round_corr_nearest = in[(bits_in-bits_out)-1];
  
  generate
      if ((bits_in-bits_out) > 1) 
        assign round_corr_nearest_safe = (~in[(-1)+bits_in] & &in[bits_in+(0-1<<1):bits_out]) ? 0 : round_corr_nearest;
  ...
  assign out = round_corr+in[(-1)+bits_in:bits_in-bits_out];
