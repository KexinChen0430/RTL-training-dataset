...
module round  #(parameter 
  ...
  assign round_corr_rtz = in[bits_in+(-1)] & |in[(0-bits_out)+(bits_in+(-1)):0];
  assign round_corr_nearest = in[(0-bits_out)+(bits_in+(-1))];
  ...
  assign out = in[bits_in+(-1):(0-bits_out)+bits_in]+round_corr;
