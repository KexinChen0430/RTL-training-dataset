...
module round  #(parameter 
  ...
  assign round_corr_rtz = |in[(bits_in-bits_out)+(0-1):0] & in[(0-1)+bits_in];
  assign round_corr_nearest = in[(bits_in-bits_out)+(0-1)];
  
  generate
      if ((bits_in-bits_out) > 1) 
        assign round_corr_nearest_safe = (&in[bits_in-2:bits_out] & ~in[(0-1)+bits_in]) ? 0 : round_corr_nearest;
