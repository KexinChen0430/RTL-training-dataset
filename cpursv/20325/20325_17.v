...
module definition;
  ...
  always @(*)
      begin
        ...
        list_put_entry_ready = ~list_v[(0-1)+CTL_LIST_DEPTH];
      end
  integer i;
  ...
  always @(posedge ctl_clk or negedge ctl_reset_n)
      begin
        if (~ctl_reset_n) 
          begin
            for (i = 0; i < CTL_LIST_DEPTH; i = 1'b1+i)
                begin
          ...
          begin
            if (list_get) 
              begin
                for (i = 1; i < CTL_LIST_DEPTH; i = 1'b1+i)
                    begin
                      list_v[i+(0-1)] <= list_v[i];
                      list[i+(0-1)] <= list[i];
                    end
                list_v[(0-1)+CTL_LIST_DEPTH] <= 0;
                for (i = 0; i < CTL_LIST_DEPTH; i = 1'b1+i)
                    begin
                    ...
                    for (i = 1; i < CTL_LIST_DEPTH; i = 1'b1+i)
                        begin
                          if (list_v[i+(0-1)] & ~list_v[i]) 
                            begin
                              list_v[i+(0-1)] <= 1'b1;
                              list[i+(0-1)] <= list_put_entry_id;
                            end
                            
                        end
                    for (i = 0; i < CTL_LIST_DEPTH; i = 1'b1+i)
                        begin
                          if (~list_v[1] & list_v[0]) 
                            begin
                              list_vector[i] <= (i == list_put_entry_id) ? 1'b1 : 1'b0;
                            end
                          else 
