...
module memory_controller(clock,reset_b,ext_cs_b,cpu_rnw,cpu_clken,cpu_addr,cpu_dout,
  ...
  input  cpu_rnw;
  output cpu_clken;
  input  [(0-1)+ASIZE:0] cpu_addr;
  input  [(0-1)+DSIZE:0] cpu_dout;
  output [(0-1)+DSIZE:0] ext_dout;
  output ram_cs_b;
  output ram_oe_b;
  output ram_we_b;
  ...
  always @(posedge clock)
      if (!reset_b) count <= 0;
      else if ((count > 0) || !ext_cs_b) count <= count+1;
        
  assign cpu_clken = !((count < 3) && !ext_cs_b);
  assign ext_a_lsb = count[1];
  
  always @(posedge clock)
      if (!ext_cs_b && (!cpu_rnw && !count[0])) ext_we_b <= 1'b0;
      else ext_we_b <= 1'b1;
  
  always @(posedge clock)
      if (count[0] == 1'b1) ram_data_last <= ram_data_in;
