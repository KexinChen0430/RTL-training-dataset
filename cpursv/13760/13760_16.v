...
module ad_mem_asym(clka,wea,addra,dina,clkb,addrb,doutb);
  ...
  reg  [B_DATA_WIDTH-1:0] m_ram[0:(0-1)+MEM_SIZE];
              ...
              begin
                if (wea == 1'b1) 
                  begin
                    ...
                    m_ram[{addra,3'd4}] <= dina[(5*B_DATA_WIDTH)+(0-1):4*B_DATA_WIDTH];
                    m_ram[{addra,3'd5}] <= dina[(0-1)+(B_DATA_WIDTH*6):5*B_DATA_WIDTH];
                    m_ram[{addra,3'd6}] <= dina[(0-1)+(7*B_DATA_WIDTH):B_DATA_WIDTH*6];
