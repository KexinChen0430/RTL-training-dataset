...
module joystick_protocols(input  wire clk,
  ...
  always @* 
      begin
        ...
        else if ((a[7:0] == KEMPSTONADDR) && (rd_n == 1'b0) && 
                 (iorq_n == 1'b0)) 
          begin
            dout = 8'h00;
            oe_n = 1'b0;
            ...
            if (joyconf[6:4] == KEMPSTON) 
              dout = dout | {3'b000,db9joyfire_processed,db9joyup,db9joydown,db9joyleft,db9joyright};
                 ...
                 ((iorq_n == 1'b0) && (a[7:0] == FULLERADDR))) 
          begin
            dout = 8'hFF;
            oe_n = 1'b0;
            if (joyconf[2:0] == FULLER) 
              dout = {~kbdjoyfire_processed,3'b111,~kbdjoyright,~kbdjoyleft,~kbdjoydown,~kbdjoyup} & dout;
              
            if (joyconf[6:4] == FULLER) 
              dout = {~db9joyfire_processed,3'b111,~db9joyright,~db9joyleft,~db9joydown,~db9joyup} & dout;
              
          end
        else if ((iorq_n == 1'b0) && (rd_n == 1'b0) && 
                 ((a[SINCLAIRP1ADDR] == 1'b0) && (a[0] == 1'b0))) 
          begin
            ...
            if (joyconf[6:4] == CURSOR) 
              kbdcol_out = {~db9joydown,~db9joyup,~db9joyright,1'b1,~db9joyfire_processed} & kbdcol_out;
              
          end
        else if ((a[0] == 1'b0) && (rd_n == 1'b0) && 
                 ((a[SINCLAIRP2ADDR] == 1'b0) && (iorq_n == 1'b0))) 
          begin
            ...
            if (joyconf[6:4] == CURSOR) kbdcol_out = kbdcol_out & {~db9joyleft,4'b1111};
