...
module joystick_protocols(input  wire clk,
  ...
  always @(posedge clk)
      begin
        if ((zxuno_regwr == 1'b1) && (zxuno_addr == JOYCONFADDR)) joyconf <= din;
  ...
  always @(posedge clk)
      begin
        ...
        if (edge_detect == 4'b0011) cont_autofire <= 1+cont_autofire;
          
      end
  wire 
       kbdjoyfire_processed = (joyconf[3] == 1'b0) ? kbdjoyfire : (kbdjoyfire & autofire);

  wire 
       db9joyfire_processed = (joyconf[7] == 1'b0) ? db9joyfire : (db9joyfire & autofire);
  ...
  always @* 
      begin
        ...
        if ((zxuno_addr == JOYCONFADDR) && (zxuno_regrd == 1'b1)) 
          begin
            oe_n = 1'b0;
            dout = joyconf;
          end
        else if ((a[7:0] == KEMPSTONADDR) && (iorq_n == 1'b0) && 
                 (rd_n == 1'b0)) 
          begin
            dout = 8'h00;
            oe_n = 1'b0;
        ...
        else if ((a[7:0] == FULLERADDR) && (iorq_n == 1'b0) && 
                 (rd_n == 1'b0)) 
          begin
            dout = 8'hFF;
            oe_n = 1'b0;
            ...
            if (joyconf[6:4] == FULLER) 
              dout = {~db9joyfire_processed,3'b111,~db9joyright,~db9joyleft,~db9joydown,~db9joyup} & dout;
                 ...
                 ((a[0] == 1'b0) && (iorq_n == 1'b0) && 
(a[SINCLAIRP1ADDR] == 1'b0))) 
          begin
            ...
            if (joyconf[6:4] == SINCLAIRP1) 
              kbdcol_out = {~db9joyleft,~db9joyright,~db9joydown,~db9joyup,~db9joyfire_processed} & kbdcol_out;
              
            if (joyconf[2:0] == CURSOR) 
              kbdcol_out = {~kbdjoydown,~kbdjoyup,~kbdjoyright,1'b1,~kbdjoyfire_processed} & kbdcol_out;
              
            if (joyconf[6:4] == CURSOR) 
              kbdcol_out = kbdcol_out & {~db9joydown,~db9joyup,~db9joyright,1'b1,~db9joyfire_processed};
              
          end
        else if ((a[0] == 1'b0) && (iorq_n == 1'b0) && 
                 (a[SINCLAIRP2ADDR] == 1'b0) && (rd_n == 1'b0)) 
          begin
            if (joyconf[2:0] == SINCLAIRP2) 
              kbdcol_out = {~kbdjoyfire_processed,~kbdjoyup,~kbdjoydown,~kbdjoyright,~kbdjoyleft} & kbdcol_out;
              
            if (joyconf[6:4] == SINCLAIRP2) 
              kbdcol_out = kbdcol_out & {~db9joyfire_processed,~db9joyup,~db9joydown,~db9joyright,~db9joyleft};
              
            if (joyconf[2:0] == CURSOR) kbdcol_out = kbdcol_out & {~kbdjoyleft,4'b1111};
