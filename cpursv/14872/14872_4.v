...
module ComWithCC3200(input  CC3200_SPI_CLK,
                     input  CC3200_SPI_CS,
  ...
  always @(negedge CC3200_SPI_CLK or negedge CC3200_SPI_CS)
      begin
        if (~CC3200_SPI_CS) 
          begin
            SPI_Counter <= 8'd0;
          ...
          begin
            if (~Envelop) 
              begin
                SPI_Counter <= 1'b1+SPI_Counter;
                CC3200_SPI_DIN <= Shift_Data[7];
                Shift_Data <= 2*Shift_Data;
              end
              
          end
      end
  
  always @(posedge CC3200_SPI_CS or posedge Envelop)
      begin
        if (Envelop) 
          begin
            Trans_Addr <= 9'd0;
