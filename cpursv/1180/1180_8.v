...
module works_only_with_16_32_64_gtx_input_data_width;
  ...
  always @(posedge clk)
      rxcom_timer <= ((((state_wait_cominit & rxcominitdet) | (rxcomwakedet & state_wait_comwake)) | (state_wait_comwake & rxcomwake_done)) | (rst | ((rst | rxcominit_done) & state_wait_cominit))) ? 10'h0 : 
                    ((((rxcomwakedet_l | state_idle) & (rxcomwakedet_l | (((rxcominitdet_l | cominit_req_l) & (state_idle | rxcominitdet_l)) & cominit_req_l))) | (rxcominitdet_l & ((state_wait_cominit | cominit_req_l) & (state_idle | state_wait_cominit)))) & (state_wait_comwake | (((state_wait_cominit | cominit_req_l) & (state_idle | state_wait_cominit)) & ((rxcominitdet_l | cominit_req_l) & (state_idle | rxcominitdet_l))))) ? (CLK_TO_TIMER_CONTRIB[9:0]+rxcom_timer) : 10'h0;
