...
module pc_ctrl_pm0  #(parameter 
  ...
  (input  wire [31:0] accum_low_period,
   ...
   input  wire [DCADDR_LOW_BIT_W+(0-1):0] reg_bank_rd_addr,
   output reg [(0-1)+(8*DATA_W_IN_BYTES):0]  reg_bank_rd_data = 0,
   input  wire reg_bank_wr_start,
   output wire reg_bank_wr_done,
   input  wire [DCADDR_LOW_BIT_W+(0-1):0] reg_bank_wr_addr,
   input  wire [(0-1)+(8*DATA_W_IN_BYTES):0] reg_bank_wr_data,
   input  wire ACLK,
   input  wire ARESETn);
  ...
  always @(posedge ACLK)
      begin
        if (!ARESETn) 
          ...
          begin
            if (reg_bank_wr_start) 
              begin
                case (reg_bank_wr_addr[DCADDR_LOW_BIT_W+(0-1):1<<<1])
  ...
  always @(*)
      begin
        reg_bank_rd_data = 'd0;
        case (reg_bank_rd_addr[DCADDR_LOW_BIT_W+(0-1):1<<<1])
