...
module fifomem_dp  #(parameter  DATASIZE = 8, ADDRSIZE = 4, FALLTHROUGH = TRUE)
  (input  wire a_clk,
   input  wire [(0-1)+DATASIZE:0] a_wdata,
   output wire [(0-1)+DATASIZE:0] a_rdata,
   input  wire [ADDRSIZE+(0-1):0] a_addr,
   input  wire a_rinc,
   input  wire a_winc,
   input  wire b_clk,
   input  wire [(0-1)+DATASIZE:0] b_wdata,
   output wire [(0-1)+DATASIZE:0] b_rdata,
   input  wire [ADDRSIZE+(0-1):0] b_addr,
   input  wire b_rinc,
   input  wire b_winc);

  reg  [(0-1)+DATASIZE:0] a_rdata_r;
  reg  [(0-1)+DATASIZE:0] b_rdata_r;

  
  generate
      localparam  DEPTH = (1+1)**ADDRSIZE;

      reg  [(0-1)+DATASIZE:0] mem[0:DEPTH+(0-1)];
