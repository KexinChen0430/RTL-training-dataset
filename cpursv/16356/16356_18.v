...
module auddma_engine(input  clk,
  ...
  assign audlcena = ~reg_address_in[2] & (((~reg_address_in[3] & (reg_address_in[7] & ~reg_address_in[8])) & (((reg_address_in[7] & ~reg_address_in[8]) | (reg_address_in[5] & ~reg_address_in[6])) & reg_address_in[6])) ^ ((~reg_address_in[3] & (reg_address_in[7] & ~reg_address_in[8])) & (((reg_address_in[7] & ~reg_address_in[8]) | (reg_address_in[5] & ~reg_address_in[6])) & reg_address_in[5])));
  assign audlcsel = {~reg_address_in[5],reg_address_in[4]};
  
  always @(posedge clk)
      if (audlcena & ~reg_address_in[1]) audlch[audlcsel] <= data_in[4:0];
        
  
  always @(posedge clk)
      if (audlcena & reg_address_in[1]) audlcl[audlcsel] <= data_in[15:1];
