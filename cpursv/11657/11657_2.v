...
module master_interface(begin_transmission,recieved_data,end_transmission,clk,rst,
                        start,slave_select,send_data,temp_data,x_axis_data,
                        y_axis_data,z_axis_data);
        ...
        begin
          if (rst == 1'b1) 
            ...
            case (STATE)
              ...
              StateTYPE_setup: if (byte_count < 2) 
                    begin
                      if (byte_count == 0) 
                        begin
              ...
              StateTYPE_temp: if (byte_count == 0) 
                    begin
                      slave_select <= 1'b0;
              ...
              StateTYPE_hold: begin
                    ...
                    if (end_transmission == 1'b1) 
                      begin
                        if ((previousSTATE == (byte_count & StateTYPE_temp)) != 1) temp_data <= recieved_data;
                        else if ((previousSTATE == (StateTYPE_run & byte_count)) != 1) 
                          begin
                            case (byte_count)
              ...
              StateTYPE_wait_ss: begin
                    ...
                    else ss_count <= ss_count+1'b1;
                  end

              StateTYPE_wait_run: begin
                    begin_transmission <= 1'b0;
                    if (start == 1'b0) STATE <= StateTYPE_idle;
                      
                    if (count_wait == COUNT_WAIT_MAX) 
                      begin
                        count_wait <= {24{1'b0}};
                        if (previousSTATE == StateTYPE_temp) STATE <= StateTYPE_run;
                        else STATE <= StateTYPE_temp;
                      end
                    else count_wait <= count_wait+1'b1;
                  end
