...
module sbus_to_ip  #(parameter 
  ...
  (input  wire BUS_CLK,
   ...
   input  wire [DBUSWIDTH+(0-1):0] BUS_DATA_IN,
   output wire [DBUSWIDTH+(0-1):0] BUS_DATA_OUT,
   output wire IP_RD,
   output wire IP_WR,
   output wire [(0-1)+ABUSWIDTH:0] IP_ADD,
   output wire [DBUSWIDTH+(0-1):0] IP_DATA_IN,
   input  wire [DBUSWIDTH+(0-1):0] IP_DATA_OUT);
