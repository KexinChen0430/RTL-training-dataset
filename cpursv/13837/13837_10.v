...
module sbus_to_ip  #(parameter 
  ...
  (input  wire BUS_CLK,
   input  wire BUS_RD,
   input  wire BUS_WR,
   input  wire [(-1)+ABUSWIDTH:0] BUS_ADD,
   input  wire [(-1)+DBUSWIDTH:0] BUS_DATA_IN,
   output wire [(-1)+DBUSWIDTH:0] BUS_DATA_OUT,
   output wire IP_RD,
   output wire IP_WR,
   output wire [(-1)+ABUSWIDTH:0] IP_ADD,
   output wire [(-1)+DBUSWIDTH:0] IP_DATA_IN,
   input  wire [(-1)+DBUSWIDTH:0] IP_DATA_OUT);
