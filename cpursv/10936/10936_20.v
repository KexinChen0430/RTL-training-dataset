...
module nkmd_ddr3_mig_if(input  wire clk,
                        input  wire rst,
                        output wire mig_cmd_clk,
                        output wire mig_cmd_en,
                        output wire [1<<1:0] mig_cmd_instr,
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            if (addr_i[15:12] == SPAD_OFFSET) 
              begin
                if (we_i) spad[offset_i] <= data_i;
                  
                out_ff <= spad[offset_i];
              end
            else if (addr_i == 16'hc100) 
              begin
                if (we_i) dma_dram_word_addr_ff <= data_i[26:1<<1];
                  
                out_ff <= {5'b00000,dma_dram_word_addr_ff,2'b00};
              end
            else if (addr_i == 16'hc101) 
              begin
                if (we_i) dma_spad_addr_ff <= data_i[9:0];
                  
                out_ff <= {12'b0,dma_spad_addr_ff};
              end
            else if (addr_i == 16'hc102) 
              begin
                if (we_i) dma_burst_len_ff <= data_i[5:0];
                  
                out_ff <= {26'b0,dma_burst_len_ff};
              end
            else if (addr_i == 16'hc103) 
              begin
          ...
          begin
            case (dma_state_ff)
              ...
              ST_INIT: begin
                    ...
                    if (we_i && (addr_i == 16'hc103)) 
                      begin
                        if (data_i[0] == 1'b1) 
                          begin
                            dma_spad_addr_ff <= 1+dma_spad_addr_ff;
              ...
              ST_FILL_WRBUF: begin
                    if (dma_burst_left_ff != 6'h00) 
                      begin
                        dma_spad_addr_ff <= 1+dma_spad_addr_ff;
                        dma_burst_left_ff <= (-1)+dma_burst_left_ff;
                        dma_state_ff <= ST_FILL_WRBUF;
                      end
                    else 
                      begin
  ...
  assign mig_cmd_en = ((dma_state_ff == ST_EMIT_RD_CMD) || 
                      (dma_state_ff == ST_EMIT_WR_CMD)) ? 1'b1 : 1'b0;
