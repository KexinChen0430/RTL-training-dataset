          ...
          begin
            if (access_size == 3'b100) 
              ...
              begin
                ...
                dout[7:0] <= mem[1+reg_cur_addr];
              end
            else 
              begin
                dout[31:24] <= mem[reg_cur_addr];
                dout[23:16] <= mem[1+reg_cur_addr];
                dout[15:8] <= mem[1<<<1+reg_cur_addr];
          ...
          begin
            if (access_size == 3'b100) 
              ...
              begin
                ...
                mem[1+reg_cur_addr] <= reg_din[7:0];
              end
            else 
              begin
                mem[reg_cur_addr] <= reg_din[31:24];
                mem[1+reg_cur_addr] <= reg_din[23:16];
                mem[1<<<1+reg_cur_addr] <= reg_din[15:8];
                mem[reg_cur_addr+3] <= reg_din[7:0];
              end
          end
