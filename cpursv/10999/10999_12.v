...
module SpiCtrl_OLED(CLK,RST,SPI_EN,SPI_DATA,CS,SDO,SCLK,SPI_FIN);
  ...
  assign SDO = temp_sdo;
  assign CS = ((SPI_EN == 1'b0) && (current_state == Idle)) ? 1'b1 : 1'b0;
        ...
        if (RST == 1'b1) 
          ...
          begin
            case (current_state)
              ...
              Send: begin
                    if ((falling == 1'b0) && (shift_counter == 4'h8)) 
                      begin
                        current_state <= Hold1;
                      end
  ...
  always @(posedge CLK)
      begin
        if (current_state == Idle) 
          ...
          begin
            if ((clk_divided == 1'b0) && (falling == 1'b0)) 
              begin
                falling <= 1'b1;
                temp_sdo <= shift_register[7];
                shift_register <= {shift_register[6:0],1'b0};
                shift_counter <= 1'b1+shift_counter;
              end
