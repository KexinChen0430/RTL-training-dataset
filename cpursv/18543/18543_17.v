  ...
  always @(posedge clk)
      begin
        if (~|shiftcnt[1<<<1:0]) 
          begin
            if (~|shiftcnt[6:3]) 
              shift <= #1 (((shiftcnt[7] & exchan) ^ ((shiftcnt[7] & (~exchan | exchan)) & shiftcnt[7])) | (~shiftcnt[7] & (exchan | (shiftcnt[7] & (~exchan | exchan))))) ? ldata_mux : rdata_mux;
