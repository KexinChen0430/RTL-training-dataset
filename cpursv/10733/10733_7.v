                ...
                if ((start == 1'b0) && (ena == 1'b1)) 
                  ...
                  begin : Encryption
                    ...
                    begin : Stage0_Stage4_Stage7_AddRoundKey
                      mem[1] <= (|mem[0] == 1'b0) ? mem[0] : {mem[0][(ADDR_WIDTH+DATA_WIDTH)+(-1):DATA_WIDTH],(mem[0][DATA_WIDTH-1:0] & (~key | (mem[0][DATA_WIDTH-1:0] | key))) ^ key};
                      mem[5] <= (|mem[4] == 1'b0) ? mem[4] : {mem[4][(ADDR_WIDTH+DATA_WIDTH)+(-1):DATA_WIDTH],(~key & (((key & ~mem[4][DATA_WIDTH-1:0]) | ~key) & mem[4][DATA_WIDTH-1:0])) | (key & ~mem[4][DATA_WIDTH-1:0])};
                      mem[8] <= (|mem[7] == 1'b0) ? mem[7] : {mem[7][(ADDR_WIDTH+DATA_WIDTH)+(-1):DATA_WIDTH],(~key & mem[7][DATA_WIDTH-1:0]) | (((((~mem[7][DATA_WIDTH-1:0] | mem[7][DATA_WIDTH-1:0]) & key) & ~mem[7][DATA_WIDTH-1:0]) | (((~mem[7][DATA_WIDTH-1:0] | mem[7][DATA_WIDTH-1:0]) & key) & ~key)) & (((((~mem[7][DATA_WIDTH-1:0] | mem[7][DATA_WIDTH-1:0]) & key) & ~mem[7][DATA_WIDTH-1:0]) | (((~mem[7][DATA_WIDTH-1:0] | mem[7][DATA_WIDTH-1:0]) & key) & ~key)) | ~key))};
                    end
