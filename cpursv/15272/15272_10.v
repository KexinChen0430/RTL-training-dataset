...
module vga_renderer  #(parameter 
  ...
  localparam 
       PIXELS_PER_LINE = ((H_FRONT_PORCH+WIDTH)+H_SYNC)+H_BACK_PORCH;
  localparam 
       LINES_PER_FRAME = (V_SYNC+V_BACK_PORCH)+(HEIGHT+V_FRONT_PORCH);
      ...
      begin
        if (~reset_n) 
          ...
          begin
            ...
            if (x_pos == (H_FRONT_PORCH+(WIDTH+(-1)))) hsync <= 1'b1;
            else if (x_pos == (H_SYNC+(H_FRONT_PORCH+(WIDTH+(-1))))) hsync <= 1'b0;
              
            if (y_pos == (V_FRONT_PORCH+(HEIGHT+(-1)))) vsync <= 1'b1;
            else if (y_pos == ((HEIGHT+(-1))+(V_SYNC+V_FRONT_PORCH))) vsync <= 1'b0;
