...
module vga_renderer  #(parameter 
  ...
  localparam 
       PIXELS_PER_LINE = (H_SYNC+H_FRONT_PORCH)+(H_BACK_PORCH+WIDTH);
  localparam 
       LINES_PER_FRAME = (V_SYNC+(V_BACK_PORCH+V_FRONT_PORCH))+HEIGHT;
      ...
      begin
        if (~reset_n) 
          ...
          begin
            ...
            else if (x_pos == ((((0-1)+H_FRONT_PORCH)+H_SYNC)+WIDTH)) hsync <= 1'b0;
              
            if (y_pos == (HEIGHT+((0-1)+V_FRONT_PORCH))) vsync <= 1'b1;
            else if (y_pos == ((V_SYNC+(0-1))+(HEIGHT+V_FRONT_PORCH))) vsync <= 1'b0;
