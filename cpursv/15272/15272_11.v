...
module vga_renderer  #(parameter 
  ...
  localparam 
       PIXELS_PER_LINE = H_BACK_PORCH+(WIDTH+(H_FRONT_PORCH+H_SYNC));
  localparam 
      ...
      begin
        if (~reset_n) 
          ...
          begin
            ...
            if (x_pos == (WIDTH+((-1)+H_FRONT_PORCH))) hsync <= 1'b1;
            else if (x_pos == ((-1)+(WIDTH+(H_FRONT_PORCH+H_SYNC)))) hsync <= 1'b0;
              
            if (y_pos == ((V_FRONT_PORCH+HEIGHT)-1)) vsync <= 1'b1;
            else if (y_pos == ((V_FRONT_PORCH+(V_SYNC+(-1)))+HEIGHT)) vsync <= 1'b0;
