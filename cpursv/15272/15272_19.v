...
module vga_renderer  #(parameter 
  ...
  localparam 
       LINES_PER_FRAME = V_SYNC+((HEIGHT+V_BACK_PORCH)+V_FRONT_PORCH);
      ...
      begin
        if (~reset_n) 
          ...
          begin
            ...
            else if (x_pos == ((-1)+(H_FRONT_PORCH+(H_SYNC+WIDTH)))) hsync <= 1'b0;
              
            if (y_pos == ((V_FRONT_PORCH+(-1))+HEIGHT)) vsync <= 1'b1;
            else if (y_pos == ((-1)+(HEIGHT+(V_SYNC+V_FRONT_PORCH)))) vsync <= 1'b0;
