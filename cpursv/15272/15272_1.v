...
module vga_renderer  #(parameter 
  ...
  localparam 
       PIXELS_PER_LINE = (H_FRONT_PORCH+(WIDTH+H_BACK_PORCH))+H_SYNC;
  localparam 
       LINES_PER_FRAME = (V_SYNC+HEIGHT)+(V_FRONT_PORCH+V_BACK_PORCH);
      ...
      begin
        if (~reset_n) 
          ...
          begin
            ...
            else if (x_pos == (((H_FRONT_PORCH+H_SYNC)+WIDTH)+(-1))) hsync <= 1'b0;
              
            if (y_pos == ((HEIGHT+(-1))+V_FRONT_PORCH)) vsync <= 1'b1;
            else if (y_pos == (((V_SYNC+(-1))+HEIGHT)+V_FRONT_PORCH)) vsync <= 1'b0;
