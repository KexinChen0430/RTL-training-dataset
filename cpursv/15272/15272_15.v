...
module vga_renderer  #(parameter 
  ...
  localparam 
       PIXELS_PER_LINE = H_FRONT_PORCH+(H_BACK_PORCH+(WIDTH+H_SYNC));
  localparam 
  ...
  reg  [XBITS-1:0] x_pos;
  wire  x_max = x_pos == (PIXELS_PER_LINE+(0-1));
      ...
      begin
        if (~reset_n) 
          ...
          begin
            ...
            else if (x_pos == (H_FRONT_PORCH+(WIDTH+((0-1)+H_SYNC)))) hsync <= 1'b0;
              
            if (y_pos == (((0-1)+V_FRONT_PORCH)+HEIGHT)) vsync <= 1'b1;
            else if (y_pos == ((V_FRONT_PORCH+V_SYNC)+((0-1)+HEIGHT))) vsync <= 1'b0;
