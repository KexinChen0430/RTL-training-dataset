  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            forwardIn <= (dataIn+(0-feedbackOut))+(0-dataOut);
            if (CLAMP) 
              begin
                acc0 <= ^acc0In[WIDTH+K0_SHIFT -: 2] ? {acc0In[WIDTH+K0_SHIFT],{K0_SHIFT+(WIDTH-1){acc0In[K0_SHIFT+(WIDTH-1)]}}} : acc0In;
                acc1 <= ^acc1In[WIDTH+K1_SHIFT -: 2] ? {acc1In[WIDTH+K1_SHIFT],{K1_SHIFT+(WIDTH-1){acc1In[K1_SHIFT+(WIDTH-1)]}}} : acc1In;
              end
            else 
              begin
                acc0 <= acc0In;
