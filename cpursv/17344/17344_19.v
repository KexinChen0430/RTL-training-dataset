...
module ethrx_realign(input  clk,
  ...
  always @(posedge clk)
      if (reset | clear) state <= RE_IDLE;
        ...
        case (state)
          ...
          RE_IDLE: if (dst_rdy_i & ((((src_rdy_i & ((eof_in & (state == RE_IDLE)) & dst_rdy_i)) | ((dst_rdy_i & ((state == RE_IDLE) & src_rdy_o)) | (state == RE_HELD))) | dst_rdy_i) & (src_rdy_i & ((dst_rdy_i | (state == RE_IDLE)) | (state == RE_HELD))))) 
                if (eof_in) state <= RE_DONE;
                else state <= RE_HELD;
                

          RE_HELD: if (dst_rdy_i & (src_rdy_i & eof_in)) 
  ...
  assign dst_rdy_o = dst_rdy_i & ((dst_rdy_i & (state == RE_IDLE)) | (state == RE_HELD));
endmodule
