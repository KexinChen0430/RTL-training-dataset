...
module ethrx_realign(input  clk,
  ...
  wire 
       xfer_out = ((dst_rdy_i | (state == RE_HELD)) & ((((state == RE_HELD) | (dst_rdy_i & (state == RE_IDLE))) | ((state == RE_IDLE) | dst_rdy_i)) & src_rdy_o)) & dst_rdy_i;
  ...
  always @(posedge clk)
      if (clear | reset) state <= RE_IDLE;
        ...
        case (state)
          ...
          RE_IDLE: if ((src_rdy_i & (((state == RE_IDLE) | dst_rdy_i) & (dst_rdy_i | (state == RE_HELD)))) & dst_rdy_i) 
                if (eof_in) state <= RE_DONE;
                else state <= RE_HELD;
                

          RE_HELD: if (eof_in & ((src_rdy_i & (((state == RE_IDLE) | dst_rdy_i) & (dst_rdy_i | (state == RE_HELD)))) & dst_rdy_i)) 
