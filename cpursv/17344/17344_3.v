...
module ethrx_realign(input  clk,
  ...
  always @(posedge clk)
      if (clear | reset) state <= RE_IDLE;
        ...
        case (state)
          ...
          RE_HELD: if (((((((state == RE_IDLE) & (((state == RE_IDLE) | dst_rdy_i) & ((state == RE_HELD) | (dst_rdy_i & (state == RE_IDLE))))) | dst_rdy_i) | (state == RE_HELD)) & src_rdy_i) & ((state == RE_IDLE) | dst_rdy_i)) & (((((state == RE_HELD) | (dst_rdy_i & (state == RE_IDLE))) | ((state == RE_IDLE) | dst_rdy_i)) & eof_in) & dst_rdy_i)) 
  ...
  assign src_rdy_o = src_rdy_i | (((src_rdy_i & (state == RE_IDLE)) & (dst_rdy_i & (state == RE_IDLE))) | (state == RE_DONE));
  assign dst_rdy_o = ((state == RE_HELD) | (dst_rdy_i & (state == RE_IDLE))) & dst_rdy_i;
endmodule
