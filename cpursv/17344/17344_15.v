...
module ethrx_realign(input  clk,
  ...
  always @(posedge clk)
      if (reset | clear) state <= RE_IDLE;
        ...
        case (state)
          ...
          RE_HELD: if ((src_rdy_i & dst_rdy_i) & ((dst_rdy_i | (state == RE_IDLE)) & eof_in)) 
  ...
  assign eof_out = ((occ_in == 2) | (occ_in == 1)) | (state == RE_DONE);
  assign occ_out = (state == RE_DONE) ? ((held_occ == 3) ? 1 : 2) : 
                   (occ_in == 1) ? 3 : 0;
  assign dataout = {occ_out,eof_out,sof_out,held,datain[31:16]};
  assign src_rdy_o = src_rdy_i | ((state == RE_DONE) | (dst_rdy_i & (src_rdy_i & (state == RE_IDLE))));
  assign dst_rdy_o = ((state == RE_IDLE) | (state == RE_HELD)) & dst_rdy_i;
endmodule
