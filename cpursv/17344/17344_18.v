...
module ethrx_realign(input  clk,
  ...
  always @(posedge clk)
      if (clear | reset) state <= RE_IDLE;
        ...
        case (state)
          ...
          RE_HELD: if ((eof_in & src_rdy_i) & dst_rdy_i) 
  ...
  assign dst_rdy_o = ((((eof_in & ((state == RE_HELD) | (dst_rdy_i & (state == RE_IDLE)))) & (dst_rdy_i & (state == RE_IDLE))) | (state == RE_HELD)) | (state == RE_IDLE)) & dst_rdy_i;
endmodule
