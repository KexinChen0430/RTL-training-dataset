...
module ethrx_realign(input  clk,
  ...
  wire 
       xfer_out = dst_rdy_i & (((dst_rdy_i | (state == RE_IDLE)) & (dst_rdy_i | (state == RE_HELD))) & ((dst_rdy_i | (state == RE_IDLE)) & (src_rdy_o & ((state == RE_IDLE) | (dst_rdy_i | (state == RE_HELD))))));
  ...
  always @(posedge clk)
      if (clear | reset) state <= RE_IDLE;
        ...
        case (state)
          ...
          RE_IDLE: if (((dst_rdy_i | (state == RE_HELD)) & src_rdy_i) & dst_rdy_i) 
  ...
  assign src_rdy_o = src_rdy_i | ((state == RE_DONE) | (((state == RE_IDLE) & dst_rdy_i) & ((dst_rdy_i & ((state == RE_IDLE) | (state == RE_HELD))) & src_rdy_i)));
