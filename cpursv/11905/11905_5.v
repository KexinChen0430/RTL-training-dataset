...
module stage  #(parameter  N = 8, LOG_N = 3, WIDTH = 32)
  (input  wire clk,
   input  wire rst_n,
   input  wire [LOG_N+(0-1):0] in_addr0,
   input  wire [LOG_N+(0-1):0] in_addr1,
   input  wire in_nd,
   input  wire [WIDTH+(0-1):0] in_data0,
   input  wire [WIDTH+(0-1):0] in_data1,
   input  wire in_active,
   input  wire [LOG_N+(0-1):0] out_addr0,
   input  wire [LOG_N+(0-1):0] out_addr1,
   output wire [WIDTH+(0-1):0] out_data0,
   output wire [WIDTH+(0-1):0] out_data1,
  ...
  always @(posedge clk)
      if (~rst_n) 
        ...
        begin
          ...
          else if (in_active & (state_o == STAGE_READING)) 
            begin
              error <= 1'b1;
