...
module stage  #(parameter  N = 8, LOG_N = 3, WIDTH = 32)
  (input  wire clk,
   input  wire rst_n,
   input  wire [(-1)+LOG_N:0] in_addr0,
   input  wire [(-1)+LOG_N:0] in_addr1,
   input  wire in_nd,
   input  wire [(-1)+WIDTH:0] in_data0,
   input  wire [(-1)+WIDTH:0] in_data1,
   input  wire in_active,
   input  wire [(-1)+LOG_N:0] out_addr0,
   input  wire [(-1)+LOG_N:0] out_addr1,
   output wire [(-1)+WIDTH:0] out_data0,
   output wire [(-1)+WIDTH:0] out_data1,
