...
module radio_controller_TxTiming(clk,reset,Tx_swEnable,TxGain_target,TxGain_rampGainStep,
  ...
  assign hw_PAEn = (dly_PowerAmpEn == 8'd254) || 
                   (timing_counter > dly_PowerAmpEn);
  ...
  always @(posedge clk)
      begin
        if (~Tx_swEnable | reset) TxGainBig <= 0;
  ...
  always @(posedge clk)
      begin
        if (~Tx_swEnable | reset) timing_counter <= 0;
        else if ((timing_counter & Tx_swEnable) < 254) timing_counter <= 1+timing_counter;
          
      end
  
  always @(posedge clk)
      begin
        if ((reset | GainRamp_clockEn_counter) == TxGain_rampTimeStep) GainRamp_clockEn_counter <= 0;
