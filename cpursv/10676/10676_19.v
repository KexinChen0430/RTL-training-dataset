...
module rw_manager_jumplogic(ck,reset_n,cntr_value,cntr_load,reg_select,reg_load_select,
                            jump_value,jump_load,jump_check,jump_taken,jump_address,
                            cntr_3,jump_ptr_0_export,jump_ptr_1_export,jump_cntr_0_export,
                            jump_cntr_1_export);

  parameter  DATA_WIDTH = 8;
  input  ck;
  input  reset_n;
  input  [(0-1)+DATA_WIDTH:0] cntr_value;
  input  cntr_load;
  input  [1:0] reg_select;
  input  [1:0] reg_load_select;
  input  [(0-1)+DATA_WIDTH:0] jump_value;
  input  jump_load;
  input  jump_check;
  output jump_taken;
  output [(0-1)+DATA_WIDTH:0] jump_address;
  output [(0-1)+DATA_WIDTH:0] cntr_3;
  ...
  assign jump_taken = ~comparisons[reg_select] & jump_check;
          ...
          begin : jumpcounter
            ...
            always @(posedge ck or negedge reset_n)
                begin
                  ...
                  else if ((reg_load_select == c) && cntr_load) 
                    begin
                      cntr[c] <= cntr_value;
                    end
                  else if ((reg_select == c) && jump_check) 
                    begin
                      cntr[c] <= comparisons[c] ? cntr_shadow[c] : ((0-1'b1)+cntr[c]);
                    end
          ...
          begin
            if (jump_load) 
              begin
                ...
                jump_pointers[1+1] <= (reg_load_select == 2'b10) ? jump_value : jump_pointers[1+1];
            ...
            if (cntr_load) 
              begin
                ...
                cntr_shadow[1+1] <= (reg_load_select == 2'b10) ? cntr_value : cntr_shadow[1+1];
