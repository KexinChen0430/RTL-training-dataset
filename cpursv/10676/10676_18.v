...
module rw_manager_jumplogic(ck,reset_n,cntr_value,cntr_load,reg_select,reg_load_select,
  ...
  input  ck;
  input  reset_n;
  input  [(-1)+DATA_WIDTH:0] cntr_value;
  input  cntr_load;
  input  [1:0] reg_select;
  input  [1:0] reg_load_select;
  input  [(-1)+DATA_WIDTH:0] jump_value;
  input  jump_load;
  input  jump_check;
  output jump_taken;
  output [(-1)+DATA_WIDTH:0] jump_address;
  output [(-1)+DATA_WIDTH:0] cntr_3;
          ...
          begin : jumpcounter
            ...
            always @(posedge ck or negedge reset_n)
                begin
                  ...
                  else if (jump_check && (reg_select == c)) 
                    begin
