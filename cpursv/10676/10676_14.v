...
module rw_manager_jumplogic(ck,reset_n,cntr_value,cntr_load,reg_select,reg_load_select,
  ...
  input  ck;
  input  reset_n;
  input  [DATA_WIDTH+(0-1):0] cntr_value;
  input  cntr_load;
  input  [1:0] reg_select;
  input  [1:0] reg_load_select;
  input  [DATA_WIDTH+(0-1):0] jump_value;
  input  jump_load;
  input  jump_check;
  output jump_taken;
  output [DATA_WIDTH+(0-1):0] jump_address;
  output [DATA_WIDTH+(0-1):0] cntr_3;
          ...
          begin : jumpcounter
            ...
            always @(posedge ck or negedge reset_n)
                begin
                  ...
                  else if (cntr_load && (reg_load_select == c)) 
                    begin
                      cntr[c] <= cntr_value;
                    end
                  else if ((reg_select == c) && jump_check) 
                    begin
                      cntr[c] <= comparisons[c] ? cntr_shadow[c] : ((-1'b1)+cntr[c]);
                    end
