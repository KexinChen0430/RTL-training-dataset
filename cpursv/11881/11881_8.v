...
module FirstStage(input  wire [7:0] plane_xpos,
      ...
      begin
        case (state)
          ...
          NORMAL_MODE: begin
                ...
                copy_addr = 0;
                saving_addr_nxt = 0;
                ram_addr = plane_xpos+((11+(0-plane_ypos))*180);
                if (save_block) 
                  begin
                  ...
                  begin
                    ...
                    block = ram_read_data;
                    if ((block == M) || 
                        ((block == P) || (block == WY) || (block == DY)) || (block == J) || 
                        ((block == D) || (block == S) || (block == C) || 
((block == N) || (block == L)) || (block == A))) 
                      begin
                        blocking = 1;
                      end
                    else 
                ...
                if (copy_backup) state_nxt = START_BACKUP;
                else state_nxt = NORMAL_MODE;
              end
          ...
          BACKUP: begin
                ...
                saving_addr_nxt = 1+saving_addr;
                ram_addr = saving_addr;
                ram_write_data = copy_read_data;
                if (saving_addr == ROM_STAGE_SIZE) 
                  begin
                    backuped_nxt = 1;
                    ram_we = 0;
