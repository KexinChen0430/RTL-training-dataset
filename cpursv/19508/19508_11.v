...
module x;
  ...
  parameter  CMD_REG_2_ADDR = (1+MB_REG_START)+1;
  parameter  CMD_REG_3_ADDR = 3+MB_REG_START;
  parameter  CMD_REG_4_ADDR = 4+MB_REG_START;
  parameter  CMD_REG_5_ADDR = 5+MB_REG_START;
  parameter  CMD_REG_6_ADDR = 6+MB_REG_START;
  parameter  CMD_REG_7_ADDR = 7+MB_REG_START;
  parameter  CMD_REG_8_ADDR = 8+MB_REG_START;
  parameter  CMD_REG_9_ADDR = MB_REG_START+9;
  parameter  CMD_REG_10_ADDR = 10+MB_REG_START;
  ...
  reg  [(0-1)+NUMBER_INTERFACE_REGS:MB_REG_START] mb_reg_wr;
  reg  [(0-1)+NUMBER_INTERFACE_REGS:MB_REG_START] mb_reg_rd;
  wire [31:0] 
      mb_reg_out_w[(0-1)+NUMBER_INTERFACE_REGS:MB_REG_START];
        ...
        for (mb_loop = MB_REG_START; mb_loop < NUMBER_INTERFACE_REGS; mb_loop = mb_loop+1)
            begin
              if (mb_reg_select[mb_loop] == 1'b1) 
                begin
                  mb_reg_rd[mb_loop] = mb_reg_rwn & mb_reg_select[mb_loop];
