...
module alu(input  [63:0] inA_i,
  ...
  wire  c62 = sumL[63];
  wire [64:63]  sumH = (c62+inA_i[63])+b[63];
  wire [63:0]  rawSums = {sumH[63],sumL[62:0]};
  wire [63:0]  sums = sum_en_i ? rawSums : 64'd0;

  assign zflag_o = ~|out_o;
  assign vflag_o = ((~sumH[64] | sumH[64]) & sumL[63]) ^ sumH[64];
  assign cflag_o = sumH[64];
  wire [63:0]  ands = and_en_i ? (b & inA_i) : 64'd0;

  wire [63:0] 
       xors = xor_en_i ? (((inA_i & (((b & ~inA_i) | ((inA_i & ~b) & ~inA_i)) | (inA_i | ~inA_i))) & ~(b & (((b & ~inA_i) | ((inA_i & ~b) & ~inA_i)) | (inA_i | ~inA_i)))) | ((b & (((b & ~inA_i) | ((inA_i & ~b) & ~inA_i)) | (inA_i | ~inA_i))) & ~(inA_i & (((b & ~inA_i) | ((inA_i & ~b) & ~inA_i)) | (inA_i | ~inA_i))))) : 64'd0;
  ...
  wire [63:0] 
       isLTS = lts_en_i ? {63'd0,((~rawSums[63] | ~vflag_o) & rawSums[63]) ^ (vflag_o & (~rawSums[63] | ~vflag_o))} : 0;

  wire [63:0]  isLTU = ltu_en_i ? {63'd0,~cflag_o} : 0;

  assign out_o = sums | ((ands | xors) | ((rshs | lshs) | (isLTU | isLTS)));
endmodule
