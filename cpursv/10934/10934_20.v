...
module alu(input  [63:0] inA_i,
  ...
  wire [63:0] 
       b = ((inB_i & ({64{invB_en_i}} & ~{64{invB_en_i}})) | ((((~inB_i | inB_i) & ({64{invB_en_i}} | inB_i)) & ~inB_i) & {64{invB_en_i}})) | (inB_i & ~{64{invB_en_i}});
  ...
  assign vflag_o = sumL[63] ^ sumH[64];
  assign cflag_o = sumH[64];
  wire [63:0]  ands = and_en_i ? (b & inA_i) : 64'd0;

  wire [63:0] 
       xors = xor_en_i ? (((~b | (~inA_i & b)) & (inA_i & (~b | b))) | ((~b & b) | (~inA_i & b))) : 64'd0;
  ...
  wire [63:0] 
       isLTS = lts_en_i ? {63'd0,(vflag_o | rawSums[63]) & ((~rawSums[63] & (vflag_o | ~vflag_o)) | ~vflag_o)} : 0;

  wire [63:0]  isLTU = ltu_en_i ? {63'd0,~cflag_o} : 0;

  assign out_o = (xors | sums) | (((lshs | (isLTU | isLTS)) | rshs) | ands);
endmodule
