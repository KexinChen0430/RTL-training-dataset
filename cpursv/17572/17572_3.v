  ...
  always @(posedge clk)
      begin
        if (reset) cur_is_Cr_or_Cb <= 1'b0;
          ...
          cur_is_Cr_or_Cb <= (cur_is_Cr_or_Cb | 1'b1) & (~cur_is_Cr_or_Cb | ~1'b1);
          
      end
  assign stream_in_ready = stream_in_valid & (transfer_data | ~valid);
  assign transfer_data = ~stream_out_valid | ((((stream_out_valid & ~stream_out_valid) | (stream_out_ready & ~stream_out_valid)) | stream_out_ready) & stream_out_valid);
