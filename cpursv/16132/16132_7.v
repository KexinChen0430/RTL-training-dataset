...
module tx_data_send(input  pclk_tx,
                    input  send_null_tx,
                    input  enable_tx,
                    input  get_data,
                    input  get_data_0,
  ...
  assign process_data_en = (fct_counter_p & txwrite_tx) ? 1'b1 : 1'b0;
      ...
      begin
        if (!enable_tx) 
          ...
          begin
            ...
            else if (process_data_en && get_data) 
              begin
                tx_data_in <= data_tx_i;
                process_data <= 1'b1;
                process_data_0 <= 1'b0;
              end
            else if (process_data_en && get_data_0) 
              begin
