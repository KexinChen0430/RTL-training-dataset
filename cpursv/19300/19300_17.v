...
module test;
  ...
  begin
    ...
    if ((str[8*40:8<<<1+1] !== 0) || (str[8<<<1:1] !== -1)) 
      begin
    ...
    if ((str[8<<<1:1] !== -2) || (str[8*40:8<<<1+1] !== 0)) 
      begin
        $display("FAILED 2nd test, expected \"-2\", got %s",str);
        pass = 1'b0;
      end
      
    $sformat(str,"%0d",-1*(1+2>>s));
    if ((str[8<<<1:1] !== -2) || (str[8*40:8<<<1+1] !== 0)) 
      begin
