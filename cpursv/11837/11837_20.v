...
module I2CTest(CLCK,SCL,SDA);
  ...
  wire 
       SDA_synched = (SDASynch[1] & SDASynch[0]) & SDASynch[1<<1];
  ...
  always @(posedge SCL_negedge)
      if (addressmatch & (((~rw & (bitcount == 17)) | addressmatch) & ((~rw & (((bitcount == 8) & ~rw) | (bitcount == 17))) | (bitcount == 8)))) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if ((addressmatch & ((bitcount >= 9) & (currvalue < valuecnt))) & rw) 
        begin
          if ((((0-(currvalue*9))+bitcount)+(-9)) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = currvalue+1;
            end
          else sdadata = datain[7-(((0-(currvalue*9))+bitcount)+(-9))];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
