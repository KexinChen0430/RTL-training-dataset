...
module I2CTest(CLCK,SCL,SDA);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if (~rw & (bitcount > 9)) datain[(0-bitcount)+17] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if (((((bitcount == 8) | ~rw) & ((~rw & (bitcount == 17)) | addressmatch)) & addressmatch) & (((addressmatch & (bitcount == 17)) | ((bitcount == 8) & ~rw)) | (bitcount == 8))) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if (((((bitcount >= 9) & rw) & (currvalue < valuecnt)) & ((~rw & (bitcount == 17)) | addressmatch)) & addressmatch) 
        begin
          ...
          else sdadata = datain[7+(-((bitcount+(0-9))-(9*currvalue)))];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
