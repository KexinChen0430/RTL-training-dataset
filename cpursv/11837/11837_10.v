...
module I2CTest(CLCK,SCL,SDA);
  ...
  always @(posedge SCL_negedge)
      if (((((bitcount == 17) & (bitcount == 8)) | ~rw) | (bitcount == 8)) & ((((((((bitcount == 17) & ~rw) | ((bitcount == 8) & ~rw)) | (bitcount == 17)) | ((bitcount == 8) & ~rw)) | (bitcount == 8)) & (((bitcount == 17) & ~rw) | addressmatch)) & addressmatch)) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if ((addressmatch & ((currvalue < valuecnt) & rw)) & (bitcount >= 9)) 
        begin
          if (((bitcount+(0-(9*currvalue)))+(-9)) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = 1+currvalue;
            end
          else sdadata = datain[(-((bitcount+(0-(9*currvalue)))+(-9)))+7];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
