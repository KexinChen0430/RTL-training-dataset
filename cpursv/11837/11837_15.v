...
module I2CTest(CLCK,SCL,SDA);
  ...
  wire 
       SDA_synched = SDASynch[0] & (SDASynch[1<<1] & SDASynch[1]);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if ((bitcount > 9) & ~rw) datain[(0-bitcount)+17] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if (((addressmatch & ~rw) & ((bitcount == 17) | (~rw & (bitcount == 8)))) | (addressmatch & (bitcount == 8))) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if (((bitcount >= 9) & ((currvalue < valuecnt) & rw)) & addressmatch) 
        begin
          ...
          else sdadata = datain[(0-(((0-9)+bitcount)+(-(currvalue*9))))+7];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
