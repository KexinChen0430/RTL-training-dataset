...
module I2CTest(CLCK,SCL,SDA);
  ...
  always @(posedge SCL_negedge)
      if ((((bitcount == 8) | ((bitcount == 17) & ~rw)) & (((bitcount == 17) | addressmatch) | (bitcount == 8))) & addressmatch) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if (((bitcount >= 9) & rw) & (((currvalue < valuecnt) & (((bitcount == 17) | addressmatch) | (bitcount == 8))) & addressmatch)) 
        begin
          if (((-9)+(bitcount+(-(9*currvalue)))) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = currvalue+1;
            end
          else sdadata = datain[(-((-9)+(bitcount+(-(9*currvalue)))))+7];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
