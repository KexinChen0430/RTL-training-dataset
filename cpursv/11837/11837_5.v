...
module I2CTest(CLCK,SCL,SDA);
  ...
  wire 
       SDA_synched = SDASynch[2] & (SDASynch[1] & SDASynch[0]);
  ...
  always @(posedge SCL_negedge)
      if (((addressmatch & (bitcount == 17)) & ((bitcount == 8) | ~rw)) | (addressmatch & (bitcount == 8))) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if (rw & (((bitcount >= 9) & (currvalue < valuecnt)) & addressmatch)) 
        begin
          if ((((0-9)+(0-(9*currvalue)))+bitcount) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = 1+currvalue;
            end
          else sdadata = datain[7-(((0-9)+(0-(9*currvalue)))+bitcount)];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
