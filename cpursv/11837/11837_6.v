...
module I2CTest(CLCK,SCL,SDA);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if (~rw & (bitcount > 9)) datain[17-bitcount] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if ((addressmatch & (~rw | ((bitcount == 8) | (((bitcount == 8) & ~rw) | (((bitcount == 17) & (bitcount == 8)) | (((bitcount == 8) & ~rw) | (~rw & (bitcount == 17)))))))) & ((addressmatch & (bitcount == 8)) | (~rw & (bitcount == 17)))) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
