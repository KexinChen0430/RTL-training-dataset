...
module I2CTest(CLCK,SCL,SDA);
  ...
  always @(posedge SCL_negedge)
      if ((~rw | (bitcount == 8)) & (((bitcount == 8) | (((bitcount == 17) & (~rw | addressmatch)) & addressmatch)) & addressmatch)) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
        ...
        begin
          if ((((-(9*currvalue))+bitcount)+(0-9)) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = currvalue+1;
            end
          else sdadata = datain[(0-(((-(9*currvalue))+bitcount)+(0-9)))+7];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
