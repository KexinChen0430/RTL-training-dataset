...
module I2CTest(CLCK,SCL,SDA);
  ...
  wire 
       SDA_synched = SDASynch[0] & (SDASynch[1] & SDASynch[2]);
  ...
  always @(posedge SCL_negedge)
      if ((((bitcount == 8) | ((~rw & ((bitcount == 17) | ((bitcount == 8) & ~rw))) & (bitcount == 17))) & (addressmatch | (bitcount == 17))) & addressmatch) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if (((currvalue < valuecnt) & (bitcount >= 9)) & ((rw & (((bitcount == 8) | addressmatch) | (bitcount == 17))) & addressmatch)) 
        begin
          ...
          else sdadata = datain[(0-(((-(currvalue*9))+bitcount)+(-9)))+7];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
