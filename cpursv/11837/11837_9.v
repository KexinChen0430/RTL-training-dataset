...
module I2CTest(CLCK,SCL,SDA);
  ...
  always @(posedge SCL_negedge)
      if ((addressmatch & ((bitcount == 17) | (bitcount == 8))) & ((bitcount == 8) | (((bitcount == 17) | (~rw & (bitcount == 8))) & ~rw))) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if (addressmatch & ((bitcount >= 9) & (rw & (currvalue < valuecnt)))) 
        begin
          if ((((0-(9*currvalue))+(0-9))+bitcount) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = currvalue+1;
            end
          else 
            sdadata = datain[7+(0-(((0-(9*currvalue))+(0-9))+bitcount))];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
