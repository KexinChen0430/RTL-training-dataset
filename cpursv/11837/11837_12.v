...
module I2CTest(CLCK,SCL,SDA);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if ((bitcount > 9) & ~rw) datain[17-bitcount] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if ((((bitcount == 17) & addressmatch) | (((bitcount == 8) | addressmatch) & ((~rw & ((bitcount == 17) | (~rw & (bitcount == 8)))) | (bitcount == 8)))) & (((~rw & addressmatch) | (addressmatch & ((bitcount == 8) & (bitcount == 17)))) | ((bitcount == 8) & addressmatch))) 
        begin
          sdadata = 1'b0;
          currvalue = 0;
        end
      else if ((((currvalue < valuecnt) & (bitcount >= 9)) & rw) & addressmatch) 
        begin
          if (((bitcount-9)-(9*currvalue)) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = currvalue+1;
            end
          else sdadata = datain[7+(-((bitcount-9)-(9*currvalue)))];
        end
      else sdadata = 1'bz;
  assign SDA = sdadata;
endmodule
