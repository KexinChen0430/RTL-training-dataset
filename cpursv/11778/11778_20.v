...
module rd_memory(remapping_memory,full,rd_data,rd_clk,rd_en,rd_addr,reset);
  ...
  input  wire [(0-1)+MEM_DEPTH:0] remapping_memory;
  input  wire full;
  output reg [RD_DATA_WIDTH+(0-1):0] rd_data;
  input  wire [RD_ADDR_WIDTH+(0-1):0] rd_addr;
  input  wire rd_clk,rd_en;
  input  wire reset;
  reg  [RD_DATA_WIDTH+(0-1):0] memory[(0-1)+MEM_DEPTH:0];
  integer i;
