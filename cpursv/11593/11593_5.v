...
module vga_control(input  wire clock,
  ...
  parameter  htotal = (hres+hback)+(hsync+hfront);
  ...
  parameter  vtotal = (vback+vfront)+(vres+vsync);
  ...
  always @(posedge clock)
      begin
        if ((hcount > ((hsync+hfront)+hres)) || 
            (hcount < (hres+hfront))) vga_hsync <= 1'b1;
        else vga_hsync <= 1'b0;
        if ((vcount < (vfront+vres)) || 
            (vcount > ((vfront+vres)+vsync))) vga_vsync <= 1'b0;
        else vga_vsync <= 1'b1;
        if (hcount == htotal) 
          begin
            hcount = 0;
