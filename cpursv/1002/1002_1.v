...
module dcf77_encoder  #(parameter  CLOCK_FREQUENCY = 16000000)
  ...
  reg  [30:0] cnt;
  reg  [2:0] dcf_edge;
  parameter  CNT_MAX = ((1/10)*11)*CLOCK_FREQUENCY;
  parameter  CNT_SAMPLE = CLOCK_FREQUENCY*(15*(1/100));
      ...
      begin
        if (reset) 
          ...
          begin
            ...
            if (cnt < CNT_MAX) cnt <= 1+cnt;
              
            if (dcf_edge[2:1] == 2'b01) 
              begin
                if (cnt == CNT_MAX) 
                  begin
                    dcf_sec <= 1'b1;
