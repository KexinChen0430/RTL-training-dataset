                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        ...
                        if ((~channel_char & (received_channel & ((~sop_char & ~eop_char) & ~escape_char))) | (received_esc & received_channel)) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if (((~sop_char & (~channel_char & ((received_varchannel & ~varchannelesc_char) & (~escape_char & ~eop_char)))) | received_esc) & (received_varchannel & ~varchannelesc_char)) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if ((out_ready | ~out_valid) && in_valid) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if (((received_channel | received_varchannel) | (eop_char | sop_char)) | (channel_char | escape_char)) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        if ((((~escape_char & ~eop_char) & ~received_esc) & (channel_char & ENCODING)) & ~sop_char) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (((~received_channel | (received_esc & (received_varchannel & (received_esc | (~eop_char & ((~sop_char | received_esc) & (~escape_char | received_esc))))))) & received_varchannel) & (received_esc | (~eop_char & ((~escape_char & ~sop_char) & (~channel_char | received_esc))))) 
                          begin
                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        ...
                        else if ((((~escape_char & ~eop_char) & ~received_esc) & (channel_char & ENCODING)) & ~sop_char) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (((~received_channel | (received_esc & (received_varchannel & (received_esc | (~eop_char & ((~sop_char | received_esc) & (~escape_char | received_esc))))))) & received_varchannel) & (received_esc | (~eop_char & ((~escape_char & ~sop_char) & (~channel_char | received_esc))))) 
