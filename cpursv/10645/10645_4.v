...
module altera_avalon_st_bytes_to_packets  #(parameter  CHANNEL_WIDTH = 8, ENCODING = 0)
  ...
  assign data_out = received_esc ? ((~8'h20 | ~in_data) & (in_data | (~in_data & 8'h20))) : in_data;
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if ((received_esc & received_channel) | (((~escape_char & ~sop_char) & (~channel_char & received_channel)) & ~eop_char)) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if (((~sop_char & ~eop_char) | received_esc) & (((~escape_char & ~channel_char) | received_esc) & (received_varchannel & ~varchannelesc_char))) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if (in_valid && (out_ready | ~out_valid)) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if (escape_char | (((received_channel | received_varchannel) | channel_char) | (sop_char | eop_char))) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        if ((channel_char & (ENCODING & ~escape_char)) & (~received_esc & (~sop_char & ~eop_char))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((((received_varchannel & ~channel_char) & ((~escape_char & ~eop_char) & ~sop_char)) | (((received_varchannel & ((~sop_char & ~eop_char) | received_esc)) & (~escape_char | received_esc)) & received_esc)) & (~received_channel | (((received_varchannel & ((~sop_char & ~eop_char) | received_esc)) & (~escape_char | received_esc)) & received_esc))) 
                          begin
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        else if ((channel_char & (ENCODING & ~escape_char)) & (~received_esc & (~sop_char & ~eop_char))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((((received_varchannel & ~channel_char) & ((~escape_char & ~eop_char) & ~sop_char)) | (((received_varchannel & ((~sop_char & ~eop_char) | received_esc)) & (~escape_char | received_esc)) & received_esc)) & (~received_channel | (((received_varchannel & ((~sop_char & ~eop_char) | received_esc)) & (~escape_char | received_esc)) & received_esc))) 
