          ...
          always @* 
              begin
                ...
                if (in_valid && (out_ready | ~out_valid)) 
                  begin
                    out_valid = 1;
                    if ((channel_char | (sop_char | eop_char)) | escape_char) out_valid = 0;
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if ((received_channel & received_esc) | (((~escape_char & (~eop_char & ~sop_char)) & received_channel) & ~channel_char)) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if (((~channel_char | (received_varchannel & received_esc)) & ((((~escape_char | received_esc) & (~eop_char & ~sop_char)) | received_esc) & received_varchannel)) & ~varchannelesc_char) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if (in_valid && (out_ready | ~out_valid)) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if ((((escape_char | channel_char) | (received_varchannel | received_channel)) | sop_char) | eop_char) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        if ((((~sop_char & ~escape_char) & ~received_esc) & (~eop_char & channel_char)) & ENCODING) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((received_varchannel & (~received_channel | (received_varchannel & received_esc))) & (((~sop_char | received_esc) & ((~escape_char & (~channel_char | received_esc)) | received_esc)) & (~eop_char | received_esc))) 
                          begin
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        else if ((((~sop_char & ~escape_char) & ~received_esc) & (~eop_char & channel_char)) & ENCODING) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((received_varchannel & (~received_channel | (received_varchannel & received_esc))) & (((~sop_char | received_esc) & ((~escape_char & (~channel_char | received_esc)) | received_esc)) & (~eop_char | received_esc))) 
