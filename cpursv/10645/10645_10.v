          ...
          always @* 
              begin
                ...
                if ((~out_valid | out_ready) && in_valid) 
                  begin
                    out_valid = 1;
                    if ((escape_char | channel_char) | (sop_char | eop_char)) out_valid = 0;
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if ((((~sop_char & (~eop_char & ~escape_char)) & (received_channel & ~channel_char)) | received_esc) & received_channel) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if ((((~sop_char & ~eop_char) & (~channel_char & ~escape_char)) & (~varchannelesc_char & received_varchannel)) | (received_varchannel & (~varchannelesc_char & received_esc))) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if ((~out_valid | out_ready) && in_valid) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if (((received_varchannel | received_channel) | ((escape_char | channel_char) | sop_char)) | eop_char) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        if ((~escape_char & (~received_esc & (ENCODING & (channel_char & ~eop_char)))) & ~sop_char) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((received_varchannel & received_esc) | (~received_channel & ((~sop_char & (~eop_char & ~escape_char)) & (received_varchannel & ~channel_char)))) 
                          begin
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        else if ((~escape_char & (~received_esc & (ENCODING & (channel_char & ~eop_char)))) & ~sop_char) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((received_varchannel & received_esc) | (~received_channel & ((~sop_char & (~eop_char & ~escape_char)) & (received_varchannel & ~channel_char)))) 
