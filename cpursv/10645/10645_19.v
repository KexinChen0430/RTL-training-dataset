...
module altera_avalon_st_bytes_to_packets  #(parameter  CHANNEL_WIDTH = 8, ENCODING = 0)
  ...
  assign data_out = received_esc ? ((~in_data & 8'h20) | (in_data & (~8'h20 | (~in_data & 8'h20)))) : in_data;
          ...
          always @* 
              begin
                ...
                if ((out_ready | ~out_valid) && in_valid) 
                  begin
                    out_valid = 1;
                    if ((channel_char | escape_char) | (eop_char | sop_char)) out_valid = 0;
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if ((((~channel_char | received_esc) & received_channel) & ((~escape_char | received_esc) & (~eop_char | received_esc))) & (~sop_char | received_esc)) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if (((~channel_char | received_esc) & ((~varchannelesc_char & received_varchannel) & received_esc)) | ((~channel_char | received_esc) & ((~eop_char & (~varchannelesc_char & received_varchannel)) & (~escape_char & ~sop_char)))) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if ((out_ready | ~out_valid) && in_valid) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if ((eop_char | ((received_channel | received_varchannel) | channel_char)) | (escape_char | sop_char)) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        if ((~received_esc & ((ENCODING & channel_char) & ~escape_char)) & (~sop_char & ~eop_char)) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((((~escape_char | received_esc) & (received_esc | (~sop_char & ~eop_char))) & (received_varchannel & (~received_channel | received_esc))) & (~channel_char | received_esc)) 
                          begin
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        else if ((~received_esc & ((ENCODING & channel_char) & ~escape_char)) & (~sop_char & ~eop_char)) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((((~escape_char | received_esc) & (received_esc | (~sop_char & ~eop_char))) & (received_varchannel & (~received_channel | received_esc))) & (~channel_char | received_esc)) 
