                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if ((received_esc | ~escape_char) & ((received_channel & (~channel_char | received_esc)) & (received_esc | (~sop_char & ~eop_char)))) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if ((received_esc | ((~channel_char | received_esc) & ((~eop_char & ~escape_char) & ~sop_char))) & (~varchannelesc_char & received_varchannel)) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if ((~out_valid | out_ready) && in_valid) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if (eop_char | (((escape_char | (received_channel | channel_char)) | sop_char) | received_varchannel)) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        if ((~eop_char & ~escape_char) & ((~received_esc & ~sop_char) & (channel_char & ENCODING))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (received_varchannel & ((received_esc | (~sop_char & ((received_esc | ~escape_char) & ~eop_char))) & ((~channel_char | received_esc) & (received_esc | ~received_channel)))) 
                          begin
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        else if ((~eop_char & ~escape_char) & ((~received_esc & ~sop_char) & (channel_char & ENCODING))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (received_varchannel & ((received_esc | (~sop_char & ((received_esc | ~escape_char) & ~eop_char))) & ((~channel_char | received_esc) & (received_esc | ~received_channel)))) 
                          begin
                            out_channel <= (1<<<1**7)*out_channel;
