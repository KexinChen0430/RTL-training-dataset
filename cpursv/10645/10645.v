                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if (received_channel & (received_esc | (((~sop_char & ~eop_char) & ~escape_char) & ~channel_char))) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if ((received_varchannel & ~varchannelesc_char) & (received_esc | (((~sop_char & ~eop_char) & ~escape_char) & ~channel_char))) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if (!reset_n) 
                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        if ((channel_char & ENCODING) & (((~received_esc & ~sop_char) & ~eop_char) & ~escape_char)) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (received_varchannel & (received_esc | ((((~sop_char & ~eop_char) & ~escape_char) & ~channel_char) & ~received_channel))) 
                          begin
                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        ...
                        else if ((channel_char & ENCODING) & (((~received_esc & ~sop_char) & ~eop_char) & ~escape_char)) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (received_varchannel & (received_esc | ((((~sop_char & ~eop_char) & ~escape_char) & ~channel_char) & ~received_channel))) 
