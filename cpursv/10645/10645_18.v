          ...
          always @* 
              begin
                ...
                if (in_valid && (~out_valid | out_ready)) 
                  begin
                    out_valid = 1;
                    if ((channel_char | escape_char) | (sop_char | eop_char)) out_valid = 0;
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if (received_channel & ((received_esc & ((~escape_char | received_esc) & ((received_esc | (~sop_char & (~eop_char | received_esc))) & received_channel))) | ((~channel_char & received_esc) | (((~escape_char | received_esc) & (~sop_char & ~eop_char)) & ~channel_char)))) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if (((~escape_char | received_esc) & (received_esc | (~sop_char & (~eop_char | received_esc)))) & ((received_esc | ~channel_char) & (received_varchannel & ~varchannelesc_char))) 
                          begin
                            received_varchannel <= 0;
                          end
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        if ((ENCODING & channel_char) & (~eop_char & (~received_esc & (~escape_char & ~sop_char)))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (((received_varchannel & received_esc) | ((~escape_char & ~channel_char) & (received_varchannel & ~received_channel))) & ((received_varchannel & received_esc) | (~sop_char & ~eop_char))) 
                          begin
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        else if ((ENCODING & channel_char) & (~eop_char & (~received_esc & (~escape_char & ~sop_char)))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if (((received_varchannel & received_esc) | ((~escape_char & ~channel_char) & (received_varchannel & ~received_channel))) & ((received_varchannel & received_esc) | (~sop_char & ~eop_char))) 
