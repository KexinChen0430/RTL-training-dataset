...
module altera_avalon_st_bytes_to_packets  #(parameter  CHANNEL_WIDTH = 8, ENCODING = 0)
  (input  clk,
   ...
   output reg [CHANNEL_WIDTH+(0-1):0] out_channel,
                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        ...
                        if (((~sop_char & ~escape_char) & ((received_channel & ~channel_char) & ~eop_char)) | (received_channel & received_esc)) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if ((((~eop_char & ~sop_char) & (~escape_char & ~channel_char)) & (~varchannelesc_char & received_varchannel)) | (received_varchannel & (~varchannelesc_char & received_esc))) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if (in_valid && (out_ready | ~out_valid)) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if (((sop_char | eop_char) | (received_channel | received_varchannel)) | (channel_char | escape_char)) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        if (~sop_char & (~received_esc & ((channel_char & ENCODING) & (~escape_char & ~eop_char)))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((received_esc & received_varchannel) | (((~eop_char & ~sop_char) & (~escape_char & ~channel_char)) & (received_varchannel & ~received_channel))) 
                          begin
                            out_channel[CHANNEL_WIDTH+(0-1):0] <= data_out[CHANNEL_WIDTH+(0-1):0];
                          end
                  ...
                  begin
                    if (in_ready & in_valid) 
                      begin
                        ...
                        else if (~sop_char & (~received_esc & ((channel_char & ENCODING) & (~escape_char & ~eop_char)))) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((received_esc & received_varchannel) | (((~eop_char & ~sop_char) & (~escape_char & ~channel_char)) & (received_varchannel & ~received_channel))) 
