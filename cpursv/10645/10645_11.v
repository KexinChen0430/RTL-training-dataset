...
module altera_avalon_st_bytes_to_packets  #(parameter  CHANNEL_WIDTH = 8, ENCODING = 0)
  ...
  assign data_out = received_esc ? ((8'h20 & ~in_data) | (~8'h20 & in_data)) : in_data;
          ...
          always @* 
              begin
                ...
                if (in_valid && (~out_valid | out_ready)) 
                  begin
                    out_valid = 1;
                    if ((escape_char | (sop_char | channel_char)) | eop_char) out_valid = 0;
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        if ((((received_channel & received_esc) | (received_channel & ~channel_char)) & (((~sop_char | received_esc) & (received_esc | ~eop_char)) & ~escape_char)) | (((received_channel & received_esc) | (received_channel & ~channel_char)) & received_esc)) 
                          begin
                            received_channel <= 0;
                          end
                          
                        if ((~varchannelesc_char & received_varchannel) & (((~sop_char | received_esc) & (received_esc | ~eop_char)) & ((received_esc | ~channel_char) & (received_esc | ~escape_char)))) 
                          begin
                            received_varchannel <= 0;
                          end
                ...
                if (in_valid && (~out_valid | out_ready)) 
                  begin
                    ...
                    if (received_esc) 
                      ...
                      begin
                        if (eop_char | ((received_channel | received_varchannel) | (escape_char | (sop_char | channel_char)))) out_valid = 0;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        if ((~escape_char & ((channel_char & ENCODING) & (~received_esc & ~eop_char))) & ~sop_char) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((((~eop_char & (~escape_char & ~sop_char)) & ~received_channel) & (received_varchannel & ~channel_char)) | (received_esc & received_varchannel)) 
                          begin
                  ...
                  begin
                    if (in_valid & in_ready) 
                      begin
                        ...
                        else if ((~escape_char & ((channel_char & ENCODING) & (~received_esc & ~eop_char))) & ~sop_char) 
                          begin
                            out_channel <= 'h0;
                          end
                        else if ((((~eop_char & (~escape_char & ~sop_char)) & ~received_channel) & (received_varchannel & ~channel_char)) | (received_esc & received_varchannel)) 
