...
module ui_cmd  #(parameter 
  ...
  input  wr_req_16;
  wire 
       app_rdy_ns = ~wr_req_16 && (accept_ns && ~rd_buf_full);
  reg  app_rdy_r;
  ...
  generate
      begin
        if (MEM_ADDR_ORDER == ROW_BANK_COLUMN) 
          begin
            assign row = app_rdy_r ? app_addr_r1[BANK_WIDTH+COL_WIDTH +: ROW_WIDTH] : app_addr_r2[BANK_WIDTH+COL_WIDTH +: ROW_WIDTH];
  ...
  assign rank = (RANKS == 1) ? 1'b0 : 
                app_rdy_r ? app_addr_r1[(BANK_WIDTH+COL_WIDTH)+ROW_WIDTH +: RANK_WIDTH] : app_addr_r2[(BANK_WIDTH+COL_WIDTH)+ROW_WIDTH +: RANK_WIDTH];
