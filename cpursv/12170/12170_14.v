...
module reorder_buffer_beh(input  rst,clk,
  ...
  assign write = !full && (!recover && (isDispatch && !hazard_stall));
  assign read = retire_reg && !hazard_stall && !empty && !recover;
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if (!read && write) status_cnt <= 1+status_cnt;
        else if (read && !write) status_cnt <= status_cnt-1;
        else if (dec_tail) status_cnt <= status_cnt-1;
