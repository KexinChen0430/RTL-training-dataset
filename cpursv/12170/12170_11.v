...
module reorder_buffer_beh(input  rst,clk,
  ...
  assign write = !hazard_stall && (!recover && !full) && isDispatch;
  assign read = !hazard_stall && (retire_reg && !recover && !empty);
  ...
  always @(posedge clk or negedge rst)
      begin
        if (!rst) tail <= 4'h0;
        else if (dec_tail) tail <= (0-1)+tail;
        else if (write) 
          begin
            tail <= 1+tail;
  ...
  always @(posedge clk or negedge rst)
      begin
        ...
        else if (write && !read) status_cnt <= status_cnt+1;
        else if (!write && read) status_cnt <= (0-1)+status_cnt;
        else if (dec_tail) status_cnt <= (0-1)+status_cnt;
  ...
  assign rd_flush = rob[(0-1)+tail][16:12];
  assign PR_old_flush = rob[(0-1)+tail][11:6];
  assign PR_new_flush = rob[(0-1)+tail][5:0];
  assign RegDest_out = rob[(0-1)+tail][18];
endmodule
