...
module mig_7series_v4_0_tg_status  #(parameter  TCQ = 100, DWIDTH = 32)
  (input  clk_i,
   input  rst_i,
   input  manual_clear_error,
   input  data_error_i,
   input  [(0-1)+DWIDTH:0] cmp_data_i,
   input  [(0-1)+DWIDTH:0] rd_data_i,
   input  [31:0] cmp_addr_i,
   input  [5:0] cmp_bl_i,
   input  mcb_cmd_full_i,
   input  mcb_wr_full_i,
   input  mcb_rd_empty_i,
   output reg [(DWIDTH*1<<1)+((0-1)+64):0] error_status,
   output error);
  ...
  always @(posedge clk_i)
      begin
        if (rst_i || manual_clear_error) 
          ...
          begin
            if (~data_error_r && (data_error_i && ~error_set)) 
              begin
                ...
                error_set <= #TCQ 1'b1;
                error_status[DWIDTH+((0-1)+64):64] <= #TCQ cmp_data_i;
                error_status[(DWIDTH*1<<1)+((0-1)+64):DWIDTH+64] <= #TCQ rd_data_i;
              end
