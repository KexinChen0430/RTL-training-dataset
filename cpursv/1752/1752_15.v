          ...
          begin
            ...
            for (c = 1; c < (WIDTH-1); c = 1+c)
                begin
                  ...
                  assign sum_i = ((in_states[(r*WIDTH)+((1+(0-WIDTH))+c)]+(in_states[((r*WIDTH)+c)+(-1)]+in_states[(r*WIDTH)+(c+(0-WIDTH))]))+in_states[(c+(0-WIDTH))+((r*WIDTH)+(-1))])+((in_states[c+((WIDTH+(r*WIDTH))+(-1))]+(in_states[(((r*WIDTH)+c)+1)+WIDTH]+in_states[(WIDTH+(r*WIDTH))+c]))+in_states[((r*WIDTH)+c)+1]);
                  wire eq2_i,eq3_i;
                  ...
                  assign next_state_i = (cur_state_i & eq2_i) | eq3_i;
