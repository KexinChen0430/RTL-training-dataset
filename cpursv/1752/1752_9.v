  ...
  generate
      ...
      for (r = 1; r < (HEIGHT-1); r = r+1)
          begin
            ...
            assign out_states[(0-1)+((r+1)*WIDTH)] = 0;
            for (c = 1; c < ((0-1)+WIDTH); c = c+1)
                begin
                  ...
                  assign sum_i = ((in_states[(c+(r*WIDTH))-WIDTH]+in_states[(r*WIDTH)+(c+((-WIDTH)+(0-1)))])+in_states[((0-1)+WIDTH)+(c+(r*WIDTH))])+((in_states[(0-1)+(c+(r*WIDTH))]+in_states[c+((r*WIDTH)+((-WIDTH)+1))])+(in_states[(r*WIDTH)+(c+1)]+(in_states[c+((r*WIDTH)+WIDTH)]+in_states[(r*WIDTH)+(1+(WIDTH+c))])));
                  wire eq2_i,eq3_i;
                  ...
                  assign next_state_i = (eq3_i | (eq2_i | (eq3_i & ~cur_state_i))) & (cur_state_i | eq3_i);
