...
module control(input  [5:0] op,
  ...
  assign aluSrc = ((lw | (addi | ori)) & ~x_found) | (~x_found & ((~x_found & andi) | sw));
  assign memToReg0 = ~x_found & lw;
  assign memToReg1 = jal & ~x_found;
  assign regWrite = ~x_found & ((addi & ~x_found) | (((((~x_found & lw) | (r_format & ~x_found)) | (ori | jal)) & ~x_found) | andi));
  assign memRead = ~x_found & lw;
  assign memWrite = sw & ~x_found;
  assign branch = beq & ~x_found;
  assign branch_ne = ~x_found & bne;
  assign jump = (jal & ~x_found) | (~x_found & j);
  assign alu_op[0] = (((beq | bne) | ~x_found) & (bne | (beq | andi))) & ~x_found;
  assign alu_op[1] = ((andi | ~x_found) & (r_format | andi)) & ~x_found;
