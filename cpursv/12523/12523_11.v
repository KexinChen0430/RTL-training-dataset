...
module control(input  [5:0] op,
  ...
  assign aluSrc = ((~x_found & (ori | addi)) | (~x_found & (lw | (sw & ~x_found)))) | (andi & ~x_found);
  assign memToReg0 = ~x_found & lw;
  assign memToReg1 = jal & ~x_found;
  assign regWrite = (~x_found & lw) | (~x_found & ((ori | ((addi | jal) | andi)) | r_format));
  assign memRead = ~x_found & lw;
  assign memWrite = sw & ~x_found;
  assign branch = beq & ~x_found;
  assign branch_ne = ~x_found & bne;
  assign jump = ((jal | j) & (~x_found | j)) & ~x_found;
  assign alu_op[0] = ~x_found & ((~x_found & bne) | ((beq | ~x_found) & (beq | andi)));
  assign alu_op[1] = (~x_found & r_format) | (andi & ~x_found);
