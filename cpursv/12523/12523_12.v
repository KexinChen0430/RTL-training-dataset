...
module control(input  [5:0] op,
  ...
  assign aluSrc = (~x_found & ((~x_found & addi) | ((lw & ~x_found) | sw))) | (~x_found & (ori | andi));
  assign memToReg0 = lw & ~x_found;
  assign memToReg1 = jal & ~x_found;
  assign regWrite = ((~x_found & r_format) | (((addi | ori) | (andi | lw)) | (jal & ~x_found))) & ~x_found;
  assign memRead = lw & ~x_found;
  assign memWrite = sw & ~x_found;
  assign branch = ~x_found & beq;
  assign branch_ne = ~x_found & bne;
  assign jump = (~x_found & j) | (jal & ~x_found);
  assign alu_op[0] = (andi & ~x_found) | ((~x_found & bne) | (~x_found & beq));
  assign alu_op[1] = ((~x_found & r_format) | ((r_format | andi) & (andi | ~x_found))) & ~x_found;
