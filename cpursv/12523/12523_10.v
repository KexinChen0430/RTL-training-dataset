...
module control(input  [5:0] op,
  ...
  assign aluSrc = (((~x_found & sw) | ((lw | (addi | ori)) & ~x_found)) | (andi & ((~x_found | r_format) & (r_format | andi)))) & ~x_found;
  assign memToReg0 = lw & ~x_found;
  assign memToReg1 = jal & ~x_found;
  assign regWrite = (((lw | r_format) | andi) | ((jal & ~x_found) | (addi | ori))) & ~x_found;
  assign memRead = lw & ~x_found;
  assign memWrite = ~x_found & sw;
  assign branch = beq & ~x_found;
  assign branch_ne = bne & ~x_found;
  assign jump = ~x_found & ((jal | j) & (~x_found | j));
  assign alu_op[0] = (bne & ~x_found) | ((beq & ~x_found) | (~x_found & andi));
  assign alu_op[1] = ~x_found & ((~x_found | r_format) & (r_format | andi));
