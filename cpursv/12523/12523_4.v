...
module control(input  [5:0] op,
  ...
  assign aluSrc = ((~x_found & ori) | (sw | ((addi | andi) | lw))) & ~x_found;
  assign memToReg0 = lw & ~x_found;
  assign memToReg1 = jal & ~x_found;
  assign regWrite = ~x_found & ((~x_found & (((~x_found & andi) | ((lw & ~x_found) | (r_format & ~x_found))) | (ori | jal))) | (((andi | ((lw | r_format) | ori)) | ~x_found) & (((addi | andi) | ((lw | r_format) | ori)) | (jal & ~x_found))));
  assign memRead = lw & ~x_found;
  assign memWrite = ~x_found & sw;
  assign branch = ~x_found & beq;
  assign branch_ne = ~x_found & bne;
  assign jump = ~x_found & ((j | ~x_found) & (j | jal));
  assign alu_op[0] = (andi | (bne | beq)) & ~x_found;
