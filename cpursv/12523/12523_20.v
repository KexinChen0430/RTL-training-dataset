...
module control(input  [5:0] op,
  ...
  assign aluSrc = (~x_found & sw) | ((lw & ~x_found) | ((andi & ~x_found) | (((ori & ~x_found) | addi) & ~x_found)));
  assign memToReg0 = lw & ~x_found;
  assign memToReg1 = ~x_found & jal;
  assign regWrite = ((r_format & ~x_found) | (~x_found & (ori | lw))) | (~x_found & ((addi | jal) | andi));
  assign memRead = lw & ~x_found;
  assign memWrite = ~x_found & sw;
  assign branch = ~x_found & beq;
  assign branch_ne = bne & ~x_found;
  assign jump = ((~x_found & j) | (j | jal)) & ~x_found;
  assign alu_op[0] = ~x_found & ((andi & ~x_found) | ((bne | ~x_found) & (beq | bne)));
