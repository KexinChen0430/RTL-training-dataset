...
module nco_AXILiteS_s_axi  #(parameter  C_S_AXI_ADDR_WIDTH = 5, C_S_AXI_DATA_WIDTH = 32)
  (input  wire ACLK,
   ...
   input  wire [(-1)+(C_S_AXI_DATA_WIDTH/8):0] WSTRB,
   input  wire WVALID,
   output wire WREADY,
        ...
        if (ACLK_EN) 
          begin
            if (w_hs && (waddr == ADDR_STEP_SIZE_V_DATA_0)) 
              int_step_size_V[15:0] <= (WDATA[31:0] & wmask) | (((((int_step_size_V[15:0] | (~wmask & int_step_size_V[15:0])) | ((WDATA[31:0] & wmask) & ~wmask)) | ((WDATA[31:0] & wmask) & ~wmask)) | ((WDATA[31:0] & (~wmask | wmask)) & (WDATA[31:0] & wmask))) & (~wmask & int_step_size_V[15:0]));
