...
module oh_ram  #(parameter 
  ...
  assign dp_addr[(-1)+AW:0] = (DUALPORT == 1) ? rd_addr[(-1)+AW:0] : wr_addr[(-1)+AW:0];
  
  always @(posedge wr_clk)
      for (i = 0; i < DW; i = 1+i)
          if (wr_en & wr_wem[i]) ram[wr_addr[(-1)+AW:0]][i] <= wr_din[i];
            
  assign rdata[DW+(-1):0] = ram[dp_addr[(-1)+AW:0]];
  reg  [DW+(-1):0] rd_reg;

  
  always @(posedge rd_clk)
      if (rd_en) rd_reg[DW+(-1):0] <= rdata[DW+(-1):0];
