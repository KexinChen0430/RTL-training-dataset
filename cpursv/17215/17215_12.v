...
module e16_arbiter_roundrobin(grants,clk,clk_en,reset,en_rotate,requests);
  ...
  reg  [(ARW*1<<1)-1:0] grants_rotate_buffer;
  reg  [(-1)+ARW:0] grants;
  wire [(-1)+ARW:0] shifted_requests[(-1)+ARW:0];
  wire [(-1)+ARW:0] shifted_grants[(-1)+ARW:0];
  wire [(ARW*1<<1)-1:0] requests_rotate_buffer;
  ...
  assign requests_rotate_buffer[(ARW*1<<1)-1:0] = {requests[(-1)+ARW:0],requests[(-1)+ARW:0]};
  genvar i;
  ...
  always @* 
      begin
        ...
        for (m = 0; m < ARW; m = m+1)
            begin
              grants_rotate_buffer[(ARW*1<<1)-1:0] = {shifted_grants[m],shifted_grants[m]};
              grants[(-1)+ARW:0] = (((grants_rotate_buffer[((ARW*1<<1)+(0-m))+(-1) -: ARW] | {ARW{request_mask[m]}}) & (grants[(-1)+ARW:0] | {ARW{request_mask[m]}})) & (grants_rotate_buffer[((ARW*1<<1)+(0-m))+(-1) -: ARW] | grants[(-1)+ARW:0])) | grants[(-1)+ARW:0];
            end
      end
endmodule
