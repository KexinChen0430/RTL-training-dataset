...
module e16_arbiter_roundrobin(grants,clk,clk_en,reset,en_rotate,requests);
  ...
  assign requests_rotate_buffer[ARW<<1+(0-1):0] = {requests[ARW-1:0],requests[ARW-1:0]};
  genvar i;
      ...
      begin
        ...
        for (m = 0; m < ARW; m = m+1)
            begin
              ...
              grants[ARW-1:0] = grants[ARW-1:0] | ({ARW{request_mask[m]}} & (grants[ARW-1:0] | grants_rotate_buffer[((-m)+(0-1))+ARW<<1 -: ARW]));
            end
      end
endmodule
