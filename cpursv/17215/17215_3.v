...
module e16_arbiter_roundrobin(grants,clk,clk_en,reset,en_rotate,requests);
  ...
  reg  [(1<<1*ARW)-1:0] grants_rotate_buffer;
  reg  [ARW+(-1):0] grants;
  wire [ARW+(-1):0] shifted_requests[ARW+(-1):0];
  wire [ARW+(-1):0] shifted_grants[ARW+(-1):0];
  wire [(1<<1*ARW)-1:0] requests_rotate_buffer;
  ...
  always @(posedge clk or posedge reset)
      if (reset) request_mask[ARW+(-1):0] <= {{ARW+(-1){1'b0}},1'b1};
        ...
        if (en_rotate) 
          request_mask[ARW+(-1):0] <= {request_mask[(0-1<<1)+ARW:0],request_mask[ARW+(-1)]};
          
        
  assign requests_rotate_buffer[(1<<1*ARW)-1:0] = {requests[ARW+(-1):0],requests[ARW+(-1):0]};
  genvar i;
  ...
  always @* 
      begin
        ...
        for (m = 0; m < ARW; m = m+1)
            begin
              grants_rotate_buffer[(1<<1*ARW)-1:0] = {shifted_grants[m],shifted_grants[m]};
              grants[ARW+(-1):0] = grants[ARW+(-1):0] | ((grants_rotate_buffer[(0-m)+((1<<1*ARW)-1) -: ARW] | ((grants_rotate_buffer[(0-m)+((1<<1*ARW)-1) -: ARW] & {ARW{request_mask[m]}}) | ({ARW{request_mask[m]}} & grants[ARW+(-1):0]))) & (grants[ARW+(-1):0] | {ARW{request_mask[m]}}));
            end
      end
endmodule
