...
module e16_arbiter_roundrobin(grants,clk,clk_en,reset,en_rotate,requests);
  ...
  always @(posedge clk or posedge reset)
      if (reset) request_mask[(-1)+ARW:0] <= {{(-1)+ARW{1'b0}},1'b1};
        ...
        if (en_rotate) 
          request_mask[(-1)+ARW:0] <= {request_mask[ARW-(1+1):0],request_mask[(-1)+ARW]};
      ...
      begin
        ...
        for (m = 0; m < ARW; m = m+1)
            begin
              ...
              grants[(-1)+ARW:0] = grants[(-1)+ARW:0] | ((grants_rotate_buffer[(ARW<<1+(-1))+(-m) -: ARW] | grants[(-1)+ARW:0]) & {ARW{request_mask[m]}});
            end
      end
endmodule
