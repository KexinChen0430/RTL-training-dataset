...
module e16_arbiter_roundrobin(grants,clk,clk_en,reset,en_rotate,requests);
  ...
  assign requests_rotate_buffer[(-1)+(1<<1*ARW):0] = {requests[ARW+(-1):0],requests[ARW+(-1):0]};
  genvar i;
      ...
      begin
        ...
        for (m = 0; m < ARW; m = m+1)
            begin
              ...
              grants[ARW+(-1):0] = (grants[ARW+(-1):0] | {ARW{request_mask[m]}}) & (grants_rotate_buffer[(1<<1*ARW)+((-m)+(-1)) -: ARW] | grants[ARW+(-1):0]);
            end
      end
endmodule
