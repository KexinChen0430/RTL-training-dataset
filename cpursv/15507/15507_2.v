...
module ac97_ctlif  #(parameter  csr_addr = 4'h0)
  (input  sys_clk,
   input  sys_rst,
   input  [13:0] csr_a,
   input  csr_we,
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) dmar_finished_r <= 1'b1;
        else dmar_finished_r <= dmar_finished;
      end
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) dmaw_finished_r <= 1'b1;
        else dmaw_finished_r <= dmaw_finished;
      end
      ...
      begin
        if (sys_rst) 
          ...
          begin
            ...
            dmar_irq <= 1'b0;
            dmaw_irq <= 1'b0;
            if (down_next_frame & down_en) 
              begin
                down_addr_valid <= request_en;
                down_addr <= {~request_write,request_addr,12'd0} & {20{request_en}};
                down_data_valid <= request_write & request_en;
                down_data <= {request_data,4'd0} & {20{request_write & request_en}};
                request_en <= 1'b0;
                if (request_en) crrequest_irq <= 1'b1;
                  
              end
              
            if (up_en & up_next_frame) 
              begin
                if ((up_data_valid & up_addr_valid) & up_frame_valid) 
                  begin
                    crreply_irq <= 1'b1;
            ...
            if (dmar_next) 
              begin
                dmar_addr <= 30'd1+dmar_addr;
                dmar_remaining <= dmar_remaining+(0-16'd1);
              end
              
            if (dmaw_next) 
              begin
                dmaw_addr <= 30'd1+dmaw_addr;
                dmaw_remaining <= (0-16'd1)+dmaw_remaining;
              end
              
            if (~dmar_finished_r & dmar_finished) dmar_irq <= 1'b1;
              
            if (~dmaw_finished_r & dmaw_finished) dmaw_irq <= 1'b1;
                ...
                if (csr_we) 
                  begin
                    case (csr_a[3:0])
                      ...
                      4'b0101: dmar_addr <= csr_di[31:1<<<1];

                      4'b0110: dmar_remaining <= csr_di[17:1<<<1];

                      4'b1000: dmaw_en <= csr_di[0];

                      4'b1001: dmaw_addr <= csr_di[31:1<<<1];

                      4'b1010: dmaw_remaining <= csr_di[17:1<<<1];
