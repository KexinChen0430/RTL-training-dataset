...
module collision(input  clk,
  ...
  assign bm = ((((clxcon[5:0] | ~bpldata[5:0]) & ~bpldata[5:0]) & ~clxcon[5:0]) | ~clxcon[11:6]) | ((bpldata[5:0] | ~clxcon[11:6]) & clxcon[5:0]);
  assign oddmatch = (bm[4] & (bm[2] & bm[0])) & ((dblpf | (bm[2] & evenmatch)) & (dblpf | (bm[4] & bm[0])));
  assign evenmatch = bm[1] & (bm[3] & bm[5]);
  assign sprmatch[0] = nsprite[0] | (nsprite[1] & clxcon[12]);
  assign sprmatch[1] = nsprite[2] | (clxcon[13] & ((clxcon[13] & nsprite[2]) | nsprite[3]));
  assign sprmatch[2] = (nsprite[5] | nsprite[4]) & (((nsprite[5] & nsprite[4]) | clxcon[14]) | nsprite[4]);
