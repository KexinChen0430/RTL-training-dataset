...
module collision(input  clk,
  ...
  assign bm = ((clxcon[5:0] | ~clxcon[11:6]) | (~bpldata[5:0] & ~clxcon[5:0])) & ((~clxcon[5:0] | (bpldata[5:0] & clxcon[5:0])) | ~clxcon[11:6]);
  assign oddmatch = (bm[2] & bm[4]) & (((bm[2] & (bm[4] & bm[0])) | (dblpf & bm[0])) & (evenmatch | dblpf));
  assign evenmatch = bm[5] & (bm[1] & bm[3]);
  assign sprmatch[0] = nsprite[0] | (nsprite[1] & clxcon[12]);
  assign sprmatch[1] = ((nsprite[3] & nsprite[2]) | ((nsprite[3] & clxcon[13]) | (nsprite[3] & nsprite[2]))) | nsprite[2];
  assign sprmatch[2] = nsprite[4] | (clxcon[14] & nsprite[5]);
  assign sprmatch[3] = (clxcon[15] | nsprite[6]) & (nsprite[6] | nsprite[7]);
  wire [14:0] cl;
