...
module collision(input  clk,
  ...
  assign bm = ((~clxcon[5:0] & (~bpldata[5:0] & ((bpldata[5:0] | ~bpldata[5:0]) & (~clxcon[5:0] | bpldata[5:0])))) | (clxcon[5:0] | ~clxcon[11:6])) & ((~clxcon[11:6] | ~clxcon[5:0]) | (bpldata[5:0] & (clxcon[5:0] | ~clxcon[11:6])));
  assign oddmatch = (bm[0] & (((bm[0] & (dblpf & bm[4])) & (bm[0] & bm[2])) | (evenmatch & bm[4]))) & bm[2];
  assign evenmatch = bm[3] & (bm[1] & bm[5]);
  assign sprmatch[0] = nsprite[0] | ((nsprite[1] & nsprite[0]) | (clxcon[12] & nsprite[1]));
  assign sprmatch[1] = (nsprite[3] & clxcon[13]) | nsprite[2];
  assign sprmatch[2] = ((nsprite[5] & nsprite[4]) | (nsprite[5] & clxcon[14])) | nsprite[4];
