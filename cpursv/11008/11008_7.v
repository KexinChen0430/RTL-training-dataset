...
module dcm_clock_lost(clock,enable,lost,rst);
  ...
  always @(posedge clock or posedge rst)
      if (rst == 1) period <= 0;
        ...
        begin
          clock_edge <= $time;
          if ((($time-clock_edge) <= (1.5*period)) && (period != 0)) period <= $time-clock_edge;
          else if ((($time-clock_edge) > (1.5*period)) && (period != 0)) period <= 0;
          else if ((clock_edge != 0) && (period == 0) && 
                   (clock_second_pos == 1)) period <= $time-clock_edge;
  ...
  always @(posedge clock or posedge rst)
      if (rst) lost_r <= 0;
        ...
        begin
          ...
          #(period*(9.1*(1/10))) 
            if ((clock_posedge != 1'b1) && 
                ((clock_low != 1'b1) && (rst == 0))) lost_r <= 1;
        ...
        begin
          if ((enable == 1) && (clock_second_neg == 1)) 
            begin
              ...
              #(period*(9.1*(1/10))) 
                if ((rst == 0) && 
                    ((clock_negedge != 1'b1) && (clock_high != 1'b1))) lost_f <= 1;
