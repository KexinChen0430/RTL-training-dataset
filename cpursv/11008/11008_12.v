...
module dcm_clock_lost(clock,enable,lost,rst);
  ...
  always @(posedge clock or posedge rst)
      if (rst == 1) period <= 0;
        ...
        begin
          clock_edge <= $time;
          if ((((0-clock_edge)+$time) <= (1.5*period)) && 
              (period != 0)) period <= (0-clock_edge)+$time;
          else if ((((0-clock_edge)+$time) > (1.5*period)) && 
                   (period != 0)) period <= 0;
          else if ((period == 0) && 
                   ((clock_edge != 0) && (clock_second_pos == 1))) period <= (0-clock_edge)+$time;
  ...
  always @(posedge clock or posedge rst)
      if (rst) lost_r <= 0;
        ...
        begin
          ...
          #(period*((1/10)*9.1)) 
            if ((clock_low != 1'b1) && 
                ((rst == 0) && (clock_posedge != 1'b1))) lost_r <= 1;
