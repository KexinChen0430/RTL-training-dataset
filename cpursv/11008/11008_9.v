  ...
  always @(posedge clock or posedge rst)
      if (rst == 1) period <= 0;
        ...
        begin
          clock_edge <= $time;
          if ((period != 0) && 
              (($time+(-clock_edge)) <= (1.5*period))) period <= $time+(-clock_edge);
          else if ((period != 0) && 
                   (($time+(-clock_edge)) > (1.5*period))) period <= 0;
  ...
  always @(posedge clock or posedge rst)
      if (rst) lost_r <= 0;
        ...
        begin
          ...
          #(period*((1/10)*9.1)) 
            if ((rst == 0) && 
                ((clock_low != 1'b1) && (clock_posedge != 1'b1))) lost_r <= 1;
        ...
        begin
          if ((clock_second_neg == 1) && (enable == 1)) 
            begin
              ...
              #(period*((1/10)*9.1)) 
                if ((rst == 0) && 
                    ((clock_high != 1'b1) && (clock_negedge != 1'b1))) lost_f <= 1;
