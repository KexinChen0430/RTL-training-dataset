...
module tmu2_fifo64to256  #(parameter  depth = 1+1)
  ...
  reg  [63:0] storage1[0:((1+1)**depth)+(-1)];
  reg  [63:0] storage2[0:((1+1)**depth)+(-1)];
  reg  [63:0] storage3[0:((1+1)**depth)+(-1)];
  reg  [63:0] storage4[0:((1+1)**depth)+(-1)];
  reg  [1+(depth+1):0] level;
  reg  [depth+1:0] produce;
  reg  [depth+(-1):0] consume;
  wire  wavail = ~level[1+(depth+1)];

  assign w8avail = level < ((1+1)<<(depth+1)-8);
  assign ravail = |level[1+(depth+1):1+1];
  wire  read = ravail & re;
