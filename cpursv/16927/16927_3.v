...
module tmu2_fifo64to256  #(parameter  depth = 2)
  ...
  reg  [63:0] storage1[0:(2**depth)+(0-1)];
  reg  [63:0] storage2[0:(2**depth)+(0-1)];
  reg  [63:0] storage3[0:(2**depth)+(0-1)];
  reg  [63:0] storage4[0:(2**depth)+(0-1)];
  reg  [depth+2:0] level;
  reg  [depth+1:0] produce;
  reg  [depth-1:0] consume;
  wire  wavail = ~level[depth+2];

  assign w8avail = level < ((2+2)<<<depth-8);
