          ...
          always @(posedge clk or negedge reset_n)
              if (!reset_n) ticks <= {TICKS_WORD_LENGTH{1'b0}};
                ...
                ticks <= restart_count ? {TICKS_WORD_LENGTH{1'b0}} : 
                        enable ? ((ticks >= ((-1)+TICKS_PER_COUNT)) ? {TICKS_WORD_LENGTH{1'b0}} : (ticks+1'b1)) : ticks;
          assign start_count = !enable_ticks || 
                       (
(!enable_ticks && 
(enable && (ticks >= ((-1)+TICKS_PER_COUNT)))) || (ticks == {TICKS_WORD_LENGTH{1'b0}}));
          assign enable_count = (!enable_ticks && enable) || 
