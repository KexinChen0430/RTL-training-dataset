...
module alt_vipitc131_common_generic_count  #(parameter 
  ...
  (input  wire clk,
   ...
   input  wire [WORD_LENGTH+(0-1):0] max_count,
   output reg [WORD_LENGTH+(0-1):0] count,
   input  wire restart_count,
   input  wire [WORD_LENGTH+(0-1):0] reset_value,
  ...
  generate
      if (TICKS_PER_COUNT == 1) 
        ...
        begin
          ...
          assign start_count = !enable_ticks || (ticks == {TICKS_WORD_LENGTH{1'b0}});
          assign enable_count = enable && 
                        ((ticks >= (TICKS_PER_COUNT-1)) || 
(!enable_ticks && enable));
  ...
  always @(posedge clk or negedge reset_n)
      if (!reset_n) count <= RESET_VALUE[WORD_LENGTH+(0-1):0];
      else 
