...
module serial_txd(input  CLK_I,
  ...
  always @(posedge CLK_I)
      begin
        ...
        else if ((CYC_I == 1'b1) && (STB_I == 1'b1) && (ACK_O == 1'b0) && 
                 (WE_I == 1'b1)) 
          begin
            if (counter < 13'd8191) counter <= 13'd1+counter;
              
            if (counter < 434) uart_txd <= 1'b0;
            else if (counter < (2*434)) uart_txd <= DAT_I[0];
            else if (counter < (3*434)) uart_txd <= DAT_I[1];
            else if (counter < (4*434)) uart_txd <= DAT_I[2];
            else if (counter < (434*5)) uart_txd <= DAT_I[3];
            else if (counter < (6*434)) uart_txd <= DAT_I[4];
            else if (counter < (434*7)) uart_txd <= DAT_I[5];
            else if (counter < (434*8)) uart_txd <= DAT_I[6];
            else if (counter < (9*434)) uart_txd <= DAT_I[7];
            else if (counter < (10*434)) uart_txd <= 1'b1;
            else 
              begin
                uart_txd <= 1'b1;
                ACK_O <= 1'b1;
