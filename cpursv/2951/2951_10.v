...
module wr_memory(wr_data,wr_clk,wr_en,wr_addr,remapping_memory,full,reset);
  ...
  input  wire [WR_DATA_WIDTH+(0-1):0] wr_data;
  input  wire [WR_ADDR_WIDTH+(0-1):0] wr_addr;
  input  wire wr_clk;
  input  wire wr_en;
  input  wire full;
  output reg [(0-1)+MEM_DEPTH:0] remapping_memory;
  input  wire reset;
  reg  [WR_DATA_WIDTH+(0-1):0] memory[(0-1)+MEM_DEPTH:0];
  integer i;
