...
module wr_memory(wr_data,wr_clk,wr_en,wr_addr,remapping_memory,full,reset);
  ...
  input  wire [(0-1)+WR_DATA_WIDTH:0] wr_data;
  input  wire [(0-1)+WR_ADDR_WIDTH:0] wr_addr;
  input  wire wr_clk;
  input  wire wr_en;
  input  wire full;
  output reg [MEM_DEPTH+(0-1):0] remapping_memory;
  input  wire reset;
  reg  [(0-1)+WR_DATA_WIDTH:0] memory[MEM_DEPTH+(0-1):0];
  integer i;
