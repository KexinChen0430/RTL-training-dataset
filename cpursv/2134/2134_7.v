...
module deep_sump_ram  #(parameter  depth_len = 65536, depth_bits = 16)
  (input  wire a_clk,
   input  wire b_clk,
   input  wire a_we,
   input  wire [(0-1)+depth_bits:0] a_addr,
   input  wire [63:0] a_di,
   output wire a_overrun,
   input  wire b_rd_req,
   input  wire [(0-1)+depth_bits:0] b_addr,
   output wire [63:0] b_do);

  reg  [63:0] rle_ram_array[depth_len+(0-1):0];
  reg  [(0-1)+depth_bits:0] a_addr_p1;
  reg  [(0-1)+depth_bits:0] a_addr_p2;
  reg  a_we_p1;
  reg  a_we_p2;
  reg  [63:0] a_di_p1;
  reg  [63:0] a_di_p2;
  reg  [(0-1)+depth_bits:0] b_addr_p1;
  reg  [63:0] b_do_loc;
  reg  [63:0] b_do_p1;
