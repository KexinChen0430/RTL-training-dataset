...
module SensorFSM  #(parameter  DataWidth = 8)
  (input  Reset_n_i,
   ...
   output [(DataWidth*1<<1)-1:0] SensorValueL_o,
   output [(DataWidth*1<<1)-1:0] SensorValueR_o,
   ...
   input  [(DataWidth*1<<1)-1:0] ParamCounterPresetH_i,
   input  [(DataWidth*1<<1)-1:0] ParamCounterPresetL_i);
  ...
  reg  [(DataWidth*1<<1)-1:0] WordL;
  reg  [(DataWidth*1<<1)-1:0] WordR;
