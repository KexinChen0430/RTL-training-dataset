...
module SensorFSM  #(parameter  DataWidth = 8)
  (input  Reset_n_i,
   ...
   output [(-1)+(2*DataWidth):0] SensorValueL_o,
   output [(-1)+(2*DataWidth):0] SensorValueR_o,
   ...
   input  [(-1)+(2*DataWidth):0] ParamCounterPresetH_i,
   input  [(-1)+(2*DataWidth):0] ParamCounterPresetL_i);
  ...
  wire [(-1)+(2*DataWidth):0] SensorValue;
  reg  [(-1)+(2*DataWidth):0] WordL;
  reg  [(-1)+(2*DataWidth):0] WordR;
