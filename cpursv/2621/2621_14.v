...
module uart_rx(input  clock,
        ...
        if (uart_tick_16x) 
          begin
            if (~clock_lock) 
              ...
              begin
                clock_lock <= ((RxD_bit == 1'b1) && (state == IDLE)) ? 1'b0 : clock_lock;
              end
            bit_spacing <= clock_lock ? (1'b1+bit_spacing) : 4'b1110;
          end
        else 
          begin
        ...
        if (reset) 
          ...
          begin
            case (state)
              ...
              IDLE: state <= ((RxD_bit == 1'b0) & next_bit) ? BIT_0 : IDLE;
  ...
  wire 
       capture = ((state != STOP) & (next_bit & uart_tick_16x)) & (state != IDLE);
  ...
  assign data_ready = next_bit & ((state == STOP) & uart_tick_16x);
endmodule
