...
module HaarFilter  #(parameter 
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (dataInD1*(1<<<INTERNAL_WIDTH*1<<(0-IN_WIDTH)));
      assign in1 = counter[0] ? lowPass[index2] : ((1<<<INTERNAL_WIDTH*1<<(0-IN_WIDTH))*dataIn);
    end
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]>>>(0-OUT_WIDTH)+INTERNAL_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES-1)-outArray]>>>(0-OUT_WIDTH)+INTERNAL_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]<<<(-INTERNAL_WIDTH)*(2**OUT_WIDTH);
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES-1)-outArray]<<OUT_WIDTH-INTERNAL_WIDTH;
                end
          end
    end
endmodule
