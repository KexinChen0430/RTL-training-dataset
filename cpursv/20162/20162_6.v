...
module HaarFilter  #(parameter 
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (1<<<(0-IN_WIDTH)*dataInD1<<INTERNAL_WIDTH);
      assign in1 = counter[0] ? lowPass[index2] : (1<<<(0-IN_WIDTH)*dataIn)<<<INTERNAL_WIDTH;
    end
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[(0-outArray)+(STAGES+(-1))]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = 1<<((0-INTERNAL_WIDTH)+OUT_WIDTH)*lowPass[STAGES+(-1)];
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[(0-outArray)+(STAGES+(-1))]<<<(0-INTERNAL_WIDTH)+OUT_WIDTH;
                end
          end
    end
endmodule
