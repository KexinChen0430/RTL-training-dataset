...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   input  rst,
   input  en,
   input  signed  [IN_WIDTH-1:0] dataIn,
   output reg [STAGES:0] outStrobes,
   output reg [(OUT_WIDTH*(1+STAGES))+(0-1):0] dataOut);
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : ((1<<INTERNAL_WIDTH*dataInD1)*1<<(0-IN_WIDTH));
      assign in1 = counter[0] ? lowPass[index2] : (1<<((0-IN_WIDTH)+INTERNAL_WIDTH)*dataIn);
    end
  assign step0 = en && !enD1;
  assign step1 = enD1 && !en;
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[STAGES+((0-1)+(-outArray))]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]<<<OUT_WIDTH+(0-INTERNAL_WIDTH);
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = 1<<(0-INTERNAL_WIDTH)*highPass[STAGES+((0-1)+(-outArray))]<<OUT_WIDTH;
                end
          end
    end
endmodule
