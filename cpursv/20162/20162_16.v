...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [(-1)+((STAGES+1)*OUT_WIDTH):0] dataOut);
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : ((2**(0-IN_WIDTH))*((2**INTERNAL_WIDTH)*dataInD1));
      assign in1 = counter[0] ? lowPass[index2] : dataIn<<INTERNAL_WIDTH<<(0-IN_WIDTH);
    end
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[STAGES+((-1)+(0-outArray))]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
