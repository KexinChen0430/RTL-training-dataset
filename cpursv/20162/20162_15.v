...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [(-1)+((1+STAGES)*OUT_WIDTH):0] dataOut);
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      ...
      assign in1 = counter[0] ? lowPass[index2] : (dataIn*1<<INTERNAL_WIDTH<<<(-IN_WIDTH));
    end
  assign step0 = !enD1 && en;
  assign step1 = !en && enD1;
  assign calcOut = step1 ? (in1+(0-in0)) : (in1+in0);
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES+(-1)]>>>INTERNAL_WIDTH+(0-OUT_WIDTH);
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[OUT_WIDTH*(1+outArray) +: OUT_WIDTH] = highPass[(STAGES+(-1))-outArray]>>>INTERNAL_WIDTH+(0-OUT_WIDTH);
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[OUT_WIDTH*(1+outArray) +: OUT_WIDTH] = highPass[(STAGES+(-1))-outArray]<<<OUT_WIDTH<<<-INTERNAL_WIDTH;
                end
          end
    end
endmodule
