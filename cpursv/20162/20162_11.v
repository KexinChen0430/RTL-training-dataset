...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   input  rst,
   input  en,
   input  signed  [(-1)+IN_WIDTH:0] dataIn,
  ...
  reg  signed  [(-1)+INTERNAL_WIDTH:0] lowPass[STAGES+(-1):0];

  reg  signed  [(-1)+INTERNAL_WIDTH:0] prevArray[STAGES+(-1):0];

  reg  signed  [(-1)+INTERNAL_WIDTH:0] highPass[STAGES+(-1):0];
  ...
  reg  signed  [(-1)+IN_WIDTH:0] dataInD1;
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (1<<INTERNAL_WIDTH*dataInD1<<(-IN_WIDTH));
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES+(0-outArray))+(-1)]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = 1<<(0-INTERNAL_WIDTH)*lowPass[STAGES+(-1)]<<OUT_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES+(0-outArray))+(-1)]<<(0-INTERNAL_WIDTH)+OUT_WIDTH;
                end
          end
    end
endmodule
