...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [((1+STAGES)*OUT_WIDTH)-1:0] dataOut);
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (dataInD1<<<INTERNAL_WIDTH*1<<<(-IN_WIDTH));
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[((-outArray)+STAGES)+(0-1)]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]*1<<OUT_WIDTH<<-INTERNAL_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[((-outArray)+STAGES)+(0-1)]<<<(-INTERNAL_WIDTH)+OUT_WIDTH;
                end
