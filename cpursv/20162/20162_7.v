...
module HaarFilter  #(parameter 
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[(0-outArray)+(STAGES-1)]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]<<OUT_WIDTH*1<<<(-INTERNAL_WIDTH);
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[(0-outArray)+(STAGES-1)]<<<OUT_WIDTH*1<<<(-INTERNAL_WIDTH);
                end
          end
    end
endmodule
