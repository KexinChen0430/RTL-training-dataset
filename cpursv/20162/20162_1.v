...
module HaarFilter  #(parameter 
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      ...
      assign in1 = counter[0] ? lowPass[index2] : (1<<<(-IN_WIDTH)*dataIn)<<INTERNAL_WIDTH;
    end
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES+(-1))+(-outArray)]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = (1<<OUT_WIDTH*lowPass[STAGES+(-1)])*1<<(0-INTERNAL_WIDTH);
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES+(-1))+(-outArray)]<<<OUT_WIDTH<<<0-INTERNAL_WIDTH;
                end
          end
    end
endmodule
