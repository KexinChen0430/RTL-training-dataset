...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [((STAGES+1)*OUT_WIDTH)+(-1):0] dataOut);
  ...
  reg  signed  [(-1)+INTERNAL_WIDTH:0] lowPass[(-1)+STAGES:0];

  reg  signed  [(-1)+INTERNAL_WIDTH:0] prevArray[(-1)+STAGES:0];

  reg  signed  [(-1)+INTERNAL_WIDTH:0] highPass[(-1)+STAGES:0];
  ...
  wire signed  [(-1)+INTERNAL_WIDTH:0] in0;

  wire signed  [(-1)+INTERNAL_WIDTH:0] in1;
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (dataInD1<<<(-IN_WIDTH)*1<<INTERNAL_WIDTH);
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[(-1)+STAGES]>>>INTERNAL_WIDTH+(-OUT_WIDTH);
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[(0-outArray)+((-1)+STAGES)]>>>INTERNAL_WIDTH+(-OUT_WIDTH);
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[(-1)+STAGES]<<OUT_WIDTH+(0-INTERNAL_WIDTH);
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[(0-outArray)+((-1)+STAGES)]<<0-INTERNAL_WIDTH<<<OUT_WIDTH;
                end
          end
    end
endmodule
