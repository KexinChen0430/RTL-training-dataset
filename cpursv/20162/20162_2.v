...
module HaarFilter  #(parameter 
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[OUT_WIDTH*(1+outArray) +: OUT_WIDTH] = highPass[(STAGES+(-1))+(0-outArray)]>>>INTERNAL_WIDTH+(-OUT_WIDTH);
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = (2**OUT_WIDTH)*lowPass[STAGES+(-1)]<<<0-INTERNAL_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[OUT_WIDTH*(1+outArray) +: OUT_WIDTH] = highPass[(STAGES+(-1))+(0-outArray)]<<<OUT_WIDTH-INTERNAL_WIDTH;
                end
          end
    end
endmodule
