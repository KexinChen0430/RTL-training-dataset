...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [(OUT_WIDTH*(STAGES+1))+(0-1):0] dataOut);

  
  initial  
  begin
    if ((STAGES > 8) || (STAGES < 2)) 
      begin
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (1<<(-IN_WIDTH)*dataInD1)<<INTERNAL_WIDTH;
      assign in1 = counter[0] ? lowPass[index2] : (dataIn*1<<(-IN_WIDTH))<<INTERNAL_WIDTH;
    end
                ...
                begin
                  if ((index == i) && step1) 
                    begin
                      ...
                      outStrobes[STAGES+(0-i)] <= 1'b1;
                    end
                  else 
                    begin
                      outStrobes[STAGES+(0-i)] <= 1'b0;
                    end
                  if (step0 && (index == i)) 
                    begin
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[((0-1)+(0-outArray))+STAGES]>>>(-OUT_WIDTH)+INTERNAL_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]*((2**(0-INTERNAL_WIDTH))*(2**OUT_WIDTH));
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[((0-1)+(0-outArray))+STAGES]<<0-INTERNAL_WIDTH<<<OUT_WIDTH;
                end
          end
    end
endmodule
