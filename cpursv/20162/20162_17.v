...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   input  rst,
   input  en,
   input  signed  [(0-1)+IN_WIDTH:0] dataIn,
   output reg [STAGES:0] outStrobes,
   output reg [(0-1)+((STAGES+1)*OUT_WIDTH):0] dataOut);

  
  initial  
  begin
    if ((STAGES < (1+1)) || (STAGES > 8)) 
      begin
  ...
  reg  signed  [INTERNAL_WIDTH+(0-1):0] lowPass[STAGES-1:0];

  reg  signed  [INTERNAL_WIDTH+(0-1):0] prevArray[STAGES-1:0];

  reg  signed  [INTERNAL_WIDTH+(0-1):0] highPass[STAGES-1:0];
  ...
  wire signed  [INTERNAL_WIDTH+(0-1):0] in0;

  wire signed  [INTERNAL_WIDTH+(0-1):0] in1;

  reg  signed  [(0-1)+IN_WIDTH:0] dataInD1;
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    begin
      assign in0 = counter[0] ? prevArray[index2] : dataInD1>>>((0-INTERNAL_WIDTH)+IN_WIDTH);
      assign in1 = counter[0] ? lowPass[index2] : dataIn>>>((0-INTERNAL_WIDTH)+IN_WIDTH);
    end
  else 
    begin
      assign in0 = counter[0] ? prevArray[index2] : (1<<INTERNAL_WIDTH<<<(-IN_WIDTH)*dataInD1);
      assign in1 = counter[0] ? lowPass[index2] : (((1+1)**(-IN_WIDTH))*(dataIn*1<<INTERNAL_WIDTH));
    end
  assign step0 = !enD1 && en;
  assign step1 = !en && enD1;
  assign calcOut = step1 ? ((0-in0)+in1) : (in0+in1);
                ...
                begin
                  if ((index == i) && step1) 
                    begin
                      ...
                      outStrobes[(-i)+STAGES] <= 1'b1;
                    end
                  else 
                    begin
                      outStrobes[(-i)+STAGES] <= 1'b0;
                    end
                  if (step0 && (index == i)) 
                    begin
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(-outArray)+(STAGES-1)]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]*((1+1)**(0-INTERNAL_WIDTH))<<OUT_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = 1+outArray)
                begin
                  dataOut[(1+outArray)*OUT_WIDTH +: OUT_WIDTH] = highPass[(-outArray)+(STAGES-1)]<<0-INTERNAL_WIDTH<<OUT_WIDTH;
                end
          end
    end
endmodule
