...
module HaarFilter  #(parameter 
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (1<<<(-IN_WIDTH)<<<INTERNAL_WIDTH*dataInD1);
      assign in1 = counter[0] ? lowPass[index2] : (dataIn*1<<<(-IN_WIDTH)<<<INTERNAL_WIDTH);
    end
  assign step0 = !enD1 && en;
  assign step1 = enD1 && !en;
  assign calcOut = step1 ? ((-in0)+in1) : (in1+in0);
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[(STAGES-1)+(-outArray)]>>>(0-OUT_WIDTH)+INTERNAL_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = 1<<<OUT_WIDTH*(1<<<(0-INTERNAL_WIDTH)*lowPass[STAGES-1]);
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[OUT_WIDTH*(outArray+1) +: OUT_WIDTH] = highPass[(STAGES-1)+(-outArray)]<<OUT_WIDTH*1<<<(0-INTERNAL_WIDTH);
                end
          end
    end
endmodule
