...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [(OUT_WIDTH*(STAGES+1))-1:0] dataOut);
  ...
  reg  signed  [INTERNAL_WIDTH-1:0] lowPass[STAGES+(0-1):0];

  reg  signed  [INTERNAL_WIDTH-1:0] prevArray[STAGES+(0-1):0];

  reg  signed  [INTERNAL_WIDTH-1:0] highPass[STAGES+(0-1):0];
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : dataInD1<<<INTERNAL_WIDTH<<<(0-IN_WIDTH);
      assign in1 = counter[0] ? lowPass[index2] : (1<<(0-IN_WIDTH)*(dataIn*1<<<INTERNAL_WIDTH));
    end
  assign step0 = !enD1 && en;
  assign step1 = enD1 && !en;
  assign calcOut = step1 ? ((0-in0)+in1) : (in1+in0);
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES+(0-1))-outArray]>>>INTERNAL_WIDTH+(-OUT_WIDTH);
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = ((1+1)**(OUT_WIDTH+(-INTERNAL_WIDTH)))*lowPass[STAGES+(0-1)];
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[(STAGES+(0-1))-outArray]<<OUT_WIDTH+(-INTERNAL_WIDTH);
                end
          end
    end
endmodule
