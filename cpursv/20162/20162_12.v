...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [((STAGES+1)*OUT_WIDTH)+(0-1):0] dataOut);
  ...
  reg  signed  [(0-1)+INTERNAL_WIDTH:0] lowPass[STAGES+(0-1):0];

  reg  signed  [(0-1)+INTERNAL_WIDTH:0] prevArray[STAGES+(0-1):0];

  reg  signed  [(0-1)+INTERNAL_WIDTH:0] highPass[STAGES+(0-1):0];
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (dataInD1<<<(0-IN_WIDTH)*1<<<INTERNAL_WIDTH);
      assign in1 = counter[0] ? lowPass[index2] : (dataIn*1<<<INTERNAL_WIDTH)<<(0-IN_WIDTH);
    end
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES+(0-1)]>>>INTERNAL_WIDTH-OUT_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[(-outArray)+(STAGES+(0-1))]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES+(0-1)]*1<<<OUT_WIDTH<<0-INTERNAL_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[(-outArray)+(STAGES+(0-1))]*1<<<(0-INTERNAL_WIDTH)<<<OUT_WIDTH;
                end
          end
    end
endmodule
