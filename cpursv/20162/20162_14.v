...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [(0-1)+(OUT_WIDTH*(1+STAGES)):0] dataOut);
  ...
  reg  signed  [INTERNAL_WIDTH+(0-1):0] lowPass[STAGES+(0-1):0];

  reg  signed  [INTERNAL_WIDTH+(0-1):0] prevArray[STAGES+(0-1):0];

  reg  signed  [INTERNAL_WIDTH+(0-1):0] highPass[STAGES+(0-1):0];
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : ((1<<<(0-IN_WIDTH)*(1<<1**INTERNAL_WIDTH))*dataInD1);
      assign in1 = counter[0] ? lowPass[index2] : dataIn<<(0-IN_WIDTH)<<INTERNAL_WIDTH;
    end
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES+(0-1)]>>>(-OUT_WIDTH)+INTERNAL_WIDTH;
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[(0-1)+((0-outArray)+STAGES)]>>>(-OUT_WIDTH)+INTERNAL_WIDTH;
                end
          end
    end
  else 
