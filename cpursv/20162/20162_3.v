...
module HaarFilter  #(parameter 
  ...
  (input  clk,
   ...
   output reg [(-1)+(OUT_WIDTH*(1+STAGES)):0] dataOut);
  ...
  if (IN_WIDTH > INTERNAL_WIDTH) 
    ...
    begin
      assign in0 = counter[0] ? prevArray[index2] : (1<<<INTERNAL_WIDTH*dataInD1)<<<(0-IN_WIDTH);
      assign in1 = counter[0] ? lowPass[index2] : (1<<<INTERNAL_WIDTH*dataIn<<<(0-IN_WIDTH));
    end
                ...
                begin
                  if ((index == i) && step1) 
                    begin
                      ...
                      outStrobes[(0-i)+STAGES] <= 1'b1;
                    end
                  else 
                    begin
                      outStrobes[(0-i)+STAGES] <= 1'b0;
                    end
                  if ((index == i) && step0) 
                    begin
  ...
  if (INTERNAL_WIDTH > OUT_WIDTH) 
      ...
      always @(*)
          begin
            ...
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[((-1)+(0-outArray))+STAGES]>>>INTERNAL_WIDTH-OUT_WIDTH;
                end
          end
    end
  else 
    begin
      ...
      always @(*)
          begin
            dataOut[0 +: OUT_WIDTH] = lowPass[STAGES-1]*(1<<<1**(OUT_WIDTH-INTERNAL_WIDTH));
            for (outArray = 0; outArray < STAGES; outArray = outArray+1)
                begin
                  dataOut[(outArray+1)*OUT_WIDTH +: OUT_WIDTH] = highPass[((-1)+(0-outArray))+STAGES]<<<OUT_WIDTH-INTERNAL_WIDTH;
                end
          end
    end
endmodule
