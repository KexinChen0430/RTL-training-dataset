...
module top(clk,reset,start,din,params,dout,ready,iir_start,iir_done);
      ...
      begin
        case (obf_state)
          ...
          compute_a: begin
                ...
                temp_ysum = yo2[31:3]+yo1[31:3];
              end

          compute_b: begin
                obf_next_state = compute_yk;
                temp_utmp = b2t+(b1t+b0t);
              end
          ...
          wait4_count: begin
                ...
                temp_wait_counter = (-1)+wait_counter;
              end

          latch_dout: if (count0) obf_next_state = idle;
  ...
  always @(posedge clk)
      begin
        ...
        else if (iir_start) finite_counter <= (-1)+finite_counter;
  ...
  assign iir_done = ~del_count0 && count0;
endmodule
