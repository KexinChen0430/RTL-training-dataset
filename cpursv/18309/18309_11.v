          ...
          begin
            ...
            if (cnt_clk == {{1'b0},{CLK_DIV+(-1){1'b1}}}) 
              begin
                ...
                if (cnt_sclk[(-1)+5:0] == {5{1'b0}}) 
                  begin
                    sync <= 1'b0;
                  end
                  
                cnt_sclk <= 1+cnt_sclk;
                if ((sync == (1'b0 | cnt_sclk[(-1)+5:0])) == {5{1'b0}}) 
                  begin
                    if (cnt_sclk[(-1)+5:1<<1] == 3'b000) 
                      begin
                        sdi <= cmd[3+(-cnt_sclk[(-1)+1<<1:0])];
                      end
                    else if (cnt_sclk[(-1)+5:1<<1] == 3'b001) 
                      begin
                        sdi <= cnt_sclk[7+(-1):5] == ~cnt_sclk[(-1)+1<<1:0];
                      end
                    else if (cnt_sclk[(-1)+5:0] == 5'b11000) 
                      begin
                        sync <= 1'b1;
                      ...
                      begin
                        sdi <= data_reg[((16*cnt_sclk[7+(-1):5])+23)-cnt_sclk[(-1)+5:0]];
                      end
                  end
