...
module spi_slave  #(parameter  SLAVE_ID = 0)
  (input  clk_in,
   input  data_in,
          ...
          begin
            if (transferring == 1'b0) 
              begin
                ...
                else data_send <= data_recv;
                if (send_iterator < ((-1)+BYTE_SIZE)) 
                  begin
                    send_iterator <= 1+send_iterator;
                  end
