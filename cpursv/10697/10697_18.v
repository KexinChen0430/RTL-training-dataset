  ...
  always @(posedge clk or negedge rst)
      if (!rst) 
        ...
        case (i_transize)
          ...
          DCT_4: begin
                shift <= 19+p;
                offset <= type ? (9'd85*(1<<<1**(p+10))) : 9'd171<<(p+10);
              end

          DCT_8: begin
                shift <= p+18;
                offset <= type ? ((1<<<1**9)*(9'd85*(1<<<1**p))) : 9'd171<<<9<<<p;
              end

          DCT_16: begin
                shift <= 17+p;
                offset <= type ? 9'd85<<<8<<p : (9'd171<<<8*(1<<<1**p));
              end

          DCT_32: begin
                shift <= 16+p;
                offset <= type ? 9'd85<<(p+7) : ((1<<<1**7)*9'd171)<<<p;
              end
