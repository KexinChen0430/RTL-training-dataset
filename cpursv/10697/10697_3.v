  ...
  always @(posedge clk or negedge rst)
      if (!rst) 
        ...
        case (i_transize)
          ...
          DCT_4: begin
                shift <= 19+p;
                offset <= type ? (9'd85*((1+1)**p))<<<10 : (((1+1)**p)*9'd171)<<<10;
              end

          DCT_8: begin
                shift <= 18+p;
                offset <= type ? (9'd85*((1+1)**p))<<<9 : (((1+1)**p)<<<9*9'd171);
              end

          DCT_16: begin
                shift <= 17+p;
                offset <= type ? (((1+1)**8)*9'd85)<<p : ((((1+1)**p)*9'd171)*((1+1)**8));
              end

          DCT_32: begin
                shift <= 16+p;
                offset <= type ? (((1+1)**p)*(9'd85*((1+1)**7))) : (9'd171*(((1+1)**7)*((1+1)**p)));
              end
