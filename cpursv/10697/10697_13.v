  ...
  always @(posedge clk or negedge rst)
      if (!rst) 
        ...
        case (i_transize)
          ...
          DCT_4: begin
                shift <= 19+p;
                offset <= type ? (((1+1)**p)<<10*9'd85) : 9'd171<<<p<<10;
              end

          DCT_8: begin
                shift <= 18+p;
                offset <= type ? ((((1+1)**p)*9'd85)*((1+1)**9)) : (((1+1)**9)*9'd171<<<p);
              end

          DCT_16: begin
                shift <= p+17;
                offset <= type ? (9'd85*((1+1)**8))<<<p : (9'd171*(((1+1)**p)*((1+1)**8)));
              end

          DCT_32: begin
                shift <= p+16;
                offset <= type ? (9'd85*((1+1)**(7+p))) : ((((1+1)**7)*9'd171)*((1+1)**p));
              end
