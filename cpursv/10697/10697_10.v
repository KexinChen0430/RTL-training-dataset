  ...
  always @(posedge clk or negedge rst)
      if (!rst) 
        ...
        case (i_transize)
          ...
          DCT_4: begin
                shift <= p+19;
                offset <= type ? ((1<<<1**10)*((1<<<1**p)*9'd85)) : ((1<<<1**10)*9'd171<<p);
              end

          DCT_8: begin
                shift <= p+18;
                offset <= type ? ((1<<<1**p)<<9*9'd85) : ((1<<<1**p)*9'd171<<9);
              end

          DCT_16: begin
                shift <= p+17;
                offset <= type ? (((1<<<1**p)*9'd85)*(1<<<1**8)) : ((1<<<1**p)*(9'd171*(1<<<1**8)));
              end

          DCT_32: begin
                shift <= 16+p;
                offset <= type ? 9'd85<<<7<<<p : (9'd171*(1<<<1**7)<<<p);
              end
