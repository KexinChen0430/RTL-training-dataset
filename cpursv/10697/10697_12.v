  ...
  always @(posedge clk or negedge rst)
      if (!rst) 
        ...
        case (i_transize)
          ...
          DCT_4: begin
                shift <= p+19;
                offset <= type ? (9'd85*(1<<1**(10+p))) : ((1<<1**10)<<<p*9'd171);
              end

          DCT_8: begin
                shift <= p+18;
                offset <= type ? (9'd85*(1<<1**9)<<<p) : 9'd171<<<9<<p;
              end

          DCT_16: begin
                shift <= 17+p;
                offset <= type ? (((1<<1**8)*(1<<1**p))*9'd85) : ((1<<1**8)*9'd171)<<<p;
              end

          DCT_32: begin
                shift <= 16+p;
                offset <= type ? 9'd85<<<(p+7) : 9'd171<<<(p+7);
              end
