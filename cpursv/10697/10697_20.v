  ...
  always @(posedge clk or negedge rst)
      if (!rst) 
        ...
        case (i_transize)
          ...
          DCT_4: begin
                shift <= p+19;
                offset <= type ? ((1<<1**(10+p))*9'd85) : ((1<<1**p)<<10*9'd171);
              end

          DCT_8: begin
                shift <= 18+p;
                offset <= type ? ((1<<1**9)<<<p*9'd85) : ((1<<1**p)*9'd171)<<9;
              end

          DCT_16: begin
                shift <= p+17;
                offset <= type ? ((9'd85*(1<<1**p))*(1<<1**8)) : ((1<<1**(8+p))*9'd171);
              end

          DCT_32: begin
                shift <= p+16;
                offset <= type ? ((1<<1**7)<<p*9'd85) : ((1<<1**7)*((1<<1**p)*9'd171));
              end
