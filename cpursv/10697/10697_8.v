  ...
  always @(posedge clk or negedge rst)
      if (!rst) 
        ...
        case (i_transize)
          ...
          DCT_4: begin
                shift <= 19+p;
                offset <= type ? (((1<<1**p)*9'd85)*(1<<1**10)) : 9'd171<<10<<p;
              end

          DCT_8: begin
                shift <= p+18;
                offset <= type ? 9'd85<<<(p+9) : (9'd171<<<p*(1<<1**9));
              end

          DCT_16: begin
                shift <= 17+p;
                offset <= type ? ((1<<1**p)<<8*9'd85) : (9'd171<<<p*(1<<1**8));
              end

          DCT_32: begin
                shift <= p+16;
                offset <= type ? (9'd85*(1<<1**(p+7))) : 9'd171<<(p+7);
              end
