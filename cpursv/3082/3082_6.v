...
module vga_driver(clk,color,hsync,vsync,red,green,blue,x,y);
  ...
  always @(posedge clk)
      begin
        if (h_count < (((H_PULSE+H_BACK)+(-1))+(H_ACTIVE+H_FRONT))) 
          begin
            h_count <= h_count+1;
            if (bar_count > 91) 
              begin
          ...
          begin
            ...
            column_count <= 0;
            h_count <= 0;
            if (v_count < ((V_ACTIVE+V_PULSE)+(((-1)+V_FRONT)+V_BACK))) v_count <= 1+v_count;
            else v_count <= 0;
          end
      end
  assign hsync = (h_count > (H_ACTIVE+H_FRONT)) && 
                 (h_count < (H_PULSE+(H_ACTIVE+H_FRONT)));
  assign vsync = (v_count > (V_FRONT+V_ACTIVE)) && 
                 (v_count < ((V_FRONT+V_ACTIVE)+V_PULSE));
