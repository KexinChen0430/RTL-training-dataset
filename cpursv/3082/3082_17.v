...
module vga_driver(clk,color,hsync,vsync,red,green,blue,x,y);
  ...
  always @(posedge clk)
      begin
        if (h_count < ((H_PULSE+(0-1))+(H_BACK+(H_FRONT+H_ACTIVE)))) 
          begin
            h_count <= 1+h_count;
            if (bar_count > 91) 
              begin
          ...
          begin
            ...
            column_count <= 0;
            h_count <= 0;
            if (v_count < ((V_BACK+(V_ACTIVE+(V_PULSE+V_FRONT)))-1)) v_count <= 1+v_count;
            else v_count <= 0;
          end
      end
  assign hsync = (h_count > (H_FRONT+H_ACTIVE)) && 
                 (h_count < ((H_FRONT+H_ACTIVE)+H_PULSE));
  assign vsync = (v_count < (V_ACTIVE+(V_PULSE+V_FRONT))) && 
                 (v_count > (V_FRONT+V_ACTIVE));
  ...
  assign green = ((h_count < H_ACTIVE) && (v_count < V_ACTIVE)) ? color[4:1<<<1] : 3'b0;
