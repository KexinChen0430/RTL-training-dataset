...
module vga_driver(clk,color,hsync,vsync,red,green,blue,x,y);
  ...
  always @(posedge clk)
      begin
        if (h_count < (H_BACK+((-1)+((H_FRONT+H_ACTIVE)+H_PULSE)))) 
          begin
            h_count <= 1+h_count;
            if (bar_count > 91) 
              begin
          ...
          begin
            ...
            column_count <= 0;
            h_count <= 0;
            if (v_count < ((V_ACTIVE+(V_FRONT+V_PULSE))+((-1)+V_BACK))) v_count <= v_count+1;
            else v_count <= 0;
          end
      end
  assign hsync = (h_count > (H_FRONT+H_ACTIVE)) && 
                 (h_count < ((H_FRONT+H_ACTIVE)+H_PULSE));
  assign vsync = (v_count < (V_ACTIVE+(V_FRONT+V_PULSE))) && 
