...
module vga_driver(clk,color,hsync,vsync,red,green,blue,x,y);
  ...
  always @(posedge clk)
      begin
        if (h_count < ((H_ACTIVE+H_PULSE)+(((-1)+H_FRONT)+H_BACK))) 
          begin
            h_count <= h_count+1;
            if (bar_count > 91) 
              begin
          ...
          begin
            ...
            column_count <= 0;
            h_count <= 0;
            if (v_count < (((V_ACTIVE+V_FRONT)+(V_BACK+V_PULSE))-1)) v_count <= v_count+1;
            else v_count <= 0;
          end
      end
  assign hsync = (h_count < ((H_ACTIVE+H_FRONT)+H_PULSE)) && 
