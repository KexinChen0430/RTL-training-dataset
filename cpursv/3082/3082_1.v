...
module vga_driver(clk,color,hsync,vsync,red,green,blue,x,y);
  ...
  always @(posedge clk)
      begin
        if (h_count < ((H_BACK+H_FRONT)+(H_PULSE+((-1)+H_ACTIVE)))) 
          begin
            h_count <= h_count+1;
            if (bar_count > 91) 
              begin
          ...
          begin
            ...
            column_count <= 0;
            h_count <= 0;
            if (v_count < ((V_FRONT+(-1))+(V_ACTIVE+(V_BACK+V_PULSE)))) v_count <= 1+v_count;
            else v_count <= 0;
          end
      end
  assign hsync = (h_count > (H_ACTIVE+H_FRONT)) && 
                 (h_count < (H_FRONT+(H_ACTIVE+H_PULSE)));
  assign vsync = (v_count < ((V_PULSE+V_FRONT)+V_ACTIVE)) && 
