...
module vga_driver(clk,color,hsync,vsync,red,green,blue,x,y);
  ...
  always @(posedge clk)
      begin
        if (h_count < (H_BACK+((H_ACTIVE+(0-1))+(H_FRONT+H_PULSE)))) 
          begin
            h_count <= 1+h_count;
            if (bar_count > 91) 
              begin
          ...
          begin
            ...
            column_count <= 0;
            h_count <= 0;
            if (v_count < ((V_PULSE+(0-1))+((V_FRONT+V_BACK)+V_ACTIVE))) v_count <= v_count+1;
            else v_count <= 0;
          end
      end
  assign hsync = (h_count < ((H_FRONT+H_PULSE)+H_ACTIVE)) && 
                 (h_count > (H_ACTIVE+H_FRONT));
  assign vsync = (v_count < ((V_PULSE+V_ACTIVE)+V_FRONT)) && 
