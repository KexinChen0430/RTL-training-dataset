...
module vga_driver(clk,color,hsync,vsync,red,green,blue,x,y);
  ...
  always @(posedge clk)
      begin
        if (h_count < ((H_ACTIVE+(H_BACK+H_FRONT))+(H_PULSE+(0-1)))) 
          begin
            h_count <= 1+h_count;
            if (bar_count > 91) 
              begin
          ...
          begin
            ...
            column_count <= 0;
            h_count <= 0;
            if (v_count < ((V_ACTIVE+V_PULSE)+((V_BACK+V_FRONT)+(0-1)))) v_count <= v_count+1;
            else v_count <= 0;
          end
      end
  assign hsync = (h_count < (H_ACTIVE+(H_PULSE+H_FRONT))) && 
                 (h_count > (H_FRONT+H_ACTIVE));
