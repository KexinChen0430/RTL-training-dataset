...
module Video_System_Chroma_Resampler(clk,reset,stream_in_data,stream_in_startofpacket,
  ...
  always @(posedge clk)
      begin
        ...
        else if ((valid & (((stream_in_valid & ~valid) | transfer_data) | ~valid)) & transfer_data) saved_CrCb <= data[15:8];
  ...
  always @(posedge clk)
      begin
        if (reset) cur_is_Cr_or_Cb <= 1'b0;
          ...
          cur_is_Cr_or_Cb <= (~cur_is_Cr_or_Cb & 1'b1) | ((~1'b1 & (cur_is_Cr_or_Cb | 1'b1)) & cur_is_Cr_or_Cb);
          
      end
  assign stream_in_ready = ((stream_in_valid & ~valid) | transfer_data) & stream_in_valid;
  assign transfer_data = ~stream_out_valid | ((stream_out_valid & ~stream_out_valid) | stream_out_ready);
