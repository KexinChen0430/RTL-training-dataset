...
module ddr2_mem_model_ram_module(data,rdaddress,wraddress,wrclock,wren,q);
  ...
  always @(rdaddress)
      begin
        ...
        for (i = 0; i < ARRAY_DEPTH; i = 1+i)
            begin
              if ((rdaddress == mem_array[i][57-1:(0-24)+57]) && 
                  mem_array[i][0]) 
                begin
                  out = mem_array[i][(0-1)+((0-24)+57):(-32)+((0-24)+57)];
  ...
  always @(posedge wrclock)
      if (wren) 
        begin
          ...
          for (i = 0; i < ARRAY_DEPTH; i = 1+i)
              begin
                if ((wraddress == mem_array[i][57-1:(0-24)+57]) && 
                    !data_written) 
                  begin
                    mem_array[i][(0-1)+((0-24)+57):(-32)+((0-24)+57)] <= data;
                    mem_array[i][0] <= 1'b1;
                    data_written = 1'b1;
                  end
