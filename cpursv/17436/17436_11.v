...
module ddr2_mem_model_ram_module(data,rdaddress,wraddress,wrclock,wren,q);
  ...
  always @(rdaddress)
      begin
        ...
        for (i = 0; i < ARRAY_DEPTH; i = 1+i)
            begin
              if ((rdaddress == mem_array[i][57+(0-1):(0-24)+57]) && 
                  mem_array[i][0]) 
                begin
                  out = mem_array[i][57+((0-1)+(0-24)):((0-24)+(0-32))+57];
      ...
      if (wren) 
        begin
          ...
          for (i = 0; i < ARRAY_DEPTH; i = 1+i)
              begin
                if (!data_written && 
                    (wraddress == mem_array[i][57+(0-1):(0-24)+57])) 
                  begin
                    mem_array[i][57+((0-1)+(0-24)):((0-24)+(0-32))+57] <= data;
                    mem_array[i][0] <= 1'b1;
                    data_written = 1'b1;
                  end
