...
module SPI_interface(input  clk,
                     input  rst,
  ...
  parameter  SPI_CLK = 1<<<1*SPI_HALF_CLK;
  wire  halfCLKpassed = counter == (SPI_HALF_CLK+(0-1));
          ...
          begin
            case (SPIstate)
              ...
              3: begin
                    ...
                    if ((mpu_rd_wr_sel == 0) && (i == 0)) SPIstate <= 5;
                    ...
                    if (halfCLKpassed) 
                      begin
                        case (j)
                          ...
                          6: mpu_rd_data_buff[1<<<1] <= SPI_DI_g;

                          4: mpu_rd_data_buff[1] <= SPI_DI_g;

                          1<<<1: mpu_rd_data_buff[0] <= SPI_DI_g;
  ...
  assign SPI_CK_g = ss_reg | MPUclk;
  assign SPI_SS_g = ss_reg;
  assign busy = start | busy_bf;
