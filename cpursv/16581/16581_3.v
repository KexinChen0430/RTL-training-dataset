...
module zdos(input  wire fclk,
  ...
  wire 
       trdemu_on = romnram && (fdd_mask[vg_a] && vg_rdwr_fclk && dos);
  ...
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) in_trdemu <= 1'b0;
      else if (clr_nmi) in_trdemu <= 1'b0;
      else if (trdemu_on) in_trdemu <= 1'b1;
        
  
  always @(posedge fclk or negedge rst_n)
      if (!rst_n) trdemu_wr_disable <= 1'b0;
      else if (zpos && !m1_n) trdemu_wr_disable <= 1'b0;
      else if (trdemu_on) trdemu_wr_disable <= 1'b1;
