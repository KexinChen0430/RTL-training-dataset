...
module barrel_shifter(output [15:0] yout,
  ...
  generate
      for (m = 0; m < 15; m = m+1)
          begin : REVERSE_INPUT
            assign a[m] = left ? ain[14+(0-m)] : ain[m];
          end
  endgenerate
  ...
  generate
      for (c = 0; c < 16; c = c+1)
          begin : B3
            ...
            else assign b3[c] = b[3] ? rb3[(0-8)+c] : a[c];
          end
  endgenerate
  ...
  generate
      for (f = 0; f < 16; f = f+1)
          begin : B2
            ...
            else assign b2[f] = b[1<<<1] ? rb2[f+(0-12)] : b3[f];
          end
  endgenerate
  ...
  generate
      for (h = 0; h < 16; h = h+1)
          begin : B1
            if (h < 14) assign b1[h] = b[1] ? b2[1<<<1+h] : b2[h];
            else assign b1[h] = b[1] ? rb1[(0-14)+h] : b2[h];
          end
  endgenerate
