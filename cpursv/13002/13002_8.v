...
module barrel_shifter(output [15:0] yout,
  ...
  generate
      for (m = 0; m < 15; m = m+1)
          begin : REVERSE_INPUT
            assign a[m] = left ? ain[(0-m)+14] : ain[m];
          end
  endgenerate
  ...
  generate
      for (c = 0; c < 16; c = 1+c)
          begin : B3
            if (c < 8) assign b3[c] = b[3] ? a[c+8] : a[c];
            else assign b3[c] = b[3] ? rb3[c-8] : a[c];
          end
  endgenerate
  ...
  generate
      for (f = 0; f < 16; f = 1+f)
          begin : B2
            if (f < 12) assign b2[f] = b[2] ? b3[4+f] : b3[f];
            else assign b2[f] = b[2] ? rb2[f+(-12)] : b3[f];
          end
  endgenerate
  ...
  generate
      for (h = 0; h < 16; h = h+1)
          begin : B1
            if (h < 14) assign b1[h] = b[1] ? b2[h+2] : b2[h];
            else assign b1[h] = b[1] ? rb1[h-14] : b2[h];
          end
  endgenerate
  ...
  generate
      for (j = 0; j < 16; j = 1+j)
          begin : B0
            if (j < 15) assign y[j] = b[0] ? b1[1+j] : b1[j];
  ...
  generate
      for (n = 0; n < 16; n = n+1)
          begin : REVERSE_OUTPUT
            assign yout[n] = left ? y[15+(0-n)] : y[n];
          end
  endgenerate
