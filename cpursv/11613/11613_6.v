...
module SHDCollector  #(parameter  DNA_DATA_WIDTH = 128, NUM_PES = 4)
  (input  clk,
   input  rst,
   output reg [(0-1)+NUM_PES:0] pe_rd_en,
   input  [(8*NUM_PES)-1:0] pe_data,
   input  [(0-1)+NUM_PES:0] pe_valid,
   input  sender_ready,
  ...
  reg  [(0-1)+PE_BITS:0]  pe_iterator = 0;
  wire advance_pe_it;
      ...
      begin
        if (rst) 
          ...
          begin
            if (accept_pe_data) 
              begin
                dna_err_r <= pe_data[8*pe_iterator +: 8];
