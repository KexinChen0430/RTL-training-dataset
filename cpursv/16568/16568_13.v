...
module fifo(in_data,in_d_rdy,clk,out_data,out_d_rdy,rdy2rcv);
  ...
  parameter  BUFF_LENm = (0-1)+BUFF_LEN;
  parameter 
...
(BUFF_LENm[0:0] == 1'b1) ? 1 : 0;
  ...
  reg  [WIDTH+(0-1):0] fifo_buff[0:(0-1)+BUFF_LEN];
      ...
      begin
        if (in_d_rdy == 1'b1) 
          begin
            ...
            if ((fifo_buff_cnt != {BUFF_LEN_WIDTH_bits{1'b0}}) && 
                (rdy2rcv == 1'b1)) 
              begin
                fifo_buff_sptr <= 1'b1+fifo_buff_sptr;
