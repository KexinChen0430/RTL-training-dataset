...
module fifo(in_data,in_d_rdy,clk,out_data,out_d_rdy,rdy2rcv);
  ...
  parameter  BUFF_LENm = BUFF_LEN+(0-1);
  parameter 
       BUFF_LEN_WIDTH_bits = (BUFF_LENm[7:7] == 1'b1) ? 8 : 
...
(BUFF_LENm[0:0] == 1'b1) ? 1 : 0;
  input  wire [(0-1)+WIDTH:0] in_data;
  input  wire in_d_rdy,clk,rdy2rcv;
  output wire [(0-1)+WIDTH:0] out_data;
  output wire out_d_rdy;
  reg  [BUFF_LEN_WIDTH_bits+(0-1):0] fifo_buff_wptr;

  
  initial    fifo_buff_wptr <= {BUFF_LEN_WIDTH_bits{1'b0}};
  reg  [BUFF_LEN_WIDTH_bits+(0-1):0] fifo_buff_sptr;

  
  initial    fifo_buff_sptr <= {BUFF_LEN_WIDTH_bits{1'b0}};
  reg  [BUFF_LEN_WIDTH_bits+(0-1):0] fifo_buff_cnt;

  
  initial    fifo_buff_cnt <= {BUFF_LEN_WIDTH_bits{1'b0}};
  reg  [(0-1)+WIDTH:0] fifo_buff[0:BUFF_LEN+(0-1)];
  ...
  initial  
  begin
    for (i = 0; i < BUFF_LEN; i = 1+i)
        begin
          fifo_buff[i] <= {WIDTH{1'b0}};
        end
  end
  assign out_d_rdy = rdy2rcv && (fifo_buff_cnt > 0);
      ...
      begin
        if (in_d_rdy == 1'b1) 
          begin
            ...
            fifo_buff_wptr <= 1'b1+fifo_buff_wptr;
            if ((rdy2rcv == 1'b1) && 
                (fifo_buff_cnt != {BUFF_LEN_WIDTH_bits{1'b0}})) 
              begin
              ...
              begin
                fifo_buff_cnt <= 1'b1+fifo_buff_cnt;
              end
          end
        else 
          begin
