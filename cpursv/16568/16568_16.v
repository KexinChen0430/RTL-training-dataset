...
(BUFF_LENm[0:0] == 1'b1) ? 1 : 0;
  ...
  output wire [(-1)+WIDTH:0] out_data;
  output wire out_d_rdy;
  reg  [(-1)+BUFF_LEN_WIDTH_bits:0] fifo_buff_wptr;

  
  initial    fifo_buff_wptr <= {BUFF_LEN_WIDTH_bits{1'b0}};
  reg  [(-1)+BUFF_LEN_WIDTH_bits:0] fifo_buff_sptr;

  
  initial    fifo_buff_sptr <= {BUFF_LEN_WIDTH_bits{1'b0}};
  reg  [(-1)+BUFF_LEN_WIDTH_bits:0] fifo_buff_cnt;

  
  initial    fifo_buff_cnt <= {BUFF_LEN_WIDTH_bits{1'b0}};
  reg  [(-1)+WIDTH:0] fifo_buff[0:BUFF_LEN-1];
