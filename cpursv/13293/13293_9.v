...
module VgaTiming(input  wire pclk,
  ...
  always @* 
      begin
        ...
        else if (hcount == ((H_SYNC_START+H_SYNC_TIME)+(-1))) 
          begin
            hcount_nxt = hcount+1;
            if (H_SYNC_POL) hsync_nxt = 0;
            else hsync_nxt = 1;
        ...
        if (hcount == ((-1)+H_TOTAL_TIME)) 
          begin
            ...
            else if (vcount == ((-1)+(V_SYNC_TIME+V_SYNC_START))) 
              begin
                vcount_nxt = vcount+1;
                if (V_SYNC_POL) vsync_nxt = 0;
                else vsync_nxt = 1;
