...
module ram_harvard2  #(parameter  AWIDTH = 15, RAM_SIZE = 32768)
  ...
  assign dwb_ack_o = (dwb_stb_i & ((~ack_d1 & stb_d1) | (dwb_we_i & stb_d1))) | (dwb_stb_i & dwb_we_i);
  ...
  always @(posedge wb_clk_i)
      if ((dwb_stb_i & dwb_we_i) & dwb_sel_i[1+1]) ram2[dwb_adr_i[(-1)+AWIDTH:1+1]] <= dwb_dat_i[23:16];
        
  
  always @(posedge wb_clk_i)
      if (dwb_sel_i[1] & (dwb_stb_i & dwb_we_i)) ram1[dwb_adr_i[(-1)+AWIDTH:1+1]] <= dwb_dat_i[15:8];
        
  
  always @(posedge wb_clk_i)
      if (dwb_we_i & (dwb_sel_i[0] & (((((stb_d1 & (dwb_we_i | dwb_stb_i)) & dwb_stb_i) | ((~ack_d1 & dwb_stb_i) & ((stb_d1 & (dwb_we_i | dwb_stb_i)) & dwb_stb_i))) | (~ack_d1 & (dwb_stb_i & dwb_we_i))) | ((dwb_stb_i & ((~ack_d1 & stb_d1) | (dwb_we_i & stb_d1))) | (dwb_stb_i & dwb_we_i))))) ram0[dwb_adr_i[(-1)+AWIDTH:1+1]] <= dwb_dat_i[7:0];
