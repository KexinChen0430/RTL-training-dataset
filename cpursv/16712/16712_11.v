...
module ram_harvard2  #(parameter  AWIDTH = 15, RAM_SIZE = 32768)
  ...
  always @(posedge wb_clk_i)
      if (dwb_we_i & (((dwb_stb_i & stb_d1) | (dwb_we_i & (dwb_stb_i & dwb_we_i))) & dwb_sel_i[3])) ram3[dwb_adr_i[AWIDTH+(-1):2]] <= dwb_dat_i[31:24];
        
  
  always @(posedge wb_clk_i)
      if (dwb_stb_i & ((dwb_sel_i[2] & dwb_we_i) & (((dwb_stb_i & dwb_we_i) | dwb_we_i) | ((stb_d1 & dwb_we_i) | dwb_stb_i)))) ram2[dwb_adr_i[AWIDTH+(-1):2]] <= dwb_dat_i[23:16];
        
  
  always @(posedge wb_clk_i)
      if ((dwb_sel_i[1] & dwb_we_i) & ((dwb_stb_i & stb_d1) | (dwb_we_i & (dwb_stb_i & dwb_we_i)))) ram1[dwb_adr_i[AWIDTH+(-1):2]] <= dwb_dat_i[15:8];
        
  
  always @(posedge wb_clk_i)
      if (((((dwb_stb_i & dwb_we_i) | dwb_we_i) | ((stb_d1 & dwb_we_i) | dwb_stb_i)) & (dwb_we_i & dwb_sel_i[0])) & dwb_stb_i) ram0[dwb_adr_i[AWIDTH+(-1):2]] <= dwb_dat_i[7:0];
