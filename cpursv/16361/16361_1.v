...
module ac97_dma(input  sys_rst,
  ...
  always @(posedge sys_clk)
      begin
        if (load_read_addr) wbm_adr_o <= {dmar_addr,2'b00};
        else if (load_write_addr) wbm_adr_o <= {dmaw_addr,2'b00};
      ...
      begin
        if (load_downpcm) 
          begin
            ...
            down_pcmleft <= {{wbm_dat_i[31:16],wbm_dat_i[30:27]}} & {20{dmar_en}};
            down_pcmright <= {{wbm_dat_i[15:0],wbm_dat_i[14:11]}} & {20{dmar_en}};
          end
      ...
      begin
        ...
        case (state)
          ...
          IDLE: begin
                ...
                if (down_next_frame) 
                  begin
                    if (dmar_en) down_en = 1'b0;
                    else load_downpcm = 1'b1;
                  end
                  
                if (up_next_frame) 
                  begin
                    if (dmaw_en) up_en = 1'b0;
                      
                  end
                  
                if ((dmar_en & ~dmar_finished) & down_next_frame) 
                  begin
                    load_read_addr = 1'b1;
                    next_state = DMAR;
                  end
                else if ((up_next_frame & ~dmaw_finished) & dmaw_en) 
                  begin
