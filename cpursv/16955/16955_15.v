...
module rnd_vec_gen(clk,init,save,restore,next,out);
  ...
  reg  [OUT_SIZE+(0-1):0] rndbase_main[0:LFSR_LENGTH+(0-1)];
  reg  [OUT_SIZE+(0-1):0] rndbase_store[0:LFSR_LENGTH+(0-1)];
  ...
  task shift_lfsr;
    ...
    begin
      ...
      sum = rndbase_main[LFSR_LENGTH+(0-1)]+rndbase_main[LFSR_FEEDBACK+(0-1)];
      for (i = 1; i < LFSR_LENGTH; i = i+1)
          rndbase_main[i] <= rndbase_main[i+(0-1)];
