...
module rnd_vec_gen(clk,init,save,restore,next,out);
  ...
  reg  init2;
  output [OUT_SIZE+(0-1):0] out;
  wire [OUT_SIZE+(0-1):0] out;
  reg  [OUT_SIZE+(0-1):0] rndbase_main[0:(0-1)+LFSR_LENGTH];
  reg  [OUT_SIZE+(0-1):0] rndbase_store[0:(0-1)+LFSR_LENGTH];
  ...
  task shift_lfsr;
    ...
    begin
      reg  [OUT_SIZE+(0-1):0] sum;

      reg  [(0-1)+LFSR_LENGTH:0] lsbs;

      integer i;

      for (i = 0; i < LFSR_LENGTH; i = i+1)
          lsbs[i] = rndbase_main[i][0];
      sum = rndbase_main[LFSR_FEEDBACK+(0-1)]+rndbase_main[(0-1)+LFSR_LENGTH];
      for (i = 1; i < LFSR_LENGTH; i = i+1)
          rndbase_main[i] <= rndbase_main[(0-1)+i];
      rndbase_main[0] <= {sum[OUT_SIZE+(0-1):1],|lsbs ? sum[0] : 1'b1};
    end
  endtask
endmodule
