...
module jt12_div(input  rst,
  ...
  always @(posedge clk)
      begin
        ...
        pre_clk_en <= cen_int & cen;
        pre_clk_en_2 <= (div2 == 2'b00) && cen;
        pre_clk_en_ssg <= use_ssg ? (cen_ssg_int & cen) : 1'b0;
        pre_clk_en_666 <= cen_adpcm_int & cen;
        pre_clk_en_111 <= (cen & cen_adpcm3_int) & cen_adpcm_int;
        pre_clk_en_55 <= cen & ((cen_55_int & cen_adpcm3_int) & cen_adpcm_int);
      end
  ...
  always @(posedge clk)
      if (cen) 
        begin
          adpcm_cnt666 <= (adpcm_cnt666 == 5'd11) ? 5'd0 : (5'd1+adpcm_cnt666);
          if (adpcm_cnt666 == 5'd0) 
            begin
              adpcm_cnt111 <= (adpcm_cnt111 == 3'd5) ? 3'd0 : (3'd1+adpcm_cnt111);
              if (adpcm_cnt111 == 3'd0) 
                adpcm_cnt55 <= (adpcm_cnt55 == 3'd1) ? 3'd0 : (3'd1+adpcm_cnt55);
