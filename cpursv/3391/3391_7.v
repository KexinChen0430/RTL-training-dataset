...
module axis_srl_fifo_64  #(parameter 
  ...
  reg  [(KEEP_WIDTH+(DATA_WIDTH+1))+((0-1)+1):0] data_reg[DEPTH-1:0];
  reg  [$clog2(1+DEPTH)+(0-1):0]  ptr_reg = 0,ptr_next;
  reg   full_reg = 0,full_next;
  reg   empty_reg = 1,empty_next;

  assign {output_axis_tlast,output_axis_tuser,output_axis_tkeep,output_axis_tdata} = data_reg[ptr_reg+(0-1)];
  ...
  always @* 
      begin
        ...
        if (~full_reg & (input_axis_tvalid & output_axis_tready)) 
          begin
            shift = 1;
            inc = ptr_empty;
            empty_next = 0;
