...
module axis_srl_fifo_64  #(parameter 
  ...
  reg  [(KEEP_WIDTH+(-1))+(1<<1+DATA_WIDTH):0] data_reg[(-1)+DEPTH:0];
  reg  [(-1)+$clog2(1+DEPTH):0]  ptr_reg = 0,ptr_next;
  ...
  always @* 
      begin
        ...
        if (output_axis_tready & (input_axis_tvalid & ~full_reg)) 
          begin
            shift = 1;
            inc = ptr_empty;
            empty_next = 0;
          end
        else if (output_axis_tvalid & output_axis_tready) 
          begin
            dec = 1;
            full_next = 0;
