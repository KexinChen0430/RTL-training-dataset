...
module z80ctrl(input  [4:2] SDA_L,
  ...
  assign nSDMRD = nSDRD | nMREQ;
  assign nSDMWR = nSDWR | nMREQ;
  assign nIORD = nSDRD | nIORQ;
  assign nIOWR = nSDWR | nIORQ;
  assign nSDZ80R = (nIORD | SDA_L[2]) | SDA_L[3];
  assign nSDZ80CLR = (nIOWR | SDA_L[2]) | SDA_L[3];
  assign n2610RD = ~SDA_L[2] | (nIORD | SDA_L[3]);
  assign n2610WR = (SDA_L[3] | ~SDA_L[2]) | nIOWR;
  assign n2610CS = n2610RD & n2610WR;
  assign nSDRD0 = SDA_L[2] | (~SDA_L[3] | nIORD);
  assign nNMI_SET = (SDA_L[2] | ~SDA_L[3]) | nIOWR;
  assign nSDRD1 = (~SDA_L[3] | ~SDA_L[2]) | nIORD;
  assign nSDZ80W = ~SDA_L[2] | (~SDA_L[3] | nIOWR);
  assign nNMI_RESET = nRESET & nSDZ80R;
