...
module fx3_bus_controller(input  clk,
                          input  rst,
                          input  i_master_rdy,
  ...
  assign w_in_path_idle = !o_in_path_enable && !w_in_data_avail;
  assign w_in_cmd_path_idle = !o_in_path_enable;
  assign w_out_path_idle = !i_out_path_busy && 
                           (!i_out_path_finished && 
(!o_out_path_enable && !i_out_path_ready));
  assign w_in_data_avail = i_in_ch1_rdy || i_in_ch0_rdy;
  assign w_out_buf_avail = i_out_ch1_rdy || i_out_ch0_rdy;
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            if (i_in_ch0_rdy && !i_in_ch1_rdy) 
              begin
                r_in_buf_sel_next <= 0;
              end
            else if (!i_in_ch0_rdy && i_in_ch1_rdy) 
              begin
                r_in_buf_sel_next <= 1;
              end
              
            if (!i_out_ch1_rdy && i_out_ch0_rdy) 
              begin
                r_out_buf_sel_next <= 0;
              end
            else if (i_out_ch1_rdy && !i_out_ch0_rdy) 
              begin
                r_out_buf_sel_next <= 1;
              end
              
            if (!o_in_path_enable && (w_in_data_avail && i_master_rdy)) 
              begin
                o_socket_addr <= {1'b0,r_in_buf_sel_next};
                r_in_buf_sel <= r_in_buf_sel_next;
                o_in_path_enable <= 1;
              end
            else if (i_in_path_finished) 
              begin
                o_in_path_enable <= 0;
              end
            ...
            else if (w_in_path_idle && w_out_path_idle) 
              begin
                o_in_path_cmd_enable <= 0;
              end
              
            if (i_out_path_ready && w_in_path_idle) 
              begin
                o_out_path_enable <= 1;
                o_out_dma_buf_ready <= 0;
              end
            else if (i_out_path_busy) 
              begin
                if (w_out_buf_avail && !o_out_dma_buf_ready) 
                  begin
                    o_socket_addr <= {1'b1,r_out_buf_sel_next};
                    o_out_dma_buf_ready <= 1;
                  end
                  
                if (i_out_dma_buf_finished) 
                  begin
                    o_out_dma_buf_ready <= 0;
                  end
                  
              end
            else if (i_out_path_finished) 
              begin
                o_out_dma_buf_ready <= 0;
                o_out_path_enable <= 0;
              end
