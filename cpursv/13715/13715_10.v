...
module rrp_arbiter  #(parameter  WIDTH = 4)
  (input  wire RST,
   input  wire CLK,
   input  wire [WIDTH+(0-1):0] WRITE_REQ,
   input  wire [WIDTH+(0-1):0] HOLD_REQ,
   input  wire [(WIDTH*32)+(0-1):0] DATA_IN,
   output wire [WIDTH+(0-1):0] READ_GRANT,
   input  wire READY_OUT,
   output wire WRITE_OUT,
   output wire [31:0] DATA_OUT);

  integer m;
  reg  [WIDTH+(0-1):0] prev_select;
  reg  [WIDTH+(0-1):0] select;
  reg  hold;
      ...
      begin
        ...
        if (HOLD_REQ_OR && !hold) 
          ...
          begin
            ...
            while ((m < WIDTH) && !(select & WRITE_REQ))
              begin
                m = 1+m;
                select = select<<1;
                if (select == 0) select = 1;
