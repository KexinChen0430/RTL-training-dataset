...
module rrp_arbiter  #(parameter  WIDTH = 4)
  (input  wire RST,
   input  wire CLK,
   input  wire [(-1)+WIDTH:0] WRITE_REQ,
   input  wire [(-1)+WIDTH:0] HOLD_REQ,
   input  wire [(WIDTH*32)-1:0] DATA_IN,
   output wire [(-1)+WIDTH:0] READ_GRANT,
   input  wire READY_OUT,
   output wire WRITE_OUT,
   output wire [31:0] DATA_OUT);

  integer m;
  reg  [(-1)+WIDTH:0] prev_select;
  reg  [(-1)+WIDTH:0] select;
  reg  hold;
      ...
      for (i = 0; i < 32; i = i+1)
          begin : gen
            for (j = 0; j < WIDTH; j = j+1)
                begin : gen2
                  assign DATA_A[i][j] = DATA_IN[i+(j*32)];
                end
          end
  endgenerate
  ...
  assign READ_GRANT = {WIDTH{READY_OUT}} & (WRITE_REQ & select);
endmodule
