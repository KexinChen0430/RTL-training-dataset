...
module ID(instr,imm,p0Addr,p1Addr,regAddr,shamt,aluOp,branchOp,regRe0,
          regRe1,regWe,memRe,memWe,addz,branch,jal,jr,aluSrc0,aluSrc1,
          ovEn,zrEn,neEn);
  ...
  assign p1Addr = (lhb | sw) ? instr[11:8] : 
                  lw ? instr[7:4] : 
                  llb ? 4'h0 : instr[3:0];
  assign regAddr = jal ? 4'hf : instr[11:8];
  assign shamt = instr[3:0];
  assign {regRe0,regRe1} = {1'b1,1'b1};
  assign regWe = !sw & ((!jr & !branch) & !addz);
  assign memRe = lw;
  assign memWe = sw;
  assign ovEn = (addz | sub) | add;
  assign zrEn = !instr[15];
  assign neEn = ovEn;
  assign aluSrc0 = lhb | llb;
  assign aluSrc1 = sw | lw;
