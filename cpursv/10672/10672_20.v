...
module testbench(input  clk_i,
  ...
  reg  [DATA_WIDTH+(-1):0] test_values[(-1)+DATA_COUNT:0];
  ...
  reg  [(-1)+AUB:0] memory[MEMORY_SIZE-1:0];
                        ...
                        if (wb_we_i == 1) 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = memory_index+1)
                                begin
                                  memory[memory_index+((0-BASE_ADDRESS)+wb_adr_i)] <= wb_dat_i[memory_index*AUB +: AUB];
                                end
                          end
                        else 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = memory_index+1)
                                begin
                                  wb_dat_o[memory_index*AUB +: AUB] <= memory[memory_index+((0-BASE_ADDRESS)+wb_adr_i)];
                                end
                          end
                      end
                    else 
