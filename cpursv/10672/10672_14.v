...
module testbench(input  clk_i,
  ...
  reg  [DATA_WIDTH+(0-1):0] test_values[DATA_COUNT+(0-1):0];
          ...
          begin
            if (master_state == S_MASTER_INIT_WRITE) 
              begin
                ...
                mem_address_o <= BASE_ADDRESS+(value_iterator*AU_IN_DATA);
              end
  ...
  reg  [AUB+(0-1):0] memory[MEMORY_SIZE-1:0];
            ...
            if (slave_state == S_WAIT) 
              begin
                if ((wb_cyc_i == 1) && (wb_stb_i == 1)) 
                  begin
                    if ((wb_adr_i < (BASE_ADDRESS+MEMORY_SIZE)) && 
                        (wb_adr_i >= BASE_ADDRESS)) 
                      begin
                        wb_ack_o <= 1;
                        if (wb_we_i == 1) 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = memory_index+1)
                                begin
                                  memory[wb_adr_i+((-BASE_ADDRESS)+memory_index)] <= wb_dat_i[AUB*memory_index +: AUB];
                                end
                          end
                        else 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = memory_index+1)
                                begin
                                  wb_dat_o[AUB*memory_index +: AUB] <= memory[wb_adr_i+((-BASE_ADDRESS)+memory_index)];
                                end
                          end
                      end
                    else 
