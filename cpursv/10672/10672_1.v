...
module testbench(input  clk_i,
  ...
  reg  [(-1)+DATA_WIDTH:0] test_values[DATA_COUNT-1:0];
                ...
                if ((wb_stb_i == 1) && (wb_cyc_i == 1)) 
                  begin
                    if ((wb_adr_i >= BASE_ADDRESS) && 
                        (wb_adr_i < (BASE_ADDRESS+MEMORY_SIZE))) 
                      begin
                        wb_ack_o <= 1;
                        if (wb_we_i == 1) 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  memory[(wb_adr_i+(0-BASE_ADDRESS))+memory_index] <= wb_dat_i[AUB*memory_index +: AUB];
                                end
                          end
                        else 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  wb_dat_o[AUB*memory_index +: AUB] <= memory[(wb_adr_i+(0-BASE_ADDRESS))+memory_index];
                                end
                          end
                      end
                    else 
