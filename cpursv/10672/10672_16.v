...
module testbench(input  clk_i,
  ...
  reg  [(0-1)+DATA_WIDTH:0] test_values[DATA_COUNT-1:0];
                        ...
                        if (wb_we_i == 1) 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  memory[(memory_index+wb_adr_i)+(-BASE_ADDRESS)] <= wb_dat_i[memory_index*AUB +: AUB];
                                end
                          end
                        else 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  wb_dat_o[memory_index*AUB +: AUB] <= memory[(memory_index+wb_adr_i)+(-BASE_ADDRESS)];
                                end
                          end
                      end
                    else 
