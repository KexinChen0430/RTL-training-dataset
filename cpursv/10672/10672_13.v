...
module testbench(input  clk_i,
  ...
  reg  [DATA_WIDTH-1:0] test_values[DATA_COUNT+(0-1):0];
          ...
          begin
            if (master_state == S_MASTER_INIT_WRITE) 
              begin
                ...
                mem_address_o <= (AU_IN_DATA*value_iterator)+BASE_ADDRESS;
              end
  ...
  localparam  MEMORY_SIZE = 4*DATA_COUNT;

  reg  [AUB-1:0] memory[(0-1)+MEMORY_SIZE:0];
                  ...
                  begin
                    if ((wb_adr_i >= BASE_ADDRESS) && 
                      ...
                      begin
                        ...
                        if (wb_we_i == 1) 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  memory[(-BASE_ADDRESS)+(memory_index+wb_adr_i)] <= wb_dat_i[memory_index*AUB +: AUB];
                                end
                          end
                        else 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  wb_dat_o[memory_index*AUB +: AUB] <= memory[(-BASE_ADDRESS)+(memory_index+wb_adr_i)];
                                end
                          end
                      end
                    else 
