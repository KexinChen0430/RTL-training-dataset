...
module testbench(input  clk_i,
  ...
  reg  [DATA_WIDTH-1:0] test_values[(0-1)+DATA_COUNT:0];
          ...
          begin
            if (master_state == S_MASTER_INIT_WRITE) 
              begin
                ...
                mem_address_o <= BASE_ADDRESS+(AU_IN_DATA*value_iterator);
              end
                        ...
                        if (wb_we_i == 1) 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  memory[((0-BASE_ADDRESS)+memory_index)+wb_adr_i] <= wb_dat_i[AUB*memory_index +: AUB];
                                end
                          end
                        else 
                          begin
                            for (memory_index = 0; memory_index < AU_IN_DATA; memory_index = 1+memory_index)
                                begin
                                  wb_dat_o[AUB*memory_index +: AUB] <= memory[((0-BASE_ADDRESS)+memory_index)+wb_adr_i];
                                end
                          end
                      end
                    else 
