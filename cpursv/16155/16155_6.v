...
module arbiter  #(parameter  NUM_PORTS = 6)
  (input  clk,
   input  rst,
   input  [NUM_PORTS+(0-1):0] request,
   output reg [NUM_PORTS+(0-1):0] grant,
   output reg active);

  localparam  WRAP_LENGTH = NUM_PORTS*1<<<1;
  integer yy;
  wire next;
  wire [NUM_PORTS+(0-1):0] order;
  reg  [NUM_PORTS+(0-1):0] token;
  wire [NUM_PORTS+(0-1):0] token_lookahead[NUM_PORTS+(0-1):0];
