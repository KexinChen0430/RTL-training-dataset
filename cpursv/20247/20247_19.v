...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = (!wait_start || restart[1]) && 
                            (!wait_start || !restart[0]) && 
                            (
(
(restart[1] && 
(!restart[0] && 
((!wait_start || restart[1]) && 
(!wait_start || !restart[0])))) || !ser_di) && 
((!restart[0] || wait_pause) && 
(restart[1] || wait_pause)));
  assign error = bit_half_end && (receiving_byte && last_half_bit) && 
                 !ser_filt_di;
  assign sample_bit = !bit_cntr[0] && shift_en && bit_half_end;
  assign reset_bit_duration = ser_rst || bit_half_end || (reset_wait_pause || start);
  assign wstart = !ser_filt_di && ser_filt_di_d && wait_start;
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],(ser_rst || (receiving_byte && bit_half_end)) && 
(last_half_bit || ser_rst)};
        wait_pause <= !ser_rst && 
...
(bit_half_end && (receiving_byte || reset_wait_pause) && 
(last_half_bit || reset_wait_pause))));
        start <= wstart;
        ts_stb <= !wait_pause && wstart;
        bit_half_end <= (bit_dur_cntr[15:0] == 16'h1) && !reset_bit_duration;
        wait_start <= 
((!wstart || last_half_bit) && 
(!wstart || 
((wait_pause && last_half_bit) || receiving_byte)) && ((bit_half_end || !wstart) && !ser_rst) && wait_start) || 
                     (last_half_bit && 
((wait_pause && bit_half_end) || 
(receiving_byte && bit_half_end)) && 
((!wstart || last_half_bit) && 
(!wstart || 
((wait_pause && last_half_bit) || receiving_byte)) && ((bit_half_end || !wstart) && !ser_rst)));
        receiving_byte <= !ser_rst && 
                         (!(bit_half_end && last_half_bit) || start) && (receiving_byte || start);
