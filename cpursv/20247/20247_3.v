...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = (!restart[0] || !ser_di) && 
                            ((restart[1] || (wait_pause && !wait_start)) && 
((wait_pause && !wait_start) || !restart[0])) && (!ser_di || restart[1]);
...
(ser_rst || 
(bit_half_end && (last_half_bit || ser_rst))) && (ser_rst || receiving_byte)};
        wait_pause <= 
...
!(!ser_filt_di && wait_start)) || reset_wait_pause) && (reset_wait_pause || wait_pause)));
        start <= wstart;
...
(bit_half_end && 
(
((wait_pause || receiving_byte) && 
(last_half_bit && !ser_rst)) || (!ser_rst && (wait_start && !wstart)))) || (!ser_rst && (wait_start && !wstart));
        receiving_byte <= 
