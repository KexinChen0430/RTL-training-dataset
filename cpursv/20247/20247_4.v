...
(!wait_start && !ser_di))) || (restart[1] && !restart[0]);
  assign error = !ser_filt_di && (last_half_bit && receiving_byte) && 
                 bit_half_end;
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],((last_half_bit && bit_half_end) || ser_rst) && 
(receiving_byte || ser_rst)};
        wait_pause <= 
(!(wait_start && !ser_filt_di) || 
(last_half_bit && (bit_half_end && receiving_byte)) || reset_wait_pause) && !ser_rst && 
                     (
(receiving_byte && (reset_wait_pause || bit_half_end) && 
last_half_bit) || 
(reset_wait_pause || 
...
(!ser_rst && (receiving_byte || wait_pause))) || (!ser_rst && wait_start && !wstart);
        receiving_byte <= 
(receiving_byte && !(last_half_bit && bit_half_end) && 
!ser_rst) || (!ser_rst && start);
