...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = (!restart[0] || (wait_pause && !wait_start)) && 
                            ((wait_pause && !wait_start) || restart[1]) && 
                            (((!ser_di || !restart[0]) && restart[1]) || !ser_di);
  assign error = receiving_byte && bit_half_end && last_half_bit && 
                 !ser_filt_di;
  assign sample_bit = !bit_cntr[0] && shift_en && bit_half_end;
  assign reset_bit_duration = bit_half_end || ser_rst || (reset_wait_pause || start);
...
(((receiving_byte || reset_wait_pause) && !ser_rst) || 
(!(bit_half_end && last_half_bit) && 
(!(wait_start && !ser_filt_di) && wait_pause) && !ser_rst)) && reset_wait_pause) || 
                     (
(bit_half_end || 
(!(bit_half_end && last_half_bit) && 
...
(!(wait_start && !ser_filt_di) && wait_pause) && !ser_rst))));
        ...
        wait_start <= !ser_rst && 
                     (
(last_half_bit && 
((bit_half_end && wait_pause) || wait_start || 
(receiving_byte && bit_half_end))) || wait_start) && 
                     (
(bit_half_end && last_half_bit && 
...
(receiving_byte && bit_half_end))) || wait_start))));
        receiving_byte <= 
((start && !ser_rst) || 
(!(bit_half_end && last_half_bit) && receiving_byte)) && !ser_rst;
