...
!wait_start)) && !ser_di) || (restart[1] && !restart[0]);
  assign error = !ser_filt_di && (bit_half_end && last_half_bit) && 
                 receiving_byte;
  assign sample_bit = shift_en && (!bit_cntr[0] && bit_half_end);
  assign reset_bit_duration = start || (bit_half_end || reset_wait_pause || ser_rst);
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],(bit_half_end || ser_rst) && 
(ser_rst || last_half_bit) && (ser_rst || receiving_byte)};
        wait_pause <= 
((!ser_rst && bit_half_end) || 
((!ser_rst && reset_wait_pause) || 
(!ser_rst && 
(!(wait_start && !ser_filt_di) && wait_pause) && !(bit_half_end && last_half_bit)))) && 
...
(!wstart || (bit_half_end && last_half_bit)))) && 
                     ((receiving_byte && last_half_bit && bit_half_end) || 
wait_start || (bit_half_end && last_half_bit && wait_pause));
        receiving_byte <= ((start && !ser_rst) || receiving_byte) && 
                         ((start || !(bit_half_end && last_half_bit)) && 
!ser_rst);
        wait_just_pause <= wait_pause && !wait_start;
        if (reset_bit_duration) bit_dur_cntr[15:0] <= bitHalfPeriod[15:0];
        else bit_dur_cntr[15:0] <= (0-1)+bit_dur_cntr[15:0];
        if (reset_wait_pause || ser_rst) bit_cntr[4:0] <= 5'h13;
        else if (start) bit_cntr[4:0] <= 5'h12;
        else if (bit_half_end) bit_cntr[4:0] <= (0-1)+bit_cntr[4:0];
