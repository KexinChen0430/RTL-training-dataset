...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = (wait_pause || !restart[0]) && 
                            (wait_pause || restart[1]) && 
                            ((!restart[0] && (!ser_di || restart[1])) || !ser_di) && 
                            ((!wait_start || !restart[0]) && 
(restart[1] || !wait_start));
  assign error = receiving_byte && last_half_bit && 
...
(ser_rst || receiving_byte)) || ser_rst};
        wait_pause <= !ser_rst && 
...
(
(reset_wait_pause || 
((reset_wait_pause || receiving_byte) && last_half_bit)) && 
(reset_wait_pause || bit_half_end || 
!(!ser_filt_di && wait_start))));
        start <= wstart;
        ts_stb <= !wait_pause && wstart;
        bit_half_end <= (bit_dur_cntr[15:0] == 16'h1) && !reset_bit_duration;
        wait_start <= (bit_half_end || wait_start) && 
                     (bit_half_end || !wstart) && 
                     (
(wait_pause || 
(wait_start && 
((bit_half_end || wait_start) && 
(bit_half_end || !wstart) && !wstart)) || receiving_byte) && !ser_rst && 
(last_half_bit || 
(wait_start && 
((bit_half_end || wait_start) && 
(bit_half_end || !wstart) && !wstart))));
        receiving_byte <= (start || receiving_byte) && 
                         (!ser_rst && 
(!(last_half_bit && bit_half_end) || start));
