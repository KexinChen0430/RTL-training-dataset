...
(
((!restart[0] && restart[1]) || (!ser_di && wait_pause)) && !wait_start) || (!restart[0] && restart[1]);
  assign error = last_half_bit && receiving_byte && bit_half_end && 
                 !ser_filt_di;
  assign sample_bit = !bit_cntr[0] && (shift_en && bit_half_end);
  assign reset_bit_duration = ser_rst || (reset_wait_pause || start || bit_half_end);
...
(reset_wait_pause || !(wait_start && !ser_filt_di)))) || 
                     (bit_half_end && 
(
...
(last_half_bit && receiving_byte)) && !ser_rst));
        ...
        wait_start <= (wait_start || bit_half_end) && 
                     (
(!wstart || 
(receiving_byte || (last_half_bit && wait_pause))) && (!wstart || last_half_bit) && 
((bit_half_end || !wstart) && !ser_rst) && 
(
(wait_start || 
(receiving_byte || (last_half_bit && wait_pause))) && (wait_start || last_half_bit)));
        receiving_byte <= !ser_rst && (start || (receiving_byte && !ser_rst)) && 
