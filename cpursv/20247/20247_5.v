...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = (!wait_start && !ser_di && wait_pause) || 
                            (!restart[0] && restart[1]);
  assign error = bit_half_end && !ser_filt_di && 
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],(receiving_byte && last_half_bit && bit_half_end) || 
ser_rst};
        wait_pause <= 
(!(!ser_filt_di && wait_start) || reset_wait_pause || 
(receiving_byte && 
(last_half_bit && (bit_half_end || reset_wait_pause)))) && 
                     (
(
(!ser_rst && 
((receiving_byte && last_half_bit && bit_half_end) || 
reset_wait_pause)) || (!(last_half_bit && bit_half_end) && !ser_rst)) && 
(wait_pause || 
((receiving_byte && last_half_bit && bit_half_end) || 
reset_wait_pause)));
        start <= wstart;
        ...
        wait_start <= (!wstart && (wait_start && !ser_rst)) || 
                     (((last_half_bit && wait_pause) || receiving_byte) && 
(last_half_bit && (!ser_rst && bit_half_end)));
        receiving_byte <= (start && !ser_rst) || 
                         (!ser_rst && 
(
(!(last_half_bit && bit_half_end) && receiving_byte) || (start && !(last_half_bit && bit_half_end))));
