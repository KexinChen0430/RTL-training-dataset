...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = (restart[1] || !wait_start) && 
                            (!restart[0] || !wait_start) && 
                            (
(restart[1] && 
((restart[1] || !ser_di) && (!restart[0] || !ser_di) && 
!restart[0])) || (wait_pause && !ser_di));
  assign error = last_half_bit && bit_half_end && 
                 (!ser_filt_di && receiving_byte);
  assign sample_bit = bit_half_end && (shift_en && !bit_cntr[0]);
  assign reset_bit_duration = reset_wait_pause || ser_rst || bit_half_end || start;
...
(!(last_half_bit && bit_half_end) && 
(!(wait_start && !ser_filt_di) && wait_pause))) && 
                     ((reset_wait_pause || bit_half_end || wait_pause) && 
(reset_wait_pause || bit_half_end || 
(!(wait_start && !ser_filt_di) && 
!(last_half_bit && bit_half_end)))) && 
                     (
(reset_wait_pause || receiving_byte || 
(!(last_half_bit && bit_half_end) && 
(!(wait_start && !ser_filt_di) && wait_pause))) && !ser_rst);
        start <= wstart;
        ts_stb <= !wait_pause && wstart;
        bit_half_end <= !reset_bit_duration && (bit_dur_cntr[15:0] == 16'h1);
        wait_start <= 
((receiving_byte || wait_pause) && 
(!ser_rst && (last_half_bit && bit_half_end))) || 
                     (
((bit_half_end && !ser_rst && wait_pause) || 
(!ser_rst && receiving_byte && bit_half_end) || (wait_start && !ser_rst)) && (last_half_bit || (wait_start && !ser_rst)) && !wstart);
        receiving_byte <= !ser_rst && (receiving_byte || (start && !ser_rst)) && 
