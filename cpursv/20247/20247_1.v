...
(!ser_di || ((!ser_di || !restart[0]) && restart[1])) && 
                            (
(!restart[0] && 
(restart[1] || (wait_pause && !wait_start))) || (wait_pause && !wait_start));
  assign error = !ser_filt_di && 
                 (receiving_byte && (last_half_bit && bit_half_end));
  assign sample_bit = bit_half_end && (shift_en && !bit_cntr[0]);
  assign reset_bit_duration = bit_half_end || start || (ser_rst || reset_wait_pause);
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],(last_half_bit || ser_rst) && 
(ser_rst || 
((ser_rst || receiving_byte) && bit_half_end))};
        wait_pause <= 
((!ser_rst && bit_half_end) || 
(!ser_rst && 
(
(!(last_half_bit && bit_half_end) && 
(wait_pause && !ser_rst && !(wait_start && !ser_filt_di))) || reset_wait_pause))) && 
                     ((receiving_byte && last_half_bit) || 
(!ser_rst && 
(
(!(last_half_bit && bit_half_end) && 
(wait_pause && !ser_rst && !(wait_start && !ser_filt_di))) || reset_wait_pause)));
        ...
        wait_start <= (last_half_bit || wait_start) && 
                     ((bit_half_end && (receiving_byte || wait_pause)) || 
wait_start) && 
                     ((!wstart || bit_half_end) && 
((!wstart || last_half_bit) && 
(receiving_byte || wait_pause || !wstart)) && !ser_rst);
        receiving_byte <= (!(last_half_bit && bit_half_end) || start) && 
                         (!ser_rst && (receiving_byte || start));
