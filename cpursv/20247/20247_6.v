...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = ((!restart[0] && restart[1]) || wait_pause) && 
                            (
...
!wait_start) || (!restart[0] && restart[1]));
  ...
  assign reset_bit_duration = start || (reset_wait_pause || bit_half_end) || ser_rst;
...
(receiving_byte && last_half_bit && bit_half_end)};
        wait_pause <= (reset_wait_pause && !ser_rst) || 
                     (wait_pause && !ser_rst && 
...
(reset_wait_pause || receiving_byte)));
        ...
        wait_start <= (wait_start && !ser_rst && !wstart) || 
                     (bit_half_end && !ser_rst && (wait_pause && last_half_bit)) || 
                     (receiving_byte && last_half_bit && 
(bit_half_end && !ser_rst));
        receiving_byte <= 
(!ser_rst && 
(receiving_byte && !(bit_half_end && last_half_bit))) || (!ser_rst && start);
