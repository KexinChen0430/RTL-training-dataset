...
(!restart[0] && 
(((!ser_di || !restart[0]) && restart[1]) || !ser_di) && restart[1]) || (wait_pause && (!ser_di && !wait_start));
  assign error = !ser_filt_di && receiving_byte && 
...
(!ser_rst && wait_pause && !(last_half_bit && bit_half_end))) || 
                     (!ser_rst && bit_half_end && 
(reset_wait_pause || (last_half_bit && receiving_byte))) || (!ser_rst && reset_wait_pause);
        start <= wstart;
        ts_stb <= wstart && !wait_pause;
        bit_half_end <= (bit_dur_cntr[15:0] == 16'h1) && !reset_bit_duration;
        wait_start <= 
(wait_pause || (wait_start && !wstart) || receiving_byte) && 
                     ((!ser_rst && bit_half_end) || 
(!wstart && (wait_start && !ser_rst))) && ((wait_start && !wstart) || last_half_bit);
        receiving_byte <= (start || !(last_half_bit && bit_half_end)) && 
                         ((receiving_byte || start) && !ser_rst);
