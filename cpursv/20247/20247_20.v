...
(
((!wait_start || (!restart[0] && restart[1])) && 
wait_pause && !ser_di) || (!restart[0] && restart[1] && !ser_di) || restart[1]) && 
                            (
((!wait_start || (!restart[0] && restart[1])) && 
((wait_pause || !restart[0]) && 
(restart[1] || wait_pause)) && !restart[0]) || 
(
((!wait_start || (!restart[0] && restart[1])) && 
wait_pause && !ser_di) || (!restart[0] && restart[1] && !ser_di)));
  assign error = bit_half_end && last_half_bit && !ser_filt_di && 
                 receiving_byte;
  assign sample_bit = bit_half_end && shift_en && !bit_cntr[0];
  assign reset_bit_duration = bit_half_end || ser_rst || (start || reset_wait_pause);
  assign wstart = wait_start && (!ser_filt_di && ser_filt_di_d);
...
(bit_half_end && receiving_byte && last_half_bit)};
        wait_pause <= 
((bit_half_end || reset_wait_pause) && 
(
(receiving_byte && (last_half_bit || reset_wait_pause)) || 
(
((!(bit_half_end && last_half_bit) && wait_pause) || 
reset_wait_pause) && 
(reset_wait_pause || !(wait_start && !ser_filt_di)))) && !ser_rst) || 
                     (wait_pause && !ser_rst && 
...
(
(!wstart || 
(receiving_byte || (wait_pause && last_half_bit))) && (last_half_bit || !wstart) && bit_half_end) || !wstart) && !ser_rst && 
                     (
(
((wait_pause && bit_half_end) || 
(bit_half_end && receiving_byte)) && (!ser_rst && last_half_bit)) || wait_start);
        receiving_byte <= 
(
(!ser_rst && 
(!(bit_half_end && last_half_bit) && receiving_byte)) || start) && !ser_rst;
