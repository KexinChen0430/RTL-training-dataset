...
(!restart[0] && 
(restart[1] && 
((!restart[0] || wait_pause) && 
(wait_pause || restart[1]))))) && 
                            ((!restart[0] || wait_pause) && 
(wait_pause || restart[1]));
  assign error = last_half_bit && 
                 (!ser_filt_di && bit_half_end && receiving_byte);
  assign sample_bit = !bit_cntr[0] && (bit_half_end && shift_en);
  assign reset_bit_duration = reset_wait_pause || ser_rst || start || bit_half_end;
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],
(last_half_bit && (receiving_byte && bit_half_end)) || (ser_rst && receiving_byte) || ser_rst};
        wait_pause <= 
(
(!ser_rst && bit_half_end && 
(
(receiving_byte && (reset_wait_pause || last_half_bit)) || reset_wait_pause)) || 
(
(!ser_rst && !(last_half_bit && bit_half_end) && wait_pause) || (!ser_rst && reset_wait_pause))) && 
                     (
(!ser_rst && !(wait_start && !ser_filt_di) && 
(
((receiving_byte || reset_wait_pause) && 
((reset_wait_pause || last_half_bit) && bit_half_end)) || 
((!(last_half_bit && bit_half_end) && wait_pause) || 
reset_wait_pause))) || 
((receiving_byte || reset_wait_pause) && 
((reset_wait_pause || last_half_bit) && 
(
((receiving_byte || reset_wait_pause) && 
((reset_wait_pause || last_half_bit) && bit_half_end)) || 
((!(last_half_bit && bit_half_end) && wait_pause) || 
reset_wait_pause))) && (reset_wait_pause || bit_half_end)));
        start <= wstart;
        ts_stb <= wstart && !wait_pause;
        bit_half_end <= !reset_bit_duration && (bit_dur_cntr[15:0] == 16'h1);
        wait_start <= (wait_start || bit_half_end) && 
                     (bit_half_end || !wstart) && 
                     (
((!ser_rst && !wstart && wait_start) || last_half_bit) && 
((!ser_rst && (receiving_byte || wait_pause)) || 
(!ser_rst && !wstart && wait_start)));
        receiving_byte <= !ser_rst && (start || receiving_byte) && 
                         (start || !(last_half_bit && bit_half_end));
