...
(!ser_di || ((!restart[0] || !ser_di) && restart[1])));
  assign error = last_half_bit && (receiving_byte && bit_half_end) && 
                 !ser_filt_di;
  assign sample_bit = shift_en && bit_half_end && !bit_cntr[0];
  assign reset_bit_duration = start || (ser_rst || bit_half_end) || reset_wait_pause;
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],(ser_rst || receiving_byte) && 
(ser_rst || 
(bit_half_end && (last_half_bit || ser_rst)))};
        wait_pause <= 
(
((!ser_rst && (receiving_byte && bit_half_end)) || 
(!(!ser_filt_di && wait_start) && 
!(last_half_bit && bit_half_end) && wait_pause && !ser_rst)) && 
(
(!(!ser_filt_di && wait_start) && 
!(last_half_bit && bit_half_end) && wait_pause && !ser_rst) || last_half_bit)) || (!ser_rst && reset_wait_pause);
        start <= wstart;
        ts_stb <= wstart && !wait_pause;
        bit_half_end <= !reset_bit_duration && (bit_dur_cntr[15:0] == 16'h1);
        wait_start <= 
((!wstart && (!ser_rst && wait_start)) || 
((receiving_byte || wait_pause) && last_half_bit)) && 
                     (
((!wstart && (!ser_rst && wait_start)) || bit_half_end) && !ser_rst);
        receiving_byte <= (!ser_rst && start) || 
                         (!ser_rst && 
