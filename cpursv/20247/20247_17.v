...
module rs232_rcv(xclk,bitHalfPeriod,ser_di,ser_rst,ts_stb,wait_just_pause,
  ...
  assign reset_wait_pause = (wait_pause && (!ser_di && !wait_start)) || 
                            (!restart[0] && 
((restart[1] || wait_pause) && 
(wait_pause || !restart[0]) && restart[1]));
  assign error = !ser_filt_di && 
                 (last_half_bit && receiving_byte && bit_half_end);
  assign sample_bit = !bit_cntr[0] && (shift_en && bit_half_end);
  assign reset_bit_duration = reset_wait_pause || bit_half_end || start || ser_rst;
  ...
  always @(posedge xclk)
      begin
        ...
        restart[1:0] <= {restart[0],(ser_rst || last_half_bit) && 
((bit_half_end && receiving_byte) || ser_rst)};
        wait_pause <= 
(
(last_half_bit && (reset_wait_pause || bit_half_end) && 
receiving_byte) || !(last_half_bit && bit_half_end) || reset_wait_pause) && 
                     (wait_pause || reset_wait_pause || 
(last_half_bit && (reset_wait_pause || bit_half_end) && 
...
(!ser_rst && 
(wait_start && 
((bit_half_end || wait_start) && 
(bit_half_end || !wstart)) && !wstart)) || 
                     (
((!ser_rst && (last_half_bit && wait_pause)) || 
(receiving_byte && (!ser_rst && last_half_bit))) && bit_half_end);
        receiving_byte <= (receiving_byte || start) && !ser_rst && 
...
(!ser_rst && start));
        ...
        else bit_dur_cntr[15:0] <= (-1)+bit_dur_cntr[15:0];
