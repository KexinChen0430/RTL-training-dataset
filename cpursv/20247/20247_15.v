...
(
(!ser_di || ((!ser_di || !restart[0]) && restart[1])) && !restart[0] && restart[1]) || 
                            (
(!ser_di && 
(
(!ser_di || ((!ser_di || !restart[0]) && restart[1])) && !restart[0] && restart[1])) || 
(!wait_start && 
((!ser_di || restart[1]) && 
(restart[1] || (!wait_start && wait_pause)) && (!ser_di && wait_pause))));
  assign error = last_half_bit && receiving_byte && 
                 (!ser_filt_di && bit_half_end);
...
(((last_half_bit && receiving_byte) || ser_rst) && 
bit_half_end)};
        wait_pause <= !ser_rst && 
                     (
(((bit_half_end || reset_wait_pause) && !ser_rst) || 
(!(bit_half_end && last_half_bit) && 
(wait_pause && !(wait_start && !ser_filt_di)))) && 
((last_half_bit && receiving_byte) || 
(
((wait_pause && !(bit_half_end && last_half_bit)) || 
reset_wait_pause) && 
(!(wait_start && !ser_filt_di) || reset_wait_pause))));
        start <= wstart;
...
(bit_half_end && !ser_rst))) || 
                     (((last_half_bit && receiving_byte) || wait_pause) && 
last_half_bit && 
((!ser_rst && (!wstart && wait_start)) || 
(bit_half_end && !ser_rst)));
        receiving_byte <= (start || !(bit_half_end && last_half_bit)) && 
                         ((start || (receiving_byte && !ser_rst)) && !ser_rst);
