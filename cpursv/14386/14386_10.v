...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = (select == 2'b10) | (((select == 2'b00) | (select == 2'b10)) & ((daten & (select == 2'b00)) | ((select == 2'b10) & (select == 2'b00))));
  ...
  always @(posedge BCLK)
      if (!ILO_SIG) ilo_flag <= 1'b0;
        ...
        ilo_flag <= (ilo_keep | ((ilo_keep | sel_dp) & D_IORD)) | DCWACC[0];
  assign ilo_keep = (ilo_flag & ~DCWACC[1]) & ~D_IOWR;
  assign ILO = ((D_IOWR & ILO_SIG) | (ILO_SIG & (DCWACC[1] | DCWACC[0]))) | (((ilo_flag & ILO_SIG) | ((((((ilo_flag | DCWACC[0]) | (sel_dp & ((D_IORD | (D_IORD & DCWACC[0])) | DCWACC[0]))) & (((sel_dp & D_IORD) & ILO_SIG) | (DCWACC[0] & ILO_SIG))) | ((DCWACC[0] & ILO_SIG) | (ilo_flag & ILO_SIG))) | (ILO_SIG & (D_IORD & ((ilo_flag | DCWACC[0]) | (DCWACC[0] | sel_dp))))) & (sel_dp & D_IORD))) | ((sel_dp & D_IORD) & ILO_SIG));
endmodule
