...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = (select == 2'b10) | ((((((select == 2'b00) & (select == 2'b10)) | (daten & (select == 2'b00))) | (select == 2'b00)) | (select == 2'b10)) & (((select == 2'b00) & (select == 2'b10)) | (daten & (select == 2'b00))));
  ...
  always @(posedge BCLK)
      if (!ILO_SIG) ilo_flag <= 1'b0;
        ...
        ilo_flag <= DCWACC[0] | (((D_IORD & sel_dp) | ilo_keep) | (D_IORD & DCWACC[0]));
  assign ilo_keep = ~DCWACC[1] & (ilo_flag & ~D_IOWR);
  assign ILO = ((D_IORD & ((ilo_flag | sel_dp) | (D_IOWR | ((DCWACC[1] | (DCWACC[0] & sel_dp)) | DCWACC[0])))) | ((D_IOWR | DCWACC[0]) | (ilo_flag | DCWACC[1]))) & ILO_SIG;
endmodule
