...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = (select == 2'b10) | (((daten & (select == 2'b10)) | (daten & (select == 2'b00))) | ((select == 2'b00) & (select == 2'b10)));
  ...
  always @(posedge BCLK)
      ...
      else ilo_flag <= (sel_dp & D_IORD) | (DCWACC[0] | ilo_keep);
  assign ilo_keep = ~D_IOWR & (~DCWACC[1] & ilo_flag);
  assign ILO = (((D_IORD & ILO_SIG) & sel_dp) | ((DCWACC[1] | ilo_flag) & ILO_SIG)) | (ILO_SIG & (DCWACC[1] | (DCWACC[0] | D_IOWR)));
endmodule
