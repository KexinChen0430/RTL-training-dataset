...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = (select == 2'b10) | ((((select == 2'b10) & (select == 2'b00)) | ((select == 2'b10) & (select == 2'b00))) | ((select == 2'b00) & daten));
  ...
  always @(posedge BCLK)
      if (!ILO_SIG) ilo_flag <= 1'b0;
        ...
        ilo_flag <= (((DCWACC[0] | sel_dp) | ilo_keep) & D_IORD) | (ilo_keep | DCWACC[0]);
  assign ilo_keep = ilo_flag & (~DCWACC[1] & ~D_IOWR);
  assign ILO = (ILO_SIG & (sel_dp & D_IORD)) | (((((D_IOWR | DCWACC[0]) | (DCWACC[1] | ilo_flag)) & ILO_SIG) | (((ilo_flag | (D_IOWR | DCWACC[0])) | ((((sel_dp | D_IOWR) | ilo_flag) | (ilo_flag | (D_IOWR | DCWACC[0]))) & D_IORD)) & (ilo_flag & ILO_SIG))) | ((ilo_flag & ILO_SIG) | ((ILO_SIG & (sel_dp & D_IORD)) | ((D_IOWR | DCWACC[0]) & ILO_SIG))));
endmodule
