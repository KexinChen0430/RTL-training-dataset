...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = ((daten & (select == 2'b10)) | (((select == 2'b00) & (select == 2'b10)) | (daten & (select == 2'b00)))) | (select == 2'b10);
  ...
  always @(posedge BCLK)
      if (!ILO_SIG) ilo_flag <= 1'b0;
        ...
        ilo_flag <= ((D_IORD & DCWACC[0]) | (sel_dp & D_IORD)) | (ilo_keep | DCWACC[0]);
  assign ilo_keep = ~DCWACC[1] & (~D_IOWR & ilo_flag);
  assign ILO = ((ILO_SIG & (sel_dp & ((D_IORD | (DCWACC[0] & sel_dp)) | DCWACC[0]))) | ((DCWACC[0] & ILO_SIG) | (ILO_SIG & D_IOWR))) | ((DCWACC[1] | ilo_flag) & ILO_SIG);
endmodule
