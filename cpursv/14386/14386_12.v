...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = (select == 2'b10) | (daten & ((select == 2'b00) | (daten & (select == 2'b10))));
  ...
  always @(posedge BCLK)
      if (!ILO_SIG) ilo_flag <= 1'b0;
        ...
        ilo_flag <= ((ilo_keep | sel_dp) & (((ilo_keep | ilo_keep) | D_IORD) | (ilo_keep | DCWACC[0]))) | DCWACC[0];
  assign ilo_keep = ~D_IOWR & (~DCWACC[1] & ilo_flag);
  assign ILO = ((((ILO_SIG & (DCWACC[0] | DCWACC[1])) | (((((DCWACC[0] | D_IORD) & ILO_SIG) & DCWACC[0]) | (sel_dp & (ILO_SIG & D_IORD))) & ilo_flag)) | (ilo_flag & ILO_SIG)) | (sel_dp & (ILO_SIG & D_IORD))) | (((sel_dp | DCWACC[0]) & (ILO_SIG & D_IORD)) | (ILO_SIG & ((D_IOWR | (sel_dp & DCWACC[0])) | DCWACC[0])));
endmodule
