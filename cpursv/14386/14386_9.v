...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = ((((select == 2'b10) & (select == 2'b00)) | (daten & (select == 2'b00))) | ((select == 2'b10) & (select == 2'b00))) | (select == 2'b10);
  ...
  always @(posedge BCLK)
      if (!ILO_SIG) ilo_flag <= 1'b0;
        ...
        ilo_flag <= ilo_keep | (DCWACC[0] | ((sel_dp & DCWACC[0]) | (sel_dp & D_IORD)));
  assign ilo_keep = ilo_flag & (~D_IOWR & ~DCWACC[1]);
  assign ILO = ((((((ilo_flag | (sel_dp & DCWACC[0])) | DCWACC[0]) | (sel_dp & D_IORD)) & ((ILO_SIG & DCWACC[0]) | (sel_dp & (D_IORD & ILO_SIG)))) | (((ILO_SIG & DCWACC[0]) | (ILO_SIG & (ilo_flag | sel_dp))) & (ilo_flag | D_IORD))) | (((ILO_SIG & DCWACC[0]) | (ILO_SIG & (ilo_flag | sel_dp))) & DCWACC[0])) | (((ILO_SIG & DCWACC[1]) | (ILO_SIG & DCWACC[0])) | ((D_IOWR | (sel_dp & D_IORD)) & ILO_SIG));
endmodule
