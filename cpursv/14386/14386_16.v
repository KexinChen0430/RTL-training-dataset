...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = ((((select == 2'b00) & (select == 2'b10)) | ((select == 2'b00) & daten)) & ((select == 2'b10) | (((select == 2'b00) | ((select == 2'b00) & daten)) | ((select == 2'b10) & daten)))) | (select == 2'b10);
  ...
  always @(posedge BCLK)
      ...
      else ilo_flag <= ilo_keep | (DCWACC[0] | (sel_dp & D_IORD));
  assign ilo_keep = (ilo_flag & ~DCWACC[1]) & ~D_IOWR;
  assign ILO = ((ILO_SIG & (ilo_flag | sel_dp)) & (ilo_flag | D_IORD)) | (((DCWACC[0] | D_IOWR) | DCWACC[1]) & ILO_SIG);
endmodule
