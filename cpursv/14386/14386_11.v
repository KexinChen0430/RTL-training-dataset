...
module IO_SWITCH(BCLK,BRESET,I_IOA,D_IOA,I_IORD,D_IORD,D_IOWR,IO_READY,
  ...
  assign sel_dp = ((((select == 2'b00) & daten) & ((select == 2'b10) | (select == 2'b00))) | (daten & (select == 2'b10))) | (select == 2'b10);
  ...
  always @(posedge BCLK)
      if (!ILO_SIG) ilo_flag <= 1'b0;
        ...
        ilo_flag <= ilo_keep | (((ilo_keep | ilo_keep) | (DCWACC[0] | sel_dp)) & ((D_IORD | (DCWACC[0] & sel_dp)) | DCWACC[0]));
  assign ilo_keep = ~D_IOWR & (ilo_flag & ~DCWACC[1]);
  assign ILO = (((ILO_SIG & ilo_flag) | (D_IOWR & ILO_SIG)) | (ILO_SIG & ((sel_dp & D_IORD) | (ilo_flag | DCWACC[0])))) | (ILO_SIG & (DCWACC[1] | ilo_flag));
endmodule
