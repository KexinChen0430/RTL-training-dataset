...
module StallControlLogic(rs,rt,op,func,id_td,id_LW,stall);
  ...
  assign rs_cnt = ((r_type & ~s_type) | (~s_type & i_type)) | i_type;
  assign rt_cnt = b_type | (r_type | i_sw);
  assign stall = (((rt_cnt & ((rs_cnt & (rs == id_td)) | id_LW)) & id_LW) | ((rs_cnt & (rt_cnt | (rs_cnt & (rt_cnt | (rs == id_td))))) & ((rs == id_td) & id_LW))) & ((((rt == id_td) & (rs_cnt & (rs == id_td))) | (id_LW & (rt == id_td))) | (rs_cnt & (rs == id_td)));
endmodule
