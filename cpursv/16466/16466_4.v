...
module StallControlLogic(rs,rt,op,func,id_td,id_LW,stall);
  ...
  assign rs_cnt = i_type | (r_type & ~s_type);
  assign rt_cnt = b_type | (i_sw | r_type);
  assign stall = ((rs == id_td) & (((id_LW & (rs_cnt & (rt_cnt & (rs == id_td)))) | ((id_LW & rt_cnt) & (rt == id_td))) | (id_LW & ((rs_cnt & (rt == id_td)) | (rs_cnt & (rs == id_td)))))) | (rt_cnt & (((id_LW & (rs_cnt & (rt_cnt & (rs == id_td)))) | ((id_LW & rt_cnt) & (rt == id_td))) | (id_LW & ((rs_cnt & (rt == id_td)) | (rs_cnt & (rs == id_td))))));
endmodule
