...
module StallControlLogic(rs,rt,op,func,id_td,id_LW,stall);
  ...
  assign rs_cnt = i_type | ((i_type & ~s_type) | (~s_type & r_type));
  assign rt_cnt = (b_type | r_type) | i_sw;
  assign stall = ((rt_cnt | rt_cnt) | rs_cnt) & ((((id_LW & ((((rs == id_td) & ((rt_cnt & rs_cnt) | ((rt == id_td) & rt_cnt))) | (((rt_cnt & rs_cnt) | ((rt == id_td) & rt_cnt)) & (rt == id_td))) & (rs_cnt | id_LW))) | ((rt == id_td) & id_LW)) | ((rs_cnt & (rs == id_td)) & ((id_LW & ((rt_cnt | (rs_cnt & (rs == id_td))) & rs_cnt)) & (rs == id_td)))) & (rt_cnt | (rs == id_td)));
endmodule
