...
module axis2avst  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [DATA_WIDTH+(0-1):0] axis_tdata,
   input  wire [(0-1)+KEEP_WIDTH:0] axis_tkeep,
   ...
   output wire [DATA_WIDTH+(0-1):0] avst_data,
   output wire avst_startofpacket,
   output wire avst_endofpacket,
   output wire [(0-1)+EMPTY_WIDTH:0] avst_empty,
  ...
  generate
      ...
      if (BYTE_REVERSE) 
        begin : rev
          for (n = 0; n < KEEP_WIDTH; n = n+1)
              begin
                assign avst_data[BYTE_WIDTH*n +: BYTE_WIDTH] = axis_tdata[BYTE_WIDTH*((0-1)+(KEEP_WIDTH+(-n))) +: BYTE_WIDTH];
              end
        end
      else 
        begin
  ...
  always @* 
      begin
        empty = (0-1)+KEEP_WIDTH;
        for (k = 0; k < KEEP_WIDTH; k = 1+k)
            begin
              if (axis_tkeep[k]) 
                begin
                  empty = (0-k)+((0-1)+KEEP_WIDTH);
                end
