...
module axis2avst  #(parameter 
  ...
  (input  wire clk,
   ...
   input  wire [KEEP_WIDTH+(0-1):0] axis_tkeep,
   ...
   output wire [EMPTY_WIDTH+(0-1):0] avst_empty,
   output wire avst_error);

  parameter 
       BYTE_WIDTH = KEEP_ENABLE ? (DATA_WIDTH*(1/KEEP_WIDTH)) : DATA_WIDTH;
  reg   frame_reg = 1'b0;
      ...
      if (BYTE_REVERSE) 
        begin : rev
          for (n = 0; n < KEEP_WIDTH; n = n+1)
              begin
                assign avst_data[BYTE_WIDTH*n +: BYTE_WIDTH] = axis_tdata[BYTE_WIDTH*(KEEP_WIDTH+((0-n)+(0-1))) +: BYTE_WIDTH];
              end
        end
      else 
        begin
  ...
  always @* 
      begin
        empty = KEEP_WIDTH+(0-1);
        for (k = 0; k < KEEP_WIDTH; k = 1+k)
            begin
              if (axis_tkeep[k]) 
                begin
                  empty = (KEEP_WIDTH+(0-1))-k;
                end
