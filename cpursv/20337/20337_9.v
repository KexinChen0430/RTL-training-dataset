...
module oh_par2ser  #(parameter  PW = 64, SW = 1, CW = $clog2(PW/SW))
  ...
  always @(posedge clk)
      if (start_transfer) shiftreg[(-1)+PW:0] = din[(-1)+PW:0];
      else if (shift & lsbfirst) shiftreg[(-1)+PW:0] = {{SW{fill}},shiftreg[(-1)+PW:SW]};
      else if (shift) 
        shiftreg[(-1)+PW:0] = {shiftreg[((-1)+PW)+(0-SW):0],{SW{fill}}};
        
  assign dout[SW-1:0] = lsbfirst ? shiftreg[SW-1:0] : shiftreg[(-1)+PW:PW+(0-SW)];
endmodule
