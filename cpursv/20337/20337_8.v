...
module oh_par2ser  #(parameter  PW = 64, SW = 1, CW = $clog2((1/SW)*PW))
  ...
  always @(posedge clk or negedge nreset)
      ...
      else if (busy & shift) count[CW+(0-1):0] <= count[CW+(0-1):0]-1'b1;
  ...
  always @(posedge clk)
      if (start_transfer) shiftreg[PW+(0-1):0] = din[PW+(0-1):0];
        ...
        shiftreg[PW+(0-1):0] = {shiftreg[(PW-SW)+(0-1):0],{SW{fill}}};
        
  assign dout[SW+(0-1):0] = lsbfirst ? shiftreg[SW+(0-1):0] : shiftreg[PW+(0-1):PW-SW];
endmodule
