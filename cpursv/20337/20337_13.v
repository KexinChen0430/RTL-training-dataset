...
module oh_par2ser  #(parameter  PW = 64, SW = 1, CW = $clog2((1/SW)*PW))
  ...
  always @(posedge clk or negedge nreset)
      ...
      else if (shift & busy) count[CW-1:0] <= count[CW-1:0]+(0-1'b1);
  ...
  always @(posedge clk)
      ...
      else if (shift) shiftreg[PW-1:0] = {shiftreg[((-SW)+PW)-1:0],{SW{fill}}};
        
  assign dout[(-1)+SW:0] = lsbfirst ? shiftreg[(-1)+SW:0] : shiftreg[PW-1:(-SW)+PW];
endmodule
