...
module hazard_unit(input  clk,
  ...
  always @(posedge clk or posedge rst)
      begin
        ...
        else if ((id_opcode == LW) || (id_opcode == SW)) mem_wait_r = (id_opcode == LW) || (id_opcode == SW);
  ...
  always @* 
      begin
        coincidence = 0;
        if (
((id_rs == ex_dst_reg) | id_rt_is_source && (id_rt == ex_dst_reg)) && ex_reg_write) coincidence[0] = 1'b1;
        else if (
((id_rs == mem_dst_reg) | (id_rt == mem_dst_reg) && id_rt_is_source) && mem_reg_write) coincidence[1] = 1'b1;
  ...
  always @* 
      begin
        ...
        if (coincidence[0]) 
          begin
            if ((ex_opcode == LW) && (id_opcode == BEQ)) hazard_detected = 1;
          ...
          begin
            if ((id_opcode == BEQ) && (mem_opcode == LW)) hazard_detected = 1;
