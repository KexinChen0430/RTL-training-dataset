...
module uart_2  #(parameter  tam_data = 11, read_bit = 4)
  ...
  always @(posedge tx_clk)
      begin
        ...
        if (tx_busy && ((cnt_data > 0) && (cnt_data < tam_data))) 
          begin
            tx_channel <= tx_data[cnt_data];
            cnt_data <= 1+cnt_data;
          end
  ...
  always @(posedge rx_clk)
      begin
        if (reset) 
          ...
          begin
            if ((cnt_read < tam_data) && (cnt_rx < read_bit)) 
              begin
                rx_busy <= 1;
                cnt_rx <= 1+cnt_rx;
              end
            ...
            if ((cnt_rx > read_bit) && 
                ((cnt_rx <= ((0-1)+(read_bit+read_bit))) && rx_busy)) 
              begin
                if (rx_busy && (cnt_rx == ((0-1)+(read_bit+read_bit)))) 
                  begin
                    cnt_read <= 1+cnt_read;
                    cnt_rx <= 0;
                  end
