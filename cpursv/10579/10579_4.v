...
module cdc_pulse_sync(input  wire clk_in,
                      input  wire pulse_in,
                      input  wire clk_out,
  ...
  always @(posedge clk_in)
      begin
        if (aq_sync) in_sync_pulse <= 0;
        else if (in_pre_sync[0] && !in_pre_sync[1]) in_sync_pulse <= 1;
  ...
  assign pulse_out = out_sync[1] && !out_sync[2];
  reg  [1:0] aq_sync_ff;
