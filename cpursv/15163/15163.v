  ...
  generate
      for (b = 0; b < DATA_WIDTH; b = b+1)
          begin : bit_loop
            if (AFI_RATIO == 4) 
              begin
                assign error_compute[b] = ((((((((read_data_r[b] ^ reference_data[b]) & ~mask[0]) | ((read_data_r[b+DATA_WIDTH] ^ reference_data[b+DATA_WIDTH]) & ~mask[1])) | ((read_data_r[b+(2*DATA_WIDTH)] ^ reference_data[b+(2*DATA_WIDTH)]) & ~mask[2])) | ((read_data_r[b+(3*DATA_WIDTH)] ^ reference_data[b+(3*DATA_WIDTH)]) & ~mask[3])) | ((read_data_r[b+(4*DATA_WIDTH)] ^ reference_data[b+(4*DATA_WIDTH)]) & ~mask[4])) | ((read_data_r[b+(5*DATA_WIDTH)] ^ reference_data[b+(5*DATA_WIDTH)]) & ~mask[5])) | ((read_data_r[b+(6*DATA_WIDTH)] ^ reference_data[b+(6*DATA_WIDTH)]) & ~mask[6])) | ((read_data_r[b+(7*DATA_WIDTH)] ^ reference_data[b+(7*DATA_WIDTH)]) & ~mask[7]);
              end
            else if (AFI_RATIO == 2) 
              begin
                assign error_compute[b] = ((((read_data_r[b] ^ reference_data[b]) & ~mask[0]) | ((read_data_r[b+DATA_WIDTH] ^ reference_data[b+DATA_WIDTH]) & ~mask[1])) | ((read_data_r[b+(2*DATA_WIDTH)] ^ reference_data[b+(2*DATA_WIDTH)]) & ~mask[2])) | ((read_data_r[b+(3*DATA_WIDTH)] ^ reference_data[b+(3*DATA_WIDTH)]) & ~mask[3]);
              end
            else 
              begin
                assign error_compute[b] = ((read_data_r[b] ^ reference_data[b]) & ~mask[0]) | ((read_data_r[b+DATA_WIDTH] ^ reference_data[b+DATA_WIDTH]) & ~mask[1]);
              end
          end
  endgenerate
