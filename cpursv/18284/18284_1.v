...
module IDDR(Q1,Q2,C,CE,D,R,S);
  ...
  always @(posedge C)
      if (CE) Q1_pos <= #(HOLDHACK) D;
        
  
  always @(posedge C)
      if (CE) Q1_reg <= #(HOLDHACK) Q1_pos;
        
  
  always @(negedge C)
      if (CE) Q2_neg <= #(HOLDHACK) D;
        
  
  always @(posedge C)
      if (CE) Q2_pos <= #(HOLDHACK) Q2_neg;
        
  assign Q1 = (DDR_CLK_EDGE_REG == SAME_EDGE_PIPELINED) ? Q1_reg : 
              (DDR_CLK_EDGE_REG == SAME_EDGE) ? Q1_pos : 1'b0;
  assign Q2 = (DDR_CLK_EDGE_REG == SAME_EDGE_PIPELINED) ? Q2_pos : 
              (DDR_CLK_EDGE_REG == SAME_EDGE) ? Q2_pos : 1'b0;
endmodule
