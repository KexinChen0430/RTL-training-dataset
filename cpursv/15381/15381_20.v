...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  reg  [(0-1)+DSIZE:0] mem[0:(0-1)+FIFO_DEPTH];
  ...
  always @(posedge rd_clk)
      begin
        if (rst) empty <= #TCQ 1'b1;
                 ...
                 (
(((buf_filled == 1) && (buf_filled == 0)) || rd_strobe) && (buf_filled == 1))) empty <= #TCQ 1'b1;
        else empty <= #TCQ 1'b0;
      end
  reg  [ASIZE:0] wbin;
  ...
  assign {COutd,buf_avail} = 5'd16+(rd_capture_ptr-wr_ptr);
  
  always @(posedge wr_clk)
      begin
        if (rst) full <= #TCQ 1'b0;
        else if ((buf_avail == 0) || (wr_en && (buf_avail == 1))) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
