...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  (input  wr_clk,
   input  rst,
   input  wr_en,
   input  [(-1)+DSIZE:0] wr_data,
   input  rd_en,
   input  rd_clk,
   output [(-1)+DSIZE:0] rd_data,
   output reg full,
   output reg empty,
   output reg almost_full);

  reg  [(-1)+DSIZE:0] mem[0:(-1)+FIFO_DEPTH];
  ...
  wire [(-1)+ASIZE:0] wr_addr,rd_addr;
  wire COutb,COutd;
  reg  COuta,COutc;
  ...
  assign wr_addr = wr_ptr[(-1)+ASIZE:0];
  assign rd_data = mem[rd_addr];
  
  always @(posedge wr_clk)
      begin
        if (wr_en && !full) mem[wr_addr] <= #TCQ wr_data;
          
      end
  assign rd_addr = rd_ptr_cp[(-1)+ASIZE:0];
      ...
      begin
        if (rst) 
          ...
          begin
            if (rd_strobe) 
              begin
                {COuta,rd_ptr} <= #TCQ 1'b1+rd_ptr;
                rd_ptr_cp <= #TCQ 1'b1+rd_ptr_cp;
              end
  ...
  always @(posedge rd_clk)
      begin
        ...
        else if ((buf_filled == 0) || 
                 ((rd_strobe && (buf_filled == 0)) || 
(
(rd_strobe && 
((rd_strobe && (buf_filled == 0)) || (buf_filled == 1))) || ((buf_filled == 1) && (buf_filled == 0))))) empty <= #TCQ 1'b1;
        else empty <= #TCQ 1'b0;
      end
  reg  [ASIZE:0] wbin;
  ...
  always @(posedge rd_clk)
      begin
        if (rst) 
          ...
          begin
            if (wr_en) {COutc,wr_ptr} <= #TCQ wr_ptr+1'b1;
              
          end
      end
  assign {COutd,buf_avail} = (5'd16+(0-wr_ptr))+rd_capture_ptr;
...
((buf_avail == 0) || 
(((buf_avail == 0) && (buf_avail == 1)) || 
((buf_avail == 1) && wr_en) || wr_en)) && 
                 (((buf_avail == 0) && wr_en) || (buf_avail == 1) || 
(buf_avail == 0))) full <= #TCQ 1'b1;
  ...
  always @(posedge wr_clk)
      begin
        ...
        else if (((buf_avail == ((0-3)+FIFO_DEPTH)) && wr_en) || 
