...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  (input  wr_clk,
   input  rst,
   input  wr_en,
   input  [DSIZE+(0-1):0] wr_data,
   input  rd_en,
   input  rd_clk,
   output [DSIZE+(0-1):0] rd_data,
   output reg full,
   output reg empty,
   output reg almost_full);

  reg  [DSIZE+(0-1):0] mem[0:FIFO_DEPTH+(0-1)];
  ...
  assign {COutb,buf_filled} = (0-rd_ptr)+wr_capture_ptr;
  ...
  always @(posedge wr_clk)
      begin
        ...
        else if (((buf_avail == 1) && (buf_avail == 0)) || 
                 (
(wr_en && 
((buf_avail == 1) || (wr_en && (buf_avail == 0)))) || ((buf_avail == 1) && (buf_avail == 0))) || (buf_avail == 0)) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
  ...
  always @(posedge wr_clk)
      begin
        ...
        else if ((buf_avail == ((0-1<<1)+FIFO_DEPTH)) || 
                 ((buf_avail == (FIFO_DEPTH-3)) && 
(wr_en || 
((buf_avail == (FIFO_DEPTH-3)) && 
(buf_avail == ((0-1<<1)+FIFO_DEPTH)))))) almost_full <= #TCQ 1'b1;
