...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  reg  [DSIZE-1:0] mem[0:(0-1)+FIFO_DEPTH];
  ...
  always @(posedge wr_clk)
      begin
        ...
        else if (((buf_avail == 1) && wr_en) || (buf_avail == 0)) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
...
((buf_avail == (FIFO_DEPTH+(0-3))) && 
(buf_avail == (FIFO_DEPTH+(0-2)))) || 
                 ((buf_avail == (FIFO_DEPTH+(0-3))) && 
(
((buf_avail == (FIFO_DEPTH+(0-2))) || 
(buf_avail == (FIFO_DEPTH+(0-3)))) && wr_en)) || (buf_avail == (FIFO_DEPTH+(0-2)))) almost_full <= #TCQ 1'b1;
