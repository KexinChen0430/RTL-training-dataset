...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  reg  [(-1)+DSIZE:0] mem[0:FIFO_DEPTH-1];
  ...
  assign {COutb,buf_filled} = wr_capture_ptr+(0-rd_ptr);
...
((rd_strobe && (buf_filled == 1)) || 
((buf_filled == 1) && (buf_filled == 0))))) empty <= #TCQ 1'b1;
        else empty <= #TCQ 1'b0;
      end
  reg  [ASIZE:0] wbin;
  ...
  assign {COutd,buf_avail} = (5'd16+(-wr_ptr))+rd_capture_ptr;
  
  always @(posedge wr_clk)
      begin
        if (rst) full <= #TCQ 1'b0;
        else if ((wr_en && (buf_avail == 1)) || 
                 ((buf_avail == 0) && (buf_avail == 1)) || (buf_avail == 0)) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
  ...
  always @(posedge wr_clk)
      begin
        ...
        else if ((buf_avail == (FIFO_DEPTH-1<<<1)) || 
                 (
(((buf_avail == (FIFO_DEPTH-1<<<1)) || wr_en) && 
(buf_avail == (FIFO_DEPTH-3))) || ((buf_avail == (FIFO_DEPTH-1<<<1)) && wr_en))) almost_full <= #TCQ 1'b1;
