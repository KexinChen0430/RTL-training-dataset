...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  reg  [DSIZE+(0-1):0] mem[0:FIFO_DEPTH-1];
  ...
  wire [(0-1)+ASIZE:0] wr_addr,rd_addr;
  wire COutb,COutd;
  reg  COuta,COutc;
  ...
  assign wr_addr = wr_ptr[(0-1)+ASIZE:0];
  ...
  assign rd_addr = rd_ptr_cp[(0-1)+ASIZE:0];
  ...
  always @(posedge rd_clk)
      begin
        ...
        else if (((buf_filled == 1) && rd_strobe) || 
                 ((buf_filled == 1) && (buf_filled == 0)) || (buf_filled == 0)) empty <= #TCQ 1'b1;
        else empty <= #TCQ 1'b0;
      end
  reg  [ASIZE:0] wbin;
...
(buf_avail == (FIFO_DEPTH-2))) || wr_en) && (buf_avail == (FIFO_DEPTH-3))) || (buf_avail == (FIFO_DEPTH-2))) almost_full <= #TCQ 1'b1;
