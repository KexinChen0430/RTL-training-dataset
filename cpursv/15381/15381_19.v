...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  reg  [DSIZE-1:0] mem[0:FIFO_DEPTH+(0-1)];
  ...
  always @(posedge wr_clk)
      begin
        if (rst) full <= #TCQ 1'b0;
                 ...
                 ((buf_avail == 1) && wr_en) || (buf_avail == 0)) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
...
(buf_avail == ((-3)+FIFO_DEPTH)) || (buf_avail == (FIFO_DEPTH+(-1<<<1)))) && (wr_en || (buf_avail == (FIFO_DEPTH+(-1<<<1))))) almost_full <= #TCQ 1'b1;
