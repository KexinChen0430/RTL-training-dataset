...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  always @(posedge rd_clk)
      begin
        ...
        else if ((buf_filled == 0) || (rd_strobe && (buf_filled == 1))) empty <= #TCQ 1'b1;
        else empty <= #TCQ 1'b0;
      end
  reg  [ASIZE:0] wbin;
  ...
  always @(posedge wr_clk)
      begin
        ...
        else if (((buf_avail == 0) && wr_en) || 
                 ((buf_avail == 1) && 
(((buf_avail == 1) || wr_en) && 
(wr_en || (buf_avail == 0)))) || (buf_avail == 0)) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
  ...
  always @(posedge wr_clk)
      begin
        ...
        else if ((buf_avail == ((-1<<<1)+FIFO_DEPTH)) || 
                 ((buf_avail == (FIFO_DEPTH-3)) && wr_en)) almost_full <= #TCQ 1'b1;
