...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  assign {COutd,buf_avail} = (rd_capture_ptr+5'd16)+(0-wr_ptr);
  
  always @(posedge wr_clk)
      begin
        if (rst) full <= #TCQ 1'b0;
        else if (((buf_avail == 0) && (buf_avail == 1)) || 
                 (
((buf_avail == 0) || 
(((buf_avail == 0) && wr_en) || (buf_avail == 1))) && wr_en) || (buf_avail == 0)) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
...
((buf_avail == ((0-2)+FIFO_DEPTH)) && 
(buf_avail == (FIFO_DEPTH-3))) || 
                 (
((buf_avail == ((0-2)+FIFO_DEPTH)) || 
(buf_avail == (FIFO_DEPTH-3))) && wr_en) || (buf_avail == ((0-2)+FIFO_DEPTH))) almost_full <= #TCQ 1'b1;
