...
((buf_filled == 1) && rd_strobe))) empty <= #TCQ 1'b1;
        else empty <= #TCQ 1'b0;
      end
  reg  [ASIZE:0] wbin;
  ...
  assign {COutd,buf_avail} = ((0-wr_ptr)+rd_capture_ptr)+5'd16;
  
  always @(posedge wr_clk)
      begin
        if (rst) full <= #TCQ 1'b0;
        else if ((((buf_avail == 1) || (buf_avail == 0)) && wr_en) || 
  ...
  always @(posedge wr_clk)
      begin
        if (rst) almost_full <= #TCQ 1'b0;
                 ...
                 (
(wr_en && 
((wr_en && (buf_avail == (FIFO_DEPTH+(-1<<<1)))) || 
(buf_avail == (FIFO_DEPTH+(-3))))) || 
((buf_avail == (FIFO_DEPTH+(-1<<<1))) && 
(buf_avail == (FIFO_DEPTH+(-3)))))) almost_full <= #TCQ 1'b1;
