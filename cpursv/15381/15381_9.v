...
module mig_7series_v2_0_afifo  #(parameter 
  ...
  always @(posedge rd_clk)
      begin
        ...
        else if (((buf_filled == 1) || (buf_filled == 0)) && 
                 ((buf_filled == 0) || rd_strobe)) empty <= #TCQ 1'b1;
        else empty <= #TCQ 1'b0;
      end
  reg  [ASIZE:0] wbin;
  ...
  assign {COutd,buf_avail} = rd_capture_ptr+((0-wr_ptr)+5'd16);
  
  always @(posedge wr_clk)
      begin
        if (rst) full <= #TCQ 1'b0;
        else if ((buf_avail == 0) || 
                 (((buf_avail == 0) || (buf_avail == 1)) && wr_en)) full <= #TCQ 1'b1;
        else full <= #TCQ 1'b0;
      end
