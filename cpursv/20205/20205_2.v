...
module stream_tg  #(parameter 
  ...
  (input  aclk,
   ...
   output [DATA_WIDTH+(0-1):0] write_data,
   output write_data_valid,
   input  write_data_ready,
   output [KEEP_WIDTH+(0-1):0] write_data_keep,
   output write_data_last,
   output [71:0] read_cmd,
   output read_cmd_valid,
   input  read_cmd_ready,
   input  [DATA_WIDTH+(0-1):0] read_data,
   input  read_data_valid,
   input  [KEEP_WIDTH+(0-1):0] read_data_keep,
  ...
  reg  [1<<1:0] test_state_r;
  reg  [71:0] write_cmd_r;
  reg  write_cmd_valid_r;
  reg  [DATA_WIDTH+(0-1):0] write_data_r,write_data_r1;
  reg  write_data_valid_r;
  reg  [KEEP_WIDTH+(0-1):0] write_data_keep_r,write_data_keep_r1;
  reg  write_data_last_r;
  reg  [71:0] read_cmd_r;
  reg  read_cmd_valid_r;
  reg  [DATA_WIDTH+(0-1):0] read_data_r,read_data_r1;
  reg  read_data_valid_r;
  reg  [KEEP_WIDTH+(0-1):0] read_data_keep_r,read_data_keep_r1;
        ...
        begin
          case (test_state_r)
            ...
            WRITE_CMD: begin
                  if (write_cmd_ready) 
                    begin
                      ...
                      write_cmd_r[63:32] <= 1+write_cmd_r[63:32];
                    end
            ...
            WRITE_DATA: begin
                  ...
                  if (write_sts_data[7] & write_sts_valid) 
                    begin
            ...
            READ_CMD: begin
                  ...
                  if (read_sts_data[7] & read_sts_valid) 
                    begin
            ...
            COMPARE: begin
                  if ((read_data_r1 != write_data_r1) | (read_data_keep_r1 != write_data_keep_r1)) 
                    begin
