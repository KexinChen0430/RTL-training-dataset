...
module stream_tg  #(parameter 
  ...
  (input  aclk,
   ...
   output [(-1)+DATA_WIDTH:0] write_data,
   output write_data_valid,
   input  write_data_ready,
   output [(-1)+KEEP_WIDTH:0] write_data_keep,
   output write_data_last,
   output [71:0] read_cmd,
   output read_cmd_valid,
   input  read_cmd_ready,
   input  [(-1)+DATA_WIDTH:0] read_data,
   input  read_data_valid,
   input  [(-1)+KEEP_WIDTH:0] read_data_keep,
  ...
  reg  [(-1)+DATA_WIDTH:0] write_data_r,write_data_r1;
  reg  write_data_valid_r;
  reg  [(-1)+KEEP_WIDTH:0] write_data_keep_r,write_data_keep_r1;
  reg  write_data_last_r;
  reg  [71:0] read_cmd_r;
  reg  read_cmd_valid_r;
  reg  [(-1)+DATA_WIDTH:0] read_data_r,read_data_r1;
  reg  read_data_valid_r;
  reg  [(-1)+KEEP_WIDTH:0] read_data_keep_r,read_data_keep_r1;
