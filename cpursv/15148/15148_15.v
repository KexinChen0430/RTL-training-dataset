...
module arriaii_termination(rdn,rup,scanclock,scanin,scaninmux,scanshiftmux,
                           terminationuserclear,terminationuserclock,comparatorprobe,
                           scanout,terminationclockout,terminationcontrolprobe,
                           terminationdataout,terminationdone,terminationselectout);
  ...
  always @(terminationuserclock or terminationuserclear)
      begin
        if ((terminationuserclear == 1'b1) && 
            (terminationuserclear_prev == 1'b0)) 
          begin
            reset_oct_reg = 1'b1;
        ...
        if (terminationselectout_tmp == 1'b1) terminationclockout_tmp = terminationuserclock;
          
        if ((internal_clk_enable == 1'b1) && 
            (terminationuserclock == 1'b1)) 
          begin
            if (cal_shift_cycles == 27) terminationdone_tmp = 1'b0;
            else 
              begin
                if (cal_shift_cycles == 10) terminationselectout_tmp = 1'b1;
                  
                cal_shift_cycles = 1+cal_shift_cycles;
              end
          end
          
        if ((terminationuserclock == 1'b0) && 
            (cal_shift_cycles == 27)) terminationselectout_tmp = 1'b0;
