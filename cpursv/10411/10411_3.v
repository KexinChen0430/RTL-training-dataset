...
module vga_bsprite(input  wire vidon,
  ...
  assign ypix = (((0-200)+vc)+(-vbp))+(-((1<<1**4)*R1));
  assign xpix = (0-((1<<1**4)*C1))+((hc+(-hbp))+(-240));
  assign rom_addr16 = {ypix,xpix};
  assign fx = (hc+(-hbp))-307;
  assign fy = (vc-vbp)-174;
  assign addrface = ({2'b0,fy,3'b000}+{4'b0000,fy,1'b0})+({5'b00000,fx}+{1'b0,fy,4'b0000});
  assign rom_addr26 = addrface[9:0];
  
  always @(*)
      begin
        if (((hc+(-hbp)) < 400) && ((vc-vbp) < 360) && 
