...
module Fifo  #(parameter  WIDTH = 8, LOG2_DEPTH = 4)
  ...
  reg  [(0-1)+WIDTH:0] memory[(2**LOG2_DEPTH)+(0-1):0];
  reg  [LOG2_DEPTH+(0-1):0] pointer;
  wire zero;
  integer i;
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            dataPresent <= ((~read & dataPresent) & (((write | ((dataPresent | ~zero) | (dataPresent & write))) & zero) & ((zero & dataPresent) | ((~zero & dataPresent) | write)))) | ((~zero & dataPresent) | write);
            case ({read,write})
  ...
  assign zero = ~|pointer;
  assign halfFull = pointer[LOG2_DEPTH+(0-1)];
  assign full = &pointer;
endmodule
