...
module phsaligner  #(parameter 
  ...
  always @(posedge clk)
      begin
        ...
        blnkbgn <= #1 rcvd_ctkn & !rcvd_ctkn_q;
      end
  reg  [SRCHTIMERWD+(0-1):0] ctkn_srh_timer;
  ...
  always @(posedge clk)
      begin
        ...
        else ctkn_srh_timer <= #1 ctkn_srh_timer+1'b1;
      end
  reg  ctkn_srh_tout;
  ...
  reg  [(0-1)+CTKNCNTWD:0] ctkn_counter;
  ...
  parameter  BITSLIP = (1<<<1*1<<<1)*6'b1;

  parameter  RCVDCTKN = 6'b1<<3;

  parameter  BLNKPRD = 6'b1<<4;

  parameter  PSALGND = 1<<<5*6'b1;

  parameter  nSTATES = 6;

  reg  [nSTATES+(0-1):0]  cstate = {{nSTATES+(0-1){1'b0}},1'b1};

  reg  [nSTATES+(0-1):0] nstate;
          ...
          begin
            case (cstate)
              ...
              BITSLIP: begin
                    ctkn_srh_rst <= #1 1'b1;
                    bitslip <= #1 1'b1;
                    bitslip_cnt <= #1 1'b1+bitslip_cnt;
              ...
              BLNKPRD: begin
                    blnkprd_cnt <= #1 1'b1+blnkprd_cnt;
                  end
