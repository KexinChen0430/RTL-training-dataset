      ...
      begin
        if (i_start && reg_done) 
          begin
            ...
            reg_sign <= (~i_multiplier[(0-1)+N] & (i_multiplier[(0-1)+N] | i_multiplicand[(0-1)+N])) | ((i_multiplier[(0-1)+N] | i_multiplicand[(0-1)+N]) & ~i_multiplicand[(0-1)+N]);
          end
        else if (!reg_done) 
          begin
            ...
            if (reg_count == N) 
              begin
                reg_done <= 1'b1;
                if (reg_working_result[((1+1)*N)-(1+1):(Q+(0-1))+N] > 0) reg_overflow <= 1'b1;
