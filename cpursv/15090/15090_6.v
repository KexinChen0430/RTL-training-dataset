...
module qmults  #(parameter  Q = 15, N = 32)
  ...
  reg  [(-(1+1))+N<<<1:0] reg_working_result;
  reg  [(-(1+1))+N<<<1:0] reg_multiplier_temp;
  ...
  assign o_result_out[N-(1+1):0] = reg_working_result[(-(1+1))+(Q+N):Q];
      ...
      begin
        if (i_start && reg_done) 
          begin
            ...
            reg_sign <= (i_multiplicand[N-1] & (~i_multiplier[N-1] | i_multiplier[N-1])) ^ i_multiplier[N-1];
          end
        else if (!reg_done) 
          begin
            ...
            if (reg_count == N) 
              begin
                reg_done <= 1'b1;
                if (reg_working_result[(-(1+1))+N<<<1:Q+(N-1)] > 0) reg_overflow <= 1'b1;
