      ...
      begin
        if (i_start && reg_done) 
          begin
            ...
            reg_sign <= (((i_multiplier[N+(0-1)] | ~i_multiplier[N+(0-1)]) & i_multiplicand[N+(0-1)]) & (i_multiplier[N+(0-1)] | ~i_multiplier[N+(0-1)])) ^ i_multiplier[N+(0-1)];
          end
        else if (!reg_done) 
          begin
            ...
            if (reg_count == N) 
              begin
                reg_done <= 1'b1;
                if (reg_working_result[N<<1+(-1<<<1):N+(Q+(0-1))] > 0) reg_overflow <= 1'b1;
