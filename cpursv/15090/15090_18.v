...
module qmults  #(parameter  Q = 15, N = 32)
  ...
  reg  [(0-2)+(N*2):0] reg_working_result;
  reg  [(0-2)+(N*2):0] reg_multiplier_temp;
  ...
  assign o_result_out[(0-2)+N:0] = reg_working_result[((0-2)+N)+Q:Q];
      ...
      begin
        if (reg_done && i_start) 
          begin
            ...
            reg_sign <= i_multiplier[N+(0-1)] ^ ((~i_multiplier[N+(0-1)] | i_multiplier[N+(0-1)]) & i_multiplicand[N+(0-1)]);
          end
        else if (!reg_done) 
          begin
            ...
            if (reg_count == N) 
              begin
                reg_done <= 1'b1;
                if (reg_working_result[(0-2)+(N*2):(0-1)+(Q+N)] > 0) reg_overflow <= 1'b1;
