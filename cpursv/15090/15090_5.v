...
module qmults  #(parameter  Q = 15, N = 32)
  ...
  assign o_result_out[N-1<<1:0] = reg_working_result[(Q+(0-1<<1))+N:Q];
      ...
      begin
        if (reg_done && i_start) 
          begin
            ...
            reg_sign <= (i_multiplicand[N+(0-1)] & (~i_multiplier[N+(0-1)] | ((i_multiplier[N+(0-1)] & ~i_multiplier[N+(0-1)]) | ~i_multiplicand[N+(0-1)]))) | (i_multiplier[N+(0-1)] & ~i_multiplicand[N+(0-1)]);
          end
        else if (!reg_done) 
          begin
            ...
            if (reg_count == N) 
              begin
                reg_done <= 1'b1;
                if (reg_working_result[(0-1<<1)+N<<<1:(Q+(0-1))+N] > 0) reg_overflow <= 1'b1;
