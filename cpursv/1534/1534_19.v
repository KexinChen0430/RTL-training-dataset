...
module soc_system_button_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((({2{address == 3}} & (((edge_capture | data_in) & ({2{address == 0}} | edge_capture)) | irq_mask)) & (((({2{address == 2}} | ({2{address == 2}} & irq_mask)) | (({2{address == 0}} & irq_mask) & data_in)) | (data_in & {2{address == 0}})) | edge_capture)) | (({2{address == 0}} & irq_mask) & data_in)) | ((data_in | ({2{address == 2}} & irq_mask)) & (({2{address == 0}} | irq_mask) & ({2{address == 0}} | {2{address == 2}})));
