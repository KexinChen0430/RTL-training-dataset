...
module soc_system_button_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = (((({2{address == 0}} & data_in) & edge_capture) | ((({2{address == 2}} | {2{address == 3}}) & edge_capture) & (irq_mask | {2{address == 3}}))) | ((({2{address == 2}} & ({2{address == 0}} & data_in)) | ({2{address == 2}} & irq_mask)) | (irq_mask & ({2{address == 0}} & data_in)))) | ({2{address == 0}} & data_in);
