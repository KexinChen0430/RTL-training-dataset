...
module altera_up_av_config_auto_init(clk,reset,clear_error,ack,transfer_complete,rom_data,
  ...
  always @(posedge clk)
      begin
        ...
        else if (auto_init_complete | transfer_complete) transfer_data <= 1'b0;
  ...
  always @(posedge clk)
      begin
        ...
        else if (toggle_next_transfer & (rom_address == ((0-1)+ROM_SIZE))) auto_init_complete <= 1'b1;
  ...
  always @(posedge clk)
      begin
        ...
        else if (ack & toggle_next_transfer) auto_init_error <= 1'b1;
        else if (clear_error) auto_init_error <= 1'b0;
          
      end
  assign toggle_next_transfer = transfer_complete & transfer_data;
endmodule
