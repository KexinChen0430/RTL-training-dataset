...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  parameter [DEC_SZ+(0-1):0] 
       CNTRL1_D = (2**CNTRL1)*BASE_REG,
       CNTRL2_D = BASE_REG<<CNTRL2,
       CNTRL3_D = BASE_REG<<CNTRL3,
       CNTRL4_D = BASE_REG*(2**CNTRL4);
  wire 
       reg_sel = per_en & (per_addr[13:(0-1)+DEC_WD] == BASE_ADDR[14:DEC_WD]);
  wire [(0-1)+DEC_WD:0]  reg_addr = {1'b0,per_addr[DEC_WD-2:0]};
  wire [DEC_SZ+(0-1):0] 
       reg_dec = ((({DEC_SZ{reg_addr == CNTRL1>>1}} | CNTRL2_D) & ({DEC_SZ{reg_addr == CNTRL1>>1}} | {DEC_SZ{reg_addr == CNTRL2>>1}})) & CNTRL1_D) | ((CNTRL3_D & ({DEC_SZ{reg_addr == CNTRL3>>1}} & (CNTRL2_D | ((({DEC_SZ{reg_addr == CNTRL1>>1}} & CNTRL1_D) | CNTRL3_D) & (({DEC_SZ{reg_addr == CNTRL1>>1}} & CNTRL1_D) | {DEC_SZ{reg_addr == CNTRL3>>1}}))))) | (({DEC_SZ{reg_addr == CNTRL2>>1}} | (CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}})) & ((CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}}) | CNTRL2_D)));
  wire  reg_lo_write = per_we[0] & reg_sel;
  wire  reg_hi_write = reg_sel & per_we[1];
  wire  reg_read = reg_sel & ~|per_we;
  wire [DEC_SZ+(0-1):0]  reg_hi_wr = {DEC_SZ{reg_hi_write}} & reg_dec;
  wire [DEC_SZ+(0-1):0]  reg_lo_wr = {DEC_SZ{reg_lo_write}} & reg_dec;
  wire [DEC_SZ+(0-1):0]  reg_rd = reg_dec & {DEC_SZ{reg_read}};
  reg  [7:0] cntrl1;
  wire 
  ...
  wire [15:0] 
       cntrl4_rd = {8'h00,cntrl4 & {8{reg_rd[CNTRL4]}}}*(2**({4{CNTRL4[0]}} & 8));

  wire [15:0] 
       per_dout = cntrl2_rd | ((cntrl3_rd | cntrl1_rd) | cntrl4_rd);
