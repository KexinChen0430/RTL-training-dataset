...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  wire 
       reg_sel = per_en & (per_addr[13:(-1)+DEC_WD] == BASE_ADDR[14:DEC_WD]);
  wire [(-1)+DEC_WD:0]  reg_addr = {1'b0,per_addr[DEC_WD+(-1<<1):0]};
  wire [DEC_SZ-1:0] 
       reg_dec = ({DEC_SZ{reg_addr == CNTRL1>>1}} & CNTRL1_D) | (({DEC_SZ{reg_addr == CNTRL4>>1}} & CNTRL4_D) | ((CNTRL2_D | ({DEC_SZ{reg_addr == CNTRL3>>1}} & CNTRL3_D)) & ({DEC_SZ{reg_addr == CNTRL2>>1}} | ({DEC_SZ{reg_addr == CNTRL3>>1}} & CNTRL3_D))));
  ...
  wire [15:0] 
       cntrl3_rd = (1<<1**({4{CNTRL3[0]}} & 8))*{8'h00,{8{reg_rd[CNTRL3]}} & cntrl3};

  wire [15:0] 
       cntrl4_rd = {8'h00,{8{reg_rd[CNTRL4]}} & cntrl4}<<<8 & {4{CNTRL4[0]}};

  wire [15:0] 
       per_dout = cntrl4_rd | ((cntrl1_rd | cntrl2_rd) | cntrl3_rd);
