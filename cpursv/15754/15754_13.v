...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  wire [(0-1)+DEC_WD:0]  reg_addr = {1'b0,per_addr[(-(1+1))+DEC_WD:0]};
  wire [DEC_SZ-1:0] 
       reg_dec = ((CNTRL1_D | ((CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}}) | ({DEC_SZ{reg_addr == CNTRL3>>1}} & ((CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}}) | CNTRL3_D)))) & (((CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}}) | ({DEC_SZ{reg_addr == CNTRL3>>1}} & ((CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}}) | CNTRL3_D))) | {DEC_SZ{reg_addr == CNTRL1>>1}})) | ((({DEC_SZ{reg_addr == CNTRL1>>1}} & (CNTRL1_D | CNTRL2_D)) | CNTRL2_D) & {DEC_SZ{reg_addr == CNTRL2>>1}});
  ...
  wire [15:0] 
       cntrl3_rd = ((1+1)**(8 & {4{CNTRL3[0]}}))*{8'h00,cntrl3 & {8{reg_rd[CNTRL3]}}};
