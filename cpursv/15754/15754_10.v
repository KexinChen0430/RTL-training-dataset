...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  parameter [DEC_SZ+(-1):0] 
       CNTRL1_D = BASE_REG*(2**CNTRL1),
       CNTRL2_D = BASE_REG<<CNTRL2,
       CNTRL3_D = BASE_REG<<CNTRL3,
       CNTRL4_D = BASE_REG*(2**CNTRL4);
  wire 
       reg_sel = per_en & (per_addr[13:DEC_WD-1] == BASE_ADDR[14:DEC_WD]);
  wire [DEC_WD-1:0]  reg_addr = {1'b0,per_addr[(0-2)+DEC_WD:0]};
  wire [DEC_SZ+(-1):0] 
       reg_dec = (({DEC_SZ{reg_addr == CNTRL2>>1}} | ((CNTRL1_D | {DEC_SZ{reg_addr == CNTRL2>>1}}) & {DEC_SZ{reg_addr == CNTRL1>>1}})) & (CNTRL2_D | (({DEC_SZ{reg_addr == CNTRL1>>1}} | CNTRL2_D) & CNTRL1_D))) | (((CNTRL3_D & {DEC_SZ{reg_addr == CNTRL3>>1}}) | CNTRL4_D) & (({DEC_SZ{reg_addr == CNTRL3>>1}} | {DEC_SZ{reg_addr == CNTRL4>>1}}) & (CNTRL3_D | {DEC_SZ{reg_addr == CNTRL4>>1}})));
  ...
  wire [15:0] 
       cntrl3_rd = (2**(8 & {4{CNTRL3[0]}}))*{8'h00,cntrl3 & {8{reg_rd[CNTRL3]}}};

  wire [15:0] 
       cntrl4_rd = {8'h00,cntrl4 & {8{reg_rd[CNTRL4]}}}<<<{4{CNTRL4[0]}} & 8;

  wire [15:0] 
       per_dout = (cntrl1_rd | cntrl4_rd) | (cntrl2_rd | cntrl3_rd);
