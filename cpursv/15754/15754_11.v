...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  wire [DEC_SZ+(-1):0] 
       reg_dec = ((({DEC_SZ{reg_addr == CNTRL1>>1}} | ({DEC_SZ{reg_addr == CNTRL2>>1}} & CNTRL2_D)) & CNTRL1_D) | (({DEC_SZ{reg_addr == CNTRL2>>1}} & CNTRL2_D) | (({DEC_SZ{reg_addr == CNTRL4>>1}} & CNTRL4_D) | {DEC_SZ{reg_addr == CNTRL3>>1}}))) & ((({DEC_SZ{reg_addr == CNTRL2>>1}} & CNTRL2_D) | (CNTRL3_D | ({DEC_SZ{reg_addr == CNTRL4>>1}} & CNTRL4_D))) | (({DEC_SZ{reg_addr == CNTRL1>>1}} | ({DEC_SZ{reg_addr == CNTRL2>>1}} & CNTRL2_D)) & CNTRL1_D));
  ...
  wire [15:0] 
       cntrl1_rd = {8'h00,cntrl1 & {8{reg_rd[CNTRL1]}}}*(1<<<1**(8 & {4{CNTRL1[0]}}));
