...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  wire [(0-1)+DEC_WD:0]  reg_addr = {1'b0,per_addr[DEC_WD+(0-1<<1):0]};
  wire [DEC_SZ-1:0] 
       reg_dec = (((CNTRL3_D | (({DEC_SZ{reg_addr == CNTRL1>>1}} | {DEC_SZ{reg_addr == CNTRL2>>1}}) & ({DEC_SZ{reg_addr == CNTRL1>>1}} | CNTRL2_D))) | (CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}})) & (((CNTRL1_D | CNTRL3_D) | ({DEC_SZ{reg_addr == CNTRL2>>1}} & CNTRL2_D)) | (CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}}))) & ((((CNTRL4_D & {DEC_SZ{reg_addr == CNTRL4>>1}}) | {DEC_SZ{reg_addr == CNTRL3>>1}}) | (CNTRL1_D & {DEC_SZ{reg_addr == CNTRL1>>1}})) | (CNTRL2_D & (({DEC_SZ{reg_addr == CNTRL1>>1}} & (CNTRL1_D | {DEC_SZ{reg_addr == CNTRL2>>1}})) | {DEC_SZ{reg_addr == CNTRL2>>1}})));
  ...
  wire [15:0] 
       per_dout = cntrl3_rd | ((cntrl2_rd | cntrl1_rd) | cntrl4_rd);
