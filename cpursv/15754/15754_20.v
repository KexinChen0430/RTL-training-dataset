...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  wire [(0-1)+DEC_SZ:0] 
       reg_dec = ((CNTRL2_D & ({DEC_SZ{reg_addr == CNTRL2>>1}} | ({DEC_SZ{reg_addr == CNTRL1>>1}} & ({DEC_SZ{reg_addr == CNTRL2>>1}} | CNTRL1_D)))) | ((({DEC_SZ{reg_addr == CNTRL3>>1}} & CNTRL3_D) | {DEC_SZ{reg_addr == CNTRL1>>1}}) & (({DEC_SZ{reg_addr == CNTRL3>>1}} & CNTRL3_D) | CNTRL1_D))) | ((((CNTRL3_D | CNTRL4_D) & (CNTRL4_D | {DEC_SZ{reg_addr == CNTRL3>>1}})) | (({DEC_SZ{reg_addr == CNTRL1>>1}} | {DEC_SZ{reg_addr == CNTRL2>>1}}) & (((CNTRL1_D | CNTRL2_D) & (CNTRL2_D | {DEC_SZ{reg_addr == CNTRL1>>1}})) & ({DEC_SZ{reg_addr == CNTRL2>>1}} | CNTRL1_D)))) & {DEC_SZ{reg_addr == CNTRL4>>1}});
  ...
  wire [(0-1)+DEC_SZ:0]  reg_lo_wr = {DEC_SZ{reg_lo_write}} & reg_dec;
  ...
  wire [15:0] 
       cntrl2_rd = {8'h00,{8{reg_rd[CNTRL2]}} & cntrl2}*((1+1)**({4{CNTRL2[0]}} & 8));

  wire [15:0] 
       cntrl3_rd = {8'h00,cntrl3 & {8{reg_rd[CNTRL3]}}}<<<{4{CNTRL3[0]}} & 8;

  wire [15:0] 
       cntrl4_rd = {8'h00,{8{reg_rd[CNTRL4]}} & cntrl4}<<<8 & {4{CNTRL4[0]}};

  wire [15:0] 
       per_dout = (cntrl2_rd | cntrl1_rd) | (cntrl3_rd | cntrl4_rd);
