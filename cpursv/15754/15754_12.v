...
module template_periph_8b(per_dout,mclk,per_addr,per_din,per_en,per_we,puc_rst);
  ...
  parameter [(-1)+DEC_SZ:0]  BASE_REG = {{(-1)+DEC_SZ{1'b0}},1'b1};
  parameter [(-1)+DEC_SZ:0] 
  ...
  wire [(-1)+DEC_SZ:0] 
       reg_dec = ((((CNTRL1_D | {DEC_SZ{reg_addr == CNTRL2>>1}}) & ({DEC_SZ{reg_addr == CNTRL1>>1}} | {DEC_SZ{reg_addr == CNTRL2>>1}})) & CNTRL2_D) | (((CNTRL1_D & ((CNTRL2_D | {DEC_SZ{reg_addr == CNTRL1>>1}}) & (CNTRL1_D | CNTRL2_D))) | ({DEC_SZ{reg_addr == CNTRL4>>1}} & CNTRL4_D)) & (({DEC_SZ{reg_addr == CNTRL4>>1}} & CNTRL4_D) | {DEC_SZ{reg_addr == CNTRL1>>1}}))) | (CNTRL3_D & {DEC_SZ{reg_addr == CNTRL3>>1}});
  wire  reg_lo_write = reg_sel & per_we[0];
  wire  reg_hi_write = per_we[1] & reg_sel;
  wire  reg_read = ~|per_we & reg_sel;
  wire [(-1)+DEC_SZ:0]  reg_hi_wr = {DEC_SZ{reg_hi_write}} & reg_dec;
  wire [(-1)+DEC_SZ:0]  reg_lo_wr = reg_dec & {DEC_SZ{reg_lo_write}};
  wire [(-1)+DEC_SZ:0]  reg_rd = reg_dec & {DEC_SZ{reg_read}};
  reg  [7:0] cntrl1;
  wire 
  ...
  wire [15:0] 
       per_dout = ((cntrl1_rd | cntrl2_rd) | cntrl3_rd) | cntrl4_rd;
