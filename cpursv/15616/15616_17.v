...
module MusicScore(ReadOrWrite,Address,KeyInput,KeyOutput,TimeInput,TimeOutput,
  ...
  input  [(-1)+DataLength:0] KeyInput,TimeInput;
  output reg [(-1)+DataLength:0] KeyOutput,TimeOutput;
  ...
  reg  [(-1)+DataLength:0] Keys[0:MemorySize+(-1)];

  reg  [(-1)+DataLength:0] Times[0:MemorySize+(-1)];
