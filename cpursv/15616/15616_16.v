...
module MusicScore(ReadOrWrite,Address,KeyInput,KeyOutput,TimeInput,TimeOutput,
  ...
  input  [DataLength+(0-1):0] KeyInput,TimeInput;
  output reg [DataLength+(0-1):0] KeyOutput,TimeOutput;
  parameter  AddressBits = 5;
  input  [AddressBits+(0-1):0] Address;
  ...
  reg  [DataLength+(0-1):0] Keys[0:MemorySize+(0-1)];

  reg  [DataLength+(0-1):0] Times[0:MemorySize+(0-1)];
  ...
  always @(posedge Clock or posedge Reset)
      if (Reset == 1) 
        ...
        begin
          KeyOutput <= Keys[Address+MusicOffset];
          TimeOutput <= Times[Address+MusicOffset];
        end
      else 
        begin
