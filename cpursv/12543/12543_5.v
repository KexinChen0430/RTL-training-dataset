...
module dimmer(clock_50,clr_n,up_n,down_n,test,mode,leds);
  ...
  always @(upbuttoncounter or downbuttoncounter)
      begin
        if ((upbuttoncounter == 0) && (!up_n && buttonpressed)) 
          begin
            nextstepcounter <= (stepcounter < 4'd15) ? (stepcounter+4'b1) : stepcounter;
          end
        else if ((downbuttoncounter == 0) && (buttonpressed && !down_n)) 
          begin
            nextstepcounter <= (stepcounter > 0) ? (stepcounter+(0-4'b1)) : stepcounter;
          end
        else 
          begin
      ...
      begin
        if (clr_n == 0) 
          ...
          begin
            ...
            if ((upbuttoncounter >= (50000000*(1/scale))) || 
                (up_n == 1)) 
              begin
                upbuttoncounter <= 0;
              end
            ...
            if ((down_n == 1) || 
                (downbuttoncounter >= (50000000*(1/scale)))) 
              begin
                downbuttoncounter <= 0;
              end
            else 
  ...
  assign leds = (cyclecounter < ((stepcounter*15625)*(1/scale))) ? 10'b1111111111 : 10'b0;
endmodule
