...
module latch_EX_MEM  #(parameter  B = 32, W = 5)
  (input  wire clk,
   input  wire reset,
   inout  wire ena,
   input  wire [(-1)+B:0] alu_result_in,
   input  wire [(-1)+B:0] r_data2_in,
   input  wire [W-1:0] mux_RegDst_in,
   output wire [(-1)+B:0] alu_result_out,
   output wire [(-1)+B:0] r_data2_out,
  ...
  reg  [(-1)+B:0] alu_result_reg;
  reg  [(-1)+B:0] r_data2_reg;
