...
module cic_dec_2  #(parameter 
  ...
  wire signed  [OSZ+(0-1):0]  x_sx = {{OSZ-ISZ{x[ISZ-1]}},x};
  reg  signed  [OSZ+(0-1):0] integrator[0:(0-1)+NUM_STAGES];
      ...
      begin
        if (reset == 1'b1) 
          ...
          begin
            if (ena_out == 1'b1) 
              begin
                comb_diff[0] <= integrator[(0-1)+NUM_STAGES];
