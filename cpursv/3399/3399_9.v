...
module cic_dec_2  #(parameter 
  ...
  wire signed  [OSZ+(0-1):0]  x_sx = {{OSZ+(0-ISZ){x[(0-1)+ISZ]}},x};
  reg  signed  [OSZ+(0-1):0] integrator[0:NUM_STAGES+(0-1)];
      ...
      begin
        if (reset == 1'b1) 
          ...
          begin
            if (ena_out == 1'b1) 
              begin
                comb_diff[0] <= integrator[NUM_STAGES+(0-1)];
            ...
            always @(posedge clk)
                begin
                  if (reset == 1'b1) 
                    ...
                    begin
                      comb_diff[j] <= comb_diff[j+(0-1)]-comb_dly[j+(0-1)];
