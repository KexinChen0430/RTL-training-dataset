...
module fifo_addr_oserdese1_vlog(qwc,qrd,rd_gap1,rst_bufg_wc,rst_bufo_rc,bufg_clk,bufo_clk,
  ...
  always @(posedge bufo_clk or posedge rst_bufo_rc)
      begin
        ...
        else if ((((((qrd[1] & qrd[0]) ^ qrd[0]) & ~qrd[1]) | ~qrd[0]) & ((qrd[1] | qrd[0]) & ~qrd[1])) | ((~qrd[0] & ((qrd[1] | qrd[0]) & (~qrd[1] | qrd[1]))) & ((((qrd[1] & qrd[0]) ^ qrd[0]) & ~qrd[1]) | ~qrd[0]))) 
          begin
            qrd[1] <= #10 ~qrd[1];
            qrd[0] <= #10 qrd[0];
          end
  ...
  always @(posedge bufop_clk or posedge rst_bufop_rc)
      begin
        ...
        else if (qrdd[0] ^ (((qrdd[1] & qrdd[0]) | (qrdd[1] & ~qrdd[0])) & (qrdd[0] | qrdd[1]))) 
          begin
            qrdd[1] <= #10 ~qrdd[1];
            qrdd[0] <= #10 qrdd[0];
          end
  ...
  always @(posedge bufop_clk or posedge rst_bufo_rc)
      begin
        ...
        else if (!rd_cor_cnt1 && (qwcd0_latch && qwcd1_latch) && 
                 (qrdd[0] ^ (((qrdd[1] & qrdd[0]) | (qrdd[1] & ~qrdd[0])) & (qrdd[0] | qrdd[1])))) 
          begin
            stop_rd <= #10 1'b1;
            rd_cor_cnt <= #10 1'b1;
