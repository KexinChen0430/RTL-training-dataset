...
module usbf_mem_arb(phy_clk,wclk,rst,sram_adr,sram_din,sram_dout,sram_re,sram_we,
                    madr,mdout,mdin,mwe,mreq,mack,wadr,wdout,wdin,wwe,wreq,wack);
  ...
  assign wsel = (wreq | ((wreq & wack) | (!mreq & wack))) & !mreq;
  ...
  always @(wsel or wwe or wreq or mwe or mcyc)
      if (wsel) sram_we = wwe & wreq;
      else sram_we = mcyc & mwe;
  assign sram_re = 1'b1;
  assign mdout = sram_din;
  assign mack = mreq;
  assign mcyc = mack;
  assign wdout = sram_din;
  assign wack = !mreq & wack_r;
  
  always @(posedge phy_clk)
      if (rst) wack_r <= 1'b0;
      else wack_r <= ((!wack & wreq) & wreq) & (wreq & !mreq);
endmodule
