...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((bit1 | bit5) | ((bit2 | bit0) | (bit4 | bit3)));
  parameter  one = ~(bit1 | bit2);
  parameter  two = ~((bit3 | bit1) | ((bit4 | bit6) | bit0));
  parameter  three = ~(((bit6 | bit2) | bit1) | (bit3 | bit0));
  parameter  four = ~((bit1 | bit2) | (bit5 | bit6));
  parameter  five = ~((bit2 | (bit0 | bit5)) | (bit6 | bit3));
  parameter 
       six = ~(((bit0 | bit5) | bit6) | (bit2 | (bit4 | bit3)));
  parameter  seven = ~((bit1 | bit0) | bit2);
  parameter 
       eight = ~(((bit2 | bit0) | (bit4 | bit3)) | ((bit5 | bit6) | bit1));
  parameter  nine = ~((bit1 | bit5) | (bit6 | (bit2 | bit0)));
