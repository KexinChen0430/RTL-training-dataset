...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((((bit1 | bit0) | bit2) | (bit3 | bit4)) | bit5);
  parameter  one = ~(bit2 | bit1);
  parameter  two = ~((bit0 | bit6) | (bit1 | (bit3 | bit4)));
  parameter  three = ~((bit2 | (bit0 | bit6)) | (bit3 | bit1));
  parameter  four = ~((bit5 | (bit6 | bit2)) | bit1);
  parameter  five = ~((bit2 | bit0) | (bit5 | (bit6 | bit3)));
  parameter 
       six = ~(((bit2 | bit3) | (bit6 | bit4)) | (bit0 | bit5));
  parameter  seven = ~((bit1 | bit0) | bit2);
  parameter 
       eight = ~((bit1 | (bit3 | bit4)) | (bit5 | (bit2 | (bit0 | bit6))));
