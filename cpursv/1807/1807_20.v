...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((bit0 | (bit3 | bit4)) | (bit5 | (bit1 | bit2)));
  parameter  one = ~(bit1 | bit2);
  parameter  two = ~(bit6 | ((bit1 | bit0) | (bit3 | bit4)));
  parameter  three = ~(((bit1 | bit0) | bit6) | (bit2 | bit3));
  parameter  four = ~((bit6 | (bit1 | bit5)) | bit2);
  parameter  five = ~((bit5 | bit0) | (bit3 | (bit2 | bit6)));
  parameter 
       six = ~(bit4 | ((bit5 | bit0) | (bit3 | (bit2 | bit6))));
  parameter  seven = ~((bit1 | bit2) | bit0);
  parameter 
       eight = ~(((bit5 | bit6) | bit2) | ((bit1 | bit0) | (bit3 | bit4)));
  parameter  nine = ~((bit2 | (bit5 | bit0)) | (bit1 | bit6));
