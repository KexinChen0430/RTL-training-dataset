...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((bit5 | bit0) | ((bit1 | bit3) | (bit4 | bit2)));
  parameter  one = ~(bit2 | bit1);
  parameter  two = ~((bit0 | bit6) | ((bit1 | bit3) | bit4));
  parameter  three = ~(((bit0 | bit1) | (bit6 | bit3)) | bit2);
  parameter  four = ~((bit2 | (bit5 | bit6)) | bit1);
  parameter  five = ~((bit6 | (bit5 | bit0)) | (bit3 | bit2));
  parameter 
       six = ~((((bit3 | bit2) | bit0) | (bit4 | bit5)) | bit6);
  parameter  seven = ~(bit1 | (bit2 | bit0));
  parameter 
       eight = ~((((bit3 | bit2) | bit0) | (bit4 | bit5)) | (bit6 | bit1));
  parameter  nine = ~(bit6 | ((bit0 | (bit1 | bit5)) | bit2));
