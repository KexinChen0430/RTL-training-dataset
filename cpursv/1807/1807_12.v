...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((bit4 | bit1) | ((bit2 | bit3) | (bit5 | bit0)));
  parameter  one = ~(bit1 | bit2);
  parameter  two = ~((bit3 | bit0) | (bit6 | (bit4 | bit1)));
  parameter  three = ~(((bit0 | bit1) | bit6) | (bit2 | bit3));
  parameter  four = ~(((bit2 | bit5) | bit6) | bit1);
  parameter  five = ~((bit2 | bit0) | (bit5 | (bit6 | bit3)));
  parameter 
       six = ~(((bit4 | bit5) | (bit2 | bit6)) | (bit3 | bit0));
  parameter  seven = ~(bit0 | (bit1 | bit2));
  parameter 
       eight = ~((bit6 | (bit2 | bit0)) | (bit3 | ((bit1 | bit5) | bit4)));
  parameter  nine = ~(bit0 | (((bit2 | bit5) | bit6) | bit1));
