...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((bit4 | (bit2 | bit0)) | (bit3 | (bit5 | bit1)));
  parameter  one = ~(bit1 | bit2);
  parameter  two = ~(((bit4 | bit1) | (bit3 | bit6)) | bit0);
  parameter  three = ~((bit1 | bit2) | (bit3 | (bit0 | bit6)));
  parameter  four = ~(bit5 | ((bit6 | bit1) | bit2));
  parameter  five = ~((bit3 | (bit2 | bit5)) | (bit0 | bit6));
  parameter 
       six = ~(((bit3 | (bit2 | bit5)) | bit4) | (bit0 | bit6));
  parameter  seven = ~(bit0 | (bit1 | bit2));
  parameter 
       eight = ~(((bit5 | bit3) | bit4) | ((bit2 | bit0) | (bit6 | bit1)));
  parameter  nine = ~((bit6 | bit1) | (bit2 | (bit5 | bit0)));
