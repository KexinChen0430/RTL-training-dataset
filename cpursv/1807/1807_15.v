...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~(((bit1 | bit4) | bit5) | ((bit0 | bit3) | bit2));
  parameter  one = ~(bit2 | bit1);
  parameter  two = ~((bit0 | bit4) | (bit3 | (bit6 | bit1)));
  parameter  three = ~(((bit2 | bit3) | (bit0 | bit6)) | bit1);
  parameter  four = ~((bit1 | (bit5 | bit6)) | bit2);
  parameter  five = ~((bit0 | (bit3 | bit6)) | (bit5 | bit2));
  parameter 
       six = ~(bit3 | (bit2 | ((bit5 | bit4) | (bit0 | bit6))));
  parameter  seven = ~(bit2 | (bit0 | bit1));
  parameter 
       eight = ~(((bit6 | (bit0 | bit4)) | (bit2 | (bit1 | bit3))) | bit5);
  parameter  nine = ~((bit1 | (bit5 | bit0)) | (bit6 | bit2));
