...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((bit5 | (bit0 | bit2)) | ((bit1 | bit3) | bit4));
  parameter  one = ~(bit2 | bit1);
  parameter  two = ~((bit6 | bit0) | ((bit1 | bit3) | bit4));
  parameter  three = ~(((bit2 | bit6) | bit0) | (bit1 | bit3));
  parameter  four = ~((bit2 | bit6) | (bit5 | bit1));
  parameter  five = ~((bit5 | bit3) | ((bit2 | bit6) | bit0));
  parameter 
       six = ~(bit0 | (((bit5 | bit6) | bit2) | (bit3 | bit4)));
  parameter  seven = ~(bit0 | (bit2 | bit1));
  parameter 
       eight = ~(((bit3 | bit0) | bit2) | ((bit5 | bit6) | (bit1 | bit4)));
  parameter  nine = ~((bit5 | bit1) | ((bit2 | bit6) | bit0));
