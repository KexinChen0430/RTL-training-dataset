...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~(((bit4 | bit1) | (bit3 | bit5)) | (bit0 | bit2));
  parameter  one = ~(bit2 | bit1);
  parameter  two = ~((bit3 | bit4) | (bit0 | (bit1 | bit6)));
  parameter  three = ~((bit0 | (bit2 | bit1)) | (bit6 | bit3));
  parameter  four = ~((bit6 | bit2) | (bit1 | bit5));
  parameter  five = ~((((bit0 | bit2) | bit6) | bit3) | bit5);
  parameter 
       six = ~((bit5 | bit2) | ((bit0 | bit3) | (bit6 | bit4)));
  parameter  seven = ~(bit0 | (bit2 | bit1));
  parameter 
       eight = ~((bit6 | bit3) | ((bit0 | bit2) | (bit4 | (bit1 | bit5))));
  parameter  nine = ~((bit0 | ((bit5 | bit2) | bit6)) | bit1);
