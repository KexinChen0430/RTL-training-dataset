...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~(((bit0 | bit2) | bit1) | ((bit4 | bit3) | bit5));
  parameter  one = ~(bit2 | bit1);
  parameter  two = ~(((bit6 | bit0) | bit3) | (bit1 | bit4));
  parameter  three = ~((bit0 | bit2) | (bit3 | (bit6 | bit1)));
  parameter  four = ~(((bit5 | bit1) | bit6) | bit2);
  parameter  five = ~(bit3 | ((bit5 | (bit2 | bit6)) | bit0));
  parameter 
       six = ~((bit4 | ((bit0 | bit2) | bit5)) | (bit3 | bit6));
  parameter  seven = ~((bit0 | bit2) | bit1);
  parameter 
       eight = ~((bit0 | (bit2 | (bit1 | bit4))) | ((bit3 | bit5) | bit6));
  parameter  nine = ~(bit1 | ((bit5 | (bit2 | bit6)) | bit0));
