...
module sevenseg(displayout,inputbcd);
  ...
  parameter 
       zero = ~((bit1 | (bit2 | bit5)) | (bit0 | (bit4 | bit3)));
  parameter  one = ~(bit2 | bit1);
  parameter  two = ~((bit1 | bit6) | (bit0 | (bit4 | bit3)));
  parameter  three = ~(((bit3 | bit2) | (bit0 | bit1)) | bit6);
  parameter  four = ~((bit2 | bit5) | (bit1 | bit6));
  parameter  five = ~(((bit5 | bit0) | bit6) | (bit3 | bit2));
  parameter 
       six = ~((bit5 | (bit3 | bit6)) | (bit0 | (bit4 | bit2)));
  parameter  seven = ~(bit1 | (bit0 | bit2));
  parameter 
       eight = ~((bit6 | ((bit4 | bit3) | bit2)) | (bit1 | (bit5 | bit0)));
  parameter  nine = ~((bit2 | bit6) | (bit1 | (bit5 | bit0)));
