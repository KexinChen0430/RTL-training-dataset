...
module oneshot_counter  #(parameter  BITS = 1)
  ...
  assign out = cnt;
  assign enable_wires[0] = (((ena & oneshot) | (((start & oneshot) & ena) | (ena & ~oneshot))) & ~oneshot) | (((ena & oneshot) | (((start & oneshot) & ena) | (ena & ~oneshot))) & ((start & oneshot) | shot_ena));
  genvar i;
  
  generate
      for (i = 0; i < BITS; i = i+1)
          begin : gen_cnt
            assign enable_wires[i+1] = ((cnt[i] & enable_wires[i]) & updown) | (~cnt[i] & ((((cnt[i] & enable_wires[i]) & updown) | ~updown) & enable_wires[i]));
                ...
                begin
                  if (!rst) 
                    ...
                    begin
                      if (i == 0) 
                        begin
                          shot_ena <= ((start & oneshot) | shot_ena) & (oneshot & (start | ~ovf));
                        end
