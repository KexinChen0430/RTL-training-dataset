...
module oneshot_counter  #(parameter  BITS = 1)
  ...
  assign ovf = ena & (end_down_count | (end_up_count & ena));
  assign out = cnt;
  assign enable_wires[0] = (~oneshot & ena) | (((start & oneshot) | shot_ena) & ena);
  genvar i;
  
  generate
      for (i = 0; i < BITS; i = i+1)
          begin : gen_cnt
            assign enable_wires[i+1] = (~updown | (cnt[i] & (updown & enable_wires[i]))) & ((((enable_wires[i] & ~cnt[i]) | updown) & enable_wires[i]) & (cnt[i] | (enable_wires[i] & ~cnt[i])));
                ...
                begin
                  if (!rst) 
                    ...
                    begin
                      if (i == 0) 
                        begin
                          shot_ena <= (start | (shot_ena & ~ovf)) & oneshot;
                        end
