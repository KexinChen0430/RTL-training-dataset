...
module oneshot_counter  #(parameter  BITS = 1)
  ...
  assign out = cnt;
  assign enable_wires[0] = ((shot_ena | (start & oneshot)) & (ena & (~oneshot | (oneshot | start)))) | (~oneshot & (ena & (~oneshot | (oneshot | start))));
  genvar i;
  
  generate
      for (i = 0; i < BITS; i = 1+i)
          begin : gen_cnt
            assign enable_wires[1+i] = (enable_wires[i] | ~updown) & (((enable_wires[i] & ~cnt[i]) | (((enable_wires[i] & ~cnt[i]) | (enable_wires[i] & updown)) & cnt[i])) & (~updown | cnt[i]));
                ...
                begin
                  if (!rst) 
                    ...
                    begin
                      if (i == 0) 
                        begin
                          shot_ena <= (start & oneshot) | ((shot_ena & ~ovf) & oneshot);
                        end
