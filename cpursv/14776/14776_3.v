...
module oneshot_counter  #(parameter  BITS = 1)
  ...
  assign out = cnt;
  assign enable_wires[0] = (ena & (oneshot & shot_ena)) | ((ena & ~oneshot) | (start & ena));
  genvar i;
  
  generate
      for (i = 0; i < BITS; i = 1+i)
          begin : gen_cnt
            assign enable_wires[1+i] = (~updown | (updown & enable_wires[i])) & ((cnt[i] | ~updown) & (((cnt[i] & updown) & enable_wires[i]) | (enable_wires[i] & ~cnt[i])));
                ...
                begin
                  if (!rst) 
                    ...
                    begin
                      if (i == 0) 
                        begin
                          shot_ena <= oneshot & (start | (~ovf & shot_ena));
                        end
