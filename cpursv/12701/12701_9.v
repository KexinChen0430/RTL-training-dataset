...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  assign blk_we = wb_stb_i && wb_we_i && (ENA_TMR && (wb_adr_i == 3'b111));
  assign wb_ack_o = 
(((state == DELAY) && (cnt_blk_r == 0)) || 
((cnt_blk_r == 0) && !blk_we) || !blk_we) && wb_stb_i;
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (wb_rst_i || 
            (((state == IDLE) && blk_we) || (wb_rst_i && blk_we))) pre_bk_r <= CNT_PRESC+(-1);
        else 
          begin
            pre_bk_r <= pre_bk_r+(-1);
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if (wb_stb_i && (!wb_rst_i && (!blk_we || wb_stb_i)) && 
            (wb_adr_i != 3'b111) && (wb_we_i && (wb_adr_i != 3'b110))) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        if (wb_rst_i) pwm_e_r <= 0;
                 ...
                 (wb_we_i && (wb_adr_i == 3'b110) && (!blk_we || wb_stb_i))) pwm_e_r <= wb_dat_i[5:0];
