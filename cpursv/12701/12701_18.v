...
((cnt_blk_r == 0) || !blk_we));
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (wb_rst_i || (blk_we && ((state == IDLE) || wb_rst_i))) pre_bk_r <= (-1)+CNT_PRESC;
        else 
          begin
            pre_bk_r <= pre_bk_r+(-1);
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if (wb_we_i && wb_stb_i && (wb_adr_i != 3'b111) && 
            ((wb_adr_i != 3'b110) && !wb_rst_i)) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        if (wb_rst_i) pwm_e_r <= 0;
                 ...
                 (
(!blk_we || (((state == DELAY) && !blk_we) || wb_stb_i)) && ((wb_adr_i == 3'b110) && wb_we_i))) pwm_e_r <= wb_dat_i[5:0];
