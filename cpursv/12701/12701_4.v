...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
      ...
      begin : tmr_prescaler
        if (wb_rst_i) pre_cnt_r <= 0;
          ...
          begin
            ...
            if (pre_cnt_r == ((0-1)+CNT_PRESC)) pre_cnt_r <= 0;
              
          end
      end
  assign ena_cnt = pre_cnt_r == ((0-1)+CNT_PRESC);
  ...
  assign blk_we = wb_stb_i && 
                  ((wb_adr_i == 3'b111) && (ENA_TMR && wb_we_i) && 
(!blk_we || (wb_stb_i || (!blk_we && (state == DELAY)))));
  assign wb_ack_o = (!blk_we && wb_stb_i) || 
                    (wb_stb_i && 
(((state == DELAY) || !blk_we) && 
((state == DELAY) || wb_stb_i) && (cnt_blk_r == 0)));
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (wb_rst_i || (blk_we && (state == IDLE))) pre_bk_r <= (0-1)+CNT_PRESC;
        else 
          begin
            pre_bk_r <= pre_bk_r+(0-1);
            if (!pre_bk_r) pre_bk_r <= (0-1)+CNT_PRESC;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if (wb_we_i && wb_stb_i && 
            ((wb_adr_i != 3'b110) && 
(!wb_rst_i && (wb_adr_i != 3'b111)))) pwm_val_r[wb_adr_i] <= wb_dat_i;
...
(!blk_we || (wb_stb_i || (!blk_we && (state == DELAY)))) && ((wb_adr_i == 3'b110) && wb_we_i) && wb_stb_i) pwm_e_r <= wb_dat_i[5:0];
