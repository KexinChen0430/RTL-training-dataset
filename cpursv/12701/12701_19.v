...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  wire ena_cnt;
  reg  [(0-1)+CNT_BITS:0] pre_cnt_r;
  reg  [7:0]  cnt_blk_r = 0;
  wire ena_blk_cnt;
  reg  [(0-1)+CNT_BITS:0] pre_bk_r;
  ...
  always @(posedge wb_clk_i)
      begin : do_cnt_us
        ...
        else if (ena_cnt) cnt_us_r <= 1+cnt_us_r;
  ...
  assign blk_we = ENA_TMR && (wb_adr_i == 3'b111) && wb_we_i && wb_stb_i;
  assign wb_ack_o = wb_stb_i && 
                    (!blk_we || 
(((state == DELAY) && !blk_we) || 
((cnt_blk_r == 0) && (state == DELAY))));
  ...
  always @(posedge wb_clk_i)
      begin : do_bk_cnt
        ...
        else if (ena_blk_cnt) cnt_blk_r <= (0-1)+cnt_blk_r;
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (wb_rst_i || 
            (((state == IDLE) || wb_rst_i) && 
((state == IDLE) && blk_we))) pre_bk_r <= CNT_PRESC+(0-1);
        else 
          begin
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if (wb_we_i && (wb_adr_i != 3'b110) && 
            ((wb_adr_i != 3'b111) && wb_stb_i && !wb_rst_i)) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        ...
        else if ((wb_adr_i == 3'b110) && wb_stb_i && wb_we_i) pwm_e_r <= wb_dat_i[5:0];
