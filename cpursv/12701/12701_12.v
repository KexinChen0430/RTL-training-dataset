...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  always @(posedge wb_clk_i)
      begin : do_cnt_us2
        if ((wb_rst_i && (state == IDLE)) || tc_cnt_us2 || wb_rst_i) cnt_us2_r <= 0;
  ...
  assign blk_we = ENA_TMR && (wb_adr_i == 3'b111) && (wb_stb_i && wb_we_i);
  assign wb_ack_o = 
(!blk_we || 
((cnt_blk_r == 0) || ((state == DELAY) && !blk_we))) && 
                    (
(
((wb_stb_i && (state == DELAY)) || 
((state == DELAY) && !blk_we) || !blk_we) && wb_stb_i) || (cnt_blk_r == 0)) && wb_stb_i && 
                    (!blk_we || ((state == DELAY) && (cnt_blk_r == 0)));
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if ((wb_rst_i || (state == IDLE)) && (blk_we || wb_rst_i)) pre_bk_r <= CNT_PRESC-1;
        else 
          begin
            pre_bk_r <= pre_bk_r+(-1);
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if ((wb_adr_i != 3'b111) && 
            ((wb_adr_i != 3'b110) && !wb_rst_i) && (wb_stb_i && wb_we_i)) pwm_val_r[wb_adr_i] <= wb_dat_i;
