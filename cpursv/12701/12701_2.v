...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  assign blk_we = (wb_adr_i == 3'b111) && ENA_TMR && 
                  (wb_we_i && 
(!blk_we || ((!blk_we && (state == DELAY)) || wb_stb_i)) && wb_stb_i);
  assign wb_ack_o = 
((!blk_we && wb_stb_i) || 
((state == DELAY) && (cnt_blk_r == 0))) && wb_stb_i;
...
(
(((state == IDLE) && blk_we) || 
(wb_rst_i && (state == IDLE))) && ((state == IDLE) || wb_rst_i)) || wb_rst_i) pre_bk_r <= CNT_PRESC+(-1);
        else 
          begin
            pre_bk_r <= (-1)+pre_bk_r;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if ((wb_adr_i != 3'b111) && 
            (wb_stb_i && 
(
(!blk_we || ((!blk_we && (state == DELAY)) || wb_stb_i)) && !wb_rst_i)) && ((wb_adr_i != 3'b110) && wb_we_i)) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        if (wb_rst_i) pwm_e_r <= 0;
                 ...
                 ((wb_adr_i == 3'b110) && 
(!blk_we || ((!blk_we && (state == DELAY)) || wb_stb_i)) && wb_stb_i)) pwm_e_r <= wb_dat_i[5:0];
