...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  assign blk_we = (wb_adr_i == 3'b111) && 
                  (wb_we_i && (wb_stb_i || !blk_we) && wb_stb_i) && ENA_TMR;
  assign wb_ack_o = wb_stb_i && 
                    (((state == DELAY) && !blk_we) || 
(wb_stb_i && 
(((state == DELAY) || !blk_we) && (cnt_blk_r == 0))) || !blk_we);
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (wb_rst_i || 
            (((state == IDLE) && blk_we) || 
(wb_rst_i && (state == IDLE)) || (wb_rst_i && (state == IDLE)))) pre_bk_r <= CNT_PRESC+(-1);
        else 
          begin
            pre_bk_r <= (-1)+pre_bk_r;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if (wb_we_i && (!wb_rst_i && (wb_adr_i != 3'b110)) && 
            (wb_stb_i && (wb_adr_i != 3'b111))) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        ...
        else if ((wb_stb_i || !blk_we) && 
                 (wb_we_i && (wb_adr_i == 3'b110)) && wb_stb_i) pwm_e_r <= wb_dat_i[5:0];
