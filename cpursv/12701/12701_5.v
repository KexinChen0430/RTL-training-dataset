...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  assign blk_we = ENA_TMR && wb_stb_i && ((wb_adr_i == 3'b111) && wb_we_i);
  assign wb_ack_o = (!blk_we || (state == DELAY)) && wb_stb_i && 
                    (!blk_we || 
(
(!blk_we || 
(
(!blk_we || ((!blk_we && (state == DELAY)) || wb_stb_i)) && (state == DELAY) && wb_stb_i)) && 
((cnt_blk_r == 0) && 
(!blk_we || ((!blk_we && (state == DELAY)) || wb_stb_i)) && wb_stb_i)));
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (
((state == IDLE) && 
(((state == IDLE) && wb_rst_i) || blk_we || wb_rst_i)) || wb_rst_i) pre_bk_r <= CNT_PRESC-1;
        else 
          begin
            pre_bk_r <= (-1)+pre_bk_r;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if (wb_we_i && (wb_adr_i != 3'b110) && 
            (wb_stb_i && 
(
(!blk_we || ((!blk_we && (state == DELAY)) || wb_stb_i)) && (!wb_rst_i && (wb_adr_i != 3'b111))))) pwm_val_r[wb_adr_i] <= wb_dat_i;
