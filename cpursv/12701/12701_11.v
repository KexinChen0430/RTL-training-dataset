...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  wire ena_cnt;
  reg  [CNT_BITS+(0-1):0] pre_cnt_r;
  reg  [7:0]  cnt_blk_r = 0;
  wire ena_blk_cnt;
  reg  [CNT_BITS+(0-1):0] pre_bk_r;
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler
        if (wb_rst_i) pre_cnt_r <= 0;
          ...
          begin
            pre_cnt_r <= 1+pre_cnt_r;
            if (pre_cnt_r == (CNT_PRESC+(0-1))) pre_cnt_r <= 0;
              
          end
      end
  assign ena_cnt = pre_cnt_r == (CNT_PRESC+(0-1));
  ...
  always @(posedge wb_clk_i)
      begin : do_cnt_us2
        if (wb_rst_i || tc_cnt_us2) cnt_us2_r <= 0;
        else if (ena_cnt) cnt_us2_r <= cnt_us2_r+1;
  ...
  assign blk_we = wb_stb_i && ((wb_stb_i || !blk_we) && wb_we_i) && 
                  (ENA_TMR && (wb_adr_i == 3'b111));
  assign wb_ack_o = 
(!blk_we || 
((cnt_blk_r == 0) && wb_stb_i && 
(((state == DELAY) || !blk_we) && 
((state == DELAY) || wb_stb_i)))) && wb_stb_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_bk_cnt
        ...
        else if ((state == IDLE) && blk_we) cnt_blk_r <= wb_dat_i;
        else if (ena_blk_cnt) cnt_blk_r <= cnt_blk_r+(0-1);
          
      end
  
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (wb_rst_i || ((state == IDLE) && (wb_rst_i || blk_we))) pre_bk_r <= CNT_PRESC+(0-1);
        else 
          begin
            pre_bk_r <= pre_bk_r+(0-1);
            if (!pre_bk_r) pre_bk_r <= CNT_PRESC+(0-1);
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if ((wb_adr_i != 3'b110) && !wb_rst_i && wb_we_i && 
            ((wb_adr_i != 3'b111) && wb_stb_i)) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        ...
        else if (wb_stb_i && ((wb_adr_i == 3'b110) && wb_we_i)) pwm_e_r <= wb_dat_i[5:0];
