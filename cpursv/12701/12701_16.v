...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
  ...
  always @(posedge wb_clk_i)
      begin : do_cnt_us2
        if (wb_rst_i || 
            (tc_cnt_us2 || ((state == IDLE) && wb_rst_i))) cnt_us2_r <= 0;
...
((!blk_we || (state == DELAY)) && wb_stb_i && !blk_we)) && 
                    (
((!blk_we || (state == DELAY)) && wb_stb_i && !blk_we) || (state == DELAY));
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if ((wb_rst_i || blk_we) && ((state == IDLE) || wb_rst_i)) pre_bk_r <= CNT_PRESC-1;
        else 
          begin
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if ((wb_adr_i != 3'b111) && 
            (!wb_rst_i && (wb_we_i && (wb_adr_i != 3'b110))) && wb_stb_i) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        ...
        else if ((wb_adr_i == 3'b110) && wb_we_i && (!blk_we || wb_stb_i) && 
                 wb_stb_i) pwm_e_r <= wb_dat_i[5:0];
