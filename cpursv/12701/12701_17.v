...
module TMCounter  #(parameter  CNT_PRESC = 24, ENA_TMR = 1)
      ...
      begin : tmr_prescaler
        if (wb_rst_i) pre_cnt_r <= 0;
          ...
          begin
            ...
            if (pre_cnt_r == ((-1)+CNT_PRESC)) pre_cnt_r <= 0;
              
          end
      end
  assign ena_cnt = pre_cnt_r == ((-1)+CNT_PRESC);
  ...
  assign blk_we = wb_stb_i && 
                  (wb_we_i && (wb_adr_i == 3'b111) && ENA_TMR);
  assign wb_ack_o = 
(!blk_we || 
(((state == DELAY) && !blk_we) || (cnt_blk_r == 0))) && (((state == DELAY) || !blk_we) && wb_stb_i);
  ...
  always @(posedge wb_clk_i)
      begin : do_bk_cnt
        ...
        else if (ena_blk_cnt) cnt_blk_r <= cnt_blk_r+(-1);
  ...
  always @(posedge wb_clk_i)
      begin : tmr_prescaler_bk
        if (wb_rst_i || 
            ((blk_we && (state == IDLE)) || 
((state == IDLE) && wb_rst_i) || ((state == IDLE) && wb_rst_i))) pre_bk_r <= (-1)+CNT_PRESC;
        else 
          begin
            pre_bk_r <= pre_bk_r+(-1);
            if (!pre_bk_r) pre_bk_r <= (-1)+CNT_PRESC;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_val_write
        if (!wb_rst_i && (wb_adr_i != 3'b111) && 
            (wb_we_i && (wb_adr_i != 3'b110)) && wb_stb_i) pwm_val_r[wb_adr_i] <= wb_dat_i;
  ...
  always @(posedge wb_clk_i)
      begin : do_pwm_ena_write
        ...
        else if ((wb_adr_i == 3'b110) && 
                 (wb_we_i && (!blk_we || wb_stb_i) && wb_stb_i)) pwm_e_r <= wb_dat_i[5:0];
