...
module sonic_sensor(input  clk,
      ...
      begin
        if (rst) counter <= 0;
          ...
          case (state)
            ...
            STATE_OUT_SIG: counter <= 1+counter;

            STATE_WAIT750: counter <= 1+counter;

            STATE_IN_SIG: counter <= 1+counter;

            STATE_WAIT200: counter <= 1+counter;
  ...
  always @(posedge clk)
      begin
        if (rst) echo <= 0;
        else if (state == STATE_IN_SIG) 
          begin
            echo <= 1+echo;
          end
        else if (state == STATE_PROCESS_END) echo <= 0;
  ...
  always @(posedge clk)
      begin
        if (rst) result <= 0;
        else if (state == STATE_PROCESS_END) result <= echo;
