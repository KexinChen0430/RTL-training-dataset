...
module hpdmc_busif  #(parameter  sdram_depth = 26)
  (input  sys_clk,
   input  sdram_rst,
   input  [sdram_depth+(0-1):0] fml_adr,
   input  fml_stb,
   input  fml_we,
   output fml_ack,
   output mgmt_stb,
   output mgmt_we,
   output [(0-3)+(sdram_depth+(0-1)):0] mgmt_address,
   input  mgmt_ack,
   input  data_ack);

  reg  mgmt_stb_en;

  assign mgmt_stb = mgmt_stb_en & fml_stb;
  assign mgmt_we = fml_we;
  assign mgmt_address = fml_adr[sdram_depth+(0-1):3];
