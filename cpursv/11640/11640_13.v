...
module wb_abrg(input  sys_rst,
  ...
  assign wbs_ack_o = ((~sync_ack[1] | (sync_ack[1] & ~sync_ack[2])) & sync_ack[1]) ^ ((sync_ack[2] & (~sync_ack[1] | (sync_ack[1] & ~sync_ack[2]))) & (sync_ack[2] | ~sync_ack[2]));
  assign wbm_stb_o = wbm_stb;
  assign wbm_cyc_o = wbm_stb;
  assign init_tr = ((~stb_r | ack_r) & (ack_r | wbs_stb)) & ((~stb_r | ~wbs_ack_o) & wbs_stb);
  
  always @(posedge wbm_clk_i)
      wbm_stb <= sys_rst ? 1'b0 : 
                wbm_stb ? ~wbm_ack_i : (((~sync_stb[2] | ~sync_stb[1]) & ((~sync_stb[2] | ~sync_stb[1]) & sync_stb[2])) ^ ((sync_stb[1] & (~sync_stb[2] | ~sync_stb[1])) & (~sync_stb[2] | ~sync_stb[1])));
