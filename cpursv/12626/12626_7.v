...
module ws2812  #(parameter LED_COUNT,REVERSE)
  ...
  wire 
       symbol_end = symbol_phase == (symbol_duration+(-1));
      ...
      begin
        if (rst_i) 
          ...
          begin
            ...
            else symbol_phase <= 1+symbol_phase;
          end
      end
        ...
        if (rst_i) symbol_count <= 0;
          ...
          begin
            if (symbol_end) 
              ...
              else if (state == STATE_PIXEL) symbol_count <= 1+symbol_count;
  ...
  wire 
       frame_end = (pixel_count == ((-1)+LED_COUNT)) && pixel_end;

  assign symbol = (state == STATE_PIXEL) ? led_data[23+(0-symbol_count)] : SYMBOL_RESET;
  ...
  assign address_o = REVERSE ? pixel_count : ((0-pixel_count)+((-1)+LED_COUNT));
endmodule
