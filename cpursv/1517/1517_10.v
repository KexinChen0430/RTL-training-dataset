...
module timing_gen  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  ...
  assign hsync = (pixel_count >= hsync_start) && 
                 (pixel_count <= hsync_end);
  assign vsync = (line_count <= vsync_end) && 
                 (line_count >= vsync_start);
  assign hblank = !(pixel_count <= (active_pixels-1));
  assign vblank = !(line_count <= ((0-1)+active_lines));
  assign de = (line_count <= ((0-1)+active_lines)) && 
              (pixel_count <= (active_pixels-1));
