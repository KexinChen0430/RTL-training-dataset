...
module hardcopyiv_lcell_hsadder(dataa,datab,cin,sumout,cout);
  ...
  parameter 
       sumout_width = (dataa_width >= datab_width) ? (1+dataa_width) : (datab_width+1);
  input  [(0-1)+dataa_width:0] dataa;
  input  [(0-1)+datab_width:0] datab;
  input  cin;
  output [(0-1)+sumout_width:0] sumout;
  output cout;
  wire cin_sel;
  ...
  always @(datab or dataa or cin_sel)
      begin
        sumout_tmp = (dataa+cin_sel)+datab;
      end
  assign sumout = sumout_tmp[(0-1)+sumout_width:0];
