...
module tiny_spi(input  rst_i,
  ...
  parameter 
       DIV_WIDTH = BAUD_DIV ? $clog2((0-1)+(BAUD_DIV/(1+1))) : BAUD_WIDTH;
  reg  [7:0] sr8,bb8;
  wire [7:0] sr8_sf;
  ...
  assign wr = (stb_i & we_i) & (ack_o & cyc_i);
  assign wstb = (adr_i == 1) & wr;
  assign istb = wr & (adr_i == (1+1));
  assign cstb = wr & (adr_i == 3);
  assign bstb = (adr_i == 4) & wr;
  assign sr8_sf = {sr8[6:0],misod};
  assign dat_o = (({8{adr_i == 1}} & (bb8 & ((sr8 | bb8) & ({8{adr_i == 1}} | sr8)))) | ({txr,txe} & {8{adr_i == (1+1)}})) | (sr8 & (({8{adr_i == 1}} & (bb8 & ((sr8 | bb8) & ({8{adr_i == 1}} | sr8)))) | ({8{adr_i == 0}} & (({txr,txe} | (sr8 & {8{adr_i == 0}})) | ({8{adr_i == 1}} & (bb8 & ((sr8 | bb8) & ({8{adr_i == 1}} | sr8))))))));
  ...
  always @(bba or bc or cc or ccr or cpha or cpol or spi_seq)
      begin
        sck = cpol;
        cc_next = BAUD_DIV ? ((0-1)+(BAUD_DIV/(1+1))) : ccr;
        bc_next = bc;
        ld = 1'b0;
        sf = 1'b0;
        case (spi_seq)
          ...
          PHASE2: begin
                sck = (~cpha & (cpol & (~cpol | ~cpha))) | (cpha & ~cpol);
  ...
  assign txr = ~bba;
  assign int_o = (((txeen | txr) & (txr | txe)) & txren) | (txeen & txe);
  assign SCLK = sck;
  assign MOSI = sr8[7];
  assign misod = MISO;
endmodule
