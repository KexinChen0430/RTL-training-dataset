...
module tiny_spi(input  rst_i,
  ...
  assign wr = (stb_i & ack_o) & (we_i & cyc_i);
  assign wstb = (adr_i == 1) & wr;
  assign istb = wr & (adr_i == 1<<<1);
  assign cstb = (adr_i == 3) & wr;
  assign bstb = (adr_i == 4) & wr;
  assign sr8_sf = {sr8[6:0],misod};
  assign dat_o = (({8{adr_i == 1<<<1}} | {8{adr_i == 1}}) | ({8{adr_i == 0}} & ({8{adr_i == 1}} | sr8))) & ((({8{adr_i == 0}} & sr8) | ({8{adr_i == 1<<<1}} | bb8)) & (({8{adr_i == 0}} & sr8) | ((bb8 & {8{adr_i == 1}}) | {txr,txe})));
  ...
  always @(bba or bc or cc or ccr or cpha or cpol or spi_seq)
      begin
        ...
        case (spi_seq)
          ...
          PHASE2: begin
                sck = (~cpha & cpol) | (((~cpha | cpha) & (cpha | cpol)) & ~cpol);
  ...
  assign txr = ~bba;
  assign int_o = (((txe | txren) & txeen) & (txe | txr)) | (txr & txren);
  assign SCLK = sck;
  assign MOSI = sr8[7];
  assign misod = MISO;
endmodule
