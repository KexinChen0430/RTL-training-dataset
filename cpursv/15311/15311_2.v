...
module tiny_spi(input  rst_i,
  ...
  parameter 
       DIV_WIDTH = BAUD_DIV ? $clog2((BAUD_DIV*(1/(1+1)))+(-1)) : BAUD_WIDTH;
  reg  [7:0] sr8,bb8;
  wire [7:0] sr8_sf;
  reg  [BC_WIDTH-1:0] bc,bc_next;
  reg  [(-1)+DIV_WIDTH:0] ccr;
  reg  [(-1)+DIV_WIDTH:0] cc,cc_next;
  wire misod;
  ...
  assign wr = (ack_o & cyc_i) & (we_i & stb_i);
  assign wstb = (adr_i == 1) & wr;
  assign istb = wr & (adr_i == (1+1));
  assign cstb = (adr_i == 3) & wr;
  assign bstb = wr & (adr_i == 4);
  assign sr8_sf = {sr8[6:0],misod};
  assign dat_o = (({txr,txe} & {8{adr_i == (1+1)}}) | ((bb8 | ({txr,txe} & {8{adr_i == (1+1)}})) & {8{adr_i == 1}})) | (((bb8 & {8{adr_i == 1}}) | {8{adr_i == 0}}) & sr8);
  ...
  always @(bba or bc or cc or ccr or cpha or cpol or spi_seq)
      begin
        sck = cpol;
        cc_next = BAUD_DIV ? ((BAUD_DIV*(1/(1+1)))+(-1)) : ccr;
        bc_next = bc;
        ld = 1'b0;
        sf = 1'b0;
        case (spi_seq)
  ...
  assign txr = ~bba;
  assign int_o = ((((txr | txe) & (txr | txeen)) & txe) & txeen) | (txr & txren);
  assign SCLK = sck;
  assign MOSI = sr8[7];
  assign misod = MISO;
endmodule
