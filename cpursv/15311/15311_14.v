...
module tiny_spi(input  rst_i,
  ...
  assign wr = (cyc_i & stb_i) & (ack_o & we_i);
  assign wstb = wr & (adr_i == 1);
  assign istb = wr & (adr_i == 1<<1);
  assign cstb = wr & (adr_i == 3);
  assign bstb = wr & (adr_i == 4);
  assign sr8_sf = {sr8[6:0],misod};
  assign dat_o = (((sr8 & {8{adr_i == 0}}) | ((bb8 & (({8{adr_i == 1}} | {8{adr_i == 0}}) & ({8{adr_i == 1}} | sr8))) | {8{adr_i == 1<<1}})) & ({txr,txe} | (sr8 & {8{adr_i == 0}}))) | (bb8 & {8{adr_i == 1}});
  ...
  assign txr = ~bba;
  assign int_o = ((txeen & (txren | txe)) & (txe | txr)) | (txren & txr);
  assign SCLK = sck;
  assign MOSI = sr8[7];
  assign misod = MISO;
endmodule
