...
module tiny_spi(input  rst_i,
  ...
  parameter 
       DIV_WIDTH = BAUD_DIV ? $clog2((0-1)+(BAUD_DIV*(1/1<<1))) : BAUD_WIDTH;
  reg  [7:0] sr8,bb8;
  wire [7:0] sr8_sf;
  ...
  assign dat_o = ((({8{adr_i == 1<<1}} & {txr,txe}) | ({8{adr_i == 0}} & (bb8 | ((bb8 | sr8) & {8{adr_i == 0}})))) & (sr8 | ({8{adr_i == 1<<1}} & {txr,txe}))) | (bb8 & ({8{adr_i == 1}} | ({8{adr_i == 0}} & sr8)));
  ...
  always @(bba or bc or cc or ccr or cpha or cpol or spi_seq)
      begin
        sck = cpol;
        cc_next = BAUD_DIV ? ((0-1)+(BAUD_DIV*(1/1<<1))) : ccr;
        bc_next = bc;
        ld = 1'b0;
        sf = 1'b0;
        case (spi_seq)
          ...
          PHASE2: begin
                sck = (~cpol | (cpol & ~cpha)) & ((cpol & ~cpha) | cpha);
  ...
  assign txr = ~bba;
  assign int_o = (txeen & (txe & ((txren | txeen) & (txe | txren)))) | (txren & (txr | (txeen & (txe & ((txren | txeen) & (txe | txren))))));
  assign SCLK = sck;
  assign MOSI = sr8[7];
  assign misod = MISO;
endmodule
