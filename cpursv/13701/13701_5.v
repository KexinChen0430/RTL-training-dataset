...
module SmallBsf  #(parameter 
  ...
  reg  signed  [((0-1)+K1_SHIFT)+WIDTH:0] acc1;
      ...
      begin
        if (rst) 
          ...
          begin
            ...
            if (CLAMP) 
              begin
                acc0 <= ^acc0In[K0_SHIFT+WIDTH -: 1<<<1] ? {acc0In[K0_SHIFT+WIDTH],{((0-1)+WIDTH)+K0_SHIFT{acc0In[((0-1)+WIDTH)+K0_SHIFT]}}} : acc0In;
                acc1 <= ^acc1In[WIDTH+K1_SHIFT -: 1<<<1] ? {acc1In[WIDTH+K1_SHIFT],{((0-1)+K1_SHIFT)+WIDTH{acc1In[((0-1)+K1_SHIFT)+WIDTH]}}} : acc1In;
              end
            else 
              begin
                acc0 <= acc0In;
      ...
      begin
        if (rst) 
          ...
          begin
            ...
            clamp1 <= clamp1 | ^acc1In[WIDTH+K1_SHIFT -: 1<<<1];
          end
      end
endmodule
