      ...
      begin
        if (rst) 
          ...
          begin
            ...
            if (CLAMP) 
              begin
                acc0 <= ^acc0In[WIDTH+K0_SHIFT -: 1+1] ? {acc0In[WIDTH+K0_SHIFT],{(WIDTH+K0_SHIFT)+(-1){acc0In[(WIDTH+K0_SHIFT)+(-1)]}}} : acc0In;
                acc1 <= ^acc1In[WIDTH+K1_SHIFT -: 1+1] ? {acc1In[WIDTH+K1_SHIFT],{(WIDTH+K1_SHIFT)-1{acc1In[(WIDTH+K1_SHIFT)-1]}}} : acc1In;
              end
            else 
              begin
                acc0 <= acc0In;
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            clamp0 <= clamp0 | ^acc0In[WIDTH+K0_SHIFT -: 1+1];
            clamp1 <= ^acc1In[WIDTH+K1_SHIFT -: 1+1] | clamp1;
          end
      end
endmodule
