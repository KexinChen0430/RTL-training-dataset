...
module SmallBsf  #(parameter 
  ...
  assign acc0In = ((0-acc1Out)+forwardPath)+acc0;
          ...
          begin
            ...
            if (CLAMP) 
              begin
                ...
                acc1 <= ^acc1In[WIDTH+K1_SHIFT -: 2] ? {acc1In[WIDTH+K1_SHIFT],{(WIDTH+K1_SHIFT)+(0-1){acc1In[(WIDTH+K1_SHIFT)+(0-1)]}}} : acc1In;
              end
            else 
              begin
                acc0 <= acc0In;
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            clamp0 <= ^acc0In[WIDTH+K0_SHIFT -: 2] | clamp0;
