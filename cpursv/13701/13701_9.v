...
module SmallBsf  #(parameter 
  ...
  assign acc0In = acc0+(forwardPath+(0-acc1Out));
      ...
      begin
        if (rst) 
          ...
          begin
            ...
            if (CLAMP) 
              begin
                acc0 <= ^acc0In[K0_SHIFT+WIDTH -: 2] ? {acc0In[K0_SHIFT+WIDTH],{(-1)+(K0_SHIFT+WIDTH){acc0In[(-1)+(K0_SHIFT+WIDTH)]}}} : acc0In;
                acc1 <= ^acc1In[WIDTH+K1_SHIFT -: 2] ? {acc1In[WIDTH+K1_SHIFT],{(WIDTH+K1_SHIFT)-1{acc1In[(WIDTH+K1_SHIFT)-1]}}} : acc1In;
              end
            else 
              begin
                acc0 <= acc0In;
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            clamp0 <= clamp0 | ^acc0In[K0_SHIFT+WIDTH -: 2];
            clamp1 <= ^acc1In[WIDTH+K1_SHIFT -: 2] | clamp1;
          end
      end
endmodule
