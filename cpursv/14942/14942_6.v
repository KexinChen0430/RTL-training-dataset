...
module axi_conduit_merger  #(parameter 
  ...
  (output m_awvalid,
   ...
   input  [1:0] m_bresp,
   output m_bready,
   output [(-1)+ADDRESS_WIDTH:0] m_awaddr,
   output [ID_WIDTH-1:0] m_awid,
   output [(-1)+ADDRESS_WIDTH:0] m_araddr,
   output [ID_WIDTH-1:0] m_arid,
   input  [(-1)+DATA_WIDTH:0] m_rdata,
   input  [ID_WIDTH-1:0] m_rid,
   output [(-1)+DATA_WIDTH:0] m_wdata,
   ...
   output [1:0] s_bresp,
   input  s_bready,
   input  [(-1)+ADDRESS_WIDTH:0] s_awaddr,
   input  [ID_WIDTH-1:0] s_awid,
   input  [(-1)+ADDRESS_WIDTH:0] s_araddr,
   input  [ID_WIDTH-1:0] s_arid,
   output [(-1)+DATA_WIDTH:0] s_rdata,
   output [ID_WIDTH-1:0] s_rid,
   input  [(-1)+DATA_WIDTH:0] s_wdata,
