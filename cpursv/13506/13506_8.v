...
module grid(input  clk,
        ...
        if (en == 1) 
          begin
            for (i = 0; i < 4; i = i+5'd1)
                begin
                  if (col != 15) grid[i+row] <= grid[i+row] | block_row[i]>>col;
                  else grid[i+row] <= grid[i+row] | block_row[i]<<1;
                end
          end
            ...
            if (grid[count+row] == 14'b11111111111111) 
              begin
                ...
                for (i = 5'd20; i > 0; i = i-5'd1)
                    if ((i+(0-1)) <= (count+row)) 
                      if ((i+(0-1)) > 0) grid[i-5'd1] <= grid[i-5'd2];
                      else grid[i-5'd1] <= 0;
  ...
  wire [0:13] 
       row_overlay = (col != 15) ? block_row[(0-row)+addr_row_vga]>>col : (block_row[(0-row)+addr_row_vga]*(1+1));

  
  always @(posedge gfx_clk)
      begin
        if ((addr_row_vga >= row) && (addr_row_vga < (4+row))) d_vga <= row_overlay | grid[addr_row_vga];
