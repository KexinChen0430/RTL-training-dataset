...
module grid(input  clk,
          ...
          begin
            for (i = 0; i < 4; i = i+5'd1)
                begin
                  ...
                  else grid[row+i] <= grid[row+i] | (block_row[i]*2);
                end
          end
            ...
            if (grid[row+count] == 14'b11111111111111) 
              begin
                ...
                for (i = 5'd20; i > 0; i = i-5'd1)
                    if ((i-1) <= (row+count)) 
                      if ((i-1) > 0) grid[i-5'd1] <= grid[(-5'd2)+i];
                      else grid[i-5'd1] <= 0;
  ...
  wire [0:13] 
       row_overlay = (col != 15) ? block_row[(0-row)+addr_row_vga]>>col : block_row[(0-row)+addr_row_vga]<<<1;
