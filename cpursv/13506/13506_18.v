...
module grid(input  clk,
          ...
          begin
            if (grid[row+count] == 14'b11111111111111) 
              begin
                ...
                for (i = 5'd20; i > 0; i = i+(0-5'd1))
                    if (((0-1)+i) <= (row+count)) 
                      if (((0-1)+i) > 0) grid[i+(0-5'd1)] <= grid[(0-5'd2)+i];
                      else grid[i+(0-5'd1)] <= 0;
  ...
  wire [0:13] 
       row_overlay = (col != 15) ? block_row[addr_row_vga-row]>>col : (1<<<1*block_row[addr_row_vga-row]);
