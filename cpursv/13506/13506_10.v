...
module grid(input  clk,
          ...
          begin
            for (i = 0; i < 4; i = i+5'd1)
                begin
                  ...
                  else grid[row+i] <= grid[row+i] | block_row[i]<<<1;
                end
          end
          ...
          begin
            if (grid[count+row] == 14'b11111111111111) 
              begin
                ...
                for (i = 5'd20; i > 0; i = (0-5'd1)+i)
                    if ((i+(-1)) <= (count+row)) 
                      if ((i+(-1)) > 0) grid[(0-5'd1)+i] <= grid[(-5'd2)+i];
                      else grid[(0-5'd1)+i] <= 0;
  ...
  wire [0:13] 
       row_overlay = (col != 15) ? block_row[addr_row_vga-row]>>col : (block_row[addr_row_vga-row]*(1+1));

  
  always @(posedge gfx_clk)
      begin
        if ((addr_row_vga >= row) && (addr_row_vga < (4+row))) d_vga <= grid[addr_row_vga] | row_overlay;
