...
module grid(input  clk,
          ...
          begin
            for (i = 0; i < 4; i = 5'd1+i)
                begin
                  ...
                  else grid[i+row] <= grid[i+row] | (1<<1*block_row[i]);
                end
          end
  ...
  wire [0:13] 
       row_overlay = (col != 15) ? block_row[addr_row_vga+(0-row)]>>col : (block_row[addr_row_vga+(0-row)]*1<<1);

  
  always @(posedge gfx_clk)
      begin
        if ((addr_row_vga < (row+4)) && (addr_row_vga >= row)) d_vga <= row_overlay | grid[addr_row_vga];
