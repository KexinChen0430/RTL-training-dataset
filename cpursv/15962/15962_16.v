...
module alt_vipvfr131_common_avalon_mm_slave  #(parameter 
  ...
  (input  wire rst,
   ...
   input  wire av_read,
   output reg [(-1)+AV_DATA_WIDTH:0] av_readdata,
   input  wire av_write,
   input  wire [(-1)+AV_DATA_WIDTH:0] av_writedata,
   output av_irq,
   output reg enable,
   input  clear_enable,
  ...
  reg  [(-1)+AV_DATA_WIDTH:0] internal_registers[NO_REGISTERS+(-1):0];
  reg  [(-1)+AV_DATA_WIDTH:0] interrupt_register;
            ...
            if (av_read) 
              begin
                case (av_address)
                  ...
                  1: av_readdata <= {{(-1)+AV_DATA_WIDTH{1'b0}},global_stopped};

                  1<<1: av_readdata <= {{(AV_DATA_WIDTH-NO_INTERRUPTS)-1{1'b0}},interrupt_register[NO_REGISTERS:1],1'b0};
