...
module alt_vipvfr131_common_avalon_mm_slave  #(parameter 
  ...
  (input  wire rst,
   ...
   output wire [(-1)+(AV_DATA_WIDTH*NO_REGISTERS):0] registers,
   input  wire [(-1)+(AV_DATA_WIDTH*NO_REGISTERS):0] registers_in,
            ...
            if (av_read) 
              begin
                case (av_address)
                  ...
                  0: av_readdata <= {{(AV_DATA_WIDTH+(0-NO_INTERRUPTS))-1{1'b0}},interrupt_enables,enable};

                  1: av_readdata <= {{AV_DATA_WIDTH-1{1'b0}},global_stopped};

                  1+1: av_readdata <= {{(AV_DATA_WIDTH+(0-NO_INTERRUPTS))-1{1'b0}},interrupt_register[NO_REGISTERS:1],1'b0};
