...
module alt_mem_ddrx_mm_st_converter  #(parameter 
  ...
  input  [(-1)+((1/8)*AVL_DATA_WIDTH):0] avl_be;
  ...
  output [(-1)+((1/8)*AVL_DATA_WIDTH):0] itf_wr_data_byte_en;
  ...
  always @(posedge ctl_clk or negedge ctl_reset_n)
      begin
        if (!ctl_reset_n) burst_counter <= 0;
          ...
          begin
            if ((avl_size > 1) && itf_cmd_ready && itf_wr_if_ready) burst_counter <= (-1)+avl_size;
