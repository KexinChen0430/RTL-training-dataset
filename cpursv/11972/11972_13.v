...
module motor_speed_measurement(input  rsi_MRST_reset,
                               input  csi_MCLK_clk,
  ...
  always @(posedge csi_PWMCLK_clk)
      tempgate <= gate_frequence+tempgate;
  ...
  always @(posedge frequent)  temp <= gate;
  assign f1 = temp & frequent;
  assign f2 = temp & csi_PWMCLK_clk;
  reg  [7:0] state;
  ...
  always @(state or temp)
      case (state)
        ...
        1: if (temp == 0) nextstate = 2;
            else nextstate = 1;

        2: if (temp == 1) nextstate = 3;
            else nextstate = 2;

        3: if (temp == 0) nextstate = 4;
            else nextstate = 3;
