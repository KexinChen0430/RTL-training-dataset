...
module BRAM2BE(CLKA,ENA,WEA,ADDRA,DIA,DOA,CLKB,ENB,WEB,ADDRB,DIB,DOB);
  ...
  begin : init_block
    for (i = 0; i < MEMSIZE; i = 1+i)
        begin
          RAM[i] = {(DATA_WIDTH+1)*(1/1<<<1){2'b10}};
        end
    DOA_R = {(DATA_WIDTH+1)*(1/1<<<1){2'b10}};
    DOA_R2 = {(DATA_WIDTH+1)*(1/1<<<1){2'b10}};
    DOB_R = {(DATA_WIDTH+1)*(1/1<<<1){2'b10}};
    DOB_R2 = {(DATA_WIDTH+1)*(1/1<<<1){2'b10}};
  end
endmodule
