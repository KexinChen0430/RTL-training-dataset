...
module fric_client_master(clk,rst,fric_in,fric_out,ctyp,port,addr,wdat,tstb,trdy,rstb,
                          rdat);
  ...
  always @(ctyp or fis_ack or fos_state or tstb)
      begin
        ...
        case (fos_state)
          ...
          fos_idle: begin
                if ((ctyp == 4'b0010) && (tstb == 1'b1)) 
                  begin
                    fric_out_sel = 1;
                    next_fos_state = fos_wr_adr;
                  end
                else if ((tstb == 1'b1) && (ctyp == 4'b0011)) 
                  begin
                    fric_out_sel = 1;
  ...
  always @(fis_state or fric_inr)
      begin
        ...
        case (fis_state)
          ...
          fis_idle: begin
                if (fric_inr[7:4] == 4'b0100) next_fis_state = fis_wr_ack;
                else if (fric_inr[7:4] == 4'b0101) next_fis_state = fis_rd_ack;
      ...
      begin
        if (rst == 1'b1) 
          ...
          begin
            ...
            fric_inr <= fric_in;
            if (rdat_cap_high == 1'b1) rdat[15:8] <= fric_inr;
              
            if (rdat_cap_low == 1'b1) rdat[7:0] <= fric_inr;
