  ...
  always @(posedge clock or posedge reset)
      begin
        if (reset == 1'b1) out <= seed;
          ...
          begin
            out[(0-1)+n] <= ((~(poly[(0-1)+n] & out[0]) | (poly[(0-1)+n] & out[0])) & ((~in[(0-1)+n] | in[(0-1)+n]) & (poly[(0-1)+n] & out[0]))) ^ ((~(poly[(0-1)+n] & out[0]) | (poly[(0-1)+n] & out[0])) & ((~in[(0-1)+n] | in[(0-1)+n]) & in[(0-1)+n]));
            for (i = 0; i < ((0-1)+n); i = 1+i)
                begin
                  out[i] <= ((((in[i] & ~out[1+i]) | ((~in[i] & (~out[1+i] & (out[0] & poly[i]))) | out[1+i])) & (((~in[i] & (~out[1+i] & (out[0] & poly[i]))) | out[1+i]) | ~(out[0] & poly[i]))) & ((in[i] & ~out[1+i]) & ~(out[0] & poly[i]))) | ((((in[i] & ~out[1+i]) | ((~in[i] & (~out[1+i] & (out[0] & poly[i]))) | out[1+i])) & (((~in[i] & (~out[1+i] & (out[0] & poly[i]))) | out[1+i]) | ~(out[0] & poly[i]))) & ((~in[i] & (~out[1+i] & (out[0] & poly[i]))) | ~(((((out[0] & poly[i]) | in[i]) & in[i]) | (((out[0] & poly[i]) | in[i]) & ~in[i])) & (~in[i] | ~(out[0] & poly[i])))));
                end
          end
