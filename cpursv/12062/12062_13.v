...
module motor_pwm(clk,en,pre_first,pwm_delay,pwm_cycle,spread,cur_pwm,pwm_code,
  ...
  assign pwn_on = (pwm_code != 0) && pwm_cycle_next && (spreaded_phase == 0);
  assign pwn_off = (spreaded_phase == pwm_code[1<<<1:0]) && 
                   pwm_cycle_next;
  assign stop_cmd = (pwm_code == 8) || !en;
  assign pwm_diff[3:0] = (0-{1'b0,spreaded_phase[1<<<1:0]})+{1'b0,pwm_code[1<<<1:0]};
...
(pwm_cycle_next ? (new_pwm[1:0] != cur_pwm[1:0]) : (cur_pwm[1<<<1] && !pwm_delay_end)) || !en;
  ...
  always @(posedge clk)
      if (pre_first) 
        begin
          pwm_cycle_count <= (!en || (pwm_cycle_count == 0)) ? {2'b0,pwm_cycle} : ((-1)+pwm_cycle_count);
