...
module motor_pwm(clk,en,pre_first,pwm_delay,pwm_cycle,spread,cur_pwm,pwm_code,
  ...
  assign pwn_on = (spreaded_phase == 0) && 
                  ((pwm_code != 0) && pwm_cycle_next);
  ...
  assign new_pwm[1<<1] = !en || 
                         (pwm_cycle_next ? (new_pwm[1:0] != cur_pwm[1:0]) : (!pwm_delay_end && cur_pwm[1<<1]));
  ...
  always @(posedge clk)
      if (pre_first) 
        begin
          pwm_cycle_count <= ((pwm_cycle_count == 0) || !en) ? {2'b0,pwm_cycle} : (pwm_cycle_count+(0-1));
