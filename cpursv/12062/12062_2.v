...
module motor_pwm(clk,en,pre_first,pwm_delay,pwm_cycle,spread,cur_pwm,pwm_code,
  ...
  assign new_pwm[1] = !stop_cmd && 
                      (pwm_cycle_next ? ((pwm_code[2:0] != 0) && !pwm_diff[3]) : cur_pwm[1]);
  assign new_pwm[2] = 
(pwm_cycle_next ? (new_pwm[1:0] != cur_pwm[1:0]) : (cur_pwm[2] && !pwm_delay_end)) || !en;
      ...
      if (pre_first) 
        begin
          ...
          pwm_delay_count <= (!en || pwm_cycle_next) ? {1'b0,pwm_delay} : 
                            (pwm_delay_count == 0) ? 4'b0 : ((0-1)+pwm_delay_count);
