...
module motor_pwm(clk,en,pre_first,pwm_delay,pwm_cycle,spread,cur_pwm,pwm_code,
  ...
  assign pwn_on = (spreaded_phase == 0) && (pwm_code != 0) && pwm_cycle_next;
  ...
  assign new_pwm[1] = 
(pwm_cycle_next ? (!pwm_diff[3] && (pwm_code[1<<1:0] != 0)) : cur_pwm[1]) && !stop_cmd;
  ...
  always @(posedge clk)
      if (pre_first) 
        begin
          pwm_cycle_count <= (!en || (pwm_cycle_count == 0)) ? {2'b0,pwm_cycle} : ((0-1)+pwm_cycle_count);
