...
module SLEEP_CONTROLv4(output reg MBC_ISOLATE,
  ...
  always @* 
      begin
        if ((((RESETn | ~MBUS_DIN) & (WAKEUP_REQ_ORED | ~MBUS_DIN)) & (RESETn | WAKEUP_REQ_ORED)) & (RESETn & (((MBC_SLEEP_int | RESETn) & ((~SLEEP_REQ | MBC_SLEEP_int) | (WAKEUP_REQ_ORED | (MBC_SLEEP_int & ~MBUS_DIN)))) & (WAKEUP_REQ_ORED | MBC_SLEEP_int)))) set_tran_to_wake = 1'b1;
        else set_tran_to_wake = 1'b0;
      end
  
  always @* 
      begin
        if ((((((~RESETn | ~SLEEP_REQ) | (~SLEEP_REQ | MBC_SLEEP_int)) & ~MBUS_DIN) | ~SLEEP_REQ) | ~RESETn) | WAKEUP_REQ_ORED) rst_tran_to_wake <= 1'b1;
