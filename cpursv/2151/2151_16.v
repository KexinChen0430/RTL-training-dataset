...
module SLEEP_CONTROLv4(output reg MBC_ISOLATE,
  ...
  always @* 
      begin
        if ((RESETn & ((RESETn & MBC_SLEEP_int) | WAKEUP_REQ_ORED)) & (((RESETn & MBC_SLEEP_int) | WAKEUP_REQ_ORED) & ((~MBUS_DIN | RESETn) & (~MBUS_DIN | WAKEUP_REQ_ORED)))) set_tran_to_wake = 1'b1;
        else set_tran_to_wake = 1'b0;
      end
  
  always @* 
      begin
        if (((~MBUS_DIN & (WAKEUP_REQ_ORED | MBC_SLEEP_int)) | ~SLEEP_REQ) | (~RESETn | WAKEUP_REQ_ORED)) rst_tran_to_wake <= 1'b1;
        else rst_tran_to_wake <= 1'b0;
      end
  wire 
       tran_to_wake_r = RESETn & (rst_tran_to_wake & (((WAKEUP_REQ_ORED | MBC_SLEEP_int) & (((RESETn & ~MBUS_DIN) | ((~MBUS_DIN & (WAKEUP_REQ_ORED | MBC_SLEEP_int)) & WAKEUP_REQ_ORED)) | WAKEUP_REQ_ORED)) | RESETn));
  ...
  assign MBC_SLEEP = MBC_SLEEP_int & ~(((~MBUS_DIN & (WAKEUP_REQ_ORED | MBC_SLEEP_int)) & MBC_SLEEP_int) | WAKEUP_REQ_ORED);
