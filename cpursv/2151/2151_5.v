...
module SLEEP_CONTROLv4(output reg MBC_ISOLATE,
  ...
  always @* 
      begin
        if ((((~MBUS_DIN | WAKEUP_REQ_ORED) & (RESETn | WAKEUP_REQ_ORED)) & (WAKEUP_REQ_ORED | (RESETn & MBC_SLEEP_int))) & (((((RESETn | WAKEUP_REQ_ORED) & (WAKEUP_REQ_ORED | MBC_SLEEP_int)) & (RESETn | MBC_SLEEP_int)) & (RESETn | WAKEUP_REQ_ORED)) & RESETn)) set_tran_to_wake = 1'b1;
        else set_tran_to_wake = 1'b0;
      end
  
  always @* 
      begin
        if ((((~SLEEP_REQ | (MBC_SLEEP_int & ~RESETn)) | ~RESETn) | (WAKEUP_REQ_ORED & ~MBUS_DIN)) | (((((~MBUS_DIN | WAKEUP_REQ_ORED) | ((WAKEUP_REQ_ORED & MBC_SLEEP_int) | ((MBC_SLEEP_int & ~MBUS_DIN) | ~SLEEP_REQ))) & MBC_SLEEP_int) | (WAKEUP_REQ_ORED & MBC_SLEEP_int)) | WAKEUP_REQ_ORED)) rst_tran_to_wake <= 1'b1;
  ...
  assign MBC_SLEEP = MBC_SLEEP_int & ~(((WAKEUP_REQ_ORED & MBC_SLEEP_int) | (MBC_SLEEP_int & (~MBUS_DIN | WAKEUP_REQ_ORED))) | WAKEUP_REQ_ORED);
