...
module SLEEP_CONTROLv4(output reg MBC_ISOLATE,
  ...
  always @* 
      begin
        if (((MBC_SLEEP_int | (WAKEUP_REQ_ORED | (~MBUS_DIN & MBC_SLEEP_int))) & RESETn) & ((RESETn & ~MBUS_DIN) | WAKEUP_REQ_ORED)) set_tran_to_wake = 1'b1;
        else set_tran_to_wake = 1'b0;
      end
  
  always @* 
      begin
        if (WAKEUP_REQ_ORED | (~RESETn | ((~RESETn | (~MBUS_DIN | ~SLEEP_REQ)) & ((MBC_SLEEP_int | (~SLEEP_REQ & WAKEUP_REQ_ORED)) | ~SLEEP_REQ)))) rst_tran_to_wake <= 1'b1;
