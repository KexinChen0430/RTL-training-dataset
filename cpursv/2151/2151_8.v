...
module SLEEP_CONTROLv4(output reg MBC_ISOLATE,
  ...
  always @* 
      begin
        if ((((WAKEUP_REQ_ORED & MBC_SLEEP_int) | (((WAKEUP_REQ_ORED & MBC_SLEEP_int) | (((RESETn & (WAKEUP_REQ_ORED | MBC_SLEEP_int)) & (((RESETn | (WAKEUP_REQ_ORED & ~MBUS_DIN)) | WAKEUP_REQ_ORED) & MBC_SLEEP_int)) & (RESETn & (~MBUS_DIN & (WAKEUP_REQ_ORED | MBC_SLEEP_int))))) | (RESETn & (WAKEUP_REQ_ORED & ~MBUS_DIN)))) | WAKEUP_REQ_ORED) & RESETn) set_tran_to_wake = 1'b1;
        else set_tran_to_wake = 1'b0;
      end
  
  always @* 
      begin
        if (((~RESETn | (((~MBUS_DIN | (WAKEUP_REQ_ORED & MBC_SLEEP_int)) | (WAKEUP_REQ_ORED & MBC_SLEEP_int)) | WAKEUP_REQ_ORED)) & (WAKEUP_REQ_ORED | MBC_SLEEP_int)) | (((~RESETn | (WAKEUP_REQ_ORED & ~SLEEP_REQ)) | ~SLEEP_REQ) | (WAKEUP_REQ_ORED & MBC_SLEEP_int))) rst_tran_to_wake <= 1'b1;
