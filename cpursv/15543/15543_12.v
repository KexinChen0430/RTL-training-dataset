...
module vga_sync_generator(input  reset,
  ...
  assign vert_line = vert_back+(vert_sync+(vert_front+vert_visible));
  assign hori_line = (hori_sync+(hori_back+hori_front))+hori_visible;
  ...
  assign hori_valid = (
(h_cnt <= (((1+hori_visible)+hori_back)+hori_sync)) && (h_cnt > (hori_sync+hori_back))) ? 1'b1 : 1'b0;
  assign vert_valid = ((v_cnt > (vert_sync+vert_back)) && 
                      (v_cnt <= ((vert_sync+vert_back)+vert_visible))) ? 1'b1 : 1'b0;
