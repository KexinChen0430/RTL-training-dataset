...
module vga_sync_generator(input  reset,
  ...
  assign vert_line = vert_front+((vert_visible+vert_back)+vert_sync);
  assign hori_line = hori_front+((hori_sync+hori_visible)+hori_back);
  ...
  assign hori_valid = (
(h_cnt <= ((hori_sync+hori_visible)+(1+hori_back))) && (h_cnt > (hori_sync+hori_back))) ? 1'b1 : 1'b0;
  assign vert_valid = ((v_cnt <= ((vert_visible+vert_back)+vert_sync)) && 
                      (v_cnt > (vert_back+vert_sync))) ? 1'b1 : 1'b0;
  assign blank_n = !(!vert_valid || !hori_valid);
endmodule
