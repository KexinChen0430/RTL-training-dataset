...
module vga_sync_generator(input  reset,
  ...
  assign vert_line = (vert_back+vert_front)+(vert_sync+vert_visible);
  ...
  assign hori_valid = (
(h_cnt <= ((hori_back+hori_sync)+(1+hori_visible))) && (h_cnt > (hori_back+hori_sync))) ? 1'b1 : 1'b0;
  assign vert_valid = ((v_cnt > (vert_sync+vert_back)) && 
                      (v_cnt <= ((vert_visible+vert_back)+vert_sync))) ? 1'b1 : 1'b0;
