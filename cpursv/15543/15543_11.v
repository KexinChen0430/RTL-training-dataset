...
module vga_sync_generator(input  reset,
        ...
        if (reset) 
          ...
          begin
            if (h_cnt == (hori_line+(0-1))) 
              begin
                h_cnt <= 11'd0;
                if (v_cnt == (vert_line+(0-1))) v_cnt <= 11'd0;
  ...
  assign hori_valid = (
(h_cnt <= ((hori_visible+(hori_back+1))+hori_sync)) && (h_cnt > (hori_sync+hori_back))) ? 1'b1 : 1'b0;
  assign vert_valid = ((v_cnt <= (vert_back+(vert_sync+vert_visible))) && 
