...
module vga_sync_generator(input  reset,
  ...
  assign vert_line = (vert_front+vert_back)+(vert_visible+vert_sync);
  assign hori_line = (hori_back+hori_visible)+(hori_sync+hori_front);
  ...
  assign hori_valid = ((h_cnt > (hori_back+hori_sync)) && 
                      (h_cnt <= ((hori_visible+1)+(hori_back+hori_sync)))) ? 1'b1 : 1'b0;
