...
module vga_sync_generator(input  reset,
  ...
  assign vert_line = (vert_sync+vert_back)+(vert_front+vert_visible);
  assign hori_line = (hori_back+hori_front)+(hori_visible+hori_sync);
  ...
  assign hori_valid = ((h_cnt > (hori_back+hori_sync)) && 
                      (h_cnt <= (1+((hori_back+hori_sync)+hori_visible)))) ? 1'b1 : 1'b0;
  assign vert_valid = ((v_cnt <= ((vert_back+vert_visible)+vert_sync)) && 
