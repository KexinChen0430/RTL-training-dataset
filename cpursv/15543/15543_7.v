...
module vga_sync_generator(input  reset,
  ...
  assign vert_line = (vert_sync+vert_visible)+(vert_front+vert_back);
  assign hori_line = (hori_back+hori_sync)+(hori_front+hori_visible);
  ...
  assign hori_valid = (
(h_cnt <= ((1+hori_visible)+(hori_back+hori_sync))) && (h_cnt > (hori_back+hori_sync))) ? 1'b1 : 1'b0;
