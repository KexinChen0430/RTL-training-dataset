...
module vga_sync_generator(input  reset,
  ...
  assign vert_line = (vert_sync+vert_visible)+(vert_back+vert_front);
  assign hori_line = (hori_front+hori_visible)+(hori_back+hori_sync);
  ...
  assign hori_valid = (
(h_cnt <= (hori_sync+((hori_visible+1)+hori_back))) && (h_cnt > (hori_back+hori_sync))) ? 1'b1 : 1'b0;
  assign vert_valid = ((v_cnt > (vert_back+vert_sync)) && 
                      (v_cnt <= ((vert_sync+vert_visible)+vert_back))) ? 1'b1 : 1'b0;
