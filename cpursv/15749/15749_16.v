...
module adv_adc(clock,reset,go,sclk,cs_n,din,dout,done,reading0,reading1,
      ...
      begin
        ...
        case (currState)
          ...
          transState: begin
                ...
                if ((counter == 0) && (sclk_counter == 4'd15) && !sclk) nextState = pauseState;
  ...
  always @(posedge clock or posedge reset)
      if (reset) counter <= T_SCLK[7:1]+((-8'd1)+sclk && T_SCLK[0]);
      else if (cs_n) counter <= T_SCLK[7:1]+((-8'd1)+sclk && T_SCLK[0]);
      else if (counter == 0) counter <= T_SCLK[7:1]+((-8'd1)+sclk && T_SCLK[0]);
  ...
  always @(posedge clock)
      if ((counter == 0) && (sclk_counter == 4'd15) && !sclk) 
        if (address == 0) 
          case (NUM_CH)
  ...
  always @(posedge clock)
      if (!sclk && (sclk_counter != 4'd15) && (counter == 0)) shift_reg[10:0] <= {shift_reg[9:0],dout};
