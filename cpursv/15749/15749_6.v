...
module adv_adc(clock,reset,go,sclk,cs_n,din,dout,done,reading0,reading1,
  ...
  always @(posedge clock or posedge reset)
      if (reset) counter <= (T_SCLK[0] && sclk+T_SCLK[7:1])+(0-8'd1);
      else if (cs_n) counter <= (T_SCLK[0] && sclk+T_SCLK[7:1])+(0-8'd1);
      else if (counter == 0) counter <= (T_SCLK[0] && sclk+T_SCLK[7:1])+(0-8'd1);
  ...
  always @(posedge clock)
      if (currState == resetState) address <= 3'd1;
        ...
        if (address >= (NUM_CH+(0-1))) address <= 3'd0;
  ...
  always @(posedge clock)
      if (go && (currState == waitState)) addr_shift_reg <= 6'b000001;
        ...
        if (address >= (NUM_CH+(0-1))) addr_shift_reg <= 6'b0;
