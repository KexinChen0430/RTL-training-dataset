...
module adv_adc(clock,reset,go,sclk,cs_n,din,dout,done,reading0,reading1,
  ...
  always @(posedge clock or posedge reset)
      if (reset) counter <= T_SCLK[7:1]+((0-8'd1)+sclk && T_SCLK[0]);
      else if (cs_n) counter <= T_SCLK[7:1]+((0-8'd1)+sclk && T_SCLK[0]);
      else if (counter == 0) counter <= T_SCLK[7:1]+((0-8'd1)+sclk && T_SCLK[0]);
