...
module adv_adc(clock,reset,go,sclk,cs_n,din,dout,done,reading0,reading1,
  ...
  always @(posedge clock or posedge reset)
      if (reset) counter <= sclk && T_SCLK[0]+((0-8'd1)+T_SCLK[7:1]);
      else if (cs_n) counter <= sclk && T_SCLK[0]+((0-8'd1)+T_SCLK[7:1]);
      else if (counter == 0) counter <= sclk && T_SCLK[0]+((0-8'd1)+T_SCLK[7:1]);
      else counter <= (0-8'b1)+counter;
  ...
  always @(posedge clock)
      ...
      else if (!sclk && (counter == 0)) sclk_counter <= sclk_counter+4'b1;
  ...
  always @(posedge clock)
      if (currState == resetState) address <= 3'd1;
        ...
        if (address >= ((0-1)+NUM_CH)) address <= 3'd0;
        else address <= next_addr;
        
  
  always @(posedge clock)
      if ((sclk_counter != 4'd15) && (!sclk && (counter == 0))) shift_reg[10:0] <= {shift_reg[9:0],dout};
        
  
  always @(posedge clock)
      if (go && (currState == waitState)) addr_shift_reg <= 6'b000001;
      else if (currState == pauseState) 
        if (address >= ((0-1)+NUM_CH)) addr_shift_reg <= 6'b0;
