...
module adv_adc(clock,reset,go,sclk,cs_n,din,dout,done,reading0,reading1,
      ...
      begin
        ...
        case (currState)
          ...
          transState: begin
                ...
                if ((sclk_counter == 4'd15) && (!sclk && (counter == 0))) nextState = pauseState;
  ...
  always @(posedge clock or posedge reset)
      if (reset) counter <= (sclk && T_SCLK[0]+T_SCLK[7:1])+(0-8'd1);
      else if (cs_n) counter <= (sclk && T_SCLK[0]+T_SCLK[7:1])+(0-8'd1);
      else if (counter == 0) counter <= (sclk && T_SCLK[0]+T_SCLK[7:1])+(0-8'd1);
  ...
  always @(posedge clock)
      if ((currState == waitState) || (currState == doneState)) sclk_counter <= 4'b0;
      else if (!sclk && (counter == 0)) sclk_counter <= 4'b1+sclk_counter;
        
  
  always @(posedge clock)
      if ((sclk_counter == 4'd15) && (!sclk && (counter == 0))) 
        if (address == 0) 
          case (NUM_CH)
  ...
  always @(posedge clock)
      if (currState == resetState) address <= 3'd1;
        ...
        if (address >= (NUM_CH-1)) address <= 3'd0;
  ...
  always @(posedge clock)
      if (go && (currState == waitState)) addr_shift_reg <= 6'b000001;
      else if (currState == pauseState) 
        if (address >= (NUM_CH-1)) addr_shift_reg <= 6'b0;
        else addr_shift_reg <= {3'b0,next_addr};
      else if (sclk && (counter == 0)) addr_shift_reg <= {addr_shift_reg[4:0],1'b0};
        
  
  always @(posedge clock)  next_addr <= 3'b1+address;
endmodule
