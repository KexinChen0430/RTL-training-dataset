...
module rasterint_ctrl(input  wire clk,
                      input  wire rst_n,
  ...
  always @(posedge clk)
      begin
        if (rst_n == 1'b0) 
          ...
          begin
            if ((zxuno_addr == RASTERLINE) && (zxuno_regwr == 1'b1)) rasterline_reg <= din;
              
            if ((zxuno_addr == RASTERCTRL) && (zxuno_regwr == 1'b1)) {vretrace_disable,raster_enable,raster_8th_bit} <= din[2:0];
  ...
  always @* 
      begin
        dout = 8'hFF;
        oe_n = 1'b1;
        if ((zxuno_addr == RASTERLINE) && (zxuno_regrd == 1'b1)) 
          begin
            dout = rasterline_reg;
            oe_n = 1'b0;
          end
          
        if ((zxuno_regrd == 1'b1) && (zxuno_addr == RASTERCTRL)) 
          begin
