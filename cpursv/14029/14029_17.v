...
module axis_async_frame_fifo  #(parameter 
  ...
  reg   drop_frame = 1'b0;
  reg  [(-1)+((DATA_WIDTH+1)+1):0]  data_out_reg = {1'b0,{DATA_WIDTH{1'b0}}};
  reg  [(-1)+((DATA_WIDTH+1)+1):0] mem[(2**ADDR_WIDTH)-1:0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(-1)+((DATA_WIDTH+1)+1):0]  data_in = {input_axis_tlast,input_axis_tdata};
  wire 
       full = 
                  ...
                  begin
                    if (input_axis_tuser) 
                      ...
                      begin
                        ...
                        wr_ptr <= wr_ptr_next;
                        wr_ptr_gray <= ((~wr_ptr_next | (wr_ptr_next>>1 & ~wr_ptr_next>>1)) | ~wr_ptr_next>>1) & (wr_ptr_next>>1 | ((wr_ptr_next & (~wr_ptr_next>>1 | wr_ptr_next>>1)) & ((~wr_ptr_next | (wr_ptr_next>>1 & ~wr_ptr_next>>1)) | ~wr_ptr_next>>1)));
                      end
                  end
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= ((~rd_ptr_next & rd_ptr_next) | (rd_ptr_next & ~rd_ptr_next>>1)) | (rd_ptr_next>>1 & ~rd_ptr_next);
          end
