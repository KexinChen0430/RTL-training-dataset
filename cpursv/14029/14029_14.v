...
module axis_async_frame_fifo  #(parameter 
  ...
  reg  [1+((1+(-1))+DATA_WIDTH):0] mem[(2**ADDR_WIDTH)+(-1):0];
...
(wr_ptr[ADDR_WIDTH-1:0] == wr_ptr_cur[ADDR_WIDTH-1:0]);
  wire 
       write = (input_axis_tvalid & ~full) | ((DROP_WHEN_FULL & (~full | input_axis_tvalid)) & input_axis_tvalid);
  wire 
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= (rd_ptr_next>>1 & ((~rd_ptr_next & ((rd_ptr_next>>1 | rd_ptr_next) | ~rd_ptr_next>>1)) | (~rd_ptr_next>>1 & rd_ptr_next))) | ((~rd_ptr_next & rd_ptr_next) | (~rd_ptr_next>>1 & rd_ptr_next));
          end
