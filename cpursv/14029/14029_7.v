...
module axis_async_frame_fifo  #(parameter 
  ...
  reg   drop_frame = 1'b0;
  reg  [(DATA_WIDTH+1)+(1+(0-1)):0]  data_out_reg = {1'b0,{DATA_WIDTH{1'b0}}};
  reg  [(DATA_WIDTH+1)+(1+(0-1)):0] mem[(1<<1**ADDR_WIDTH)+(0-1):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(DATA_WIDTH+1)+(1+(0-1)):0]  data_in = {input_axis_tlast,input_axis_tdata};
  wire 
       full = 
...
(wr_ptr[ADDR_WIDTH-1:0] == wr_ptr_cur[ADDR_WIDTH-1:0]) && (wr_ptr[ADDR_WIDTH] != wr_ptr_cur[ADDR_WIDTH]);
  ...
  wire 
       read = ~empty & ((output_axis_tready | ~empty) & (output_axis_tready | ~output_axis_tvalid_reg));
                  ...
                  begin
                    if (input_axis_tuser) 
                      ...
                      begin
                        ...
                        wr_ptr <= wr_ptr_next;
                        wr_ptr_gray <= (wr_ptr_next & (wr_ptr_next>>1 | ~wr_ptr_next>>1)) ^ wr_ptr_next>>1;
                      end
                  end
