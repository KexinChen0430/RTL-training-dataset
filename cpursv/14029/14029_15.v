...
module axis_async_frame_fifo  #(parameter 
  ...
  reg   drop_frame = 1'b0;
  reg  [(1+DATA_WIDTH)+(1+(0-1)):0]  data_out_reg = {1'b0,{DATA_WIDTH{1'b0}}};
  reg  [(1+DATA_WIDTH)+(1+(0-1)):0] mem[(1<<1**ADDR_WIDTH)+(0-1):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(1+DATA_WIDTH)+(1+(0-1)):0]  data_in = {input_axis_tlast,input_axis_tdata};
  wire 
       full = 
...
(wr_ptr[ADDR_WIDTH-1:0] == wr_ptr_cur[ADDR_WIDTH-1:0]) && (wr_ptr[ADDR_WIDTH] != wr_ptr_cur[ADDR_WIDTH]);
  wire 
       write = (input_axis_tvalid & ~full) | (DROP_WHEN_FULL & input_axis_tvalid);
  wire 
