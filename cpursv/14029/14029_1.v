...
module axis_async_frame_fifo  #(parameter 
  ...
  reg   drop_frame = 1'b0;
  reg  [(1+(0-1))+(1+DATA_WIDTH):0]  data_out_reg = {1'b0,{DATA_WIDTH{1'b0}}};
  reg  [(1+(0-1))+(1+DATA_WIDTH):0] mem[(0-1)+((1+1)**ADDR_WIDTH):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(1+(0-1))+(1+DATA_WIDTH):0]  data_in = {input_axis_tlast,input_axis_tdata};
  wire 
       full = 
...
(wr_ptr_gray[ADDR_WIDTH+(0-1)] != rd_ptr_gray_sync2[ADDR_WIDTH+(0-1)]));
  ...
  wire 
       full_cur = 
(wr_ptr[ADDR_WIDTH+(0-1):0] == wr_ptr_cur[ADDR_WIDTH+(0-1):0]) && (wr_ptr[ADDR_WIDTH] != wr_ptr_cur[ADDR_WIDTH]);
  wire 
       write = (DROP_WHEN_FULL | (~full & (input_axis_tvalid | DROP_WHEN_FULL))) & input_axis_tvalid;
  wire 
       read = ((output_axis_tready | ~empty) & (~output_axis_tvalid_reg | output_axis_tready)) & ~empty;

  assign {output_axis_tlast,output_axis_tdata} = data_out_reg;
  assign input_axis_tready = (DROP_WHEN_FULL | (~full & (input_axis_tvalid | DROP_WHEN_FULL))) | ~full;
                  ...
                  begin
                    if (input_axis_tuser) 
                      ...
                      begin
                        ...
                        wr_ptr <= wr_ptr_next;
                        wr_ptr_gray <= (((wr_ptr_next & (~wr_ptr_next>>1 | (wr_ptr_next>>1 | wr_ptr_next))) & ~wr_ptr_next) | (wr_ptr_next>>1 & ~wr_ptr_next)) | ((wr_ptr_next>>1 | wr_ptr_next) & ~wr_ptr_next>>1);
                      end
                  end
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= (~rd_ptr_next>>1 & rd_ptr_next) | ((~rd_ptr_next & (~rd_ptr_next>>1 & rd_ptr_next)) | (~rd_ptr_next & (rd_ptr_next | (rd_ptr_next>>1 & (~rd_ptr_next | rd_ptr_next)))));
          end
