...
module AccumulateQueue  #(parameter 
  ...
  reg  [A_BITS-1:0] fifo_e[0:(FIFO_LENGTH*(1/(1+1)))-1];
  reg  [A_BITS-1:0] fifo_o[0:(FIFO_LENGTH*(1/(1+1)))-1];
  reg  [A_BITS-1:0] head;
  ...
  wire [INDEX_BITS+(-1):0]  index_next = 1+index;

  wire [INDEX_BITS+(0-(1+1)):0]  fifo_index = index[INDEX_BITS+(-1):1];

  wire [INDEX_BITS+(0-(1+1)):0]  fifo_index_next = index_next[INDEX_BITS+(-1):1];
