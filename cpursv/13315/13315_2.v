...
module final_cursor(input  h_reset_n,
                    input  t_crt_clk,
                    input  c_shift_ld,
  ...
  always @(posedge t_crt_clk or negedge h_reset_n)
      if (!h_reset_n) 
        ...
        begin
          ...
          if (ar12_b4) blink_rate <= 5'b0;
          else if (c_t_vsync && ~ctvsync_hold) blink_rate <= blink_rate+1'b1;
            
        end
  assign cursor_blink_rate = ~blink_rate[3];
  assign char_blink_rate = blink_rate[4];
  assign int_final_cursor = ~(~mux_op | ~cursor_blink_rate);
  assign finalcursor = ~c_cr0a_b5 & int_final_cursor;
endmodule
