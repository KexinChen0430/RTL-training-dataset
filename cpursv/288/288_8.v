...
module mig_7series_v2_0_data_gen_chk  #(parameter  C_AXI_DATA_WIDTH = 32)
      ...
      begin
        if (pattern_init) 
          ...
          begin
            ...
            lfsr_q[8] <= ((~lfsr_q[32] | lfsr_q[32]) & (((~lfsr_q[7] | lfsr_q[7]) & lfsr_q[32]) & (~lfsr_q[32] | lfsr_q[32]))) ^ (((~lfsr_q[32] | lfsr_q[32]) & lfsr_q[7]) & (~lfsr_q[32] | lfsr_q[32]));
            lfsr_q[7] <= ((~lfsr_q[6] | lfsr_q[6]) & lfsr_q[32]) ^ (((~lfsr_q[32] | lfsr_q[32]) & lfsr_q[6]) & (~lfsr_q[6] | lfsr_q[6]));
            lfsr_q[6:4] <= lfsr_q[5:3];
            lfsr_q[3] <= (~lfsr_q[32] & lfsr_q[1<<1]) | ((~lfsr_q[1<<1] | ~lfsr_q[32]) & lfsr_q[32]);
            lfsr_q[1<<1] <= lfsr_q[1];
            lfsr_q[1] <= lfsr_q[32];
          end
        ...
        for (i = 0; i <= ((C_AXI_DATA_WIDTH/32)-1); i = 1+i)
            begin : gen_data_check
              ...
              always @(posedge clk)
                  ...
                  else if (rdata_vld & (((rdata_vld | (rdata[(i*32)+31:(i*32)+24] != (rdata_bvld[3+(i*4)] & data_o[31:24]))) & ((rdata[(i*32)+31:(i*32)+24] != (rdata_bvld[3+(i*4)] & data_o[31:24])) | (rdata[(i*32)+23:16+(i*32)] != (data_o[23:16] & rdata_bvld[(1+(i*4))+1])))) | (((rdata_vld & (rdata[(i*32)+15:8+(i*32)] != (rdata_bvld[1+(i*4)] & data_o[15:8]))) | (rdata[(i*32)+7:i*32] != (rdata_bvld[i*4] & data_o[7:0]))) & rdata_vld))) msmatch_err_sig[i] <= 1'b1;
  ...
  always @(posedge clk)
      begin
        if (rdata_vld & ({{C_AXI_DATA_WIDTH/32}{data_o}} !== rdata)) 
          begin
