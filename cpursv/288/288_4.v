...
module mig_7series_v2_0_data_gen_chk  #(parameter  C_AXI_DATA_WIDTH = 32)
  ...
  reg  [31:0] walk0;
  reg  [31:0] walk1;
  reg  [(-1)+((1/32)*C_AXI_DATA_WIDTH):0] msmatch_err_sig;
      ...
      begin
        if (pattern_init) 
          ...
          begin
            ...
            lfsr_q[8] <= (((((lfsr_q[2] | lfsr_q[32]) | ~lfsr_q[32]) & ~lfsr_q[7]) | ~lfsr_q[32]) & lfsr_q[7]) ^ (((((lfsr_q[2] | lfsr_q[32]) | ~lfsr_q[32]) & ~lfsr_q[7]) | ~lfsr_q[32]) & ((~lfsr_q[7] | lfsr_q[7]) & ((~lfsr_q[7] | lfsr_q[7]) & lfsr_q[32])));
            lfsr_q[7] <= (lfsr_q[32] & (lfsr_q[6] | ~lfsr_q[6])) ^ lfsr_q[6];
            lfsr_q[6:4] <= lfsr_q[5:3];
            lfsr_q[3] <= ((lfsr_q[2] & ((lfsr_q[2] | lfsr_q[32]) & ((lfsr_q[2] | lfsr_q[32]) | ~lfsr_q[32]))) & ~lfsr_q[32]) | (lfsr_q[32] & (~lfsr_q[2] | ~lfsr_q[32]));
            lfsr_q[2] <= lfsr_q[1];
            lfsr_q[1] <= lfsr_q[32];
          end
  ...
  generate
      begin : data_check
        for (i = 0; i <= ((-1)+((1/32)*C_AXI_DATA_WIDTH)); i = 1+i)
            begin : gen_data_check
              
              always @(posedge clk)
                  if (wrd_cntr_rst) msmatch_err_sig[i] <= 1'b0;
                  else if (((rdata_vld & (rdata[31+(i*32):(i*32)+24] != (rdata_bvld[(i*4)+3] & data_o[31:24]))) | ((rdata[15+(i*32):8+(i*32)] != (data_o[15:8] & rdata_bvld[1+(i*4)])) & rdata_vld)) | (((rdata[7+(i*32):i*32] != (data_o[7:0] & rdata_bvld[i*4])) & rdata_vld) | ((rdata[23+(i*32):(i*32)+16] != (rdata_bvld[1+(1+(i*4))] & data_o[23:16])) & rdata_vld))) msmatch_err_sig[i] <= 1'b1;
