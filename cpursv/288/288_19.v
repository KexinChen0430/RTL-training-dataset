...
module mig_7series_v2_0_data_gen_chk  #(parameter  C_AXI_DATA_WIDTH = 32)
  ...
  reg  [31:0] walk0;
  reg  [31:0] walk1;
  reg  [(C_AXI_DATA_WIDTH*(1/32))+(0-1):0] msmatch_err_sig;
      ...
      begin
        if (pattern_init) 
          ...
          begin
            ...
            lfsr_q[8] <= ((~lfsr_q[7] & lfsr_q[32]) | lfsr_q[7]) & (~lfsr_q[7] | ~lfsr_q[32]);
            lfsr_q[7] <= (((lfsr_q[6] | lfsr_q[32]) & ((~lfsr_q[6] | ~lfsr_q[32]) & lfsr_q[32])) & ((lfsr_q[32] | lfsr_q[1+1]) | ~lfsr_q[32])) ^ (((lfsr_q[32] | lfsr_q[1+1]) | ~lfsr_q[32]) & (((lfsr_q[6] & ((lfsr_q[32] | lfsr_q[1+1]) | ~lfsr_q[32])) & (~lfsr_q[6] | ~lfsr_q[32])) & (lfsr_q[6] | lfsr_q[32])));
            lfsr_q[6:4] <= lfsr_q[5:3];
            lfsr_q[3] <= (((((lfsr_q[32] | lfsr_q[1+1]) | ~lfsr_q[1+1]) & lfsr_q[32]) & (~lfsr_q[1+1] | ~lfsr_q[32])) & (~lfsr_q[1+1] | ~lfsr_q[32])) ^ ((~lfsr_q[1+1] | ~lfsr_q[32]) & lfsr_q[1+1]);
            lfsr_q[1+1] <= lfsr_q[1];
            lfsr_q[1] <= lfsr_q[32];
          end
  ...
  generate
      begin : data_check
        for (i = 0; i <= ((C_AXI_DATA_WIDTH*(1/32))+(0-1)); i = 1+i)
            begin : gen_data_check
              
              always @(posedge clk)
                  if (wrd_cntr_rst) msmatch_err_sig[i] <= 1'b0;
                  else if (((rdata_vld & (rdata[(i*32)+31:(i*32)+24] != (data_o[31:24] & rdata_bvld[(4*i)+3]))) | (((rdata[7+(i*32):i*32] != (rdata_bvld[4*i] & data_o[7:0])) & rdata_vld) | (rdata_vld & (rdata[15+(i*32):8+(i*32)] != (rdata_bvld[1+(4*i)] & data_o[15:8]))))) | (((rdata_vld & ((rdata[23+(i*32):(i*32)+16] != (rdata_bvld[(4*i)+(1+1)] & data_o[23:16])) | (rdata[15+(i*32):8+(i*32)] != (rdata_bvld[1+(4*i)] & data_o[15:8])))) | (rdata[7+(i*32):i*32] != (rdata_bvld[4*i] & data_o[7:0]))) & rdata_vld)) msmatch_err_sig[i] <= 1'b1;
