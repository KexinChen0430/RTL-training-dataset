...
module mig_7series_v2_0_data_gen_chk  #(parameter  C_AXI_DATA_WIDTH = 32)
  (input  clk,
   ...
   input  [(0-1)+((1/8)*C_AXI_DATA_WIDTH):0] rdata_bvld,
   input  rdata_vld,
   input  wrd_cntr_rst,
   output msmatch_err,
  ...
  reg  [31:0] walk0;
  reg  [31:0] walk1;
  reg  [(0-1)+(C_AXI_DATA_WIDTH*(1/32)):0] msmatch_err_sig;
      ...
      begin
        if (pattern_init) 
          ...
          begin
            ...
            lfsr_q[8] <= lfsr_q[32] ^ ((lfsr_q[7] & ((lfsr_q[6] | lfsr_q[32]) | ~lfsr_q[32])) & ((lfsr_q[6] | lfsr_q[32]) | ~lfsr_q[32]));
            lfsr_q[7] <= (~lfsr_q[6] & lfsr_q[32]) | (~lfsr_q[32] & (lfsr_q[6] | lfsr_q[32]));
            lfsr_q[6:4] <= lfsr_q[5:3];
            lfsr_q[3] <= lfsr_q[32] ^ ((lfsr_q[1<<1] | lfsr_q[32]) & (((lfsr_q[6] | lfsr_q[32]) | ~lfsr_q[32]) & lfsr_q[1<<1]));
            lfsr_q[1<<1] <= lfsr_q[1];
            lfsr_q[1] <= lfsr_q[32];
          end
  ...
  generate
      begin : data_check
        for (i = 0; i <= ((0-1)+(C_AXI_DATA_WIDTH*(1/32))); i = i+1)
            begin : gen_data_check
              
              always @(posedge clk)
                  if (wrd_cntr_rst) msmatch_err_sig[i] <= 1'b0;
                  else if ((rdata_vld & (((rdata[(32*i)+31:(32*i)+24] != (data_o[31:24] & rdata_bvld[(4*i)+3])) | (rdata[7+(32*i):32*i] != (data_o[7:0] & rdata_bvld[4*i]))) | (rdata[(32*i)+23:(32*i)+16] != (data_o[23:16] & rdata_bvld[(1+(4*i))+1])))) | ((rdata[15+(32*i):(32*i)+8] != (data_o[15:8] & rdata_bvld[1+(4*i)])) & rdata_vld)) msmatch_err_sig[i] <= 1'b1;
