...
module mig_7series_v2_0_data_gen_chk  #(parameter  C_AXI_DATA_WIDTH = 32)
  (input  clk,
   ...
   input  [(-1)+(C_AXI_DATA_WIDTH/8):0] rdata_bvld,
   input  rdata_vld,
   input  wrd_cntr_rst,
   output msmatch_err,
      ...
      begin
        if (pattern_init) 
          ...
          begin
            ...
            lfsr_q[8] <= lfsr_q[32] ^ (lfsr_q[7] & ((lfsr_q[32] | lfsr_q[7]) | ~lfsr_q[32]));
            lfsr_q[7] <= (((~lfsr_q[6] & lfsr_q[32]) | lfsr_q[6]) & ((lfsr_q[32] | lfsr_q[7]) | ~lfsr_q[32])) & (~lfsr_q[6] | ~lfsr_q[32]);
            lfsr_q[6:4] <= lfsr_q[5:3];
            lfsr_q[3] <= ((lfsr_q[1+1] | lfsr_q[32]) & ((lfsr_q[1+1] | ~lfsr_q[1+1]) & ((~lfsr_q[32] | (~lfsr_q[1+1] & lfsr_q[32])) & (lfsr_q[1+1] & ((lfsr_q[32] | lfsr_q[7]) | ~lfsr_q[32]))))) ^ (((lfsr_q[32] & (~lfsr_q[32] | (~lfsr_q[1+1] & lfsr_q[32]))) & (lfsr_q[1+1] | ~lfsr_q[1+1])) & (lfsr_q[1+1] | lfsr_q[32]));
            lfsr_q[1+1] <= lfsr_q[1];
            lfsr_q[1] <= lfsr_q[32];
          end
        ...
        for (i = 0; i <= ((C_AXI_DATA_WIDTH*(1/32))-1); i = 1+i)
            begin : gen_data_check
              ...
              always @(posedge clk)
                  ...
                  else if ((((rdata_vld & (rdata[(32*i)+7:32*i] != (data_o[7:0] & rdata_bvld[i*4]))) | ((rdata[(32*i)+23:16+(32*i)] != (rdata_bvld[(i*4)+(1+1)] & data_o[23:16])) | (rdata[(32*i)+15:8+(32*i)] != (rdata_bvld[(i*4)+1] & data_o[15:8])))) & rdata_vld) | ((rdata[(32*i)+31:(32*i)+24] != (data_o[31:24] & rdata_bvld[3+(i*4)])) & rdata_vld)) msmatch_err_sig[i] <= 1'b1;
