      ...
      begin
        if (pattern_init) 
          ...
          begin
            ...
            lfsr_q[8] <= ((lfsr_q[7] | lfsr_q[32]) & (~lfsr_q[32] | (lfsr_q[6] | lfsr_q[32]))) & (~lfsr_q[7] | ~lfsr_q[32]);
            lfsr_q[7] <= ((lfsr_q[6] & (~lfsr_q[32] | (lfsr_q[6] | lfsr_q[32]))) & (lfsr_q[6] | ~lfsr_q[6])) ^ (lfsr_q[32] & (lfsr_q[6] | ~lfsr_q[6]));
            lfsr_q[6:4] <= lfsr_q[5:3];
            lfsr_q[3] <= ((((~lfsr_q[32] | (lfsr_q[6] | lfsr_q[32])) & lfsr_q[1<<1]) & (~lfsr_q[1<<1] | lfsr_q[1<<1])) & (~lfsr_q[1<<1] | ~lfsr_q[32])) ^ (((~lfsr_q[1<<1] | lfsr_q[1<<1]) & lfsr_q[32]) & (~lfsr_q[1<<1] | ~lfsr_q[32]));
            lfsr_q[1<<1] <= lfsr_q[1];
            lfsr_q[1] <= lfsr_q[32];
          end
        ...
        for (i = 0; i <= (((1/32)*C_AXI_DATA_WIDTH)-1); i = i+1)
            begin : gen_data_check
              ...
              always @(posedge clk)
                  ...
                  else if (rdata_vld & (((rdata[31+(i*32):(i*32)+24] != (data_o[31:24] & rdata_bvld[3+(i*4)])) | rdata_vld) & ((rdata[(i*32)+7:i*32] != (data_o[7:0] & rdata_bvld[i*4])) | ((rdata[15+(i*32):8+(i*32)] != (data_o[15:8] & rdata_bvld[(i*4)+1])) | ((rdata[31+(i*32):(i*32)+24] != (data_o[31:24] & rdata_bvld[3+(i*4)])) | (rdata[23+(i*32):16+(i*32)] != (rdata_bvld[(i*4)+1<<1] & data_o[23:16]))))))) msmatch_err_sig[i] <= 1'b1;
