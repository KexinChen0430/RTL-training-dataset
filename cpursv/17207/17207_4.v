...
module vga(input  wire clk,
  ...
  parameter  X_START = H_SYNC_BACK+(H_SYNC_CYC+4);
  ...
  assign vga_r = ((h_cnt >= (X_START+9)) && 
                 ((h_cnt < ((X_START+H_SYNC_ACT)+9)) && 
(v_cnt < (Y_START+V_SYNC_ACT)) && (v_cnt >= Y_START))) ? r : 0;
  assign vga_g = ((h_cnt >= (X_START+9)) && 
                 ((h_cnt < ((X_START+H_SYNC_ACT)+9)) && 
(v_cnt < (Y_START+V_SYNC_ACT)) && (v_cnt >= Y_START))) ? g : 0;
  assign vga_b = ((h_cnt >= (X_START+9)) && 
                 ((h_cnt < ((X_START+H_SYNC_ACT)+9)) && 
(v_cnt < (Y_START+V_SYNC_ACT)) && (v_cnt >= Y_START))) ? b : 0;
  ...
  always @(posedge clk or negedge rst)
      begin
        if (!rst) 
          ...
          begin
            if ((h_cnt >= X_START) && (v_cnt < (Y_START+V_SYNC_ACT)) && 
                (v_cnt >= Y_START) && (h_cnt < (X_START+H_SYNC_ACT))) 
              begin
                x <= h_cnt+(-X_START);
                y <= v_cnt-Y_START;
              end
