...
module vga(input  wire clk,
  ...
  assign vga_sync = 0;
  assign vga_r = ((v_cnt < (Y_START+V_SYNC_ACT)) && (v_cnt >= Y_START) && 
                 ((h_cnt < (H_SYNC_ACT+(X_START+9))) && 
(h_cnt >= (X_START+9)))) ? r : 0;
  assign vga_g = ((v_cnt < (Y_START+V_SYNC_ACT)) && (v_cnt >= Y_START) && 
                 ((h_cnt < (H_SYNC_ACT+(X_START+9))) && 
(h_cnt >= (X_START+9)))) ? g : 0;
  assign vga_b = ((v_cnt < (Y_START+V_SYNC_ACT)) && (v_cnt >= Y_START) && 
                 ((h_cnt < (H_SYNC_ACT+(X_START+9))) && 
(h_cnt >= (X_START+9)))) ? b : 0;
...
(v_cnt < (Y_START+V_SYNC_ACT)) && (h_cnt >= X_START))) 
              begin
                x <= (-X_START)+h_cnt;
                y <= v_cnt-Y_START;
              end
