...
module vga(input  wire clk,
  ...
  assign vga_sync = 0;
  assign vga_r = ((v_cnt >= Y_START) && 
                 ((h_cnt >= (9+X_START)) && 
(v_cnt < (V_SYNC_ACT+Y_START)) && (h_cnt < (H_SYNC_ACT+(9+X_START))))) ? r : 0;
  assign vga_g = ((v_cnt >= Y_START) && 
                 ((h_cnt >= (9+X_START)) && 
(v_cnt < (V_SYNC_ACT+Y_START)) && (h_cnt < (H_SYNC_ACT+(9+X_START))))) ? g : 0;
  assign vga_b = ((v_cnt >= Y_START) && 
                 ((h_cnt >= (9+X_START)) && 
(v_cnt < (V_SYNC_ACT+Y_START)) && (h_cnt < (H_SYNC_ACT+(9+X_START))))) ? b : 0;
  ...
  always @(posedge clk or negedge rst)
      begin
        if (!rst) 
          ...
          begin
            if ((v_cnt < (V_SYNC_ACT+Y_START)) && (v_cnt >= Y_START) && 
