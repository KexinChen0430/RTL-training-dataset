...
module vga(input  wire clk,
  ...
  assign vga_sync = 0;
  assign vga_r = ((h_cnt < ((9+X_START)+H_SYNC_ACT)) && 
                 (v_cnt < (Y_START+V_SYNC_ACT)) && ((h_cnt >= (9+X_START)) && (v_cnt >= Y_START))) ? r : 0;
  assign vga_g = ((h_cnt < ((9+X_START)+H_SYNC_ACT)) && 
                 (v_cnt < (Y_START+V_SYNC_ACT)) && ((h_cnt >= (9+X_START)) && (v_cnt >= Y_START))) ? g : 0;
  assign vga_b = ((h_cnt < ((9+X_START)+H_SYNC_ACT)) && 
                 (v_cnt < (Y_START+V_SYNC_ACT)) && ((h_cnt >= (9+X_START)) && (v_cnt >= Y_START))) ? b : 0;
  ...
  always @(posedge clk or negedge rst)
      begin
        if (!rst) 
          ...
          begin
            if ((h_cnt >= X_START) && 
                ((h_cnt < (X_START+H_SYNC_ACT)) && 
((v_cnt < (Y_START+V_SYNC_ACT)) && (v_cnt >= Y_START)))) 
              begin
                x <= (0-X_START)+h_cnt;
                y <= v_cnt-Y_START;
              end
