...
module vga(input  wire clk,
  ...
  assign vga_r = ((h_cnt >= (X_START+9)) && 
                 ((v_cnt >= Y_START) && 
(h_cnt < (H_SYNC_ACT+(X_START+9))) && (v_cnt < (Y_START+V_SYNC_ACT)))) ? r : 0;
  assign vga_g = ((h_cnt >= (X_START+9)) && 
                 ((v_cnt >= Y_START) && 
(h_cnt < (H_SYNC_ACT+(X_START+9))) && (v_cnt < (Y_START+V_SYNC_ACT)))) ? g : 0;
  assign vga_b = ((h_cnt >= (X_START+9)) && 
                 ((v_cnt >= Y_START) && 
(h_cnt < (H_SYNC_ACT+(X_START+9))) && (v_cnt < (Y_START+V_SYNC_ACT)))) ? b : 0;
