...
(h_cnt < ((H_SYNC_ACT+X_START)+9)))) ? r : 0;
  assign vga_g = ((h_cnt >= (9+X_START)) && 
                 ((v_cnt >= Y_START) && (v_cnt < (Y_START+V_SYNC_ACT)) && 
(h_cnt < ((H_SYNC_ACT+X_START)+9)))) ? g : 0;
  assign vga_b = ((h_cnt >= (9+X_START)) && 
                 ((v_cnt >= Y_START) && (v_cnt < (Y_START+V_SYNC_ACT)) && 
(h_cnt < ((H_SYNC_ACT+X_START)+9)))) ? b : 0;
  ...
  always @(posedge clk or negedge rst)
      begin
        if (!rst) 
          ...
          begin
            if ((v_cnt < (Y_START+V_SYNC_ACT)) && (h_cnt >= X_START) && 
                ((v_cnt >= Y_START) && (h_cnt < (H_SYNC_ACT+X_START)))) 
              begin
                x <= (0-X_START)+h_cnt;
                y <= v_cnt+(0-Y_START);
              end
