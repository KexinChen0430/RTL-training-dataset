...
module vga(input  wire clk,
  ...
  assign vga_sync = 0;
  assign vga_r = ((v_cnt < (V_SYNC_ACT+Y_START)) && (v_cnt >= Y_START) && 
                 ((h_cnt >= (9+X_START)) && 
(h_cnt < ((X_START+H_SYNC_ACT)+9)))) ? r : 0;
  assign vga_g = ((v_cnt < (V_SYNC_ACT+Y_START)) && (v_cnt >= Y_START) && 
                 ((h_cnt >= (9+X_START)) && 
(h_cnt < ((X_START+H_SYNC_ACT)+9)))) ? g : 0;
  assign vga_b = ((v_cnt < (V_SYNC_ACT+Y_START)) && (v_cnt >= Y_START) && 
                 ((h_cnt >= (9+X_START)) && 
(h_cnt < ((X_START+H_SYNC_ACT)+9)))) ? b : 0;
        ...
        if (!rst) 
          ...
          begin
            if ((h_cnt >= X_START) && (h_cnt < (X_START+H_SYNC_ACT)) && 
                ((v_cnt < (V_SYNC_ACT+Y_START)) && (v_cnt >= Y_START))) 
              begin
                x <= (0-X_START)+h_cnt;
                y <= v_cnt-Y_START;
              end
