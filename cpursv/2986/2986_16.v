...
module cache(clk,rst_n,addr,wr_data,wdirty,we,re,rd_data,tag_out,hit,
  ...
  always @(clk or we_filt or negedge rst_n)
      if (!rst_n) 
        for (x = 0; x < 8; x = x+1)
            mem[x] = {2'b00,{75{1'bx}}};
      else if (we_filt && ~clk) mem[addr[2:0]] = {1'b1,wdirty,addr[13:3],wr_data};
        
  
  always @(clk or re or addr)
      if (re && clk) line = mem[addr[2:0]];
        
  assign hit = ((line[74:64] == addr[13:3]) && (we | re)) ? line[76] : 1'b0;
  assign dirty = line[75] & line[76];
