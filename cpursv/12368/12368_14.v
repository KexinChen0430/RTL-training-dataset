...
module export_master(clk,reset,address,read,readdata,readdatavalid,write,
  ...
  input  clk;
  input  reset;
  input  [(-1)+WORD_ADDRESS_WIDTH:0] address;
  input  read;
  output [DATA_WIDTH-1:0] readdata;
  output readdatavalid;
  input  write;
  input  [DATA_WIDTH-1:0] writedata;
  input  [(-1)+BURSTCOUNT_WIDTH:0] burstcount;
  input  burstbegin;
  input  [(-1)+NUM_BYTES:0] byteenable;
  output waitrequest;
  output interrupt;
  ...
  output [(-1)+BURSTCOUNT_WIDTH:0] export_burstcount;
  output export_burstbegin;
  output [(-1)+NUM_BYTES:0] export_byteenable;
  input  export_waitrequest;
  input  export_interrupt;

  assign export_address = address*(2**ADDRESS_SHIFT);
