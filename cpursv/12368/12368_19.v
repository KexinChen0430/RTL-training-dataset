...
module export_master(clk,reset,address,read,readdata,readdatavalid,write,
  ...
  input  read;
  output [(-1)+DATA_WIDTH:0] readdata;
  output readdatavalid;
  input  write;
  input  [(-1)+DATA_WIDTH:0] writedata;
  ...
  output export_read;
  input  [(-1)+DATA_WIDTH:0] export_readdata;
  input  export_readdatavalid;
  output export_write;
  output [(-1)+DATA_WIDTH:0] export_writedata;
  ...
  assign export_address = address*(1<<<1**ADDRESS_SHIFT);
