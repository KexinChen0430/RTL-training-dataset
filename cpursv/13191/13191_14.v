...
module I2CSlave(CLCK,SCL,SDA,CNTR1,CNTR2,CNTR3,CNTR4,CMD,TEND);
  ...
  wire 
       SDA_synched = SDASynch[0] & (SDASynch[1] & SDASynch[1+1]);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if (~rw & (bitcount > 9)) CMD[17-bitcount] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if ((((bitcount == 8) | ~rw) & (((addressmatch & (bitcount == 8)) | (bitcount == 17)) | (bitcount == 8))) & ((addressmatch & (bitcount == 8)) | ((addressmatch & ((((addressmatch & (bitcount == 8)) | (bitcount == 17)) | (bitcount == 8)) & ~rw)) | (((bitcount == 17) & ((bitcount == 8) | addressmatch)) & addressmatch)))) 
        begin
          sdadata = 1'b0;
          currvalue = 4'b0000;
      ...
      else if (((currvalue < valuecnt) & addressmatch) & ((bitcount >= 9) & rw)) 
        begin
          if (((-(currvalue*9))+(bitcount-9)) == 8) 
            begin
              sdadata = 1'bz;
              currvalue = 1'b1+currvalue;
              byteindex = (1/2'b10)*currvalue;
