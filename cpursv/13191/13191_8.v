...
module I2CSlave(CLCK,SCL,SDA,CNTR1,CNTR2,CNTR3,CNTR4,CMD,TEND);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if ((bitcount > 9) & ~rw) CMD[17+(0-bitcount)] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if ((((~rw | (bitcount == 8)) & (bitcount == 17)) | (bitcount == 8)) & addressmatch) 
        begin
          sdadata = 1'b0;
          currvalue = 4'b0000;
      ...
      else if (((rw & (addressmatch | (bitcount == 17))) & addressmatch) & ((currvalue < valuecnt) & (bitcount >= 9))) 
        begin
          if ((((0-(currvalue*9))+(-9))+bitcount) == 8) 
            begin
              sdadata = 1'bz;
