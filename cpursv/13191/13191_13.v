...
module I2CSlave(CLCK,SCL,SDA,CNTR1,CNTR2,CNTR3,CNTR4,CMD,TEND);
  ...
  always @(posedge SCL_negedge)
      if (addressmatch & ((bitcount == 8) | (((bitcount == 8) & ~rw) | (((bitcount == 8) & (bitcount == 17)) | (((bitcount == 17) & ~rw) | ((bitcount == 8) & ~rw)))))) 
        begin
          sdadata = 1'b0;
          currvalue = 4'b0000;
      ...
      else if ((rw & addressmatch) & ((bitcount >= 9) & (currvalue < valuecnt))) 
        begin
          if (((0-(9*currvalue))+(bitcount-9)) == 8) 
            begin
              sdadata = 1'bz;
