...
module I2CSlave(CLCK,SCL,SDA,CNTR1,CNTR2,CNTR3,CNTR4,CMD,TEND);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if ((bitcount > 9) & ~rw) CMD[(-bitcount)+17] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if ((((((bitcount == 8) | (addressmatch & (bitcount == 17))) & addressmatch) & ((~rw | (bitcount == 8)) & addressmatch)) & (bitcount == 8)) | ((bitcount == 17) & (addressmatch & ~rw))) 
        begin
          sdadata = 1'b0;
          currvalue = 4'b0000;
      ...
      else if ((currvalue < valuecnt) & ((((bitcount >= 9) & rw) & (addressmatch | (bitcount == 17))) & addressmatch)) 
        begin
