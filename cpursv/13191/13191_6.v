...
module I2CSlave(CLCK,SCL,SDA,CNTR1,CNTR2,CNTR3,CNTR4,CMD,TEND);
  ...
  always @(posedge SCL_negedge)
      if (((addressmatch & ((bitcount == 17) | (bitcount == 8))) & (~rw | ((bitcount == 17) | (bitcount == 8)))) & ((bitcount == 8) | ~rw)) 
        begin
          sdadata = 1'b0;
          currvalue = 4'b0000;
      ...
      else if (((rw & (addressmatch | (bitcount == 17))) & addressmatch) & ((bitcount >= 9) & (currvalue < valuecnt))) 
        begin
          if ((bitcount+((-(currvalue*9))+(-9))) == 8) 
            begin
              sdadata = 1'bz;
