...
module I2CSlave(CLCK,SCL,SDA,CNTR1,CNTR2,CNTR3,CNTR4,CMD,TEND);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if (~rw & (bitcount > 9)) CMD[17+(0-bitcount)] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if ((((bitcount == 8) | ~rw) & ((bitcount == 8) | (bitcount == 17))) & addressmatch) 
        begin
          sdadata = 1'b0;
          currvalue = 4'b0000;
      ...
      else if ((rw & (currvalue < valuecnt)) & (addressmatch & (bitcount >= 9))) 
        begin
          if (((-(currvalue*9))+((0-9)+bitcount)) == 8) 
            begin
              sdadata = 1'bz;
