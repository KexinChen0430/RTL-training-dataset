...
module I2CSlave(CLCK,SCL,SDA,CNTR1,CNTR2,CNTR3,CNTR4,CMD,TEND);
  ...
  wire 
       SDA_synched = SDASynch[0] & (SDASynch[2] & SDASynch[1]);
  ...
  always @(posedge SCL_posedge or negedge incycle)
      if (~incycle) 
        ...
        begin
          ...
          if (bitcount < 8) address[(0-bitcount)+7] = SDA_synched;
  ...
  always @(posedge SCL_negedge)
      if ((addressmatch & ((bitcount == 8) | ((addressmatch & ~rw) | (~rw & (bitcount == 8))))) & ((bitcount == 8) | ((bitcount == 17) | ((~rw & (bitcount == 17)) | (~rw & (bitcount == 8)))))) 
        begin
          sdadata = 1'b0;
          currvalue = 4'b0000;
          byteindex = currvalue*(1/2'b10);
          bitindex = 4'b1111;
        end
      else if (addressmatch & (rw & ((currvalue < valuecnt) & (bitcount >= 9)))) 
        begin
