...
module tx_DS_sel(input  txClk,
  ...
  wire 
       isEOP = ((dat_i[1:0] == 2'b01) || (dat_i[1:0] == 2'b10)) && 
(dat_i[8] == 1);
          ...
          begin
            if (~isBusy && ready_i) 
              begin
                ...
                else if (haveCredit && (isValid && reqData_i)) 
                  begin
                    ackData_o <= 1;
                    sendingData <= 1;
                    dat_o <= dat_i;
              ...
              begin
                if (~sendingData && ~sendingIdle && escPending && 
                ...
                if (~sendingIdle && ~sendingCredit && 
                    (~escPending && (sendingData && sendingTimecode))) 
                  begin
                    sendingTimecode <= 0;
                    sendingData <= 0;
                  end
                ...
                if (~sendingTimecode && 
                    (~sendingCredit && 
(~sendingData && (escPending && sendingIdle)))) 
                  begin
                    escPending <= 0;
                    sendingCredit <= 1;
                    dat_o <= 9'b1_0000_0000;
                ...
                if (~escPending && ~sendingTimecode && 
                    (~sendingData && (sendingCredit && sendingIdle))) 
                  begin
                    sendingCredit <= 0;
                    sendingIdle <= 0;
                  end
