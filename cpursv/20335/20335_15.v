...
module tx_DS_sel(input  txClk,
  ...
  wire 
       isEOP = ((dat_i[1:0] == 2'b10) || (dat_i[1:0] == 2'b01)) && 
(dat_i[8] == 1);
        ...
        if (txReset) 
          ...
          begin
            if (~isBusy && ready_i) 
              ...
              begin
                if (escPending && sendingTimecode && ~sendingCredit && 
                ...
                if (sendingTimecode && sendingData && 
                    (~sendingCredit && ~sendingIdle) && ~escPending) 
                  begin
                    sendingTimecode <= 0;
                    sendingData <= 0;
                  end
                ...
                if (sendingCredit && 
                    (~sendingIdle && (~escPending && ~sendingData)) && ~sendingTimecode) 
                  begin
                    sendingCredit <= 0;
                  end
                  
                if (~sendingCredit && ~sendingTimecode && escPending && 
