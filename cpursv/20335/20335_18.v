          ...
          begin
            if (~isBusy && ready_i) 
              begin
                ...
                else if (reqData_i && (haveCredit && isValid)) 
                  begin
                    ackData_o <= 1;
                    sendingData <= 1;
                    dat_o <= dat_i;
              ...
              begin
                ...
                if (sendingTimecode && 
                    (sendingData && (~escPending && ~sendingCredit) && 
~sendingIdle)) 
                  begin
                    sendingTimecode <= 0;
                    sendingData <= 0;
                ...
                if (~escPending && sendingCredit && 
                    (~sendingData && ~sendingIdle) && ~sendingTimecode) 
                  begin
                    sendingCredit <= 0;
                  end
                  
                if (sendingIdle && 
                    (escPending && ~sendingData && ~sendingCredit) && ~sendingTimecode) 
                  begin
                    escPending <= 0;
                    sendingCredit <= 1;
                    dat_o <= 9'b1_0000_0000;
                ...
                if (sendingIdle && (~sendingData && ~escPending) && 
                    sendingCredit && ~sendingTimecode) 
                  begin
                    sendingCredit <= 0;
                    sendingIdle <= 0;
                  end
