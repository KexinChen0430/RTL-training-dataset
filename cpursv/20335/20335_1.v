...
module tx_DS_sel(input  txClk,
  ...
  wire 
       isEOP = ((dat_i[8] == 1) && (dat_i[1:0] == 2'b10)) || 
            ...
            if (ready_i && ~isBusy) 
              ...
              begin
                if (sendingTimecode && escPending && 
                    (~sendingCredit && (~sendingData && ~sendingIdle))) 
                  begin
                    escPending <= 0;
                    sendingData <= 1;
                ...
                if (~sendingIdle && (sendingData && ~sendingCredit) && 
                ...
                if (~escPending && ~sendingIdle && 
                    (sendingCredit && ~sendingData && ~sendingTimecode)) 
                  begin
                    sendingCredit <= 0;
                  end
                ...
                if (~sendingCredit && 
                    (sendingIdle && 
(~sendingTimecode && ~sendingData && escPending))) 
                  begin
                    escPending <= 0;
                    sendingCredit <= 1;
                    dat_o <= 9'b1_0000_0000;
                ...
                if (sendingCredit && (~escPending && ~sendingData) && 
