          ...
          begin
            if (ready_i && ~isBusy) 
              begin
                ...
                else if (haveCredit && reqData_i && isValid) 
                  begin
                    ackData_o <= 1;
                    sendingData <= 1;
                    dat_o <= dat_i;
              ...
              begin
                if (sendingTimecode && escPending && 
                    (~sendingIdle && ~sendingCredit && ~sendingData)) 
                  begin
                    escPending <= 0;
                    sendingData <= 1;
                ...
                if (sendingTimecode && sendingData && 
                    (~escPending && ~sendingCredit && ~sendingIdle)) 
                  begin
                    sendingTimecode <= 0;
                    sendingData <= 0;
                  end
                  
                if (~escPending && ~sendingIdle && 
                    (~sendingTimecode && sendingCredit && ~sendingData)) 
                  begin
                    sendingCredit <= 0;
                  end
                  
                if (~sendingCredit && (escPending && sendingIdle) && 
                ...
                if (sendingCredit && sendingIdle && 
                    (~sendingData && ~escPending && ~sendingTimecode)) 
                  begin
                    sendingCredit <= 0;
                    sendingIdle <= 0;
                  end
