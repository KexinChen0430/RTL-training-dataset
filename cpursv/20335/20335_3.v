        ...
        if (txReset) 
          ...
          begin
            if (~isBusy && ready_i) 
              ...
              begin
                if (~sendingCredit && (escPending && sendingTimecode) && 
                ...
                if (sendingTimecode && 
                    (sendingData && 
(~sendingCredit && (~escPending && ~sendingIdle)))) 
                  begin
                    sendingTimecode <= 0;
                    sendingData <= 0;
                  end
                ...
                if (~escPending && ~sendingIdle && 
                    (~sendingData && (~sendingTimecode && sendingCredit))) 
                  begin
                    sendingCredit <= 0;
                  end
                  
                if (escPending && ~sendingData && 
                    (sendingIdle && (~sendingCredit && ~sendingTimecode))) 
                  begin
                    escPending <= 0;
                    sendingCredit <= 1;
                    dat_o <= 9'b1_0000_0000;
                ...
                if (~sendingTimecode && ~escPending && 
                    (sendingCredit && sendingIdle && ~sendingData)) 
                  begin
                    sendingCredit <= 0;
                    sendingIdle <= 0;
                  end
