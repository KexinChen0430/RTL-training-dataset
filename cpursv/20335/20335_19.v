        ...
        if (txReset) 
          ...
          begin
            if (~isBusy && ready_i) 
              ...
              begin
                if (escPending && (sendingTimecode && ~sendingIdle) && 
                ...
                if (sendingData && sendingTimecode && 
                    (~sendingIdle && ~sendingCredit) && ~escPending) 
                  begin
                    sendingTimecode <= 0;
                    sendingData <= 0;
                  end
                  
                if (~sendingIdle && ~sendingData && 
                    (~escPending && sendingCredit && ~sendingTimecode)) 
                  begin
                    sendingCredit <= 0;
                  end
                  
                if (~sendingData && 
                    (~sendingCredit && escPending && ~sendingTimecode && 
sendingIdle)) 
                  begin
                    escPending <= 0;
                    sendingCredit <= 1;
                ...
                if (sendingCredit && (sendingIdle && ~escPending) && 
                    (~sendingTimecode && ~sendingData)) 
                  begin
                    sendingCredit <= 0;
                    sendingIdle <= 0;
                  end
