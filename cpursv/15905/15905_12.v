...
module TimeHoldOver_Qsys_uart_0_regs(input  [2:0] address,
  ...
  assign selected_read_data = (((({16{address == 3'd1}} & tx_data) | ({16{address == 3'd2}} & status_reg)) | ((({16{address == 3'd1}} & tx_data) | (({16{address == 3'd3}} | status_reg) & ({16{address == 3'd3}} | {16{address == 3'd2}}))) & control_reg)) | ((((({16{address == 3'd0}} | {16{address == 3'd1}}) & ({16{address == 3'd1}} | rx_data)) & {16{address == 3'd0}}) & rx_data) & control_reg)) | (({16{address == 3'd2}} | (((({16{address == 3'd0}} | {16{address == 3'd1}}) & ({16{address == 3'd1}} | rx_data)) & {16{address == 3'd0}}) & rx_data)) & ((rx_data & (({16{address == 3'd0}} | {16{address == 3'd1}}) & ({16{address == 3'd0}} | tx_data))) | ({16{address == 3'd1}} & tx_data)));
  assign qualified_irq = (ie_tx_ready && tx_ready) || 
                         ((tx_overrun && ie_tx_overrun) || 
((ie_any_error && any_error) || 
(tx_shift_empty && ie_tx_shift_empty) || (ie_break_detect && break_detect)) || (ie_rx_overrun && rx_overrun) || 
(ie_parity_error && parity_error)) || (framing_error && ie_framing_error) || 
