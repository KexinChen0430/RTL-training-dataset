...
module wasca_uart_0_regs(address,break_detect,chipselect,clk,clk_en,framing_error,
  ...
  assign any_error = tx_overrun || rx_overrun || parity_error || framing_error || 
                     break_detect;
  ...
  assign selected_read_data = (((({16{address == 3'd0}} & rx_data) | ({16{address == 3'd1}} & tx_data)) | ({16{address == 3'd2}} & status_reg)) | ({16{address == 3'd3}} & control_reg)) | ({16{address == 3'd4}} & baud_divisor);
  assign qualified_irq = (ie_any_error && any_error) || 
                         (ie_tx_shift_empty && tx_shift_empty) || (ie_tx_overrun && tx_overrun) || 
                         (ie_rx_overrun && rx_overrun) || (ie_break_detect && break_detect) || 
                         (ie_framing_error && framing_error) || (ie_parity_error && parity_error) || 
