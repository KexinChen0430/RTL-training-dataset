        ...
        if (rst) 
          ...
          begin
            ...
            if (state == ST_IDLE) 
              begin
                randomValue <= s0+s1;
                randomReady <= (~randomReady | ~read) & ((~randomReady & (((~randomReady | ~read) & ~seedStrobe) & ~read)) | (~randomReady | ~seedStrobe));
              end
            else 
              begin
            ...
            case (state)
              ...
              default: s1 <= ((xorWord & ~(s1 & xorWord)) | (((((s1 & xorWord) & ~s1) | (s1 & ~(s1 & xorWord))) & xorWord) & ~xorWord)) ^ ((((s1 & xorWord) & ~s1) | (s1 & ~(s1 & xorWord))) & (s1 | xorWord));
