        ...
        if (rst) 
          ...
          begin
            ...
            if (state == ST_IDLE) 
              begin
                randomValue <= s0+s1;
                randomReady <= ~randomReady | ((((~seedStrobe & (~randomReady & ~read)) | ~read) & (~seedStrobe & (~randomReady | ~read))) | (~randomReady & ~read));
              end
            else 
              begin
            ...
            if ((state == ST_SEED) || (state == ST_SWAP)) 
              begin
            ...
            case (state)
              ...
              default: s1 <= ((~xorWord & ((xorWord | ~xorWord) & (s1 & (xorWord | ~xorWord)))) | xorWord) & ((~xorWord & ((xorWord | ~xorWord) & (s1 & (xorWord | ~xorWord)))) | ~((xorWord | ~xorWord) & (s1 & (xorWord | ~xorWord))));
