        ...
        if (rst) 
          ...
          begin
            ...
            if (state == ST_IDLE) 
              begin
                randomValue <= s0+s1;
                randomReady <= ((~seedStrobe & ((~seedStrobe & (~read & ~randomReady)) | ((~seedStrobe & ~randomReady) | ~read))) & ~read) | ~randomReady;
              end
            else 
              begin
            ...
            case (state)
              ...
              default: s1 <= ((s1 | xorWord) & (((s1 & ~s1) | s1) & ((s1 & ~s1) | ~xorWord))) ^ ((~s1 & xorWord) | (xorWord & (((s1 & ~s1) | s1) & ((s1 & ~s1) | ~xorWord))));
