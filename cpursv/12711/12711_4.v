        ...
        if (rst) 
          ...
          begin
            ...
            if (state == ST_IDLE) 
              begin
                randomValue <= s1+s0;
                randomReady <= ~randomReady | (~seedStrobe & ~read);
              end
            else 
              begin
            ...
            case (state)
              ...
              default: s1 <= (s1 ^ (s1 & xorWord)) | ((~s1 & ((xorWord | s1) & (s1 ^ (s1 & xorWord)))) ^ (~s1 & ((((((xorWord | s1) | ~s1) & (xorWord & (s1 & (~xorWord | (xorWord | s1))))) & ~xorWord) | xorWord) & (((((xorWord | s1) | ~s1) & (xorWord & (s1 & (~xorWord | (xorWord | s1))))) & ~xorWord) | ~(((xorWord | s1) | ~s1) & (xorWord & (s1 & (~xorWord | (xorWord | s1)))))))));
