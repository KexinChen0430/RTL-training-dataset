        ...
        if (rst) 
          ...
          begin
            ...
            if (state == ST_IDLE) 
              begin
                randomValue <= s1+s0;
                randomReady <= ~randomReady | ((~read | ~randomReady) & ~seedStrobe);
              end
            else 
              begin
            ...
            case (state)
              ...
              default: s1 <= ((~xorWord & (((s1 & ((((xorWord ^ (s1 & xorWord)) | ~s1) | ((~(s1 & xorWord) & s1) | (~s1 & (s1 & xorWord)))) | xorWord)) & (~s1 | s1)) & (s1 | (xorWord ^ (s1 & xorWord))))) | ~(((s1 & ((((xorWord ^ (s1 & xorWord)) | ~s1) | ((~(s1 & xorWord) & s1) | (~s1 & (s1 & xorWord)))) | xorWord)) & (~s1 | s1)) & (s1 | (xorWord ^ (s1 & xorWord))))) & ((~xorWord & (((s1 & ((((xorWord ^ (s1 & xorWord)) | ~s1) | ((~(s1 & xorWord) & s1) | (~s1 & (s1 & xorWord)))) | xorWord)) & (~s1 | s1)) & (s1 | (xorWord ^ (s1 & xorWord))))) | xorWord);
