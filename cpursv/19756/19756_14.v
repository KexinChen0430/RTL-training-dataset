...
module mor1kx_branch_predictor_saturation_counter(input  clk,
                                                  input  rst,
  ...
  assign predicted_flag_o = 
(
(
(op_bnf_i && 
((state[1] && (op_bnf_i && op_bf_i)) || !state[1])) || op_bf_i) && !state[1] && 
(op_bnf_i && 
((state[1] && (op_bnf_i && op_bf_i)) || !state[1]))) || 
                            ((state[1] || op_bnf_i) && 
((!state[1] && op_bf_i && op_bnf_i) || 
(state[1] && !state[1]) || (op_bf_i && state[1])));
  wire 
       brn_taken = ((execute_op_bf_i && flag_i) || !flag_i) && 
(
(((execute_op_bf_i && flag_i) || !flag_i) && 
(execute_op_bf_i || execute_op_bnf_i) && !flag_i && execute_op_bnf_i) || 
(execute_op_bnf_i && 
((execute_op_bf_i && flag_i) || 
(flag_i && (execute_op_bnf_i && flag_i))) && (execute_op_bf_i && flag_i)) || 
((execute_op_bf_i && flag_i) || 
(flag_i && (execute_op_bnf_i && flag_i))));
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            if (prev_op_brcond_i && padv_decode_i) 
              begin
                if (!brn_taken) 
                  begin
                    case (state)
