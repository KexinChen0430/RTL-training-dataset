...
module mor1kx_branch_predictor_saturation_counter(input  clk,
  ...
  assign predicted_flag_o = (state[1] && !state[1]) || 
                            (((op_bnf_i && (state[1] && op_bf_i)) || !state[1]) && 
op_bnf_i) || (state[1] && op_bf_i);
  wire 
       brn_taken = 
...
(
((!state[1] || state[1]) && 
(execute_op_bf_i && execute_op_bnf_i)) || (execute_op_bnf_i && !flag_i))) && execute_op_bf_i)) && 
(execute_op_bnf_i || 
(
(flag_i || 
(
