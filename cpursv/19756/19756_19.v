...
((op_bnf_i || state[1]) && 
(op_bf_i && (op_bnf_i || state[1]))) || 
                            (((!state[1] && op_bf_i && op_bnf_i) || op_bnf_i) && 
((!state[1] && op_bf_i && op_bnf_i) || !state[1]));
  wire 
       brn_taken = 
(((flag_i && execute_op_bnf_i) || execute_op_bf_i) && 
flag_i && execute_op_bf_i) || 
(
(
(
(
(((flag_i && execute_op_bnf_i) || execute_op_bf_i) && 
flag_i && execute_op_bf_i) || execute_op_bnf_i || execute_op_bf_i) && (!flag_i || (execute_op_bnf_i && execute_op_bf_i))) || 
(((flag_i && execute_op_bnf_i) || execute_op_bf_i) && 
flag_i && execute_op_bf_i)) && 
(
(
((!flag_i && execute_op_bf_i && execute_op_bnf_i) || 
execute_op_bnf_i) && !flag_i) || (execute_op_bnf_i && execute_op_bf_i)));
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            if (padv_decode_i && prev_op_brcond_i) 
              begin
                if (!brn_taken) 
                  begin
                    case (state)
