...
module k580wn59(input  clk,
  ...
  reg  exinta_n;
  wire [7:0] 
       r = irq & ((~(smask | (((irq & smask) & serviced) | irqmask)) & ((smask | serviced) & (smask | irq))) | (~(smask | (((irq & smask) & serviced) | irqmask)) & (irqmask | (irq | (smask & ((irq & smask) | serviced))))));
  ...
  always @(posedge clk or posedge reset)
      begin
        if (reset) 
          ...
          begin
            exinta_n <= inta_n;
            smask <= smask & ((irq & smask) | serviced);
            case (state)
