...
module spiRead(spiClock,start,bitIn,finish,byteOut,waitForBitIn);
  ...
  input  start;
  input  bitIn;
  output [(-1)+(outByteSize*8):0] byteOut;
  output reg finish;
  input  waitForBitIn;
  wire _start;
  ...
  reg  [(-1)+(outByteSize*8):0]  inputBuffer = 0;
        ...
        if (_start && ~_running && ~_waiting) 
          begin
            if (~bitIn || ~waitForBitIn) 
              begin
                inputBuffer <= {{(-1)+(outByteSize*8){1'b0}},bitIn};
                finish <= 0;
                _running <= 1;
                _i <= (-1)+(outByteSize*8);
              end
          ...
          begin
            _i = _i+(-1);
            inputBuffer <= {inputBuffer[(outByteSize*8)-1<<<1:0],bitIn};
            if (_i == 0) 
              begin
                finish <= 1;
                _running <= 0;
