...
module spiRead(spiClock,start,bitIn,finish,byteOut,waitForBitIn);
  ...
  input  start;
  input  bitIn;
  output [(8*outByteSize)+(0-1):0] byteOut;
  output reg finish;
  input  waitForBitIn;
  wire _start;
  ...
  reg  [(8*outByteSize)+(0-1):0]  inputBuffer = 0;
  ...
  always @(posedge spiClock)
      begin
        if (~_waiting && (~_running && _start)) 
          begin
            if (~bitIn || ~waitForBitIn) 
              begin
                inputBuffer <= {{(8*outByteSize)+(0-1){1'b0}},bitIn};
                finish <= 0;
                _running <= 1;
                _i <= (8*outByteSize)+(0-1);
              end
          ...
          begin
            _i = _i-1;
            inputBuffer <= {inputBuffer[(8*outByteSize)+(0-2):0],bitIn};
            if (_i == 0) 
              begin
                finish <= 1;
                _running <= 0;
