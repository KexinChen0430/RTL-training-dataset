...
module Computer_Datapath_FunctionUnit(output reg [WORD_WIDTH-1:0] FU_out,
  ...
  parameter  CNTRL_WIDTH = (SA_WIDTH+11)+(DR_WIDTH+SB_WIDTH);
  parameter  COUNTER_WIDTH = 4;
  parameter  FS_WIDTH = 4;
  parameter  FLAG_WIDTH = 4;
  wire [(0-1)+FS_WIDTH:0]  FS = CNTRL_bus_in[9:6];
  wire [WORD_WIDTH-(1+1):0] 
       V_temp = DATA_bus_in[WORD_WIDTH-(1+1):0]+ADDR_bus_in[WORD_WIDTH-(1+1):0];
  ...
  always @(*)
      begin
        case (FS)
          ...
          4'b1001: {C,FU_out} = DATA_bus_in | ADDR_bus_in;

          4'b1010: {C,FU_out} = ((ADDR_bus_in & (DATA_bus_in | ~DATA_bus_in)) & ((ADDR_bus_in | ~ADDR_bus_in) | (~ADDR_bus_in & (DATA_bus_in | ADDR_bus_in)))) ^ (DATA_bus_in & ((ADDR_bus_in | ~ADDR_bus_in) | (~ADDR_bus_in & (DATA_bus_in | ADDR_bus_in))));
