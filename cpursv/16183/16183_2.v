...
module Computer_Datapath_FunctionUnit(output reg [(-1)+WORD_WIDTH:0] FU_out,
                                      output [FLAG_WIDTH+(-1):0] FLAG_bus_out,
                                      input  [(-1)+WORD_WIDTH:0] ADDR_bus_in,DATA_bus_in,
  ...
  parameter  CNTRL_WIDTH = ((SA_WIDTH+DR_WIDTH)+SB_WIDTH)+11;
  parameter  COUNTER_WIDTH = 4;
  parameter  FS_WIDTH = 4;
  parameter  FLAG_WIDTH = 4;
  wire [(-1)+FS_WIDTH:0]  FS = CNTRL_bus_in[9:6];
  wire [WORD_WIDTH+(0-2):0] 
       V_temp = DATA_bus_in[WORD_WIDTH+(0-2):0]+ADDR_bus_in[WORD_WIDTH+(0-2):0];
  wire  V = V_temp[WORD_WIDTH+(0-2):0];
  wire  N = FU_out[(-1)+WORD_WIDTH];
  ...
  always @(*)
      begin
        case (FS)
          ...
          4'b1001: {C,FU_out} = ADDR_bus_in | ((ADDR_bus_in | (~ADDR_bus_in | ~DATA_bus_in)) & DATA_bus_in);

          4'b1010: {C,FU_out} = DATA_bus_in ^ ADDR_bus_in;
