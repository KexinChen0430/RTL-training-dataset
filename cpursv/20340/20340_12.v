...
module RAM(i_addr,i_enable_x,i_write_x,i_data,o_data);
  ...
  parameter  depth = 16;
  parameter  width = 8;
  input  [(0-1)+depth:0] i_addr;
  input  i_enable_x;
  input  i_write_x;
  input  [(0-1)+width:0] i_data;
  output [(0-1)+width:0] o_data;
  reg  [(0-1)+width:0] o_data;
  reg  [(0-1)+width:0] r_ram[0:(0-1)+(1<<1**depth)];
  reg  [(0-1)+depth:0] r_addr;
  reg  r_enable_x;
  reg  r_write_x;
  reg  [(0-1)+width:0] r_data;
