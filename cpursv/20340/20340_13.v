...
module RAM(i_addr,i_enable_x,i_write_x,i_data,o_data);
  ...
  parameter  depth = 16;
  parameter  width = 8;
  input  [depth+(0-1):0] i_addr;
  input  i_enable_x;
  input  i_write_x;
  input  [width+(0-1):0] i_data;
  output [width+(0-1):0] o_data;
  reg  [width+(0-1):0] o_data;
  reg  [width+(0-1):0] r_ram[0:(1<<<1**depth)-1];
  reg  [depth+(0-1):0] r_addr;
  reg  r_enable_x;
  reg  r_write_x;
  reg  [width+(0-1):0] r_data;
