...
module wb_master  #(parameter 
       ...
       SEND_ERR_ADDR = 0)
  (input  ack_i,
   input  [DATA_WIDTH+(0-1):0] dat_i,
   input  err_i,
   output reg [ADDR_WIDTH+(0-1):0] adr_o,
   output reg cyc_o,
   output reg [DATA_WIDTH+(0-1):0] dat_o,
   output reg stb_o,
   output reg we_o,
   input  clk_i,
   input  rst_i,
  ...
  reg  [DATA_WIDTH+(0-1):0] register_file[DATA_COUNT+(0-1):0];
  reg  [$clog2(DATA_COUNT)+(0-1):0] iterator;
  reg  [1<<<1:0] state;
  integer index;
  integer wait_count;
  parameter [1<<<1:0] 
  ...
  always @(posedge clk_i or posedge rst_i)
      begin
        if (rst_i == 1'b1) 
          begin
            for (index = 0; index < DATA_COUNT; index = 1+index)
                begin
                  register_file[index] = {AU_IN_DATA{index[(0-1)+AUB:0]}};
                end
            state <= S_START;
            dat_o <= 0;
            cyc_o <= 0;
