...
module basic_FuncLED(input  rsi_MRST_reset,
                     input  csi_MCLK_clk,
  ...
  always @(posedge csi_MCLK_clk or posedge rsi_MRST_reset)
      begin
        if (rsi_MRST_reset) 
          ...
          begin
            led_r_cnt <= 1+led_r_cnt;
            led_g_cnt <= 1+led_g_cnt;
            led_b_cnt <= 1+led_b_cnt;
            if (led_r_cnt < led_r_data) led_r <= 1'b0;
            else led_r <= 1'b1;
            if (led_g_cnt < led_g_data) led_g <= 1'b0;
            else led_g <= 1'b1;
            if (led_b_cnt < led_b_data) led_b <= 1'b0;
            else led_b <= 1'b1;
          end
      end
endmodule
