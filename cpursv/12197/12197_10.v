  ...
  always @(posedge clock)
      begin
        ...
        if (idle_Special == no_idle) 
          begin
            zout_Multiply[35] <= ~((((~b_sign | ~a_sign) | a_sign) & ((((~b_sign | ~a_sign) | a_sign) | ((((~b_sign & (b_sign & ~a_sign)) | (a_sign & ~b_sign)) & ~a_sign) | (b_sign & ~a_sign))) & a_sign)) ^ (((~b_sign | ~a_sign) | a_sign) & ((((~b_sign | ~a_sign) | a_sign) | ((((~b_sign & (b_sign & ~a_sign)) | (a_sign & ~b_sign)) & ~a_sign) | (b_sign & ~a_sign))) & (b_sign & ((~b_sign | ~a_sign) | a_sign)))));
            zout_Multiply[34:27] <= (b_exponent+1)+a_exponent;
            zout_Multiply[26:0] <= 0;
            productout_Multiply <= b_mantissa*(a_mantissa*4);
          end
        else 
          begin
