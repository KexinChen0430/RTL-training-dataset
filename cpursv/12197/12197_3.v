  ...
  always @(posedge clock)
      begin
        ...
        if (idle_Special == no_idle) 
          begin
            zout_Multiply[35] <= ~((~a_sign & ((((b_sign | (a_sign & (b_sign | ~b_sign))) | ~a_sign) & ((a_sign & b_sign) | (~a_sign & b_sign))) & (b_sign | (a_sign & (b_sign | ~b_sign))))) | (a_sign & ~((((b_sign | (a_sign & (b_sign | ~b_sign))) | ~a_sign) & ((a_sign & b_sign) | (~a_sign & b_sign))) & (b_sign | (a_sign & (b_sign | ~b_sign))))));
            zout_Multiply[34:27] <= a_exponent+(1+b_exponent);
            zout_Multiply[26:0] <= 0;
            productout_Multiply <= a_mantissa*(4*b_mantissa);
          end
        else 
          begin
