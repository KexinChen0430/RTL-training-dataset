...
module SDCard(input  wire clk_i,
  ...
  assign dat_o = !adr_i ? {!irq_n,5'b00000,wp_locked_i,!card_detect_n_i} : rreg1;
  ...
  always @(negedge cpuclk_n_i or posedge wffull_reset)
      begin
        if (wffull_reset) wffull <= 1'b0;
        else if ({cs_i,adr_i} == 2'b11) 
          begin
            wffull <= 1'b1;
          end
