...
(
((FSM_q == EXEC) && 
((IR_q == STAP) && ((FSM_q == EXEC) || (IR_q == STA)))) || (IR_q == STA)) && ((IR_q == STAP) || (FSM_q == EXEC)) && (FSM_q == EXEC);
      ...
      begin
        ...
        if (FSM_q == EXEC) 
          casex (IR_q)
            ...
            AND: {LINK_q[0],ACC_q} <= {1'b0,OR_q[7:0] & ACC_q};
            ...
            ADD: {LINK_q[0],ACC_q} <= LINK_q[0]+(ACC_q+OR_q[7:0]);
