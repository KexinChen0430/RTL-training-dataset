...
module flash_writer(input  p_ready,
                    input  WE_CLK,
                    input  iFLASH_RY_N,
                    input  iOSC_28,
                    input  iERASE,
  ...
  always @(posedge iRESET_N or posedge ck_prog)
      begin
        if (iRESET_N) 
          begin
          ...
          begin
            case (ST_P)
              ...
              7: begin
                    if (addr_prog == end_address) ST_P <= 9;
                    else 
                      begin
                        addr_prog <= 1+addr_prog;
                        ST_P <= 0;
                      end
                  end
  ...
  always @(posedge iRESET_N or posedge ck_read)
      begin
        if (iRESET_N) 
          begin
        ...
        else if (addr_read < end_address) addr_read = addr_read+1;
  ...
  wire 
       verify_tr = (addr_read < end_address) ? ~ck_read : 0;

  assign oFLASH_TR = (porgram_tr | erase_tr) | verify_tr;
  assign oREAD_PRO_END = end_prog & end_read;
