...
module niosII_system_button_shift_up(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((data_in & ({1{address == 0}} & irq_mask)) | ((data_in & ({1{address == 0}} & irq_mask)) | ({1{address == 2}} & irq_mask))) | ((data_in & {1{address == 0}}) | (((edge_capture | ({1{address == 0}} & (irq_mask | data_in))) | (({1{address == 2}} & ((irq_mask | data_in) & ({1{address == 0}} | irq_mask))) | irq_mask)) & ((((({1{address == 0}} | {1{address == 2}}) & data_in) | edge_capture) | {1{address == 2}}) & {1{address == 3}})));
