...
module lut  #(parameter 
  ...
  generate
      ...
      for (i = 1; i < (MEM_SIZE/CONFIG_WIDTH); i = 1+i)
          begin
            ...
            always @(posedge config_clk)
                begin
                  if (config_en) 
                    mem[((1+i)*CONFIG_WIDTH)-1:CONFIG_WIDTH*i] <= mem[(CONFIG_WIDTH*i)-1:((0-1)+i)*CONFIG_WIDTH];
      ...
      assign config_out = mem[(0-1)+MEM_SIZE:(-CONFIG_WIDTH)+MEM_SIZE];
  endgenerate
