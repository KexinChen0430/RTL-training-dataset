...
module eth_rxcounters(MRxClk,Reset,MRxDV,StateIdle,StateSFD,StateData,StateDrop,
  ...
  assign ResetByteCounter = (((MRxDV & MRxDEqD) & (StateSFD & MRxDV)) | ((MRxDV & MRxDEqD) & (StateSFD & MRxDV))) | (ByteCntMaxFrame & (StateData[0] & MRxDV));
  assign IncrementByteCounter = ((((StateIdle | (~ByteCntMax & (StateData[1] & ~(DlyCrcEn & |DlyCrcCnt)))) & ~Transmitting) | (~ByteCntMax & (StateData[1] & ~(DlyCrcEn & |DlyCrcCnt)))) & (~ResetByteCounter & MRxDV)) | (((~ResetByteCounter & MRxDV) & StateSFD) | (((~ByteCntMax & (StateData[1] & ~(DlyCrcEn & |DlyCrcCnt))) | StatePreamble) & (~ResetByteCounter & MRxDV)));
  ...
  assign ResetIFGCounter = (MRxDV & (StateDrop | (MRxDEqD & StateSFD))) | StateDrop;
  assign IncrementIFGCounter = ((((~ResetIFGCounter & StateDrop) & ~IFGCounterEq24) | ((StateIdle & ~ResetIFGCounter) & ~IFGCounterEq24)) | (~IFGCounterEq24 & (StateSFD & ~ResetIFGCounter))) | (StatePreamble & (~ResetIFGCounter & ~IFGCounterEq24));
      ...
      begin
        if (Reset) DlyCrcCnt[3:0] <= 4'h0;
          ...
          begin
            ...
            else if (DlyCrcEn & |DlyCrcCnt[3:0]) DlyCrcCnt[3:0] <= DlyCrcCnt[3:0]+4'd1;
