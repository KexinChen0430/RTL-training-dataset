...
module eth_rxcounters(MRxClk,Reset,MRxDV,StateIdle,StateSFD,StateData,StateDrop,
  ...
  assign ResetByteCounter = (((MRxDEqD & StateSFD) | ((StateData[0] | StateSFD) & MRxDEqD)) | StateData[0]) & ((((ByteCntMaxFrame | MRxDEqD) & StateSFD) | ByteCntMaxFrame) & MRxDV);
  assign IncrementByteCounter = ((StateSFD & (~ResetByteCounter & MRxDV)) | ((~ResetByteCounter & MRxDV) & (StatePreamble | ((StateData[1] & ((((StateData[1] & ~ByteCntMax) | StateIdle) | (StatePreamble | StateSFD)) & ((StateIdle | (StatePreamble | StateSFD)) | (~(DlyCrcEn & |DlyCrcCnt) | StateSFD)))) & (~(DlyCrcEn & |DlyCrcCnt) & ~ByteCntMax))))) | ((StateData[1] & (~ByteCntMax & (~(DlyCrcEn & |DlyCrcCnt) & (~ResetByteCounter & MRxDV)))) | ((StateIdle & (~ResetByteCounter & MRxDV)) & ~Transmitting));
  ...
  assign ResetIFGCounter = StateDrop | (((MRxDV & MRxDEqD) | StateDrop) & StateSFD);
  assign IncrementIFGCounter = ((~IFGCounterEq24 & ~ResetIFGCounter) & StateSFD) | (((((StatePreamble & ~ResetIFGCounter) & ~IFGCounterEq24) | (~ResetIFGCounter & StateIdle)) | (StateDrop & ~ResetIFGCounter)) & ~IFGCounterEq24);
      ...
      begin
        if (Reset) DlyCrcCnt[3:0] <= 4'h0;
          ...
          begin
            ...
            else if (DlyCrcEn & StateSFD) DlyCrcCnt[3:0] <= 4'h1;
            else if (|DlyCrcCnt[3:0] & DlyCrcEn) DlyCrcCnt[3:0] <= DlyCrcCnt[3:0]+4'd1;
