...
module eth_rxcounters(MRxClk,Reset,MRxDV,StateIdle,StateSFD,StateData,StateDrop,
  ...
  assign ResetByteCounter = ((StateSFD | ByteCntMaxFrame) & (((MRxDEqD & (((StateSFD | ByteCntMaxFrame) & (ByteCntMaxFrame | MRxDEqD)) & StateSFD)) | StateSFD) | StateData[0])) & ((MRxDEqD | (ByteCntMaxFrame & (StateData[0] & ((StateSFD | ByteCntMaxFrame) & (((MRxDEqD & (((StateSFD | ByteCntMaxFrame) & (ByteCntMaxFrame | MRxDEqD)) & StateSFD)) | StateSFD) | StateData[0]))))) & MRxDV);
  assign IncrementByteCounter = (((StateIdle & ~Transmitting) | StateData[1]) | (StatePreamble | StateSFD)) & ((((~ByteCntMax & ~(|DlyCrcCnt & DlyCrcEn)) | StatePreamble) | ((StateSFD | (StatePreamble | (StateIdle & ~Transmitting))) & (((~ByteCntMax & StateData[1]) | (StateIdle & ~Transmitting)) | StateSFD))) & (MRxDV & ~ResetByteCounter));
  ...
  assign ResetIFGCounter = (MRxDEqD | StateDrop) & (((StateDrop | (((MRxDV & ~ResetByteCounter) & ~(|DlyCrcCnt & DlyCrcEn)) & (~ByteCntMax & StateData[1]))) | MRxDV) & (StateSFD | StateDrop));
  assign IncrementIFGCounter = ((~IFGCounterEq24 & ~ResetIFGCounter) & (StateDrop | (StateIdle | StatePreamble))) | (~ResetIFGCounter & (~IFGCounterEq24 & StateSFD));
