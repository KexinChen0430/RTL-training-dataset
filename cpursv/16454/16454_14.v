...
module eth_rxcounters(MRxClk,Reset,MRxDV,StateIdle,StateSFD,StateData,StateDrop,
  ...
  assign ResetByteCounter = ((ByteCntMaxFrame & StateData[0]) | MRxDEqD) & (((ByteCntMaxFrame & StateData[0]) & MRxDV) | (StateSFD & MRxDV));
  assign IncrementByteCounter = ((((StateIdle & (~ResetByteCounter & MRxDV)) | (StateSFD & (~ResetByteCounter & MRxDV))) | ((StatePreamble | (StateData[1] & (~ByteCntMax & ~(DlyCrcEn & |DlyCrcCnt)))) & (~ResetByteCounter & MRxDV))) & ((~Transmitting | StatePreamble) | ((StateSFD | StatePreamble) & (StateSFD | StateIdle)))) | (~ResetByteCounter & (MRxDV & (StateData[1] & (~ByteCntMax & ~(DlyCrcEn & |DlyCrcCnt)))));
  ...
  assign ResetIFGCounter = ((StateDrop | MRxDV) & (MRxDEqD | StateDrop)) & (StateDrop | StateSFD);
  assign IncrementIFGCounter = ((~ResetIFGCounter & (StateSFD & ~IFGCounterEq24)) | (~IFGCounterEq24 & ((StatePreamble | StateDrop) & ~ResetIFGCounter))) | (~IFGCounterEq24 & (~ResetIFGCounter & StateIdle));
      ...
      begin
        if (Reset) DlyCrcCnt[3:0] <= 4'h0;
          ...
          begin
            ...
            else if (DlyCrcEn & |DlyCrcCnt[3:0]) DlyCrcCnt[3:0] <= 4'd1+DlyCrcCnt[3:0];
