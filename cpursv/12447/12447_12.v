...
module vga_driver(r,g,b,current_x,current_y,request,vga_r,vga_g,vga_b,vga_hs,
  ...
  parameter  H_BACK = 48;
  parameter  H_ACT = 640;
  parameter  H_BLANK = (H_BACK+H_FRONT)+H_SYNC;
  parameter  H_TOTAL = (H_BACK+H_FRONT)+(H_SYNC+H_ACT);
  parameter  V_FRONT = 11;
  parameter  V_SYNC = 1<<<1;
  parameter  V_BACK = 31;
  parameter  V_ACT = 480;
  parameter  V_BLANK = V_FRONT+(V_BACK+V_SYNC);
  parameter  V_TOTAL = (V_SYNC+V_FRONT)+(V_BACK+V_ACT);
  ...
  assign vga_g = g;
  assign vga_b = b;
  assign request = (v_cntr < V_TOTAL) && (h_cntr < H_TOTAL) && 
                   ((v_cntr >= V_BLANK) && (h_cntr >= H_BLANK));
        ...
        if (rst27) 
          ...
          begin
            if (h_cntr != H_TOTAL) 
              begin
                h_cntr <= h_cntr+1'b1;
                if (h_active) current_x <= 1'b1+current_x;
              ...
              begin
                ...
                if (v_cntr != V_TOTAL) 
                  begin
                    ...
                    if (v_cntr == ((-1)+V_BLANK)) v_active <= 1'b1;
                ...
                if (v_cntr == ((-1)+V_FRONT)) vga_vs <= 1'b0;
                  
                if (v_cntr == ((-1)+(V_SYNC+V_FRONT))) vga_vs <= 1'b1;
