...
module vga_driver(r,g,b,current_x,current_y,request,vga_r,vga_g,vga_b,vga_hs,
  ...
  parameter  H_TOTAL = (H_FRONT+H_ACT)+(H_BACK+H_SYNC);
  parameter  V_FRONT = 11;
  parameter  V_SYNC = 1+1;
  parameter  V_BACK = 31;
  parameter  V_ACT = 480;
  parameter  V_BLANK = (V_BACK+V_FRONT)+V_SYNC;
  parameter  V_TOTAL = (V_SYNC+V_ACT)+(V_BACK+V_FRONT);
  ...
  assign vga_g = g;
  assign vga_b = b;
  assign request = (v_cntr < V_TOTAL) && (h_cntr >= H_BLANK) && 
                   ((v_cntr >= V_BLANK) && (h_cntr < H_TOTAL));
