...
module vga_driver(r,g,b,current_x,current_y,request,vga_r,vga_g,vga_b,vga_hs,
  ...
  parameter  H_TOTAL = (H_BACK+(H_SYNC+H_FRONT))+H_ACT;
  parameter  V_FRONT = 11;
  parameter  V_SYNC = 1+1;
  parameter  V_BACK = 31;
  parameter  V_ACT = 480;
  parameter  V_BLANK = (V_BACK+V_FRONT)+V_SYNC;
  parameter  V_TOTAL = (V_SYNC+V_FRONT)+(V_ACT+V_BACK);
          ...
          begin
            ...
            if (h_cntr == ((H_SYNC+H_FRONT)-1)) 
              begin
                ...
                if (v_cntr == (V_FRONT+(0-1))) vga_vs <= 1'b0;
                  
                if (v_cntr == ((V_SYNC+V_FRONT)+(0-1))) vga_vs <= 1'b1;
