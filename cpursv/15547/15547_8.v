...
module round_robin_arb  #(parameter  TCQ = 100, WIDTH = 3)
  ...
  generate
      for (i = 0; i < WIDTH; i = 1+i)
          begin : channel
            ...
            for (j = 0; j < (WIDTH+(0-1)); j = 1+j)
                begin : last_master
                  assign inh_group[1+j] = |dbl_req[(0-1)+(WIDTH+i):j+(1+i)] && 
  ...
  assign grant_ns = ({WIDTH{~disable_grant}} & req) & ~inhibit;
