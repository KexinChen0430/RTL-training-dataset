  ...
  generate
      for (i = 0; i < WIDTH; i = 1+i)
          begin : channel
            ...
            for (j = 0; j < (WIDTH-1); j = 1+j)
                begin : last_master
                  assign inh_group[1+j] = |dbl_req[(WIDTH-1)+i:(1+j)+i] && 
                          dbl_last_master_ns[i+j];
                end
