...
module arriaii_saturate_block(datain,saturate,round,signa,signb,datain_width,dataout,
                              saturation_overflow);
...
(operation_mode == accumulator))) ? datain[accum_width] : 
               (operation_mode == two_level_adder) ? datain[1+dataa_width] : 
               ((operation_mode == one_level_adder) || 
               (operation_mode == loopback)) ? datain[dataa_width] : datain[dataa_width+(0-1)];
          ...
          begin
            ...
            if (saturate_mode == symmetric) 
              begin
                ...
                for (i = datain_width+(-round_width); i < (datain_width+(0-saturate_width)); i = i+1)
                    begin
            ...
            if (saturate_mode == asymmetric) 
              begin
                for (i = 0; i < (datain_width+(0-saturate_width)); i = i+1)
                    begin
                      max[i] = 1'b1;
                      min[i] = 1'b0;
                    end
            ...
            if (saturate_width == 1) 
              begin
                if (msb != datain[datain_width+(0-1)]) saturation_overflow_tmp = 1'b1;
                else sign_tmp = sign_tmp & datain[datain_width+(0-1)];
              end
            else 
              begin
                for (i = datain_width+(0-saturate_width); i < datain_width; i = i+1)
                    begin
                      sign_tmp = sign_tmp & datain[i];
                      if (datain[datain_width+(0-1)] != datain[i]) saturation_overflow_tmp = 1'b1;
                        
                    end
              end
            if ((sign_tmp == 1'b1) && (data_tmp == 1'b0) && 
                (saturate_mode == symmetric)) saturation_overflow_tmp = 1'b1;
