        ...
        if (rst) 
          ...
          begin
            if ((MEMWBregisterRd != 0) && MEMWBregWrite && 
                ((MEMWBregisterRd == rs) && 
!((EXMEMregisterRd == rs) && 
((EXMEMregisterRd != 0) && EXMEMregWrite)))) forwardA <= 2'b01;
            else if ((EXMEMregisterRd == rs) && 
                     ((EXMEMregisterRd != 0) && EXMEMregWrite)) forwardA <= 2'b10;
            else forwardA <= 0;
            if (
!((EXMEMregisterRd == rt) && 
((EXMEMregisterRd != 0) && EXMEMregWrite)) && (MEMWBregisterRd != 0) && 
                ((MEMWBregisterRd == rt) && MEMWBregWrite)) forwardB <= 2'b01;
            else if ((EXMEMregisterRd == rt) && 
                     ((EXMEMregisterRd != 0) && EXMEMregWrite)) forwardB <= 2'b10;
            else forwardB <= 0;
          end
      end
endmodule
