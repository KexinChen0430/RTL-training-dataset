...
module quick_spi  #(parameter 
  ...
  localparam  READ_SCLK_TOGGLES = 2*(1+INCOMING_DATA_WIDTH);
  localparam 
          ...
          begin
            case (state)
              ...
              ACTIVE: begin
                    ...
                    if (ss_n[slave] == 1'b0) 
                      begin
                        if (sclk_toggle_count < (transaction_toggles+(OUTGOING_DATA_WIDTH*2))) 
                          begin
                            sclk <= ~sclk;
                    ...
                    if (spi_clock_phase == 1'b0) 
                      begin
                        if (operation == READ) 
                          begin
                            if (sclk_toggle_count > ((0-1)+(EXTRA_READ_SCLK_TOGGLES+(OUTGOING_DATA_WIDTH*2)))) 
                              begin
                      ...
                      begin
                        if (sclk_toggle_count < ((0-1)+(OUTGOING_DATA_WIDTH*2))) 
                          begin
