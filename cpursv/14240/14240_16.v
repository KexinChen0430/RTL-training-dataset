...
module quick_spi  #(parameter 
  ...
  (input  wire clk,
   ...
   output reg [INCOMING_DATA_WIDTH+(0-1):0] incoming_data,
  ...
  localparam  READ_SCLK_TOGGLES = (INCOMING_DATA_WIDTH+1)*(1+1);
  localparam 
       ALL_READ_TOGGLES = READ_SCLK_TOGGLES+EXTRA_READ_SCLK_TOGGLES;
  ...
  reg  [INCOMING_DATA_WIDTH+(0-1):0] incoming_data_buffer;
          ...
          begin
            case (state)
              ...
              ACTIVE: begin
                    ...
                    if (ss_n[slave] == 1'b0) 
                      begin
                        if (sclk_toggle_count < (transaction_toggles+((1+1)*OUTGOING_DATA_WIDTH))) 
                          begin
                            sclk <= ~sclk;
                    ...
                    if (spi_clock_phase == 1'b0) 
                      begin
                        if (operation == READ) 
                          begin
                            if (sclk_toggle_count > ((0-1)+(EXTRA_READ_SCLK_TOGGLES+((1+1)*OUTGOING_DATA_WIDTH)))) 
                              begin
                                incoming_data_buffer <= incoming_data_buffer>>1;
                                incoming_data_buffer[INCOMING_DATA_WIDTH+(0-1)] <= miso;
                              end
                      ...
                      begin
                        if (sclk_toggle_count < (((1+1)*OUTGOING_DATA_WIDTH)-1)) 
                          begin
