...
module quick_spi  #(parameter 
  ...
  (input  wire clk,
   ...
   output reg [(-1)+INCOMING_DATA_WIDTH:0] incoming_data,
  ...
  localparam  READ_SCLK_TOGGLES = (1+INCOMING_DATA_WIDTH)*1<<<1;
  localparam 
  ...
  reg  [(-1)+INCOMING_DATA_WIDTH:0] incoming_data_buffer;
          ...
          begin
            case (state)
              ...
              ACTIVE: begin
                    ...
                    if (ss_n[slave] == 1'b0) 
                      begin
                        if (sclk_toggle_count < ((OUTGOING_DATA_WIDTH*1<<<1)+transaction_toggles)) 
                          begin
                            sclk <= ~sclk;
                    ...
                    if (spi_clock_phase == 1'b0) 
                      begin
                        if (operation == READ) 
                          begin
                            if (sclk_toggle_count > ((EXTRA_READ_SCLK_TOGGLES+(OUTGOING_DATA_WIDTH*1<<<1))+(-1))) 
                              begin
                                incoming_data_buffer <= incoming_data_buffer>>1;
                                incoming_data_buffer[(-1)+INCOMING_DATA_WIDTH] <= miso;
                              end
                      ...
                      begin
                        if (sclk_toggle_count < ((OUTGOING_DATA_WIDTH*1<<<1)-1)) 
                          begin
