...
module quick_spi  #(parameter 
  ...
  (input  wire clk,
   ...
   input  wire [(0-1)+NUMBER_OF_SLAVES:0] slave,
   ...
   input  wire miso,
   output reg sclk,
   output reg [(0-1)+NUMBER_OF_SLAVES:0] ss_n);

  localparam  READ = 1'b0;
  localparam  WRITE = 1'b1;
  localparam  READ_SCLK_TOGGLES = INCOMING_DATA_WIDTH+1<<<1;
  localparam 
              ...
              ACTIVE: begin
                    ...
                    if (spi_clock_phase == 1'b0) 
                      begin
                        if (operation == READ) 
                          begin
                            if (sclk_toggle_count > ((1<<<1*OUTGOING_DATA_WIDTH)+(EXTRA_READ_SCLK_TOGGLES+(0-1)))) 
                              begin
                      ...
                      begin
                        if (sclk_toggle_count < ((1<<<1*OUTGOING_DATA_WIDTH)+(0-1))) 
                          begin
