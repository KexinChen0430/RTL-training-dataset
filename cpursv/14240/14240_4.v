...
module quick_spi  #(parameter 
  ...
  localparam  READ_SCLK_TOGGLES = 2+INCOMING_DATA_WIDTH<<<1;
  localparam 
          ...
          begin
            case (state)
              ...
              ACTIVE: begin
                    ...
                    if (ss_n[slave] == 1'b0) 
                      begin
                        if (sclk_toggle_count < (transaction_toggles+(2*OUTGOING_DATA_WIDTH))) 
                          begin
                            sclk <= ~sclk;
                    ...
                    if (spi_clock_phase == 1'b0) 
                      begin
                        if (operation == READ) 
                          begin
                            if (sclk_toggle_count > (EXTRA_READ_SCLK_TOGGLES+((2*OUTGOING_DATA_WIDTH)-1))) 
                              begin
                      ...
                      begin
                        if (sclk_toggle_count < ((2*OUTGOING_DATA_WIDTH)-1)) 
                          begin
