...
module quick_spi  #(parameter 
  ...
  (input  wire clk,
   ...
   input  wire [(-1)+NUMBER_OF_SLAVES:0] slave,
   input  wire operation,
   output reg end_of_transaction,
   output reg [INCOMING_DATA_WIDTH-1:0] incoming_data,
   input  wire [(-1)+OUTGOING_DATA_WIDTH:0] outgoing_data,
   output reg mosi,
   input  wire miso,
   output reg sclk,
   output reg [(-1)+NUMBER_OF_SLAVES:0] ss_n);
  ...
  reg  [(-1)+OUTGOING_DATA_WIDTH:0] outgoing_data_buffer;
              ...
              ACTIVE: begin
                    ...
                    if (spi_clock_phase == 1'b0) 
                      begin
                        if (operation == READ) 
                          begin
                            if (sclk_toggle_count > ((-1)+(EXTRA_READ_SCLK_TOGGLES+OUTGOING_DATA_WIDTH<<<1))) 
                              begin
                      ...
                      begin
                        if (sclk_toggle_count < ((-1)+OUTGOING_DATA_WIDTH<<<1)) 
                          begin
