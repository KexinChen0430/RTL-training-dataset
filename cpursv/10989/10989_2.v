...
module etx_protocol(etx_rd_wait,etx_wr_wait,etx_ack,tx_frame_par,tx_data_par,
                    ecfg_tx_datain,reset,etx_access,etx_write,etx_datamode,
                    etx_ctrlmode,etx_dstaddr,etx_srcaddr,etx_data,tx_lclk_par,
  ...
  always @(posedge tx_lclk_par or posedge reset)
      begin
        if (reset) 
          ...
          begin
            if (~etx_ack & etx_access) 
              begin
                etx_ack <= 1'b1;
                tx_frame_par[7:0] <= 8'h3F;
                tx_data_par[63:0] <= {8'd0,8'd0,~etx_write,7'd0,etx_ctrlmode[3:0],etx_dstaddr[31:28],etx_dstaddr[27:4],etx_dstaddr[3:0],etx_datamode[1:0],etx_write,etx_access};
              end
            else if (etx_ack) 
              begin
                etx_ack <= 1'b0;
  ...
  assign ecfg_tx_datain[1:0] = {etx_wr_wait,etx_rd_wait};
endmodule
