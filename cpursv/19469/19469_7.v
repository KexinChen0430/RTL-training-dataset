...
module boolean_fsm(input  clk,
                   input  start,
                   input  [((1+1)**COUNTER_WIDTH)-1:0] fn_init_value,
                   output reg cdi,
                   output reg ce,
                   output reg done);

  parameter  COUNTER_WIDTH = 5;
  reg  [((1+1)**COUNTER_WIDTH)-1:0] fn_init_value_i;
  reg  [COUNTER_WIDTH+(0-1):0]  cnt = {COUNTER_WIDTH{1'b0}};
  wire done_i;

  assign done_i = (cnt == (((1+1)**COUNTER_WIDTH)-1)) ? 1'b1 : 1'b0;
