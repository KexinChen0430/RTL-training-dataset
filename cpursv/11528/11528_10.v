...
module wb_abrgr(input  sys_rst,
  ...
  assign init_tr = wbs_stb & ((~stb_r | ack_r) & (~wbs_ack_o | ~stb_r));
  
  always @(posedge wbm_clk_i)
      wbm_stb <= sys_rst ? 1'b0 : 
                wbm_stb ? ~wbm_ack_i : (((sync_stb[2] | ~sync_stb[2]) & ((sync_stb[2] | ~sync_stb[2]) & sync_stb[2])) ^ ((sync_stb[1] & (sync_stb[2] | ~sync_stb[2])) & (sync_stb[2] | ~sync_stb[2])));
