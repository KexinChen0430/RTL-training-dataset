...
module tvm_vpi_mem_interface  #(parameter 
  ...
  (input  clk,
   ...
   input  write_en,
   input  [(-1)+WRITE_WIDTH:0] write_data_in,
   output write_data_ready,
   input  host_read_req,
   input  [(-1)+ADDR_WIDTH:0] host_read_addr,
   input  [(-1)+SIZE_WIDTH:0] host_read_size,
   input  host_write_req,
   input  [(-1)+ADDR_WIDTH:0] host_write_addr,
   input  [(-1)+SIZE_WIDTH:0] host_write_size);
