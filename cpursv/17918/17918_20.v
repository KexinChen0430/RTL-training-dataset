...
module uart_rx(input  clk,
  ...
  wire 
       start_detect = (~serial[2] & (serial[5] & serial[4])) & serial[3];
  wire  one_detect = serial[4] & (serial[3] & serial[5]);
  wire 
       zero_detect = ~serial[3] & (~serial[4] & ~serial[5]);
      ...
      begin
        ...
        if (state == 4'd0) 
          ...
          begin
            ...
            if (~one_detect & ~zero_detect) state <= 4'd0;
            else if (~zero_detect & (state == 4'd1)) state <= 4'd0;
