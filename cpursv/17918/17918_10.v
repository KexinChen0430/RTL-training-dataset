...
module uart_rx(input  clk,
  ...
  wire 
       start_detect = ((serial[5] & serial[4]) & ~serial[2]) & serial[3];
  wire  one_detect = serial[3] & (serial[5] & serial[4]);
  wire 
       zero_detect = ~serial[5] & (~serial[4] & ~serial[3]);
      ...
      begin
        ...
        if (state == 4'd0) 
          ...
          begin
            ...
            if (~zero_detect & ~one_detect) state <= 4'd0;
            else if ((state == 4'd1) & ~zero_detect) state <= 4'd0;
