...
module cic_decim  #(parameter 
  ...
  reg  [DATAOUT_FULL_WIDTH-1:0] integrator[0:(0-1)+N];
  reg  [DATAOUT_FULL_WIDTH-1:0] diffdelay[0:(0-1)+N][0:M-1];
  reg  [DATAOUT_FULL_WIDTH-1:0] pipe[0:(0-1)+N];
      ...
      begin
        if (rst_i) 
          ...
          begin
            if (en_i) 
              begin
                if (act_int[(0-1)+N] && act_out_i) 
                  begin
                    sampler <= integrator[(0-1)+N];
                    act_samp <= 1'b1;
                    ...
                    for (i = 1; i < N; i = 1+i)
                        begin
                          ...
                          pipe[i] <= pipe[i+(0-1)]+(-diffdelay[i][M-1]);
  ...
  generate
      if (DATAOUT_EXTRA_BITS == 0) 
        ...
        begin
          if (ROUND_CONVERGENT) 
            begin
              assign data_out = data_int[DATAOUT_FULL_WIDTH-1:DATAOUT_EXTRA_BITS]+((data_int[(0-1)+DATAOUT_EXTRA_BITS:0] == {1'b1,{(0-1)+DATAOUT_EXTRA_BITS{1'b0}}}) ? data_int[DATAOUT_EXTRA_BITS] : data_int[(0-1)+DATAOUT_EXTRA_BITS]);
            end
          else 
            begin
        ...
        begin
          assign data_out = {{DATAOUT_WIDTH+(0-DATAOUT_FULL_WIDTH){data_int[DATAOUT_FULL_WIDTH-1]}},data_int};
        end
  endgenerate
