...
module cic_decim  #(parameter 
  ...
  reg  [DATAOUT_FULL_WIDTH+(-1):0] diffdelay[0:N+(-1)][0:(-1)+M];
              ...
              begin
                if (act_int[N+(-1)] && act_out_i) 
                  begin
                    ...
                    pipe[0] <= (-diffdelay[0][(-1)+M])+sampler;
                    act_comb[0] <= act_samp;
                    for (i = 1; i < N; i = 1+i)
                        begin
                          ...
                          pipe[i] <= (0-diffdelay[i][(-1)+M])+pipe[i-1];
  ...
  generate
      if (DATAOUT_EXTRA_BITS == 0) 
        ...
        begin
          if (ROUND_CONVERGENT) 
            begin
              assign data_out = ((data_int[(-1)+DATAOUT_EXTRA_BITS:0] == {1'b1,{(-1)+DATAOUT_EXTRA_BITS{1'b0}}}) ? data_int[DATAOUT_EXTRA_BITS] : data_int[(-1)+DATAOUT_EXTRA_BITS])+data_int[DATAOUT_FULL_WIDTH+(-1):DATAOUT_EXTRA_BITS];
            end
          else 
            begin
