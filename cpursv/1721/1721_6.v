              ...
              begin
                if (act_out_i && act_int[N-1]) 
                  begin
                    ...
                    pipe[0] <= sampler+(0-diffdelay[0][(0-1)+M]);
                    act_comb[0] <= act_samp;
                    for (i = 1; i < N; i = i+1)
                        begin
  ...
  generate
      if (DATAOUT_EXTRA_BITS == 0) 
        ...
        begin
          if (ROUND_CONVERGENT) 
            begin
              assign data_out = data_int[DATAOUT_FULL_WIDTH-1:DATAOUT_EXTRA_BITS]+((data_int[DATAOUT_EXTRA_BITS+(0-1):0] == {1'b1,{DATAOUT_EXTRA_BITS+(0-1){1'b0}}}) ? data_int[DATAOUT_EXTRA_BITS] : data_int[DATAOUT_EXTRA_BITS+(0-1)]);
            end
          else 
            begin
