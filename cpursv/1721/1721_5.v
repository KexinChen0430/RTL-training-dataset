...
module cic_decim  #(parameter 
  ...
  reg  [DATAOUT_FULL_WIDTH+(-1):0] diffdelay[0:(-1)+N][0:M-1];
  ...
  generate
      if (DATAOUT_EXTRA_BITS == 0) 
        ...
        begin
          if (ROUND_CONVERGENT) 
            begin
              assign data_out = data_int[DATAOUT_FULL_WIDTH+(-1):DATAOUT_EXTRA_BITS]+((data_int[(-1)+DATAOUT_EXTRA_BITS:0] == {1'b1,{(-1)+DATAOUT_EXTRA_BITS{1'b0}}}) ? data_int[DATAOUT_EXTRA_BITS] : data_int[(-1)+DATAOUT_EXTRA_BITS]);
            end
          else 
            begin
