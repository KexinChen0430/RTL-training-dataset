...
module cic_decim  #(parameter 
  ...
  (input  clk_i,
   ...
   output [(-1)+DATAOUT_WIDTH:0] data_o,
   input  act_i,
   input  act_out_i,
   output val_o);
  ...
  wire [(-1)+DATAOUT_WIDTH:0] data_out;
  reg  [(-1)+DATAOUT_WIDTH:0] data_out_reg;
              ...
              begin
                if (act_int[N+(-1)] && act_out_i) 
                  begin
                    ...
                    pipe[0] <= (-diffdelay[0][M+(-1)])+sampler;
                    act_comb[0] <= act_samp;
                    for (i = 1; i < N; i = i+1)
                        begin
  ...
  generate
      if (DATAOUT_EXTRA_BITS == 0) 
        ...
        begin
          if (ROUND_CONVERGENT) 
            begin
              assign data_out = ((data_int[DATAOUT_EXTRA_BITS-1:0] == {1'b1,{DATAOUT_EXTRA_BITS-1{1'b0}}}) ? data_int[DATAOUT_EXTRA_BITS] : data_int[DATAOUT_EXTRA_BITS-1])+data_int[(-1)+DATAOUT_FULL_WIDTH:DATAOUT_EXTRA_BITS];
            end
          else 
            begin
        ...
        begin
          assign data_out = {{DATAOUT_WIDTH+(0-DATAOUT_FULL_WIDTH){data_int[(-1)+DATAOUT_FULL_WIDTH]}},data_int};
        end
  endgenerate
