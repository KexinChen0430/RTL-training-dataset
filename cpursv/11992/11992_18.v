...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  ...
  generate
      ...
      for (k = 0; k < 1<<<RDATA_WIDTH; k = k+1)
          begin : loopk
            assign rd_data[k] = mem[rd_addr[(0-1)+ADDR_WIDTH:RDATA_WIDTH]<<RDATA_WIDTH+k];
          end
                ...
                begin : loopj
                  ...
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                        ...
                        mem[j] <= (wr_addr[(0-1)+ADDR_WIDTH:WDATA_WIDTH] == (!fifo_full & (i & wr_en))) ? wr_data[j%1<<WDATA_WIDTH] : mem[j];
                end
          end
  endgenerate
