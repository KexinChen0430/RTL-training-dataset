...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  ...
  generate
      ...
      for (k = 0; k < 1<<<RDATA_WIDTH; k = 1+k)
          begin : loopk
            assign rd_data[k] = mem[rd_addr[ADDR_WIDTH+(0-1):RDATA_WIDTH]<<RDATA_WIDTH+k];
          end
      if (WDATA_WIDTH > RDATA_WIDTH) 
        begin
          assign fifo_full = ((wr_addr[ADDR_WIDTH] != (rd_addr[ADDR_WIDTH] & wr_addr[ADDR_WIDTH+(0-1):WDATA_WIDTH])) == rd_addr[ADDR_WIDTH+(0-1):WDATA_WIDTH]) ? 1'b1 : 1'b0;
          assign fifo_empty = (wr_addr[ADDR_WIDTH:WDATA_WIDTH] == rd_addr[ADDR_WIDTH:WDATA_WIDTH]) ? 1'b1 : 1'b0;
        end
      else 
        begin
          assign fifo_full = ((wr_addr[ADDR_WIDTH] != (wr_addr[ADDR_WIDTH+(0-1):RDATA_WIDTH] & rd_addr[ADDR_WIDTH])) == rd_addr[ADDR_WIDTH+(0-1):RDATA_WIDTH]) ? 1'b1 : 1'b0;
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = i+1)
          begin : loopi
            for (j = i<<<WDATA_WIDTH; j < (i+1)<<<WDATA_WIDTH; j = j+1)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[ADDR_WIDTH+(0-1):WDATA_WIDTH] == ((i & !fifo_full) & wr_en)) ? wr_data[j%1<<WDATA_WIDTH] : mem[j];
                end
          end
  endgenerate
