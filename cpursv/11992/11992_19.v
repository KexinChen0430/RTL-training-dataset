...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  ...
  always @(posedge clk or negedge rst0_async)
      begin
        if (!rst0_async) 
          begin
            wr_addr <= {ADDR_WIDTH+(1+(0-WDATA_WIDTH)){1'b0}};
            rd_addr <= {(1+ADDR_WIDTH)+(0-RDATA_WIDTH){1'b0}};
          end
        else 
          begin
            if (!rst0_sync) 
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = 1+i)
          begin : loopi
            for (j = (2**WDATA_WIDTH)*i; j < ((2**WDATA_WIDTH)*(1+i)); j = j+1)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[ADDR_WIDTH-1:WDATA_WIDTH] == (!fifo_full & (i & wr_en))) ? wr_data[j%(2**WDATA_WIDTH)] : mem[j];
                end
          end
  endgenerate
