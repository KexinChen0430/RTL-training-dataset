...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  (input  clk,
   ...
   input  rst0_sync,
   input  wr_en,
   input  [(0-1)+(1<<<1**WDATA_WIDTH):0] wr_data,
   input  rd_en,
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = i+1)
          begin : loopi
            for (j = (1<<<1**WDATA_WIDTH)*i; j < (i+1)<<<WDATA_WIDTH; j = 1+j)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[ADDR_WIDTH-1:WDATA_WIDTH] == ((i & wr_en) & !fifo_full)) ? wr_data[j%(1<<<1**WDATA_WIDTH)] : mem[j];
                end
          end
  endgenerate
