...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  ...
  generate
      ...
      for (k = 0; k < 1<<<RDATA_WIDTH; k = k+1)
          begin : loopk
            assign rd_data[k] = mem[(rd_addr[(-1)+ADDR_WIDTH:RDATA_WIDTH]*1<<<RDATA_WIDTH)+k];
          end
      ...
      begin
        if (!rst0_async) 
          begin
            ...
            rd_addr <= {ADDR_WIDTH+((0-RDATA_WIDTH)+1){1'b0}};
          end
        else 
          begin
            if (!rst0_sync) 
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = i+1)
          begin : loopi
            for (j = i<<<WDATA_WIDTH; j < (1<<<WDATA_WIDTH*(i+1)); j = j+1)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[(-1)+ADDR_WIDTH:WDATA_WIDTH] == ((i & wr_en) & !fifo_full)) ? wr_data[j%1<<<WDATA_WIDTH] : mem[j];
                end
          end
  endgenerate
