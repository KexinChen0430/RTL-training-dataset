...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  (input  clk,
   ...
   input  rst0_sync,
   input  wr_en,
   input  [1<<WDATA_WIDTH+(0-1):0] wr_data,
   input  rd_en,
   output [1<<RDATA_WIDTH+(0-1):0] rd_data,
   output fifo_full,
   output fifo_empty);
  ...
  always @(posedge clk or negedge rst0_async)
      begin
        if (!rst0_async) 
          begin
            wr_addr <= {(1+ADDR_WIDTH)+(0-WDATA_WIDTH){1'b0}};
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = i+1)
          begin : loopi
            for (j = 1<<WDATA_WIDTH*i; j < (i+1)<<WDATA_WIDTH; j = j+1)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[(0-1)+ADDR_WIDTH:WDATA_WIDTH] == ((wr_en & i) & !fifo_full)) ? wr_data[j%1<<WDATA_WIDTH] : mem[j];
                end
          end
  endgenerate
