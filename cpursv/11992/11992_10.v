...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  (input  clk,
   ...
   input  rd_en,
   output [(2**RDATA_WIDTH)+(0-1):0] rd_data,
   output fifo_full,
   output fifo_empty);
      ...
      begin
        if (!rst0_async) 
          begin
            ...
            rd_addr <= {1+(ADDR_WIDTH-RDATA_WIDTH){1'b0}};
          end
        else 
          begin
            if (!rst0_sync) 
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = i+1)
          begin : loopi
            for (j = i<<<WDATA_WIDTH; j < (i+1)<<<WDATA_WIDTH; j = 1+j)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[ADDR_WIDTH-1:WDATA_WIDTH] == (wr_en & (!fifo_full & i))) ? wr_data[j%1<<WDATA_WIDTH] : mem[j];
                end
          end
  endgenerate
