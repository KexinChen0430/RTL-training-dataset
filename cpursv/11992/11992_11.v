...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  (input  clk,
   ...
   input  rst0_sync,
   input  wr_en,
   input  [(-1)+1<<<WDATA_WIDTH:0] wr_data,
   input  rd_en,
  ...
  generate
      ...
      for (k = 0; k < 1<<<RDATA_WIDTH; k = k+1)
          begin : loopk
            assign rd_data[k] = mem[k+rd_addr[(-1)+ADDR_WIDTH:RDATA_WIDTH]<<<RDATA_WIDTH];
          end
      if (WDATA_WIDTH > RDATA_WIDTH) 
        begin
          assign fifo_full = ((wr_addr[ADDR_WIDTH] != (wr_addr[(-1)+ADDR_WIDTH:WDATA_WIDTH] & rd_addr[ADDR_WIDTH])) == rd_addr[(-1)+ADDR_WIDTH:WDATA_WIDTH]) ? 1'b1 : 1'b0;
          assign fifo_empty = (wr_addr[ADDR_WIDTH:WDATA_WIDTH] == rd_addr[ADDR_WIDTH:WDATA_WIDTH]) ? 1'b1 : 1'b0;
        end
      else 
        begin
          assign fifo_full = ((wr_addr[ADDR_WIDTH] != (rd_addr[ADDR_WIDTH] & wr_addr[(-1)+ADDR_WIDTH:RDATA_WIDTH])) == rd_addr[(-1)+ADDR_WIDTH:RDATA_WIDTH]) ? 1'b1 : 1'b0;
      ...
      begin
        if (!rst0_async) 
          begin
            ...
            rd_addr <= {(-RDATA_WIDTH)+(1+ADDR_WIDTH){1'b0}};
          end
        else 
          begin
            if (!rst0_sync) 
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = 1+i)
          begin : loopi
            for (j = i*1<<<WDATA_WIDTH; j < (1<<<WDATA_WIDTH*(1+i)); j = j+1)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[(-1)+ADDR_WIDTH:WDATA_WIDTH] == (!fifo_full & (wr_en & i))) ? wr_data[j%1<<<WDATA_WIDTH] : mem[j];
                end
          end
  endgenerate
