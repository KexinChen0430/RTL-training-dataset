...
module usb_fifo_sync  #(parameter  ADDR_WIDTH = 4, WDATA_WIDTH = 0, RDATA_WIDTH = 0)
  ...
  generate
      ...
      if (WDATA_WIDTH > RDATA_WIDTH) 
        begin
          assign fifo_full = ((wr_addr[ADDR_WIDTH] != (rd_addr[ADDR_WIDTH] & wr_addr[(-1)+ADDR_WIDTH:WDATA_WIDTH])) == rd_addr[(-1)+ADDR_WIDTH:WDATA_WIDTH]) ? 1'b1 : 1'b0;
  ...
  generate
      ...
      for (i = 0; i < FIFO_LENGTH>>WDATA_WIDTH; i = i+1)
          begin : loopi
            for (j = i<<WDATA_WIDTH; j < (1<<WDATA_WIDTH*(i+1)); j = 1+j)
                begin : loopj
                  
                  always @(posedge clk or negedge rst0_async)
                      if (!rst0_async) mem[j] <= 1'b0;
                      else 
                        mem[j] <= (wr_addr[(-1)+ADDR_WIDTH:WDATA_WIDTH] == (i & (wr_en & !fifo_full))) ? wr_data[(j+(0%1<<WDATA_WIDTH))%1<<WDATA_WIDTH] : mem[j];
                end
          end
  endgenerate
