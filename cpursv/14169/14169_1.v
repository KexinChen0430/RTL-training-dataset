      ...
      begin
        if (saturate == 1'b0) 
          ...
          begin
            ...
            if (round == 1'b1) 
              begin
                for (i = 0; i < ((-round_width)+datain_width); i = i+1)
                    begin
                      min[i] = 1'bX;
                      max[i] = 1'bX;
                    end
            ...
            if (saturate_mode == symmetric) 
              begin
                ...
                for (i = (-round_width)+datain_width; i < ((-saturate_width)+datain_width); i = i+1)
                    begin
                      data_tmp = data_tmp | datain[i];
                      max[i] = 1'b1;
                      min[i] = 1'b0;
                    end
                if (round == 1'b1) min[(-round_width)+datain_width] = 1'b1;
                else min[0] = 1'b1;
              end
              
            if (saturate_mode == asymmetric) 
              begin
                for (i = 0; i < ((-saturate_width)+datain_width); i = i+1)
                    begin
                      max[i] = 1'b1;
                      min[i] = 1'b0;
                    end
            ...
            if (saturate_width == 1) 
              begin
                ...
                else sign_tmp = sign_tmp & datain[(0-1)+datain_width];
              end
            else 
              begin
                for (i = (-saturate_width)+datain_width; i < datain_width; i = i+1)
                    begin
            ...
            if (saturation_overflow_tmp) 
              begin
                if ((operation_mode == output_only) || 
                    (operation_mode == two_level_adder_chain_out) || (operation_mode == accumulator_chain_out)) 
                  begin
