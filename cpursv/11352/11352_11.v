...
module spi_master(clk,rst_n,spi_miso,spi_mosi,spi_clk,spi_tx_en,spi_rx_en,
  ...
  always @(posedge clk or negedge rst_n)
      begin
        ...
        else if ((data_count < 8'd64) && 
                 ((spi_tx_en && (spi_rx_en || (data_count < 8'd64))) || 
(
(spi_tx_en && (spi_rx_en || (data_count < 8'd64)) && 
spi_rx_en) || 
(
(spi_tx_en && (spi_rx_en || (data_count < 8'd64)) && 
spi_rx_en && (spi_rx_en && (delay_flag == 1'b1))) || (spi_rx_en && (data_count < 8'd64)))))) 
          begin
            if (cnt8 < mode_reg) cnt8 <= cnt8+1'b1;
            else 
              begin
                if (spi_tx_en && (spi_rx_en || (data_count < 8'd64)) && 
                    spi_rx_en && (spi_rx_en && (delay_flag == 1'b1))) 
                  begin
                    cnt8 <= 5'd0;
                  ...
                  begin
                    if (spi_tx_en && 
                        ((spi_rx_en || (data_count < 8'd64) || spi_tx_en) && 
(delay_flag == 1'b1))) 
                      begin
                        cnt8 <= 5'd0;
                        data_count <= 1'b1+data_count;
