...
module spi_master(clk,rst_n,spi_miso,spi_mosi,spi_clk,spi_tx_en,spi_rx_en,
                  spi_over,mode_select_CPHA,mode_select_CPOL,receive_status);
  ...
  always @(posedge clk or negedge rst_n)
      begin
        ...
        else if ((spi_rx_en && (data_count < 8'd64)) || 
                 ((data_count < 8'd64) && spi_tx_en)) 
          begin
...
((delay_flag == 1'b1) && 
(spi_tx_en || 
(((data_count < 8'd64) && spi_tx_en) || spi_rx_en))) || 
(
((delay_flag == 1'b1) && 
((spi_rx_en && (data_count < 8'd64)) || 
((data_count < 8'd64) && spi_tx_en) || (spi_tx_en && spi_rx_en))) || 
(
((spi_tx_en && spi_rx_en) || 
((data_count < 8'd64) && spi_tx_en) || (data_count < 8'd64)) && (delay_flag == 1'b1)))) && spi_rx_en) 
                  begin
                    cnt8 <= 5'd0;
                    data_count <= 1'b1+data_count;
...
(
((spi_tx_en && spi_rx_en) || 
((data_count < 8'd64) && spi_tx_en) || (data_count < 8'd64)) && (delay_flag == 1'b1))))) 
                      begin
                        cnt8 <= 5'd0;
                        data_count <= 1'b1+data_count;
                        spi_tx_db <= spi_tx_db+1'b1;
  ...
  always @(posedge clk or negedge rst_n)
      begin
        if (!rst_n) spi_clkr <= mode_select_CPOL ? 1'b1 : 1'b0;
        else if ((cnt8 < mode_reg) && (cnt8 > start_reg)) spi_clkr <= ~spi_clkr;
  ...
  always @(posedge clk or negedge rst_n)
      begin
        if (!rst_n) receive_status <= 1'b0;
        else receive_status <= (recv_detect == 8'd64) ? 1'b1 : 1'b0;
      end
  reg  delay_flag;
  ...
  always @(posedge clk or negedge rst_n)
      begin
        if (!rst_n) 
        ...
        else if ((cnt8 == mode_reg) && (delay_cnt == 'hF)) 
          begin
            delay_cnt <= 'h0;
            delay_flag <= 1'b1;
          end
          ...
          begin
            delay_cnt <= (cnt8 == mode_reg) ? (1'b1+delay_cnt) : 'h0;
            delay_flag <= 1'b0;
          end
      end
endmodule
