...
((spi_tx_en && (data_count < 8'd64)) || 
(spi_rx_en && ((data_count < 8'd64) || spi_tx_en))) && (data_count < 8'd64)) 
          begin
            if (cnt8 < mode_reg) cnt8 <= 1'b1+cnt8;
            else 
              begin
                if (spi_rx_en && 
                    (
...
(
((spi_tx_en && (data_count < 8'd64)) || 
(spi_rx_en && ((data_count < 8'd64) || spi_tx_en))) && (data_count < 8'd64)) || spi_rx_en) && ((spi_rx_en || spi_tx_en) && (delay_flag == 1'b1))) && (spi_rx_en && spi_tx_en)) 
                  begin
                    cnt8 <= 5'd0;
                    data_count <= data_count+1'b1;
                    spi_tx_db <= 1'b1+spi_tx_db;
                    recv_detect <= (spi_rx_db == data_count) ? (1'b1+recv_detect) : recv_detect;
                  end
                else 
                  begin
                    if (spi_tx_en && (delay_flag == 1'b1)) 
                      begin
                        cnt8 <= 5'd0;
                        data_count <= data_count+1'b1;
                        spi_tx_db <= 1'b1+spi_tx_db;
  ...
  always @(posedge clk or negedge rst_n)
      begin
        ...
        else if ((delay_cnt == 'hF) && (cnt8 == mode_reg)) 
          begin
            delay_cnt <= 'h0;
            delay_flag <= 1'b1;
          end
