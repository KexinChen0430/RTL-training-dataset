...
module pixelq_op_OUTPUT_STREAM_reg_slice  #(parameter  N = 8)
  ...
  assign load_p1 = 
(((state == ONE) || ((state == TWO) && m_ready)) && 
(((state == TWO) && m_ready) || (m_ready && s_valid))) || 
                   (
(
(((state == ONE) || s_valid) && 
((state == ZERO) || (state == ONE)) && (state == ZERO) && s_valid) || (state == TWO)) && 
(
(((state == ONE) || s_valid) && 
((state == ZERO) || (state == ONE)) && (state == ZERO) && s_valid) || 
(m_ready && s_valid && 
(((state == ONE) && s_valid) || (state == ZERO)))));
