      ...
      begin
        if (reset == 1'b1) 
          ...
          case ({a_prev[1<<1],b_prev[1<<1],a_prev[1],b_prev[1]})
            ...
            4'b0001: {dir_reg,count_reg} <= {1'b0,count_reg+(0-32'b1)};

            4'b0111: {dir_reg,count_reg} <= {1'b0,count_reg+(0-32'b1)};

            4'b1110: {dir_reg,count_reg} <= {1'b0,count_reg+(0-32'b1)};

            4'b1000: {dir_reg,count_reg} <= {1'b0,count_reg+(0-32'b1)};
