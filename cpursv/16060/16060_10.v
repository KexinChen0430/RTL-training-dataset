...
module quad(clk,A,B,Z,zr,out);
  ...
  wire 
       count_enable = (((((Ad[1+1] & (Ad[1] & ~Ad[1+1])) | (~Ad[1] & Ad[1+1])) & ~Bd[1]) | (~((~Ad[1+1] | ~Ad[1]) & ((~Ad[1] | Ad[1]) & (Ad[1] | Ad[1+1]))) & Bd[1])) | ((Ad[1] & ~Ad[1+1]) & ~Bd[1])) ^ Bd[1+1];

  wire 
       count_direction = (~Bd[1+1] & Ad[1]) | (Bd[1+1] & ~Ad[1]);
