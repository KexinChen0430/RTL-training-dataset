...
module altsyncram(clocken0,wren_a,clock0,byteena_a,address_a,address_b,data_a,
  ...
  input  wire [widthad_a+(0-1):0] address_a;
  input  wire [(0-1)+widthad_b:0] address_b;
  input  wire [(0-1)+width_a:0] data_a;
  output wire [(0-1)+width_a:0] q_a;
  output wire [width_b-1:0] q_b;
  reg  [(0-1)+width_a:0] ram[numwords_a+(0-1):0];
  reg  [widthad_a+(0-1):0] addr_a_delayed;
              ...
              begin
                ...
                if (wren_a_delayed) 
                  $display("%5d altsyncram ram[%x] <= %x",$time,
                           addr_a_delayed,((wr_mask & (~wr_mask & data_a_delayed)) | ((~wr_mask & ram[addr_a_delayed]) | (ram[addr_a_delayed] & data_a_delayed))) | (((wr_mask & (~wr_mask & ram[addr_a_delayed])) | ((~wr_mask | wr_mask) & (ram[addr_a_delayed] & (wr_mask & data_a_delayed)))) | (wr_mask & data_a_delayed)));
                  
              end
              
            if (wren_a_delayed) 
              begin
                ram[addr_a_delayed] <= ((wr_mask & (~wr_mask & data_a_delayed)) | ((~wr_mask & ram[addr_a_delayed]) | (ram[addr_a_delayed] & data_a_delayed))) | (((wr_mask & (~wr_mask & ram[addr_a_delayed])) | ((~wr_mask | wr_mask) & (ram[addr_a_delayed] & (wr_mask & data_a_delayed)))) | (wr_mask & data_a_delayed));
              end
