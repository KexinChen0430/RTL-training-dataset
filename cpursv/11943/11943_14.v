...
module altsyncram(clocken0,wren_a,clock0,byteena_a,address_a,address_b,data_a,
  ...
  reg  [(-1)+width_a:0] ram[(-1)+numwords_a:0];
              ...
              begin
                ...
                if (wren_a_delayed) 
                  $display("%5d altsyncram ram[%x] <= %x",$time,
                           addr_a_delayed,(data_a_delayed & wr_mask) | ((ram[addr_a_delayed] & ~wr_mask) | (data_a_delayed & (~wr_mask & wr_mask))));
                  
              end
              
            if (wren_a_delayed) 
              begin
                ram[addr_a_delayed] <= (data_a_delayed & wr_mask) | ((ram[addr_a_delayed] & ~wr_mask) | (data_a_delayed & (~wr_mask & wr_mask)));
              end
