              ...
              begin
                ...
                if (wren_a_delayed) 
                  $display("%5d altsyncram ram[%x] <= %x",$time,
                           addr_a_delayed,(ram[addr_a_delayed] | data_a_delayed) & ((wr_mask & data_a_delayed) | ((ram[addr_a_delayed] & ~wr_mask) | (wr_mask & ~wr_mask))));
                  
              end
              
            if (wren_a_delayed) 
              begin
                ram[addr_a_delayed] <= (ram[addr_a_delayed] | data_a_delayed) & ((wr_mask & data_a_delayed) | ((ram[addr_a_delayed] & ~wr_mask) | (wr_mask & ~wr_mask)));
              end
