...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        next_active = arm && enable | active;
        next_mask_flag = mask_flag | arm && enable;
        ...
        mismatch = |(((last_data & ~(((~masked_dataIn | masked_dataIn) & ((last_data | ~last_data) & masked_dataIn)) & (last_data | ~last_data))) | (((~masked_dataIn | masked_dataIn) & ((last_data | ~last_data) & masked_dataIn)) & (last_data | ~last_data))) & ((last_data & ~(((~masked_dataIn | masked_dataIn) & ((last_data | ~last_data) & masked_dataIn)) & (last_data | ~last_data))) | ~last_data));
        if (active) 
          begin
            next_validOut = FALSE;
            next_last_valid = last_valid | validIn;
            if (validIn && last_valid) 
              if (mismatch || (!enable || count_full)) 
                begin
                  next_active = enable;
                  next_validOut = TRUE;
