...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        ...
        mismatch = |(((((last_data & (masked_dataIn & (~last_data | last_data))) & last_data) & ~last_data) | (~((last_data & (masked_dataIn & (~last_data | last_data))) & last_data) & last_data)) ^ (((last_data | masked_dataIn) & ((masked_dataIn & last_data) ^ (masked_dataIn & ((~last_data | last_data) & (masked_dataIn & (~last_data | last_data)))))) & (~last_data | last_data)));
        if (active) 
          begin
            next_validOut = FALSE;
