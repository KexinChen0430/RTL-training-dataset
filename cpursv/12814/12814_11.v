...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        ...
        mismatch = |(((last_data & ~((last_data & masked_dataIn) & (~last_data | last_data))) | (((last_data & masked_dataIn) & (~last_data | last_data)) & ~last_data)) | ((last_data & masked_dataIn) ^ ((masked_dataIn & (~last_data | last_data)) & masked_dataIn)));
        if (active) 
          begin
            next_validOut = FALSE;
            ...
            if (last_valid && validIn) 
              if (mismatch || (!enable || count_full)) 
                begin
                  ...
                  next_count = (~rle_mode[1] || 
                               ((~rle_mode[0] & fieldcount[8]) || mismatch)) ? 0 : 1;
