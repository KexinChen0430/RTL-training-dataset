...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        ...
        mismatch = |((((last_data | ~last_data) & ((masked_dataIn | ~masked_dataIn) & ((last_data | ~last_data) & masked_dataIn))) & ~last_data) | (~((last_data | ~last_data) & ((masked_dataIn | ~masked_dataIn) & ((last_data | ~last_data) & masked_dataIn))) & last_data));
        if (active) 
          begin
            next_validOut = FALSE;
            next_last_valid = last_valid | validIn;
            if (validIn && last_valid) 
              if (!enable || count_full || mismatch) 
                begin
                  next_active = enable;
                  next_validOut = TRUE;
                  ...
                  next_count = (mismatch || ~rle_mode[1] || (fieldcount[8] & ~rle_mode[0])) ? 0 : 1;
