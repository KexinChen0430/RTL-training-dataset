...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        ...
        next_count = {31{active}} & count;
        next_fieldcount = {9{active}} & fieldcount;
        next_track = {2{active}} & track;
        mismatch = |(((((~masked_dataIn & (masked_dataIn & last_data)) | (masked_dataIn & ~(masked_dataIn & last_data))) & (masked_dataIn | last_data)) & (((~masked_dataIn & (masked_dataIn & last_data)) | (masked_dataIn & ~(masked_dataIn & last_data))) | ~masked_dataIn)) ^ ((((~masked_dataIn & (masked_dataIn & last_data)) | (masked_dataIn & ~(masked_dataIn & last_data))) | ~masked_dataIn) & last_data));
        if (active) 
          begin
            next_validOut = FALSE;
            next_last_valid = validIn | last_valid;
            if (validIn && last_valid) 
              if (count_full || (mismatch || !enable)) 
                begin
                  next_active = enable;
                  next_validOut = TRUE;
                  ...
                  next_count = (mismatch || 
                               ((fieldcount[8] & ~rle_mode[0]) || ~rle_mode[1])) ? 0 : 1;
