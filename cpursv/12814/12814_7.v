...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        ...
        next_mask_flag = arm && enable | mask_flag;
        ...
        mismatch = |((~last_data & ((last_data | (masked_dataIn ^ (masked_dataIn & last_data))) & (masked_dataIn ^ (masked_dataIn & last_data)))) | ((~masked_dataIn & last_data) ^ (~masked_dataIn & masked_dataIn)));
        if (active) 
          begin
            next_validOut = FALSE;
            ...
            if (validIn && last_valid) 
              if (!enable || mismatch || count_full) 
                begin
                  ...
                  next_count = (mismatch || 
                               ((~rle_mode[0] & fieldcount[8]) || ~rle_mode[1])) ? 0 : 1;
