...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        ...
        mismatch = |(last_data ^ (masked_dataIn & ((masked_dataIn | last_data) | ~last_data)));
        if (active) 
          begin
            next_validOut = FALSE;
            next_last_valid = last_valid | validIn;
            if (last_valid && validIn) 
              if (count_full || !enable || mismatch) 
                begin
                  next_active = enable;
                  next_validOut = TRUE;
                  ...
                  next_count = (~rle_mode[1] || (~rle_mode[0] & fieldcount[8]) || mismatch) ? 0 : 1;
