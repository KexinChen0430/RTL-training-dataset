...
module rle_enc(clock,reset,enable,arm,rle_mode,disabledGroups,dataIn,
  ...
  always @* 
      begin
        ...
        mismatch = |((((masked_dataIn & last_data) & (~last_data | last_data)) ^ last_data) | ((last_data | masked_dataIn) & ~last_data));
        if (active) 
          begin
            next_validOut = FALSE;
            ...
            if (validIn && last_valid) 
              if (mismatch || (!enable || count_full)) 
                begin
                  ...
                  next_count = ((~rle_mode[0] & fieldcount[8]) || mismatch || ~rle_mode[1]) ? 0 : 1;
