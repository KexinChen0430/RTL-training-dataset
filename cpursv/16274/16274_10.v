...
module altpcierd_npcred_monitor  #(parameter  CRED_LAT = 10)
  (input  clk_in,
   input  rstn,
   input  [2:0] np_cred,
   input  np_sent,
  ...
  assign update_creds = (np_cred > 0) & (cred_sm == LATCH_CRED);
        ...
        if (rstn == 1'b0) 
          ...
          begin
            if (update_creds) 
              ...
              begin
                creds_sent_on_tx <= 8'h1+creds_sent_on_tx;
              end
            ...
            case (cred_sm)
              ...
              GRANT_CRED: begin
                    local_creds_avail <= np_sent ? (local_creds_avail+(0-3'h1)) : local_creds_avail;
                    have_cred <= (np_sent & (local_creds_avail == 3'h1)) ? 1'b0 : 1'b1;
                    if (np_sent & (local_creds_avail == 3'h1)) 
                      begin
              ...
              WAIT_LAST_NP_TX: begin
                    if (creds_sent_on_tx == creds_granted) 
                      begin
              ...
              WAIT_CRED_UPD: begin
                    latency_counter <= 8'h1+latency_counter;
                    if (latency_counter == CRED_LAT) 
                      begin
                        cred_sm <= LATCH_CRED;
                      end
                    else 
