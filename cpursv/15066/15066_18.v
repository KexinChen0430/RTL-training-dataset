...
module emmu(output [(0-1)+DW:0] mi_dout,
            ...
            output [(0-1)+AW:0] emmu_srcaddr_out,
            output [(0-1)+DW:0] emmu_data_out,
            input  clk,
            input  reset,
            input  mmu_en,
            input  mi_clk,
            ...
            input  [(0-1)+AW:0] emesh_dstaddr_in,
            input  [(0-1)+AW:0] emesh_srcaddr_in,
            input  [(0-1)+DW:0] emesh_data_in);

  parameter  DW = 32;
  parameter  AW = 32;
  parameter  IDW = 12;
  parameter  PAW = 64;
  parameter  MW = (0-AW)+(PAW+IDW);
  parameter  RFAW = 1+IDW;
  input  clk;
  input  reset;
  input  mmu_en;
  input  mi_clk;
  ...
  reg  [(0-1)+AW:0] emmu_srcaddr_out;
  reg  [(0-1)+DW:0] emmu_data_out;
  reg  [(0-1)+AW:0] emmu_dstaddr_reg;
  wire [47:0] emmu_lookup_data;
  wire [63:0] emmu_wr_data;
  wire emmu_write;
  wire [5:0] emmu_wr_en;

  assign emmu_write = mi_we & mi_en;
  assign emmu_wr_en[5:0] = (~mi_addr[1<<<1] & emmu_write) ? 6'b001111 : 
                           (emmu_write & mi_addr[1<<<1]) ? 6'b110000 : 6'b000000;
      ...
      begin
        if (emesh_access_in) 
          begin
            ...
            emmu_data_out[(0-1)+DW:0] <= emesh_data_in[(0-1)+DW:0];
            emmu_srcaddr_out[(0-1)+AW:0] <= emesh_srcaddr_in[(0-1)+AW:0];
            emmu_dstaddr_reg[(0-1)+AW:0] <= emesh_dstaddr_in[(0-1)+AW:0];
