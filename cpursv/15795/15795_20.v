...
module ogfx_reg_fifo(fifo_cnt_o,fifo_empty_cnt_o,fifo_data_o,fifo_done_evt_o,
                     fifo_empty_o,fifo_full_o,fifo_ovfl_evt_o,mclk,puc_rst,
                     fifo_data_i,fifo_enable_i,fifo_pop_i,fifo_push_i);
  ...
  input  fifo_pop_i;
  input  fifo_push_i;
  parameter  FIFO_EMPTY = 4'h0, FIFO_FULL = 4'hF;
  reg  [3:0] fifo_cnt_o;
  wire [3:0] fifo_cnt_nxt;

  assign fifo_full_o = fifo_cnt_o == FIFO_FULL;
  assign fifo_empty_o = fifo_cnt_o == FIFO_EMPTY;
  assign fifo_empty_cnt_o = (-fifo_cnt_o)+FIFO_FULL;
  wire  fifo_push_int = !fifo_full_o & fifo_push_i;

  wire  fifo_pop_int = fifo_pop_i & !fifo_empty_o;

  assign fifo_done_evt_o = (fifo_cnt_nxt == FIFO_EMPTY) & ~fifo_empty_o;
  assign fifo_ovfl_evt_o = fifo_full_o & fifo_push_i;
  assign fifo_cnt_nxt = ~fifo_enable_i ? FIFO_EMPTY : 
                        (fifo_pop_int & fifo_push_int) ? fifo_cnt_o : 
                        fifo_push_int ? (3'h1+fifo_cnt_o) : 
                        fifo_pop_int ? (fifo_cnt_o+(-3'h1)) : fifo_cnt_o;
  ...
  always @(posedge mclk or posedge puc_rst)
      if (puc_rst) wr_ptr <= 4'h0;
        ...
        begin
          if (wr_ptr == ((-1)+FIFO_FULL)) wr_ptr <= 4'h0;
          else wr_ptr <= 4'h1+wr_ptr;
        end
  ...
  always @(posedge mclk or posedge puc_rst)
      if (puc_rst) rd_ptr <= 4'h0;
        ...
        begin
          if (rd_ptr == ((-1)+FIFO_FULL)) rd_ptr <= 4'h0;
