...
module corr(clk,new_data,in_l_re,in_l_im,in_r_re,in_r_im,corr_out);
  ...
  output [(0-1)+(2*sh_reg_w):0] corr_out;
  reg  [(0-1)+(2*sh_reg_w):0] corr_out;
  ...
  reg  [(0-1)+(2*sh_reg_w):0] lrexrre_reg;
  reg  [(0-1)+(2*sh_reg_w):0] limxrim_reg;
  reg  [(0-1)+(2*sh_reg_w):0] corr_out_tmp;
