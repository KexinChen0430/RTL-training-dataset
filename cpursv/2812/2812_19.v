...
module DEBUG_AE(DBG_IN,READ,WRITE,USER,VIRTUELL,ACC_OK,VADR_R,MMU_Q,ENBYTE,
  ...
  assign make = (((ud | sd) & (~USER | ud)) & (((((crd & READ) | cwr) & WRITE) & (VIRTUELL == vnp)) | ((crd & READ) & (VIRTUELL == vnp)))) & ((sd | USER) & ((~USER | (((cwr & WRITE) & (USER & ud)) & (VIRTUELL == vnp))) | USER));
  ...
  assign DBG_HIT = (byte_en & (make & ACC_OK)) & (page_adr & (VIRTUELL ? virt_adr : real_adr));
endmodule
