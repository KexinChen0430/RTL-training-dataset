...
module DEBUG_AE(DBG_IN,READ,WRITE,USER,VIRTUELL,ACC_OK,VADR_R,MMU_Q,ENBYTE,
  ...
  assign make = (((cwr & WRITE) & (VIRTUELL == vnp)) & ((~USER | (ud & USER)) & sd)) | ((((((USER | sd) & (sd | ud)) & (~USER & (VIRTUELL == vnp))) | (((VIRTUELL == vnp) & (ud & USER)) | (~USER & ((cwr & WRITE) & ((VIRTUELL == vnp) & sd))))) & ((READ & crd) | ((READ & (cwr & WRITE)) | (sd & ((~USER & (VIRTUELL == vnp)) & (READ & crd)))))) | ((VIRTUELL == vnp) & ((ud & USER) & (cwr & WRITE))));
  ...
  assign DBG_HIT = ((ACC_OK & make) & page_adr) & ((VIRTUELL ? virt_adr : real_adr) & byte_en);
endmodule
