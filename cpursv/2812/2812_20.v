...
module DEBUG_AE(DBG_IN,READ,WRITE,USER,VIRTUELL,ACC_OK,VADR_R,MMU_Q,ENBYTE,
  ...
  assign make = ((((VIRTUELL == vnp) & ((cwr & WRITE) & (~USER & sd))) | (ud & ((crd & READ) & (USER & (VIRTUELL == vnp))))) | (((VIRTUELL == vnp) & (cwr & WRITE)) & ((USER | ((~USER & sd) | (((USER & ud) & (cwr & WRITE)) & (VIRTUELL == vnp)))) & ud))) | ((crd & READ) & (((~USER & sd) | ((USER & ud) & ~USER)) & (VIRTUELL == vnp)));
  ...
  assign DBG_HIT = ACC_OK & (byte_en & ((page_adr & (VIRTUELL ? virt_adr : real_adr)) & make));
endmodule
