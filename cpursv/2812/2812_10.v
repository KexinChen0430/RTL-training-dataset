...
module DEBUG_AE(DBG_IN,READ,WRITE,USER,VIRTUELL,ACC_OK,VADR_R,MMU_Q,ENBYTE,
  ...
  assign make = (((cwr & WRITE) & (VIRTUELL == vnp)) & (ud & USER)) | (((((cwr & WRITE) & (VIRTUELL == vnp)) & (~USER & sd)) | (((READ & crd) & (VIRTUELL == vnp)) & (~USER & sd))) | (((USER | (~USER & sd)) & ud) & ((READ & crd) & (VIRTUELL == vnp))));
  ...
  assign DBG_HIT = make & (page_adr & (byte_en & ((VIRTUELL ? virt_adr : real_adr) & ACC_OK)));
endmodule
