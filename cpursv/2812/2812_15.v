...
module DEBUG_AE(DBG_IN,READ,WRITE,USER,VIRTUELL,ACC_OK,VADR_R,MMU_Q,ENBYTE,
  ...
  assign make = (((VIRTUELL == vnp) & (crd & READ)) & (sd & ~USER)) | ((((WRITE & (cwr & (((READ | cwr) & WRITE) | READ))) & (VIRTUELL == vnp)) & (USER & ud)) | ((((WRITE & (cwr & (((READ | cwr) & WRITE) | READ))) & (VIRTUELL == vnp)) & (((USER & ud) & ~USER) | (sd & ~USER))) | ((crd & READ) & ((VIRTUELL == vnp) & (USER & ud)))));
  ...
  assign DBG_HIT = (ACC_OK & (make & (byte_en & page_adr))) & (VIRTUELL ? virt_adr : real_adr);
endmodule
