...
module DEBUG_AE(DBG_IN,READ,WRITE,USER,VIRTUELL,ACC_OK,VADR_R,MMU_Q,ENBYTE,
  ...
  assign make = ((((WRITE & cwr) & (VIRTUELL == vnp)) | ((((WRITE & cwr) | READ) & crd) & (VIRTUELL == vnp))) & ((~USER | (ud & USER)) & sd)) | ((ud & USER) & ((VIRTUELL == vnp) & (((ud & USER) | sd) & ((cwr & (WRITE | (crd & READ))) | (crd & READ)))));
  ...
  assign DBG_HIT = make & (page_adr & (ACC_OK & (byte_en & (VIRTUELL ? virt_adr : real_adr))));
endmodule
