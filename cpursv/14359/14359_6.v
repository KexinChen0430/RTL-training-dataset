...
module wb_to_userport(input  wire wb_clk,
  ...
  reg  [31:0] burst_data_buf[(-1)+BURST_LENGTH:0];
  ...
  wire [(-1)+BURST_ADDR_ALIGN:0] addr_align_zeroes;

  assign addr_align_zeroes = 0;
  assign cache_addr = {burst_addr,addr_align_zeroes};
  assign wb_req = ((wb_cyc_i & ddr2_calib_done) & (!ddr2_px_cmd_full & wb_stb_i)) & ((((ddr2_calib_done & wb_stb_i) | !ddr2_px_wr_full) & (!ddr2_px_wr_full | wb_cyc_i)) & ((!ddr2_px_wr_full | !wb_we_i) & (!ddr2_px_wr_full | !ddr2_px_cmd_full)));
  ...
  assign wb_read_req = !wb_we_i & (((!wb_ack_read | (((wb_cti_i == 3'b010) & (WB_BURSTING == TRUE)) | ((WB_BURSTING == TRUE) & !wb_ack_read))) & (!wb_ack_read | (!wb_we_i & wb_req))) & wb_req);
      ...
      begin
        ...
        if (wb_rst) 
          ...
          begin
            ...
            if (burst_cnt >= wb_adr_i[(-1)+BURST_ADDR_ALIGN:1<<1]) read_done <= 1'b1;
  ...
  assign ddr2_px_cmd_bl = wb_ack_write ? 6'b0 : ((-1)+BURST_LENGTH);
  ...
  assign wb_dat_o = burst_data_buf[wb_adr_i[(-1)+BURST_ADDR_ALIGN:1<<1]];
endmodule
