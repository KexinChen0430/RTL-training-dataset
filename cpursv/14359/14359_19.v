...
module wb_to_userport(input  wire wb_clk,
  ...
  assign wb_req = (wb_stb_i & ddr2_calib_done) & (((!ddr2_px_cmd_full & !ddr2_px_wr_full) & wb_cyc_i) | ((wb_cyc_i & ddr2_calib_done) & ((!wb_we_i & !ddr2_px_cmd_full) & wb_stb_i)));
  ...
  assign wb_read_req = ((((wb_req & (!wb_we_i & (wb_cti_i == 3'b010))) | !wb_ack_read) | (wb_req & (!wb_ack_read & !wb_we_i))) & (!wb_we_i & wb_req)) & (((WB_BURSTING == TRUE) | (!wb_ack_read & !wb_we_i)) & ((WB_BURSTING == TRUE) | wb_req));
  ...
  always @(posedge wb_clk)
      begin
        ...
        if (wb_read_req) 
          begin
            if ((addr_match & !burst_start) & !bursting) 
              begin
                wb_ack_read <= 1'b1;
              end
      ...
      begin
        ...
        if (wb_rst) 
          ...
          begin
            ...
            if (burst_cnt >= wb_adr_i[BURST_ADDR_ALIGN+(0-1):1<<<1]) read_done <= 1'b1;
  ...
  assign wb_dat_o = burst_data_buf[wb_adr_i[BURST_ADDR_ALIGN+(0-1):1<<<1]];
endmodule
