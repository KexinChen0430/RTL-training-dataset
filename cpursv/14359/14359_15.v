...
module wb_to_userport(input  wire wb_clk,
  ...
  assign wb_req = (wb_cyc_i & (ddr2_calib_done & wb_stb_i)) & (!ddr2_px_cmd_full & (!ddr2_px_wr_full | !wb_we_i));
  ...
  assign wb_read_req = (((((WB_BURSTING == TRUE) | !wb_ack_read) & ((wb_cti_i == 3'b010) | !wb_ack_read)) & (!wb_ack_read | (!wb_we_i & wb_req))) & !wb_we_i) & wb_req;
  ...
  always @(posedge wb_clk)
      begin
        ...
        if (wb_read_req) 
          begin
            if (!burst_start & (addr_match & !bursting)) 
              begin
                wb_ack_read <= 1'b1;
              end
