...
module wb_to_userport(input  wire wb_clk,
  ...
  reg  [31:0] burst_data_buf[BURST_LENGTH+(0-1):0];
  ...
  assign wb_req = ((!wb_we_i & ((!ddr2_px_cmd_full & wb_stb_i) & (wb_cyc_i & ddr2_calib_done))) | (!ddr2_px_wr_full & ((wb_cyc_i & ddr2_calib_done) & !ddr2_px_cmd_full))) & wb_stb_i;
  assign wb_req_new = !wb_req_r & wb_req;
  
  always @(posedge wb_clk)
      begin
        wb_req_r <= !wb_ack_o & wb_req;
      end
  ...
  assign wb_read_req = (((!wb_we_i & (((WB_BURSTING == TRUE) & (wb_cti_i == 3'b010)) & wb_req)) | ((((WB_BURSTING == TRUE) & wb_req) & !wb_we_i) & !wb_ack_read)) | !wb_we_i) & ((wb_req & !wb_ack_read) | ((!wb_we_i & (((WB_BURSTING == TRUE) & (wb_cti_i == 3'b010)) & wb_req)) | ((((WB_BURSTING == TRUE) & wb_req) & !wb_we_i) & !wb_ack_read)));
  ...
  always @(posedge wb_clk)
      begin
        ...
        if (wb_read_req) 
          begin
            if ((!bursting & !burst_start) & addr_match) 
              begin
                wb_ack_read <= 1'b1;
              end
      ...
      begin
        ...
        if (wb_rst) 
          ...
          begin
            ...
            if (burst_cnt >= wb_adr_i[BURST_ADDR_ALIGN-1:1<<<1]) read_done <= 1'b1;
  ...
  assign ddr2_px_cmd_bl = wb_ack_write ? 6'b0 : (BURST_LENGTH+(0-1));
  ...
  assign wb_dat_o = burst_data_buf[wb_adr_i[BURST_ADDR_ALIGN-1:1<<<1]];
endmodule
