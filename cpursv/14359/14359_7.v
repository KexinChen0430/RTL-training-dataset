...
module wb_to_userport(input  wire wb_clk,
  ...
  reg  [(-1)+BURST_ADDR_WIDTH:0] burst_cnt;
  reg  bursting;
  wire addr_match;
  reg  burst_start;
  reg  read_done;
  ...
  assign wb_req = ddr2_calib_done & (((wb_cyc_i & wb_stb_i) & (!ddr2_px_cmd_full & !ddr2_px_wr_full)) | (((((!ddr2_px_cmd_full & wb_cyc_i) & wb_stb_i) & ddr2_calib_done) & (wb_stb_i & (!wb_we_i & (!ddr2_px_cmd_full & wb_cyc_i)))) & ddr2_calib_done));
  ...
  assign wb_read_req = (!wb_we_i & (((wb_cti_i == 3'b010) & wb_req) | (!wb_ack_read & (!wb_we_i & wb_req)))) & (!wb_ack_read | ((((!wb_ack_read & (WB_BURSTING == TRUE)) | (wb_cti_i == 3'b010)) & (WB_BURSTING == TRUE)) & (!wb_we_i & wb_req)));
