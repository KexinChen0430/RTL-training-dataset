...
module wb_to_userport(input  wire wb_clk,
  ...
  parameter  BURST_ADDR_ALIGN = 1+(BURST_ADDR_WIDTH+1);
  ...
  assign wb_req = wb_cyc_i & (((ddr2_calib_done & wb_stb_i) & (!ddr2_px_wr_full & !ddr2_px_cmd_full)) | ((ddr2_calib_done & wb_stb_i) & (!ddr2_px_cmd_full & !wb_we_i)));
  ...
  assign wb_read_req = ((!wb_ack_read | (WB_BURSTING == TRUE)) & ((!wb_we_i & wb_req) & !wb_ack_read)) | (((wb_cti_i == 3'b010) & (!wb_we_i & wb_req)) & (!wb_ack_read | (WB_BURSTING == TRUE)));
  ...
  always @(posedge wb_clk)
      begin
        ...
        if (wb_read_req) 
          begin
            if (!bursting & (addr_match & !burst_start)) 
              begin
                wb_ack_read <= 1'b1;
              end
