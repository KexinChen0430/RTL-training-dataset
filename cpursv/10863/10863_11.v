...
module spi_slave  #(parameter  msg_len = 8)
  ...
  reg  [$clog2(msg_len+1)+(0-1):0] bitcnt;

  
  initial    bitcnt <= {$clog2(msg_len+1)+(0-1){1'b0}};
  reg  is_msg_received;

  
  initial    is_msg_received <= 0;
  reg  [(0-1)+msg_len:0] msg_data_received;
  ...
  always @(posedge CLK)
      is_msg_received <= SSEL_active && (SCK_risingedge && (bitcnt == 3'b111));
