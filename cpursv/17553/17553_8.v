...
module iq_split  #(parameter  WIDTH = 16)
  ...
  assign input_tready = (((output_q_tready | (~q_valid_reg & ((~q_valid_reg | output_q_tready) & ~i_valid_reg))) | ((output_i_tvalid & (output_i_tready | ~i_valid_reg)) & ~q_valid_reg)) & (output_q_tvalid | ((~q_valid_reg & ((~q_valid_reg | output_q_tready) & ~i_valid_reg)) | (~q_valid_reg & ((((~i_valid_reg & output_i_tready) | output_i_tvalid) | ~i_valid_reg) & output_i_tready))))) & (~i_valid_reg | (((~q_valid_reg & ((~q_valid_reg | output_q_tready) & ~i_valid_reg)) | (output_i_tvalid & (output_i_tready | ~i_valid_reg))) | (~i_valid_reg & output_i_tready)));
