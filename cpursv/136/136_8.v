        ...
        begin
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    ...
                    pipe_out <= pipe_gen[(BIT_WIDTH*(NUMBER_OF_STAGES-1))+(-1):(NUMBER_OF_STAGES-2)*BIT_WIDTH];
                  end
              end
          for (i = 1; i < (NUMBER_OF_STAGES-1); i = 1+i)
              begin : pipeline
                ...
                always @(posedge clk or negedge reset_n)
                    pipe_gen[((1+i)*BIT_WIDTH)+(-1):BIT_WIDTH*i] <= !reset_n ? 0 : pipe_gen[(-1)+(BIT_WIDTH*i):BIT_WIDTH*(i-1)];
              end
        end
  endgenerate
