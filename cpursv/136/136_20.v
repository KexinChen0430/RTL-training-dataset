...
module pipeline_registers(input  clk,
  ...
  generate
      ...
      if (NUMBER_OF_STAGES == 0) 
        ...
        begin
          reg  [(-1)+(BIT_WIDTH*((-1)+NUMBER_OF_STAGES)):0] pipe_gen;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    ...
                    pipe_out <= pipe_gen[(-1)+(BIT_WIDTH*((-1)+NUMBER_OF_STAGES)):BIT_WIDTH*(NUMBER_OF_STAGES-1<<<1)];
                  end
              end
          for (i = 1; i < ((-1)+NUMBER_OF_STAGES); i = 1+i)
              begin : pipeline
                ...
                always @(posedge clk or negedge reset_n)
                    pipe_gen[(BIT_WIDTH*(1+i))+(-1):BIT_WIDTH*i] <= !reset_n ? 0 : pipe_gen[(-1)+(BIT_WIDTH*i):BIT_WIDTH*(i-1)];
              end
        end
  endgenerate
