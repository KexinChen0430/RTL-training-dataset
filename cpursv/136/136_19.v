...
module pipeline_registers(input  clk,
  ...
  generate
      ...
      if (NUMBER_OF_STAGES == 0) 
        ...
        begin
          reg  [((NUMBER_OF_STAGES-1)*BIT_WIDTH)-1:0] pipe_gen;
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    ...
                    pipe_out <= pipe_gen[((NUMBER_OF_STAGES-1)*BIT_WIDTH)-1:BIT_WIDTH*(NUMBER_OF_STAGES-1<<<1)];
                  end
              end
          for (i = 1; i < (NUMBER_OF_STAGES-1); i = 1+i)
              begin : pipeline
                ...
                always @(posedge clk or negedge reset_n)
                    pipe_gen[((1+i)*BIT_WIDTH)-1:BIT_WIDTH*i] <= !reset_n ? 0 : pipe_gen[(BIT_WIDTH*i)+(0-1):(i+(0-1))*BIT_WIDTH];
              end
        end
  endgenerate
