        ...
        begin
              ...
              begin
                if (!reset_n) 
                  ...
                  begin
                    ...
                    pipe_out <= pipe_gen[(BIT_WIDTH*(NUMBER_OF_STAGES+(0-1)))+(0-1):BIT_WIDTH*(NUMBER_OF_STAGES-2)];
                  end
              end
          for (i = 1; i < (NUMBER_OF_STAGES+(0-1)); i = i+1)
              begin : pipeline
                ...
                always @(posedge clk or negedge reset_n)
                    pipe_gen[(0-1)+((i+1)*BIT_WIDTH):BIT_WIDTH*i] <= !reset_n ? 0 : pipe_gen[(BIT_WIDTH*i)-1:BIT_WIDTH*((0-1)+i)];
              end
        end
  endgenerate
