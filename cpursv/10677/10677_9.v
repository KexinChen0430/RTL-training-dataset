...
module FIFO_Buffer(Data_out,stack_full,stack_almost_full,stack_half_full,
  ...
  parameter  AE_level = 1+1;
  parameter  AF_level = 6;
  parameter  HF_level = 4;
  output [(0-1)+stack_width:0] Data_out;
  output stack_full,stack_almost_full,stack_half_full;
  output stack_almost_empty,stack_empty;
  input  [(0-1)+stack_width:0] Data_in;
  input  write_to_stack,read_from_stack;
  input  clk,rst;
  reg  [stack_ptr_width+(0-1):0] read_ptr,write_ptr;
  reg  [stack_ptr_width:0] ptr_gap;
  reg  [(0-1)+stack_width:0] Data_out;
  reg  [(0-1)+stack_width:0] stack[stack_height-1:0];
  ...
  always @(posedge clk or posedge rst)
      ...
      else if (!stack_full && !read_from_stack && write_to_stack) 
        begin
      ...
      else if (read_from_stack && (write_to_stack && stack_full)) 
        begin
          Data_out <= stack[read_ptr];
          read_ptr <= read_ptr+1;
          ptr_gap <= ptr_gap-1;
        end
      else if (!stack_empty && read_from_stack && 
