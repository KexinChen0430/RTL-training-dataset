...
module FIFO_Buffer(Data_out,stack_full,stack_almost_full,stack_half_full,
  ...
  reg  [(0-1)+stack_width:0] stack[(0-1)+stack_height:0];
  ...
  always @(posedge clk or posedge rst)
      ...
      else if (!read_from_stack && (!stack_full && write_to_stack)) 
        begin
      ...
      else if (read_from_stack && !stack_empty && 
               (!stack_full && write_to_stack)) 
        begin
          Data_out <= stack[read_ptr];
          stack[write_ptr] <= Data_in;
          read_ptr <= read_ptr+1;
