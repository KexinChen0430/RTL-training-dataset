...
module jt10_adpcmb(input  rst_n,
  ...
  reg  signed  [(-1)+xw:0] x1,next_x5;
  reg  [stepw-1:0] step1;
  reg  [1+stepw:0] next_step3;

  assign pcm = x1[(-1)+xw:xw-16];
  wire [(-1)+xw:0]  limpos = {1'b0,{(-1)+xw{1'b1}}};

  wire [(-1)+xw:0]  limneg = {1'b1,{(-1)+xw{1'b0}}};
            ...
            begin
              ...
              if (chon) 
                begin
                  if (adv2[0]) 
                    begin
                      if ((x1[(-1)+xw] != next_x5[(-1)+xw]) && 
                          (sign_data5 == x1[(-1)+xw])) x1 <= x1[(-1)+xw] ? limneg : limpos;
                      else x1 <= next_x5;
