...
module jt10_adpcmb(input  rst_n,
                   input  clk,
                   input  cen,
                   input  [3:0] data,
                   input  chon,
  ...
  reg  signed  [(0-1)+xw:0] x1,next_x5;
  reg  [stepw+(0-1):0] step1;
  reg  [1+stepw:0] next_step3;

  assign pcm = x1[(0-1)+xw:(0-16)+xw];
  wire [(0-1)+xw:0]  limpos = {1'b0,{(0-1)+xw{1'b1}}};

  wire [(0-1)+xw:0]  limneg = {1'b1,{(0-1)+xw{1'b0}}};
  ...
  wire [3:0]  data_use = (~chon || clr) ? 4'd0 : data;
        ...
        begin
          ...
          if (cen) 
            begin
              ...
              if (adv) 
                begin
                  d2 <= {data_use[1<<1:0],1'b1};
                  sign_data2 <= data_use[3];
                  adv2[3] <= 1'b1;
                end
                
              d3 <= {{(0-16)+xw{1'b0}},d2l[18:3]};
              ...
              if (chon) 
                begin
                  if (adv2[0]) 
                    begin
                      if ((sign_data5 == x1[(0-1)+xw]) && 
                          (x1[(0-1)+xw] != next_x5[(0-1)+xw])) x1 <= x1[(0-1)+xw] ? limneg : limpos;
                      else x1 <= next_x5;
