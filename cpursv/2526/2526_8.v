...
module jt10_adpcm_cnt(input  rst_n,
  ...
  wire 
       active5 = 
(
((cur_ch[2] || en_ch[5]) && (en_ch[5] || en_ch[4])) || 
((en_ch[1] && cur_ch[4]) || 
(en_ch[2] && 
((en_ch[1] || cur_ch[5]) && (cur_ch[4] || cur_ch[5]))) || (en_ch[2] || (cur_ch[1] && en_ch[3])))) && 
(
((cur_ch[2] || en_ch[5]) && (en_ch[5] || en_ch[4])) || 
((cur_ch[1] && en_ch[3]) || 
(
(cur_ch[4] || cur_ch[5] || (cur_ch[0] && en_ch[2])) && (en_ch[1] || cur_ch[5] || (cur_ch[0] && en_ch[2]))))) && 
(
(
(
(
((en_ch[4] && cur_ch[2]) || 
(en_ch[3] || 
((cur_ch[4] || en_ch[2]) && (en_ch[2] || en_ch[1])))) && 
(
(
(
((en_ch[1] || cur_ch[5]) && (cur_ch[4] || cur_ch[5])) || cur_ch[0]) && 
(
((cur_ch[4] || en_ch[2]) && (en_ch[2] || en_ch[1])) || cur_ch[0])) || en_ch[3] || (en_ch[4] && cur_ch[2]))) || cur_ch[3]) && cur_ch[3]) || 
(
(
(
((en_ch[4] && cur_ch[2]) || 
...
((en_ch[1] || cur_ch[5]) && (cur_ch[4] || cur_ch[5])) || cur_ch[0]) && 
(
((cur_ch[4] || en_ch[2]) && (en_ch[2] || en_ch[1])) || cur_ch[0])) || en_ch[3] || (en_ch[4] && cur_ch[2]))) || cur_ch[3]) && 
((en_ch[4] && cur_ch[2]) || cur_ch[1] || 
((en_ch[1] && cur_ch[4]) || 
(en_ch[2] && 
((en_ch[1] || cur_ch[5]) && (cur_ch[4] || cur_ch[5]))) || (cur_ch[0] && en_ch[2])))));
  ...
  always @(posedge clk or negedge rst_n)
      if (!rst_n) 
        ...
        begin
          ...
          clr5 <= clr4;
          done5 <= ~(clr4 && on4) && 
                  ((addr4[20:9] == end4) && (addr4[8:0] == ~9'b0));
          start5 <= start4;
          end5 <= end4;
          bank5 <= bank4;
          skip5 <= skip4;
