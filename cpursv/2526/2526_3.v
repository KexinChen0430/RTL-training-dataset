...
(
(
((en_ch[2] || cur_ch[4]) && (cur_ch[5] || cur_ch[4])) || en_ch[3]) && 
(en_ch[3] || 
((en_ch[1] || en_ch[2]) && (en_ch[1] || cur_ch[5])))) || 
(((cur_ch[4] && en_ch[1]) || en_ch[2]) && en_ch[2] && 
cur_ch[0])) && 
(
(
((cur_ch[4] && (en_ch[1] || cur_ch[5])) || 
((en_ch[1] || cur_ch[5]) && cur_ch[5])) && ((cur_ch[4] && en_ch[1]) || en_ch[2])) || 
(cur_ch[1] || 
(((cur_ch[4] && en_ch[1]) || en_ch[2]) && en_ch[2] && 
cur_ch[0])))) || (cur_ch[3] || (en_ch[4] && cur_ch[2]))) && 
(
(
(cur_ch[4] && 
(en_ch[1] || 
...
(((cur_ch[4] && en_ch[1]) || en_ch[2]) && en_ch[2] && 
cur_ch[0])) || cur_ch[5] || 
((en_ch[3] || (en_ch[4] && cur_ch[2])) && 
((en_ch[4] && cur_ch[2]) || cur_ch[1])) || en_ch[5]) && 
(en_ch[5] || 
(
(en_ch[2] || 
((cur_ch[4] && (en_ch[1] || cur_ch[5])) || 
...
((en_ch[3] || (en_ch[4] && cur_ch[2])) && 
((en_ch[4] && cur_ch[2]) || cur_ch[1]))) && 
(
((en_ch[3] || (en_ch[4] && cur_ch[2])) && 
((en_ch[4] && cur_ch[2]) || cur_ch[1])) || 
(en_ch[2] || ((cur_ch[4] && en_ch[1]) || en_ch[2]))))));
  ...
  always @(posedge clk or negedge rst_n)
      if (!rst_n) 
        ...
        begin
          flags <= ((flags & (set_flags | ~clr_flags)) | set_flags) & ~clr_flags;
        end
      ...
      if (!rst_n) 
        ...
        begin
          ...
          addr1 <= (clr6 && on6) ? {start6,9'd0} : 
                  (~skip6 && sumup6) ? (21'd1+addr6) : addr6;
          on1 <= on6;
          done1 <= done6;
          start1 <= start6;
          end1 <= end6;
