...
(
(
((cur_ch[5] || en_ch[1]) && (en_ch[2] || en_ch[1])) || cur_ch[0]) && 
(cur_ch[0] || 
((en_ch[2] || cur_ch[4]) && (cur_ch[5] || cur_ch[4]))))) && 
(
(
(
((cur_ch[5] || en_ch[1]) && (en_ch[2] || en_ch[1])) || cur_ch[0]) && 
(cur_ch[0] || 
((en_ch[2] || cur_ch[4]) && (cur_ch[5] || cur_ch[4])))) || cur_ch[1])) || cur_ch[3]) && 
(en_ch[4] || 
(
(en_ch[3] || 
(
((en_ch[2] && cur_ch[0]) || (en_ch[2] || cur_ch[4])) && (en_ch[2] || en_ch[1] || (en_ch[2] && cur_ch[0])))) && 
(
(
((en_ch[2] && cur_ch[0]) || (en_ch[2] || cur_ch[4])) && (en_ch[2] || en_ch[1] || (en_ch[2] && cur_ch[0]))) || cur_ch[1])) || cur_ch[3]) && 
(
(
((cur_ch[1] || (en_ch[4] && cur_ch[2]) || en_ch[5]) && 
(en_ch[3] || (en_ch[4] && cur_ch[2]) || en_ch[5])) || 
(
(en_ch[2] && ((en_ch[2] && cur_ch[0]) || cur_ch[5])) || cur_ch[4])) && 
(
((cur_ch[1] || (en_ch[4] && cur_ch[2]) || en_ch[5]) && 
(en_ch[3] || (en_ch[4] && cur_ch[2]) || en_ch[5])) || 
(en_ch[1] || 
(en_ch[2] && ((en_ch[2] && cur_ch[0]) || cur_ch[5])))) && 
(cur_ch[3] || 
(
(
(
((en_ch[2] && cur_ch[0]) || (en_ch[2] || cur_ch[4])) && (en_ch[2] || en_ch[1] || (en_ch[2] && cur_ch[0]))) || 
((en_ch[3] || cur_ch[2]) && (cur_ch[1] || cur_ch[2]))) && 
(
((en_ch[3] || cur_ch[2]) && (cur_ch[1] || cur_ch[2])) || 
((en_ch[2] && cur_ch[0]) || cur_ch[5] || 
(en_ch[1] && 
((cur_ch[5] || cur_ch[4]) && (cur_ch[5] || en_ch[1])) && cur_ch[4]))))));
  ...
  always @(posedge clk or negedge rst_n)
      if (!rst_n) 
        ...
        begin
          ...
          on5 <= on4;
          clr5 <= clr4;
          done5 <= (addr4[8:0] == ~9'b0) && 
                  ((addr4[20:9] == end4) && ~(on4 && clr4));
          start5 <= start4;
          end5 <= end4;
          bank5 <= bank4;
          skip5 <= skip4;
          ...
          addr1 <= (on6 && clr6) ? {start6,9'd0} : 
                  (~skip6 && sumup6) ? (addr6+21'd1) : addr6;
          on1 <= on6;
          done1 <= done6;
          start1 <= start6;
          end1 <= end6;
