...
module jt10_adpcm_cnt(input  rst_n,
  ...
  assign decon = decon1;
  wire 
       active5 = (en_ch[4] && cur_ch[2]) || 
(
(
(
((en_ch[2] || cur_ch[4]) && (cur_ch[4] || cur_ch[5])) || 
((en_ch[3] || en_ch[2]) && (cur_ch[0] || en_ch[3]))) && 
(
((en_ch[1] || cur_ch[5]) && (en_ch[2] || en_ch[1])) || 
((en_ch[3] || en_ch[2]) && (cur_ch[0] || en_ch[3]))) && cur_ch[1]) || 
(
(
(
(((en_ch[1] || cur_ch[5]) && cur_ch[5]) || 
(cur_ch[4] && (en_ch[1] || cur_ch[5]))) && en_ch[1] && cur_ch[4]) || (cur_ch[5] || (cur_ch[0] && en_ch[2])) || 
(en_ch[5] && cur_ch[3])) && 
(
(
(
...
(
((en_ch[1] || cur_ch[5]) && (en_ch[2] || en_ch[1])) || 
((en_ch[3] || en_ch[2]) && (cur_ch[0] || en_ch[3]))) && en_ch[2]) || 
(
(((en_ch[1] || cur_ch[5]) && cur_ch[5]) || 
(cur_ch[4] && (en_ch[1] || cur_ch[5]))) && en_ch[1] && cur_ch[4]) || (en_ch[5] && cur_ch[3]))));

  wire  sumup5 = !done5 && on5 && active5;
  ...
  always @(posedge clk or negedge rst_n)
      if (!rst_n) 
        ...
        begin
          flags <= ~clr_flags & ((flags | ~clr_flags) & (((set_flags | ~clr_flags) & flags) | set_flags));
        end
