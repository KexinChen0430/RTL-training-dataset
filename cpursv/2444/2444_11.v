...
module adapter_ppfifo_2_axi_stream  #(parameter 
  ...
  (input  rst,
   ...
   input  [1+(DATA_WIDTH+(0-1)):0] i_ppfifo_data,
   output o_ppfifo_stb,
   input  i_axi_clk,
   output [3:0] o_axi_user,
   input  i_axi_ready,
   output [DATA_WIDTH+(0-1):0] o_axi_data,
   output o_axi_last,
  ...
  assign o_axi_data = i_ppfifo_data[DATA_WIDTH+(0-1):0];
  ...
  assign o_axi_last = (o_axi_valid & o_ppfifo_act) & ((r_count+1) >= i_ppfifo_size);
