...
module adapter_ppfifo_2_axi_stream  #(parameter 
       DATA_WIDTH   = 24,
  ...
  (input  rst,
   ...
   input  [((0-1)+DATA_WIDTH)+1:0] i_ppfifo_data,
   output o_ppfifo_stb,
   input  i_axi_clk,
  ...
  assign o_axi_data = i_ppfifo_data[(0-1)+DATA_WIDTH:0];
  assign o_ppfifo_stb = o_axi_valid & i_axi_ready;
  assign o_axi_user[0] = (r_count < i_ppfifo_size) ? i_ppfifo_data[DATA_WIDTH] : 1'b0;
  assign o_axi_user[3:1] = 3'h0;
  assign o_axi_last = (o_ppfifo_act & ((1+r_count) >= i_ppfifo_size)) & o_axi_valid;
        ...
        if (rst) 
          ...
          begin
            case (state)
              ...
              READY: begin
                    if (r_count < i_ppfifo_size) 
                      begin
                        o_axi_valid <= 1;
                        if (i_axi_ready && o_axi_valid) 
                          begin
                            r_count <= 1+r_count;
                            if ((1+r_count) >= i_ppfifo_size) 
                              begin
                                o_axi_valid <= 0;
                              end
