...
module rgb_led(pclk,nreset,bus_write_en,bus_read_en,bus_addr,bus_write_data,
  ...
  always @(posedge pclk)
      begin
        ...
        brightness_factor <= 16'd3921*((1/32'd100000000)*brightness);
      end
        ...
        if (~nreset) 
          ...
          begin
            ...
            if (duty_cycle_counter == 32'd1000000) 
              ...
              begin
                if ((red*brightness_factor) == duty_cycle_counter) 
                  begin
                    pwm_r <= 0;
                  end
                  
                if ((blue*brightness_factor) == duty_cycle_counter) 
                  begin
                    pwm_b <= 0;
                  end
                  
                if ((brightness_factor*green) == duty_cycle_counter) 
                  begin
                    pwm_g <= 0;
                  end
