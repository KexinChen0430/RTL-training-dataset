...
module spi_master_reduced(clk,rst_n,spi_miso,spi_mosi,spi_clk,spi_tx_en,spi_rx_en,
                          mode_select,receive_status);
...
(spi_rx_en || (spi_tx_en && (data_count < DATA_LENGTH)) || 
spi_tx_en) && (data_count < DATA_LENGTH)) 
          begin
            if (cnt8 < mode_reg) cnt8 <= 1'b1+cnt8;
            else 
              begin
                if (spi_tx_en && 
                    (spi_rx_en && ((data_count < DATA_LENGTH) || spi_tx_en))) 
                  begin
                    cnt8 <= 5'd0;
