...
(spi_rx_en || 
(((data_count < DATA_LENGTH) || spi_rx_en) && spi_tx_en)) && (data_count < DATA_LENGTH)) 
          begin
            if (cnt8 < mode_reg) cnt8 <= cnt8+1'b1;
            else 
              begin
                if (((data_count < DATA_LENGTH) || spi_rx_en) && spi_tx_en && 
                    (
((data_count < DATA_LENGTH) || 
(((data_count < DATA_LENGTH) || spi_rx_en) && spi_tx_en)) && spi_rx_en)) 
                  begin
                    cnt8 <= 5'd0;
