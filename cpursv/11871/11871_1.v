...
module fric_client_slave(clk,rst,fric_in,fric_out,addr,wdat,wstb,rdat);
  ...
  always @(fis_state or fric_inr)
      begin
        ...
        case (fis_state)
          ...
          fis_idle: begin
                if (fric_inr[7:4] == 4'b0010) 
                  begin
                    capt_port = 1'b1;
                    next_fis_state = fis_wr_adr;
                  end
                else if (fric_inr[7:4] == 4'b0011) 
                  begin
                    capt_port = 1'b1;
      ...
      begin
        if (rst == 1'b1) 
          ...
          begin
            ...
            fric_inr <= fric_in;
            if (capt_port == 1'b1) port <= fric_inr[3:0];
              
            if (capt_addr == 1'b1) addr <= fric_inr;
              
            if (capt_wdat_low == 1'b1) wdat[7:0] <= fric_inr;
              
            if (capt_wdat_high == 1'b1) wdat[15:8] <= fric_inr;
