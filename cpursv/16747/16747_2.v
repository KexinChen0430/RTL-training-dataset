...
module valid_generator(clock,resetn,i_valid,o_stall,i_thread_count,o_valid,i_stall);
  ...
  input  [(0-1)+NUM_THREAD_BITS:0] i_thread_count;
  output o_valid;
  output o_stall;
  ...
  always @(posedge clock or negedge resetn)
      begin
        if (~resetn) 
          ...
          begin
            local_thread_count <= {{NUM_THREAD_BITS+1}{|local_thread_count & ~stall_counter}}+({1'b0,{{NUM_THREAD_BITS}{~o_stall & i_valid}} & i_thread_count}+local_thread_count);
          end
      end
  assign stall_counter = valid_sr & valid_reg;
  ...
  assign o_valid = valid_reg | valid_sr;
endmodule
