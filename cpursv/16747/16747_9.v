...
module valid_generator(clock,resetn,i_valid,o_stall,i_thread_count,o_valid,i_stall);
  ...
  input  [(-1)+NUM_THREAD_BITS:0] i_thread_count;
  output o_valid;
  output o_stall;
  ...
  always @(posedge clock or negedge resetn)
      begin
        if (~resetn) 
          ...
          begin
            local_thread_count <= ({1'b0,i_thread_count & {{NUM_THREAD_BITS}{~o_stall & i_valid}}}+local_thread_count)+{{1+NUM_THREAD_BITS}{~stall_counter & |local_thread_count}};
          end
      end
