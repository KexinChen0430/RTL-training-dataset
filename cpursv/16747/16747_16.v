...
module valid_generator(clock,resetn,i_valid,o_stall,i_thread_count,o_valid,i_stall);
  ...
  input  [NUM_THREAD_BITS+(0-1):0] i_thread_count;
  output o_valid;
  output o_stall;
  ...
  always @(posedge clock or negedge resetn)
      begin
        if (~resetn) 
          ...
          begin
            local_thread_count <= local_thread_count+({{NUM_THREAD_BITS+1}{~stall_counter & |local_thread_count}}+{1'b0,{{NUM_THREAD_BITS}{~o_stall & i_valid}} & i_thread_count});
          end
      end
