...
module aur1_CHANNEL_ERROR_DETECT(SOFT_ERROR,HARD_ERROR,LANE_UP,USER_CLK,POWER_DOWN,
  ...
  assign channel_soft_error_c = (soft_error_r[3] | soft_error_r[2]) | (soft_error_r[1] | soft_error_r[0]);
  
  always @(posedge USER_CLK)
      CHANNEL_SOFT_ERROR <= #1 channel_soft_error_c;
  
  initial    CHANNEL_HARD_ERROR = 1'b1;
  assign channel_hard_error_c = hard_error_r[1] | ((hard_error_r[0] | hard_error_r[2]) | hard_error_r[3]);
  
  always @(posedge USER_CLK)
      CHANNEL_HARD_ERROR <= #1 channel_hard_error_c;
  
  initial    RESET_CHANNEL = 1'b1;
  assign reset_channel_c = (!LANE_UP[2] | !LANE_UP[1]) | (!LANE_UP[0] | !LANE_UP[3]);
