...
module VGAController(PixelClock,inRed,inGreen,inBlue,outRed,outGreen,outBlue,
  ...
  assign XPosition = XTiming-(XBackPorch+XSynchPulse);
  ...
  always @(posedge PixelClock)
      begin
        ...
        else if ((YTiming < YLimit) && (XTiming >= XLimit)) YTiming <= YTiming+1;
  ...
  assign outRed = ((XTiming >= (XBackPorch+XSynchPulse)) && 
                  (XTiming <= (XBackPorch+(XVisible+XSynchPulse)))) ? inRed : 8'b0;
  assign outGreen = ((XTiming >= (XBackPorch+XSynchPulse)) && 
                    (XTiming <= (XBackPorch+(XVisible+XSynchPulse)))) ? inGreen : 8'b0;
  assign outBlue = ((XTiming >= (XBackPorch+XSynchPulse)) && 
                   (XTiming <= (XBackPorch+(XVisible+XSynchPulse)))) ? inBlue : 8'b0;
