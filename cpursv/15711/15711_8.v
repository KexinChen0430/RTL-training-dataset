...
module VGAController(PixelClock,inRed,inGreen,inBlue,outRed,outGreen,outBlue,
  ...
  assign YPosition = YTiming+(0-(YSynchPulse+YBackPorch));
  
  always @(posedge PixelClock)
      begin
        if (XTiming >= XLimit) XTiming <= 11'd0;
        else XTiming <= XTiming+1;
      end
  
  always @(posedge PixelClock)
      begin
        if ((XTiming >= XLimit) && (YTiming >= YLimit)) YTiming <= 11'd0;
        else if ((XTiming >= XLimit) && (YTiming < YLimit)) YTiming <= 1+YTiming;
        else YTiming <= YTiming;
      end
  
  always @(posedge PixelClock)
      begin
        if ((YTiming < YSynchPulse) && (YTiming >= 0)) VertSynch <= 1'b0;
        else VertSynch <= 1'b1;
      end
  ...
  assign outRed = ((XTiming <= ((XBackPorch+XVisible)+XSynchPulse)) && 
                  (XTiming >= (XSynchPulse+XBackPorch))) ? inRed : 8'b0;
  assign outGreen = ((XTiming <= ((XBackPorch+XVisible)+XSynchPulse)) && 
                    (XTiming >= (XSynchPulse+XBackPorch))) ? inGreen : 8'b0;
  assign outBlue = ((XTiming <= ((XBackPorch+XVisible)+XSynchPulse)) && 
                   (XTiming >= (XSynchPulse+XBackPorch))) ? inBlue : 8'b0;
