...
module VGAController(PixelClock,inRed,inGreen,inBlue,outRed,outGreen,outBlue,
  ...
  assign XPosition = XTiming+(0-(XSynchPulse+XBackPorch));
  assign YPosition = YTiming+(0-(YBackPorch+YSynchPulse));
  ...
  assign outRed = ((XTiming <= (XVisible+(XSynchPulse+XBackPorch))) && 
                  (XTiming >= (XSynchPulse+XBackPorch))) ? inRed : 8'b0;
  assign outGreen = ((XTiming <= (XVisible+(XSynchPulse+XBackPorch))) && 
                    (XTiming >= (XSynchPulse+XBackPorch))) ? inGreen : 8'b0;
  assign outBlue = ((XTiming <= (XVisible+(XSynchPulse+XBackPorch))) && 
