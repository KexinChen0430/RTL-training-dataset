...
module altera_onchip_flash_convert_address(address,flash_addr);
  ...
  assign flash_addr = (address <= ADDR_RANGE1_END_ADDR[(0-1)+FLASH_ADDR_WIDTH:0]) ? (address+ADDR_RANGE1_OFFSET[(0-1)+FLASH_ADDR_WIDTH:0]) : 
                      (address <= ADDR_RANGE2_END_ADDR[(0-1)+FLASH_ADDR_WIDTH:0]) ? (address+ADDR_RANGE2_OFFSET[(0-1)+FLASH_ADDR_WIDTH:0]) : (ADDR_RANGE3_OFFSET[(0-1)+FLASH_ADDR_WIDTH:0]+address);
endmodule
