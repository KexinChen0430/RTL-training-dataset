...
module altera_onchip_flash_convert_address(address,flash_addr);
  ...
  assign flash_addr = (address <= ADDR_RANGE1_END_ADDR[FLASH_ADDR_WIDTH-1:0]) ? (ADDR_RANGE1_OFFSET[FLASH_ADDR_WIDTH-1:0]+address) : 
                      (address <= ADDR_RANGE2_END_ADDR[FLASH_ADDR_WIDTH-1:0]) ? (address+ADDR_RANGE2_OFFSET[FLASH_ADDR_WIDTH-1:0]) : (address+ADDR_RANGE3_OFFSET[FLASH_ADDR_WIDTH-1:0]);
endmodule
