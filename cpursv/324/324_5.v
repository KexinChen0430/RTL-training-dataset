...
module data_loader  #(parameter 
  ...
  (input  clk,
   ...
   output reg [(-1)+ADDRESS_WIDTH:0] control_write_base,
   output reg [(-1)+ADDRESS_WIDTH:0] control_write_length,
   output reg control_go,
   input  wire control_done,
   output reg user_write_buffer,
   output reg [(-1)+DATA_WIDTH:0] user_buffer_input_data,
  ...
  reg  [(-1)+ADDRESS_WIDTH:0] control_write_base_next;
  reg  [(-1)+ADDRESS_WIDTH:0] control_write_length_next;
  reg  control_go_next;
  reg  user_write_buffer_next;
  reg  [(-1)+DATA_WIDTH:0] user_buffer_input_data_next;
  reg  [NUM_STATES+(-1):0] state,state_next;
  reg  [(-1)+DATA_WIDTH:0] key,val,key_next,val_next;
