...
module data_loader  #(parameter 
  ...
  (input  clk,
   ...
   output reg [ADDRESS_WIDTH+(0-1):0] control_write_base,
   output reg [ADDRESS_WIDTH+(0-1):0] control_write_length,
  ...
  reg  [ADDRESS_WIDTH+(0-1):0] control_write_base_next;
  reg  [ADDRESS_WIDTH+(0-1):0] control_write_length_next;
  reg  control_go_next;
      ...
      begin
        ...
        case (state)
          ...
          STATE_COMPUTE_ADDRESS: begin
                control_write_base_next = (key*(2**BLOCK_SIZE))+DRAM_BASE_ADDR;
