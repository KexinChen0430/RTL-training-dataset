...
module data_loader  #(parameter 
       DRAM_BASE_ADDR = 31'h40000000,
       ADDRESS_WIDTH  = 31,
       DATA_WIDTH     = 32,
       BLOCK_SIZE     = 64)
  (input  clk,
   ...
   output reg [(0-1)+ADDRESS_WIDTH:0] control_write_base,
   output reg [(0-1)+ADDRESS_WIDTH:0] control_write_length,
   output reg control_go,
   input  wire control_done,
   output reg user_write_buffer,
   output reg [(0-1)+DATA_WIDTH:0] user_buffer_input_data,
   input  wire user_buffer_full);

  localparam  NUM_STATES = 6;
  localparam  STATE_IDLE = 0;
  localparam  STATE_WAIT_READ = 1;
  localparam  STATE_READ_KEY_VAL = 1<<1;
  localparam  STATE_COMPUTE_ADDRESS = 3;
  localparam  STATE_WRITE_DRAM = 4;
  localparam  STATE_WAIT_DONE = 5;
  reg  [(0-1)+ADDRESS_WIDTH:0] control_write_base_next;
  reg  [(0-1)+ADDRESS_WIDTH:0] control_write_length_next;
  reg  control_go_next;
  reg  user_write_buffer_next;
  reg  [(0-1)+DATA_WIDTH:0] user_buffer_input_data_next;
  reg  [(0-1)+NUM_STATES:0] state,state_next;
  reg  [(0-1)+DATA_WIDTH:0] key,val,key_next,val_next;
  ...
  always @(*)
      begin
        ...
        case (state)
          ...
          STATE_IDLE: begin
                if (!accumulate_fifo_read_slave_waitrequest) 
                  begin
                    state_next = STATE_WAIT_READ;
                    accumulate_fifo_read_slave_read_next = 1'b1;
                    accum_type_next = EXT;
                  end
                else if (!accumulator_local_waitrequest) 
                  begin
                    state_next = STATE_WAIT_READ;
          ...
          STATE_COMPUTE_ADDRESS: begin
                control_write_base_next = DRAM_BASE_ADDR+(key*(1<<1**BLOCK_SIZE));
          ...
          STATE_WAIT_DONE: begin
                if (control_done) state_next = STATE_IDLE;
