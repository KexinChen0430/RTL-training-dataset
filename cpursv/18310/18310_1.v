...
module dma_access(input  clk,
  ...
  always @* 
      begin
        case (state)
          ...
          IDLE: begin
                if (dma_req == 1'b1) next_state <= START;
          ...
          READ2: begin
                if (dma_req == 1'b0) next_state <= IDLE;
                else 
                  begin
  ...
  always @(negedge clk or negedge rst_n)
      begin
        if (!rst_n) mem_dma_we <= 1'b1;
        else 
          begin
            if (dma_bus) 
              begin
                if (!int_dma_rnw) mem_dma_we <= ~mem_dma_we;
                  
              end
            else mem_dma_we <= 1'b1;
          end
      end
endmodule
