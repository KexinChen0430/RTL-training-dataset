...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = WRITE | ((USER & (WRITE & RMW)) | READ);
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : (USER & MCR_FLAGS[2]);
  assign as_sorte = (MVALID[31:16] & maske) != 16'h0;
  assign match = (((MVALID[15:0] & maske) != 16'h0000) & (VADR_R[31:20] == MMU_VA[31:20])) & (adr_space == as_sorte);
  assign alles_ok = ((((~PROT_ERROR | (match & ~WRITE)) & (match | ~WRITE)) & ~PROT_ERROR) & match) & ((MMU_VA[17] & match) | (((match & ~WRITE) & (~WRITE & MMU_VA[17])) | ((match & ~WRITE) & (~PROT_ERROR & ~WRITE))));
  assign MMU_HIT = zugriff ? (VIRTUELL ? alles_ok : 1'b1) : 1'b0;
  assign val_bits = IVAR[1] ? (MVALID[15:0] & (match ? ~maske : 16'hFFFF)) : (maske | MVALID[15:0]);
  assign as_bits = IVAR[1] ? MVALID[31:16] : 
                   adr_space ? (maske | MVALID[31:16]) : (MVALID[31:16] & ~maske);
  assign UPDATE = {as_bits,val_bits};
  assign ena_prot = ((match | ~WRITE) & VIRTUELL) & (((MMU_VA[17] | match) & ((match | ~WRITE) & zugriff)) & match);
  ...
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})
        ...
        3'b110: PROT_ERROR = (WRITE | RMW) & USER;
