...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = WRITE | (((RMW & WRITE) & USER) | READ);
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : (((USER | RMW) & (MCR_FLAGS[2] & (WRITE | USER))) & USER);
  assign as_sorte = (MVALID[31:16] & maske) != 16'h0;
  assign match = ((maske & MVALID[15:0]) != 16'h0000) & ((adr_space == as_sorte) & (VADR_R[31:20] == MMU_VA[31:20]));
  assign alles_ok = (match & ~PROT_ERROR) & ((~WRITE & ~PROT_ERROR) | (MMU_VA[17] | ((match & ~PROT_ERROR) & (~WRITE & ~PROT_ERROR))));
  ...
  assign ena_prot = (VIRTUELL & match) & zugriff;
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = ((RMW | ((USER | RMW) & WRITE)) | (WRITE | (RMW & USER))) | USER;

        3'b101: PROT_ERROR = USER;

        3'b110: PROT_ERROR = (WRITE | (RMW & USER)) & USER;
