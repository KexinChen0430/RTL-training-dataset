...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign match = (adr_space == as_sorte) & ((VADR_R[31:20] == MMU_VA[31:20]) & ((maske & MVALID[15:0]) != 16'h0000));
  assign alles_ok = (~PROT_ERROR & (match | ~WRITE)) & ((match & ~WRITE) | ((~PROT_ERROR & (match | ~WRITE)) & (MMU_VA[17] & match)));
  ...
  assign ena_prot = ((VIRTUELL & ((match | ~WRITE) & zugriff)) & (MMU_VA[17] | match)) & match;
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = (RMW | (USER & WRITE)) | (USER | WRITE);
