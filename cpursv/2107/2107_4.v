...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = WRITE | (READ | ((WRITE & USER) & (RMW & WRITE)));
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : (((RMW | USER) & MCR_FLAGS[2]) & USER);
  ...
  assign alles_ok = ((match & ((~WRITE | match) & ~PROT_ERROR)) | (~WRITE & ~PROT_ERROR)) & (((match & ((~WRITE | match) & ~PROT_ERROR)) | (match & (MMU_VA[17] & (~WRITE | match)))) & (~WRITE | ((~WRITE & ~PROT_ERROR) | (match & (MMU_VA[17] & (~WRITE | match))))));
  ...
  assign ena_prot = match & (VIRTUELL & zugriff);
  ...
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})
        ...
        3'b110: PROT_ERROR = (WRITE & USER) | (USER & RMW);
