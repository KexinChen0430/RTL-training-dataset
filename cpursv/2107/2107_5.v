...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = (READ | (USER & WRITE)) | WRITE;
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : (((USER | RMW) & (MCR_FLAGS[2] & (USER | WRITE))) & USER);
  ...
  assign alles_ok = ((MMU_VA[17] & match) | ~WRITE) & (match & ~PROT_ERROR);
  ...
  assign ena_prot = ((match | MMU_VA[17]) & (VIRTUELL & ((((MMU_VA[17] & match) | ~WRITE) | match) & zugriff))) & match;
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = ((USER | WRITE) & (RMW | WRITE)) | (USER | RMW);
