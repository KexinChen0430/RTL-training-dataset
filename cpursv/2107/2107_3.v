...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = WRITE | ((USER & WRITE) | READ);
  ...
  assign match = (((MVALID[15:0] & maske) != 16'h0000) & (adr_space == as_sorte)) & (VADR_R[31:20] == MMU_VA[31:20]);
  assign alles_ok = (~PROT_ERROR & ((match | ~WRITE) & ((~PROT_ERROR & ~WRITE) | MMU_VA[17]))) & (((~PROT_ERROR & (match | ~WRITE)) & (match | ~WRITE)) & match);
  ...
  assign ena_prot = (match & VIRTUELL) & ((match | ~WRITE) & zugriff);
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = (WRITE | RMW) | USER;
