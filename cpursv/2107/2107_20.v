...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : (USER & (MCR_FLAGS[2] & (WRITE | USER)));
  ...
  assign alles_ok = ((~WRITE | ((match & (~PROT_ERROR & MMU_VA[17])) | (MMU_VA[17] & (~WRITE & match)))) & (~PROT_ERROR & (match | ~WRITE))) & match;
  ...
  assign ena_prot = (VIRTUELL & match) & (zugriff & (match | ~WRITE));
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = ((USER & WRITE) | RMW) | (WRITE | USER);

        3'b101: PROT_ERROR = USER;

        3'b110: PROT_ERROR = USER & (((USER & WRITE) | RMW) & (WRITE | USER));
