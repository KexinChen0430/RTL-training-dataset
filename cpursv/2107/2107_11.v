...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = ((USER & WRITE) | READ) | WRITE;
  ...
  assign alles_ok = (~PROT_ERROR & match) & (MMU_VA[17] | ~WRITE);
  ...
  assign ena_prot = match & ((((~WRITE | match) & VIRTUELL) & zugriff) & ((MMU_VA[17] | (~PROT_ERROR & match)) | match));
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = ((((RMW | USER) & (USER | ((USER & RMW) | WRITE))) & WRITE) | USER) | (((USER & RMW) | WRITE) | RMW);

        3'b101: PROT_ERROR = USER;

        3'b110: PROT_ERROR = ((((USER & RMW) | WRITE) | RMW) & ((RMW | USER) & (USER | ((USER & RMW) | WRITE)))) & USER;
