...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = ((WRITE & USER) | READ) | WRITE;
  ...
  assign match = (VADR_R[31:20] == MMU_VA[31:20]) & ((adr_space == as_sorte) & ((maske & MVALID[15:0]) != 16'h0000));
  assign alles_ok = (~PROT_ERROR & match) & (((match & MMU_VA[17]) | (~WRITE & ~PROT_ERROR)) | MMU_VA[17]);
  ...
  assign ena_prot = VIRTUELL & (((((~WRITE & ((match & (~WRITE | (match & MMU_VA[17]))) | MMU_VA[17])) | match) | (MMU_VA[17] | (((~PROT_ERROR & match) | ((MMU_VA[17] & (~WRITE & match)) | ((~WRITE & ~PROT_ERROR) & (~WRITE & match)))) | (match & MMU_VA[17])))) & zugriff) & match);
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = (RMW | USER) | (WRITE | (USER & (WRITE | RMW)));
