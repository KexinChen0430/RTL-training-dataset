...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : ((((WRITE | USER) & MCR_FLAGS[2]) & (USER | RMW)) & USER);
  ...
  assign alles_ok = (match & ~PROT_ERROR) & ((((match & ~WRITE) & (match & MMU_VA[17])) | (match & (MMU_VA[17] & ~PROT_ERROR))) | (match & ~WRITE));
  ...
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})
        ...
        3'b100: PROT_ERROR = RMW | (WRITE | USER);

        3'b101: PROT_ERROR = USER;

        3'b110: PROT_ERROR = ((WRITE | USER) & (((USER & WRITE) | RMW) | WRITE)) & USER;
