...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = WRITE | (((RMW & USER) & WRITE) | READ);
  ...
  assign alles_ok = (~PROT_ERROR & match) & (~WRITE | (MMU_VA[17] & match));
  ...
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})
        ...
        3'b100: PROT_ERROR = (RMW | USER) | (USER | ((RMW & USER) | WRITE));

        3'b101: PROT_ERROR = USER;

        3'b110: PROT_ERROR = ((RMW & USER) | WRITE) & USER;
