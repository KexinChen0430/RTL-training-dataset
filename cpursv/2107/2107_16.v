...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = WRITE | (((USER & RMW) & WRITE) | READ);
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : (((WRITE | USER) & MCR_FLAGS[2]) & USER);
  ...
  assign alles_ok = (match & ~PROT_ERROR) & ((~WRITE | match) & (MMU_VA[17] | (~PROT_ERROR & ~WRITE)));
  ...
  assign ena_prot = ((zugriff & (~WRITE | match)) & match) & (VIRTUELL & (~WRITE | match));
  ...
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})
        ...
        3'b110: PROT_ERROR = ((WRITE | USER) & (((((USER & RMW) | WRITE) & ((RMW | USER) & WRITE)) & USER) | RMW)) & USER;
