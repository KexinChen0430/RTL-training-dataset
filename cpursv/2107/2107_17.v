...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign adr_space = IVAR[1] ? IVAR[0] : ((MCR_FLAGS[2] & (RMW | USER)) & USER);
  assign as_sorte = (maske & MVALID[31:16]) != 16'h0;
  assign match = (adr_space == as_sorte) & ((VADR_R[31:20] == MMU_VA[31:20]) & ((MVALID[15:0] & maske) != 16'h0000));
  assign alles_ok = (~PROT_ERROR & match) & (MMU_VA[17] | (match & ~WRITE));
  ...
  assign as_bits = IVAR[1] ? MVALID[31:16] : 
                   adr_space ? (MVALID[31:16] | maske) : (~maske & MVALID[31:16]);
  assign UPDATE = {as_bits,val_bits};
  assign ena_prot = (VIRTUELL & zugriff) & match;
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = (RMW | USER) | (USER | WRITE);

        3'b101: PROT_ERROR = USER;

        3'b110: PROT_ERROR = ((RMW & (USER | WRITE)) & USER) | (WRITE & USER);
