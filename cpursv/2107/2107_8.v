...
module MMU_MATCH(USER,READ,WRITE,RMW,MCR_FLAGS,MVALID,VADR_R,MMU_VA,IVAR,
  ...
  assign zugriff = (((USER & RMW) & ((RMW | USER) & WRITE)) | READ) | WRITE;
  ...
  assign match = ((VADR_R[31:20] == MMU_VA[31:20]) & ((maske & MVALID[15:0]) != 16'h0000)) & (adr_space == as_sorte);
  assign alles_ok = (MMU_VA[17] | (~WRITE & ~PROT_ERROR)) & (match & ~PROT_ERROR);
  ...
  assign ena_prot = ((((MMU_VA[17] | (match & (~WRITE & ~PROT_ERROR))) | ((match & ~PROT_ERROR) | (match & MMU_VA[17]))) | match) & (((match | (((match & ~PROT_ERROR) | (~WRITE & ~PROT_ERROR)) | ~WRITE)) & zugriff) & VIRTUELL)) & match;
  
  always @(ena_prot or MMU_VA or USER or WRITE or RMW)
      case ({ena_prot,MMU_VA[19:18]})

        3'b100: PROT_ERROR = (RMW | USER) | WRITE;

        3'b101: PROT_ERROR = USER;

        3'b110: PROT_ERROR = (USER & RMW) | (USER & WRITE);
