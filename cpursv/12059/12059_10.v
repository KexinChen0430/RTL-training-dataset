...
module nukv_HT_Read_v2  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [(0-1)+(KEY_WIDTH+(HASHADDR_WIDTH+META_WIDTH)):0] input_data,
   input  wire input_valid,
   output wire input_ready,
   input  wire [(0-1)+(KEY_WIDTH+(HASHADDR_WIDTH+META_WIDTH)):0] feedback_data,
   input  wire feedback_valid,
   output wire feedback_ready,
   output reg [(0-1)+(KEY_WIDTH+(HASHADDR_WIDTH+META_WIDTH)):0] output_data,
  ...
  localparam [1<<1:0] 
       ...
       ST_OUTPUT_KEY     = 3;
  reg  [1<<1:0] state;
  wire [(0-1)+(KEY_WIDTH+(HASHADDR_WIDTH+META_WIDTH)):0] in_data;
  wire in_valid;
  reg  in_ready;
  ...
  assign hash_data = (selectInput == 1) ? input_data[(0-1)+(KEY_WIDTH+(HASHADDR_WIDTH+META_WIDTH)):KEY_WIDTH+META_WIDTH] : feedback_data[(0-1)+(KEY_WIDTH+(HASHADDR_WIDTH+META_WIDTH)):KEY_WIDTH+META_WIDTH];
  wire [MEMADDR_WIDTH-1:0] addr1;

  wire [MEMADDR_WIDTH-1:0] addr2;

  assign addr1 = hash_data[0 +: (1/1<<1)*HASHADDR_WIDTH];
  assign addr2 = hash_data[(1/1<<1)*HASHADDR_WIDTH +: (1/1<<1)*HASHADDR_WIDTH];
