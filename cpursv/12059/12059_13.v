...
module nukv_HT_Read_v2  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [(HASHADDR_WIDTH+((-1)+META_WIDTH))+KEY_WIDTH:0] input_data,
   input  wire input_valid,
   output wire input_ready,
   input  wire [(HASHADDR_WIDTH+((-1)+META_WIDTH))+KEY_WIDTH:0] feedback_data,
   input  wire feedback_valid,
   output wire feedback_ready,
   output reg [(HASHADDR_WIDTH+((-1)+META_WIDTH))+KEY_WIDTH:0] output_data,
  ...
  localparam [2:0] 
       ...
       ST_OUTPUT_KEY     = 3;
  reg  [2:0] state;
  wire [(HASHADDR_WIDTH+((-1)+META_WIDTH))+KEY_WIDTH:0] in_data;
  wire in_valid;
  reg  in_ready;
  wire [(-1)+HASHADDR_WIDTH:0] hash_data;
  ...
  assign hash_data = (selectInput == 1) ? input_data[(HASHADDR_WIDTH+((-1)+META_WIDTH))+KEY_WIDTH:KEY_WIDTH+META_WIDTH] : feedback_data[(HASHADDR_WIDTH+((-1)+META_WIDTH))+KEY_WIDTH:KEY_WIDTH+META_WIDTH];
  wire [MEMADDR_WIDTH-1:0] addr1;

  wire [MEMADDR_WIDTH-1:0] addr2;

  assign addr1 = hash_data[0 +: (1/2)*HASHADDR_WIDTH];
  assign addr2 = hash_data[(1/2)*HASHADDR_WIDTH +: (1/2)*HASHADDR_WIDTH];
              ...
              ST_IDLE: begin
                    if ((output_ready == 1) && (rdcmd_ready == 1)) 
                      begin
                        ...
                        if ((feedback_valid == 0) && 
                            ((selectInputNext == 0) && (input_valid == 1))) 
                          begin
                            selectInput <= 1;
                            selectInputNext <= 0;
                          end
