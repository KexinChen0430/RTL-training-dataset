...
module nukv_HT_Read_v2  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [((HASHADDR_WIDTH+META_WIDTH)+KEY_WIDTH)+(-1):0] input_data,
   input  wire input_valid,
   output wire input_ready,
   input  wire [((HASHADDR_WIDTH+META_WIDTH)+KEY_WIDTH)+(-1):0] feedback_data,
   input  wire feedback_valid,
   output wire feedback_ready,
   output reg [((HASHADDR_WIDTH+META_WIDTH)+KEY_WIDTH)+(-1):0] output_data,
  ...
  localparam [1<<1:0] 
       ...
       ST_OUTPUT_KEY     = 3;
  reg  [1<<1:0] state;
  wire [((HASHADDR_WIDTH+META_WIDTH)+KEY_WIDTH)+(-1):0] in_data;
  wire in_valid;
  reg  in_ready;
  ...
  assign hash_data = (selectInput == 1) ? input_data[((HASHADDR_WIDTH+META_WIDTH)+KEY_WIDTH)+(-1):META_WIDTH+KEY_WIDTH] : feedback_data[((HASHADDR_WIDTH+META_WIDTH)+KEY_WIDTH)+(-1):META_WIDTH+KEY_WIDTH];
              ...
              ST_IDLE: begin
                    if ((output_ready == 1) && (rdcmd_ready == 1)) 
                      begin
                        ...
                        if ((feedback_valid == 1) && (selectInputNext == 1) && 
                            (input_valid == 0)) 
                          begin
                            selectInput <= 0;
                            selectInputNext <= 1;
