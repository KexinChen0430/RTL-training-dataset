...
module nukv_HT_Read_v2  #(parameter 
  ...
  (input  wire clk,
   input  wire rst,
   input  wire [META_WIDTH+(KEY_WIDTH+(HASHADDR_WIDTH+(0-1))):0] input_data,
   input  wire input_valid,
   output wire input_ready,
   input  wire [META_WIDTH+(KEY_WIDTH+(HASHADDR_WIDTH+(0-1))):0] feedback_data,
   input  wire feedback_valid,
   output wire feedback_ready,
   output reg [META_WIDTH+(KEY_WIDTH+(HASHADDR_WIDTH+(0-1))):0] output_data,
  ...
  localparam [2:0] 
       ...
       ST_OUTPUT_KEY     = 3;
  reg  [2:0] state;
  wire [META_WIDTH+(KEY_WIDTH+(HASHADDR_WIDTH+(0-1))):0] in_data;
  wire in_valid;
  reg  in_ready;
  ...
  assign hash_data = (selectInput == 1) ? input_data[META_WIDTH+(KEY_WIDTH+(HASHADDR_WIDTH+(0-1))):KEY_WIDTH+META_WIDTH] : feedback_data[META_WIDTH+(KEY_WIDTH+(HASHADDR_WIDTH+(0-1))):KEY_WIDTH+META_WIDTH];
