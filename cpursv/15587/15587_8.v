...
module icache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  ...
  localparam  TAGWIDTH = (0-CWIDTH)+((0-(1+1))+AWIDTH);
  ...
  wire 
       cache_hit = valid_out & (tag_out == iwb_adr_i[(-1)+AWIDTH:(1+CWIDTH)+1]);
  ...
  assign iwb_ack_o = iwb_stb_i & ((~ack_d1 | cache_hit) & (stb_d1 | cache_hit));
  assign iram_adr_o = iwb_adr_i;
  assign iram_en_o = 1'b1;
  assign rd_line = iwb_adr_i[1+CWIDTH:1+1];
  assign wr_line = rd_line;
  assign wr_tags = iwb_adr_i[(-1)+AWIDTH:(1+CWIDTH)+1];
  assign store_in_cache = (~ack_d1 & (stb_d1 & iwb_stb_i)) & (miss_d1 & (cache_hit | (iwb_stb_i | (cache_hit & stb_d1))));
endmodule
