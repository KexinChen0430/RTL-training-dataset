...
module icache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  ...
  localparam  TAGWIDTH = AWIDTH+((0-1<<<1)+(0-CWIDTH));
  reg  stb_d1,ack_d1,miss_d1;
  reg  [AWIDTH-1:0] held_addr;
  reg  [31:0] idata[0:1<<CWIDTH+(0-1)];
  reg  [(0-1)+TAGWIDTH:0] itags[0:1<<CWIDTH+(0-1)];
  reg  ivalid[0:1<<CWIDTH+(0-1)];
  wire [CWIDTH+(0-1):0] rd_line,wr_line;
  wire [(0-1)+TAGWIDTH:0] wr_tags;
  wire store_in_cache;
  integer i;
  ...
  wire [(0-1)+TAGWIDTH:0]  tag_out = itags[rd_line];
  ...
  wire 
       cache_hit = valid_out & (tag_out == iwb_adr_i[AWIDTH-1:(1+CWIDTH)+1]);
  ...
  assign iwb_ack_o = ((((stb_d1 | cache_hit) & (iwb_stb_i | stb_d1)) & (iwb_stb_i | stb_d1)) & iwb_stb_i) & (((stb_d1 & ~ack_d1) | iwb_stb_i) & (((stb_d1 & cache_hit) | ~ack_d1) | cache_hit));
