...
module icache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  ...
  reg  [31:0] idata[0:(-1)+1<<CWIDTH];
  reg  [TAGWIDTH-1:0] itags[0:(-1)+1<<CWIDTH];
  reg  ivalid[0:(-1)+1<<CWIDTH];
  ...
  assign iwb_ack_o = (iwb_stb_i & (cache_hit | ((cache_hit & stb_d1) | (iwb_stb_i & ~ack_d1)))) & ((iwb_stb_i | stb_d1) & (cache_hit | (stb_d1 | ((cache_hit & ~ack_d1) | (cache_hit & stb_d1)))));
  ...
  assign store_in_cache = ((miss_d1 & (cache_hit | (iwb_stb_i | (cache_hit & ~ack_d1)))) & stb_d1) & (iwb_stb_i & ~ack_d1);
endmodule
