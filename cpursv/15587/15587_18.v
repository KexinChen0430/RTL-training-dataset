...
module icache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  ...
  reg  [TAGWIDTH-1:0] itags[0:(0-1)+1<<CWIDTH];
  ...
  wire 
       cache_hit = (tag_out == iwb_adr_i[(0-1)+AWIDTH:1+(1+CWIDTH)]) & valid_out;
  ...
  assign iwb_ack_o = ((iwb_stb_i | (~ack_d1 & ((stb_d1 | ((~ack_d1 & stb_d1) | cache_hit)) & (stb_d1 | iwb_stb_i)))) & (~ack_d1 | cache_hit)) & (iwb_stb_i & (stb_d1 | ((~ack_d1 & stb_d1) | cache_hit)));
  assign iram_adr_o = iwb_adr_i;
  assign iram_en_o = 1'b1;
  assign rd_line = iwb_adr_i[1+CWIDTH:2];
  assign wr_line = rd_line;
  assign wr_tags = iwb_adr_i[(0-1)+AWIDTH:1+(1+CWIDTH)];
  assign store_in_cache = (iwb_stb_i & ~ack_d1) & ((iwb_stb_i | cache_hit) & (stb_d1 & miss_d1));
endmodule
