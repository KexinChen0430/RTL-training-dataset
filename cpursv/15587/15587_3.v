...
module icache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  ...
  wire 
       cache_hit = (tag_out == iwb_adr_i[(-1)+AWIDTH:CWIDTH+1<<1]) & valid_out;
  ...
  assign iwb_ack_o = (iwb_stb_i & (cache_hit | ((iwb_stb_i & stb_d1) | (stb_d1 & cache_hit)))) & (cache_hit | ~ack_d1);
  assign iram_adr_o = iwb_adr_i;
  assign iram_en_o = 1'b1;
  assign rd_line = iwb_adr_i[CWIDTH+1:1<<1];
  assign wr_line = rd_line;
  assign wr_tags = iwb_adr_i[(-1)+AWIDTH:CWIDTH+1<<1];
  assign store_in_cache = iwb_stb_i & ((miss_d1 & (stb_d1 | (cache_hit | ((iwb_stb_i & stb_d1) | (stb_d1 & cache_hit))))) & (~ack_d1 & stb_d1));
endmodule
