...
module ecc_dec_fix  #(parameter 
  ...
  input  [((nCK_PER_CLK*DQ_WIDTH)*1<<1)-1:0] phy_rddata;
  wire [(-1)+((nCK_PER_CLK*ECC_WIDTH)*1<<1):0] syndrome_ns;
  ...
  reg  [(-1)+((nCK_PER_CLK*ECC_WIDTH)*1<<1):0] syndrome_r;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < nCK_PER_CLK<<1; s = s+1)
          if (correct_en) 
            rd_data[PAYLOAD_WIDTH*s +: DATA_WIDTH] = (~ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH] & ((ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH] & (flip_bits[DATA_WIDTH*s +: DATA_WIDTH] | ~flip_bits[DATA_WIDTH*s +: DATA_WIDTH])) | flip_bits[DATA_WIDTH*s +: DATA_WIDTH])) | (((ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH] & (flip_bits[DATA_WIDTH*s +: DATA_WIDTH] | ~flip_bits[DATA_WIDTH*s +: DATA_WIDTH])) | flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & ((~flip_bits[DATA_WIDTH*s +: DATA_WIDTH] | ((flip_bits[DATA_WIDTH*s +: DATA_WIDTH] | ~flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & ~ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH])) & (ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH] & (flip_bits[DATA_WIDTH*s +: DATA_WIDTH] | ~flip_bits[DATA_WIDTH*s +: DATA_WIDTH]))));
          else 
