...
module ecc_dec_fix  #(parameter 
      ...
      for (k = 0; k < nCK_PER_CLK<<<1; k = k+1)
          begin : ecc_word
            for (m = 0; m < ECC_WIDTH; m = 1+m)
                begin : ecc_bit
                  assign syndrome_ns[(k*ECC_WIDTH)+m] = ^(phy_rddata[k*DQ_WIDTH +: CODE_WIDTH] & h_rows[m*CODE_WIDTH +: CODE_WIDTH]);
                end
          end
  endgenerate
  ...
  wire [(nCK_PER_CLK*PAYLOAD_WIDTH)<<1+(-1):0] ecc_rddata_ns;
  ...
  reg  [(nCK_PER_CLK*PAYLOAD_WIDTH)<<1+(-1):0] ecc_rddata_r;
  ...
  output reg [(nCK_PER_CLK*PAYLOAD_WIDTH)<<1+(-1):0] rd_data;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < nCK_PER_CLK<<<1; s = s+1)
          if (correct_en) 
            rd_data[PAYLOAD_WIDTH*s +: DATA_WIDTH] = ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH] ^ ((ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH] | ~ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH]) & ((flip_bits[DATA_WIDTH*s +: DATA_WIDTH] & ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH]) | (flip_bits[DATA_WIDTH*s +: DATA_WIDTH] & ~ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH])));
          else 
