...
module ecc_dec_fix  #(parameter 
      ...
      for (k = 0; k < nCK_PER_CLK<<1; k = k+1)
          begin : ecc_word
            for (m = 0; m < ECC_WIDTH; m = 1+m)
                begin : ecc_bit
                  assign syndrome_ns[m+(ECC_WIDTH*k)] = ^(phy_rddata[k*DQ_WIDTH +: CODE_WIDTH] & h_rows[m*CODE_WIDTH +: CODE_WIDTH]);
                end
          end
  endgenerate
  ...
  wire [(-1)+(nCK_PER_CLK<<1*PAYLOAD_WIDTH):0] ecc_rddata_ns;
  ...
  reg  [(-1)+(nCK_PER_CLK<<1*PAYLOAD_WIDTH):0] ecc_rddata_r;
  ...
  wire [(-1)+(DATA_WIDTH*nCK_PER_CLK)<<<1:0] flip_bits;
  ...
  output reg [(-1)+(nCK_PER_CLK<<1*PAYLOAD_WIDTH):0] rd_data;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < nCK_PER_CLK<<1; s = 1+s)
          if (correct_en) 
            rd_data[s*PAYLOAD_WIDTH +: DATA_WIDTH] = (((((flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) & flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) ^ ((flip_bits[s*DATA_WIDTH +: DATA_WIDTH] & (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] & (~flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH]))) & (((flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & flip_bits[s*DATA_WIDTH +: DATA_WIDTH]))) | ((~flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]);
          else 
