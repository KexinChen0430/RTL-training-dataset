...
module ecc_dec_fix  #(parameter 
  ...
  wire [(0-1)+(ECC_WIDTH*nCK_PER_CLK<<<1):0] syndrome_ns;
  ...
  reg  [(0-1)+(ECC_WIDTH*nCK_PER_CLK<<<1):0] syndrome_r;
  ...
  wire [(0-1)+(DATA_WIDTH*nCK_PER_CLK)<<<1:0] flip_bits;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < nCK_PER_CLK<<<1; s = 1+s)
          if (correct_en) 
            rd_data[s*PAYLOAD_WIDTH +: DATA_WIDTH] = (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] & (~flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH])) ^ flip_bits[s*DATA_WIDTH +: DATA_WIDTH];
          else 
