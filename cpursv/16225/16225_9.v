...
module ecc_dec_fix  #(parameter 
  ...
  input  [(-1)+(((1+1)*nCK_PER_CLK)*DQ_WIDTH):0] phy_rddata;
  wire [(-1)+(((1+1)*nCK_PER_CLK)*ECC_WIDTH):0] syndrome_ns;

  genvar k;
  genvar m;
  
  generate
      for (k = 0; k < ((1+1)*nCK_PER_CLK); k = k+1)
          begin : ecc_word
            for (m = 0; m < ECC_WIDTH; m = m+1)
                begin : ecc_bit
                  assign syndrome_ns[m+(ECC_WIDTH*k)] = ^(h_rows[m*CODE_WIDTH +: CODE_WIDTH] & phy_rddata[DQ_WIDTH*k +: CODE_WIDTH]);
                end
          end
  endgenerate

  reg  [(-1)+(((1+1)*nCK_PER_CLK)*ECC_WIDTH):0] syndrome_r;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < ((1+1)*nCK_PER_CLK); s = s+1)
          if (correct_en) 
            rd_data[s*PAYLOAD_WIDTH +: DATA_WIDTH] = (((ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] & (~flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH])))) ^ (((ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & ((((ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) & (~flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH]))));
          else 
  ...
  output wire [(-1)+((1+1)*nCK_PER_CLK):0] ecc_single;

  output wire [(-1)+((1+1)*nCK_PER_CLK):0] ecc_multiple;

  genvar v;
  
  generate
      for (v = 0; v < ((1+1)*nCK_PER_CLK); v = 1+v)
