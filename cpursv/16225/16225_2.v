...
module ecc_dec_fix  #(parameter 
  ...
  input  [(1<<1*(DQ_WIDTH*nCK_PER_CLK))-1:0] phy_rddata;
  ...
  wire [(-1)+((nCK_PER_CLK*1<<1)*PAYLOAD_WIDTH):0] ecc_rddata_ns;
  ...
  reg  [(-1)+((nCK_PER_CLK*1<<1)*PAYLOAD_WIDTH):0] ecc_rddata_r;
  ...
  wire [(DATA_WIDTH*(nCK_PER_CLK*1<<1))-1:0] flip_bits;
  ...
  output reg [(-1)+((nCK_PER_CLK*1<<1)*PAYLOAD_WIDTH):0] rd_data;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < (nCK_PER_CLK*1<<1); s = s+1)
          if (correct_en) 
            rd_data[PAYLOAD_WIDTH*s +: DATA_WIDTH] = ((~flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) & ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH]) ^ ((flip_bits[s*DATA_WIDTH +: DATA_WIDTH] & (ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH] | ~ecc_rddata_r[PAYLOAD_WIDTH*s +: DATA_WIDTH])) & (~flip_bits[s*DATA_WIDTH +: DATA_WIDTH] | flip_bits[s*DATA_WIDTH +: DATA_WIDTH]));
          else 
  ...
  output wire [(nCK_PER_CLK*1<<1)+(-1):0] ecc_single;

  output wire [(nCK_PER_CLK*1<<1)+(-1):0] ecc_multiple;
