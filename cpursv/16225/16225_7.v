...
module ecc_dec_fix  #(parameter 
      ...
      for (k = 0; k < nCK_PER_CLK<<<1; k = 1+k)
          begin : ecc_word
            for (m = 0; m < ECC_WIDTH; m = 1+m)
                begin : ecc_bit
                  assign syndrome_ns[m+(k*ECC_WIDTH)] = ^(phy_rddata[k*DQ_WIDTH +: CODE_WIDTH] & h_rows[CODE_WIDTH*m +: CODE_WIDTH]);
                end
          end
  endgenerate
  ...
  wire [(nCK_PER_CLK*PAYLOAD_WIDTH)<<<1+(0-1):0] ecc_rddata_ns;
  ...
  reg  [(nCK_PER_CLK*PAYLOAD_WIDTH)<<<1+(0-1):0] ecc_rddata_r;
  ...
  output reg [(nCK_PER_CLK*PAYLOAD_WIDTH)<<<1+(0-1):0] rd_data;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < nCK_PER_CLK<<<1; s = s+1)
          if (correct_en) 
            rd_data[s*PAYLOAD_WIDTH +: DATA_WIDTH] = (((ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & flip_bits[s*DATA_WIDTH +: DATA_WIDTH]) & (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH])) ^ ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH];
          else 
      ...
      if (RAW_BIT_WIDTH > 0) 
        for (t = 0; t < nCK_PER_CLK<<<1; t = 1+t)
            begin : copy_raw_bits
              ...
              always @(ecc_rddata_r)
                  rd_data[(0-1)+((1+t)*PAYLOAD_WIDTH) -: RAW_BIT_WIDTH] = ecc_rddata_r[(0-1)+((1+t)*PAYLOAD_WIDTH) -: RAW_BIT_WIDTH];
            end
