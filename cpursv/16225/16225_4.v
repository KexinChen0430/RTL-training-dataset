...
module ecc_dec_fix  #(parameter 
  ...
  wire [((nCK_PER_CLK*ECC_WIDTH)*(1+1))-1:0] syndrome_ns;
      ...
      for (k = 0; k < nCK_PER_CLK<<<1; k = k+1)
          begin : ecc_word
            for (m = 0; m < ECC_WIDTH; m = m+1)
                begin : ecc_bit
                  assign syndrome_ns[m+(ECC_WIDTH*k)] = ^(h_rows[CODE_WIDTH*m +: CODE_WIDTH] & phy_rddata[DQ_WIDTH*k +: CODE_WIDTH]);
                end
          end
  endgenerate

  reg  [((nCK_PER_CLK*ECC_WIDTH)*(1+1))-1:0] syndrome_r;
  ...
  wire [(nCK_PER_CLK*DATA_WIDTH<<1)-1:0] flip_bits;
  ...
  always @(correct_en or ecc_rddata_r or flip_bits)
      for (s = 0; s < nCK_PER_CLK<<<1; s = s+1)
          if (correct_en) 
            rd_data[s*PAYLOAD_WIDTH +: DATA_WIDTH] = ((flip_bits[DATA_WIDTH*s +: DATA_WIDTH] | ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & ~flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) | (((((((flip_bits[DATA_WIDTH*s +: DATA_WIDTH] & (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH])) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & ~((ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH])) | (~(((flip_bits[DATA_WIDTH*s +: DATA_WIDTH] & (ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH])) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & ((ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]))) & ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) ^ ((((~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH] | ~flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]) & flip_bits[DATA_WIDTH*s +: DATA_WIDTH]) & ~ecc_rddata_r[s*PAYLOAD_WIDTH +: DATA_WIDTH]));
          else 
