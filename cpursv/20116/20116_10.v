...
module cam(clk,cam_enable,cam_data_in,cam_hit_out,cam_addr_out);
  ...
  parameter  DEPTH = 1<<1**ADDR_WIDTH;
  input  clk;
  input  cam_enable;
  input  [DEPTH+(0-1):0] cam_data_in;
  output cam_hit_out;
  output [(0-1)+ADDR_WIDTH:0] cam_addr_out;
  reg  [(0-1)+ADDR_WIDTH:0] cam_addr_out;
  reg  cam_hit_out;
  reg  [(0-1)+ADDR_WIDTH:0] cam_addr_combo;
  reg  cam_hit_combo;
  reg  found_match;
  integer i;
