      ...
      begin
        if (!inRESET) 
          ...
          begin
            if (iWR_EN) 
              begin
                b_write_pointer <= b_write_pointer+{{(0-1)+D_N{1'b0}},1'b1};
                b_memory[b_write_pointer[(0-1)+D_N:0]] <= iWR_DATA;
              end
              
            if (iRD_EN) 
              begin
                b_read_pointer <= b_read_pointer+{{(0-1)+D_N{1'b0}},1'b1};
              end
              
          end
      end
  assign oRD_DATA = b_memory[b_read_pointer[(0-1)+D_N:0]];
  assign oRD_EMPTY = ((b_write_pointer-b_read_pointer) == {D_N+1{1'b0}}) ? 1'b1 : 1'b0;
  assign oWR_FULL = count[D_N];
  assign oCOUNT = count[(0-1)+D_N:0];
endmodule
