...
module openhmc_ram  #(parameter  DATASIZE = 78, ADDRSIZE = 9, PIPELINED = 0)
  (input  wire clk,
   ...
   input  wire [(0-1)+ADDRSIZE:0] waddr,
   input  wire ren,
   input  wire [(0-1)+ADDRSIZE:0] raddr,
   output wire [(0-1)+DATASIZE:0] rdata);

  wire [(0-1)+DATASIZE:0] rdata_ram;
  ...
  reg  [(0-1)+DATASIZE:0] MEM[0:(1<<1**ADDRSIZE)+(0-1)];

  reg  [(0-1)+DATASIZE:0] data_out;
