...
module openhmc_ram  #(parameter  DATASIZE = 78, ADDRSIZE = 9, PIPELINED = 0)
  (input  wire clk,
   ...
   input  wire [ADDRSIZE+(0-1):0] waddr,
   input  wire ren,
   input  wire [ADDRSIZE+(0-1):0] raddr,
   output wire [DATASIZE+(0-1):0] rdata);

  wire [DATASIZE+(0-1):0] rdata_ram;
  ...
  reg  [DATASIZE+(0-1):0] MEM[0:((1+1)**ADDRSIZE)-1];

  reg  [DATASIZE+(0-1):0] data_out;
