...
module spi_master(input  data_in,
  ...
  reg  [(-1)+BYTE_SIZE:0] data_recv;
  reg  [(-1)+BYTE_SIZE:0] data_send;
  reg  [(-1)+BYTE_INDEX_SIZE:0] send_iterator;
  reg  [(-1)+BYTE_INDEX_SIZE:0] recv_iterator;
  reg  [1:0] state;
  parameter [1:0] 
       S_WAIT     = 2'd0,
       S_TRANSFER = 2'd1,
          ...
          begin
            if (state == S_WAIT) 
              ...
              begin
                ...
                if ((send_iterator > 0) && 
                    (recv_iterator < ((-1)+BYTE_SIZE))) 
                  begin
                    data_recv[recv_iterator] <= data_in;
                    recv_iterator = recv_iterator+1;
                  end
                  
                if (recv_iterator >= ((-1)+BYTE_SIZE)) 
                  begin
                    state <= S_DEASSERT;
                  end
                else send_iterator <= send_iterator+1;
              end
            else if (state == S_DEASSERT) 
              begin
                data_recv[(-1)+BYTE_SIZE] <= data_in;
                state <= S_WAIT;
                slave_select1_out <= 1;
                data_out <= 1'bz;
                send_iterator <= 0;
