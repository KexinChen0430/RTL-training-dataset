...
module spi_master(input  data_in,
  ...
  reg  [(0-1)+BYTE_SIZE:0] data_recv;
  reg  [(0-1)+BYTE_SIZE:0] data_send;
  reg  [BYTE_INDEX_SIZE+(0-1):0] send_iterator;
  reg  [BYTE_INDEX_SIZE+(0-1):0] recv_iterator;
  reg  [1:0] state;
  parameter [1:0] 
       S_WAIT     = 2'd0,
       S_TRANSFER = 2'd1,
          ...
          begin
            if (state == S_WAIT) 
              ...
              begin
                ...
                if ((recv_iterator < ((0-1)+BYTE_SIZE)) && 
                    (send_iterator > 0)) 
                  begin
              ...
              begin
                data_recv[(0-1)+BYTE_SIZE] <= data_in;
                state <= S_WAIT;
                slave_select1_out <= 1;
                data_out <= 1'bz;
                send_iterator <= 0;
