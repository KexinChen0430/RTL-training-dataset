          ...
          begin
            ...
            if (datain_width > round_width) 
              begin
                ...
                for (i = 0; i < (((-round_width)+(-1))+datain_width); i = i+1)
                    begin
                      sign = datain[i] | sign;
                      dataout_value[i] = 1'bX;
                    end
                dataout_value[((-round_width)+(-1))+datain_width] = 1'bX;
                if (datain[((-round_width)+(-1))+datain_width] == 1'b0) 
                  begin
                    dataout_tmp = result_tmp;
                  end
                else if ((sign == 1'b1) && 
                         (datain[((-round_width)+(-1))+datain_width] == 1'b1)) 
                  begin
