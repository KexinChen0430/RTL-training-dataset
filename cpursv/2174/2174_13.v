...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = H_SYNC_DATA+(H_SYNC_FRONT+(H_SYNC_BACK+H_SYNC_PULSE));
  parameter  V_SYNC_PULSE = 2;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = (V_SYNC_BACK+(V_SYNC_FRONT+V_SYNC_DATA))+V_SYNC_PULSE;
  parameter 
...
(mVCounter >= V_START_PULSE))) ? 0 : 1;
  assign oHSync = ((mHCounter < H_STOP_PULSE) && inRst && 
                  (mHCounter >= H_START_PULSE)) ? 0 : 1;
  assign oFrameValid = (inRst && (mVCounter < V_STOP_DATA) && 
                       (mVCounter >= V_START_DATA)) ? 1 : 0;
  assign oLineValid = (oFrameValid && (mHCounter < H_STOP_DATA) && 
  ...
  always @(posedge iClk or negedge inRst)
      begin
        if (~inRst) mHCounter <= 0;
          ...
          begin
            if (mHCounter == ((-1)+H_SYNC_TOTAL)) mHCounter <= 0;
