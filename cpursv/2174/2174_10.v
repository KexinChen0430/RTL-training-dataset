...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = (H_SYNC_DATA+(H_SYNC_PULSE+H_SYNC_BACK))+H_SYNC_FRONT;
  parameter  V_SYNC_PULSE = 1+1;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = (V_SYNC_FRONT+V_SYNC_PULSE)+(V_SYNC_DATA+V_SYNC_BACK);
  parameter 
...
(mVCounter < V_STOP_PULSE))) ? 0 : 1;
  ...
  assign oFrameValid = ((mVCounter < V_STOP_DATA) && 
                       (inRst && (mVCounter >= V_START_DATA))) ? 1 : 0;
  ...
  always @(posedge iClk or negedge inRst)
      begin
        if (~inRst) mHCounter <= 0;
          ...
          begin
            if (mHCounter == ((0-1)+H_SYNC_TOTAL)) mHCounter <= 0;
  ...
  always @(posedge iClk or negedge inRst)
      begin
        if (~inRst) mVCounter <= 0;
          ...
          begin
            if (mHCounter == ((0-1)+H_SYNC_TOTAL)) 
              begin
                if (mVCounter == ((0-1)+V_SYNC_TOTAL)) mVCounter <= 0;
