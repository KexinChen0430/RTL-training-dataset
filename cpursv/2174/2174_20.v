...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = ((H_SYNC_FRONT+H_SYNC_DATA)+H_SYNC_PULSE)+H_SYNC_BACK;
  parameter  V_SYNC_PULSE = 1+1;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = ((V_SYNC_PULSE+V_SYNC_FRONT)+V_SYNC_BACK)+V_SYNC_DATA;
  parameter 
       H_START_DATA = (H_SYNC_FRONT+H_SYNC_BACK)+H_SYNC_PULSE;
  parameter 
       V_START_DATA = (V_SYNC_PULSE+V_SYNC_FRONT)+V_SYNC_BACK;
  ...
  assign oVSync = ((mVCounter < V_STOP_PULSE) && inRst && 
  ...
  assign oFrameValid = ((mVCounter >= V_START_DATA) && 
                       (inRst && (mVCounter < V_STOP_DATA))) ? 1 : 0;
        ...
        if (~inRst) mVCounter <= 0;
          ...
          begin
            if (mHCounter == (H_SYNC_TOTAL+(-1))) 
              begin
                if (mVCounter == ((-1)+V_SYNC_TOTAL)) mVCounter <= 0;
