...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = (H_SYNC_FRONT+(H_SYNC_BACK+H_SYNC_PULSE))+H_SYNC_DATA;
  parameter  V_SYNC_PULSE = 1<<1;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = (V_SYNC_BACK+(V_SYNC_FRONT+V_SYNC_PULSE))+V_SYNC_DATA;
  parameter 
       H_START_DATA = H_SYNC_FRONT+(H_SYNC_BACK+H_SYNC_PULSE);
  parameter 
       V_START_DATA = V_SYNC_BACK+(V_SYNC_FRONT+V_SYNC_PULSE);
  ...
  assign oVSync = (inRst && (mVCounter >= V_START_PULSE) && 
                  (mVCounter < V_STOP_PULSE)) ? 0 : 1;
  assign oHSync = ((mHCounter < H_STOP_PULSE) && 
                  (inRst && (mHCounter >= H_START_PULSE))) ? 0 : 1;
