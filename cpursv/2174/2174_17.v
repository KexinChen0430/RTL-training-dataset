...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = H_SYNC_PULSE+(H_SYNC_DATA+(H_SYNC_FRONT+H_SYNC_BACK));
  parameter  V_SYNC_PULSE = 1+1;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = V_SYNC_DATA+((V_SYNC_PULSE+V_SYNC_BACK)+V_SYNC_FRONT);
  parameter 
       H_START_DATA = H_SYNC_PULSE+(H_SYNC_FRONT+H_SYNC_BACK);
  parameter 
       V_START_DATA = (V_SYNC_PULSE+V_SYNC_BACK)+V_SYNC_FRONT;
  ...
  assign oVSync = (inRst && 
                  ((mVCounter < V_STOP_PULSE) && 
(mVCounter >= V_START_PULSE))) ? 0 : 1;
  assign oHSync = ((mHCounter >= H_START_PULSE) && 
                  ((mHCounter < H_STOP_PULSE) && inRst)) ? 0 : 1;
  assign oFrameValid = ((mVCounter < V_STOP_DATA) && inRst && 
                       (mVCounter >= V_START_DATA)) ? 1 : 0;
  assign oLineValid = ((mHCounter < H_STOP_DATA) && 
                      (oFrameValid && (mHCounter >= H_START_DATA))) ? 1 : 0;
  assign oR = (oLineValid && (inRst && oFrameValid)) ? iR : 0;
  assign oG = (oLineValid && (inRst && oFrameValid)) ? iG : 0;
  assign oB = (oLineValid && (inRst && oFrameValid)) ? iB : 0;
        ...
        if (~inRst) mVCounter <= 0;
          ...
          begin
            if (mHCounter == (H_SYNC_TOTAL+(0-1))) 
              begin
                if (mVCounter == (V_SYNC_TOTAL+(0-1))) mVCounter <= 0;
