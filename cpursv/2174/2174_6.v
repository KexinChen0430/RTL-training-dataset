...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = H_SYNC_DATA+((H_SYNC_PULSE+H_SYNC_FRONT)+H_SYNC_BACK);
  parameter  V_SYNC_PULSE = 1<<1;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = (V_SYNC_DATA+V_SYNC_FRONT)+(V_SYNC_BACK+V_SYNC_PULSE);
  parameter 
       H_START_DATA = (H_SYNC_PULSE+H_SYNC_FRONT)+H_SYNC_BACK;
  parameter 
       V_START_DATA = V_SYNC_FRONT+(V_SYNC_BACK+V_SYNC_PULSE);
  ...
  parameter  H_STOP_PULSE = H_SYNC_PULSE+H_SYNC_FRONT;
  ...
  assign oVSync = ((mVCounter < V_STOP_PULSE) && 
                  (inRst && (mVCounter >= V_START_PULSE))) ? 0 : 1;
  assign oHSync = (inRst && (mHCounter >= H_START_PULSE) && 
                  (mHCounter < H_STOP_PULSE)) ? 0 : 1;
  assign oFrameValid = ((mVCounter >= V_START_DATA) && 
                       ((mVCounter < V_STOP_DATA) && inRst)) ? 1 : 0;
  assign oLineValid = ((mHCounter >= H_START_DATA) && 
                      (oFrameValid && (mHCounter < H_STOP_DATA))) ? 1 : 0;
  assign oR = (oFrameValid && oLineValid && inRst) ? iR : 0;
  assign oG = (oFrameValid && oLineValid && inRst) ? iG : 0;
  assign oB = (oFrameValid && oLineValid && inRst) ? iB : 0;
