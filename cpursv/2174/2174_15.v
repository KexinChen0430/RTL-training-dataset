...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = (H_SYNC_BACK+H_SYNC_DATA)+(H_SYNC_FRONT+H_SYNC_PULSE);
  parameter  V_SYNC_PULSE = 1+1;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = ((V_SYNC_PULSE+V_SYNC_BACK)+V_SYNC_FRONT)+V_SYNC_DATA;
  parameter 
       H_START_DATA = (H_SYNC_BACK+H_SYNC_PULSE)+H_SYNC_FRONT;
  parameter 
  ...
  assign oVSync = ((mVCounter >= V_START_PULSE) && 
                  (mVCounter < V_STOP_PULSE) && inRst) ? 0 : 1;
