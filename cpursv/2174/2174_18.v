...
module VGAController(input  iClk,
  ...
  parameter 
       H_SYNC_TOTAL = (H_SYNC_PULSE+H_SYNC_DATA)+(H_SYNC_BACK+H_SYNC_FRONT);
  parameter  V_SYNC_PULSE = 1+1;
  parameter  V_SYNC_BACK = 36;
  parameter  V_SYNC_DATA = 480;
  parameter  V_SYNC_FRONT = 7;
  parameter 
       V_SYNC_TOTAL = V_SYNC_DATA+((V_SYNC_PULSE+V_SYNC_FRONT)+V_SYNC_BACK);
  parameter 
  ...
  assign oVSync = ((mVCounter >= V_START_PULSE) && inRst && 
                  (mVCounter < V_STOP_PULSE)) ? 0 : 1;
  assign oHSync = ((mHCounter < H_STOP_PULSE) && 
                  (mHCounter >= H_START_PULSE) && inRst) ? 0 : 1;
  assign oFrameValid = ((mVCounter >= V_START_DATA) && inRst && 
                       (mVCounter < V_STOP_DATA)) ? 1 : 0;
  assign oLineValid = ((mHCounter >= H_START_DATA) && 
                      (mHCounter < H_STOP_DATA) && oFrameValid) ? 1 : 0;
  assign oR = (inRst && (oFrameValid && oLineValid)) ? iR : 0;
  assign oG = (inRst && (oFrameValid && oLineValid)) ? iG : 0;
  assign oB = (inRst && (oFrameValid && oLineValid)) ? iB : 0;
