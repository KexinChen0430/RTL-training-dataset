...
module sound_mulacc(clock,vol_in,dat_in,mode_inv7b,load,clr_sum,ready,sum_out);
  ...
  always @(posedge clock)
      begin
        if (!load) 
          ...
          begin
            sum_reg[6:0] <= 7'd0;
            shifter[7] <= ~((dat_in[7] & ~(((dat_in[7] | ~dat_in[7]) & mode_inv7b) & (~mode_inv7b | (mode_inv7b | dat_in[7])))) | ((((dat_in[7] | ~dat_in[7]) & mode_inv7b) & (~mode_inv7b | (mode_inv7b | dat_in[7]))) & ~dat_in[7]));
  ...
  assign temp_sum[1:0] = (old_data_in+carry_in)+mul_out;
