          ...
          begin
            ...
            if (datain_width > round_width) 
              begin
                ...
                for (i = 0; i < (datain_width+((-1)+(-round_width))); i = i+1)
                    begin
                      sign = sign | datain[i];
                      dataout_value[i] = 1'bX;
                    end
                dataout_value[datain_width+((-1)+(-round_width))] = 1'bX;
                if (datain[datain_width+((-1)+(-round_width))] == 1'b0) 
                  begin
                    dataout_tmp = result_tmp;
                  end
                else if ((sign == 1'b1) && 
                         (datain[datain_width+((-1)+(-round_width))] == 1'b1)) 
                  begin
