...
module e16_arbiter_priority(grant,arb_wait,clk,clk_en,reset,hold,request);
  ...
  always @(posedge clk or posedge reset)
      ...
      else if (clk_en) grant_hold[(0-1)+ARW:0] <= {ARW{hold}} & grant[(0-1)+ARW:0];
        
  genvar i;
  
  generate
      for (i = 0; i < ((0-1)+ARW); i = 1+i)
          begin : gen_block
            assign request_mask[i] = request[i] & ~|grant_hold[(0-1)+ARW:1+i];
          end
  ...
  generate
      for (j = (0-1)+ARW; j >= 1; j = j-1)
          begin : gen_arbiter
            assign grant_mask[j] = |request_mask[j-1:0];
          end
  endgenerate

  assign grant[(0-1)+ARW:0] = ~grant_mask[(0-1)+ARW:0] & request_mask[(0-1)+ARW:0];
  assign arb_wait[(0-1)+ARW:0] = request[(0-1)+ARW:0] & ({ARW{hold}} | (({ARW{hold}} | request[(0-1)+ARW:0]) & ~grant[(0-1)+ARW:0]));
  
  always @* 
      if ((~reset & (|(~request[(0-1)+ARW:0] & grant_hold[(0-1)+ARW:0]) & $time)) > 0) 
        begin
