...
module e16_arbiter_priority(grant,arb_wait,clk,clk_en,reset,hold,request);
  ...
  generate
      for (i = 0; i < (ARW+(0-1)); i = i+1)
          begin : gen_block
            assign request_mask[i] = request[i] & ~|grant_hold[ARW+(0-1):i+1];
          end
  ...
  generate
      for (j = ARW+(0-1); j >= 1; j = j+(0-1))
          begin : gen_arbiter
            assign grant_mask[j] = |request_mask[j+(0-1):0];
          end
  endgenerate

  assign grant[ARW+(0-1):0] = request_mask[ARW+(0-1):0] & ~grant_mask[ARW+(0-1):0];
  assign arb_wait[ARW+(0-1):0] = request[ARW+(0-1):0] & (~grant[ARW+(0-1):0] | {ARW{hold}});
  
  always @* 
      if ((|(~request[ARW+(0-1):0] & grant_hold[ARW+(0-1):0]) & ($time & ~reset)) > 0) 
        begin
