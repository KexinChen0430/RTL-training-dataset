...
module e16_arbiter_priority(grant,arb_wait,clk,clk_en,reset,hold,request);
  ...
  generate
      for (i = 0; i < ((0-1)+ARW); i = i+1)
          begin : gen_block
            assign request_mask[i] = request[i] & ~|grant_hold[(0-1)+ARW:i+1];
          end
  ...
  assign arb_wait[(0-1)+ARW:0] = (request[(0-1)+ARW:0] & ~grant[(0-1)+ARW:0]) | (request[(0-1)+ARW:0] & {ARW{hold}});
  
  always @* 
      if (($time & (|(grant_hold[(0-1)+ARW:0] & ~request[(0-1)+ARW:0]) & ~reset)) > 0) 
        begin
