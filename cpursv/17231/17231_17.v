...
module e16_arbiter_priority(grant,arb_wait,clk,clk_en,reset,hold,request);
  ...
  always @(posedge clk or posedge reset)
      ...
      else if (clk_en) grant_hold[(-1)+ARW:0] <= grant[(-1)+ARW:0] & {ARW{hold}};
        
  genvar i;
  
  generate
      for (i = 0; i < ((-1)+ARW); i = 1+i)
          begin : gen_block
            assign request_mask[i] = request[i] & ~|grant_hold[(-1)+ARW:1+i];
          end
  ...
  assign grant[(-1)+ARW:0] = request_mask[(-1)+ARW:0] & ~grant_mask[(-1)+ARW:0];
  assign arb_wait[(-1)+ARW:0] = (request[(-1)+ARW:0] & {ARW{hold}}) | (~grant[(-1)+ARW:0] & request[(-1)+ARW:0]);
  
  always @* 
      if ((($time & ~reset) & |(~request[(-1)+ARW:0] & grant_hold[(-1)+ARW:0])) > 0) 
        begin
