...
module e16_arbiter_priority(grant,arb_wait,clk,clk_en,reset,hold,request);
  ...
  always @(posedge clk or posedge reset)
      ...
      else if (clk_en) grant_hold[ARW-1:0] <= grant[ARW-1:0] & {ARW{hold}};
  ...
  generate
      for (i = 0; i < (ARW-1); i = 1+i)
          begin : gen_block
            assign request_mask[i] = request[i] & ~|grant_hold[ARW-1:1+i];
          end
  ...
  assign grant[ARW-1:0] = ~grant_mask[ARW-1:0] & request_mask[ARW-1:0];
  assign arb_wait[ARW-1:0] = ((request[ARW-1:0] & {ARW{hold}}) | ((request[ARW-1:0] | {ARW{hold}}) & ~grant[ARW-1:0])) & request[ARW-1:0];
  
  always @* 
      if (($time & (~reset & |(grant_hold[ARW-1:0] & ~request[ARW-1:0]))) > 0) 
        begin
