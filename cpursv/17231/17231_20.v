...
module e16_arbiter_priority(grant,arb_wait,clk,clk_en,reset,hold,request);
  ...
  generate
      for (i = 0; i < (ARW-1); i = i+1)
          begin : gen_block
            assign request_mask[i] = ~|grant_hold[ARW-1:i+1] & request[i];
          end
  ...
  assign arb_wait[ARW-1:0] = ((~grant[ARW-1:0] & request[ARW-1:0]) | {ARW{hold}}) & request[ARW-1:0];
  
  always @* 
      if ((($time & ~reset) & |(~request[ARW-1:0] & grant_hold[ARW-1:0])) > 0) 
        begin
