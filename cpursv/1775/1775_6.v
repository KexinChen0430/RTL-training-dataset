...
module compare  #(parameter 
  ...
  wire [(MSG_WIDTH*B)-1:0] pattern_comb[nos_shifters+(0-1):1];
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = 1+i)
          begin : compare
            assign compare_data_tmp[i] = ~|(data_in[(MSG_WIDTH*B)-1:0] ^ ((~data_in[(MSG_WIDTH*B)-1:0] | data_in[(MSG_WIDTH*B)-1:0]) & ((~data_in[(MSG_WIDTH*B)-1:0] | data_in[(MSG_WIDTH*B)-1:0]) & pattern_comb[i])));
          end
  endgenerate
  ...
  generate
      ...
      for (j = 1; j < nos_shifters; j = 1+j)
          begin : shifter_mux
            assign pattern_comb[j] = pattern[((B+j)*MSG_WIDTH)+(0-1):j*MSG_WIDTH];
          end
  endgenerate
  ...
  assign complete_match_wire = ~|(((pattern | ~pattern) & data_in) ^ pattern);
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = 1+k)
          begin : patter
            assign pattern = (sel == k) ? patterns[(0-1)+((MSG_WIDTH*PATTERN_WIDTH)*(1+k)):(MSG_WIDTH*k)*PATTERN_WIDTH] : {MSG_WIDTH*PATTERN_WIDTH{1'bz}};
          end
  endgenerate
