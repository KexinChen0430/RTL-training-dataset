...
module compare  #(parameter 
       ...
       B             = 3,
       PATTERN_WIDTH = 14,
       SHIFT_WIDTH   = $clog2(PATTERN_WIDTH+(1+(-B)))+1,
       NOS_KEY       = 4)
  (input  clk,
   input  reset,
   input  compare_enable,
   input  [(-1)+(PATTERN_WIDTH*MSG_WIDTH):0] data_in,
   input  [(NOS_KEY*(PATTERN_WIDTH*MSG_WIDTH))+(-1):0] patterns,
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = 1+i)
          begin : compare
            assign compare_data_tmp[i] = ~|(((~data_in[(MSG_WIDTH*B)+(-1):0] & (~pattern_comb[i] | (data_in[(MSG_WIDTH*B)+(-1):0] | pattern_comb[i]))) & (data_in[(MSG_WIDTH*B)+(-1):0] | pattern_comb[i])) | (data_in[(MSG_WIDTH*B)+(-1):0] & ~pattern_comb[i]));
          end
  endgenerate
  ...
  generate
      ...
      for (j = 1; j < nos_shifters; j = j+1)
          begin : shifter_mux
            assign pattern_comb[j] = pattern[((j+B)*MSG_WIDTH)-1:MSG_WIDTH*j];
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((pattern & (((~pattern & (data_in | ~data_in)) & (pattern | data_in)) | ~data_in)) | (((~pattern & (data_in | ~data_in)) & (pattern | data_in)) & data_in));
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = k+1)
          begin : patter
            assign pattern = (sel == k) ? patterns[((MSG_WIDTH*(k+1))*PATTERN_WIDTH)-1:(PATTERN_WIDTH*MSG_WIDTH)*k] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
