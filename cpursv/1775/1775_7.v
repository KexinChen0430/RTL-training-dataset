...
module compare  #(parameter 
       ...
       B             = 3,
       PATTERN_WIDTH = 14,
       SHIFT_WIDTH   = $clog2(1+((-B)+PATTERN_WIDTH))+1,
       NOS_KEY       = 4)
  (input  clk,
   input  reset,
  ...
  localparam  nos_shifters = 1+((-B)+PATTERN_WIDTH);
  wire [nos_shifters-1:1] compare_data,compare_data_tmp,priority_cmp;
  wire [(-1)+(MSG_WIDTH*B):0] pattern_comb[nos_shifters-1:1];
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = i+1)
          begin : compare
            assign compare_data_tmp[i] = ~|((pattern_comb[i] & ~data_in[(-1)+(MSG_WIDTH*B):0]) | (data_in[(-1)+(MSG_WIDTH*B):0] & (~pattern_comb[i] & (data_in[(-1)+(MSG_WIDTH*B):0] | pattern_comb[i]))));
          end
  endgenerate
  ...
  generate
      ...
      for (j = 1; j < nos_shifters; j = j+1)
          begin : shifter_mux
            assign pattern_comb[j] = pattern[(-1)+((B+j)*MSG_WIDTH):MSG_WIDTH*j];
          end
  endgenerate
  ...
  assign complete_match_wire = ~|(((~data_in | ~pattern) & (((~pattern | (pattern | data_in)) | ((pattern | data_in) & ~pattern)) & pattern)) ^ ((~data_in | ~pattern) & (((~pattern | (pattern | data_in)) | ((pattern | data_in) & ~pattern)) & ((~pattern & data_in) | (data_in & pattern)))));
        ...
        if (reset) 
          ...
          begin
            if (partial_match_wire == 1) 
              ...
              begin
                shift_amount <= 1+((-B)+PATTERN_WIDTH);
              end
            if (compare_enable) 
              begin
                sel <= sel+1;
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = k+1)
          begin : patter
            assign pattern = (sel == k) ? patterns[((PATTERN_WIDTH*(k+1))*MSG_WIDTH)+(-1):PATTERN_WIDTH*(k*MSG_WIDTH)] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
