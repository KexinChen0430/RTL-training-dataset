...
module compare  #(parameter 
  ...
  (input  clk,
   ...
   input  [(-1)+(PATTERN_WIDTH*(MSG_WIDTH*NOS_KEY)):0] patterns,
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = i+1)
          begin : compare
            assign compare_data_tmp[i] = ~|(data_in[(-1)+(MSG_WIDTH*B):0] ^ (pattern_comb[i] & (data_in[(-1)+(MSG_WIDTH*B):0] | (~pattern_comb[i] | (~data_in[(-1)+(MSG_WIDTH*B):0] | (~pattern_comb[i] & (pattern_comb[i] & data_in[(-1)+(MSG_WIDTH*B):0])))))));
          end
  endgenerate
  ...
  assign complete_match_wire = ~|(pattern ^ ((pattern | ~pattern) & data_in));
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = k+1)
          begin : patter
            assign pattern = (sel == k) ? patterns[((PATTERN_WIDTH*(k+1))*MSG_WIDTH)-1:k*(PATTERN_WIDTH*MSG_WIDTH)] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
