...
module compare  #(parameter 
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = i+1)
          begin : compare
            assign compare_data_tmp[i] = ~|(data_in[(0-1)+(B*MSG_WIDTH):0] ^ ((data_in[(0-1)+(B*MSG_WIDTH):0] & pattern_comb[i]) | (pattern_comb[i] & ~data_in[(0-1)+(B*MSG_WIDTH):0])));
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((pattern & (((data_in | pattern) & ~pattern) | (~pattern | pattern))) ^ ((((data_in | pattern) & ~pattern) | (~pattern | pattern)) & ((~pattern | pattern) & data_in)));
        ...
        if (reset) 
          ...
          begin
            if (partial_match_wire == 1) 
              ...
              begin
                shift_amount <= ((-B)+1)+PATTERN_WIDTH;
              end
            if (compare_enable) 
              begin
                sel <= 1+sel;
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = 1+k)
          begin : patter
            assign pattern = (sel == k) ? patterns[(0-1)+((1+k)*(PATTERN_WIDTH*MSG_WIDTH)):(k*MSG_WIDTH)*PATTERN_WIDTH] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
