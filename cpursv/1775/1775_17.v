...
module compare  #(parameter 
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = 1+i)
          begin : compare
            assign compare_data_tmp[i] = ~|(pattern_comb[i] ^ data_in[(MSG_WIDTH*B)+(-1):0]);
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((pattern | data_in) & ((~pattern & ((pattern | data_in) | ~data_in)) | ~data_in));
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = k+1)
          begin : patter
            assign pattern = (sel == k) ? patterns[(-1)+((k+1)*(PATTERN_WIDTH*MSG_WIDTH)):(PATTERN_WIDTH*MSG_WIDTH)*k] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
