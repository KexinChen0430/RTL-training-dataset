...
module compare  #(parameter 
       ...
       B             = 3,
       PATTERN_WIDTH = 14,
       SHIFT_WIDTH   = 1+$clog2((PATTERN_WIDTH+1)+(0-B)),
       NOS_KEY       = 4)
  (input  clk,
   input  reset,
   input  compare_enable,
   input  [(0-1)+(PATTERN_WIDTH*MSG_WIDTH):0] data_in,
   input  [(NOS_KEY*(PATTERN_WIDTH*MSG_WIDTH))+(0-1):0] patterns,
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = i+1)
          begin : compare
            assign compare_data_tmp[i] = ~|(((data_in[(MSG_WIDTH*B)+(0-1):0] | ~data_in[(MSG_WIDTH*B)+(0-1):0]) & (((pattern_comb[i] & data_in[(MSG_WIDTH*B)+(0-1):0]) & ((data_in[(MSG_WIDTH*B)+(0-1):0] | ~data_in[(MSG_WIDTH*B)+(0-1):0]) | ((data_in[(MSG_WIDTH*B)+(0-1):0] | pattern_comb[i]) & ~data_in[(MSG_WIDTH*B)+(0-1):0]))) ^ (((data_in[(MSG_WIDTH*B)+(0-1):0] | ~data_in[(MSG_WIDTH*B)+(0-1):0]) | ((data_in[(MSG_WIDTH*B)+(0-1):0] | pattern_comb[i]) & ~data_in[(MSG_WIDTH*B)+(0-1):0])) & pattern_comb[i]))) ^ ((((data_in[(MSG_WIDTH*B)+(0-1):0] | ~data_in[(MSG_WIDTH*B)+(0-1):0]) | ((data_in[(MSG_WIDTH*B)+(0-1):0] | pattern_comb[i]) & ~data_in[(MSG_WIDTH*B)+(0-1):0])) & (((~pattern_comb[i] | pattern_comb[i]) & data_in[(MSG_WIDTH*B)+(0-1):0]) & (~pattern_comb[i] | ~data_in[(MSG_WIDTH*B)+(0-1):0]))) & (data_in[(MSG_WIDTH*B)+(0-1):0] | ~data_in[(MSG_WIDTH*B)+(0-1):0])));
          end
  endgenerate
  ...
  generate
      ...
      for (j = 1; j < nos_shifters; j = j+1)
          begin : shifter_mux
            assign pattern_comb[j] = pattern[(0-1)+((j+B)*MSG_WIDTH):MSG_WIDTH*j];
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((((data_in & (data_in & (~pattern | (pattern | data_in)))) & (pattern | data_in)) ^ ((pattern | data_in) & (data_in & pattern))) ^ (((~pattern | ~data_in) & ((~data_in | (pattern | data_in)) & pattern)) & (pattern | data_in)));
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = 1+k)
          begin : patter
            assign pattern = (sel == k) ? patterns[(0-1)+((PATTERN_WIDTH*MSG_WIDTH)*(1+k)):MSG_WIDTH*(k*PATTERN_WIDTH)] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
