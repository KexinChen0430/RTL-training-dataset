...
module compare  #(parameter 
  ...
  (input  clk,
   ...
   input  [(0-1)+(PATTERN_WIDTH*(MSG_WIDTH*NOS_KEY)):0] patterns,
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = i+1)
          begin : compare
            assign compare_data_tmp[i] = ~|((data_in[(0-1)+(B*MSG_WIDTH):0] & (~data_in[(0-1)+(B*MSG_WIDTH):0] | ~pattern_comb[i])) ^ (pattern_comb[i] ^ (data_in[(0-1)+(B*MSG_WIDTH):0] & pattern_comb[i])));
          end
  endgenerate
  ...
  generate
      ...
      for (j = 1; j < nos_shifters; j = j+1)
          begin : shifter_mux
            assign pattern_comb[j] = pattern[((j+B)*MSG_WIDTH)-1:j*MSG_WIDTH];
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((((pattern | (~pattern & data_in)) | ~data_in) & (data_in & (~pattern | pattern))) ^ (pattern & ((pattern | (~pattern & data_in)) | ~data_in)));
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = 1+k)
          begin : patter
            assign pattern = (sel == k) ? patterns[(PATTERN_WIDTH*((1+k)*MSG_WIDTH))-1:PATTERN_WIDTH*(k*MSG_WIDTH)] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
