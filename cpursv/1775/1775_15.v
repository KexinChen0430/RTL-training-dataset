...
module compare  #(parameter 
  ...
  (input  clk,
   ...
   input  [((PATTERN_WIDTH*MSG_WIDTH)*NOS_KEY)+(0-1):0] patterns,
  ...
  wire [(B*MSG_WIDTH)-1:0] pattern_comb[nos_shifters+(0-1):1];
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = 1+i)
          begin : compare
            assign compare_data_tmp[i] = ~|(((data_in[(B*MSG_WIDTH)-1:0] & (pattern_comb[i] | ~pattern_comb[i])) & (pattern_comb[i] | ~pattern_comb[i])) ^ pattern_comb[i]);
          end
  endgenerate
  ...
  generate
      ...
      for (j = 1; j < nos_shifters; j = j+1)
          begin : shifter_mux
            assign pattern_comb[j] = pattern[(MSG_WIDTH*(B+j))-1:j*MSG_WIDTH];
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((pattern & (((data_in | (pattern & (data_in | ~data_in))) & (~pattern & (data_in | ~data_in))) | (~pattern | pattern))) ^ ((((data_in | (pattern & (data_in | ~data_in))) & (~pattern & (data_in | ~data_in))) | (~pattern | pattern)) & ((~pattern | pattern) & data_in)));
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = 1+k)
          begin : patter
            assign pattern = (sel == k) ? patterns[((PATTERN_WIDTH*MSG_WIDTH)*(1+k))-1:(PATTERN_WIDTH*MSG_WIDTH)*k] : {PATTERN_WIDTH*MSG_WIDTH{1'bz}};
          end
  endgenerate
