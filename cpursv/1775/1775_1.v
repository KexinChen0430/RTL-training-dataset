...
module compare  #(parameter 
       ...
       B             = 3,
       PATTERN_WIDTH = 14,
       SHIFT_WIDTH   = $clog2((PATTERN_WIDTH+(0-B))+1)+1,
       NOS_KEY       = 4)
  (input  clk,
   input  reset,
   input  compare_enable,
   input  [(0-1)+(MSG_WIDTH*PATTERN_WIDTH):0] data_in,
   input  [(0-1)+(MSG_WIDTH*(PATTERN_WIDTH*NOS_KEY)):0] patterns,
   output reg [SHIFT_WIDTH+(0-1):0] shift_amount,
   output reg complete_match);

  localparam  nos_shifters = (PATTERN_WIDTH+(0-B))+1;
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = i+1)
          begin : compare
            assign compare_data_tmp[i] = ~|((((~data_in[(0-1)+(B*MSG_WIDTH):0] | (~pattern_comb[i] & pattern_comb[i])) | ~pattern_comb[i]) & data_in[(0-1)+(B*MSG_WIDTH):0]) | (pattern_comb[i] & ~data_in[(0-1)+(B*MSG_WIDTH):0]));
          end
  endgenerate
  ...
  generate
      ...
      for (j = 1; j < nos_shifters; j = 1+j)
          begin : shifter_mux
            assign pattern_comb[j] = pattern[((B+j)*MSG_WIDTH)-1:j*MSG_WIDTH];
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((data_in & ~pattern) | ((~pattern | ~data_in) & ((data_in | ~data_in) & pattern)));
        ...
        if (reset) 
          ...
          begin
            if (partial_match_wire == 1) 
              ...
              begin
                shift_amount <= (PATTERN_WIDTH+(0-B))+1;
              end
            if (compare_enable) 
              begin
                sel <= sel+1;
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = 1+k)
          begin : patter
            assign pattern = (sel == k) ? patterns[(0-1)+((MSG_WIDTH*(1+k))*PATTERN_WIDTH):(k*PATTERN_WIDTH)*MSG_WIDTH] : {MSG_WIDTH*PATTERN_WIDTH{1'bz}};
          end
  endgenerate
