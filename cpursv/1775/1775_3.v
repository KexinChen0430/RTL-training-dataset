...
module compare  #(parameter 
  ...
  generate
      ...
      for (i = 1; i < nos_shifters; i = i+1)
          begin : compare
            assign compare_data_tmp[i] = ~|((data_in[(-1)+(MSG_WIDTH*B):0] & (~data_in[(-1)+(MSG_WIDTH*B):0] | ~pattern_comb[i])) | (((pattern_comb[i] & data_in[(-1)+(MSG_WIDTH*B):0]) | (~data_in[(-1)+(MSG_WIDTH*B):0] & pattern_comb[i])) & (~data_in[(-1)+(MSG_WIDTH*B):0] | ~pattern_comb[i])));
          end
  endgenerate
  ...
  assign complete_match_wire = ~|((pattern & (~data_in | data_in)) ^ ((data_in & (pattern | ~pattern)) & (~data_in | data_in)));
  ...
  generate
      ...
      for (k = 0; k < NOS_KEY; k = 1+k)
          begin : patter
            assign pattern = (sel == k) ? patterns[((MSG_WIDTH*(1+k))*PATTERN_WIDTH)+(-1):k*(MSG_WIDTH*PATTERN_WIDTH)] : {MSG_WIDTH*PATTERN_WIDTH{1'bz}};
          end
  endgenerate
