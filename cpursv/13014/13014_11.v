...
module Sincronizador(_clk_,boton_arriba_in,boton_abajo_in,boton_izq_in,
                     boton_der_in,boton_elige_in,boton_arriba_out,boton_abajo_out,
                     boton_izq_out,boton_der_out,boton_elige_out);
  ...
  always @(posedge _clk_)
      begin
        if (contador == contador_max) contador <= 1'b0;
  ...
  always @(posedge _clk_)
      begin
        if (contador == contador_max) 
          begin
            sr_boton_arriba <= (2*sr_boton_arriba) | boton_arriba_sync_1;
            sr_boton_abajo <= boton_abajo_sync_1 | (sr_boton_abajo*2);
            sr_boton_izq <= boton_izq_sync_1 | sr_boton_izq<<1;
            sr_boton_der <= boton_der_sync_1 | sr_boton_der<<1;
            sr_boton_elige <= boton_elige_sync_1 | (sr_boton_elige*2);
          end
