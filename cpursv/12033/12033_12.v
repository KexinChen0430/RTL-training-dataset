...
module wb_mcb_8(input  wire clk,
                input  wire rst,
  ...
  assign mcb_cmd_bl = 0;
  assign mcb_cmd_byte_addr = 32'hFFFFFFFC & wb_adr_i;
            ...
            if (cycle_reg) 
              begin
                if (~mcb_rd_empty) 
                  begin
                    cycle_reg <= 0;
                    wb_dat_reg <= mcb_rd_data[(3 & wb_adr_i)*8 +: 8];
                    wb_ack_reg <= 1;
                  end
                  
              end
            else if ((~wb_ack_o & wb_cyc_i) & wb_stb_i) 
              begin
                if (wb_we_i) 
                  begin
