...
module spi_shift  #(parameter  SPI_CHAR_LEN_BITS = 10)
  ...
  assign last = !|cnt;
  assign rx_clk = 
(s_clk || (!last && (rx_negedge ? neg_edge : pos_edge))) && (rx_negedge ? neg_edge : pos_edge);
              ...
              begin
                if (!lsb) 
                  begin
                    ...
                    data <= {data[(-(1+1))+SPI_MAX_CHAR:0],1'b1};
                  end
                else 
                  begin
                    s_out <= data[0];
      ...
      begin
        if (rst) 
          ...
          begin
            if (rx_clk) 
              begin
                miso_data <= {miso_data[(-(1+1))+SPI_MAX_CHAR:0],s_in};
              end
