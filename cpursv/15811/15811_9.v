...
module ntsc_to_zbt(clk,vclk,fvh,dv,din,ntsc_addr,ntsc_data,ntsc_we,sw);
  ...
  reg  even_odd;
  wire  frame = fvh[2];
  wire  frame_edge = ~old_frame & frame;
  ...
  always @(posedge vclk)
      begin
        ...
        if (!fvh[2]) 
          begin
            col <= fvh[0] ? COL_START : 
                  (!fvh[2] && dv && (!fvh[1] && (col < 1024))) ? (col+1) : col;
            row <= fvh[1] ? ROW_START : 
                  (!fvh[2] && (row < 768) && fvh[0]) ? (1+row) : row;
            vdata <= (!fvh[2] && dv) ? din : vdata;
          end
  ...
  always @(posedge clk)
      if (we_edge) mydata <= {mydata[23:0],data[1]};
  ...
  wire [18:0] 
       myaddr = {1'b0,y_addr[8:0],eo_delay[3],x_addr[9:2]};

  wire [31:0]  mydata2 = {data[1],data[1],data[1],data[1]};

  wire [18:0] 
       myaddr2 = {1'b0,y_addr[8:0],eo_delay[3],x_addr[7:0]};
  ...
  wire 
       ntsc_we = sw ? we_edge : ((x_delay[31:30] == 2'b00) & we_edge);
