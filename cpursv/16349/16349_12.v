...
module alt5gxb_reset_controller(input  inclk,
  ...
  localparam [1:0]  stable_tx_pll = 1<<1;
      ...
      begin
        if (async_reset == 1'b1) 
          ...
          begin
            ...
            else if (rx_pll_locked == 1'b0) rx_pll_locked_cnt <= (-1)+rx_pll_locked_cnt;
            ...
            if (ld_ws_tmr == (ld_ws_tmr_short | 1'b1)) 
              begin
                ws_tmr_eq_0 <= 1'b0;
              end
            else if (waitstate_timer == 20'h00000) 
            ...
            case (rst_ctrl_sm)
              ...
              strobe_txpll_locked: begin
                    ld_ws_tmr <= 1'b0;
                    if ((pll_locked == (ws_tmr_eq_0 & 1'b1)) == 1'b1) 
                      begin
              ...
              wait_state: begin
                    if (rx_pll_locked_r == 1'b1) 
                      begin
                        ld_ws_tmr_short <= 1'b0;
                        if ((ld_ws_tmr_short == (ws_tmr_eq_0 & 1'b0)) == 1'b1) 
                          begin
                            rst_ctrl_sm <= idle;
                            txdigitalreset_r <= 1'b0;
                            rxanalogreset_r <= 1'b0;
