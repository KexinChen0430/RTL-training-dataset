...
module FixedRoundSigned2(clk,fixed_num,round);
  ...
  always @(posedge clk)
      begin
        num_orig <= (fixed_num[(-1)+num_width] == 0) ? fixed_num : {fixed_num[(-1)+num_width],~((-1)+fixed_num[num_width-1<<<1:0])};
        num_comp <= (num_orig[(-1)+num_width] == 0) ? {num_orig[(-1)+num_width],num_orig[res_width+(fixed_pos-1):fixed_pos]} : {num_orig[(-1)+num_width],1+~num_orig[res_width+(fixed_pos-1):fixed_pos]};
        ...
        case (num_orig_sflag)
          ...
          1: round <= (num_orig_rflag == 0) ? num_comp : ((-1)+num_comp);
