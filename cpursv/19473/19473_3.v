...
module FixedRoundSigned2(clk,fixed_num,round);
  ...
  always @(posedge clk)
      begin
        ...
        num_comp <= (num_orig[num_width+(-1)] == 0) ? {num_orig[num_width+(-1)],num_orig[(fixed_pos+res_width)-1:fixed_pos]} : {num_orig[num_width+(-1)],1+~num_orig[(fixed_pos+res_width)-1:fixed_pos]};
