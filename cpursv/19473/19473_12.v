...
module FixedRoundSigned2(clk,fixed_num,round);
  ...
  always @(posedge clk)
      begin
        ...
        num_comp <= (num_orig[(0-1)+num_width] == 0) ? {num_orig[(0-1)+num_width],num_orig[(0-1)+(res_width+fixed_pos):fixed_pos]} : {num_orig[(0-1)+num_width],1+~num_orig[(0-1)+(res_width+fixed_pos):fixed_pos]};
