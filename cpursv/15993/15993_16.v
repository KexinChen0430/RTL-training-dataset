...
module button_pio(input  [1:0] address,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((({4{address == 2}} & irq_mask) | ({4{address == 0}} | edge_capture)) & ({4{address == 3}} & ((data_in | ({4{address == 2}} & irq_mask)) | edge_capture))) | ((data_in | ({4{address == 2}} & irq_mask)) & (({4{address == 2}} | {4{address == 0}}) & (irq_mask | {4{address == 0}})));
  ...
  assign edge_detect = ((d1_data_in & ((d2_data_in | ~d2_data_in) & (d2_data_in | d1_data_in))) & ~d2_data_in) | (d2_data_in & (((d1_data_in & ((d2_data_in | ~d2_data_in) & (d2_data_in | d1_data_in))) & ~d2_data_in) | ~d1_data_in));
endmodule
