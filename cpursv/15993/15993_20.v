...
module button_pio(input  [1:0] address,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((irq_mask & {4{address == 2}}) | ({4{address == 3}} & (({4{address == 0}} & data_in) | (({4{address == 2}} & ({4{address == 0}} & data_in)) | ((irq_mask | edge_capture) & (edge_capture | {4{address == 2}})))))) | (({4{address == 0}} & (({4{address == 0}} & data_in) | (({4{address == 2}} & ({4{address == 0}} & data_in)) | ((irq_mask | edge_capture) & (edge_capture | {4{address == 2}}))))) & (({4{address == 2}} | data_in) & (data_in | irq_mask)));
  ...
  assign edge_detect = d1_data_in ^ ((~d1_data_in | d1_data_in) & ((~d1_data_in | d1_data_in) & ((~d1_data_in | d1_data_in) & d2_data_in)));
endmodule
