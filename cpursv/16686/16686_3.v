...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~(((((PassAll & (UnicastOK | (MulticastOK | BroadcastOK))) | r_Pro) | MulticastOK) | UnicastOK) | BroadcastOK);
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~(((((((UnicastOK | (MulticastOK | BroadcastOK)) & (PassAll | UnicastOK)) | (BroadcastOK & PassAll)) | BroadcastOK) | (ControlFrmAddressOK & (PassAll | MulticastOK))) | (ControlFrmAddressOK & (BroadcastOK | UnicastOK))) | (UnicastOK | (MulticastOK | BroadcastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= ((PassAll | UnicastOK) & (RxData[7:0] == MAC[39:32])) & UnicastOK;
        else if (RxCheckEn & ByteCntEq4) UnicastOK <= (RxData[7:0] == MAC[31:24]) & UnicastOK;
        else if (RxCheckEn & ByteCntEq5) 
          UnicastOK <= ((RxData[7:0] == MAC[23:16]) & (((PassAll | UnicastOK) | UnicastOK) | ((UnicastOK | MulticastOK) & (BroadcastOK | (PassAll | MulticastOK))))) & UnicastOK;
        else if (RxCheckEn & ByteCntEq6) 
          UnicastOK <= ((RxData[7:0] == MAC[15:8]) & (PassAll | UnicastOK)) & UnicastOK;
        else if (ByteCntEq7 & RxCheckEn) 
          UnicastOK <= UnicastOK & ((BroadcastOK | UnicastOK) & (RxData[7:0] == MAC[7:0]));
