...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~(((((BroadcastOK | PassAll) | BroadcastOK) | UnicastOK) & ((BroadcastOK | MulticastOK) | UnicastOK)) | (r_Pro | MulticastOK));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~(((BroadcastOK | MulticastOK) | UnicastOK) | ((((BroadcastOK | (ControlFrmAddressOK & (UnicastOK | MulticastOK))) | (PassAll & ControlFrmAddressOK)) | ControlFrmAddressOK) & PassAll));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= UnicastOK & ((RxData[7:0] == MAC[39:32]) & ((BroadcastOK | MulticastOK) | UnicastOK));
        else if (RxCheckEn & ByteCntEq4) 
          UnicastOK <= ((RxData[7:0] == MAC[31:24]) & ((UnicastOK | UnicastOK) | ((BroadcastOK | MulticastOK) | PassAll))) & UnicastOK;
        else if (RxCheckEn & ByteCntEq5) 
          UnicastOK <= ((RxData[7:0] == MAC[23:16]) & ((UnicastOK | UnicastOK) | ((BroadcastOK | MulticastOK) | PassAll))) & UnicastOK;
        else if (ByteCntEq6 & RxCheckEn) 
          UnicastOK <= UnicastOK & ((((BroadcastOK | PassAll) | BroadcastOK) | UnicastOK) & (RxData[7:0] == MAC[15:8]));
        else if (ByteCntEq7 & RxCheckEn) UnicastOK <= (RxData[7:0] == MAC[7:0]) & UnicastOK;
