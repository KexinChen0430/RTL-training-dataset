...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~(((UnicastOK | BroadcastOK) | r_Pro) | ((UnicastOK | UnicastOK) | (MulticastOK | BroadcastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~(((BroadcastOK | ((UnicastOK | PassAll) | (MulticastOK | UnicastOK))) & (ControlFrmAddressOK | UnicastOK)) | (MulticastOK | BroadcastOK));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= ((MulticastOK | UnicastOK) & (RxData[7:0] == MAC[39:32])) & UnicastOK;
        else if (RxCheckEn & ByteCntEq4) 
          UnicastOK <= UnicastOK & ((UnicastOK | BroadcastOK) & (RxData[7:0] == MAC[31:24]));
        else if (ByteCntEq5 & RxCheckEn) UnicastOK <= (RxData[7:0] == MAC[23:16]) & UnicastOK;
        else if (RxCheckEn & ByteCntEq6) 
          UnicastOK <= UnicastOK & (((UnicastOK | PassAll) | BroadcastOK) & (RxData[7:0] == MAC[15:8]));
