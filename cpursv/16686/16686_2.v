...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~((r_Pro | MulticastOK) | ((BroadcastOK | UnicastOK) | (((MulticastOK | (UnicastOK | PassAll)) & ((PassAll | BroadcastOK) | ((BroadcastOK | BroadcastOK) | MulticastOK))) & (MulticastOK | UnicastOK))));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~(((ControlFrmAddressOK & PassAll) | (BroadcastOK | UnicastOK)) | (((MulticastOK | (UnicastOK | PassAll)) & ((PassAll | BroadcastOK) | ((BroadcastOK | BroadcastOK) | MulticastOK))) & (MulticastOK | UnicastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        ...
        else if (ByteCntEq4 & RxCheckEn) UnicastOK <= (RxData[7:0] == MAC[31:24]) & UnicastOK;
        else if (ByteCntEq5 & RxCheckEn) UnicastOK <= UnicastOK & (RxData[7:0] == MAC[23:16]);
        else if (ByteCntEq6 & RxCheckEn) UnicastOK <= UnicastOK & (RxData[7:0] == MAC[15:8]);
        else if (ByteCntEq7 & RxCheckEn) 
          UnicastOK <= ((BroadcastOK | UnicastOK) & (RxData[7:0] == MAC[7:0])) & UnicastOK;
