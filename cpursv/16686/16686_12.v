...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~(((UnicastOK | r_Pro) | (((BroadcastOK | (MulticastOK & PassAll)) | MulticastOK) & (BroadcastOK | UnicastOK))) | ((PassAll | MulticastOK) & ((BroadcastOK | (MulticastOK & PassAll)) | MulticastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~((UnicastOK | MulticastOK) | ((BroadcastOK | PassAll) & (BroadcastOK | ControlFrmAddressOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= UnicastOK & ((((MulticastOK | ControlFrmAddressOK) | BroadcastOK) | (BroadcastOK | UnicastOK)) & (RxData[7:0] == MAC[39:32]));
        else if (ByteCntEq4 & RxCheckEn) 
          UnicastOK <= ((RxData[7:0] == MAC[31:24]) & (BroadcastOK | UnicastOK)) & UnicastOK;
        else if (ByteCntEq5 & RxCheckEn) UnicastOK <= (RxData[7:0] == MAC[23:16]) & UnicastOK;
        else if (RxCheckEn & ByteCntEq6) 
          UnicastOK <= ((((UnicastOK | MulticastOK) | ((BroadcastOK | (MulticastOK & PassAll)) | MulticastOK)) | ((BroadcastOK | PassAll) | (BroadcastOK | UnicastOK))) & (RxData[7:0] == MAC[15:8])) & UnicastOK;
        else if (ByteCntEq7 & RxCheckEn) 
          UnicastOK <= ((RxData[7:0] == MAC[7:0]) & (((PassAll | UnicastOK) | ((PassAll | UnicastOK) & MulticastOK)) | MulticastOK)) & UnicastOK;
