...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~(((UnicastOK | (r_Pro | BroadcastOK)) | (((MulticastOK | PassAll) & ((PassAll | (BroadcastOK & UnicastOK)) | UnicastOK)) & MulticastOK)) | MulticastOK);
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~(((UnicastOK | MulticastOK) | BroadcastOK) | ((BroadcastOK & ControlFrmAddressOK) | (ControlFrmAddressOK & ((PassAll | (BroadcastOK & UnicastOK)) | UnicastOK))));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= ((((MulticastOK | PassAll) | (BroadcastOK & (UnicastOK | MulticastOK))) | (UnicastOK | MulticastOK)) & (RxData[7:0] == MAC[39:32])) & UnicastOK;
        else if (ByteCntEq4 & RxCheckEn) 
          UnicastOK <= UnicastOK & ((RxData[7:0] == MAC[31:24]) & ((((MulticastOK | PassAll) | (BroadcastOK & (UnicastOK | MulticastOK))) | (UnicastOK | MulticastOK)) | ((UnicastOK | BroadcastOK) & (PassAll | (BroadcastOK | (((UnicastOK | MulticastOK) | BroadcastOK) & (MulticastOK | PassAll)))))));
        else if (ByteCntEq5 & RxCheckEn) 
          UnicastOK <= UnicastOK & ((UnicastOK | MulticastOK) & (RxData[7:0] == MAC[23:16]));
        else if (RxCheckEn & ByteCntEq6) 
          UnicastOK <= ((RxData[7:0] == MAC[15:8]) & ((UnicastOK | MulticastOK) | BroadcastOK)) & UnicastOK;
        else if (RxCheckEn & ByteCntEq7) UnicastOK <= UnicastOK & (RxData[7:0] == MAC[7:0]);
