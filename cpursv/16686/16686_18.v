...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~((BroadcastOK | r_Pro) | (((BroadcastOK | BroadcastOK) | UnicastOK) | MulticastOK));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~((((BroadcastOK | BroadcastOK) | UnicastOK) | MulticastOK) | ((PassAll & ControlFrmAddressOK) | (((PassAll & (((BroadcastOK | BroadcastOK) | UnicastOK) | MulticastOK)) | (PassAll & MulticastOK)) | MulticastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= UnicastOK & ((RxData[7:0] == MAC[23:16]) & (PassAll | UnicastOK));
        else if (RxCheckEn & ByteCntEq6) 
          UnicastOK <= (((BroadcastOK | BroadcastOK) | UnicastOK) & (RxData[7:0] == MAC[15:8])) & UnicastOK;
