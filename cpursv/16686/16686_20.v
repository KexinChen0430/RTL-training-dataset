...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~(((r_Pro | (BroadcastOK & (MulticastOK | UnicastOK))) | BroadcastOK) | ((MulticastOK | (PassAll | UnicastOK)) & ((UnicastOK | UnicastOK) | (BroadcastOK | MulticastOK))));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~((ControlFrmAddressOK & (MulticastOK | PassAll)) | ((UnicastOK | UnicastOK) | (BroadcastOK | MulticastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= UnicastOK & ((RxData[7:0] == MAC[39:32]) & (((UnicastOK | ((MulticastOK | (PassAll | UnicastOK)) & ControlFrmAddressOK)) | ControlFrmAddressOK) | BroadcastOK));
        else if (RxCheckEn & ByteCntEq4) 
          UnicastOK <= UnicastOK & ((RxData[7:0] == MAC[31:24]) & (MulticastOK | (PassAll | UnicastOK)));
        else if (RxCheckEn & ByteCntEq5) UnicastOK <= UnicastOK & (RxData[7:0] == MAC[23:16]);
        else if (RxCheckEn & ByteCntEq6) 
          UnicastOK <= ((RxData[7:0] == MAC[15:8]) & (((BroadcastOK | UnicastOK) | PassAll) | ((PassAll | BroadcastOK) | MulticastOK))) & UnicastOK;
