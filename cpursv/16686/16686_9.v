...
module eth_rxaddrcheck(MRxClk,Reset,RxData,Broadcast,r_Bro,r_Pro,ByteCntEq2,
  ...
  assign RxAddressInvalid = ~(((UnicastOK | r_Pro) | ((((UnicastOK | PassAll) | MulticastOK) & ((BroadcastOK | MulticastOK) | (BroadcastOK | PassAll))) & ((UnicastOK | (BroadcastOK & ((BroadcastOK | UnicastOK) & MulticastOK))) | MulticastOK))) | ((((BroadcastOK | UnicastOK) | PassAll) & ((BroadcastOK | MulticastOK) | (BroadcastOK | PassAll))) & (BroadcastOK | UnicastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) AddressMiss <= 1'b0;
          ...
          AddressMiss <= ~(((BroadcastOK | MulticastOK) | UnicastOK) | ((UnicastOK | PassAll) & ((BroadcastOK | (ControlFrmAddressOK | UnicastOK)) | MulticastOK)));
  ...
  always @(posedge MRxClk or posedge Reset)
      begin
        if (Reset) UnicastOK <= 1'b0;
          ...
          UnicastOK <= (((BroadcastOK | (ControlFrmAddressOK | UnicastOK)) | MulticastOK) & (RxData[7:0] == MAC[39:32])) & UnicastOK;
        else if (ByteCntEq4 & RxCheckEn) UnicastOK <= UnicastOK & (RxData[7:0] == MAC[31:24]);
        else if (RxCheckEn & ByteCntEq5) 
          UnicastOK <= UnicastOK & ((RxData[7:0] == MAC[23:16]) & (BroadcastOK | UnicastOK));
        else if (RxCheckEn & ByteCntEq6) 
          UnicastOK <= UnicastOK & ((UnicastOK | PassAll) & (RxData[7:0] == MAC[15:8]));
        else if (ByteCntEq7 & RxCheckEn) 
          UnicastOK <= UnicastOK & ((BroadcastOK | (ControlFrmAddressOK | UnicastOK)) & (RxData[7:0] == MAC[7:0]));
