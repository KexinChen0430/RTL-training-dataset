...
module axi_hp_abort(input  hclk,
  ...
  reg  [7:0]  r_count = 0;
  reg   adav = 0;
  wire  arwr = afi_arvalid && (!hrst && afi_arready);
  ...
  assign busy = busy_r;
  assign afi_rready = busy_aborting && |r_count && 
                      (
((afi_rcount[0] || |afi_rcount[7:1]) && !afi_rready_r) || |afi_rcount[7:1]);
  ...
  always @(posedge hclk)
      begin
        ...
        adav <= (!reset_counters && |aw_count[5:1]) || 
               ((awr && aw_count[0]) || 
((awr || (aw_count[0] || (!ard && awr))) && 
((!reset_counters && (!ard || |aw_count[5:1])) || 
(!ard && awr) || !ard)));
        ard_r <= !ard && 
                ((w_count[3:0] > aw_lengths_ram[aw_lengths_raddr]) && 
adav);
        if (reset_counters) w_count <= 0;
          ...
          w_count <= (0-{4'b0,aw_lengths_ram[aw_lengths_raddr]})+((-1)+w_count);
  ...
  always @(posedge hclk)
      begin
        ...
        if ((|afi_rcount && abort) || 
            ((abort && |afi_racount) || 
((|afi_wacount || |afi_wcount) && abort))) busy_aborting <= 1;
