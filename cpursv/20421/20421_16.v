...
module axi_hp_abort(input  hclk,
  ...
  reg   ard_r = 0;
  wire  ard = (adav && |w_count[7:4]) || (adav && ard_r);
  wire  wwr = afi_wvalid_in && afi_wready && !hrst;
  reg  afi_rready_r;
  reg  afi_wlast_r;
  reg  busy_aborting;
...
(|afi_rcount[7:1] || (afi_rcount[0] && !afi_rready_r) || 
afi_rcount[0]) && !afi_rready_r) || 
                      (|afi_rcount[7:1] && 
(busy_aborting && |r_count && 
(|afi_rcount[7:1] || (afi_rcount[0] && !afi_rready_r) || 
afi_rcount[0])));
  assign afi_wlast = adav && busy_aborting && 
  ...
  always @(posedge hclk)
      begin
        ...
        adav <= 
(awr && 
((|aw_count[5:1] && !reset_counters) || 
(aw_count[0] || (!ard && awr)))) || 
               (
(!ard || (!reset_counters && (!ard || |aw_count[5:1]))) && awr) || 
               (
((|aw_count[5:1] && !reset_counters) || 
((!ard && aw_count[0]) || awr)) && 
((!ard && aw_count[0]) || 
(|aw_count[5:1] && !reset_counters)));
        ...
        if (reset_counters) w_count <= 0;
          ...
          w_count <= ((-1)+w_count)+(-{4'b0,aw_lengths_ram[aw_lengths_raddr]});
          
        dirty <= |aw_count || 
                (busy_aborting && !afi_rready_r && |r_count) || |r_count;
      end
  ...
  always @(posedge hclk)
      begin
        ...
        if (abort && 
            (|afi_wcount || 
((abort && |afi_rcount) || (abort && |afi_racount) || 
(|afi_wacount && abort)))) busy_aborting <= 1;
