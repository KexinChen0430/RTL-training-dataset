...
module axi_hp_abort(input  hclk,
  ...
  reg   ard_r = 0;
  wire  ard = (ard_r && adav) || (|w_count[7:4] && adav);
  ...
  assign afi_rready = 
(|afi_rcount[7:1] || 
(!afi_rready_r && (afi_rcount[0] || |afi_rcount[7:1]))) && (|r_count && busy_aborting);
  ...
  always @(posedge hclk)
      begin
        ...
        adav <= (!ard && awr) || (awr && aw_count[0]) || 
               (
(awr || 
((|aw_count[5:1] || (awr && aw_count[0])) && 
!reset_counters) || (!ard && aw_count[0])) && 
...
(!ard && aw_count[0])));
        ...
        if (reset_counters) w_count <= 0;
          ...
          w_count <= ((-{4'b0,aw_lengths_ram[aw_lengths_raddr]})+(-1))+w_count;
  ...
  always @(posedge hclk)
      begin
        ...
        if (abort && 
            ((abort && |afi_wcount) || 
(|afi_racount || |afi_rcount) || |afi_wacount)) busy_aborting <= 1;
