...
(busy_aborting && |r_count) && !afi_rready_r);
  assign afi_wlast = adav && busy_aborting && 
                     (w_count[3:0] == aw_lengths_ram[aw_lengths_raddr]);
  assign afi_wvalid = !afi_wlast_r && (adav && busy_aborting);
  ...
  always @(posedge hclk)
      begin
        ...
        adav <= (awr && !ard) || (!ard && aw_count[0]) || 
               (((awr && aw_count[0]) || !reset_counters) && 
(|aw_count[5:1] || (awr && aw_count[0])));
        ard_r <= (w_count[3:0] > aw_lengths_ram[aw_lengths_raddr]) && 
                !ard && adav;
        if (reset_counters) w_count <= 0;
        else if (wwr) 
          if (ard) 
            w_count <= w_count+(0-{4'b0,aw_lengths_ram[aw_lengths_raddr]});
          else w_count <= 1+w_count;
        else if (ard) 
          w_count <= (w_count+(0-{4'b0,aw_lengths_ram[aw_lengths_raddr]}))-1;
