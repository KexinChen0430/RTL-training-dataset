...
(awr || 
((|aw_count[5:1] && !reset_counters) || 
(aw_count[0] && awr)))) && !ard)) || (aw_count[0] && awr) || 
               (|aw_count[5:1] && !reset_counters);
        ard_r <= !ard && 
                ((w_count[3:0] > aw_lengths_ram[aw_lengths_raddr]) && 
adav);
...
!afi_rready_r) || |r_count;
  ...
  always @(posedge hclk)
      begin
        ...
        if (
(((|afi_racount && abort) || |afi_rcount) && abort) || 
            ((abort && |afi_wcount) || (|afi_wacount && abort))) busy_aborting <= 1;
