...
module axi_hp_abort(input  hclk,
  ...
  reg   ard_r = 0;
  wire  ard = adav && (ard_r || |w_count[7:4]);
  ...
  assign afi_rready = 
(!afi_rready_r && 
(afi_rcount[0] && busy_aborting && |r_count)) || 
                      (!afi_rready_r && 
((afi_rcount[0] && busy_aborting && |r_count) || 
(busy_aborting && (|r_count && |afi_rcount[7:1])))) || (busy_aborting && (|r_count && |afi_rcount[7:1]));
  ...
  always @(posedge hclk)
      begin
        ...
        else if (ard && !awr) aw_count <= aw_count+(0-1);
          
        adav <= 
(aw_count[0] && 
(awr || (aw_count[0] && !ard) || 
(|aw_count[5:1] && 
((aw_count[0] && awr) || !reset_counters)))) || 
               (
(aw_count[0] || 
((awr || aw_count[0]) && 
(!reset_counters && |aw_count[5:1])) || (!reset_counters && |aw_count[5:1])) && 
((!ard || |aw_count[5:1]) && (!ard || !reset_counters))) || (!ard && awr);
        ard_r <= !ard && adav && 
...
(
((abort && |afi_wcount) || 
(|afi_racount || |afi_rcount)) && abort) || (|afi_wacount && abort)) busy_aborting <= 1;
