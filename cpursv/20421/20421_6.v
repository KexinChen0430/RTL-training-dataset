...
(!afi_rready_r && afi_rcount[0] && |r_count)) && busy_aborting;
  ...
  always @(posedge hclk)
      begin
        ...
        adav <= (aw_count[0] && awr) || 
               ((awr && !ard) || (aw_count[0] && !ard)) || 
               (|aw_count[5:1] && 
((aw_count[0] && !ard) || 
((awr && !ard) || !reset_counters)));
  ...
  always @(posedge hclk)
      begin
        ...
        if (
((|afi_wcount || |afi_wacount || |afi_racount) && abort) || (|afi_rcount && abort)) busy_aborting <= 1;
        else if (done_w) busy_aborting <= 0;
          
        done <= done_w;
        afi_rready_r <= afi_rready;
        afi_wlast_r <= afi_wlast;
        axi_mismatch <= dirty && !busy_aborting && busy;
      end
endmodule
