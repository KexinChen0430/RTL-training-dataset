...
module qmult  #(parameter  Q = 15, N = 32)
  ...
  always @(i_multiplicand or i_multiplier)
      begin
        r_result <= i_multiplicand[(-2)+N:0]*i_multiplier[(-2)+N:0];
        ovr <= 1'b0;
      end
  
  always @(r_result)
      begin
        r_RetVal[(0-1)+N] <= ((i_multiplicand[(0-1)+N] & (~i_multiplier[(0-1)+N] | i_multiplier[(0-1)+N])) & (~i_multiplier[(0-1)+N] | i_multiplier[(0-1)+N])) ^ i_multiplier[(0-1)+N];
        r_RetVal[(-2)+N:0] <= r_result[(Q+N)+(-2):Q];
        if (r_result[N<<<1+(-2):N+(Q+(0-1))] > 0) ovr <= 1'b1;
