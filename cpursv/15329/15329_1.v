
module interrupt_controller(input  wire clock,
                            input  wire reset,
                            input  wire m1_n,
                            input  wire iorq_n,
                            output wire int_n,
                            input  wire [4:0] int_req,
                            output reg [4:0] int_ack,
                            output wire [7:0] jump_addr,
                            input  wire [15:0] A,
                            input  wire [7:0] Di,
                            output wire [7:0] Do,
                            input  wire wr_n,
                            input  wire rd_n,
                            input  wire cs);

  wire [7:0] IF;
  reg  [7:0] IE;
  parameter  POLL_STATE = 0;
  parameter  WAIT_STATE = 1;
  parameter  ACK_STATE = 2;
  parameter  CLEAR_STATE = 3;
  parameter  VBLANK_INT = 0;
  parameter  LCDC_INT = 1;
  parameter  TIMER_INT = 2;
  parameter  SERIAL_INT = 3;
  parameter  INPUT_INT = 4;
  parameter  VBLANK_JUMP = 8'hA0;
  parameter  LCDC_JUMP = 8'hA2;
  parameter  TIMER_JUMP = 8'hA4;
  parameter  SERIAL_JUMP = 8'hA6;
  parameter  INPUT_JUMP = 8'hA8;
  reg  [1:0] state;
  reg  [2:0] interrupt;
  reg  [7:0] reg_out;

  
  always @(posedge clock)
      begin
        if (reset) 
          begin
            IE <= 8'h0;
            state <= POLL_STATE;
          end
        else 
          begin
            if (cs) 
              begin
                if (!wr_n) 
                  begin
                    case (A)

                      16'hFFFF: IE <= Di;

                    endcase

                  end
                else if (!rd_n) 
                  begin
                    case (A)

                      16'hFF0F: reg_out <= IF;

                      16'hFFFF: reg_out <= IE;

                    endcase

                  end
                  
              end
              
            case (state)

              POLL_STATE: begin
                    if (IE[VBLANK_INT] && IF[VBLANK_INT]) 
                      begin
                        interrupt <= VBLANK_INT;
                        state <= WAIT_STATE;
                      end
                    else if (IE[LCDC_INT] && IF[LCDC_INT]) 
                      begin
                        interrupt <= LCDC_INT;
                        state <= WAIT_STATE;
                      end
                    else if (IE[TIMER_INT] && IF[TIMER_INT]) 
                      begin
                        interrupt <= TIMER_INT;
                        state <= WAIT_STATE;
                      end
                    else if (IE[SERIAL_INT] && IF[SERIAL_INT]) 
                      begin
                        interrupt <= SERIAL_INT;
                        state <= WAIT_STATE;
                      end
                    else if (IE[INPUT_INT] && IF[INPUT_INT]) 
                      begin
                        interrupt <= INPUT_INT;
                        state <= WAIT_STATE;
                      end
                      
                  end

              WAIT_STATE: begin
                    if (!m1_n && !iorq_n) state <= ACK_STATE;
                      
                  end

              ACK_STATE: begin
                    int_ack[interrupt] <= 1'b1;
                    state <= CLEAR_STATE;
                  end

              CLEAR_STATE: begin
                    int_ack[interrupt] <= 1'b0;
                    if (iorq_n || m1_n) state <= POLL_STATE;
                      
                  end

            endcase

          end
      end
  assign IF = int_req;
  assign Do = cs ? reg_out : 8'hFF;
  assign int_n = (state == WAIT_STATE) ? 1'b0 : 1'b1;
  assign jump_addr = (interrupt == VBLANK_INT) ? VBLANK_JUMP : 
                     (interrupt == LCDC_INT) ? LCDC_JUMP : 
                     (interrupt == TIMER_INT) ? TIMER_JUMP : 
                     (interrupt == SERIAL_INT) ? SERIAL_JUMP : 
                     (interrupt == INPUT_INT) ? INPUT_JUMP : 8'hZZ;
endmodule

