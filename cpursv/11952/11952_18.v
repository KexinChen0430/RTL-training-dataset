          ...
          begin
            ...
            if (datain_width > round_width) 
              begin
                ...
                for (i = 0; i < ((-1)+((-round_width)+datain_width)); i = 1+i)
                    begin
                      sign = datain[i] | sign;
                      dataout_value[i] = 1'bX;
                    end
                dataout_value[(-1)+((-round_width)+datain_width)] = 1'bX;
                if (datain[(-1)+((-round_width)+datain_width)] == 1'b0) 
                  begin
                    dataout_tmp = result_tmp;
                  end
                else if ((sign == 1'b1) && 
                         (datain[(-1)+((-round_width)+datain_width)] == 1'b1)) 
                  begin
