...
module stratixiv_round_block(datain,round,datain_width,dataout);
        ...
        if (round == 1'b0) dataout_value = datain;
          ...
          begin
            ...
            if (datain_width > round_width) 
              begin
                for (i = (-round_width)+datain_width; i < datain_width; i = i+1)
                    begin
                      result_tmp[j] = datain[i];
                      j = 1+j;
                    end
                for (i = 0; i < ((0-1)+((-round_width)+datain_width)); i = i+1)
                    begin
                      sign = sign | datain[i];
                      dataout_value[i] = 1'bX;
                    end
                dataout_value[(0-1)+((-round_width)+datain_width)] = 1'bX;
                if (datain[(0-1)+((-round_width)+datain_width)] == 1'b0) 
                  begin
                    dataout_tmp = result_tmp;
                  end
                else if (
(datain[(0-1)+((-round_width)+datain_width)] == 1'b1) && (sign == 1'b1)) 
                  begin
