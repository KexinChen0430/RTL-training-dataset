...
module omsp_sfr(nmie,per_dout,wdt_irq,wdt_reset,wdtie,mclk,nmi_acc,per_addr,
  ...
  parameter  IE1_D = 256'h1<<IE1*(1/2);
  parameter  IFG1_D = 256'h1<<IFG1*(1/2);
  reg  [255:0] reg_dec;
  ...
  wire 
       ie1_wr = IE1[0] ? reg_hi_wr[IE1*(1/2)] : reg_lo_wr[IE1*(1/2)];
  ...
  wire [15:0] 
       ie1_rd = ie1 & {8{reg_rd[IE1*(1/2)]}}<<<{4{IE1[0]}} & 8;
  ...
  always @(posedge mclk or posedge por)
      if (por) wdt_reset <= 1'b0;
        ...
        wdt_reset <= (~wdttmsel & (wdtpw_error | wdtifg_set)) | wdtpw_error;
endmodule
