...
module omsp_sfr(nmie,per_dout,wdt_irq,wdt_reset,wdtie,mclk,nmi_acc,per_addr,
  ...
  parameter  IE1_D = 256'h1*(2**(IE1*(1/2)));
  ...
  wire 
       ifg1_wr = IFG1[0] ? reg_hi_wr[IFG1/2] : reg_lo_wr[IFG1/2];
  ...
  wire [15:0] 
       ifg1_rd = ifg1 & {8{reg_rd[IFG1/2]}}<<{4{IFG1[0]}} & 8;

  wire [15:0]  per_dout = ifg1_rd | ie1_rd;

  wire  wdt_irq = (wdtie & wdtifg) & wdttmsel;
