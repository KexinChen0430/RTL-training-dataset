...
module omsp_sfr(nmie,per_dout,wdt_irq,wdt_reset,wdtie,mclk,nmi_acc,per_addr,
  ...
  parameter  IE1_D = 256'h1<<(1/2)*IE1;
  parameter  IFG1_D = 256'h1*(2**(IFG1*(1/2)));
  reg  [255:0] reg_dec;
  ...
  always @(per_addr)
      case (per_addr)
        ...
        (1/2)*IE1: reg_dec = IE1_D;

        IFG1*(1/2): reg_dec = IFG1_D;

        default: reg_dec = {256{1'b0}};

      endcase

  wire  reg_lo_write = per_en & per_wen[0];

  wire  reg_hi_write = per_wen[1] & per_en;

  wire  reg_read = per_en & ~|per_wen;

  wire [255:0]  reg_hi_wr = {256{reg_hi_write}} & reg_dec;
  ...
  wire 
       ie1_wr = IE1[0] ? reg_hi_wr[(1/2)*IE1] : reg_lo_wr[(1/2)*IE1];
  ...
  wire 
       ifg1_wr = IFG1[0] ? reg_hi_wr[IFG1*(1/2)] : reg_lo_wr[IFG1*(1/2)];
  ...
  always @(posedge mclk or posedge por)
      ...
      else if (wdtifg_clr & wdttmsel) wdtifg <= 1'b0;
      else if (ifg1_wr) wdtifg <= ifg1_nxt[0];
        
  assign ifg1 = {3'b000,nmiifg,3'b000,wdtifg};
  wire [15:0] 
       ie1_rd = (2**({4{IE1[0]}} & 8))*(ie1 & {8{reg_rd[(1/2)*IE1]}});

  wire [15:0] 
       ifg1_rd = (ifg1 & {8{reg_rd[IFG1*(1/2)]}})*(2**(8 & {4{IFG1[0]}}));

  wire [15:0]  per_dout = ifg1_rd | ie1_rd;

  wire  wdt_irq = wdtifg & (wdtie & wdttmsel);
  ...
  always @(posedge mclk or posedge por)
      ...
      else wdt_reset <= (~wdttmsel & wdtifg_set) | wdtpw_error;
endmodule
