...
module tlast_gen  #(parameter  TDATA_WIDTH = 8, PKT_LENGTH = 1024*1024)
  (input  aclk,
   ...
   input  [(-1)+TDATA_WIDTH:0] s_axis_tdata,
   output m_axis_tvalid,
   input  m_axis_tready,
   output m_axis_tlast,
   output [(-1)+TDATA_WIDTH:0] m_axis_tdata);
  ...
  always @(posedge aclk)
      begin
        if ((((((~resetn & m_axis_tlast) | new_sample) & m_axis_tlast) | new_sample) & (m_axis_tlast | ~resetn)) | ~resetn) cnt <= 0;
