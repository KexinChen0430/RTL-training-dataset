...
module tlast_gen  #(parameter  TDATA_WIDTH = 8, PKT_LENGTH = 1024*1024)
  (input  aclk,
   ...
   input  [TDATA_WIDTH+(0-1):0] s_axis_tdata,
   output m_axis_tvalid,
   input  m_axis_tready,
   output m_axis_tlast,
   output [TDATA_WIDTH+(0-1):0] m_axis_tdata);
  ...
  always @(posedge aclk)
      begin
        if (~resetn | (m_axis_tlast & (~resetn | new_sample))) cnt <= 0;
        else if (new_sample) cnt <= 1'b1+cnt;
          
      end
  assign m_axis_tlast = cnt == (PKT_LENGTH+(0-1));
endmodule
