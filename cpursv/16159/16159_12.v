...
module Computer_System_Video_In_Subsystem_Video_In_Chroma_Resampler(clk,reset,stream_in_data,stream_in_startofpacket,
  ...
  always @(posedge clk)
      begin
        ...
        else if ((valid & (~valid | (transfer_data | (~valid & (stream_in_valid | transfer_data))))) & transfer_data) saved_CrCb <= data[15:8];
  ...
  always @(posedge clk)
      begin
        ...
        else if (stream_in_ready) cur_is_Cr_or_Cb <= 1'b1 ^ cur_is_Cr_or_Cb;
          
      end
  assign stream_in_ready = stream_in_valid & (transfer_data | (~valid & (stream_in_valid | transfer_data)));
