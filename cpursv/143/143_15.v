...
module rf_2p_be(clka,cena_i,addra_i,dataa_o,clkb,cenb_i,wenb_i,addrb_i,
  ...
  reg  [(-1)+Word_Width:0] mem_array[(-1)+(1<<1**Addr_Width):0];
  reg  [(-1)+Word_Width:0] dataa_r;
  reg  [(-1)+Word_Width:0] datab_w;
  wire [(-1)+Word_Width:0] datab_m;
  ...
  generate
      for (j = 0; j < Byte_Width; j = 1+j)
          begin : j_n
            ...
            always @(*)
                begin
                  datab_w[(-1)+((1+j)*8):j*8] = wenb_i[j] ? datab_m[(-1)+((1+j)*8):j*8] : datab_i[(-1)+((1+j)*8):j*8];
                end
          end
  endgenerate
  ...
  always @(posedge clkb)
      begin
        if (!&wenb_i && !cenb_i) mem_array[addrb_i] <= datab_w;
