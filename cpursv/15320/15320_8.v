...
module qmult  #(parameter  Q = 15, N = 32)
  ...
  always @(r_result)
      begin
        r_RetVal[(-1)+N] <= (i_multiplier[(-1)+N] | (i_multiplicand[(-1)+N] & (~i_multiplicand[(-1)+N] | ~i_multiplier[(-1)+N]))) & (~i_multiplicand[(-1)+N] | ~i_multiplier[(-1)+N]);
        r_RetVal[(0-(1+1))+N:0] <= r_result[(N+Q)+(0-(1+1)):Q];
        if (r_result[(0-(1+1))+N<<<1:(N+Q)+(-1)] > 0) ovr <= 1'b1;
        else ovr <= 1'b0;
      end
endmodule
