...
module qmult  #(parameter  Q = 15, N = 32)
  ...
  always @(r_result)
      begin
        r_RetVal[(-1)+N] <= i_multiplicand[(-1)+N] ^ ((~i_multiplicand[(-1)+N] | i_multiplicand[(-1)+N]) & (i_multiplier[(-1)+N] & (~i_multiplicand[(-1)+N] | i_multiplicand[(-1)+N])));
        r_RetVal[N+(-1<<<1):0] <= r_result[(-1<<<1)+(Q+N):Q];
        if (r_result[(-1<<<1)+N<<<1:(-1)+(Q+N)] > 0) ovr <= 1'b1;
        else ovr <= 1'b0;
      end
endmodule
