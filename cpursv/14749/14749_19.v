...
module bltfill(input  ife,
  ...
  always @(fci or in[0:0])
      carry[0] = (fci & ~in[0]) | ((((fci & (in[0] & fci)) ^ (fci & (((~fci | fci) & in[0]) & in[0]))) & ~fci) ^ ((((in[0] & fci) & in[0]) ^ in[0]) & ~fci));
  
  always @(in or carry)
      for (j = 1; j <= 15; j = j+1)
          carry[j] = ((((in[j] | ~in[j]) & carry[(-1)+j]) & (in[j] | ~in[j])) & ~in[j]) | (~(((in[j] | ~in[j]) & carry[(-1)+j]) & (in[j] | ~in[j])) & in[j]);
  assign fco = carry[15];
