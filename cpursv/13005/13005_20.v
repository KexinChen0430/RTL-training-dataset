...
module wb_dp_ram  #(parameter 
  ...
  (input  wire a_clk,
   input  wire [ADDR_WIDTH+(0-1):0] a_adr_i,
   input  wire [DATA_WIDTH+(0-1):0] a_dat_i,
   output wire [DATA_WIDTH+(0-1):0] a_dat_o,
   input  wire a_we_i,
   input  wire [SELECT_WIDTH+(0-1):0] a_sel_i,
   input  wire a_stb_i,
   output wire a_ack_o,
   input  wire a_cyc_i,
   input  wire b_clk,
   input  wire [ADDR_WIDTH+(0-1):0] b_adr_i,
   input  wire [DATA_WIDTH+(0-1):0] b_dat_i,
   output wire [DATA_WIDTH+(0-1):0] b_dat_o,
   input  wire b_we_i,
   input  wire [SELECT_WIDTH+(0-1):0] b_sel_i,
   input  wire b_stb_i,
   output wire b_ack_o,
   input  wire b_cyc_i);
  ...
  reg  [DATA_WIDTH+(0-1):0]  a_dat_o_reg = {DATA_WIDTH{1'b0}};
  reg   a_ack_o_reg = 1'b0;
  reg  [DATA_WIDTH+(0-1):0]  b_dat_o_reg = {DATA_WIDTH{1'b0}};
  reg   b_ack_o_reg = 1'b0;
  reg  [DATA_WIDTH+(0-1):0] mem[(1<<<1**VALID_ADDR_WIDTH)-1:0];
  wire [VALID_ADDR_WIDTH-1:0] 
       a_adr_i_valid = a_adr_i>>(0-VALID_ADDR_WIDTH)+ADDR_WIDTH;
  wire [VALID_ADDR_WIDTH-1:0] 
       b_adr_i_valid = b_adr_i>>(0-VALID_ADDR_WIDTH)+ADDR_WIDTH;
  ...
  initial  
  begin
    for (i = 0; i < (1<<<1**VALID_ADDR_WIDTH); i = 1+i)
        begin
          mem[i] = 0;
        end
  end
