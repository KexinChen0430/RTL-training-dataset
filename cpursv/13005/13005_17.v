...
module wb_dp_ram  #(parameter 
  ...
  (input  wire a_clk,
   input  wire [(0-1)+ADDR_WIDTH:0] a_adr_i,
   input  wire [DATA_WIDTH+(0-1):0] a_dat_i,
   output wire [DATA_WIDTH+(0-1):0] a_dat_o,
   input  wire a_we_i,
   input  wire [(0-1)+SELECT_WIDTH:0] a_sel_i,
   input  wire a_stb_i,
   output wire a_ack_o,
   input  wire a_cyc_i,
   input  wire b_clk,
   input  wire [(0-1)+ADDR_WIDTH:0] b_adr_i,
   input  wire [DATA_WIDTH+(0-1):0] b_dat_i,
   output wire [DATA_WIDTH+(0-1):0] b_dat_o,
   input  wire b_we_i,
   input  wire [(0-1)+SELECT_WIDTH:0] b_sel_i,
   input  wire b_stb_i,
   output wire b_ack_o,
   input  wire b_cyc_i);
  ...
  parameter  WORD_SIZE = DATA_WIDTH*(1/WORD_WIDTH);
  ...
  reg  [DATA_WIDTH+(0-1):0] mem[(2**VALID_ADDR_WIDTH)-1:0];
  ...
  initial  
  begin
    for (i = 0; i < (2**VALID_ADDR_WIDTH); i = i+1)
        begin
          mem[i] = 0;
        end
  end
