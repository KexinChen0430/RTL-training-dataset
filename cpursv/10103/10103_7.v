...
module fifo  #(parameter  FIFO_SIZE = 8, DATA_WIDTH = 32)
  (input  wire enable,
   ...
   input  wire [DATA_WIDTH+(0-1):0] in_data,
   output wire [DATA_WIDTH+(0-1):0] out_data,
   output wire popped_last,
   output wire pushed_last);

  reg  [DATA_WIDTH+(0-1):0] fifo_data[(0-1)+FIFO_SIZE:0];
  reg  [DATA_WIDTH+(0-1):0] buffer;
                ...
                if (push_clock && ~pop_clock) 
                  ...
                  begin
                    if (data_count >= 1) 
                      begin
                        buffer <= fifo_data[0];
                        data_count <= (0-1)+data_count;
                        pushed_last_value <= 0;
                        for (counter = 0; counter < ((0-1)+FIFO_SIZE); counter = counter+1)
                            fifo_data[counter] <= fifo_data[counter+1];
                        fifo_data[(0-1)+FIFO_SIZE] <= 0;
