...
module fifo  #(parameter  FIFO_SIZE = 8, DATA_WIDTH = 32)
  ...
  reg  [DATA_WIDTH+(0-1):0] fifo_data[FIFO_SIZE+(0-1):0];
                  ...
                  begin
                    if (data_count >= 1) 
                      begin
                        ...
                        pushed_last_value <= 0;
                        for (counter = 0; counter < (FIFO_SIZE+(0-1)); counter = counter+1)
                            fifo_data[counter] <= fifo_data[counter+1];
                        fifo_data[FIFO_SIZE+(0-1)] <= 0;
