...
module MultiUartTx(CLK,RST_X,TXD,ERR,DT01,WE01,DT02,WE02,DT03,WE03,DT04,WE04);
  ...
  assign ERR = (ERR01 | ERR02) | (ERR04 | ERR03);
  assign RE01 = TxRdy & RDY01;
  assign RE02 = (RDY02 & (~en01 & ~RDY01)) & TxRdy;
  assign RE03 = ((~en02 & ~RDY02) & TxRdy) & (RDY03 & (~en01 & ~RDY01));
  assign RE04 = ((~en03 & (~en02 & ~RDY02)) & (TxRdy & ~en01)) & (~RDY01 & (~RDY03 & RDY04));
  assign data = en01 ? {data01,8'h30} : 
                en02 ? {data02,8'h31} : 
                en03 ? {data03,8'h32} : {data04,8'h33};
  assign en = (en03 | en01) | (en02 | en04);
