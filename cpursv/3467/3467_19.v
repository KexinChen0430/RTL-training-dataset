...
module MultiUartTx(CLK,RST_X,TXD,ERR,DT01,WE01,DT02,WE02,DT03,WE03,DT04,WE04);
  ...
  assign ERR = ERR03 | (ERR04 | (ERR01 | ERR02));
  assign RE01 = RDY01 & TxRdy;
  assign RE02 = (RDY02 & TxRdy) & (~en01 & ~RDY01);
  assign RE03 = ~RDY02 & ((~RDY01 & RDY03) & ((~en02 & ~en01) & TxRdy));
  assign RE04 = ((RDY04 & ~en01) & (~RDY01 & ~en02)) & ((~en03 & ~RDY03) & (TxRdy & ~RDY02));
  assign data = en01 ? {data01,8'h30} : 
                en02 ? {data02,8'h31} : 
                en03 ? {data03,8'h32} : {data04,8'h33};
  assign en = en04 | (en01 | (en02 | en03));
