...
module MultiUartTx(CLK,RST_X,TXD,ERR,DT01,WE01,DT02,WE02,DT03,WE03,DT04,WE04);
  ...
  assign ERR = ((ERR04 | ERR01) | ERR02) | ERR03;
  assign RE01 = TxRdy & RDY01;
  assign RE02 = (RDY02 & (~RDY01 & ~en01)) & TxRdy;
  assign RE03 = (~RDY02 & (~RDY01 & ~en01)) & (TxRdy & (~en02 & RDY03));
  assign RE04 = (~en03 & (((~RDY02 & ~en02) & ~RDY03) & (RDY04 & ~en01))) & (TxRdy & ~RDY01);
  assign data = en01 ? {data01,8'h30} : 
                en02 ? {data02,8'h31} : 
                en03 ? {data03,8'h32} : {data04,8'h33};
  assign en = en03 | (en02 | (en04 | en01));
