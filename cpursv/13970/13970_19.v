...
module dma_engine_rr_arb(input  [3:0] dma_wr_mac,
  ...
  always @(posedge clk)
      begin
        if (cnet_reprog || reset) pref_to_read <= 4'h0;
        else if (ctrl_done && xfer_is_rd) pref_to_read <= 'h1+pref_to_read;
  ...
  always @(posedge clk)
      begin
        if (cnet_reprog || reset) 
          begin
            dma_rd_request_q <= 4'hf;
            mac_search <= 4'hf;
            dma_rd_request_q_vld <= 1'b0;
          end
        else if (dma_rd_request && xfer_is_rd) 
          begin
            dma_rd_request_q_vld <= 1'b0;
          end
          ...
          begin
            if (dma_rd_request_q != pref_to_read) 
              begin
                ...
                else mac_search <= (-'h1)+mac_search;
                if (mac_search_one_hot & dma_pkt_avail) 
                  begin
