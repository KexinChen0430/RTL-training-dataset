  ...
  always @(posedge RATE_CLK)
      begin
        if (!RATE_RST_N) txdata_wait_cnt <= 4'd0;
                 ...
                 (txdata_wait_cnt < TXDATA_WAIT_MAX)) txdata_wait_cnt <= txdata_wait_cnt+4'd1;
        else if ((fsm == FSM_TXDATA_WAIT) && 
                 (txdata_wait_cnt == TXDATA_WAIT_MAX)) txdata_wait_cnt <= txdata_wait_cnt;
          ...
          begin
            if (fsm == FSM_RATE_DONE) 
              begin
                ...
                if (rxratedone && txratedone && phystatus) ratedone <= 1'd1;
