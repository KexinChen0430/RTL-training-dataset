...
module sprdma(input  wire clk_in,
              input  wire rst_in,
  ...
  always @* 
      begin
        ...
        if (q_state == S_READY) 
          begin
            if (!cpu_r_nw_in && (cpumc_a_in == 16'h4014)) 
              begin
                d_state = S_ACTIVE;
          ...
          begin
            case (q_cnt)
              ...
              2'h2: begin
                    ...
                    cpumc_r_nw_out = 1'b0;
                    d_cnt = 2'h0;
                    if (q_addr[7:0] == 8'hff) d_state = S_COOLDOWN;
                    else d_addr = q_addr+16'h0001;
                  end
          ...
          begin
            if (cpu_r_nw_in) d_state = S_READY;
