...
module crc  #(parameter  BITS = 8, POLY = 7, INIT = 0)
  ...
  assign sel = (((crcreg[BITS+(0-1)] & (i_bit | ~i_bit)) & (~crcreg[BITS+(0-1)] | ~i_bit)) & ~i_bit) | (~crcreg[BITS+(0-1)] & i_bit);
  ...
  generate
      for (i = 1; i < BITS; i = 1+i)
          begin : gen_crc_wires
            assign crcwires[i] = POLY[i] ? ((sel & ~((sel | ~sel) & crcreg[i-1])) | (((sel | ~sel) & crcreg[i-1]) & ~sel)) : crcreg[i-1];
          end
  endgenerate
