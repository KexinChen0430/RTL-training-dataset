...
module crc  #(parameter  BITS = 8, POLY = 7, INIT = 0)
  ...
  assign sel = ((~i_bit | i_bit) & ((~i_bit | i_bit) & crcreg[BITS+(0-1)])) ^ i_bit;
  ...
  generate
      for (i = 1; i < BITS; i = 1+i)
          begin : gen_crc_wires
            assign crcwires[i] = POLY[i] ? ((((sel & ~crcreg[(0-1)+i]) | (sel & crcreg[(0-1)+i])) & (~crcreg[(0-1)+i] | (sel | crcreg[(0-1)+i]))) ^ crcreg[(0-1)+i]) : crcreg[(0-1)+i];
          end
  endgenerate
