...
module my_fifo  #(parameter  max_len = 16, len_wd = 4, entry_wd = 32)
  ...
  reg  [(0-1)+entry_wd:0] reg_ram[(0-1)+max_len:0];
            ...
            if (wr && ((rd_ptr != wr_ptr) && rd)) 
              begin
                ...
                if (wr_ptr == ((0-1)+max_len)) 
                  ...
                  begin
                    wr_ptr <= #5 {{len_wd+(0-1){1'b0}},1'b1}+wr_ptr;
                  end
              end
            else if (rd && !empty) 
              begin
                len <= #5 len-{{len_wd+(0-1){1'b0}},1'b1};
                if (rd_ptr == ((0-1)+max_len)) 
                  begin
                    rd_ptr <= #5 {len_wd{1'b0}};
                  end
