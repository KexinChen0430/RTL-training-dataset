...
module my_fifo  #(parameter  max_len = 16, len_wd = 4, entry_wd = 32)
      ...
      begin
        if (rst) 
          ...
          begin
            if (wr && (rd && (rd_ptr != wr_ptr))) 
              begin
                if (rd_ptr == (max_len+(-1))) 
                  ...
                  begin
                    rd_ptr <= #5 rd_ptr+{{(-1)+len_wd{1'b0}},1'b1};
                  end
                if (wr_ptr == (max_len+(-1))) 
                  begin
                  ...
                  begin
                    wr_ptr <= #5 {{(-1)+len_wd{1'b0}},1'b1}+wr_ptr;
                  end
              end
            else if (rd && !empty) 
              begin
                len <= #5 len-{{(-1)+len_wd{1'b0}},1'b1};
                if (rd_ptr == (max_len+(-1))) 
                  begin
                    rd_ptr <= #5 {len_wd{1'b0}};
                  end
            ...
            else if (wr && ((rd || ((wr || rd) && !full)) && !full)) 
              begin
                len <= #5 {{(-1)+len_wd{1'b0}},1'b1}+len;
                if (wr_ptr == (max_len+(-1))) 
                  begin
                    wr_ptr <= #5 {len_wd{1'b0}};
                  end
  ...
  always @(posedge clk)
      begin
        if (
((rd_ptr != wr_ptr) || 
(wr && ((rd || ((wr || rd) && !full)) && !full) && 
(!rst && ((rd || ((wr || rd) && !full)) && wr)))) && (!rst && ((rd || ((wr || rd) && !full)) && wr))) 
          begin
