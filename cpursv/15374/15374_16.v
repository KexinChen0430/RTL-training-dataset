...
module dcache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  ...
  reg  [31:0] ddata[0:(1<<<1**CWIDTH)-1];
  reg  [TAGWIDTH+(-1):0] dtags[0:(1<<<1**CWIDTH)-1];
  reg  dvalid[0:(1<<<1**CWIDTH)-1];
  ...
  wire 
       cache_hit = valid_out & (tag_out == dwb_adr_i[AWIDTH-1:(1+CWIDTH)+1]);
  ...
  assign dwb_ack_o = (dwb_stb_i & (dwb_we_i & (dwb_stb_i | stb_d1))) | ((dwb_stb_i & stb_d1) & (dwb_we_i | (((~ack_d1 & (dwb_stb_i | stb_d1)) & dwb_stb_i) & ((((stb_d1 & dwb_we_i) | (~ack_d1 & stb_d1)) | (~ack_d1 & dwb_we_i)) | dwb_we_i))));
