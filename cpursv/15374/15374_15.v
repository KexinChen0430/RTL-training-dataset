...
module dcache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  ...
  localparam  TAGWIDTH = (-2)+(AWIDTH-CWIDTH);
  reg  stb_d1,ack_d1,miss_d1;
  reg  [AWIDTH-1:0] held_addr;
  reg  [31:0] ddata[0:1<<CWIDTH+(0-1)];
  reg  [TAGWIDTH-1:0] dtags[0:1<<CWIDTH+(0-1)];
  reg  dvalid[0:1<<CWIDTH+(0-1)];
  ...
  wire 
       cache_hit = valid_out & (tag_out == dwb_adr_i[AWIDTH-1:2+CWIDTH]);
  ...
  assign dwb_ack_o = (((dwb_stb_i | (stb_d1 & ~ack_d1)) & dwb_we_i) & dwb_stb_i) | ((~ack_d1 | dwb_we_i) & (stb_d1 & dwb_stb_i));
