...
module dcache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  (input  wb_clk_i,
   ...
   output [AWIDTH+(0-1):0] dram_adr_o,
   output dram_we_o,
   output dram_en_o,
  ...
  reg  [31:0] ddata[0:(0-1)+1<<<CWIDTH];
  reg  [TAGWIDTH+(0-1):0] dtags[0:(0-1)+1<<<CWIDTH];
  reg  dvalid[0:(0-1)+1<<<CWIDTH];
  ...
  wire [TAGWIDTH+(0-1):0]  tag_out = dtags[rd_line];
  ...
  wire 
       cache_hit = valid_out & (tag_out == dwb_adr_i[AWIDTH+(0-1):(CWIDTH+1)+1]);
  ...
  assign dwb_ack_o = ((dwb_we_i | ((dwb_stb_i & ~ack_d1) | (((dwb_we_i | dwb_stb_i) & ~ack_d1) & dwb_we_i))) & dwb_stb_i) & (((~ack_d1 & ((((dwb_we_i | dwb_stb_i) & ~ack_d1) & dwb_we_i) | (((stb_d1 & dwb_we_i) & ~ack_d1) | stb_d1))) | ((dwb_we_i | ((stb_d1 & dwb_we_i) | (~ack_d1 | ((((dwb_we_i | dwb_stb_i) & ~ack_d1) & dwb_we_i) | (stb_d1 & dwb_we_i))))) & stb_d1)) | dwb_we_i);
