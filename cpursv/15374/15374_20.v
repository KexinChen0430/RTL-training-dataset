...
module dcache  #(parameter  AWIDTH = 14, CWIDTH = 6)
  (input  wb_clk_i,
   ...
   output [(-1)+AWIDTH:0] dram_adr_o,
   output dram_we_o,
   output dram_en_o,
   output [3:0] dram_sel_o);

  localparam  TAGWIDTH = (AWIDTH-CWIDTH)-1<<1;
  reg  stb_d1,ack_d1,miss_d1;
  reg  [(-1)+AWIDTH:0] held_addr;
  reg  [31:0] ddata[0:(-1)+1<<<CWIDTH];
  reg  [TAGWIDTH+(-1):0] dtags[0:(-1)+1<<<CWIDTH];
  reg  dvalid[0:(-1)+1<<<CWIDTH];
  ...
  wire 
       cache_hit = valid_out & (tag_out == dwb_adr_i[(-1)+AWIDTH:1<<1+CWIDTH]);
  ...
  assign dwb_ack_o = (((dwb_we_i & ((dwb_stb_i | ((~ack_d1 & stb_d1) | (dwb_stb_i & stb_d1))) & (dwb_we_i | ((dwb_we_i & ~ack_d1) | ((dwb_we_i & ~ack_d1) | stb_d1))))) & ((dwb_we_i | ((dwb_we_i & ~ack_d1) | ((dwb_we_i & ~ack_d1) | stb_d1))) & ((dwb_stb_i | (dwb_we_i & ~ack_d1)) | dwb_we_i))) & dwb_stb_i) | (((~ack_d1 & (dwb_stb_i & stb_d1)) | dwb_we_i) & (dwb_stb_i & stb_d1));
