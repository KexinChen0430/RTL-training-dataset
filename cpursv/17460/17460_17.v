...
module soc_system_dipsw_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((data_in | irq_mask) & ({4{address == 0}} & (data_in | ({4{address == 2}} | ({4{address == 0}} & data_in))))) | (((edge_capture & ((({4{address == 2}} | ({4{address == 0}} & data_in)) | {4{address == 3}}) & (({4{address == 0}} & data_in) | (edge_capture | {4{address == 2}})))) & {4{address == 3}}) | (({4{address == 2}} | ((edge_capture & ((({4{address == 2}} | ({4{address == 0}} & data_in)) | {4{address == 3}}) & (({4{address == 0}} & data_in) | (edge_capture | {4{address == 2}})))) & {4{address == 3}})) & irq_mask));
  ...
  assign edge_detect = (~d1_data_in & d2_data_in) | (((~d1_data_in & d2_data_in) | ~d2_data_in) & d1_data_in);
endmodule
