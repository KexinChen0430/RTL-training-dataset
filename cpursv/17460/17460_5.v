...
module soc_system_dipsw_pio(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((edge_capture | ((data_in & ({4{address == 2}} | {4{address == 0}})) & ((({4{address == 0}} & (data_in | irq_mask)) | irq_mask) & {4{address == 0}}))) & (((data_in & ({4{address == 2}} | {4{address == 0}})) & ((({4{address == 0}} & (data_in | irq_mask)) | irq_mask) & {4{address == 0}})) | ((({4{address == 3}} | irq_mask) & ({4{address == 2}} | {4{address == 3}})) | ({4{address == 0}} & (irq_mask & data_in))))) | (({4{address == 2}} | {4{address == 0}}) & ((data_in | {4{address == 2}}) & irq_mask));
  ...
  assign edge_detect = ((~d2_data_in | (((d2_data_in & ~d1_data_in) | d1_data_in) & ~d1_data_in)) & d1_data_in) | (d2_data_in & ~d1_data_in);
endmodule
