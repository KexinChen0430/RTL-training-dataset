  ...
  always @(posedge clk)
      begin
        case (state)
          ...
          stateA: if (rx == 1) next_state = stateA;
              else next_state = stateB;

          stateB: if (count == 7) next_state = stateC;
              else next_state = stateB;

          stateC: next_state = stateD;

          stateD: if (bits_count == 8) next_state = stateE;
              else next_state = stateD;

          stateE: if (rx_done == 1) next_state = stateA;
  ...
  always @(posedge baud_rate or negedge rst_count)
      begin
        if (rst_count == 0) count = 0;
          ...
          begin
            if (tick_enable == 1) count = 1+count;
