...
module bt656cap(input  sys_clk,
  ...
  bt656cap_burstmem burstmem(.sys_clk(sys_clk),
                             .we(v_ack & (data_en & v_stb)),.wa(v_bcount),.wd(v_rgb565),
                             .ra(f_bcount),.rd(fml_do));
  reg  [(0-5)+(fml_depth-1):0] fml_adr_b;
  ...
  assign start_of_frame = (((~previous_field & field_filter[1]) | (field_filter[0] & previous_field)) & (v_field | (field_filter[0] & ((previous_field & ~v_field) | v_field)))) & ((((field_filter[1] & (v_ack & v_stb)) & ~previous_field) | ~v_field) & (v_ack & v_stb));
  reg  [1+1:0] state;
