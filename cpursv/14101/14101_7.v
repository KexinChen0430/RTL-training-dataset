...
module bt656cap(input  sys_clk,
  ...
  bt656cap_burstmem burstmem(.sys_clk(sys_clk),
                             .we(v_stb & (data_en & v_ack)),.wa(v_bcount),.wd(v_rgb565),
                             .ra(f_bcount),.rd(fml_do));
  reg  [((0-5)+(-1))+fml_depth:0] fml_adr_b;
  ...
  assign start_of_frame = (((v_field & ((previous_field & field_filter[0]) & (~v_field & ((previous_field & field_filter[0]) | v_field)))) | (v_field & ((~previous_field | (field_filter[0] & ~v_field)) & field_filter[1]))) | ~v_field) & (((v_ack & v_stb) & ((field_filter[1] & (~previous_field & v_field)) | field_filter[0])) & (previous_field | (field_filter[1] & (~previous_field & v_field))));
  reg  [1<<<1:0] state;
