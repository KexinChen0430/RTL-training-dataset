...
module bt656cap(input  sys_clk,
  ...
  bt656cap_burstmem burstmem(.sys_clk(sys_clk),
                             .we(v_ack & (v_stb & data_en)),.wa(v_bcount),.wd(v_rgb565),
                             .ra(f_bcount),.rd(fml_do));
  reg  [fml_depth+((0-5)+(-1)):0] fml_adr_b;
  ...
  assign start_of_frame = (field_filter[1] | ((((v_stb & ((((~previous_field | ~v_field) & (field_filter[0] | ~previous_field)) & (previous_field & ~v_field)) & field_filter[0])) | (~previous_field & ((v_stb & v_ack) & field_filter[1]))) & v_ack) & ((((~previous_field | ~v_field) & (field_filter[0] | ~previous_field)) & (previous_field & ~v_field)) & field_filter[0]))) & (((((v_stb & ((((~previous_field | ~v_field) & (field_filter[0] | ~previous_field)) & (previous_field & ~v_field)) & field_filter[0])) | (~previous_field & ((v_stb & v_ack) & field_filter[1]))) & v_ack) & ((((~previous_field | ~v_field) & (field_filter[0] | ~previous_field)) & (previous_field & ~v_field)) & field_filter[0])) | ((v_stb & v_ack) & (~previous_field & v_field)));
  reg  [1<<<1:0] state;
