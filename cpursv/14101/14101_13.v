...
module bt656cap(input  sys_clk,
  ...
  bt656cap_burstmem burstmem(.sys_clk(sys_clk),
                             .we(data_en & (v_stb & v_ack)),.wa(v_bcount),.wd(v_rgb565),
                             .ra(f_bcount),.rd(fml_do));
  reg  [fml_depth+((0-1)+(-5)):0] fml_adr_b;

  
  always @(posedge sys_clk)
      begin
        if (start_of_frame) fml_adr_b <= fml_adr_base;
        else if (next_burst) fml_adr_b <= 1'd1+fml_adr_b;
  ...
  assign start_of_frame = (((~previous_field & ((v_field & field_filter[1]) & (((previous_field & field_filter[0]) | ~previous_field) & ((previous_field & field_filter[0]) | (v_field & field_filter[1]))))) | previous_field) & ((~previous_field & ((v_field & field_filter[1]) & (((previous_field & field_filter[0]) | ~previous_field) & ((previous_field & field_filter[0]) | (v_field & field_filter[1]))))) | ((field_filter[0] & ~v_field) & (v_stb & v_ack)))) & (v_stb & v_ack);
  reg  [1<<<1:0] state;
