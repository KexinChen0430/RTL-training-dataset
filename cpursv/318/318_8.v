...
module lcdc(input  wire clk,
  ...
  always @(posedge clk)
      begin
        if (rst) 
          ...
          begin
            send_req_ff <= counter_ff == 4'hf;
            update_x_ff <= counter_ff == 4'hc;
            update_y_ff <= counter_ff == 4'hd;
            update_data_ff <= counter_ff == 4'he;
          end
      end
  ...
  always @(posedge clk)
      begin
        if (rst) lcd_nclk_ff <= 1'b0;
        else 
          begin
            lcd_nclk_ff <= counter_ff < 4'h8;
          end
      end
  reg  [8:0] curr_x_ff;
  ...
  always @(posedge clk)
      begin
        if (rst) lcd_hsync_ff <= 1'b1;
        else 
          begin
            if (update_data) 
              begin
                lcd_hsync_ff <= curr_x_ff != 9'd401;
              end
  ...
  always @(posedge clk)
      begin
        if (rst) lcd_vsync_ff <= 1'b1;
        else 
          begin
            if (update_data) 
              begin
                lcd_vsync_ff <= curr_y_ff != 7'd97;
              end
