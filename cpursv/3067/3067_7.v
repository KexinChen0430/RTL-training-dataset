...
module vga_text(clk25mhz,hindex,vindex,standard,emphasized,background,
                char_data,font_data,char_address,font_address,color);
  ...
  reg  [1+1:0]  char_col = 3'b000;
  reg  [3:0]  char_row = 4'b0000;
  reg  [1+1:0]  dark = 3'b000;
  reg  [1+1:0]  bright = 3'b000;
  reg  [3:0] foreground;
  wire [3:0] pixel_color;

  assign color[7] = dark[1+1];
  assign color[6] = bright[1+1];
  assign color[5] = 0;
  ...
  always @(posedge clk25mhz)
      begin : char_addressing
        if ((vindex >= (1+1)) && (vindex < 478)) 
          begin
            if (hindex < 640) 
              begin
                if (char_col == 7) 
              ...
              begin
                if (char_row == 13) 
                  ...
                  begin
                    char_row <= 1+char_row;
                    char_address <= (0-80)+char_address;
                  end
              end
  ...
  always @(posedge clk25mhz)
      begin : indexes
        if ((hindex < 641) && (hindex > 0) && (vindex < 480)) 
          begin
            dark <= pixel_color[1+1:0];
            if (pixel_color[3] == 1) 
              begin
                if (pixel_color[1+1:0] == 0) 
                  begin
                    bright <= 3'b111;
                  end
                else 
                  begin
                    bright <= pixel_color[1+1:0];
                  end
              end
            else 
              begin
