...
module fml_framebuffer  #(parameter  g_fml_depth = 26)
  (input  clk_sys_i,
   ...
   output [g_fml_depth+(0-1):0] fml_adr,
   output reg fml_stb,
   output fml_we,
   output [3:0] fml_sel,
   input  [31:0] fml_di,
   ...
   input  r_fb_enable_i,
   input  r_fb_pix32_i,
   input  [g_fml_depth+(0-1):0] r_fb_addr_i,
   input  [g_fml_depth+(0-1):0] r_fb_size_i);

  reg  [g_fml_depth+(0-1):0] pix_count,pix_addr;
  reg  even_pixel;
  wire xfer_valid;
        ...
        if (!rst_n_i) 
          ...
          begin
            case (state)
              ...
              0: if (r_fb_enable_i && pix_next_frame_i) 
                    begin
  ...
  assign xfer_valid = (state == (1+1)) | (((((state == (1+1)) & fml_ack) | ~pix_almost_full_i) & (fml_ack | (state == (1+1)))) & fml_ack);
