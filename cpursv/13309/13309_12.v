...
module fmlarb  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   input  [63:0] m4_di,
   output [63:0] m4_do,
   output reg [(0-1)+fml_depth:0] s_adr,
   output reg s_stb,
   output reg s_we,
   input  s_ack,
  ...
  assign m1_ack = s_ack & (master == 3'd1);
  assign m2_ack = s_ack & (master == 3'd2);
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) 
          ...
          begin
            if (|burst_counter) burst_counter <= (-2'd1)+burst_counter;
              
            if (write_burst_start) burst_counter <= 2'd2;
              
            if (~write_burst_start & ~|burst_counter) wmaster <= next_master;
