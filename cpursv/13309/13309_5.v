...
module fmlarb  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   input  [63:0] m4_di,
   output [63:0] m4_do,
   output reg [fml_depth+(0-1):0] s_adr,
   output reg s_stb,
   output reg s_we,
   input  s_ack,
  ...
  always @(posedge sys_clk)
      begin
        if (sys_rst) 
          ...
          begin
            if (|burst_counter) burst_counter <= burst_counter+(0-2'd1);
