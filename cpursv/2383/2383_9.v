...
module vga_sync(input  wire clock,
  ...
  localparam  H_BLANK = H_BACK+(H_SYNC+H_FRONT);
  localparam  H_TOTAL = (H_BACK+H_ACT)+(H_SYNC+H_FRONT);
  localparam  H_TOTAL_WIDTH = 10;
  localparam  V_FRONT = 37;
  localparam  V_SYNC = 6;
  localparam  V_BACK = 23;
  localparam  V_ACT = 600;
  localparam  V_BLANK = (V_BACK+V_SYNC)+V_FRONT;
  localparam  V_TOTAL = ((V_BACK+V_SYNC)+V_FRONT)+V_ACT;
  localparam  V_TOTAL_WIDTH = 10;

  assign vga_clk = ~clock;
  reg  [H_TOTAL_WIDTH+(-1):0] hor_pos;

  reg  [(-1)+V_TOTAL_WIDTH:0] ver_pos;
      ...
      begin
        if (~aresetn) 
          ...
          begin
            ...
            if (hor_pos == ((-1)+H_FRONT)) h_sync <= 1'b1;
              
            if (hor_pos == (H_FRONT+((-1)+H_SYNC))) h_sync <= 1'b0;
  ...
  always @(posedge h_sync)
      begin
        if (~aresetn) 
          ...
          begin
            if (ver_pos < V_TOTAL) ver_pos <= 1+ver_pos;
            else ver_pos <= 0;
            if (ver_pos == (V_FRONT-1)) v_sync <= 1'b1;
              
            if (ver_pos == ((V_FRONT-1)+V_SYNC)) v_sync <= 1'b0;
