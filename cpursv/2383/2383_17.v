...
module vga_sync(input  wire clock,
  ...
  localparam  H_BLANK = H_BACK+(H_FRONT+H_SYNC);
  ...
  localparam  V_BLANK = (V_SYNC+V_FRONT)+V_BACK;
  localparam  V_TOTAL = (V_BACK+(V_FRONT+V_ACT))+V_SYNC;
  localparam  V_TOTAL_WIDTH = 10;

  assign vga_clk = ~clock;
  reg  [(0-1)+H_TOTAL_WIDTH:0] hor_pos;

  reg  [V_TOTAL_WIDTH+(0-1):0] ver_pos;
      ...
      begin
        if (~aresetn) 
          ...
          begin
            ...
            if (hor_pos == ((0-1)+H_FRONT)) h_sync <= 1'b1;
              
            if (hor_pos == (H_FRONT+(H_SYNC+(0-1)))) h_sync <= 1'b0;
      ...
      begin
        if (~aresetn) 
          ...
          begin
            ...
            if (ver_pos == ((0-1)+V_FRONT)) v_sync <= 1'b1;
              
            if (ver_pos == (((0-1)+V_FRONT)+V_SYNC)) v_sync <= 1'b0;
  ...
  always @(posedge clock)
      begin
        ...
        else if ((ver_pos < V_BLANK) | (hor_pos < H_BLANK)) 
          begin
            R <= 8'd0;
            B <= 8'd0;
            G <= 8'd0;
