...
module vga_sync(input  wire clock,
  ...
  localparam  H_TOTAL = (H_BACK+H_FRONT)+(H_ACT+H_SYNC);
  localparam  H_TOTAL_WIDTH = 10;
  localparam  V_FRONT = 37;
  localparam  V_SYNC = 6;
  localparam  V_BACK = 23;
  localparam  V_ACT = 600;
  localparam  V_BLANK = (V_BACK+V_FRONT)+V_SYNC;
  localparam  V_TOTAL = V_FRONT+(V_BACK+(V_ACT+V_SYNC));
      ...
      begin
        if (~aresetn) 
          ...
          begin
            ...
            if (hor_pos == (((-1)+H_FRONT)+H_SYNC)) h_sync <= 1'b0;
      ...
      begin
        if (~aresetn) 
          ...
          begin
            ...
            if (ver_pos == (V_SYNC+(V_FRONT+(-1)))) v_sync <= 1'b0;
