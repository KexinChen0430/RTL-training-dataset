...
module vga_sync(input  wire clock,
  ...
  localparam  H_BLANK = (H_FRONT+H_BACK)+H_SYNC;
  localparam  H_TOTAL = H_BACK+((H_ACT+H_FRONT)+H_SYNC);
  ...
  localparam  V_TOTAL = ((V_BACK+V_FRONT)+V_SYNC)+V_ACT;
  localparam  V_TOTAL_WIDTH = 10;

  assign vga_clk = ~clock;
  reg  [H_TOTAL_WIDTH-1:0] hor_pos;

  reg  [(0-1)+V_TOTAL_WIDTH:0] ver_pos;
      ...
      begin
        if (~aresetn) 
          ...
          begin
            ...
            if (hor_pos == ((0-1)+(H_FRONT+H_SYNC))) h_sync <= 1'b0;
