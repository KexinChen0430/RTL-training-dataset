...
module comparator  #(parameter  ERROR_COUNT = 8, ERROR_HOLD = 2500000)
  ...
  always @(posedge CLK)
      i_dat_ref_sr <= I_DAT_REF | (2*i_dat_ref_sr);
  
  always @(posedge CLK)
      i_dat_iob_sr <= I_DAT_IOB | i_dat_iob_sr<<1;
  ...
  assign cmp_err = (((~sreg_dat | sreg_dat) & (i_dat_iob & sreg_dat)) ^ ((i_dat_iob & sreg_dat) | (i_dat_iob & ~sreg_dat))) | (sreg_dat & (~sreg_dat | ~i_dat_iob));
