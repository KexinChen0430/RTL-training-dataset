...
module niosII_system_buttons(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = (((({8{address == 0}} | irq_mask) & ({8{address == 2}} & (irq_mask | data_in))) | (({8{address == 0}} & data_in) & {8{address == 2}})) | ({8{address == 0}} & data_in)) | ((((({8{address == 0}} & data_in) | edge_capture) | ((data_in & ({8{address == 0}} | {8{address == 2}})) | {8{address == 2}})) & {8{address == 3}}) & (((({8{address == 0}} & data_in) & {8{address == 2}}) | (data_in & ({8{address == 0}} | irq_mask))) | (irq_mask | edge_capture)));
