...
module niosII_system_buttons(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((((data_in | (({8{address == 2}} | {8{address == 0}}) & data_in)) | (((({8{address == 2}} | {8{address == 0}}) & data_in) & {8{address == 0}}) | {8{address == 2}})) & (irq_mask & ({8{address == 2}} | {8{address == 0}}))) | ((({8{address == 2}} | {8{address == 0}}) & data_in) & ({8{address == 0}} | irq_mask))) | ({8{address == 3}} & edge_capture);
  ...
  always @(posedge clk or negedge reset_n)
      begin
        ...
        else if ((address == 2) && (~write_n && chipselect)) irq_mask <= writedata[7:0];
