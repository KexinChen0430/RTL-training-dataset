...
module spi_slave(clk,reset,spiclk,spimosi,spimiso,spicsl,we,re,wdat,addr,rdat);
  ...
  always @(posedge spiclk or posedge spi_reset)
      if (spi_reset) 
        ...
        begin
          mosi_cnt <= 1+mosi_cnt;
          mosi_shift <= {mosi_shift[(0-1<<1)+dsz:0],spimosi};
          if (mosi_cnt == 0) rd <= spimosi;
            
          if (mosi_cnt == asz) 
            begin
              addr <= {mosi_shift[(0-1<<1)+asz:0],spimosi};
              eoa <= 1'b1;
            end
          ...
          if (mosi_cnt == (dsz+asz)) 
            begin
              wdat <= {mosi_shift[(0-1<<1)+dsz:0],spimosi};
              eot <= 1'b1;
            end
  ...
  always @(negedge spiclk or posedge spi_reset)
      if (spi_reset) 
        ...
        begin
          ...
          else miso_shift <= {miso_shift[(0-1<<1)+dsz:0],1'b0};
        end
