...
module spi_slave(clk,reset,spiclk,spimosi,spimiso,spicsl,we,re,wdat,addr,rdat);
  ...
  always @(posedge spiclk or posedge spi_reset)
      if (spi_reset) 
        ...
        begin
          mosi_cnt <= 1+mosi_cnt;
          mosi_shift <= {mosi_shift[(-(1+1))+dsz:0],spimosi};
          ...
          if (mosi_cnt == (asz+dsz)) 
            begin
              wdat <= {mosi_shift[(-(1+1))+dsz:0],spimosi};
              eot <= 1'b1;
            end
            
        end
  reg  [(-1)+dsz:0] miso_shift;
  ...
  always @(negedge spiclk or posedge spi_reset)
      if (spi_reset) 
        ...
        begin
          ...
          else miso_shift <= {miso_shift[(-(1+1))+dsz:0],1'b0};
        end
  assign spimiso = eoa ? miso_shift[(-1)+dsz] : 1'b0;
  reg  [1+1:0] we_dly;
