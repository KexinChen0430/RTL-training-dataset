...
module spi_slave(clk,reset,spiclk,spimosi,spimiso,spicsl,we,re,wdat,addr,rdat);
  ...
  output we;
  output re;
  output [dsz+(0-1):0] wdat;
  output [asz-1:0] addr;
  input  [dsz+(0-1):0] rdat;
  reg  [5:0] mosi_cnt;
  reg  [dsz+(0-1):0] mosi_shift;
  reg  rd;
  reg  [asz-1:0] addr;
  reg  eoa;
  reg  re;
  ...
  always @(posedge spiclk or posedge spi_reset)
      if (spi_reset) 
        ...
        begin
          ...
          if (mosi_cnt == asz) 
            begin
              addr <= {mosi_shift[asz+(0-(1+1)):0],spimosi};
              eoa <= 1'b1;
            end
  ...
  assign spimiso = eoa ? miso_shift[dsz+(0-1)] : 1'b0;
  reg  [1+1:0] we_dly;
