...
module RegisterFile(input  clock,
  ...
  always @(posedge clock)
      begin
        for (i = 1; i < 32; i = i+1)
            begin
              vote_registers[i] <= reset ? 0 : 
                                  ((WriteReg == i) && RegWrite) ? WriteData : registers[i];
            end
      end
  genvar j;
  ...
  generate
      for (j = 1; j < 32; j = 1+j)
          begin : Voter_Signals
            assign registers[j] = registers_in[(0-1)+(j*32):((0-1)+j)*32];
            assign registers_out[(0-1)+(j*32):((0-1)+j)*32] = vote_registers[j];
          end
  endgenerate
