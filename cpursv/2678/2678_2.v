...
module axis_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [(((0-1)+1)+DATA_WIDTH)+1:0]  data_out_reg = {1'b0,1'b0,{DATA_WIDTH{1'b0}}};
  reg  [(((0-1)+1)+DATA_WIDTH)+1:0] mem[(0-1)+(2**ADDR_WIDTH):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(((0-1)+1)+DATA_WIDTH)+1:0] 
       data_in = {input_axis_tlast,input_axis_tuser,input_axis_tdata};
  wire 
       full = (wr_ptr[ADDR_WIDTH] != rd_ptr[ADDR_WIDTH]) && 
(wr_ptr[ADDR_WIDTH+(0-1):0] == rd_ptr[ADDR_WIDTH+(0-1):0]);
  wire  empty = wr_ptr == rd_ptr;
  wire  write = input_axis_tvalid & ~full;
  wire 
       read = (~empty & output_axis_tready) | (~empty & ~output_axis_tvalid_reg);
  ...
  always @(posedge clk or posedge rst)
      begin
        ...
        else if (output_axis_tready | ((~empty & output_axis_tready) | ~output_axis_tvalid_reg)) 
          begin
