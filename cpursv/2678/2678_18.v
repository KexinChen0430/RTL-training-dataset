...
module axis_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [(1+(0-1))+(1+DATA_WIDTH):0] mem[(0-1)+(2**ADDR_WIDTH):0];
...
(wr_ptr[(0-1)+ADDR_WIDTH:0] == rd_ptr[(0-1)+ADDR_WIDTH:0]);
  ...
  wire 
       read = ~empty & (((~empty | output_axis_tready) & (~output_axis_tvalid_reg | output_axis_tready)) & (~empty | ~output_axis_tvalid_reg));
