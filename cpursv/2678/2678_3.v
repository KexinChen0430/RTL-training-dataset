...
module axis_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [(DATA_WIDTH+1<<1)-1:0] mem[(1<<1**ADDR_WIDTH)+(0-1):0];
...
(wr_ptr[ADDR_WIDTH+(0-1):0] == rd_ptr[ADDR_WIDTH+(0-1):0]) && (wr_ptr[ADDR_WIDTH] != rd_ptr[ADDR_WIDTH]);
  ...
  wire 
       read = (output_axis_tready | (~output_axis_tvalid_reg & ~empty)) & ~empty;
  ...
  always @(posedge clk or posedge rst)
      begin
        ...
        else if (~output_axis_tvalid_reg | (output_axis_tready | (~output_axis_tvalid_reg & ~empty))) 
          begin
