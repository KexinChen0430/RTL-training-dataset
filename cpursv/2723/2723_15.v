...
module mc_mff_key  #(parameter  BYTES = 4)
  (input  [(8*BYTES)+(-1):0] data_in,
   ...
   input  [1:0] pix,
   input  [2:0] kcnt,
   input  [(-1)+BYTES:0] mask_in,
   output reg [(-1)+BYTES:0] key_mask);

  reg  [(-1)+BYTES:0] key_mask_tmp1;
  reg  [(-1)+BYTES:0] key_mask_tmp2;
  ...
  always @* 
      begin
        ...
        key_mask[3:0] = (key_mask_tmp2[3:0] | mask_in[3:0]) & (mask_in[3:0] | {4{kcnt[0] & kcnt[2]}});
      end
endmodule
