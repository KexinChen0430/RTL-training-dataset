...
module mc_mff_key  #(parameter  BYTES = 4)
  (input  [(0-1)+(BYTES*8):0] data_in,
   input  [31:0] key_in,
   input  [1:0] pix,
   input  [1<<<1:0] kcnt,
   input  [BYTES+(0-1):0] mask_in,
   output reg [BYTES+(0-1):0] key_mask);

  reg  [BYTES+(0-1):0] key_mask_tmp1;
  reg  [BYTES+(0-1):0] key_mask_tmp2;
  ...
  always @* 
      begin
        ...
        key_mask[3:0] = (key_mask_tmp2[3:0] & ({4{kcnt[0] & kcnt[1<<<1]}} | mask_in[3:0])) | mask_in[3:0];
      end
endmodule
