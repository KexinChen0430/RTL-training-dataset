...
module mc_mff_key  #(parameter  BYTES = 4)
  (input  [(8*BYTES)-1:0] data_in,
   ...
   input  [1:0] pix,
   input  [1<<1:0] kcnt,
   input  [(0-1)+BYTES:0] mask_in,
   output reg [(0-1)+BYTES:0] key_mask);

  reg  [(0-1)+BYTES:0] key_mask_tmp1;
  reg  [(0-1)+BYTES:0] key_mask_tmp2;
  ...
  always @* 
      begin
        ...
        key_mask[3:0] = (((key_mask_tmp2[3:0] | {4{kcnt[1<<1] & kcnt[0]}}) & (mask_in[3:0] | key_mask_tmp2[3:0])) & {4{kcnt[1<<1] & kcnt[0]}}) | mask_in[3:0];
      end
endmodule
