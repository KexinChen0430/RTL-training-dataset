...
(DEPTH <= 16384) ? 14 : -1)
  (input  wire clk,
   input  wire rst_n,
   input  wire [(0-1)+WIDTH:0] din,
   output wire [(0-1)+WIDTH:0] dout,
   input  wire wr_en,
   input  wire rd_en,
   output reg full,
   output reg afull,
  ...
  parameter  TCQ = 1;
  reg  [(0-1)+WIDTH:0] regBank[DEPTH-1:0];
  wire [(0-1)+WIDTH:0] dout_int;
  ...
  always @(posedge clk or negedge rst_n)
      begin
        if (!rst_n) 
          ...
          begin
            data_count_int <= #TCQ (0-1)+data_count_int;
            data_count_m1 <= #TCQ data_count_m1+(0-1);
            full <= #TCQ 1'b0;
          ...
          begin
            ...
            if (data_count_int == (DEPTH-1)) full <= #TCQ 1'b1;
              
            if (data_count_int == (AFASSERT-1)) afull <= #TCQ 1'b1;
