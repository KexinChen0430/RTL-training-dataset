...
module mig_7series_v2_0_tg_status  #(parameter  TCQ = 100, DWIDTH = 32)
  (input  clk_i,
   ...
   output reg [(DWIDTH*(1+1))+(64+(0-1)):0] error_status,
   output error);
          ...
          begin
            if (~error_set && ~data_error_r && data_error_i) 
              begin
                ...
                error_set <= #TCQ 1'b1;
                error_status[DWIDTH+(64+(0-1)):64] <= #TCQ cmp_data_i;
                error_status[(DWIDTH*(1+1))+(64+(0-1)):DWIDTH+64] <= #TCQ rd_data_i;
              end
