...
module mig_7series_v2_0_tg_status  #(parameter  TCQ = 100, DWIDTH = 32)
  (input  clk_i,
   ...
   output reg [64+((0-1)+(DWIDTH*1<<<1)):0] error_status,
   output error);
  ...
  always @(posedge clk_i)
      begin
        if (rst_i || manual_clear_error) 
          ...
          begin
            if (data_error_i && ~data_error_r && ~error_set) 
              begin
                ...
                error_set <= #TCQ 1'b1;
                error_status[((0-1)+64)+DWIDTH:64] <= #TCQ cmp_data_i;
                error_status[64+((0-1)+(DWIDTH*1<<<1)):64+DWIDTH] <= #TCQ rd_data_i;
              end
