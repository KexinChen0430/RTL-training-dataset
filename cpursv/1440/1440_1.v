          ...
          begin
            if (vsync_rising) 
              begin
                ...
                if (vid_field_id || 
                    (!INTERLACE || 
(vid_field_id && (!INTERLACE || count_valid)))) count_valid <= 1;
                  
                if (
((!INTERLACE && count_valid) || vid_field_id || count_valid) && 
                    (
(!INTERLACE && 
((!INTERLACE && count_valid) || vid_field_id)) || 
(count_valid && 
(vid_field_id && (!INTERLACE || count_valid)))) && count_valid) frame_complete <= 1;
