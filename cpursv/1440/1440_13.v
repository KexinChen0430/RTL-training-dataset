          ...
          begin
            if (vsync_rising) 
              begin
                vblank <= 1;
                line_count <= 0;
                if (!INTERLACE || (vid_field_id && count_valid) || 
                    vid_field_id) count_valid <= 1;
                  
                if (
((count_valid && !INTERLACE) || 
(!INTERLACE && vid_field_id) || (vid_field_id && count_valid)) && count_valid) frame_complete <= 1;
