...
module phy_emulation  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  (input  wire clk,
   ...
   input  wire [(0-1)+DATA_WIDTH:0] video_data,
   output reg error_out,
   output reg frame_complete);

  reg  [13:0] pixel_count;
  reg  [13:0] line_count;
  reg  [(0-1)+DATA_WIDTH:0] video_data_1;
  reg  de_1;
  reg  de_2;
  reg  hsync_1;
  reg  vsync_1;
  wire [(0-1)+DATA_WIDTH:0] expected_video_data;
  reg   count_valid = 0;
  wire vsync_rising;
  reg  vblank;
  wire compare_valid;

  assign vsync_rising = !vsync_1 & vsync;
          ...
          begin
            if (vsync_rising) 
              begin
                vblank <= 1;
                line_count <= 0;
                if (!INTERLACE || vid_field_id) count_valid <= 1;
                  
                if ((!INTERLACE && count_valid) || 
                    (count_valid && vid_field_id)) frame_complete <= 1;
