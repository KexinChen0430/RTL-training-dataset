...
module phy_emulation  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  (input  wire clk,
   ...
   input  wire [DATA_WIDTH+(0-1):0] video_data,
   output reg error_out,
   output reg frame_complete);

  reg  [13:0] pixel_count;
  reg  [13:0] line_count;
  reg  [DATA_WIDTH+(0-1):0] video_data_1;
  reg  de_1;
  reg  de_2;
  reg  hsync_1;
  reg  vsync_1;
  wire [DATA_WIDTH+(0-1):0] expected_video_data;
  reg   count_valid = 0;
  wire vsync_rising;
  reg  vblank;
  wire compare_valid;
  ...
  assign compare_valid = !frame_complete & (de & count_valid);
  ...
  always @(posedge clk)
      begin
        if (vid_ce) 
          begin
            if (rst || hsync) pixel_count <= 0;
            else if (de_1) pixel_count <= pixel_count+1;
          ...
          begin
            if (vsync_rising) 
              begin
                vblank <= 1;
                line_count <= 0;
                if ((vid_field_id && count_valid) || 
                    (!INTERLACE && count_valid) || !INTERLACE || 
                    (vid_field_id || 
((vid_field_id && count_valid) || 
(!INTERLACE && count_valid)))) count_valid <= 1;
                  
                if ((vid_field_id && count_valid) || 
                    (!INTERLACE && count_valid)) frame_complete <= 1;
