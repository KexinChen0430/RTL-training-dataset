...
module phy_emulation  #(parameter  DLY = 1, INTERLACE = 0, DATA_WIDTH = 24)
  (input  wire clk,
   ...
   input  wire [(-1)+DATA_WIDTH:0] video_data,
   output reg error_out,
   output reg frame_complete);

  reg  [13:0] pixel_count;
  reg  [13:0] line_count;
  reg  [(-1)+DATA_WIDTH:0] video_data_1;
  reg  de_1;
  reg  de_2;
  reg  hsync_1;
  reg  vsync_1;
  wire [(-1)+DATA_WIDTH:0] expected_video_data;
  reg   count_valid = 0;
  wire vsync_rising;
  reg  vblank;
  wire compare_valid;
  ...
  assign compare_valid = de & (!frame_complete & count_valid);
          ...
          begin
            if (vsync_rising) 
              begin
                ...
                line_count <= 0;
                if (!INTERLACE || 
                    ((!INTERLACE && count_valid) || vid_field_id)) count_valid <= 1;
                  
                if (count_valid && 
                    ((count_valid && vid_field_id) || 
((!INTERLACE && count_valid) || 
(!INTERLACE && vid_field_id)))) frame_complete <= 1;
              ...
              begin
                if (!vblank) line_count <= line_count+1;
