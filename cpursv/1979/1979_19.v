...
module axis_async_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [2+((-1)+DATA_WIDTH):0]  data_out_reg = {1'b0,1'b0,{DATA_WIDTH{1'b0}}};
  reg  [2+((-1)+DATA_WIDTH):0] mem[(-1)+(2**ADDR_WIDTH):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [2+((-1)+DATA_WIDTH):0] 
...
(wr_ptr_gray[ADDR_WIDTH] != rd_ptr_gray_sync2[ADDR_WIDTH]);
  ...
  wire 
       read = (~empty & ~output_axis_tvalid_reg) | (~empty & output_axis_tready);
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= rd_ptr_next>>1 ^ (((rd_ptr_next>>1 & (rd_ptr_next | ~rd_ptr_next)) | rd_ptr_next) & ((rd_ptr_next & (~rd_ptr_next>>1 | rd_ptr_next>>1)) & (~rd_ptr_next>>1 | rd_ptr_next>>1)));
          end
