...
module axis_async_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [((-1)+1)+(DATA_WIDTH+1):0] mem[(-1)+(2**ADDR_WIDTH):0];
...
(wr_ptr_gray[ADDR_WIDTH+(0-2):0] == rd_ptr_gray_sync2[ADDR_WIDTH+(0-2):0]);
  ...
  wire 
       read = ((output_axis_tready | ~output_axis_tvalid_reg) & (~empty | output_axis_tready)) & ~empty;
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= ((rd_ptr_next | rd_ptr_next>>1) & ~rd_ptr_next>>1) | (~rd_ptr_next & rd_ptr_next>>1);
          end
