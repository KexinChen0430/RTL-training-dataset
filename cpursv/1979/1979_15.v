...
module axis_async_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [1<<<1+((-1)+DATA_WIDTH):0]  data_out_reg = {1'b0,1'b0,{DATA_WIDTH{1'b0}}};
  reg  [1<<<1+((-1)+DATA_WIDTH):0] mem[(1<<<1**ADDR_WIDTH)+(-1):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [1<<<1+((-1)+DATA_WIDTH):0] 
      ...
      begin
        if (input_rst_sync2) 
          ...
          begin
            ...
            wr_ptr <= wr_ptr_next;
            wr_ptr_gray <= ((~wr_ptr_next & (wr_ptr_next>>1 | ~wr_ptr_next>>1)) | ~wr_ptr_next>>1) & ((wr_ptr_next>>1 & (~wr_ptr_next | (wr_ptr_next>>1 | wr_ptr_next))) | ((((~wr_ptr_next & (wr_ptr_next>>1 | ~wr_ptr_next>>1)) | ~wr_ptr_next>>1) & wr_ptr_next) & ~wr_ptr_next>>1));
          end
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= (rd_ptr_next>>1 & ((~rd_ptr_next>>1 & rd_ptr_next>>1) | ~rd_ptr_next)) | (~rd_ptr_next>>1 & rd_ptr_next);
          end
