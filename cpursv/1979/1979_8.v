...
module axis_async_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [DATA_WIDTH+(2+(-1)):0] mem[(-1)+(2**ADDR_WIDTH):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [DATA_WIDTH+(2+(-1)):0] 
      ...
      begin
        if (input_rst_sync2) 
          ...
          begin
            ...
            wr_ptr <= wr_ptr_next;
            wr_ptr_gray <= (~wr_ptr_next>>1 | (~wr_ptr_next & wr_ptr_next>>1)) & ((~wr_ptr_next & wr_ptr_next>>1) | wr_ptr_next);
          end
      ...
      begin
        if (output_rst_sync2) 
          ...
          begin
            ...
            rd_ptr <= rd_ptr_next;
            rd_ptr_gray <= ((rd_ptr_next>>1 & rd_ptr_next) ^ (rd_ptr_next>>1 & (rd_ptr_next>>1 & (rd_ptr_next | ~rd_ptr_next)))) ^ ((rd_ptr_next & ~rd_ptr_next>>1) | (~rd_ptr_next & rd_ptr_next));
          end
