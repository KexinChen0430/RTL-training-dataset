...
module axis_async_fifo  #(parameter  ADDR_WIDTH = 12, DATA_WIDTH = 8)
  ...
  reg  [(DATA_WIDTH+1)+(1+(0-1)):0]  data_out_reg = {1'b0,1'b0,{DATA_WIDTH{1'b0}}};
  reg  [(DATA_WIDTH+1)+(1+(0-1)):0] mem[(0-1)+(2**ADDR_WIDTH):0];
  reg   output_read = 1'b0;
  reg   output_axis_tvalid_reg = 1'b0;
  wire [(DATA_WIDTH+1)+(1+(0-1)):0] 
...
(wr_ptr_gray[ADDR_WIDTH+(-2):0] == rd_ptr_gray_sync2[ADDR_WIDTH+(-2):0]));
  ...
  wire 
       read = ~empty & ((~output_axis_tvalid_reg & (output_axis_tready | ~empty)) | output_axis_tready);
      ...
      begin
        if (input_rst_sync2) 
          ...
          begin
            ...
            wr_ptr <= wr_ptr_next;
            wr_ptr_gray <= ((((~wr_ptr_next & (~wr_ptr_next>>1 & wr_ptr_next)) | (~wr_ptr_next & wr_ptr_next>>1)) | (wr_ptr_next | ~wr_ptr_next)) & (wr_ptr_next & (wr_ptr_next>>1 | ~wr_ptr_next>>1))) ^ (wr_ptr_next>>1 & (((~wr_ptr_next & (~wr_ptr_next>>1 & wr_ptr_next)) | (~wr_ptr_next & wr_ptr_next>>1)) | (wr_ptr_next | ~wr_ptr_next)));
          end
