...
module counter_f(Clk,Rst,Load_In,Count_Enable,Count_Load,Count_Down,Count_Out,
  ...
  input  Clk;
  input  Rst;
  input  [C_NUM_BITS+(0-1):0] Load_In;
  input  Count_Enable;
  input  Count_Load;
  input  Count_Down;
  output [C_NUM_BITS+(0-1):0] Count_Out;
  wire [C_NUM_BITS+(0-1):0] Count_Out;
  output Carry_Out;
  wire Carry_Out;
  ...
  assign icount_out_x = {1'b0,icount_out[C_NUM_BITS+(0-1):0]};
  ...
  assign Count_Out = icount_out[C_NUM_BITS+(0-1):0];
endmodule
