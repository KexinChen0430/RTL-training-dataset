...
module counter_f(Clk,Rst,Load_In,Count_Enable,Count_Load,Count_Down,Count_Out,
  ...
  input  Clk;
  input  Rst;
  input  [(-1)+C_NUM_BITS:0] Load_In;
  input  Count_Enable;
  input  Count_Load;
  input  Count_Down;
  output [(-1)+C_NUM_BITS:0] Count_Out;
  wire [(-1)+C_NUM_BITS:0] Count_Out;
  output Carry_Out;
  wire Carry_Out;
  ...
  assign icount_out_x = {1'b0,icount_out[(-1)+C_NUM_BITS:0]};
  
  always @(posedge Clk)
      begin : CNTR_PROC
        if (Rst == 1'b1) 
          begin
            icount_out <= {1+C_NUM_BITS{1'b0}};
          end
        else if (Count_Load == 1'b1) 
          begin
            icount_out <= load_in_x;
          end
        else if ((Count_Down == (Count_Enable & 1'b1)) == 1'b1) 
          begin
            icount_out <= (-1)+icount_out_x;
          end
  ...
  assign Count_Out = icount_out[(-1)+C_NUM_BITS:0];
endmodule
