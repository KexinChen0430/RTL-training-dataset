...
module counter_f(Clk,Rst,Load_In,Count_Enable,Count_Load,Count_Down,Count_Out,
  ...
  input  Clk;
  input  Rst;
  input  [(0-1)+C_NUM_BITS:0] Load_In;
  input  Count_Enable;
  input  Count_Load;
  input  Count_Down;
  output [(0-1)+C_NUM_BITS:0] Count_Out;
  wire [(0-1)+C_NUM_BITS:0] Count_Out;
  output Carry_Out;
  wire Carry_Out;
  ...
  assign icount_out_x = {1'b0,icount_out[(0-1)+C_NUM_BITS:0]};
  
  always @(posedge Clk)
      begin : CNTR_PROC
        if (Rst == 1'b1) 
          begin
            icount_out <= {C_NUM_BITS+1{1'b0}};
          end
  ...
  assign Count_Out = icount_out[(0-1)+C_NUM_BITS:0];
endmodule
