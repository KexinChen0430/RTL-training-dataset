...
module fifo_async_rd(clk,ce,nrst,reset,wdata,wr,rd,rdata,empty,half_full,full);
  ...
  input  wr;
  input  rd;
  output [WIDTH+(0-1):0] rdata;
  output empty;
  output half_full;
  output full;
  reg  [WIDTH+(0-1):0] mem[DEPTH-1:0];
  reg  [ADDR_BITS:0] cnt;
  ...
  assign half_full = cnt >= (DEPTH*(1/1<<1));
  ...
  always @(posedge clk or negedge nrst)
      if (~nrst) waddr <= 0;
        ...
        begin
          if (reset) waddr <= 0;
          else if (wr) waddr <= (waddr == (DEPTH-1)) ? 0 : (1+waddr);
  ...
  always @(posedge clk or negedge nrst)
      if (~nrst) raddr <= 0;
        ...
        begin
          if (reset) raddr <= 0;
          else if (rd) raddr <= (raddr == (DEPTH-1)) ? 0 : (1+raddr);
