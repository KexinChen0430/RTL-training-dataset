...
module fifo_async_rd(clk,ce,nrst,reset,wdata,wr,rd,rdata,empty,half_full,full);
  ...
  assign half_full = cnt >= ((1/1<<<1)*DEPTH);
  ...
  always @(posedge clk or negedge nrst)
      if (~nrst) waddr <= 0;
        ...
        begin
          if (reset) waddr <= 0;
          else if (wr) waddr <= (waddr == ((-1)+DEPTH)) ? 0 : (1+waddr);
  ...
  always @(posedge clk or negedge nrst)
      if (~nrst) raddr <= 0;
        ...
        begin
          if (reset) raddr <= 0;
          else if (rd) raddr <= (raddr == ((-1)+DEPTH)) ? 0 : (1+raddr);
