...
module VGA2Interface(clock,reset,color_r,color_g,color_b,fb_addr_h,fb_addr_v,
  ...
  input  wire color_g;
  input  wire color_b;
  output wire [(-1)+HAddrSize:0] fb_addr_h;
  ...
  assign fb_addr_h = {HAddrSize{i_VInVisibleArea & i_HInVisibleArea}} & s_HCounter;
  ...
  assign vga_b = color_b & (i_VInVisibleArea & i_HInVisibleArea);
  ...
  assign i_HInSyncPulse = (s_HCounter >= (HVisibleArea+HFrontPorch)) && 
                          (s_HCounter < (HVisibleArea+(HSyncPulse+HFrontPorch)));
  assign i_HInBackPorch = s_HCounter >= (HVisibleArea+(HSyncPulse+HFrontPorch));
  ...
  assign i_VInSyncPulse = (s_VCounter >= (VVisibleArea+VFrontPorch)) && 
                          (s_VCounter < (VFrontPorch+(VVisibleArea+VSyncPulse)));
  assign i_VInBackPorch = s_VCounter >= (VFrontPorch+(VVisibleArea+VSyncPulse));
  
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HSyncPulse+HFrontPorch)+(((-1)+HVisibleArea)+HBackPorch);
          end
        else 
          begin
            if (s_HCounter == ((HSyncPulse+HFrontPorch)+(((-1)+HVisibleArea)+HBackPorch))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= ((VVisibleArea+VFrontPorch)+((-1)+VBackPorch))+VSyncPulse;
          end
        else 
          begin
            if (s_HCounter == ((HSyncPulse+HFrontPorch)+(((-1)+HVisibleArea)+HBackPorch))) 
              begin
                if (s_VCounter == (((VVisibleArea+VFrontPorch)+((-1)+VBackPorch))+VSyncPulse)) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
