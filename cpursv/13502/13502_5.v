...
module VGA2Interface(clock,reset,color_r,color_g,color_b,fb_addr_h,fb_addr_v,
  ...
  input  wire color_g;
  input  wire color_b;
  output wire [HAddrSize+(0-1):0] fb_addr_h;
  ...
  output wire vga_g;
  output wire vga_b;
  reg  [HAddrSize+(0-1):0] s_HCounter;
...
(s_HCounter < ((HFrontPorch+HVisibleArea)+HSyncPulse)) && (s_HCounter >= (HFrontPorch+HVisibleArea));
...
(s_VCounter < (VFrontPorch+(VSyncPulse+VVisibleArea))) && (s_VCounter >= (VVisibleArea+VFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HVisibleArea+(HBackPorch+HSyncPulse))+((0-1)+HFrontPorch);
          end
        else 
          begin
            if (s_HCounter == ((HVisibleArea+(HBackPorch+HSyncPulse))+((0-1)+HFrontPorch))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (((0-1)+VBackPorch)+(VVisibleArea+VFrontPorch))+VSyncPulse;
          end
        else 
          begin
            if (s_HCounter == ((HVisibleArea+(HBackPorch+HSyncPulse))+((0-1)+HFrontPorch))) 
              begin
                if (s_VCounter == ((((0-1)+VBackPorch)+(VVisibleArea+VFrontPorch))+VSyncPulse)) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
