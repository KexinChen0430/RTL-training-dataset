...
(s_VCounter < (VSyncPulse+(VFrontPorch+VVisibleArea))) && (s_VCounter >= (VFrontPorch+VVisibleArea));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HSyncPulse+(HBackPorch+(-1)))+(HFrontPorch+HVisibleArea);
          end
        else 
          begin
            if (s_HCounter == ((HSyncPulse+(HBackPorch+(-1)))+(HFrontPorch+HVisibleArea))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (VBackPorch+VVisibleArea)+(VFrontPorch+(VSyncPulse+(-1)));
          end
        else 
          begin
            if (s_HCounter == ((HSyncPulse+(HBackPorch+(-1)))+(HFrontPorch+HVisibleArea))) 
              begin
                if (s_VCounter == ((VBackPorch+VVisibleArea)+(VFrontPorch+(VSyncPulse+(-1))))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
