...
module VGA2Interface(clock,reset,color_r,color_g,color_b,fb_addr_h,fb_addr_v,
  ...
  assign i_HInSyncPulse = (s_HCounter >= (HFrontPorch+HVisibleArea)) && 
                          (s_HCounter < ((HFrontPorch+HSyncPulse)+HVisibleArea));
...
(s_VCounter < (VSyncPulse+(VVisibleArea+VFrontPorch))) && (s_VCounter >= (VVisibleArea+VFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= ((HVisibleArea+HBackPorch)+(0-1))+(HFrontPorch+HSyncPulse);
          end
        else 
          begin
            if (s_HCounter == (((HVisibleArea+HBackPorch)+(0-1))+(HFrontPorch+HSyncPulse))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (VFrontPorch+VBackPorch)+(VSyncPulse+((0-1)+VVisibleArea));
          end
        else 
          begin
            if (s_HCounter == (((HVisibleArea+HBackPorch)+(0-1))+(HFrontPorch+HSyncPulse))) 
              begin
                if (s_VCounter == ((VFrontPorch+VBackPorch)+(VSyncPulse+((0-1)+VVisibleArea)))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
