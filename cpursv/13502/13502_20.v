...
module VGA2Interface(clock,reset,color_r,color_g,color_b,fb_addr_h,fb_addr_v,
  ...
  assign i_HInFrontPorch = (s_HCounter >= HVisibleArea) && 
                           (s_HCounter < (HFrontPorch+HVisibleArea));
  assign i_HInSyncPulse = 
(s_HCounter < ((HSyncPulse+HFrontPorch)+HVisibleArea)) && (s_HCounter >= (HFrontPorch+HVisibleArea));
  assign i_HInBackPorch = s_HCounter >= ((HSyncPulse+HFrontPorch)+HVisibleArea);
...
(s_VCounter < (VVisibleArea+(VSyncPulse+VFrontPorch))) && (s_VCounter >= (VVisibleArea+VFrontPorch));
  assign i_VInBackPorch = s_VCounter >= (VVisibleArea+(VSyncPulse+VFrontPorch));
  
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HSyncPulse+(HFrontPorch+HBackPorch))+((-1)+HVisibleArea);
          end
        else 
          begin
            if (s_HCounter == ((HSyncPulse+(HFrontPorch+HBackPorch))+((-1)+HVisibleArea))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (VVisibleArea+(VSyncPulse+VFrontPorch))+((-1)+VBackPorch);
          end
        else 
          begin
            if (s_HCounter == ((HSyncPulse+(HFrontPorch+HBackPorch))+((-1)+HVisibleArea))) 
              begin
                if (s_VCounter == ((VVisibleArea+(VSyncPulse+VFrontPorch))+((-1)+VBackPorch))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
