...
(s_HCounter < (HVisibleArea+(HSyncPulse+HFrontPorch))) && (s_HCounter >= (HVisibleArea+HFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= HVisibleArea+(HFrontPorch+((-1)+(HSyncPulse+HBackPorch)));
          end
        else 
          begin
            if (s_HCounter == (HVisibleArea+(HFrontPorch+((-1)+(HSyncPulse+HBackPorch))))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= ((VFrontPorch+(-1))+VBackPorch)+(VSyncPulse+VVisibleArea);
          end
        else 
          begin
            if (s_HCounter == (HVisibleArea+(HFrontPorch+((-1)+(HSyncPulse+HBackPorch))))) 
              begin
                if (s_VCounter == (((VFrontPorch+(-1))+VBackPorch)+(VSyncPulse+VVisibleArea))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
