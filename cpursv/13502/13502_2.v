...
module VGA2Interface(clock,reset,color_r,color_g,color_b,fb_addr_h,fb_addr_v,
  ...
  output wire [(-1)+VAddrSize:0] fb_addr_v;
...
(s_VCounter < ((VFrontPorch+VSyncPulse)+VVisibleArea)) && (s_VCounter >= (VVisibleArea+VFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HBackPorch+HVisibleArea)+((HSyncPulse+(-1))+HFrontPorch);
          end
        else 
          begin
            if (s_HCounter == ((HBackPorch+HVisibleArea)+((HSyncPulse+(-1))+HFrontPorch))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= VFrontPorch+(VSyncPulse+(VBackPorch+((-1)+VVisibleArea)));
          end
        else 
          begin
            if (s_HCounter == ((HBackPorch+HVisibleArea)+((HSyncPulse+(-1))+HFrontPorch))) 
              begin
                if (s_VCounter == (VFrontPorch+(VSyncPulse+(VBackPorch+((-1)+VVisibleArea))))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
