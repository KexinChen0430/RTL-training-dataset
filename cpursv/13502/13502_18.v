...
(s_HCounter < ((HFrontPorch+HSyncPulse)+HVisibleArea)) && (s_HCounter >= (HVisibleArea+HFrontPorch));
...
(s_VCounter < ((VVisibleArea+VFrontPorch)+VSyncPulse)) && (s_VCounter >= (VVisibleArea+VFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HVisibleArea+HBackPorch)+(((-1)+HSyncPulse)+HFrontPorch);
          end
        else 
          begin
            if (s_HCounter == ((HVisibleArea+HBackPorch)+(((-1)+HSyncPulse)+HFrontPorch))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (VSyncPulse+VBackPorch)+(VVisibleArea+((-1)+VFrontPorch));
          end
        else 
          begin
            if (s_HCounter == ((HVisibleArea+HBackPorch)+(((-1)+HSyncPulse)+HFrontPorch))) 
              begin
                if (s_VCounter == ((VSyncPulse+VBackPorch)+(VVisibleArea+((-1)+VFrontPorch)))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
