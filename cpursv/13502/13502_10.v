...
module VGA2Interface(clock,reset,color_r,color_g,color_b,fb_addr_h,fb_addr_v,
  ...
  assign vga_r = color_r & (i_VInVisibleArea & i_HInVisibleArea);
  assign vga_g = color_g & (i_VInVisibleArea & i_HInVisibleArea);
  ...
  assign i_HInSyncPulse = (s_HCounter >= (HVisibleArea+HFrontPorch)) && 
                          (s_HCounter < ((HVisibleArea+HFrontPorch)+HSyncPulse));
  ...
  assign i_VInSyncPulse = (s_VCounter >= (VFrontPorch+VVisibleArea)) && 
                          (s_VCounter < (VVisibleArea+(VFrontPorch+VSyncPulse)));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= ((0-1)+(HBackPorch+HSyncPulse))+(HVisibleArea+HFrontPorch);
          end
        else 
          begin
            if (s_HCounter == (((0-1)+(HBackPorch+HSyncPulse))+(HVisibleArea+HFrontPorch))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (((VBackPorch+VVisibleArea)+VSyncPulse)+VFrontPorch)-1;
          end
        else 
          begin
            if (s_HCounter == (((0-1)+(HBackPorch+HSyncPulse))+(HVisibleArea+HFrontPorch))) 
              begin
                if (s_VCounter == ((((VBackPorch+VVisibleArea)+VSyncPulse)+VFrontPorch)-1)) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
