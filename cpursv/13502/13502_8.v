...
module VGA2Interface(clock,reset,color_r,color_g,color_b,fb_addr_h,fb_addr_v,
  ...
  assign fb_addr_v = s_VCounter & {VAddrSize{i_HInVisibleArea & i_VInVisibleArea}};
  assign vga_hsync = ~i_HInSyncPulse;
  assign vga_vsync = ~i_VInSyncPulse;
  assign vga_r = color_r & (i_HInVisibleArea & i_VInVisibleArea);
  assign vga_g = color_g & (i_HInVisibleArea & i_VInVisibleArea);
  assign vga_b = i_VInVisibleArea & (i_HInVisibleArea & color_b);
...
(s_HCounter < ((HSyncPulse+HFrontPorch)+HVisibleArea)) && (s_HCounter >= (HFrontPorch+HVisibleArea));
  ...
  assign i_VInSyncPulse = (s_VCounter >= (VFrontPorch+VVisibleArea)) && 
                          (s_VCounter < ((VSyncPulse+VVisibleArea)+VFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= ((0-1)+HFrontPorch)+(HVisibleArea+(HSyncPulse+HBackPorch));
          end
        else 
          begin
            if (s_HCounter == (((0-1)+HFrontPorch)+(HVisibleArea+(HSyncPulse+HBackPorch)))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (VSyncPulse+VVisibleArea)+((VFrontPorch+VBackPorch)+(0-1));
          end
        else 
          begin
            if (s_HCounter == (((0-1)+HFrontPorch)+(HVisibleArea+(HSyncPulse+HBackPorch)))) 
              begin
                if (s_VCounter == ((VSyncPulse+VVisibleArea)+((VFrontPorch+VBackPorch)+(0-1)))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
