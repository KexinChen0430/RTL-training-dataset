...
(s_HCounter < ((HSyncPulse+HVisibleArea)+HFrontPorch)) && (s_HCounter >= (HVisibleArea+HFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HVisibleArea+(-1))+(HFrontPorch+(HSyncPulse+HBackPorch));
          end
        else 
          begin
            if (s_HCounter == ((HVisibleArea+(-1))+(HFrontPorch+(HSyncPulse+HBackPorch)))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= VBackPorch+((VFrontPorch+(-1))+(VSyncPulse+VVisibleArea));
          end
        else 
          begin
            if (s_HCounter == ((HVisibleArea+(-1))+(HFrontPorch+(HSyncPulse+HBackPorch)))) 
              begin
                if (s_VCounter == (VBackPorch+((VFrontPorch+(-1))+(VSyncPulse+VVisibleArea)))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
