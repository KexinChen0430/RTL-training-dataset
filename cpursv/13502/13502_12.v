...
(s_HCounter < ((HFrontPorch+HSyncPulse)+HVisibleArea)) && (s_HCounter >= (HFrontPorch+HVisibleArea));
  assign i_HInBackPorch = s_HCounter >= ((HFrontPorch+HSyncPulse)+HVisibleArea);
  ...
  assign i_VInSyncPulse = (s_VCounter >= (VVisibleArea+VFrontPorch)) && 
                          (s_VCounter < ((VFrontPorch+VSyncPulse)+VVisibleArea));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= HSyncPulse+((HFrontPorch+HBackPorch)+(HVisibleArea+(0-1)));
          end
        else 
          begin
            if (s_HCounter == (HSyncPulse+((HFrontPorch+HBackPorch)+(HVisibleArea+(0-1))))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= (VSyncPulse+((0-1)+VBackPorch))+(VVisibleArea+VFrontPorch);
          end
        else 
          begin
            if (s_HCounter == (HSyncPulse+((HFrontPorch+HBackPorch)+(HVisibleArea+(0-1))))) 
              begin
                if (s_VCounter == ((VSyncPulse+((0-1)+VBackPorch))+(VVisibleArea+VFrontPorch))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
