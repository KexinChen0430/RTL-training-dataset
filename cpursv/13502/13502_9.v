...
(s_HCounter < (HSyncPulse+(HVisibleArea+HFrontPorch))) && (s_HCounter >= (HVisibleArea+HFrontPorch));
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_HCounter <= (HBackPorch+HSyncPulse)+((HVisibleArea+(-1))+HFrontPorch);
          end
        else 
          begin
            if (s_HCounter == ((HBackPorch+HSyncPulse)+((HVisibleArea+(-1))+HFrontPorch))) 
              begin
                s_HCounter <= 0;
              end
            else 
  ...
  always @(posedge clock)
      begin
        if (reset) 
          begin
            s_VCounter <= ((VSyncPulse+VBackPorch)+(VVisibleArea+VFrontPorch))+(-1);
          end
        else 
          begin
            if (s_HCounter == ((HBackPorch+HSyncPulse)+((HVisibleArea+(-1))+HFrontPorch))) 
              begin
                if (s_VCounter == (((VSyncPulse+VBackPorch)+(VVisibleArea+VFrontPorch))+(-1))) 
                  begin
                    s_VCounter <= 0;
                  end
                else 
