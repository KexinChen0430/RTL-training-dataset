...
module marker_tracker(input  clk_cpu,
  ...
  always @(posedge clk_camera)
      begin
        ...
        else if ((last_captured != cam_pixel_addr) && 
                 (cam_write_enable == 1)) 
          begin
            cam_pixel_y <= (1/320)*cam_pixel_addr;
  ...
  always @(posedge clk_camera)
      begin
        ...
        if (new_addr && (go && !cam_vsync)) 
          begin
            if (b_valid && (r_valid && g_valid)) 
              begin
                sum_x <= cam_pixel_x+sum_x;
        ...
        i_marker_x <= (1/sample_count)*sum_x;
        i_marker_y <= (1/sample_count)*sum_y;
      end
endmodule
