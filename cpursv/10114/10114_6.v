...
module avst2axis  #(parameter 
  ...
  (input  wire clk,
   ...
   input  wire [DATA_WIDTH+(0-1):0] avst_data,
   input  wire avst_startofpacket,
   input  wire avst_endofpacket,
   input  wire [(0-1)+EMPTY_WIDTH:0] avst_empty,
   input  wire avst_error,
   output wire [DATA_WIDTH+(0-1):0] axis_tdata,
   output wire [(0-1)+KEEP_WIDTH:0] axis_tkeep,
      ...
      if (BYTE_REVERSE) 
        begin : rev
          for (n = 0; n < KEEP_WIDTH; n = n+1)
              begin
                assign axis_tdata[n*BYTE_WIDTH +: BYTE_WIDTH] = avst_data[(((-n)+KEEP_WIDTH)-1)*BYTE_WIDTH +: BYTE_WIDTH];
              end
        end
      else 
        begin
