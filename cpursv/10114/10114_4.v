...
module avst2axis  #(parameter 
  ...
  (input  wire clk,
   ...
   input  wire [(0-1)+DATA_WIDTH:0] avst_data,
   input  wire avst_startofpacket,
   input  wire avst_endofpacket,
   input  wire [EMPTY_WIDTH+(0-1):0] avst_empty,
   input  wire avst_error,
   output wire [(0-1)+DATA_WIDTH:0] axis_tdata,
      ...
      if (BYTE_REVERSE) 
        begin : rev
          for (n = 0; n < KEEP_WIDTH; n = n+1)
              begin
                assign axis_tdata[BYTE_WIDTH*n +: BYTE_WIDTH] = avst_data[BYTE_WIDTH*(KEEP_WIDTH+((0-1)+(-n))) +: BYTE_WIDTH];
              end
        end
      else 
        begin
