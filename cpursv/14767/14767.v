          ...
          begin : merge_data_outer
            for (i = 0; i < (DATA_WIDTH/8); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(h*PAYLOAD_WIDTH)+(i*8) +: 8] = wr_data_mask_r[((h*DATA_WIDTH)/8)+i] ? rd_merge_data_r[(h*DATA_WIDTH)+(i*8) +: 8] : wr_data_r[(h*PAYLOAD_WIDTH)+(i*8) +: 8];
                end
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[(((j*DQ_WIDTH)+CODE_WIDTH)-k)-1] = ^(merged_data[j*PAYLOAD_WIDTH +: DATA_WIDTH] & h_rows[k*CODE_WIDTH +: DATA_WIDTH]);
