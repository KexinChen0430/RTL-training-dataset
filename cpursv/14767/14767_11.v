...
module ecc_merge_enc  #(parameter 
  ...
  input  [(4*(DATA_WIDTH*(1/8)))+(0-1):0] wr_data_mask;
  input  [(0-1)+(DATA_WIDTH*4):0] rd_merge_data;
  reg  [(4*PAYLOAD_WIDTH)-1:0] wr_data_r;
  reg  [(4*(DATA_WIDTH*(1/8)))+(0-1):0] wr_data_mask_r;
  reg  [(0-1)+(DATA_WIDTH*4):0] rd_merge_data_r;
      ...
      for (h = 0; h < 4; h = h+1)
          begin : merge_data_outer
            ...
            if (PAYLOAD_WIDTH > DATA_WIDTH) 
              assign merged_data[(PAYLOAD_WIDTH*(h+1))+(0-1) -: PAYLOAD_WIDTH+(0-DATA_WIDTH)] = wr_data_r[(PAYLOAD_WIDTH*(h+1))+(0-1) -: PAYLOAD_WIDTH+(0-DATA_WIDTH)];
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[(CODE_WIDTH+((0-1)+(j*DQ_WIDTH)))+(-k)] = ^(h_rows[CODE_WIDTH*k +: DATA_WIDTH] & merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH]);
