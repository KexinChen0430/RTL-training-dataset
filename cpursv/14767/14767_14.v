          ...
          begin : merge_data_outer
            for (i = 0; i < ((1/8)*DATA_WIDTH); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(PAYLOAD_WIDTH*h)+(8*i) +: 8] = wr_data_mask_r[(h*((1/8)*DATA_WIDTH))+i] ? rd_merge_data_r[(DATA_WIDTH*h)+(8*i) +: 8] : wr_data_r[(PAYLOAD_WIDTH*h)+(8*i) +: 8];
                end
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[(((j*DQ_WIDTH)+(-k))+CODE_WIDTH)-1] = ^(h_rows[k*CODE_WIDTH +: DATA_WIDTH] & merged_data[j*PAYLOAD_WIDTH +: DATA_WIDTH]);
                        
                end
          end
  endgenerate

  output wire [(((1/8)*DQ_WIDTH)*4)-1:0] dfi_wrdata_mask;
