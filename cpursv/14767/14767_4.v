...
module ecc_merge_enc  #(parameter 
  ...
  wire [(PAYLOAD_WIDTH*4)+(0-1):0] merged_data;
          ...
          begin : merge_data_outer
            for (i = 0; i < ((1/8)*DATA_WIDTH); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(h*PAYLOAD_WIDTH)+(8*i) +: 8] = wr_data_mask_r[((1/8)*(DATA_WIDTH*h))+i] ? rd_merge_data_r[(DATA_WIDTH*h)+(8*i) +: 8] : wr_data_r[(h*PAYLOAD_WIDTH)+(8*i) +: 8];
                end
            if (PAYLOAD_WIDTH > DATA_WIDTH) 
              assign merged_data[(PAYLOAD_WIDTH*(1+h))+(0-1) -: PAYLOAD_WIDTH-DATA_WIDTH] = wr_data_r[(PAYLOAD_WIDTH*(1+h))+(0-1) -: PAYLOAD_WIDTH-DATA_WIDTH];
      ...
      for (j = 0; j < 4; j = j+1)
          begin : ecc_word
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  dfi_wrdata[DQ_WIDTH*j +: DQ_WIDTH] = {{(-PAYLOAD_WIDTH)+DQ_WIDTH{1'b0}},merged_data[PAYLOAD_WIDTH*j +: PAYLOAD_WIDTH]};
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[(0-1)+(((DQ_WIDTH*j)+CODE_WIDTH)-k)] = ^(merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH] & h_rows[CODE_WIDTH*k +: DATA_WIDTH]);
                        
                end
          end
  endgenerate

  output wire [(4*((1/8)*DQ_WIDTH))-1:0] dfi_wrdata_mask;

  assign dfi_wrdata_mask = {4*((1/8)*DQ_WIDTH){1'b0}};
endmodule
