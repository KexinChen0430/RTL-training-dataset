          ...
          begin : merge_data_outer
            for (i = 0; i < (DATA_WIDTH/8); i = 1+i)
                begin : merge_data_inner
                  assign merged_data[(h*PAYLOAD_WIDTH)+(i*8) +: 8] = wr_data_mask_r[i+((h*DATA_WIDTH)/8)] ? rd_merge_data_r[(i*8)+(h*DATA_WIDTH) +: 8] : wr_data_r[(h*PAYLOAD_WIDTH)+(i*8) +: 8];
                end
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[CODE_WIDTH+(((-k)+(j*DQ_WIDTH))+(-1))] = ^(merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH] & h_rows[k*CODE_WIDTH +: DATA_WIDTH]);
