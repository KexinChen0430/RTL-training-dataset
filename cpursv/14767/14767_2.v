...
module ecc_merge_enc  #(parameter 
  ...
  input  [(-1)+(((1/8)*DATA_WIDTH)*4):0] wr_data_mask;
  input  [(DATA_WIDTH*4)-1:0] rd_merge_data;
  reg  [(PAYLOAD_WIDTH*4)+(-1):0] wr_data_r;
  reg  [(-1)+(((1/8)*DATA_WIDTH)*4):0] wr_data_mask_r;
          ...
          begin : merge_data_outer
            for (i = 0; i < ((1/8)*DATA_WIDTH); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(h*PAYLOAD_WIDTH)+(i*8) +: 8] = wr_data_mask_r[i+(h*((1/8)*DATA_WIDTH))] ? rd_merge_data_r[(h*DATA_WIDTH)+(i*8) +: 8] : wr_data_r[(h*PAYLOAD_WIDTH)+(i*8) +: 8];
                end
            if (PAYLOAD_WIDTH > DATA_WIDTH) 
              assign merged_data[((1+h)*PAYLOAD_WIDTH)-1 -: PAYLOAD_WIDTH-DATA_WIDTH] = wr_data_r[((1+h)*PAYLOAD_WIDTH)-1 -: PAYLOAD_WIDTH-DATA_WIDTH];
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[(j*DQ_WIDTH)+((-1)+(CODE_WIDTH+(-k)))] = ^(h_rows[CODE_WIDTH*k +: DATA_WIDTH] & merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH]);
                        
                end
          end
  endgenerate

  output wire [(-1)+(((1/8)*DQ_WIDTH)*4):0] dfi_wrdata_mask;

  assign dfi_wrdata_mask = {((1/8)*DQ_WIDTH)*4{1'b0}};
endmodule
