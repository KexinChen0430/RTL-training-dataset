          ...
          begin : merge_data_outer
            for (i = 0; i < ((1/8)*DATA_WIDTH); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(PAYLOAD_WIDTH*h)+(8*i) +: 8] = wr_data_mask_r[((1/8)*(DATA_WIDTH*h))+i] ? rd_merge_data_r[(8*i)+(DATA_WIDTH*h) +: 8] : wr_data_r[(PAYLOAD_WIDTH*h)+(8*i) +: 8];
                end
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[((0-k)+(CODE_WIDTH+(j*DQ_WIDTH)))+(-1)] = ^(merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH] & h_rows[CODE_WIDTH*k +: DATA_WIDTH]);
