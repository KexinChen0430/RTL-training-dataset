...
module ecc_merge_enc  #(parameter 
  ...
  input  [(-1)+(4*DATA_WIDTH):0] rd_merge_data;
  reg  [(PAYLOAD_WIDTH*4)+(-1):0] wr_data_r;
  reg  [((4*(1/8))*DATA_WIDTH)+(-1):0] wr_data_mask_r;
  reg  [(-1)+(4*DATA_WIDTH):0] rd_merge_data_r;
          ...
          begin : merge_data_outer
            for (i = 0; i < ((1/8)*DATA_WIDTH); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(PAYLOAD_WIDTH*h)+(8*i) +: 8] = wr_data_mask_r[i+((1/8)*(h*DATA_WIDTH))] ? rd_merge_data_r[(h*DATA_WIDTH)+(8*i) +: 8] : wr_data_r[(PAYLOAD_WIDTH*h)+(8*i) +: 8];
                end
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[(-1)+(((j*DQ_WIDTH)+CODE_WIDTH)+(0-k))] = ^(merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH] & h_rows[CODE_WIDTH*k +: DATA_WIDTH]);
