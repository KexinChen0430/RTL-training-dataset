...
module ecc_merge_enc  #(parameter 
  ...
  input  [((DATA_WIDTH*4)*(1/8))+(-1):0] wr_data_mask;
  input  [(DATA_WIDTH*4)+(-1):0] rd_merge_data;
  reg  [(PAYLOAD_WIDTH*4)+(-1):0] wr_data_r;
  reg  [((DATA_WIDTH*4)*(1/8))+(-1):0] wr_data_mask_r;
          ...
          begin : merge_data_outer
            for (i = 0; i < (DATA_WIDTH/8); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(8*i)+(h*PAYLOAD_WIDTH) +: 8] = wr_data_mask_r[((h*DATA_WIDTH)/8)+i] ? rd_merge_data_r[(h*DATA_WIDTH)+(8*i) +: 8] : wr_data_r[(8*i)+(h*PAYLOAD_WIDTH) +: 8];
                end
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[((-1)+(-k))+(CODE_WIDTH+(DQ_WIDTH*j))] = ^(merged_data[j*PAYLOAD_WIDTH +: DATA_WIDTH] & h_rows[CODE_WIDTH*k +: DATA_WIDTH]);
