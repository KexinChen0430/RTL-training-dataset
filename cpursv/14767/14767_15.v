...
module ecc_merge_enc  #(parameter 
  ...
  input  [(0-1)+((4*(1/8))*DATA_WIDTH):0] wr_data_mask;
  input  [(DATA_WIDTH*4)+(0-1):0] rd_merge_data;
  reg  [(PAYLOAD_WIDTH*4)-1:0] wr_data_r;
  reg  [(0-1)+((4*(1/8))*DATA_WIDTH):0] wr_data_mask_r;
          ...
          begin : merge_data_outer
            for (i = 0; i < (DATA_WIDTH*(1/8)); i = i+1)
                begin : merge_data_inner
                  assign merged_data[(i*8)+(h*PAYLOAD_WIDTH) +: 8] = wr_data_mask_r[i+((DATA_WIDTH*h)*(1/8))] ? rd_merge_data_r[(i*8)+(DATA_WIDTH*h) +: 8] : wr_data_r[(i*8)+(h*PAYLOAD_WIDTH) +: 8];
                end
            if (PAYLOAD_WIDTH > DATA_WIDTH) 
              assign merged_data[(0-1)+(PAYLOAD_WIDTH*(1+h)) -: PAYLOAD_WIDTH-DATA_WIDTH] = wr_data_r[(0-1)+(PAYLOAD_WIDTH*(1+h)) -: PAYLOAD_WIDTH-DATA_WIDTH];
  ...
  output reg [(DQ_WIDTH*4)+(0-1):0] dfi_wrdata;
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  ...
                  for (k = 0; k < ECC_WIDTH; k = k+1)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[((-k)+(0-1))+((DQ_WIDTH*j)+CODE_WIDTH)] = ^(merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH] & h_rows[CODE_WIDTH*k +: DATA_WIDTH]);
                        
                end
          end
  endgenerate

  output wire [(DQ_WIDTH*(4*(1/8)))+(0-1):0] dfi_wrdata_mask;
