      ...
      for (h = 0; h < 4; h = 1+h)
          begin : merge_data_outer
            ...
            if (PAYLOAD_WIDTH > DATA_WIDTH) 
              assign merged_data[(0-1)+(PAYLOAD_WIDTH*(1+h)) -: (0-DATA_WIDTH)+PAYLOAD_WIDTH] = wr_data_r[(0-1)+(PAYLOAD_WIDTH*(1+h)) -: (0-DATA_WIDTH)+PAYLOAD_WIDTH];
      ...
      for (j = 0; j < 4; j = j+1)
          begin : ecc_word
            ...
            always @(h_rows or merged_data or raw_not_ecc_r)
                begin
                  dfi_wrdata[DQ_WIDTH*j +: DQ_WIDTH] = {{DQ_WIDTH-PAYLOAD_WIDTH{1'b0}},merged_data[PAYLOAD_WIDTH*j +: PAYLOAD_WIDTH]};
                  for (k = 0; k < ECC_WIDTH; k = 1+k)
                      if (~raw_not_ecc_r[j]) 
                        dfi_wrdata[((CODE_WIDTH+(DQ_WIDTH*j))+(0-k))+(0-1)] = ^(h_rows[k*CODE_WIDTH +: DATA_WIDTH] & merged_data[PAYLOAD_WIDTH*j +: DATA_WIDTH]);
