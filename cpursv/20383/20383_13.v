...
module buffer_BB_to_stage  #(parameter  N = 8, LOG_N = 3, WIDTH = 32, MWIDTH = 1)
  (input  wire clk,
   ...
   input  wire [(WIDTH+(0-1))+MWIDTH:0] read_data,
   output reg read_delete,
   output wire [LOG_N+(0-1):0] out_addr0,
   output wire [LOG_N+(0-1):0] out_addr1,
   output reg out_nd,
   output reg [WIDTH+(0-1):0] out_data0,
   output reg [WIDTH+(0-1):0] out_data1,
   output reg out_mnd,
   output reg [(0-1)+MWIDTH:0] out_m,
   output wire active,
   output reg error);

  reg  [LOG_N+(0-1):0] addr;

  assign out_addr0 = addr;
  assign out_addr1 = 1+addr;
  reg  read_counter;

  wire [WIDTH+(0-1):0] read_data_s;

  wire [(0-1)+MWIDTH:0] read_data_m;
