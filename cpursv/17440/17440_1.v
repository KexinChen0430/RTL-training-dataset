...
module blockmem2rptr1w(input  wire clk,
                       input  wire reset_n,
                       input  wire [7:0] read_addr0,
  ...
  always @(posedge clk)
      begin : mem_update
        if (wr) mem[write_addr] <= write_data;
  ...
  always @(posedge clk or negedge reset_n)
      begin : reg_mem_update
        if (!reset_n) ptr_reg <= 8'h00;
        else if (ptr_we) ptr_reg <= ptr_new;
