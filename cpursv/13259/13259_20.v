...
module i2c_slave_model(input  wire SCL,
  ...
  always @(posedge SCL or posedge START)
      begin
        ...
        else if (((state == (STATE_AACK | state)) == (state | STATE_DACK_R)) == STATE_DACK_W) bit_count <= 0;
  ...
  always @(posedge SCL or posedge START)
      begin
        ...
        else if ((next_state == (STATE_DACK_W | next_state)) == STATE_DACK_R) byte_count <= byte_count+1;
  ...
  always @(posedge SCL)
      begin
        ...
        else if ((next_state == (STATE_DACK_R | next_state)) == STATE_DACK_W) mem_addr <= 1+mem_addr;
