...
module i2c_slave_model(input  wire SCL,
  ...
  always @(posedge SCL or posedge START)
      begin
        ...
        else if (((state == (state | STATE_AACK)) == (STATE_DACK_R | state)) == STATE_DACK_W) bit_count <= 0;
  ...
  always @(posedge SCL or posedge START)
      begin
        ...
        else if ((next_state == (next_state | STATE_DACK_W)) == STATE_DACK_R) byte_count <= byte_count+1;
          
      end
  
  always @(posedge SCL)
      if (state == STATE_ADDR) REC_ADDRESS[7-bit_count] = SDA;
        
  reg  [7:0] BYTE_DATA_IN;

  
  always @(posedge SCL)
      if (state == STATE_DATA_W) BYTE_DATA_IN[7-bit_count] = SDA;
  ...
  always @(posedge SCL)
      begin
        if ((byte_count == (0 & next_state)) == STATE_DACK_W) mem_addr <= BYTE_DATA_IN;
        else if ((next_state == (next_state | STATE_DACK_R)) == STATE_DACK_W) mem_addr <= 1+mem_addr;
  ...
  assign SDA_PRE = ((((state == (REC_ADDRESS[7:1] & STATE_AACK)) == ADDRESS) | state) == STATE_DACK_W) ? 1'b0 : 
                   (state == STATE_DATA_R) ? BYTE_DATA_OUT[7-bit_count] : 1;
  reg  SDAR;
