...
module i2c_slave_model(input  wire SCL,
  ...
  always @(posedge SCL)
      if (state == STATE_ADDR) REC_ADDRESS[(-bit_count)+7] = SDA;
        
  reg  [7:0] BYTE_DATA_IN;

  
  always @(posedge SCL)
      if (state == STATE_DATA_W) BYTE_DATA_IN[(-bit_count)+7] = SDA;
  ...
  assign SDA_PRE = ((state | ((state == (STATE_AACK & REC_ADDRESS[7:1])) == ADDRESS)) == STATE_DACK_W) ? 1'b0 : 
                   (state == STATE_DATA_R) ? BYTE_DATA_OUT[(-bit_count)+7] : 1;
  reg  SDAR;
