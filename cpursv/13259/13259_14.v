...
module i2c_slave_model(input  wire SCL,
  ...
  always @(posedge SCL or posedge START)
      begin
        ...
        else if ((next_state == (next_state | STATE_DACK_W)) == STATE_DACK_R) byte_count <= 1+byte_count;
          
      end
  
  always @(posedge SCL)
      if (state == STATE_ADDR) REC_ADDRESS[(0-bit_count)+7] = SDA;
        
  reg  [7:0] BYTE_DATA_IN;

  
  always @(posedge SCL)
      if (state == STATE_DATA_W) BYTE_DATA_IN[(0-bit_count)+7] = SDA;
  ...
  always @(posedge SCL)
      begin
        ...
        else if ((next_state == (next_state | STATE_DACK_R)) == STATE_DACK_W) mem_addr <= mem_addr+1;
  ...
  assign SDA_PRE = ((((state == (STATE_AACK & REC_ADDRESS[7:1])) == ADDRESS) | state) == STATE_DACK_W) ? 1'b0 : 
                   (state == STATE_DATA_R) ? BYTE_DATA_OUT[(0-bit_count)+7] : 1;
  reg  SDAR;
