...
module REGISTER_FILE  #(parameter 
       ...
       HIGH              = 1'b1)
  (input  CLK,
   input  [$clog2(REGISTER_DEPTH)+(0-1):0] RS1_ADDRESS,
   input  [$clog2(REGISTER_DEPTH)+(0-1):0] RS2_ADDRESS,
   input  [$clog2(REGISTER_DEPTH)+(0-1):0] RD_ADDRESS,
   input  [(0-1)+REGISTER_WIDTH:0] RD_DATA,
   input  RD_WRITE_EN,
   output [(0-1)+REGISTER_WIDTH:0] RS1_DATA,
   output [(0-1)+REGISTER_WIDTH:0] RS2_DATA);

  reg  [(0-1)+REGISTER_WIDTH:0] register[(0-1)+REGISTER_DEPTH:0];
  ...
  initial  
  begin
    for (i = 0; i < REGISTER_DEPTH; i = 1+i)
        begin
