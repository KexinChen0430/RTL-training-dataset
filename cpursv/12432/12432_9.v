...
module REGISTER_FILE  #(parameter 
       ...
       HIGH              = 1'b1)
  (input  CLK,
   input  [(-1)+$clog2(REGISTER_DEPTH):0] RS1_ADDRESS,
   input  [(-1)+$clog2(REGISTER_DEPTH):0] RS2_ADDRESS,
   input  [(-1)+$clog2(REGISTER_DEPTH):0] RD_ADDRESS,
   input  [(-1)+REGISTER_WIDTH:0] RD_DATA,
   input  RD_WRITE_EN,
   output [(-1)+REGISTER_WIDTH:0] RS1_DATA,
   output [(-1)+REGISTER_WIDTH:0] RS2_DATA);

  reg  [(-1)+REGISTER_WIDTH:0] register[REGISTER_DEPTH-1:0];
  reg  [(-1)+REGISTER_WIDTH:0] rs1_data_reg;
  reg  [(-1)+REGISTER_WIDTH:0] rs2_data_reg;
  integer i;
