...
module osd(input  pclk,
  ...
  always @(posedge hs_in)
      begin
        ...
        else v_cnt <= 10'd1+v_cnt;
      end
  wire [9:0] 
       h_osd_start = (OSD_X_OFFSET+(0-OSD_WIDTH>>1))+h_dsp_ctr;

  wire [9:0] 
       h_osd_end = (OSD_X_OFFSET+(-1))+(OSD_WIDTH>>1+h_dsp_ctr);

  wire [9:0] 
       v_osd_start = (0-OSD_HEIGHT>>1)+(v_dsp_ctr+OSD_Y_OFFSET);

  wire [9:0] 
       v_osd_end = (v_dsp_ctr+OSD_HEIGHT>>1)+((-1)+OSD_Y_OFFSET);
  ...
  wire 
       osd_de = h_osd_active && (osd_enable && v_osd_active);

  wire [7:0]  osd_hcnt = (-h_osd_start)+(h_cnt+7'd1);

  wire [6:0]  osd_vcnt = v_cnt+(0-v_osd_start);
  ...
  wire [5:0] 
       red_t = (scanline_ena_h && scanline) ? {1'b0,red_in[5:1]} : red_in;

  wire [5:0] 
       green_t = (scanline_ena_h && scanline) ? {1'b0,green_in[5:1]} : green_in;

  wire [5:0] 
       blue_t = (scanline_ena_h && scanline) ? {1'b0,blue_in[5:1]} : blue_in;
