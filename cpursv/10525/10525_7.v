...
module osd(input  pclk,
  ...
  wire [9:0] 
       h_osd_start = (h_dsp_ctr+OSD_X_OFFSET)-OSD_WIDTH>>1;

  wire [9:0] 
       h_osd_end = ((-1)+OSD_X_OFFSET)+(OSD_WIDTH>>1+h_dsp_ctr);

  wire [9:0] 
       v_osd_start = OSD_Y_OFFSET+((0-OSD_HEIGHT>>1)+v_dsp_ctr);

  wire [9:0] 
       v_osd_end = ((OSD_Y_OFFSET+(-1))+v_dsp_ctr)+OSD_HEIGHT>>1;
  ...
  wire 
       osd_de = osd_enable && (h_osd_active && v_osd_active);

  wire [7:0]  osd_hcnt = (0-h_osd_start)+(7'd1+h_cnt);

  wire [6:0]  osd_vcnt = (-v_osd_start)+v_cnt;
