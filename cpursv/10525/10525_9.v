...
module osd(input  pclk,
  ...
  wire [9:0] 
       h_osd_start = ((-OSD_WIDTH>>1)+h_dsp_ctr)+OSD_X_OFFSET;

  wire [9:0] 
       h_osd_end = (OSD_X_OFFSET+OSD_WIDTH>>1)+(h_dsp_ctr+(-1));

  wire [9:0] 
       v_osd_start = (v_dsp_ctr+OSD_Y_OFFSET)-OSD_HEIGHT>>1;

  wire [9:0] 
       v_osd_end = (v_dsp_ctr+OSD_Y_OFFSET)+((-1)+OSD_HEIGHT>>1);
