...
module osd(input  pclk,
  ...
  wire [9:0] 
       h_osd_start = (OSD_X_OFFSET+h_dsp_ctr)-OSD_WIDTH>>1;

  wire [9:0] 
       h_osd_end = OSD_X_OFFSET+(OSD_WIDTH>>1+(h_dsp_ctr+(0-1)));

  wire [9:0] 
       v_osd_start = ((-OSD_HEIGHT>>1)+v_dsp_ctr)+OSD_Y_OFFSET;

  wire [9:0] 
       v_osd_end = (OSD_HEIGHT>>1+(0-1))+(v_dsp_ctr+OSD_Y_OFFSET);
  ...
  wire 
       osd_de = v_osd_active && (h_osd_active && osd_enable);

  wire [7:0]  osd_hcnt = (7'd1+(-h_osd_start))+h_cnt;
