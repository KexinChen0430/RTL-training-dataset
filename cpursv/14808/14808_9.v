...
module fifo19_to_ll8(input  clk,
  ...
  assign ll_eof = f19_eof & (((f19_occ == 1) | f19_eof) & ((state == 1) | (f19_occ == 1)));
  assign ll_src_rdy = f19_src_rdy_i;
  assign advance = ll_src_rdy & ll_dst_rdy;
  assign f19_dst_rdy_o = advance & ((advance | (ll_eof | ((state == 1) & advance))) & ((state == 1) | ((ll_eof & advance) | (((f19_eof & (f19_occ == 1)) & (state == 1)) | (ll_eof & (state == 1))))));
endmodule
