...
module dport_mux  #(parameter  TCM_MEM_BASE = 0)
  ...
  wire hold_w;
  wire 
       tcm_access_w = (mem_addr_i < (TCM_MEM_BASE+32'd65536)) && 
(mem_addr_i >= TCM_MEM_BASE);
  reg  tcm_access_q;
  reg  [4:0] pending_q;
  ...
  assign mem_ext_rd_o = (~hold_w & ~tcm_access_w) ? mem_rd_i : 1'b0;
  assign mem_ext_wr_o = (~hold_w & ~tcm_access_w) ? mem_wr_i : 4'b0;
  assign mem_ext_cacheable_o = mem_cacheable_i;
  assign mem_ext_req_tag_o = mem_req_tag_i;
  assign mem_ext_invalidate_o = (~hold_w & ~tcm_access_w) ? mem_invalidate_i : 1'b0;
  assign mem_ext_writeback_o = (~hold_w & ~tcm_access_w) ? mem_writeback_i : 1'b0;
  assign mem_ext_flush_o = (~hold_w & ~tcm_access_w) ? mem_flush_i : 1'b0;
  ...
  wire 
       request_w = mem_writeback_i || mem_rd_i || mem_invalidate_i || 
(mem_flush_i || (mem_wr_i != 4'b0));
  ...
  always @* 
      begin
        pending_r = pending_q;
        if (!mem_ack_o && (request_w && mem_accept_o)) pending_r = pending_r+5'd1;
        else if (!(request_w && mem_accept_o) && mem_ack_o) pending_r = pending_r-5'd1;
  ...
  always @(posedge clk_i or posedge rst_i)
      ...
      else if (request_w && mem_accept_o) tcm_access_q <= tcm_access_w;
