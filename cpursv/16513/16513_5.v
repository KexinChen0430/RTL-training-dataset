...
(mem_addr_i >= TCM_MEM_BASE);
  ...
  wire 
       request_w = (mem_wr_i != 4'b0) || 
(mem_invalidate_i || mem_flush_i) || mem_rd_i || mem_writeback_i;
  ...
  always @* 
      begin
        pending_r = pending_q;
        if (mem_accept_o && (!mem_ack_o && request_w)) pending_r = 5'd1+pending_r;
        else if (mem_ack_o && !(request_w && mem_accept_o)) pending_r = pending_r-5'd1;
