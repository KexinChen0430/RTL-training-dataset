...
(mem_addr_i < (32'd65536+TCM_MEM_BASE));
  ...
  wire 
       request_w = mem_flush_i || mem_writeback_i || 
(mem_invalidate_i || (mem_rd_i || (mem_wr_i != 4'b0)));
  ...
  always @* 
      begin
        pending_r = pending_q;
        if (!mem_ack_o && request_w && mem_accept_o) pending_r = 5'd1+pending_r;
        else if (mem_ack_o && !(mem_accept_o && request_w)) pending_r = pending_r-5'd1;
