...
(mem_addr_i < (TCM_MEM_BASE+32'd65536));
  ...
  wire 
       request_w = mem_writeback_i || 
(mem_invalidate_i || 
(mem_flush_i || (mem_rd_i || (mem_wr_i != 4'b0))));
  ...
  always @* 
      begin
        pending_r = pending_q;
        if (mem_accept_o && request_w && !mem_ack_o) pending_r = pending_r+5'd1;
        else if (mem_ack_o && !(mem_accept_o && request_w)) pending_r = (0-5'd1)+pending_r;
