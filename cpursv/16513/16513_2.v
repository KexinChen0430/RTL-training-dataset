...
(mem_rd_i || (mem_flush_i || mem_writeback_i) || 
mem_invalidate_i);
  ...
  always @* 
      begin
        pending_r = pending_q;
        if (request_w && mem_accept_o && !mem_ack_o) pending_r = pending_r+5'd1;
        else if (mem_ack_o && !(request_w && mem_accept_o)) pending_r = (0-5'd1)+pending_r;
