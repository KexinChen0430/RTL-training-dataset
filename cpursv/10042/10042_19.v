...
module branch_control  #(parameter 
  ...
  (input  jmp_inst_in,
   ...
   input  [(-1)+DATA_WIDTH:0] reg_a_data_in,
   input  [(-1)+DATA_WIDTH:0] reg_b_data_in,
   input  [PC_OFFSET_WIDTH+(-1):0] pc_offset_in,
   output select_new_pc_out,
   output [PC_WIDTH-1:0] pc_out);

  wire [(-1)+DATA_WIDTH:0] jmp_val;
  wire [(-1)+DATA_WIDTH:0] branch_val;
  wire [PC_WIDTH-1:0] pc_jump;

  assign pc_jump = {pc_offset_in,{1+1{1'b0}}};
  assign select_new_pc_out = ((branch_result_in | jmp_inst_in) & ((branch_inst_in | branch_result_in) & (jmp_inst_in | ((jmp_inst_in & branch_result_in) | branch_inst_in)))) | jmp_inst_in;
