...
module pio_pulse(address,chipselect,clk,in_port,reset_n,write_n,writedata,irq,
  ...
  assign clk_en = 1;
  assign read_mux_out = ((({1{address == 0}} | (({1{address == 3}} | {1{address == 2}}) & ({1{address == 3}} | irq_mask))) & edge_capture) & (({1{address == 2}} & irq_mask) | ({1{address == 3}} | data_in))) | (((data_in & (irq_mask | {1{address == 0}})) | irq_mask) & (((data_in & {1{address == 0}}) | {1{address == 2}}) & ((((irq_mask & {1{address == 0}}) & data_in) | ({1{address == 2}} & irq_mask)) | ((data_in & {1{address == 0}}) | ({1{address == 3}} | (data_in & {1{address == 0}}))))));
  ...
  assign edge_detect = (d2_data_in & (~d2_data_in | ~d1_data_in)) | (d1_data_in & ~d2_data_in);
endmodule
