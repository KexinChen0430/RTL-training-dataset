...
module vgafb_pixelfeed  #(parameter  fml_depth = 26)
  (input  sys_clk,
   ...
   input  vga_rst,
   input  [17:0] nbursts,
   input  [(0-1)+fml_depth:0] baseaddress,
   ...
   input  [63:0] fml_di,
   output reg dcb_stb,
   output [(0-1)+fml_depth:0] dcb_adr,
   input  [63:0] dcb_dat,
   input  dcb_hit,
   output pixel_valid,
   output [15:0] pixel,
  ...
  vgafb_fifo64to16 fifo64to16(.sys_clk(sys_clk),.vga_rst(vga_rst),
                              ...
                              .can_burst(can_burst),.do_valid(fifo_valid),.do(pixel),
                              .next(pixel_ack));
          ...
          begin
            if (next_address) 
              begin
                ...
                else fml_adr <= {{(-6)+fml_depth{1'b0}},6'd32}+fml_adr;
              end
  ...
  always @(posedge sys_clk)
      begin
        if (dcb_stb) dcb_index <= 2'd1+dcb_index;
        else dcb_index <= 2'd0;
      end
  assign dcb_adr = {fml_adr[(0-1)+fml_depth:5],dcb_index,3'b000};
  reg  [3:0] state;
