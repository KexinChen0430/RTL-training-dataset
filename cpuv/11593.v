module vga_control (
	...
	always @(posedge clock) begin
		if ((hcount < (hres + hfront)) || (hcount > (hres + hfront + hsync))) vga_hsync <= 1'b1;
		else vga_hsync <= 1'b0;
		if ((vcount < (vres + vfront)) || (vcount > (vres + vfront + vsync))) vga_vsync <= 1'b0;
		else vga_vsync <= 1'b1;
		...
		if ((hcount <= hres) && (vcount <= vres)) begin
