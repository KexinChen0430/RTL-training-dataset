...
`ifdef SIMULATION
initial bauddiv_counter_ff = 11'd567; 
`endif
...
`ifdef SIMULATION
initial baudrate_counter_ff = 4'd14; 
`endif
always @(posedge clk) begin
    if (trig_baud16)
...
`ifdef SIMULATION
initial rx_counter_ff = 4'd3; 
initial rx_counter_ff = 4'd10; 
`endif
...
`ifdef SIMULATION
initial rx_frame_counter_ff = 4'd12; 
`endif
always @(posedge clk) begin
    if (trig_next_uartbit) begin
...
`ifdef SIMULATION
initial tx_frame_counter_ff = 4'd13; 
`endif
always @(posedge clk) begin
    if (ack_i) begin
