...
always @(*) begin
    ...
    dc_ctrl_out = 2'b00;
    case (dc_rep_state)
        dc_rep_upload_idle: begin
            if (v_dc_flits_rep) begin
                en_flits_in = 1'b1; 
                next = 1'b1; 
            end
        end
        dc_rep_upload_busy: begin
            if (rep_fifo_rdy) begin
                if (sel_cnt == flits_max_reg) begin
                    fsm_rst = 1'b1; 
                    dc_ctrl_out = 2'b11; 
                end else if (sel_cnt == 3'b000) begin
                    dc_ctrl_out = 2'b01; 
                end else begin
                    dc_ctrl_out = 2'b10; 
                end
...
always @(posedge clk) begin
    if (rst || fsm_rst) begin
        dc_rep_state <= dc_rep_upload_idle; 
    end else if (next) begin
        dc_rep_state <= dc_rep_upload_busy; 
    end
end
always @(posedge clk) begin
    if (rst || fsm_rst) begin
        dc_rep_flits <= 176'h0000; 
    end else if (en_flits_in) begin
...
always @(posedge clk) begin
    ...
    end else if (en_flits_max) begin
