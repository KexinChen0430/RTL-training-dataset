module SendChars(
    ...
    always @ (posedge Clock)
        if (Reset == 1) begin
            ...
            Transmitting <= 1'b0;
        end else if (Start == 1'b1 && Transmitting == 1'b0) begin
            Transmitting <= 1'b1;
            write_to_uart <= 1;
        end else case ({tx_full, Transmitting, uartClock, specialCharacter})
            5'b01100: begin
                write_to_uart <= 1;
                if (RAMAddress + 2 > NumberOfChars) begin
                    RAMAddress <= newline;
                    specialCharacter <= 2'd1;
                end else
