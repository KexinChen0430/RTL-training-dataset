...
always @(posedge clk_i) begin
    if (rst_i == 1'h1) begin
        mem_w_enb_o <= 1'h0;
        mem_w_cnt_o <= `ONE;
    end else if (gbl_stl_i == 1'h1 || mem_stl_i == 1'h1) begin
        mem_w_enb_o <= 1'h0;
    end else if (wait_to_fill_pipe_i == 1'h1) begin
        mem_w_enb_o <= 1'h0;
    end else begin
...
always @(posedge clk_i) begin
    if (rst_i == 1'h1) begin
        ...
        x_reg <= 8'h0;
        y_reg <= 8'h0;
    end else if (gbl_stl_i == 1'h1 || mem_stl_i == 1'h1 || wait_to_fill_pipe_i == 1'h1) begin
    end else begin
