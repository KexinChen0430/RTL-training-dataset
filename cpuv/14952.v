module gpio_rx
	...
	begin
		if(reset)
			gpio_rx_state_nxt = GPIO_RX_IDLE; 
		else
		begin
			case(gpio_rx_state) 
				GPIO_RX_IDLE:
				begin
					if(gpio_wr_req_reg) 
						gpio_rx_state_nxt = GPIO_RX_FETCH; 
					else
						gpio_rx_state_nxt = GPIO_RX_IDLE; 
				end
				GPIO_RX_CHECH: 
					gpio_rx_state_nxt = GPIO_RX_CHECK_DONE; 
				GPIO_RX_CHECK_DONE:
				begin
					if(!gpio_rd_ack_reg) 
						gpio_rx_state_nxt = GPIO_RX_DONE; 
					else
						gpio_rx_state_nxt = GPIO_RX_CHECK_DONE; 
				end
				GPIO_RX_DONE:
					...
					gpio_rx_state_nxt = GPIO_RX_IDLE; 
				default:
					gpio_rx_state_nxt <= gpio_rx_state; 
			endcase
		end
	end
	always@(posedge clk)
	...
	always@(posedge clk)
	begin
		if(gpio_rx_state == GPIO_RX_CHECK_DONE)
			gpio_wr_ack = 1'b1; 
		else if(gpio_rx_state == GPIO_RX_IDLE)
			gpio_wr_ack = 1'b0; 
		else
