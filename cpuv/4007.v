module hardcopyiii_round_block(
    datain,         
    round,          
    datain_width,   
    dataout         
...
always @(datain or round) begin
    if (round == 1'b0) begin
        ...
        if (datain_width > round_width) begin
            ...
            if (datain[datain_width - round_width - 1] == 1'b0) begin
                dataout_tmp = result_tmp;
            end else if ((datain[datain_width - round_width - 1] == 1'b1) && (sign == 1'b1)) begin
                dataout_tmp = result_tmp + 1'b1;
            end else begin
                if (round_mode == "nearest_even") begin
                    if (result_tmp % 2) begin
                        dataout_tmp = result_tmp + 1'b1;
                    end else begin
                        dataout_tmp = result_tmp;
                    end
