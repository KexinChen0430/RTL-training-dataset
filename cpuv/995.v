module encryption_module #(parameter ADDR_WIDTH = 8, DATA_WIDTH = 8) (
  input clk, 
  input rst, 
  input ena, 
  ...
  reg start;
  reg done;
  always @(posedge clk) begin : Main_Module
    case (rst)
      1'b0: begin
        if (ena == 1'b1 && start == 1'b0) begin
          ...
          mem_wr_ena <= 1'b0;
        end else if (ena == 1'b1) begin : Encryption
          begin : Stage_FetchData
            if (data_in !== {DATA_WIDTH{1'bx}}) begin
              mem[0] <= {memIn_addr - 1, data_in};
              memIn_addr <= memIn_addr + 1;
            end else begin
              done <= 1'b1;
              mem[0] <= {MEM_WIDTH{1'b0}};
            end
          end
          begin : Stage0_Stage3_Stage7_AddRoundKey
            mem[1] <= |mem[0] == 1'b0 ? mem[0] : {mem[0][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], mem[0][DATA_WIDTH-1:0] ^ key};
            mem[4] <= |mem[3] == 1'b0 ? mem[3] : {mem[3][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], mem[3][DATA_WIDTH-1:0] ^ key};
            mem[8] <= |mem[7] == 1'b0 ? mem[7] : {mem[7][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], mem[7][DATA_WIDTH-1:0] ^ key};
          end
          begin : Stage2_Stage6_SubBytes
            mem[3] <= |mem[2] == 1'b0 ? mem[2] : {mem[2][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], ~mem[2][DATA_WIDTH-1:0] + 1'b1};
            mem[7] <= |mem[6] == 1'b0 ? mem[6] : {mem[6][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], ~mem[6][DATA_WIDTH-1:0] + 1'b1};
          end
          begin : Stage1_Stage5_ShiftRows
          end
          begin : Stage4_MixColumns
          end
