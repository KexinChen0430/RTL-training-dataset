...
always @(posedge wb_clk)
  if (wb_rst)
    ...
    if (cellram_mst_sel == 2'b00) begin      
      if (wb_m0_vcache_cyc_i & wb_m0_vcache_stb_i) 
        cellram_mst_sel[1] <= 1;             
      else if (wb_m1_cpu_cyc_i & wb_m1_cpu_stb_i) 
        cellram_mst_sel[0] <= 1;             
    end
    else begin
      if (wb_s0_cellram_wb_ack_i)            
        cellram_mst_sel <= 0;                
    end
  end
...
assign wb_s0_cellram_wb_stb_o = (cellram_mst_sel[0] ? wb_m1_cpu_stb_i : cellram_mst_sel[1] ? wb_m0_vcache_stb_i : 0) & !(|cellram_rst_counter);
assign wb_s0_cellram_wb_cyc_o = cellram_mst_sel[0] ? wb_m1_cpu_cyc_i : cellram_mst_sel[1] ? wb_m0_vcache_cyc_i : 0;
