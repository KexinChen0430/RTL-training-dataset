module ir_receive(
    clk, rst, sda, receive_status, received_data
);
    input clk; 
    input rst; 
    input sda; 
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            if (start_bits == 3'b111 && data_start == 0 && time_cnt < 'd44500) begin
                time_cnt <= time_cnt + 1'b1; 
            end else if (start_bits == 3'b111 && data_start == 0 && time_cnt == 'd44500) begin
                time_cnt <= 'h0; 
                data_start <= 'h1; 
            end else begin
                if (start_bits == 3'b111 && time_cnt < 'd89000 && cyc_cnt < 'd11 && data_start == 1) begin
                    time_cnt <= time_cnt + 1'b1; 
                end else if (start_bits == 3'b111 && time_cnt == 'd89000 && cyc_cnt < 'd11 && data_start == 1) begin
                    time_cnt <= 'h0; 
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            end else if (rising_edge && time_cnt > 'd30000 && time_cnt < 'd60000 && cyc_cnt < 'd11) begin
