      ...
      1'b0: begin
        if (ena == 1'b1 && start == 1'b0) begin
          ...
          begin : Stage0_Stage4_Stage7_AddRoundKey
            ...
            mem[5] <= |mem[4] == 1'b0 ? mem[4] : {mem[4][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], mem[4][DATA_WIDTH-1:0] ^ key};
            mem[8] <= |mem[7] == 1'b0 ? mem[7] : {mem[7][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], mem[7][DATA_WIDTH-1:0] ^ key};
          end
          begin : Stage1_Stage5_SubBytes
            mem[2] <= |mem[1] == 1'b0 ? mem[1] : {mem[1][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], ~mem[1][DATA_WIDTH-1:0] + 1'b1};
            mem[6] <= |mem[5] == 1'b0 ? mem[5] : {mem[5][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], ~mem[5][DATA_WIDTH-1:0] + 1'b1};
          end
          begin : Stage2_Stage6_ShiftRows
            mem[3] <= |mem[2] == 1'b0 ? mem[2] : {mem[2][ADDR_WIDTH+DATA_WIDTH-1:DATA_WIDTH], mem[2][DATA_WIDTH-1:shift], mem[2][shift-1:0]};
