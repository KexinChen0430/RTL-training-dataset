...
input reset, clock, deserialize_complete, serialize_complete, emmit_command, big_response;
output reg enable_serializer, enable_deserializer, reset_serializer, response_ready, command_timeout, frame_deserializer;
input [135:0] sd_in; 
output [47:0] sd_out; 
...
always @(posedge clock)
    case(state)
        ...
        WAIT_RESPONSE: begin
            ...
            if(timeout_counter == 63) begin
                command_timeout <= 1; 
                state <= IDLE; 
            end else if(deserialize_complete) begin
