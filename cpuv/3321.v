...
always @(posedge clk) begin
	...
	end else if (read & (address == 2'h0)) begin
		readdata <= buffer_start_address;
	end else if (read & (address == 2'h1)) begin
		readdata <= back_buf_start_address;
	end else if (read & (address == 2'h2)) begin
...
always @(posedge clk) begin
	...
	end else if (buffer_swap & swap_addresses_enable) begin
...
always @(posedge clk) begin
	if (reset) begin
		buffer_swap <= 1'b0;
	end else if (write & (address == 2'h0)) begin
		buffer_swap <= 1'b1;
	end else if (swap_addresses_enable) begin
		buffer_swap <= 1'b0;
	end
end
always @(posedge clk) begin
	...
	end else if (write & (address == 2'h3) & byteenable[0]) begin
